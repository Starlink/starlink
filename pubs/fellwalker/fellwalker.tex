...
just visible under cloud. Copyright: Nick Thorne,
www.lakedistrict.gov.uk/learning/freephotos\# }
\label{fig:wasdale}
\end{figure}
...
same peak value of 30 (arbitrary units) and the same FWHM of 5 pixels.
Gaussian noise was then added, with a different noise level for each of
the eight successive set of tests. The eight noise levels used were
...
DeltaT parameter (the gap between contour levels) was varied from eight
...
to resolve them. For this reason, we expect too see a tail of high-valued
...
issues, the second one can be taken into account by thresholding an
artificial clump at the two sigma threshold before finding its data sum.
So at each noise level the sum of the data values in an artificial clump
is found, excluding pixel below the two sigma threshold. This value is
then used as the expected value for the total data sum in the clumps
measured at the same noise level. To simplify this comparison, the ratio
of the measured clump data sum to the expected clump data sum is used
below as a measure of the performance of each algorithm. This is referred
to as the ``gain'' in the following figures.
...
sometimes cause clumps to be split into two or more discontiguous parts.
...
pixels that are all higer than the threshold level.} as the local
