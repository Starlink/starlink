...
have only minimal dips between them\footnote{In common with other
parameters, this minimum dip parameter is specified as a multiple of the
noise level in the data.}. The raw clumps identified by the above
...
identify pixels which have a value above a user-specified threshold
...
description \citep{2007STCS,2010ASPC..434..213B} of the spatial coverage of
each clump.
...
\[ size = \sqrt{ \frac{a}{b} - \left( \frac{c}{d}\right)^2 } \]


For a clump with a Gaussian profile, this
``size'' value is equal to the standard deviation of the Gaussian.

These sizes can then optionally be reduced to take account of the
smoothing introduced by the instrumental beam\footnote{The FWHM of the
instrumental smoothing on each axis must be specified by the user.}. This
correction also increases the peak values stored within the table.
