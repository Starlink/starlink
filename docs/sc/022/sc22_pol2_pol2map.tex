...
values for both \param{OBSWEIGHT} and \param{SKYLOOP}.  Using
\task{skyloop} to produce the co-add usually requires more time and
disk space. Therefore it is usually advisable to restrict its usage within
\task{pol2map} to the external-masking phase that produces the final
required maps - that is, Steps~2 and 3 as described above. The
\param{OBSWEIGHT} parameter can generally be used on all steps as it adds
little to the time taken to run \task{pol2map}.
