...
The following description assumes that the \texttt{makemap} command is
being used to create maps. Section \ref{sec:skyloop} explains the effects
of using the \texttt{skyloop} command in place of \texttt{makemap}.
...
\makemap\ command creates a single map from the time-series data
for a single observation. If multiple observations are processed together,
multiple invocations of \makemap\ are used and the resulting maps are
co-added before moving on to the next step, as indicated in
Figure~\ref{fig:pol2drflow}. The \skyloop\ command, on the other hand,
will process multiple observations in a single invocation, creating a combined
map from all observations. Thus, when using \skyloop\ ,
Figure~\ref{fig:pol2drflow} would be effectively changed such that each occurrence
of ``three invocations of makemaps feeding one co-add" would be replaced
by a single invocation of ``\skyloop\ ". See section~\ref{sec:tailoredDR}
for more information about the effects of using \skyloop\ .
...
removed, leaving just the astronomical signal. You may specify the number of
components to remove, via a \task{makemap} configuration
...
Two options are provided by \task{pol2map} that may be useful in reducing
...
produce maps that very dissimilar to maps made from the other observations.
...
sometimes seriously under-estimate the dispersion between observations.
For instance, observations that are clearly bad (out of focus for
instance) can have relatively low pixel variance values, and thus get
included with high weight in the coadd.
...
\task{pol2map}  will cause maps to be made using the
...
supplied observations, using a method that attempt to minimise the
intrinsic instabilities of the map-making algorithm within the AST mask.
Note, convergence can require a significantly greater number of iterations
when using \task{skyloop} than when using \task{makemap}. Also,
...
structures, and prevents them growing in amplitude on successive
...
deviation maps are shown with a single scaling (different to the scaling
