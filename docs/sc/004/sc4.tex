...
Although UNIX purists recommend the Bourne shell for scripts and the
...
alias}}{sc4_gl_alias}\latexonly{ (see below)}, you must make your
...
unalias} command.  This accepts {\tt *?[~]} wildcards to \htmlref{{\sf
match}}{sc4_gl_match} the defined aliases.
...
accepts {\tt *?[~]} wildcards to \htmlref{{\sf match}}{sc4_gl_match} the names of
the shell variables.
...
\latexonly{(see SUN/95's chapter called ``NDF Sections'')}
...
text files}{sc4_se_reading_files}.}
...
database}{sun95}{se_agitate}, you may also need to redefine \$AGI\_USER
...
may use the \xref{{\bf parget}}{sun95}{PARGET} command of \KAPPAref\ to
write to \htmlref{{\sf standard output}}{sc4_gl_std_out} the value or
...
\xref{{\bf calc}}{sun95}{CALC} command.  Here we shall just deal with the latter.
...
{\em Function} & 
\begin{tabular}{c}Number of \\ arguments \end{tabular} & Description \\
\hline
\\
...
\htmlref{expression}{sc4_sc_csharith}\latexonly{ (see
...
\xref{{\bf stats}}{sun95}{STATS} command from \KAPPAref\@.  {\tt file}
...
\xref{{\bf histogram}}{sun95}{HISTOGRAM} command from \KAPPAref\@.  It
...
would report the name of each \NDFref{{\sf NDF}} and its standard deviation.
...
{\bf echo} writes to \htmlref{{\sf standard output}}{sc4_gl_std_out}, so
you can write text including values of shell variables to the screen
or redirect it to a file.  Thus the output produced by 
\xref{{\bf stats}}{sun95}{STATS} is \htmlref{{\sf piped}}{sc4_gl_pipe}
(the {\tt |} is the \htmlref{{\sf pipe}}{sc4_gl_pipe}) into the UNIX
...
\latexonly{Thus far the examples have not created a new file.} When you
...
\end{verbatim}
\normalsize
This uses \xref{{\bf block}}{sun95}{BLOCK} from \KAPPAref\ to perform
...
\htmlref{{\sf pipes}}{sc4_gl_pipe} the input name into the {\bf sed} editor which performs the
substitution. 
...
\newpage
\subsection{\xlabel{sc4_se_reading_files}Reading from text files
...
modifier}{sc4_se_filename_ext} {\tt :\$}\latexonly{ (see
...
the equivalent of the earlier {\bf while} loop \latexonly{(see
...
ARD}}{sc4_gl_ard} ellipse shape using the variables, and pipe it into a
...
There is a \htmlref{related example}{sc4_se_long1} \latexonly{in
...
\htmlref{Earlier}{sc4_se_running} \latexonly{in
...
being \xref{masked using an ARD region}{sun95}{se_ardwork}\latexonly{ (see
...
\latexonly{in Section~\ref{sc4_se_unix_options}}.
...
\item {\bf\label{sc4_gl_she_var}Shell variable}\\
      ...
      in the \htmlref{shell}{sc4_gl_she}.
