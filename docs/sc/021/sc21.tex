...
contains the Dynamic Iterative Map-Maker that will process raw
...
directory defined by the environment variable \param{ORAC\_DATA\_OUT}. A
...
Performing nneach step by hand allows more fine-tuned control of certain
...
\textbf{COM}\\*
The \texttt{COM} model removes the
...
\textbf{EXT}\\*
\texttt{EXT} applies the extinction
...
\textbf{FLT}\\*
The \texttt{FLT} model acts on the Fourier transform
...
\textbf{AST}\\*
The \texttt{AST} model is generated in
conjunction with a science map. Hence, the position of \texttt{AST} in the
...
\textbf{NOI}\\*
\texttt{NOI} should come last in the model
...
\centering\texttt{FLT}
\end{minipage}
\begin{minipage}[t]{0.88\linewidth}The \texttt{FLT} model acts on the Fourier transform
of the bolometer data. High-pass (only allows \textit{higher} frequencies to
pass) and low-pass (only allows \textit{lower} frequencies to pass) filters
can be specified. These cut-off frequencies can either be be specified by
you directly or as an angular spatial scale (which is subsequently
...
\centering\texttt{AST}
\end{minipage}
\begin{minipage}[t]{0.88\linewidth}The \texttt{AST} model is generated in
conjunction with a science map. Hence, the position of \texttt{AST} in the
model order indicates at what stage the astronomical
image should be estimated. When the \texttt{AST} model is called, the first
step is regridding the raw time-series data, (using nearest-neighbour sampling),
...
the \texttt{numiter} parameter. This is shown below for \texttt{dimmconfig.lis}.
\vspace{-0.1cm}
\begin{quote}
\begin{verbatim}
numiter = -5
...
\end{latexonly} \texttt{s8a20120720\_00030\_0003\_con\_flt.sdf},
\texttt{s8a20120720\_00030\_0003\_con\_res.sdf})\footnote{The filename
shows sub-scan 3 of Observation 30 since this is the first science
...
\smurf\ and can be found in \texttt{\$STARLINK\_DIR/share/smurf/}.
...
the parameters in \texttt{dimmconfig\_bright\_compact.lis} override
any identical ones in the hidden file \texttt{dimmconfig\_bright.lis} which in turn
...
This configuration is for reducing maps that do not fall into the other two
categories. The emission is usually extended to some degree and contains some
bright regions. Here
...
a series of results in a log file called \texttt{log.checkrms}. The
...
\begin{latexonly}
\myfig{sc21_noihist}{[b!]}{height=7.5cm}{fig:noihisto}{
  The error component viewed with \textsc{Kappa} command \task{histogram}.}{
  The error component viewed with \textsc{Kappa} command \task{histogram}.
}
\end{latexonly}
...
\begin{htmlonly}
\myfig{sc21_noihist}{[b!]}{height=10cm}{fig:noihisto}{
  The error component viewed with \textsc{Kappa} command \task{histogram}.}{
  The error component viewed with \textsc{Kappa} command \task{histogram}.
}
\end{htmlonly}
...
\begin{latexonly}
\myfig{sc21_noigaia6}{[h!]}{width=\linewidth}{fig:noigaia}{
  The error map viewed with \gaia}{
  The error component displayed with \gaia. After clicking the
  \gaiathing{Error} button you will have to rescale your map.
}
\end{latexonly}

\begin{htmlonly}
\myfig{sc21_noigaia6}{[b!]}{width=\linewidth}{fig:noigaia}{
  The error map viewed with \gaia}{
  The error component displayed with \gaia. After clicking the
  \gaiathing{Error} button you will have to rescale your map.
}
...
First open your map in \textsc{Gaia}. Select the top level NDF from list in
...
\\*\\*
The maximum quality value increases from 3 to 7 if \texttt{COM} masking is
also included.
...
\myfig{sc21_dispmask3}{[hb!]}{width=0.95\hsize}{fig:maskdisp}{
...
\myfig{sc21_plotstcshapes}{[ht!]}{width=0.99\linewidth}{fig:stcshapes}{
...
\cref{Section}{sec:bright_ex}{\textttt{dimmconfig\_bright\_extended.lis}}. The
...
The is an option to reduce the noise in your background introduced by
...
your \texttt{AST} mask on a shorter scale for the last iteration only,
...
Signal-to-noise based masking is used for both the \texttt{AST} and
\texttt{FLT} models for extended emission. The recipe
\texttt{dimmconfig\_bright\_extended.lis} sets \texttt{ast.zero\_snr}
to 5 and \texttt{ast.zero\_snr} to 3. Setting \texttt{ast.zero\_snr}
too low can cause noise spikes to be interpreted as signal so
\texttt{ast.zero\_snrlo} is used which allows the mask to only grow it
it includes pixels with the specified S/N. This typically improves the
resulting map so is worth experimenting with. Note that \texttt{AST}
...
which is performed in these \texttt{AST}-free iterations.
\\\\
...
\myfigduo{sc21_850cosmo_bf}{sc21_850cosmo_mf_crop}{}{width=0.48\linewidth}{fig:cosmomap}{2mm}{
  Cosmology field with the matched filter applied}{
...
\texttt{sYYYYMMDD*\_fmos}
\texttt{sYYYYMMDD*\_mappsf}\\\\
\texttt{gsYYYYMMD*\_wmos}
\texttt{gsYYYYMMD*\_whiten}
\texttt{gsYYYYMMD*\_cal}
\texttt{gsYYYYMMD*\_mf}
\end{minipage}
\begin{minipage}[t]{0.7\linewidth}
The map for each observation\\
The map for each observation with an artificial\\point source added at the map centre\\
The co-add of all the \texttt{\_fmos} files\\
The whitened version of \texttt{\_wmos}\\
The calibrated version of \texttt{\_whiten}\\
The matched-filtered version of \texttt{\_cal}\\
\end{minipage}
...
series. This produces a co-added signal map (\_wmos) and a
coadded PSF map (\texttt{\_mappsf}).
...
pipeline (\texttt{\_fmos}), one of these observations might show visible
...
\texttt{\_mappsf} files to create a coadded PSF using the \picard\ recipe
\xref{\drrecipe{MOSAIC\_JCMT\_IMAGES}}{sun265}{MOSAIC_JCMT_IMAGES}.
\begin{myquote}
\begin{verbatim}
...
\end{verbatim}
\end{myquote}
This will create files beginning with \texttt{pgYYYMMDD}$\ldots$ that
should have the same suffices as above: \texttt{\_wmos},
\texttt{\_whiten}, \texttt{\_cal}, and \texttt{\_mf}.
...
input files (\texttt{filelist.txt}) and
...
\myfig{sc21_gal_exptime}{[b!]}{width=0.7\linewidth}{fig:exptime}{
...
\myfig{sc21_gal12_crop}{[t!]}{width=0.75\hsize}{fig:crop_map}{
...
\begin{itemize}
\item  Configuration file: \texttt{dimmconfig.lis}
...
\begin{itemize}
\item  Configuration file = \texttt{dimmconfig\_bright\_extended.lis}
...
\begin{itemize}
\item  Configuration file = \texttt{dimmconfig\_blank\_field.lis}
...
\texttt{mypars.ini} which contains the following text.
...
\newpage
\subsection{\xlabel{cog}Aperture-photometry curve of growth}
\label{app:cog}
...
\newpage
\subsection{\xlabel{fits}Convert format from FITS to NDF}
\label{app:fits}
...
following example, the FITS file called \texttt{file.fits} is
converted to NDF format as \texttt{file.sdf} using the \starlink\
package \convert. Note that the \texttt{.sdf} file extension NDF may
...
%  It does allow different minus signs.  The $-$ looks poor in
%  hypertext, but is needed for the regular document.
\begin{htmlonly}
\begin{table}
\begin{center}
\begin{small}
...
\subsubsection{\texttt{dimmconfig\_bright\_extended.lis}}
\begin{quote}
\begin{verbatim}
...
\subsubsection{\texttt{dimmconfig\_bright\_compact.lis}}
\begin{quote}
\begin{verbatim}
...
\subsubsection{\texttt{dimmconfig\_blank\_field.lis}}
\begin{quote}
\begin{verbatim}
   ^$STARLINK_DIR/share/smurf/dimmconfig.lis
...
\subsubsection{\texttt{dimmconfig\_bright.lis}}
