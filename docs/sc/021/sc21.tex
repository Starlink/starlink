...
the \texttt{numiter} parameter. This is shown below for \file{dimmconfig.lis}.
\vspace{-0.1cm}
\begin{quote}
\begin{verbatim}
numiter = -5
...
\texttt{com.perarray} is set to 1 indicating that a \model{COM} model
...
bright regions. Here
\texttt{ast.zero\_snr} is used to constrain the \model{AST} model to
...
\file{dimmconfig\_bright\_extended.lis} sets \texttt{ast.zero\_snr}
