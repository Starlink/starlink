...
\begin{fmpage}{0.95\linewidth}
\vspace{0.1cm}
TIP: If you do not include \param{method=rebin}, the map-maker will will
...
(calculated from median of error array)
\item \file{log.nefd}---NEFD calculated for each observation and for the co-added map(s)
\end{itemize}
...
\begin{fmpage}{0.95\linewidth}
\vspace{0.1cm}
TIP: Use the error array to avoid contamination of the noise
...
\begin{htmlonly}
\textbf{TIP: Use the error array to avoid contamination of the noise
distribution from bright sources.}
\end{htmlonly}
...
(\param{comp=var}) and quality (\param{comp=qua}) arrays (if available).
...
TIP: You can write out the error array of your map into a new file
...
\begin{htmlonly}
\textbf{TIP: You can write out the error array of your map into a new file
using the \textsc{Kappa} command \ndfcopy. For example, \texttt{\% ndfcopy map comp=err map\_err}.}
\vspace{4mm}
\end{htmlonly}
...
\\*\\*
The maximum quality value increases from 3 to 7 if \model{COM} masking is
also included.
...
As an example, you can set bad all pixel outside the \model{FLT} mask by doing:
\begin{myquote}
\begin{verbatim}
% setbb map 2
\end{verbatim}
...
description (polygonal or elliptical) for each clump. These are added as extra
...
\file{smurf\_makemap.def}. This file lives in your \$SMURF\_DIR directory or
...
Other parameters you may want to utilise include \param{flagfast} and
...
\param{xxx.zero\_union}   & Controls how the masks are combined. e.g. if two
                            masks are defined, one from lowhits and one from
                            the signal-to-noise.\\
...
\\\\
\textbf{AST mask}\\
\model{AST} masking is always used where emission is expected. The map is constrained to zero at all points outside the masked region and the \model{AST} model is estimated based only on data inside the
mask.
...
Signal-to-noise based masking is used for both the \model{AST} and
\model{FLT} models for extended emission. The recipe
...
resulting map so is worth experimenting with. Note that \model{AST}
...
\file{cosmo2\_mos\_mf.sdf} and a cropped version is shown in the
...
whiten         &   0 & If non-zero a whitening filter is applied to each
                       ...
                       data-cleaning stage \\
compreprocess  &   0 & If non-zero, the \model{COM} is removed before the
...
com.zero\_mask   &      0 & Whether to use \model{COM} masking \\
...
com.zero\_snr    &      0 & Excludes samples from the \model{COM} estimate
...
com.zero\_union  &      1 & Defines how multiple \model{COM} masks
                            are combined \\
com.zero\_freeze &      0 & Freezes the \model{COM} model after
...
flt.zero\_mask   &      0 & Whether to use \model{FLT} masking \\
...
flt.zero\_lohits &      0 & Excludes pixels that contain too few data samples
                            from the mask \\
flt.zero\_snr    &      0 & Excludes samples from the \model{FLT} estimate
                            that fall within map pixels with S/N values
...
flt.zero\_union  &      1 & Defines how multiple \model{FLT} masks are
                            combined \\
flt.zero\_freeze &      0 & Freezes the \model{FLT} model after
                            com.zero\_freeze iterations \\
...
\hline
ast.mapspike     &     10 & Throw away spikes in the \model{AST} map greater
                            than $n$-$\sigma$, excluding the first iteration \\
ast.zero\_mask   &      0 & Whether to use \model{AST} masking \\
...
ast.zero\_lohits &      0 & Excludes pixels that contain too few data samples
                            from the mask \\
ast.zero\_snr    &      0 & Excludes samples from the \model{AST} estimate
                            that fall within map pixels with S/N values
...
ast.zero\_snr\_low &$-$1.1& Sets the value (0.0 to 1.0) at which to
                            ...
                            ast.zero\_snr\_fwhm \\
ast.zero\_union  &      1 & Defines how multiple \model{AST} masks are
                            combined \\
ast.zero\_freeze  &     0 & Freezes the \model{AST} model after
                            com.zero\_freeze iterations \\
\hline
\end{tabular}
\label{tab:dimmdef}
...
com.zero\_lowhits&      0 & Excludes pixels that contain too few data
                            samples from the mask \\
com.zero\_snr    &      0 & Excludes samples from the \model{COM}
...
com.zero\_union  &      1 & Defines how multiple \model{COM} masks are
                            combined \\
com.zero\_freeze &      0 & Freezes the \model{COM} model after
                            com.zero\_freeze iterations \\
...
flt.zero\_niter  &      2 & The number of iterations for which the \model{FLT}
