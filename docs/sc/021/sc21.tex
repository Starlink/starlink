...
Other command-line options include \texttt{-log xsf} where the log file
is written to either the screen [\texttt{s}], a file [\texttt{f}] or an
X-window [\texttt{x}]. \texttt{s} or \texttt{sf} is recommended as the
recipes are short and the X-window automatically closes upon completion.
...
\subsection{\xlabel{problem}Problem solving configuration files}
\label{sec:problem}
...
\begin{fmpage}{0.95\linewidth}
\vspace{0.1cm}
TIP: The recipe name \drrecipe{FAINT\_POINT\_SOURCES\_JACKKNIFE} can be used
...
\begin{htmlonly}
\textbf{TIP: The recipe names \drrecipe{FAINT\_POINT\_SOURCES\_JACKKNIFE} can be
...
\myfig{sc21_gal_exptime}{[h!]}{width=0.7\linewidth}{fig:exptime}{
