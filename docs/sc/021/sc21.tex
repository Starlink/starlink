...
\end{verbatim}
\end{myquote}
The output is shown in the \cref{Figure}{fig:noihisto}{this plot}.
...
container file} window---see \cref{Figure}{fig:noigaia}{this figure}. You
will need to adjust the scaling to view the error map properly.
...
\myfig{sc21_dispmask3}{[b!]}{width=0.95\hsize}{fig:maskdisp}{
   Contours of an \texttt{AST} mask overlaid on a map}{
...
\myfig{sc21_plotstcshapes}{[h!]}{width=0.99\linewidth}{fig:stcshapes}{
  ...
  in this way open you map with \textsc{Gaia}. Under the \gaiathing{Image-Analysis}
...
be found in \cref{Appendix}{app:dimm}{Configuration Parameters: dimmconfig.lis}.
...
in \cref{Appendix}{app:dimm}{Configuration Parameters:
...
\myfig{sc21_skyloop}{[h!]}{width=0.55\linewidth}{fig:skyloop}{
  ...
  \small Illustration of the \task{skyloop} approach
...
flagfast      & 1000 & Flag data taken the telescope was moving too fast causing
                       source smearing\\
filt\_edgelow &    0 &Specifies the highest frequency to be retained by the  initial data cleaning \\
spikethresh   &    0 & The SNR value at which to flag spikes during the initial data cleaning.\\
spikebox      &   50 &Size of the filter box for the sigma-clipper used for spike detection in the initial data cleaning \\
pcathresh     &    0 & PCA cleaning is the threshold above which components
                       will be removed from the bolometer time-series.\\
pcalen        &    0 & This specifies the chunk length that will be cleaned as a number of time slices\\
...
noisecliprecom &   0 &This sets the noise clipping to occur prior to COM subtraction instead of at the end of the cleaning phase.\\
whiten         &   0 & If non-zero a whitening filter is applied to each time stream prior to any filtering in the data cleaning stage.\\
compreprocess  &   0 & If non-zero the common-mode is removed before the iterative algorithm begins.\\
\hline
...
\hline
com.perarray     &      0 & If non-zero, calculate a separate common-mode signal for each subarray.\\
...
com.gain\_fgood  &   0.25 &The minimum fraction of good gain boxes for a usable bolometer \\
com.gain\_rat    &      4 &The ratio of the largest usable gain to the mean gain for a  bolometer not to be rejected. \\

com.zero\_mask   &      0 & Whether to use COM masking\\
com.zero\_circle &  undef & Defines the mask as a circle with a specificed radius.\\
com.zero\_lowhits&      0 &  Excludes pixels that contain too few data samples from the mask \\
com.zero\_snr    &      0 & Excludes samples from the COM estimate that fall within map pixels with SNR values  greater than com.zero\_snr\\
com.zero\_snrlo  &      0 & The basic mask created by thresholding at the SNR value specified by com.zero\_snr is allowed to expand down to an SNR equal to com.zero\_snrlo \\
com.zero\_union  &      1 & Defines how multiple COM masks are combined\\
com.zero\_freeze &      0 & Freezes the COM model after com.zero\_freeze iterations  \\

\hline
noi.calcfirst    &      0 &If a non-zero value is supplied, the bolometer noise levels are calculated immediately after pre-conditioning\\
noi.box\_size    &      0 &Specifies the number of time slices used to determine the noise level in a section of a bolometer time stream \\
\hline

flt.notfirst     &      0 &If non-zero then low frequencies will not be removed from the time streams on the first iteration \\
...
flt.zero\_mask   &      0 & Whether to use FLT masking\\
flt.zero\_circle &  undef &Defines the mask as a circle with a specificed radius. \\
flt.zero\_lohits &      0 & Excludes pixels that contain too few data samples from the mask. \\
flt.zero\_snr    &      0 & Excludes samples from the FLT estimate that fall within map pixels with SNR values  greater than flt.zero\_snr\\\\
flt.zero\_snrlo  &      0 &The basic mask created by thresholding at the SNR value specified by flt.zero\_snr is allowed to expand down to an SNR equal to flt.zero\_snrlo \\
flt.zero\_niter  &      2 & The number of iterations for which the FLT model should be masked\\
flt.zero\_union  &      1 & Defines how multiple FLT masks are combined\\
flt.zero\_freeze &      0 & Freezes the FLT model after com.zero\_freeze iterations \\

\hline
ext.tausrc       &   auto & Options = auto, wvmraw, csotau, filtertau\\
ext.taumethod    & adaptive & Options = adaptive, full, quick\\
ext.csotau       &  undef &Specifies the CSO tau value to be used by the EXT model \\
...
ast.zero\_mask   &      0 & Whether to use AST masking\\
ast.zero\_circle &  undef & Defines the mask as a circle with a specificed radius.\\
ast.zero\_lohits &      0 & Excludes pixels that contain too few data samples from the mask.\\
ast.zero\_snr    &      0 & Excludes samples from the AST estimate that fall within map pixels with SNR values  greater than ast.zero\_snr\\
ast.zero\_snrlo  &      0 & The basic mask created by thresholding at the SNR value specified by ast.zero\_snr is allowed to expand down to an SNR equal to ast.zero\_snrlo \\
ast.zero\_snr\_fwhm &   0 & The map-maker produces two maps. The first is created normally using the mask specified by ast.zero\_snr". The final SNR-based mask is smoothed using a Gaussian with FWHM equal to ast.zero\_snr\_fwhm (in arcsec). The map-maker is re-run using this smoothed mask to create the final map\\
ast.zero\_snr\_low & $-$1.1 & gives the value (0.0 to 1.0) at which to threshold the smoothed mask specified by parameter ast.zero\_snr\_fwhm\\
ast.zero\_union  &      1 & Defines how multiple AST masks are combined.\\
ast.zero\_freeze  &     0 &Freezes the AST model after com.zero\_freeze iterations \\
...
options, see the comments in smurf\_makemap.def.}
\end{footnotesize}
\end{center}
\end{table*}
...
flt.notfirst     &      0 &If non-zero then low frequencies will not be removed from the time streams on the first iteration \\
450.flt.filt\_edge_largescale & 600 & Apply a frequency filter to the
                         FLT model. The value is given in arcsecs which the
                         map-maker converts to frequency.\\
850.flt.filt\_edge_largescale & 300 & Apply a frequency filter to the
                         FLT model. The value is given in arcsecs which the
                         map-maker converts to frequency.\\
