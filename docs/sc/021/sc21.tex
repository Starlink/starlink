...
\caption[Time-domain components of the iterative models]{
   \small Time-domain components of the iterative models. These
...
\myfig{sc21_dimmconfig4}{[t!]}{width=0.99\linewidth}{fig:inherit}{
  Inheritance order of the standard configuration files}{
...
position using the \picard\ recipe \param{SCUBA2\_REGISTER\_IMAGES}.
...
(2) Runs the \Kappa\ command \histat\ to extract the median value from
the error component.
{\begin{myquote}
\begin{verbatim}
% histat comp=err map_cal_crop
\end{verbatim}
\end{myquote}
...
\begin{htmlonly}
\textbf{TIP: Use the error component to avoid contamination of the noise distribution from bright sources.}
\end{htmlonly}
...
\caption[The error map visualised in two ways.]{The error map viewed with \textsc{Kappa}:histogram.}
\label{fig:noihisto}
\end{center}
\end{figure}

\subsection{Map statistics}
The \Kappa\ commands \histat\ and \stats\ are very similar and both return a range of statistics describing any NDF. In addition to the main data array they can be passed the error (comp=err), variance (comp=var) and quality (comp=qua) arrays (if available).

The reported statistics include the pixel maximum and minimum, standard deviation, number of pixels used and omitted, along with pixel mode, mean and median.
\begin{myquote}
\begin{verbatim}
...
\textbf{Note:} the Standard deviation of the data array will give a similar result to the mean/median of the error array except with additional contamination from sources.


\subsection{Viewing the noise histogram}
You can view a histogram of the error component with the \textsc{Kappa} command \histogram. Again  \param{comp=err} must be specified.
...
\end{verbatim}
\end{myquote}
The output is shown in the \cref{Figure}{fig:noihisto}{the figure above}. For more information on the options for \histogram\ see \kappasun.

\begin{figure}[b!]
\begin{center}
\includegraphics[width=\linewidth]{sc21_noigaia6}
\caption[The error map viewed with \gaia.]{The error component displayed with \gaia. After clicking the Error button you will have to rescale your map.}
\label{fig:noigaia}
\end{center}
\end{figure}


\subsection{Examining the error map in \gaia}
It is also useful to view the error map itself. Open your reduced map in \gaia, then select the `Error' button on the `Select NDF in container file' window---see \cref{Figure}{fig:noigaia}{the figure below}. You will need to adjust the scaling to view it properly.

To assess the noise in \gaia\ go to the toolbar on the main page and click on Image-Analysis$>$Image regions. Next select the region shape you would like to check and draw it on your map by clicking and dragging the mouse. Then click the 'Stats selected' button in the `Image regions' window to get a report of the statistics.

\begin{latexonly}
\begin{center}
\begin{fmpage}{0.95\linewidth}
\vspace{0.1cm}
TIP: You can write out the error component of your map into a new file using the \textsc{Kappa} command \ndfcopy. \% ndfcopy map comp=err map\_err
\end{fmpage}
\end{center}
\end{latexonly}

\begin{htmlonly}
\textbf{TIP: You can write out the error component of your map into a new file using the \textsc{Kappa} command \ndfcopy. E.g. \% ndfcopy map comp=err map\_err.}
\end{htmlonly}

\subsection{\xlabel{regridding}Regridding your Data}
\label{sec:regrid}
To change the pixel size use the \Kappa\ command \compave. The following example increases the pixel size from 4\,arcsec to 8\,arcsec by using a compress factor of 2.
\begin{myquote}
\begin{verbatim}
...
\begin{fmpage}{0.95\linewidth}
\vspace{0.1cm}
TIP: Remember you can use ndftrace if you are unsure of the pixel size.
\end{fmpage}
\end{center}
\end{latexonly}

\begin{htmlonly}
\textbf{TIP: Remember you can use ndftrace if you are unsure of the pixel size.}
\end{htmlonly}
...
Each value in the Quality component of an NDF contains 8 "on/off"
...
specific mask (1=AST, 2=FLT or 3=COM).

There are four potential values for the common combination of
\texttt{AST} and \texttt{FLT} masks:
\\*\\*
0 = pixel is inside both FLT and AST mask (i.e. is a source pixel)\\
1 = pixel is inside AST mask but not FLT mask\\
2 = pixel is inside FLT mask but not AST mask\\
3 = pixel is inside neither mask (i.e. is a background pixel)
\\*\\*
The maximum quality value increases from 3 to 7 if COM masking is included.

You can see which bit is used for each mask with \showqual.
\begin{myquote}
\begin{verbatim}
% showqual map count=yes
\end{verbatim}
\end{myquote}

As an example, you can set bad all pixel outside the FLT mask by doing:
\begin{myquote}
\begin{verbatim}
% setbb map 2
\end{verbatim}
