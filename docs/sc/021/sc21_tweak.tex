...
paramneter values, see Section ``\xref{Specifying Groups of
...
\textbf{itermaps}\\
Setting the parameter \setparam{ITERMAP}{itermap}{1} writes out an astronomical
...
configuration file sets \setparam{FLT.FILT_EDGE_LARGESCALE}{flt.filt\_edge\_largescale}{480}
...
has \setparam{NOI.BOX_SIZE}{noi.box\_size}{-15} (i.e. half a sub-scan). As the
...
For a general description of the purpose and effects of masking, see
...
\setparam{FLT.ZERO_NITER}{flt.zero\_niter}{2}).
...
and \model{FLT} models for extended emission. The configuration file
...
The bright-extended configuration file also sets \xparam{AST.SKIP}{ast.skip} to 5. This
...
\item \setparam{FLT.FILT_ORDER}{flt.filt\_order}{4} to set a 4th-order Butterworth
  filter for the \model{FLT} model.

\item \setparam{FLT.RING_BOX1}{flt.ring\_box1}{0.5} uses a filter to identify and flag
...
\item \setparam{COM.SIG_LIMIT}{com.sig\_limit}{5} flags time slices where the
...
listed below.
\begin{itemize}[nolistsep]
\item \setparam{COM.FREEZE_FLAGS}{com.freeze\_flags}{10} stops flags generated by the \model{COM}
model determination from changing after the first ten iterations.
\item \setparam{AST.ZERO_FREEZE}{ast.zero\_freeze}{10}
\item \setparam{FLT.ZERO_FREEZE}{flt.zero\_freeze}{10}
\item \setparam{COM.ZERO_FREEZE}{com.zero\_freeze}{10}
\end{itemize}
...
These configuration files can either be called directly by an external configuration
