...
You can supply values for any of these extra parameters on the command
...
type}. The input data are a short observation of a bright compact source
(CRL~2688):
...
\emph{By default, the data are down-sampled so that the on-sky distance between
...
from the list of input files supplied to \task{makemap}.}, so you are
...
of the techniques described in \cref{Chapter}{sec:raw}{SCUBA-2 Diagnostic Tools}, as it
...
of memory (see \cref{Section}{sec:computing}{Before you start:
computing resources}), it may be
...
if possible---see \latexhtml{the box
describing \emph{Data Chunking} on Page~\pageref{box:chunk})~}
{\htmlref{Data Chunking)}{box:chunk}}.}
...
where all raw data are processed in a single chunk, ``xx'' will be ``00''
for all itermaps.
...
By default \makemap\ assumes that the supplied data are raw data that
require cleaning before being used to make a map. However, if you include
...
supply to \texttt{makemap} have already been cleaned. There are two ways
to get cleaned data:
