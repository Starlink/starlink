...
values written to the screen by makemap at the end of each iteration (see
the previous section). For instance, if you run \makemap\ as follows:
...
file whilst \makemap\ is still running, for instance from another terminal
window:
...
iterations. If the mean normalised map change is not decreasing smoothly,
...
\emph{configuration} parameter.  The above command will create a new file
...
\xref{\task{picdef}}{sun95}{PICDEF} to divide the screen up into a grid
...
\begin{terminalv}
% gdclear
% picdef mode=array xpic=3 ypic=2 prefix=pic
% picsel pic1
...
It is sometimes informative to look at the \emph{change} between
...
parameters can often be specified by position instead of by name ---
check the help information for each command to see the order in which
...
(\emph{i.e.} blank). \cref{Figure}{fig:maskeditermaps}{Initial
six itermaps with masking} shows the resulting itermaps.
...
SCUBA-2 map using the same pixel grid as other map so that you can
...
This will cause the output map in file \texttt{outmap.sdf} to be gridded
on the same pixel grid as \texttt{herschel.sdf}.  This means that a given
...
exclude blank borders, thus allowing the two maps to have differing
...
coordinates in both images. However, the output image is cropped to
remove blank borders and so has smaller dimensions than the reference
image. All this is made possible by the fact that the NDF format, unlike
...
If you want to force the output map to have certain bounds within pixel
...
configuration files distributed with \smurf\ (see
...
requires cleaning before being used to make a map (see \cref{Section}
{sec:algorithm}{The reduction step-by-step}. However, if you include
...
the end of the name indicates cleaned data. The base name,
