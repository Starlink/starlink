...
\sstroutine{
   ...
   \sstdescription{
      ...
      Parameter \texttt{"}\xref{ast.mapspike\_freeze}{sun258}{AST.MAPSPIKE_FREEZE}\texttt{"}  can be used to control
...
\sstroutine{
   ...
   \sstdescription{
      ...
      below the ast.zero\_freeze value. A value less than zero
      means that the mask is frozen as soon as the initial
      iterations specified by parameter \texttt{"}\xref{ast.skip}{sun258}{AST.SKIP}\texttt{"}  have been
...
\sstroutine{
   ...
   \sstdescription{
      ...
      can be set separately using parameter \texttt{"}\xref{ast.zero\_mask0}{sun258}{AST.ZERO_MASK0}\texttt{"} . [0]
   }
   \sstattributetype{
      integer or string
   }
...
\sstroutine{
   ...
   \sstdescription{
      If ast.zero\_niter is non-zero, it determines the number
      ...
      the count of iterations. If ast.zero\_niter is greater than
      zero but less than 1.0, the AST mask will be used until
      the normalized change in the map between iterations drops
      below the ast.zero\_freeze value. A negative value for
      ast.zero\_niter will disable all masking of the AST model.
      If a value is set for \texttt{"} ast.zero\_niter\texttt{"} , the iterative
      map-making process will not terminate until the number of
      iterations required by the supplied \texttt{"} ast.zero\_niter\texttt{"}  value
      have occurred, regardless of whether the condition specified
      by parameter \texttt{"}\xref{maptol}{sun258}{MAPTOL}\texttt{"}  is achieved earlier. [0.0]
   }
   \sstattributetype{
      real
   }
...
\sstroutine{
   ...
   \sstdescription{
      If values are supplied for ast.zero\_snrlo and
      ...
      than the ast.zero\_snr value. [\texttt{$<$undef$>$}]
   }
   \sstattributetype{
      real
   }
...
\sstroutine{
   ...
   \sstdescription{
      ...
      the normalized change in the map between iterations drops
      below the com.zero\_freeze value. A value less than zero
      means that the mask is frozen as soon as the initial
      iterations specified by parameter \texttt{"}\xref{ast.skip}{sun258}{AST.SKIP}\texttt{"}  have been
      done. A value equal to zero means that the mask is never
      frozen. [0.0]
...
\sstroutine{
   ...
   \sstdescription{
      ...
      parameter \texttt{"}\xref{com.zero\_mask0}{sun258}{COM.ZERO_MASK0}\texttt{"} . [0]
   }
   \sstattributetype{
      integer or string
   }
...
\sstroutine{
   ...
   \sstdescription{
      If values are supplied for com.zero\_snrlo and
...
\sstroutine{
   ...
   \sstdescription{
      ...
      minute of good data (taking any down-sampling into account)
      from a bolometer before the entire bolometer is flagged as
      bad. A value of zero will cause a bolometer to be rejected
      if any steps are found in the bolometer data stream. Only
      used if parameter \texttt{"}\xref{dcfitbox}{sun258}{DCFITBOX}\texttt{"}  is non-zero. [4]
   }
   \sstattributetype{
      integer
   }
...
\sstroutine{
   ...
   \sstdescription{
      ...
      to use parameter \texttt{"}\xref{exportqbits}{sun258}{EXPORTQBITS}\texttt{"}  to specify that only a
      subset of the quality flags be written out t the NDF. [0]
   }
   \sstattributetype{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      \texttt{"} tau\_filt = a ( tau\_cso $+$ b $+$ c sqrt(tau\_cso) )\texttt{"} ,
      where \texttt{"} a\texttt{"} , \texttt{"} b\texttt{"}  and \texttt{"} c\texttt{"}  are the
      ...
      parameter name. [(23.3,-0.018,0.05)]
   }
   \sstattributetype{
      (real,real,real)
   }
...
\sstroutine{
   ...
   \sstdescription{
      The 850 um equivalent to the parameter \texttt{"}\xref{ext.taurelation.450}{sun258}{EXT.TAURELATION.450}\texttt{"} . [(3.71,-0.040,0.202)]
   }
   \sstattributetype{
      (real,real,real)
   }
...
\sstroutine{
   ...
   \sstdescription{
      ...
      (iii) specify this new map for the \texttt{"} fakemap\texttt{"}  parameter below. [\texttt{$<$undef$>$}]
   }
   \sstattributetype{
      string
   }
...
\sstroutine{
   ...
   \sstdescription{
      ...
      \sstitemlist{
         ...
         be supplied for the \texttt{"} fakescale\texttt{"}  parameter. The list must be
         ...
         \sstitem
          The \texttt{"} fakescale\texttt{"}  parameter can be set to an arbitrary
         ...
         for a chunk will then be used as the scale for that chunk. [1]
      }
   }
   \sstattributetype{
      real
...
\sstroutine{
   ...
   \sstdescription{
      ...
      the normalized change in the map between iterations drops
      below the flt.zero\_freeze value. A value less than zero
      means that the mask is frozen as soon as the initial
      iterations specified by parameter \texttt{"}\xref{ast.skip}{sun258}{AST.SKIP}\texttt{"}  have been
      done. A value equal to zero means that the mask is never
      frozen. [0.0]
...
\sstroutine{
   ...
   \sstdescription{
      ...
      separately using parameter \texttt{"}\xref{flt.zero\_mask0}{sun258}{FLT.ZERO_MASK0}\texttt{"} . [0]
   }
   \sstattributetype{
      integer or string
   }
...
\sstroutine{
   ...
   \sstdescription{
      ...
      requested via parameter \texttt{"}\xref{flt.clip}{sun258}{FLT.CLIP}\texttt{"} , in addition to the
      masking requested via the \texttt{"} flt.zero\_xxx\texttt{"}  set of parameters. [2]
   }
   \sstattributetype{
      int
   }
...
\sstroutine{
   ...
   \sstdescription{
      If values are supplied for flt.zero\_snrlo and
...
\sstroutine{
   ...
   \sstdescription{
      ...
      parameter \texttt{"}\xref{maptol\_mask}{sun258}{MAPTOL_MASK}\texttt{"} , and normalised by the RMS of the
      square root of the pixel variances. Compared to parameter
      \texttt{"}\xref{chitol}{sun258}{CHITOL}\texttt{"} , this is much more like a \texttt{"} by eye\texttt{"}  test, that will
      stop the solution when the map stops changing. [0.05]
   }
   \sstattributetype{
      real
   }
