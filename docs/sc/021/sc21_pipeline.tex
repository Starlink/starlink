...
location of the files listed, unless they are given as full path names.
...
various graphical windows may appear showing the pipeline results.
Also with the default ``-log'' option (including ``x'')
a new Xwindow will appear which
...
interface (\url{https://www.cadc-ccda.hia-iha.nrc-cnrc.gc.ca/en/jcmt/}).
