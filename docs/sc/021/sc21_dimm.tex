...
Science Archive are reduced with the \file{dimmconfig\_jsa\_generic.lis}
...
The \htmladdnormallink{\file{dimmconfig\_jsa\_generic.lis}}
{https://github.com/Starlink/starlink/blob/master/applications/smurf/examples/dimmconfig_jsa_generic.lis}
configuration file is designed to handle all science data. This is a good
...
The \htmladdnormallink{\file{dimmconfig\_blank\_field.lis}}
{https://github.com/Starlink/starlink/blob/master/applications/smurf/examples/dimmconfig_blank_field.lis}
configuration is tuned for blank field surveys for which the goal
...
The \htmladdnormallink{\file{dimmconfig\_bright\_compact.lis}}
{https://github.com/Starlink/starlink/blob/master/applications/smurf/examples/dimmconfig_bright_compact.lis}
configuration should be used for producing maps of bright, isolated compact
sources that are positioned at the centre of the map (\emph{e.g.} calibrators).
mask
...
The \htmladdnormallink{\file{dimmconfig\_bright\_extended.lis}}
{https://github.com/Starlink/starlink/blob/master/applications/smurf/examples/dimmconfig_bright_extended.lis}
configuration is for reducing maps that contain emission that is extended to some degree
...
than 5$\sigma$ (see \cref{Section}{sec:astmask}{AST masking}).
...
\item \htmladdnormallink{\file{dimmconfig\_fix\_convergence.lis}}
{https://github.com/Starlink/starlink/blob/master/applications/smurf/examples/dimmconfig_fix_convergence.lis}
to help your map converge. It does this by preventing masks from changing,
and by freezing the flags that identify aberrant bolometers. Both of
these restrictions are imposed only after 10 iterations have been
performed.

\item \htmladdnormallink{\file{dimmconfig\_fix\_blobs.lis}}
{https://github.com/Starlink/starlink/blob/master/applications/smurf/examples/dimmconfig_blobs.lis}
to remove large ``blooms'' or smooth blobs of spurious emission in your
map. These can be caused by ringing induced by unidentified steps or spikes in the bolometer
time-streams. The fix is to look for oscillations within the time-streams
following the \model{FLT} model, and flag such time-slices as unusable.
In addition, time slices at which the four sub-arrays see significantly
different common-mode values are also flagged as unusable.

\end{itemize}

Unlike the other configuration files described above, these recipes do not provide
...
\file{dimmconfig\_bright\_extended.lis} and \file{dimmconfig\_fix\_blobs.lis} should
be used:
