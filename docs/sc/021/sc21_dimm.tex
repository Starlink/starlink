...
later in model-specific sub-sections. For more information on the available
masking options, see \cref{Section}{sec:mask}{Masking options}.
...
have a potential problem  in that can upset the convergence process by
...
threshold to be used without introducing lots of isolated pixels into the
mask because of noise spikes in the map.
...
to zero t the end of each iteration for all samples that fall outside the
...
that the \model{COM} is more closely constrained.
...
features will become weaker as the edge so the mask are approached.
...
iteration. For this reason, one further iteration is performed ---
...
from each bolometer time-stream. To do this it uses a FFT-based high-pass
