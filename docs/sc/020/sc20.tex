...
\stardoccopyright   {Copyright \copyright\ 2015-2025 Science and Technology Facilities Council,\\
                    \& East Asian Observatory}
\stardocnumber      {20.4}
\stardocauthors     {H.\ S.\ Thomas, Malcolm J.\ Currie, \& H.\ A.\ L.\ Parsons}
\stardocdate        {2025 January 26}
...
format produced by setting parameter \param{VECTOR} to TRUE may be more
...
You can change the way the data are displayed in the \guiwindow{Display
...
is optionally created and applied.  It involves making cubes for each
...
are created.  These are specified by two environment variables.
For the bash and zsh shell these variables are assigned like this
\begin{terminalv}
% export ORAC_DATA_IN=<path to raw data>
% export ORAC_DATA_OUT=<where you want reduced data to go>
\end{terminalv}

while for csh and tcsh shells use the following commands.
\begin{terminalv}
...
whose names have the \file{oractemp} prefix.  To retain these too, set
ORAC\_KEEP to value \texttt{temp} rather than 1.
...
\htmlref{$`$\=U$`$\=u}{sec:uu} insert of
\htmlref{N\=amakanui}{sec:namakanui} commonly exhibit strong wave
...
\typeout{sc20_swimage inserted on Page \arabic{page}}
\typeout{sc20_swspectrum inserted on Page \arabic{page}}
...
\begin{terminalv}
% fitsval ppvcube MAP_PA
% fitsval ppvcube SCAN_PA
\end{terminalv}
...
\begin{terminalv}
% collapse in=ppvcube axis=1 estimator=median out=waves
\end{terminalv}
...
programmatic access run \htmlref{\parget}{sec:parget}, for parameters
\param{LBOUND} and \param{UBOUND} returned by \ndftrace.  Note that
...
bounds in variables \param{\$lbnd} and \param{\$ubnd}.

\begin{terminalv}
% manic in=waves out=waves_cube axes="[0,1,2]" lbound=$lbnd ubound=$ubnd
\end{terminalv}

When the angle difference is 0, set \param{axes="[1,0,2]"} to recover the second
axis.

Then subtract the waves from the original ppvcube.
\begin{terminalv}
% sub in1=ppvcube in2=waves_cube out=ppvcube_nosw
\end{terminalv}
...
\typeout{sc20_noswimage inserted on Page \arabic{page}}
\typeout{sc20_noswspectrum inserted on Page \arabic{page}}
...
\typeout{sc20_swspectrum_all inserted on Page \arabic{page}}
\typeout{sc20_noswspectrum_all inserted on Page \arabic{page}}
...
axes.  For example if the map had a non-zero MAP\_PA but was forced
to be upright in the same coordinate system
by running \makecube{} with \param{crota=0} then
the rotation angle in degrees may be obtained from the FITS
header MAP\_PA with \fitsval.

\begin{terminalv}
% rotate in=ppvcube out=rotatedcube angle=19.5
\end{terminalv}
...
\begin{terminalv}
% rotate in=rotated_ppvcube_nosw out=ppvcube_nosw angle=-19.5
\end{terminalv}

The outline example was for a row where the weak emission would not
influence the median estimate of the standing wave.  Where the spectra
contain significant emission signal, such spectra should be masked
prior to the collapse step.  This could be via object detection (cf.
\cref{Section}{sec:clumpfind}{}), possibly iteratively, having a first
stab at the standing-wave signal, thereby refining the baseline
subtract, and then run \findclumps\ again to improve the mask, and
then refine the estimate of the standing-wave pattern.  Another method
sets a threshold in the integ \htmlref{moments map}{sec:momentsmap}
above which values are set to bad, in order to form a mask of the
emission regions.

This method breaks down when there are no valid spectra in a scan row.
The velocity range(s) where the emission is located can be masked, and
hence the standing-wave pattern in the baseline regions removed.  Some
form of interpolation, such as \fillbad, across a narrow line may
approximate the standing-wave signal there.  More-sophisticated
approaches are beyond the scope of this cookbook, but some are
embodied in an \ORACDR\ recipe \narrowline.  See
\cref{Table}{tab:standing_params}{} for a list of its associated
recipe parameters.
...
(in this case we select \guithing{Kilometres-per-sec (radio)}).
...
Such a walk is performed for every pixel in the data array that is
...
REF\_SPECTRUM\_REGIONS & A comma-separated list of line extents ($v$1:$v$2) of the locations
                         of absorption lines to excise, measured in velocity units \\
\hline
\end{tabular}
\end{small}
...
STANDING\_WAVE\_REFINE\_METHOD & This decides which source position-position-velocity (PPV) cube
                                 is refined.  The default is \texttt{"Original"} (may be
                                 abbreviated to \texttt{"O"}), where the original PPV cubes are
                                 reused.  The alternative is \texttt{"Corrected"} (may be
                                 abbreviated to \texttt{"C"}), where the standing-wave filtered PPV
                                 is refined.  In both methods, each PPV has an improved emission
                                 mask, thanks to the initial estimate of the standing waves being
                                 subtracted. \\

STANDING\_WAVE\_SMOOTH & If set, a block smooth is applied to the spectra before
                         subtraction.  This is normally a single positive odd integer
                         specifying the smoothing kernel width in spectral channels,
                         but there is also a multiscale facility available, if
                         a comma-separated list is assigned to this parameter. \\

\hline
\end{tabular}
\end{small}
\caption{\label{tab:standing_params}
  The recipe parameters associated with the filtering of standing
  waves.  These methods are still experimental, although they have
  been used successfully for a pair of large surveys.  These parameters
  are currently available only in the \narrowline\ recipe.  They are
  unsuited to emission that is broad spectrally.}
\end{table}
