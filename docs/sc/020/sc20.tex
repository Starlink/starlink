...
\item  Telescope: telescope, instrument and it’s backend (TELESCOP/INSTRUME/BACKEND)
\item  Object: target, molecule and molecular transition being observed (OBJECT/MOLECULE/TRANSITI)
\item  Date obs: date of the observation (DATE−OBS or Epoch in UTC)
\item  Elevation: observatory elevation (ELSTART or 0.5 * (ELSTART+ELEND))
\item  Exposure: exposure time for the spectrum (EXTIME).
\item  Exposure (median): exposure time for the spectrum (EXP_TIME).
\item  Exposure (elapsed): the time elapsed during exposure (INT_TIME otherwise DATE-END − DATE-OBS)
\item  Exposure (effective): effective exposure time (EXEFFT)
\item  Coords sys: the coordinate system code (System)
\item  Spec position: position of spectrum on the sky (EXRAX, EXDECX)
\item  Src position: position of the observation centre (EXRRA, EXRDEC)
\item  Img centre: centre of the originating image (EXRA, EXDEC)
\item  Offset: offset of spectrum from observation centre (EXRRAOF, EXRDECOF or EXRAOF, EXDECOF)
\item  Doppler RA Dec: reference position (RefRA,RefDec)
\item  SourceVel: source velocity (SourceVel)
\item  SourceVRF: source velocity reference frame (SourceVRF)
\item  SourceSys: system of the source velocity (SourceSys)
\item  StdOfRest: standard of rest (StdOfRest)
\item  RestFreq: rest frequency (RestFreq
\item  ImageFreq: image sideband equivalent of the rest frequency (ImagFreq)
