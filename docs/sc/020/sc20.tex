...
\begin{tip} 
Be careful to give world coordinate values in floating point.
...
$T_A^*$. To convert this to main beam brightness temperature, $T_{MB}$ divide
your cube by the main-beam efficiency, $\eta_\textnormal{MB}$. Alternatively, to
convert to receiver temperature, $T_{rec}$, divide by the forward
...
applying the ARD mask, the cropped region is still surrounded by blank pixels. (Right)
...
\myfigduo{sc20_holeyharp}{sc20_polyfillaharp}{[ht!]}{
  width=0.48\linewidth}{fig:fillholes}{0.5mm}{Filling holes in your
  HARP map with \fillbad.}{Filling holes in your HARP map with
  \fillbad. Compare the original map (left) to the filled map (right).
  A blue pixel indicates an undefined spectrum.}
...
the SkyFrame coordinate system set the \att{System} attribute.
\begin{terminalv}
...
\begin{terminalv}
% wcsmosaic in='"rawfile_01,rawfile_02"' out=rawfile12 lbnd=! method=nearest accept
\end{terminalv}
...
coordinates, which can then form part of an NDF section to extract the
...
\myfig{sc20_clumps}{[h!]}{width=0.7\linewidth}{fig:clumps2}{
  ...
  with \gaia.}{A velocity slice of the result of three-dimensional clump
  finding viewed with \gaia. This output NDF contains boundaries of the
  clumps with the value reflecting the number of pixels contained within that
  clump.}
...
\myfig{sc20_plotclumps5}{[h!]}{width=0.5\linewidth}{fig:gaia_clumps3}{
  Catalogue window in \gaia.}{Catalogue window in \gaia.}
...
\myfigduo{sc20_gaia_avgspec2}{sc20_gaia_avgspec3}{[h!]}{width=0.48\linewidth}
   {fig:gaia_avgspec2}{0.5mm}{
   Example of averaged spectra.}{(Left) Average of the spectra within
...
for Galactic coordinates. Find out more about the \att{Format} attribute
...
This example plots a grid of 5$\times$4 pixels around $l=61.$\udeg$06
and $b=0.$\udeg$15. Each spectrum has a range of 10 to 30 km/s.  Note
the decimal points are necessary for the spectral limits, otherwise
the spectrum would extend from pixel coordinates 10 to 30.  The
...
\end{itemize}
\myfig{sc20_clinplot.eps}{[h!]}{width=0.9\linewidth}{fig:display6}{Displaying
