...
\providecommand{\fillbad}{\xref{\task{contour}}{sun95}{FILLBAD}}
...
from the FITS header SCAN\_PA.

\begin{terminalv}
% fitsval ppvcube SCAN_PA
\end{terminalv}

This will be either 0 or 90 degrees.  If there is no field rotation,
90 means the scan is along the first pixel axis (longitude or right
ascension), and 0 indicates scanning along the second pixel axis
(latitude or declination).  For SCAN\_PA=90 and a spectral cube
called ppvcube
...
spectra along each scan row.  Substitute \texttt{axis=2} if SCAN\_PA
...
When SCAN\_PA is 0, set \param{axes="[1,0,2]"} to recover the second
axis.
