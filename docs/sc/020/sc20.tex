...
Any spectrum displayed in \gaia\ (a single position spectrum or a spectrum
averaged over some region \ref{sec:gaiaaverage}) can be sent to \splat\ for more detailed
spectral analysis.
...
\begin{enumerate}[label=(\textbf{\arabic*})]
\item Select the spectrum you wish to send to \splat.

\item Select click on \gaiathing{Send: replace} or \gaiathing{Send: add} 
button in the \gaiathing{Display image regions} window.
\end{enumerate}
...
When \spat\ is initially started you will get a main window appear and also a spectral window.
