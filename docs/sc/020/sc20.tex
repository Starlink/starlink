...
\[T_{\textnormal{A}}^*(rms) = \frac{2\,T_{\textnormal{sys}}\,\kappa}{\sqrt{\Delta\nu\,t}}\]
...
itself (known as $T_{\textnormal{R^{*}}}$).  $T_{\textnormal{R^{*}}}$ can
...
\begin{tip}
Both $T_{\textnormal{sys}}$ and $T_{\textnormal{R^{*}}})$ are stored in
...
another window---the \gaiathing{Spectral plot} (see
...
\caption[\gaia\ select NDF in container file]{\label{fig:ndfgaia}
  Select NDF in container file - displayed upon loading a new file in \textsc{Gaia}
  which contains the data, the effective time and exposure time.}
\end{center}
\end{figure}
...
Sometimes you may wish to put you data onto a new pixel size scheme. 
This may either involve regridding the data onto larger pixel sizes or
resampling the data onto smaller pixels using \Kappa. 

The key difference between resampling and rebinning is if you are 
sampling the input pixel values (if resampling), or dividing an input
pixel value between a group of neighbouring output pixels (if rebinning).
...
resampling - Resampling of the grid of input data is performed by 
transforming the coordinates of the centre of each output 
pixel into the coordinate system of the input grid. Since the 
resulting coordinates will not, in general, coincide with the 
centre of an input pixel, sub-pixel interpolation is performed 
between the neighbouring input pixels. This produces a resampled 
value which is then assigned to the output pixel.
samples input gridded data (i.e. an image), it does not integrate it. 
Thus total data value in the input image will not, in general, be conserved.

rebinning - Rebinning of the grid of input data is performed by 
transforming the coordinates of the centre of each input pixel 
into the coordinate system of the output grid. 
The input pixel value is then divided up and assigned to the 
output pixels in the neighbourhood of the central output 
coordinates. A choice of schemes are provided for determining 
