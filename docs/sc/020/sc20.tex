...
\stardoccopyright   {Copyright \copyright\ 2015-2016 Science and Technology Facilities Council}
\stardocnumber      {20.1}
\stardocauthors     {H.\ S.\ Thomas \& Malcolm J.\ Currie}
\stardocdate        {2016 June 17}
...
\newcommand{\splatthing}[1]{\textbf{\textsf{#1}}}
...
% Window names as opposed to menu and button names.
\providecommand{\gaiawindow}[1]{"\texttt{#1}"}
...
\item The names of \splat\ commands or windows appear \splatthing{like this}.
...
another window---the \gaiawindow{Spectral plot} (see
\cref{Figure}{fig:gaia2}{the second figure below}). This spectrum will
be replaced when you click on a different pixel.

You can change the way the data is displayed in the \gaiawindow{Display
...
\caption[\gaia\ main window.]{\label{fig:rawgaia}
  ...
  panel. On the right, the \gaiawindow{Display image sections of a cube}
...
\caption[Spectral plot window with \gaia.]{\label{fig:gaia2}
  ...
  in the \gaiawindow{Display image sections of a cube} window. This can be
...
vertical red bar on the \gaiawindow{Spectral plot} window. As you do so
...
Step~\ref{step:iterate}. If not, the processing is complete.
...
\item Return to Step~\ref{step:loop}.
\end{enumerate}
...
\begin{figure}[h!]
\begin{center}
\caption[Flow chart of the ORAC-DR ACSIS pipeline process.]{\label{fig:pipeline}
  Flow chart of the processes employed by the ORAC-DR ACSIS pipeline,
...
control panel - see Section \ref{sec:gaiabaseline} for an example. 
This invokes \mfittrend.
...
\hspace{0.1cm}
\begin{minipage}[c]{0.23\linewidth}
In the \gaiawindow{Image regions} window go to \gaiathing{File$>$Save
...
\gaiawindow{Select NDF in container file} window include the exposure
...
\item Select the Baseline tab in the \gaiawindow{Display image sections
of a cube} window.
...
edges of these limit lines in the \gaiawindow{Spectral plot} window.
...
\item Select the \gaiathing{Chanmap} tab in the \gaiawindow{Display
...
drag the end-bars of the limit lines in the \gaiawindow{Spectral plot}
window.
...
\item Select the file you wish to contour in the \gaiawindow{Contouring} window.

\item Generate your contours in the \gaiathing{Generate} tab found on
the left-hand side of the \gaiawindow{Contouring} window. The example
...
window} in the top menu of the \gaiawindow{Contouring} window. Here you
...
\item In the \gaiawindow{Import CUPID catalogue} window, select a file
...
\begin{enumerate}[label=(\textbf{\arabic*})]
\item Select the \gaiathing{Spectrum} tab in the \gaiawindow{Display
image sections of a cube} window.
...
The \gaiawindow{Spectral plot} window will automatically update to show
...
\gaiawindow{Display image regions of a cube} window.

\item Select the spectrum you want to use as a template for your cube.

\item Select the Collapse tab in the \gaiawindow{Display image regions
of a cube} window.
...
surfaces.../Volume rendering} in the \gaiawindow{Display image sections
of a cube} window.
...
the \gaiawindow{Volume render} window.
...
\item Click on the \gaiathing{Interop} menu item in \splat.  If the
\gaiathing{SAMP control} icon has a red circle, it means that the SAMP
communication hub is running.  If, however, the icon has a white
circle, you need to start a hub.  To do this press the
\gaiathing{Register with HUB} button at the bottom of the window, and
then press the \gaiathing{Start internal hub} button in the window that
pops up.

\item If you had \gaia\ already running before the hub was active, then in
\gaia\ select the \gaiathing{Interop$>$Register} to register your GAIA with
the hub.

\item Select the spectrum you wish to send from \gaia.

\item Click on SPLAT-VO \gaiathing{Send: replace} or \gaiathing{Send: add}
button near the bottom of the \gaiawindow{Display image sections of a cube}
...
will automatically be overlaid on the spectrum displayed.
...
\item  RestFreq
\item  ImageFreq
\item  Channel spacing
\item  Number of channels
\end{enumerate}

To show or remove the synopsis of a spectrum simply click on the
"Options" button in the spectral view window and select or deselect
"Display Synopsis". To move the position of the synopsis simply click
...
\item click on \splatthing{Change the units of the current spectrum}
...
\caption[Change the units of the current spectrum.]{\label{fig:splat_units}
  Select the \splatthing{Change the units of the current spectrum}
  button. Note that in this example the current units are Frequency.}
\end{center}
\end{figure}

\item A new window will appear - \splatthing{Current spectrum units}.
...
\caption[Current spectrum units window.]{\label{fig:splat_units2}
  Current spectrum units window.}
\end{center}
\end{figure}

\item From the Coordinates drop down menu select the desired unit
(in this case we select \splatthing{Kilometers-per-sec (radio)}).
Then click \splatthing{Apply}.
...
\caption[Changing the units via the drop down menu.]{\label{fig:splat_units3}
  Changing the units of the spectrum via the drop down menu to km/s.}
\end{center}
\end{figure}

\item You will now see that the spectrum is
displayed in \splatthing{Radio Velocity (km/s)}.
...
\caption[Spectrum with units of km/s.]{\label{fig:splat_units4}
  Spectrum with units of km/s.}
\end{center}
\end{figure}
...
\item Click on \splatthing{Cut out the regions of the current spectrum} button
...
\typeout{sc20_splat_cutregion inserted on page \arabic{page}}
\caption[Selected regions for removal.]{\label{fig:splat_crop3}
  The two regions selected.}
\end{center}
\end{figure}

\item Select the \splatthing{Remove selected button.}

\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.65\linewidth]{sc20_splat_cutregion}
\typeout{sc20_splat_cutregion2 inserted on page \arabic{page}}
\caption[Removing regions.]{\label{fig:splat_crop4}
  Selecting the \splatthing{remove selected} button.}
\end{center}
\end{figure}
...
\item Click on \splatthing{Apply a filter to the current spectrum} button
above the spectrum you wish to work on. This will open a new window.


\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.7\linewidth]{sc20_splat_rebin}
\typeout{sc20_splat_rebin inserted on page \arabic{page}}
\caption[Filter regions of a spectrum window.]{\label{fig:splat_rebin1}
  The \splat\ \splatthing{Filter regions of a spectrum window}.}
\end{center}
\end{figure}


\item Select the number of channels over which you want to 
bin - this is the \splatthing{Width}.
In this example the spectrum has a resolution of 0.055km/s. We
bin by 18 channels to rebin the spectrum to 1km/s channels.

\item Select \splatthing{Filter (replace)}. This will create a
...
\caption[The final rebinned spectrum.]{\label{fig:splat_rebin2}
  The final rebinned spectrum. }
\end{center}
\end{figure}
...
\item Click on \splatthing{Get statistics on region of spectrum}
...
\caption[Select the stats button.]{\label{fig:splat_stats1}
  Select the stats button.}
\end{center}
\end{figure}

\item Select \splatthing{Add} from the \splatthing{Regional
statistics} region window.

\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.6\linewidth]{sc20_splat_stats1}
\typeout{sc20_splat_stats1 inserted on page \arabic{page}}
\caption[Select the button in statistics.]{\label{fig:splat_stats2}
  Select the \splatthing{Add} button.}
\end{center}
\end{figure}
...
in the data we click on the \splatthing{Add} button a second time.
...
a table in the \splatthing{Regional statistic} window with some
...
\caption[Two spectral regions have been selected.]{\label{fig:splat_stats3}
  ...
  in the \splatthing{Regional statistic} window.}
\end{center}
\end{figure}

\item Click on \splatthing{Selected stats} or \splatthing{All stats}
...
\caption[statistics for both regions.]{\label{fig:splat_stats4}
  Statistics for both regions are displayed in the \splatthing{Regional
  statistic} window. }
\end{center}
\end{figure}
...
\item Click on \splatthing{Fit spectral lines using a variety of functions}
button above the spectrum you wish to work on. This will open a
new window. 
...
\caption[Select the SPLAT fit button.]{\label{fig:splat_fit1}
  Select the spectral fitting button.}
\end{center}
\end{figure}


\item Click on the \splatthing{Add} button in the
\splatthing{Measure spectral lines} window. 
...
\caption[Click on the Measure spectral lines add button.]
  {\label{fig:splat_fit2}
  Click on the \splatthing{Add} button.}
\end{center}
\end{figure}
...
\item Click on the \splatthing{Fit} button to produce both a
\splatthing{Quick} and \splatthing{Gaussian} fit to the spectral line.
...
\caption[Getting the spectral line fit.]{\label{fig:splat_fit4}
  In the \splatthing{Measure spectral lines} window we see the both
  the \splatthing{Quick} fit (black dashed line) and the
  \splatthing{Gaussian} fit (pink line) of the spectral line.}
\end{center}
\end{figure}


\item We see the results of the fit are provided in the
\splatthing{Measure spectral lines} window and the two fits can
...
\caption[Spectrum with fits.]{\label{fig:splat_stats5}
  In the spectral window we see the both the \splatthing{Quick}
  fit (black dashed line) and the \splatthing{Gaussian} fit
...
survey of the kinematics of the Perseus molecular cloud -- I. Data}}{
http://dx.doi.org/10.1111/j.1365-2966.2009.15658.x}, MNRAS, 401, 455 (DOI:10.1111/j.1365-2966.2009.15658.x)
...
\textbf{Parameter} & \textbf{Description} \\
\hline
LOWFREQ\_INTERFERENCE & If set to true (1) the spectra for each receptor are
