...
\stardocnumber      {12.1}
\stardocauthors     {Jon Lockley}
\stardocdate        {28 November 1997}
\stardoctitle       {Writing your own Data Reduction Software}
\stardocabstract  {This Cookbook shows you how to write your
...
\begin{quote}
\fbox{\textbf{Make absolutely sure you \emph{need}, to write new software.}}
\end{quote}

Use the \textsf{findme} command \xref{(SUN/188)}{sun188}{} to see if the job
...
the call to \textsf{ZAP} to fix the variance values using these pointers.

\subsection{The Quality component}

In addition to the Data and Variance components, some NDFs also contain a
\emph{Quality}\, component.  The quality array is not usually accessed
directly by the user. Instead, the processing of the Data and Variance
arrays \emph{automatically}\, takes into account the values in the Quality
...
The default used by the routine \textsf{NDF\_PROP} is \emph{not}\, to propagate
...
\emph{libraries}. These contain large amounts of code already written for
...
\textbf{Just like you had to link your application to the NDF and IMG
...
distributed. The PDA library \emph{can}\, be distributed freely. If you
...
to help implement this. The \textsf{HLP} package
...
Convert -- \emph{(Automatic) format conversion} - \xref{(SUN/55)}{sun55}{}

Findme -- \emph{Documentation search via a Web browser} -
\xref{(SUN/188)}{sun188}{}

FIO -- \emph{Fortran file handling} - \xref{(SUN/143)}{sun143}{}

HLP -- \emph{On-line documentation support} - \xref{(SUN/124)}{sun124}{}.

IMG -- \emph{Simple data access library} - \xref{(SUN/160)}{sun160}{}

MEMSYS -- \emph{Maximum entropy image reconstruction} -
\xref{(SUN/117)}{sun117}{}

NDF -- \emph{More advanced data access library} - \xref{(SUN/33)}{sun33}{}

PDA -- \emph{Public domain mathematical algorithms (as used by Starlink
Software)} - \xref{(SUN/194)}{sun194}{}

Showme -- \emph{Web-based document retriever} - \xref{(SUN/188)}{sun188}{}

SLALIB -- \emph{Positional and time information} - \xref{(SUN/67)}{sun67}{}

\subsection{Further reading}

ADAM -- \emph{The Starlink software environment} - \xref{(SG/4)}{sg4}{}

ADAM -- \emph{Introduction to ADAM programming} - \xref{(SUN/101)}{sun101}{}

ADAM -- \emph{Unix version} - \xref{(SUN/144)}{sun144}{}

ADAM -- \emph{Programmer's facilities and documentation guide} - \xref{(SG/6)}{sg6}{}

ADAM -- \emph{Graphics programmer's guide} - \xref{(SUN/113)}{sun113}{}

ADAM -- \emph{Interface module, reference module} - \xref{(SUN/115)}{sun115}{}
...
CNF -- \emph{C and Fortran mixed programming} - \xref{(SUN/209)}{sun209}{}

FITSIO -- \emph{Disk FITS input/output subroutines} - \xref{(SUN/136)}{sun136}{}

FIO/RIO -- \emph{Fortran file I/O routines} - \xref{(SUN/143)}{sun143}{}

FTNCHEK -- \emph{A Fortran77 source-code checker} - \xref{(SUN/172)}{sun172}{}

FORCHECK -- \emph{Fortran verifier and programming aid} -
\xref{(SUN/73)}{sun73}{}

HDSTRACE -- \emph{HDS data file listing}  - \xref{(SUN/102)}{sun102}{}

\emph{How to write good documents for Starlink} - \xref{(SGP/28)}{sgp28}{}
...
\emph{Starlink applications programming standard} -- \xref{(SGP/16)}{sgp16}{}

\emph{Starlink C programming standard} -- \xref{(SGP/4)}{sgp4}{}
