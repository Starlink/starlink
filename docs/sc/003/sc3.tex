...
\stardocnumber      {3.2-0} %%VERSION%%
\stardocauthors     {Martin Clayton}
\stardocdate        {10 July 2003}
\stardoctitle       {Echelle Data Reduction Cookbook}

%% The vspace is a bit of a hack to deal with the fact that
%% the Starlink class inserts space for manual type and software
%% version even if they are not defined
\startitlepic{
\vspace{-20mm}
...
are generally of two types; \emph{templates} which readers may be able to
modify to suit their particular needs and \emph{utilities} which carry out
...
\providecommand{\EchelleHomePage}{http://www.starlink.ac.uk/echomop/echelle/}
...
\item [\htmlref{\textbf{MEANARC }}{se_meanarc}]
...
\item [\htmlref{\textbf{ECHTRACT}}{se_echtract}]
...
\item [\htmlref{\textbf{STACKGEN}}{se_stackgen}]
      takes a collapsed \'{e}chelle spectrum and reads
      the orders into a \xref{DIPSO}{sun50}{} stack.

\item [\htmlref{\textbf{TRACEPOLY \& ARCPOLY}}{se_tracepoly}]
...
\item [\htmlref{\textbf{Automated ECHOMOP reduction}}{se_automated_echomop}]
...
\item [\htmlref{\textbf{ECHTRIM}}{se_echtrim}]
...
planning to use \emph{e.g.} \xref{CCDPACK}{sun139}{} or
...
\item \htmladdnormallink{\texttt{http://www.starlink.ac.uk/iraf/web/tutorials/doecslit/doecslit.html}}
...
\item \scspec{\texttt{http://www.star.ucl.ac.uk/\symbol{126}mjc/echelle/}}
...
\item [\textbf{Description:}] \mbox{} \\
     ...
     (produced using \xref{\texttt{ech\_result}}{sun152}{ech_result} or
     \xref{\texttt{echmenu} Option 14}{sun152}{option14}) or FIGARO
...
\item [\htmlref{\textbf{PREPARCS}}{se_preparcs}]
...
In the scripts included, the \xref{\texttt{echmenu}}{sun152}{ECHMENU} option
sequence:

\begin{quote}
\begin{description}

\item [4.2 \xref{\texttt{ech\_object}}{sun152}{ech_spatial}]
      Get the object channel.

\item [22 \xref{\texttt{ech\_mdlbck}}{sun152}{ech_mdlbck}]
      Model the background as scattered light.

\item [7 \xref{\texttt{ech\_profile}}{sun152}{ech_profile}]
      Model the object profile.

\item [8 \xref{\texttt{ech\_extrct}}{sun152}{ech_extrct}]
      Extract the orders.

\item [14 \xref{\texttt{ech\_result}}{sun152}{ech_result}]
