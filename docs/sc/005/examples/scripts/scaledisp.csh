...
# Purpose:
# Runs KAPPA's DISPLAY on an image but within set thresholds. Bounds are
# defined in terms of ADEV (absolute deviation of estimated sky background
# count) and are derived via the STARLINK ESP package. For many purposes
...
# History:
#  9/4/99 (ACD): Original version (created from GJP's saodisp.csh).
...
# Run KAPPA lutneg to set the palette.
  lutneg device=xw

# Run KAPPA display 
  set kapin = \
   " in=$fname device=xw mode=scale low=`cat tmp_low` high=`cat tmp_high` "
  echo " "
  echo "display $kapin"
  echo " "
  display $kapin
