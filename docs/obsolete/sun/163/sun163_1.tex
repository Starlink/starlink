...
\sstroutine{
   ...
   \sstdescription{
      ...
      quality within the IRAS90 package see the section \ref{SEC:QUAL}.

      The user selects the pixels to be operated on either by giving a
      group `mask{\tt '} NDFs (one for each input NDF), or a list of pixel
      indices (see parameter SELECT). In the first case, the bad pixels
      in each mask NDF define the selected pixels in the corresponding
      input NDF. In the second case, the pixel indices stored in the
      list define the selected pixels (a single list is provided which
      is used for all input NDFs).

      The operation to be performed on the pixels is specified by
      parameter FUNCTION. The given quality may be assigned to or
      removed from pixels within the NDFs. The pixels thus operated on
      can either be those selected by the parameters MASK and LIST,
      (i.e. the bad pixels in the mask, or the pixels in the list) or
      those not selected (i.e. the good pixels in the mask, or the
      pixels not in the list).  The quality of all other pixels is left
   ...
   \sstusage{
      SETQUAL NDF QNAME COMMENT MASK LIST=?
   }
   \sstparameters{
      \sstsubsection{
      ...
      \sstsubsection{
         ...
         the {\tt "}selected{\tt "} pixels specified using parameters MASK and
         LIST. It can take any of the values {\tt "}HS{\tt "}, {\tt "}HU{\tt "}, {\tt "}NS{\tt "}, {\tt "}NU{\tt "},
   ...
   \sstexamples{
      ...
      \sstexamplesubsection{
         SETQUAL {\tt "}M51,CENA{\tt "} SOURCE\_A SELECT=LIST LIST=\^{}SOURCE\_A.LIS FUNCTION=HS$+$NU
      }{
