...
\verb%filename% may optionally include the \verb%.ifl% extension.
\verb$filename.ifl$ will be compiled into \verb%filename.ifc%.
...
\verb%name.ifc% (where {\em name} is the name of the program's executable 
file); if that is not found, it looks for a source file, \verb%name.ifl% 
...
ENDINTERFACE
\end{verbatim} \end{quote}
\verb%programname% is a character string giving the name of the Interface 
Module. 
...
linked into a single executable file called a {\em monolith} (see SG/4).
...
ENDMONOLITH
\end{verbatim} \end{quote}
where \verb%programname_1%, \verb%programname_2%, {\em etc.}\ are the names of 
...
clarity.
\item Comments may be introduced by \verb%#% (apart from within
quoted strings).
...
sessions. The parameter file name is \verb%name.sdf% (where \verb%name% is the
name of the executable file) and, by default, it will be found in subdirectory
\verb%adam% of the user's login directory.
...
ENDPARAMETER
\end{verbatim} \end{quote}
where \verb%parametername% is a character string giving the program parameter 
...
TYPE datatype
\end{verbatim} \end{quote}
where \verb%datatype% is a character string giving the required type of the
parameter.
\verb%datatype% may be:
\begin{itemize}
...
ENDINTERFACE
\end{verbatim} \end{quote}
If the user specified a value of \verb%[1.2,511.8,255.6,512.1]%
...
conversion and can perform it for the user ({\em i.e.}\ \verb%[1,511,255,512]% 
is stored).
...
been truncated ({\em i.e.}\ \verb%[1.0,511.0,255.0,512.0]% would be supplied 
to the program).
...
\end{verbatim} \end{quote}
where \verb%parameter-type% is a character string which can currently only be
DEVICE.
...
ACCESS mode
\end{verbatim} \end{quote}
where \verb%mode% is a character string which may be:
\begin{description}
...
POSITION n
\end{verbatim} \end{quote}
where \verb%n% is an integer constant which specifies the allocated position.
...
\end{verbatim} \end{quote}
Then the HDS object \verb%iue.image.swp2099% would be associated with parameter
...
KEYWORD  name
\end{verbatim} \end{quote}
where \verb%name% is a valid keyword\footnote{See Appendix \ref{names} for a
...
SWITCH, then:
\begin{quote}
\verb%test switch% is equivalent to \verb%test switch=TRUE%
\end{quote}
and
\begin{quote}
\verb%test noswitch% is equivalent to \verb%test switch=FALSE%
\end{quote}
...
\end{verbatim} \end{quote}
where \verb%default_value% may be:
\begin{itemize}
...
A character constant (quoted string) may be given as \verb%default_value% 
...
\end{verbatim} \end{quote}
where \verb%association-specification% is a character string consisting of two 
parts -- an association operator followed by a parameter specifier.

There are three association operators:
\begin{itemize}
\item \verb%<- % The associated parameter may be used by the program (see
Sections \ref{vpath} and \ref{ppath}) but will not be updated by it.
\item \verb% ->% The associated parameter will be set to the value of the
...
\end{itemize}
\item \verb%<->% combines \verb%<-% and \verb%->%.
\end{itemize}
...
\end{verbatim} \end{quote}
where \verb%value-resolution-path% is a character string which gives a path
...
A \verb%value-resolution-path% of 'DYNAMIC,CURRENT,NOPROMPT' is
...
RANGE min, max
\end{verbatim} \end{quote}
where \verb%min% and \verb%max% are two character or number
...
If \verb%min > max%, the value must not lie between them. The values themselves
...
IN set-of-values
\end{verbatim} \end{quote}
where \verb%set-of-values% consists of a list of character or number
...
return key.
On VMS, hitting the TAB key will make the suggested value available in the 
...
\end{verbatim} \end{quote}
where \verb%value-resolution-path% is a character string 
...
\verb%\%. The suggested value will be taken for the prompted parameter and
for any remaining parameters.
It is also possible to force ACCEPT for individual parameters by putting
\verb%keyword=ACCEPT% or \verb%keyword=\% on the command line (where
\verb%keyword% is the keyword of the parameter).
...
\end{verbatim} \end{quote}
where \verb%help_specifier% is a character string of the form:
\begin{quote} \begin{verbatim}
filename key1 key2 etc...
\end{verbatim} \end{quote}
specifying that the text to be displayed may be found in the help file 
\verb%filename% and the module defined by \verb% key1 key2 etc...% (but see
...
\verb%help_specifier%  may also be \verb%*% or \verb%'*'%, in which case a
default value of:
\begin{quote} \begin{verbatim}
interface_name PARAMETERS parameter_name
\end{verbatim} \end{quote}
will be used, where \verb%interface_name% is the name specified in the
...
HELP help-specifier
\end{verbatim} \end{quote}
where \verb%help-specifier% is a character string giving help information about 
...
\verb%filename% and the module defined by \verb% key1 key2 etc...%
...
HELPLIB help_top_level_spec
\end{verbatim} \end{quote}
where \verb%help_top_level_spec% is a character string to be inserted (with an 
...
SUN/104), then a message text is required. 
...
ENDMESSAGE
\end{verbatim} \end{quote}
where \verb%parametername% is a character string specifying the Message 
...
\verb%sting% may by anything acceptable to the ERR or MSG routine 
...
\label{action}
The concept of ACTION is specific to instrumentation tasks (see SUN/134).
...
\end{verbatim} \end{quote}
where \verb%context% is \verb%OBEY% or \verb%CANCEL%.
...
ENDACTION
\end{verbatim} \end{quote}
Where \verb%actionname% is a character string specifying a valid action name.
...
KEYWORD  name
\end{verbatim} \end{quote}
where \verb%name% is a character string specifying a valid keyword.
...
HELP help-text
\end{verbatim} \end{quote}
where \verb%help-text% is a character string giving help information about the
action.
\subsubsection*{Example}
\begin{quote} \begin{verbatim}
ACTION GET_FILTER
...
\end{verbatim} \end{quote}
where \verb%menuname% is a character string specifying the name to be shown on
the screen, and \verb%XCOORD% and \verb%YCOORD% are integers giving the 
...
characters which are either all alphanumeric (with \verb%.;:+-()_[]"'<>% being
...
({\em i.e.}\ not \verb%''%) quote or the end of line.
...
Everything following \verb%#% on a line (except within a character
string) will be ignored.
Thus  \verb%#% can be used to introduce comments.
...
\verb%compifl% will go on to write the \verb%.ifc% file which could be read
...
ENDPARAMETER
\end{verbatim} \end{quote}
where \verb%value% is a primitive value of an appropriate type. Again the
...
System (see SUN/124). If the specified file name has no extension or extension
\verb%.shl% or \verb%.hlb%, \verb%.shl% is assumed; anything else is an error.

On VMS, either Portable HELP or the VMS help system
may be used.
If the given filename has extension \verb%.shl%, Portable HELP is used:
if no file extension, or \verb%.hlb%, is specified,
...
{\em E.g.}  
\begin{quote}
\verb%$environment_variable/relative_pathname%\/
\end{quote}
or
\begin{quote}
\verb%logical_name:filename%
\end{quote}
...
PROGRAM progname
\end{verbatim} \end{quote}
where \verb%progname% is a character string (or reserved word) which is
...
EPATH dpath
\end{verbatim} \end{quote}
where \verb%dpath% is a character constant listing the order of search through
...
Parameters may also now be forced to ACCEPT by the \verb%keyword=\% parameter 
specifier or by typing `\verb%\%' in response to a prompt.
