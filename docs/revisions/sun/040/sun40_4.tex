...
\newcommand{\stardocnumber}    {40.4}
\newcommand{\stardocauthors}   {A C Charles\\P C T Rees}
\newcommand{\stardocdate}      {31 March 1994}
...
None of the CHR routines report error messages; {\em i.e.}\ they do not 
use the Starlink Error Reporting System, ERR (see SUN/104).
However, some routines do use the inherited status conventions (SUN/104)
...
Two include files are available for use with the Character Handling Routines.
...
CHR\_ERR is included to define the CHR error values.
The logical names required for the use of the CHR library are
defined by executing the procedure CHR\_DEV, {\em e.g.}\
...
To link the CHR library with non-ADAM software, the linker input specifier
...
\begin {quote}
\begin{verbatim}
$ LINK PROGRAM, CHR_LINK/OPT
\end{verbatim}
\end {quote}

All the logical names required during the compilation of ADAM software 
using CHR are defined by the ADAM development start-up procedures, {\em e.g.}
...
ADAM applications will automatically be linked with the CHR library when
using the ADAM link procedure.
...
\begin {sloppypar}
To link a program with the Character Handling Routines on UNIX machines 
the command line is
\end {sloppypar}

\begin {quote}
\begin{verbatim}
% f77 program.o -L/star/lib `chr_link` -o program.out
\end{verbatim}
\end {quote}
...
\subitem {\bf CHR\_\_EOSNT} -- End of sentence.
\subitem {\bf CHR\_\_WNOTF} -- Word not found.
\indexspace
...
\sstroutine{
   ...
   \sstdescription{
      ...
      in the machine{\tt '}s character set. If no such character
      exists within the machine{\tt '}s character set, the character code 0
   ...
   \sstreturnedvalue{
      \sstsubsection{
         ...
         A character value within the machine{\tt '}s character set.
      }
   }
}
\sstroutine{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      character in the machine{\tt '}s character set.  All
   ...
   \sstarguments{
      \sstsubsection{
         ...
         be returned, e.g., {\tt '}BEL{\tt '}, {\tt '}BS{\tt '}, etc.
      }
   }
   \sstreturnedvalue{
      \sstsubsection{
...
\sstroutine{
   ...
   Translate a string from ASCII to the machine{\tt '}s character set
}{
   \sstdescription{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         The character string translated into the machine{\tt '}s character
...
\sstroutine{
   ...
   \sstarguments{
      \sstsubsection{
         ...
         String to be decoded, e.g. `10101100{\tt '}.
      }
      \sstsubsection{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      be set to {\tt '}\#{\tt '} if truncation occurs.
   }
   \sstinvocation{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         truncation occurs: if TRUNC is .TRUE., a {\tt '}\#{\tt '} will be
         written into the last element of the returned string on
         truncation; if TRUNC is .FALSE., no {\tt '}\#{\tt '} is written to the
         returned string.
      }
      \sstsubsection{
         STR2 = CHARACTER $*$ ( $*$ ) (Returned)
...
\sstroutine{
   ...
   \sstdescription{
      ...
      number is written as a real number in Fortran `exponential{\tt '}
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         as hr/deg:min:sec: if UNITS = {\tt '}HOURS{\tt '}, the maximum permitted
         value is 24.0; if UNITS = {\tt '}DEGREES{\tt '}, the  maximum permitted is
...
\sstroutine{
   ...
   \sstdescription{
      ...
      used. In the event of an error, {\tt '}$*${\tt '}s are written to the string.
   }
   \sstinvocation{
...
\sstroutine{
   ...
   Translate a string from EBCDIC to the machine{\tt '}s character set
}{
   \sstdescription{
      The string STR1, which has been written on a machine which uses
...
\sstroutine{
   ...
   \sstdescription{
      The given character, encoded using the machine{\tt '}s character set,
...
\sstroutine{
   ...
   \sstarguments{
      \sstsubsection{
         ...
         {\tt '}string1,string2,.......,stringN{\tt '} where each of the
...
\sstroutine{
   ...
   \sstdescription{
      ...
      {\tt '}$*${\tt '}s are written to the string.
   }
   \sstinvocation{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      concise a format as possible, and return the number of characters
      used. In the event of an error, {\tt '}$*${\tt '}s will be written into to the
      string.
   }
   \sstinvocation{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      machine{\tt '}s character set. The result is right-justified in the
      returned string. In the event of an error, {\tt '}$*${\tt '}s are written to
      the string.
   }
   \sstinvocation{
...
\sstroutine{
   ...
   Translate a string from the machine{\tt '}s characters set to ASCII
}{
   \sstdescription{
      The string STR1, encoded in the host machine{\tt '}s character set, is
   ...
   \sstarguments{
      \sstsubsection{
         ...
         A string represented by the host machine{\tt '}s character set.
      }
      \sstsubsection{
         STR2 = CHARACTER $*$ ( $*$ ) (Returned)
      }{
...
\sstroutine{
   ...
   Translate a string from the machine{\tt '}s character set to EBCDIC
}{
   \sstdescription{
...
\sstroutine{
   ...
   \sstdescription{
      The LOGICAL value is encoded into {\tt '}T{\tt '} or {\tt '}F{\tt '} which is then
...
\sstroutine{
   ...
   \sstdescription{
      ...
      If the absolute value of the number to be written exceeds
      a predefined maximum a conversion is not attempted, but the
      number is written as a real number in Fortran {\tt '}exponential{\tt '}
      format and a couple of question marks are appended to its
      end. This prevents silly results when very large numbers
      are input. The variable UNITS controls the maximum permitted
      value for the conversion to be carried out.
...
\sstroutine{
   ...
   \sstdescription{
      ...
      In the event of an error, {\tt '}$*${\tt '}s are written to the string.
   }
   \sstinvocation{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      There is also a literal escape character {\tt '}$\backslash${\tt '} for use when the
      characters {\tt '}$*${\tt '} and {\tt '}\%{\tt '} are to be interpreted literally within
