...
\vspace*{4ex} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Fix a nasty page break
...
{\bf AGI\_OPEN} is used in non-ADAM tasks.
\begin{verbatim}
...
There is one routine, {\bf AGI\_NUPIC}, that allows new pictures to be created
...
PGPLOT is shut down using {\bf AGP\_DEACT}.
...
The routine {\bf AGD\_NWIND} will return the size and offsets of a window that
...
Any programs requiring the AGI include files have to specify the path
...
\sstroutine{
   ...
   \sstdescription{
      ...
      AGI\_ASSOC, AGI\_BEGIN, AGD\_ACTIV and AGD\_NWIND. Also if the name
      string is not blank then AGI\_RCL is called to recall the last
...
\sstroutine{
   ...
   \sstdescription{
      ...
      AGD\_ASSOC was called is reinstated. This routine calls AGD\_DEACT,
      AGI\_END and either AGI\_CANCL or AGI\_ANNUL. This routine is
...
\sstroutine{
   ...
   \sstdescription{
      ...
      access (in AGI\_ASSOC or AGI\_OPEN) then the window will be cleared.
...
\sstroutine{
   ...
   \sstdescription{
      ...
      a call to AGI\_END. Up to eight levels of nested begin-end blocks
      are allowed.
   }
   \sstinvocation{
      CALL AGI\_BEGIN
...
\sstroutine{
   ...
   \sstdescription{
      ...
      current when the matching AGI\_BEGIN was called is made current.
...
\sstroutine{
   ...
   \sstdescription{
      ...
      this should be annulled in the application using REF\_ANNUL to
...
\sstroutine{
   ...
   \sstdescription{
      ...
      created with a call to AGI\_TNEW.
   }
   \sstinvocation{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      database is created. This routine calls AGI\_ASSOC, AGI\_BEGIN
      AGP\_ACTIV and AGP\_NVIEW. Also if the name string is not blank
      then AGI\_RCL is called to recall the last picture of that name.
      This routine should be matched by a closing call to AGP\_DEASS.
   }
   \sstinvocation{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      AGP\_ASSOC was called is reinstated. This routine calls AGP\_DEACT,
      AGI\_END and either AGI\_CANCL or AGI\_ANNUL. This routine is
      executed regardless of the given value of status.
   }
   \sstinvocation{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      cleared if the access mode is {\tt '}WRITE{\tt '} (in AGI\_ASSOC or AGI\_OPEN).
...
\sstroutine{
   ...
   \sstdescription{
      ...
      is created. This routine calls AGI\_ASSOC, AGI\_BEGIN, AGS\_ACTIV
      and AGS\_NZONE. Also if the name string is not blank then AGI\_RCL
      is called to recall the last picture of that name. This routine
      should be matched by a closing call to AGS\_DEASS.
   }
   \sstinvocation{
...
\sstroutine{
   ...
   \sstdescription{
      This is a wrap-up routine to deassociate a device from the AGI
      database and to close down SGS. The picture current when AGS\_ASSOC
      was called is reinstated. This routine call AGS\_DEACT, AGI\_END and
      either AGI\_CANCL or AGI\_ANNUL. This routine is executed regardless
...
\sstroutine{
   ...
   \sstdescription{
      ...
      will be cleared if the access mode is {\tt '}WRITE{\tt '} (in AGI\_ASSOC or
      AGI\_OPEN).
   }
   \sstinvocation{
