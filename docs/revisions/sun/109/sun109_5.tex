...
\newcommand{\stardocnumber}    {109.5}
\newcommand{\stardocauthors}   {Peter W. Draper and Nicholas Eaton}
\newcommand{\stardocdate}      {24 September 1992}
...
% The postscript file SUN109.PS is an encapsulated output from
...
\begin{center}
\begin {picture}(5,5)
\put(0,5.0){\special{include sun109.ps}}
\end {picture}
\end{center}
...
On VMS machines PISA can be run as either an ADAM monolith or as a
...
PISAFIND performs image analysis on a 2-dimensional data frame. It has
...
\subsection{Format of the results files}
...
PISAGEN generates model data using the PISA profile fitting function
...
\subsection{Simple use of PISA data in SCAR}
...
PISAKNN use the results of PISAPEAK to discriminate objects into two
...
PISAMATCH matches the first column of values (the object indices) of two
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \begin{description}
         \item [DATATYPE = FIND]~~~
           \begin{description}
            \item INDEX XPOS YPOS INTENSITY NPIX PEAK ELLIPT ANGLE SXX SYY SXY
           \end{description}
         \item [DATATYPE = SIZE] ~~~
           \begin{description}
            \item INDEX A1 A2 A3 A4 A5 A6 A7 A8
           \end{description}
         \item [DATATYPE = PEAK] ~~~
           \begin{description}
            \item INDEX RRATIO IRATIO ELLIP ABSSXY
           \end{description}
      \end{description}
   }
}
...
\sstroutine{
   ...
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{PISACUT INPUT=PISAPEAK.DAT COLUMN=2 THRESH=1.5
...
\newpage
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{PISAFIT FRAME FRAME.STARS\_ACC CANON\_PORTRAIT 3
                        NONE AGAIN=F RADIUS=15}
      }{
...
\sstroutine{
   ...
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{PISAGEN INPUT=FRAME POSITIONS=FRAME.STARS\_ACC
                        OUTPUT=STARS \\ BACKGROUND=100.0 SAT=32767
                        NOISE=NONE $\backslash$}
      }{
         Generates an output NDF STARS of the same size as FRAME. The
         objects are placed at the positions defined in
         FRAME.STARS\_ACC. The model parameterisations are those stored
         in the adam\_user:GLOBAL.SDF file, with the names PISA\_GSIGM,
...
\newpage
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{PISAPEAK IN=FRAME FINDDATA=PISAFIND.DAT
          RESULTS=PISAPEAK.DAT \\ BACKGROUND=255 THRESH=12 $\backslash$ }
      }{
      ...
      \sstexamplesubsection{
         \parbox{143mm}{PISAPEAK IN=FRAME FINDDATA=PISAFIND.DAT
                        RESULTS=PISAPEAK.DAT GSIGM=2.0 \\ CROSS=20 COMIX=0.25
                        BACKGROUND=adam\_user:pisa.pisafind.background \\
                        THRESH=adam\_user:pisa.pisafind.thresh }
      }{
         This example shows PISAPEAK using the objects found in NDF
         FRAME with parameters stored in file PISAFIND.DAT. The results
         are stored in file PISAPEAK.DAT.  The background value and
         threshold are those used by PISAFIND in its last run.
      }
   }
   \sstnotes{
      \sstitemlist{
...
PISA has only undergone minor changes this release, the main ones are.
\begin{itemize}
