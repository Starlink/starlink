...
Assuming that \verb%$ ADAMSTART% has been obeyed to start ADAM, type:
\small \begin{quote} \begin{verbatim}
$ ADAM_DEV
\end{verbatim} \end{quote} \normalsize
This will define logical names for include files such as SAE\_PAR and ACT\_ERR
and display the message
``\verb%+ logged in for ADAM program development%''.
...
/DEBUG).
\item[*] (optional) specify \verb%*% if it is required to link with ADAM object
libraries rather than the shareable images. Note that the full ADAM release
must be installed and \verb%$ @ADAM_SYS:SYSDEV% obeyed for this option to
work. If \verb%*% is specified, \verb%qual% must also be specified (use
