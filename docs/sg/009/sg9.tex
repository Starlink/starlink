...
\stardocdate        {11 December 1996}
\stardoctitle       {Introduction to Echelle Spectroscopy}
%\stardocversion     {[software-version]}
%\stardocmanual      {[manual-type]}
\stardocname  {\stardocinitials /\stardocnumber}

% -----------------------------------------------------------------------------
%  Document specific \providecommand or \newenvironment commands.
%
\providecommand{\echdiagobjectbox}[3]{
    \put(#1,#2){\oval(2.0, 0.5)}
    \put(#1,#2){\makebox(0,0){\shortstack{#3}}}
    }
%
\providecommand{\echdiagactionbox}[3]{
    \put(#1,#2){\framebox(2.0, 0.5){\shortstack{#3}}}
    }
%
\providecommand{\echdiagarmtext}[3]{
    \put(#1,#2){\makebox(0,0){\shortstack{#3}}}
    }
%
...
\htmlref{\textbf{Echelle}}{gl_echelle} \htmlref{\textbf{spectrographs}}{gl_spectrograph} are available as
...
\htmlref{\textbf{resolution}}{gl_resolution} and
...
is the preliminary handling of the \htmlref{\textbf{CCD}}{gl_ccd} data frames.
...
details.}}{\emph{(All IRAF-related hyperlinks in this document are to the
...
\item \sgspec{\texttt{http://www.star.ucl.ac.uk/\symbol{126}mjc/echelle/}}
      {\htmladdnormallink{\verb+http://www.star.ucl.ac.uk/~mjc/echelle/+}
      {http://www.star.ucl.ac.uk/\~{}mjc/echelle/}}

\end{itemize}

Terms in the body text of this Guide in \textbf{bold} are described in the
...
\item \sgspec{\texttt{http://www.sti.nasa.gov/nasa-thesaurus.html}}
...
\htmlref{\textbf{diffraction grating}}{gl_grating} or
\htmlref{\textbf{prism}}{gl_prism}\sgspec{---}{ - }is used to produce the spectrum.
This results in a single spectrum which can be imaged using a
\htmlref{\textbf{CCD}}{gl_ccd} or other
...
This leads to spectra of very high \htmlref{\textbf{dispersion}}{gl_dispersion},
but only over a short wavelength range in each order.
As well as being `short', the high orders will overlap.  To overcome
this effect a \htmlref{\textbf{cross-dispersing}}{gl_cross_dispersion}
element is used to produce an
\htmlref{\textbf{order separation}}{gl_order_separation}.
...
absorption features.  Several \htmlref{\textbf{cosmic-ray events}}{gl_cosmic_ray}
(bright spots) are also visible.

\begin{figure}
\begin{center}
\sgspec{\leavevmode\includegraphics[height=105mm]{sg9_04}}
{\leavevmode\includegraphics[height=136mm]{sg9_04}}
...
\item The high \htmlref{\textbf{dispersion}}{gl_dispersion} used can make
      it difficult to distinguish
      between true spectral features and cosmic-ray events.

\item \htmlref{\textbf{Flat fielding}}{gl_flat_field} the data can be difficult.
...
\item [\textbf{Arc frames}]
      Exposures taken with an \htmlref{\textbf{arc lamp}}{gl_arc}
...
A more complete introduction to the handling of \htmlref{\textbf{CCD}}{gl_ccd}
...
\htmlref{\textbf{bias}}{gl_bias_frame}, \htmlref{\textbf{dark}}{gl_dark_frame},
and
\htmlref{\textbf{flat-field}}{gl_flat_field} frames should be obtained.
It is important to \emph{\htmlref{\textbf{bracket}}{gl_bracketing}} the science data exposures with
...
\htmlref{\textbf{bias frames}}{gl_bias_frame}.  Instead,
a median value for the bias level is obtained by inspecting the
\htmlref{\textbf{overscan region}}{gl_overscan} in some or all of the object/arc
frames.
...
\htmlref{\textbf{dark current}}{gl_dark_current}
...
spectrograph ({\em{e.g.}} \htmlref{\textbf{UCLES}}{gl_ucles} which is in the
\htmlref{\textbf{AAT}}{gl_aao_aat} coud\'{e} room)
...
\htmlref{\textbf{dekker}}{gl_dekker} size (length)
...
\htmlref{\textbf{flat-field}}{gl_flat_field} is available across the complete
...
spectrum of the \htmlref{\textbf{arc lamp}}{gl_arc}
...
A \htmlref{\textbf{flat-field frame}}{gl_flat_field} can be used for tracing as
this is likely to give a good signal.
Therefore, if necessary, obtain a few flat fields with a narrow
\htmlref{\textbf{dekker}}{gl_dekker} to improve the traces.
...
(arc) frames should be taken \htmlref{\textbf{bracketing}}{gl_bracketing} the
science exposures \emph{if} precise wavelength scales are required.
At the high \htmlref{\textbf{dispersions}}{gl_dispersion}
...
now starting to become available (notably for \htmlref{\textbf{HST}}{gl_hst})\@.
...
\item [\textbf{Generate \htmlref{bias frame}{gl_bias_frame}}]
      Typically done by finding the median of several frames taken at
      the telescope.

\item [\textbf{Generate \htmlref{flat-field frame}{gl_flat_field}}]
...
\item [\textbf{Subtract bias frame}]
      The median bias frame is subtracted from each arc frame and
      each object frame.

\item [\textbf{Subtract zero-offset}]
      A selected area of the CCD \htmlref{\textbf{overscan region}}{gl_overscan}
      is used to find the
      `zero-level' of the camera electronics.  This is usually a number
      in the range 20\sgspec{--}{-}200 \htmlref{\textbf{ADU}}{gl_adu}\@.
...
\item [\textbf{Crop images}]
      ...
      \emph{i.e.} non-useable data, which should also be trimmed off at
      this point.
...
\'{e}chelle reduction software you choose, \emph{e.g.}, \texttt{noao.imred.ccdred} for IRAF \texttt{doecslit}\@.  There are two popular
...
This example is a section across an \htmlref{\textbf{IUE}}{gl_iue} \'{echelle}
image.
...
\begin{figure}
\begin{center}
\sgspec{\leavevmode\includegraphics[height=105mm]{sg9_01}}
{\leavevmode\includegraphics[height=136mm]{sg9_01}}
...
\htmlref{\textbf{slit}}{gl_slit}
is projected onto the final image.  The physical length of the slit
is determined by the \htmlref{\textbf{dekker}}{gl_dekker}.
In a \htmlref{\textbf{flat-field frame}}{gl_flat_field} the entire slit should
...
a \htmlref{\textbf{cross-dispersion}}{gl_cross_dispersion} profile of each
order in the frame.
...
flat-field frames (respectively, \texttt{noao.imred.echelle.apflatten} and
...
phase.  The signal due to \htmlref{\textbf{dark current}}{gl_dark_current} is
...
short-wavelength orders (left-hand side) of an \htmlref{\textbf{IUE}}{gl_iue}
...
\htmlref{\textbf{readout noise}}{gl_readout_noise} details).
...
extraction method you will need to know the \htmlref{\textbf{readout noise}}
{gl_readout_noise} and \htmlref{\textbf{gain}}{gl_ccd_gain} for the CCD
...
\begin{figure}
\begin{center}
\sgspec{\leavevmode\includegraphics[height=105mm]{sg9_03}}
{\leavevmode\includegraphics[height=136mm]{sg9_03}}
...
\htmlref{\textbf{comparison spectrum}}{gl_comparison} for your selected
\htmlref{\textbf{arc lamp}}{gl_arc},  perhaps obtained using
the same spectrograph as your data.  For example, {\sl
\htmlref{\textbf{UCLES}}{gl_ucles} Spectrum
of the Thorium-Argon Hollow-Cathode Lamp} should be available at most UK
Starlink sites (a \htmlref{\textbf{UES}}{gl_ues} version is also available).
...
useful to calibrate \emph{two} arcs which \htmlref{\textbf{bracket}}{gl_bracketing}
...
\htmlref{\textbf{blaze function}}{gl_blaze_correction} (a
\htmlref{\textbf{grating}}{gl_grating}-dependent variation in the brightness along
...
\begin{figure}
\begin{center}
\sgspec{\leavevmode\includegraphics[height=105mm]{sg9_05}}
{\leavevmode\includegraphics[height=136mm]{sg9_05}}
...
An alternative to applying the \htmlref{\textbf{blaze correction}}
...
\htmlref{\textbf{blaze-corrected}}{gl_blaze_correction} and scrunched
...
to overcome dropouts due to \htmlref{\textbf{cosmic-ray hits}}{cosmic_ray}
\emph{etc.}
...
coverage of orders overlaps; \emph{e.g.}, ignore data where one order is
much fainter than the other, use flux-weighted mean \emph{etc.}
...
\htmlref{\textbf{cosmic-ray hits}}{gl_cosmic_ray} in the data.
There are several strategies for the detection of cosmic rays, here are
some of them:

\begin{description}

\item [\textbf{Inspection by eye}]
      ...
      for object frames than, say, \htmlref{\textbf{dark frames}}{gl_dark_frame}
...
\item [\textbf{Profile Modelling}]
      ...
      unexpected\sgspec{---}{ - }\emph{i.e.}, statistically unlikely
...
\item [\textbf{Comparison of `Identical' frames}]
...
You should ensure that cosmic rays in your CCD \htmlref{\textbf{bias}}
{gl_bias_frame}, \htmlref{\textbf{dark}}{gl_dark_frame}, and
\htmlref{\textbf{flat-field}}{gl_flat_field} frames are removed prior to
...
the \htmlref{\textbf{AAT}}{gl_aao_aat} coud\'{e} \'{e}chelle spectrograph
\htmlref{\textbf{UCLES}}{gl_ucles}\@.
The author made the package sufficiently flexible that it can be used for
reduction of data from other instruments.

Part of the package is an interactive menu interface, \texttt{echmenu}, which
...
reduction.  CCD-related processing is \emph{not} included and must be done
using a suitable package ({\em{e.g.}}, \xref{CCDPACK}{sun139}{} or
\xref{FIGARO}{sun86}{})\@.  \texttt{echmenu} guides you
...
Internal data are held in a \emph{reduction structure} which is an
...
\texttt{noao.imred.echelle}, the extraction being carried out by the main task
\texttt{doecslit}\@.  This is a `mature' software package in that it has not
...
easy to use for those familiar with the IRAF shell \texttt{cl}, parameter
entry and editing, and IRAF image file handling.

An IRAF task similar to \texttt{doecslit}, \texttt{dofibers}, with
instrument-specific variants is available.  This task is quite similar to
\texttt{doecslit} except that the aperture for each object must be
...
using tasks in \texttt{noao.imred.ccdred}\@.  These tasks are also used in the
flat-field generation process.  \texttt{doecslit}, which is an IRAF script,
...
\'{e}chellogram into a single spectrum, the \texttt{scombine} task provides
...
most UK astronomers) using the \texttt{observatory} task.
Suitable extinction data are also required.

Data processing with \texttt{doecslit} can be interrupted and restarted
...
The \texttt{echelle} package has a full documentation set in the same style as
other IRAF packages.  On-line help describing the tasks and the parameters
for each are available and accessible from the IRAF \texttt{cl} shell.
...
\texttt{doecslit} usage and parameters are documented in {\sl Guide to Slit
Spectra Reduction Task DOECSLIT}\@.  This is the paper reference document
for the task.

IRAF is supported by a team at NOAO\@.

\texttt{noao.imred.echelle}, supports two extraction weighting schemes:
...
\item {\bf{Variance.}} \mbox{}\\
      An alternative name for the \emph{optimal} extraction weighting
      method.
...
\htmlref{\textbf{ESO}}{gl_eso} MIDAS provides a suite of commands and
options for the reduction of data from \'{e}chelle instruments.
The software is contained within the MIDAS context ECHELLE\@.

In a similar manner to the IRAF tasks in \texttt{noao.imred.echelle},
the MIDAS ECHELLE context inherits command and parameter style from
the host environment.

Like \texttt{noao.imred.echelle}, a full range of facilities for \'{e}chelle
...
\htmlref{\textbf{FITS}}{gl_fits} or \htmlref{\textbf{IHAP}}{gl_ihap} formats only.
...
factors to be considered with basic functionality \emph{etc.}
...
\begin{center}
{\sl
IRAF commands are from the \texttt{noao.imred.echelle} package unless otherwise
stated.

Starlink commands are part of ECHOMOP unless otherwise stated.

All MIDAS commands are accessible once \texttt{set/context echelle} has been done.
}
\vspace*{11pt}

\begin{tabular}{llll}
\hline\hline
\textbf{Task}         & IRAF                   & STARLINK             & MIDAS                     \\
\hline\hline
Rotate image       & \texttt{images.rotate   } & \texttt{FIGARO IROT90 } & \texttt{ROTATE/ECHELLE     } \\
                   &                        & \texttt{KAPPA ROTATE  } &                           \\
Subtract constant  & \texttt{images.imarith  } & \texttt{FIGARO ICSUB  } & \texttt{COMPUTE/IMAGE      } \\
from image         &                        & \texttt{KAPPA CSUB    } &                           \\
Divide images      & \texttt{images.imdivide } & \texttt{FIGARO IDIV   } & \texttt{COMPUTE/IMAGE      } \\
pixel-by-pixel     &                        & \texttt{KAPPA DIV     } &                           \\
Generate median of & \texttt{noao.imred.     } & \texttt{FIGARO MEDSKY } & \texttt{AVERAGE/IMAGES     } \\
several images     & \texttt{ccdred.combine  } &                      &                           \\
Order location     & \texttt{apfind          } & \texttt{ech\_trace    } & \texttt{SEARCH/ORDER       } \\
Order tracing      & \texttt{aptrace         } & \texttt{ech\_trace    } & \texttt{DEFINE/ECHELLE     } \\
                   &                        & \texttt{ech\_fitord   } & \texttt{DEFINE/HOUGH       } \\
Slit definition    & \texttt{apdefault       } & \texttt{ech\_spatial  } & \texttt{DEFINE/HOUGH       } \\
                   & \texttt{apedit          } & \texttt{ech\_profile  } &                           \\
Normalise          & \texttt{apnormal        } & \texttt{ech\_ffield   } & \texttt{FLAT/ECHELLE $+$   } \\
flat field         & \texttt{apflatten       } &                      & \texttt{COMPUTE/IMAGE      } \\
Model scattered    & \texttt{apscatter       } & \texttt{ech\_mdlbck   } & \texttt{BACKGROUND/ECHELLE } \\
light              &                        &                      &                           \\
Model per-order    &                        & \texttt{ech\_sky      } & \texttt{DEFINE/SKY $+$     } \\
`sky' background   &                        &                      & \texttt{EXTRACT/SKY        } \\
Extract            & \texttt{apsum           } & \texttt{ech\_extrct   } & \texttt{EXTRACT/ECHELLE    } \\
Locate arc lines   & \texttt{ecidentify      } & \texttt{ech\_linloc   } & \texttt{SEARCH/ECHELLE     } \\
Fit wavelength     & \texttt{ecidentify      } & \texttt{ech\_idwave   } & \texttt{IDENTIFY/ECHELLE   } \\
scales             &                        &                      &                           \\
Blaze correct      & \texttt{continuum       } & \texttt{ech\_blaze    } & \texttt{RIPPLE/ECHELLE     } \\
flux calibrate     & \texttt{calibrate       } & \texttt{FIGARO SPFLUX } & \texttt{RESPONSE/ECHELLE   } \\
Scrunch            &                        & \texttt{ech\_scrunch  } & \texttt{REBIN/ECHELLE      } \\
Merge              & \texttt{scombine        } & \texttt{ech\_mulmrg   } & \texttt{MERGRE/ECHELLE     } \\
\hline\hline
\end{tabular}
\end{center}
\end{table}
...
If you are already an expert at the IRAF \texttt{cl} then you may get
...
\htmlref{\textbf{FITS}}{gl_fits} format.
...
have to interpolate across any cosmic rays \emph{etc.}, prior to
...
\item \textbf{\label{gl_bracketing}Bracketing}\\
...
\item \textbf{\label{gl_centroiding}Centroiding}\\
...
\item \textbf{\label{gl_cross_dispersion}Cross-dispersion}\\
...
\item \textbf{\label{gl_dispersion}Dispersion}\\
...
\item \textbf{\label{gl_echelle}Echelle}\\
      Literally, from the French, {\sl Ladder.}
      A \htmlref{\textbf{grating}}{gl_grating} in which the
...
\item \textbf{\label{gl_fsr}Free Spectral Range (FSR)}\\
...
\item \textbf{\label{gl_ccd_gain}Gain, Output transfer function}\\
      The number of \htmlref{\textbf{ADU}}{gl_adu}
      counts per electron ({\it{i.e.}} per photon)
      in the output signal from a CCD camera.

\item \textbf{\label{gl_grating}Grating, diffraction grating}\\
...
\item \textbf{\label{gl_ghrs}GHRS}\\
      Goddard High-Resolution spectrograph.  An instrument on the
      Hubble Space Telescope.

\item \textbf{\label{gl_halation}Halation}\\
...
\item \textbf{\label{gl_hst}HST}\\
      Hubble Space Telescope.

\item \textbf{\label{gl_ids}IDS}\\
      Intermediate Dispersion Spectrograph.  An instrument at the
      ING\@.

\item \textbf{\label{gl_ihap}IHAP}\\
...
\item \textbf{\label{gl_ing}ING}\\
      The Isaac Newton Group of telescopes at the RGO on La Palma.

\item \textbf{\label{gl_int}INT}\\
      Isaac Newton Telescope at the La Palma Observatory.

\item \textbf{\label{gl_ipcs}IPCS}\\
      Image Photon Counting System.  A common optical image sensor,
      has zero \htmlref{\textbf{readout noise}}{gl_readout_noise} and good blue
      response.

\item \textbf{\label{gl_iraf}IRAF}\\
      Image Reduction and Analysis Facility.

\item \textbf{\label{gl_isis}ISIS}\\
...
\item \textbf{\label{gl_iue}IUE}\\
      International Ultraviolet Explorer.

\item \textbf{\label{gl_jkt}JKT}\\
      Jacobus Kapteyn Telescope at the La Palma Observatory.

\item \textbf{\label{gl_kappa}KAPPA}\\
...
\item \textbf{\label{gl_midas}MIDAS}\\
...
\item \textbf{\label{gl_noao}NOAO}\\
      National Optical Astronomical Observatories.

\item \textbf{\label{gl_order_separation}Order separation}\\
...
\item \textbf{\label{gl_overscan}Overscan, overscan region}\\
...
\item \textbf{\label{gl_periscopes}Periscope(s)}\\
...
\item \textbf{\label{gl_prism}Prism}\\
...
\item \textbf{\label{gl_qe}Quantum Efficiency, QE}\\
...
\item \textbf{\label{gl_scanning}Scan, scanning}\\
...
\item \textbf{\label{gl_spectrograph}Spectrograph}\\
...
\item \textbf{\label{gl_template_reduct}Template, reduction}\\
...
\item \textbf{\label{gl_throughput}Throughput}\\
...
\bibitem{echref} Michael Bessell and Max Pettini,\\
      ...
      \htmlref{\textbf{AAO}}{gl_aao_aat} UM 28.1, January 1991.
...
\bibitem{detut} Francisco Valdes,\\
      ...
      {http://www.starlink.ac.uk/iraf/web/tutorials/doecslit/doecslit.html},}\\
      \texttt{http://www.starlink.ac.uk/iraf/web/tutorials/doecslit/doecslit.html},
      \\
      April 1994.
