...
\stardocdate        {29 November 1993}
\stardoctitle     {An Astronomer's Guide to\\[1ex]
...
\section{Overview\xlabel{overview_bib}}
...
\textit{per annum}. For each article full bibliographic details, but not
...
\textbf{Astronomy}                       & \textbf{89}  \\
Solar Astronomy                       & 89-01     \\
Stellar Astronomy and Cosmology       & 89-02     \\
Meteors and Meteorites                & 89-03     \\
\textbf{Astrophysics}                    & \textbf{90}  \\
Gravitation                           & 90-01     \\
Astrophysical Plasmas                 & 90-02     \\
\textbf{Lunar and Planetary Exploration} & \textbf{91}  \\
The Moon                              & 91-01     \\
Planetary Sciences and Exploration    & 91-02     \\
\textbf{Solar Physics}                   & \textbf{92}  \\
\textbf{Space Radiation}                 & \textbf{93}  \\
...
J.L. Stoner, 1993, in \textit{Astronomical Data Analysis Software and
...
Once you are connected, the relevant directories are \texttt{listings}
which contains abstracts and \texttt{papers} which contains the full text
...
\textbf{astronomy, astrophysics and cosmology}  & \textbf{astro-ph@babbage.sissa.it} \\
...
you should obtain a copy of the source file \texttt{stsci-preprint-db.src}.
...
The list can be queried with WAIS (see Section~\ref{WAIS}), though first
you should obtain a copy of the source file \texttt{nrao-raps.src}.
A copy of this file can be obtained from the European Space Telescope
Coordinating Facility (again see Section~\ref{WAIS}, especially
Table~\ref{WAIS_SOURCE}).
...
\subsection{STScI\xlabel{stsci_toc}}
...
Finally, it is worth remembering that the printed \textit{Astronomy and
...
\section{Overview\xlabel{overview_is}}
...
\caption{\texttt{Telnet} resources available through STEIS}
\label{AIR_TEL}
...
\begin{itemize}
  ...
  \vspace{2.0 mm}
  \texttt{subscribe ~} \textit{filename} ~~ \textit{your-name}
  \vspace{2.0 mm}

   where \textit{filename}\, is one of the filenames listed in
   Table~\ref{STEIS_FILES} and \textit{your-name} is your full name.

  \item To `unsubscribe' to file \textit{filename:}

  \vspace{2.0 mm}
  \texttt{unsubscribe ~} \textit{filename}
  \vspace{2.0 mm}

  \item For a summary of the commands available: \texttt{HELP}
...
\begin{itemize}
  ...
  \item the electronic journal \textit{Lunar and Planetary Information
   Bulletin}.
...
Currently only \textit{The Times}\, newspaper is available. The text for
...
of the \textit{The Sunday Times}\, are added on the Wednesday following
...
(\textbf{e-MATH}). You have to register to use some of these databases.
...
However, WAIS is accessed using the \texttt{swais} client which is
...
The \textit{Internet Resource Guide} will continue to be available for some
...
E.D. Feigelson and F. Murtagh (1992, \textit{PASP}, \textbf{104}, pp574-581).

Various public domain software archives may be accessed through the
\texttt{Archive Services} menu of NISS (see Section~\ref{NISS}). Archie
...
\section{Overview\xlabel{overview_internet}}
...
are given in \textit{The Whole Internet User's Guide and Catalog}\, and
\textit{Intelligent Information Retrieval: The Case of Astronomy and
...
\begin{description}
  ...
  \item[\texttt{cd} \textit{directory}] -- select directory \textit{directory}\,
  ...
  \item[\texttt{dir}] -- list the files in the current directory.

  \item[\texttt{get} \textit{filename}] -- retrieve a copy of file \textit{filename}\, to your local computer. ftp should be in ASCII mode when
   retrieving text files and in binary mode when retrieving binary
   files (see commands \texttt{ascii} and \texttt{binary}). A description of
   compressed and archived (`tar') files is beyond the scope of this
   document; see Chapter Six of \textit{The Whole Internet User's Guide
  ...
  \item[\texttt{pwd}] -- display the current directory.

  \item[\texttt{ascii}] -- switch to ASCII mode; ASCII is the default mode.

  \item[\texttt{binary}] -- switch to binary mode.

  \item[\texttt{help}] -- will often display information about the commands
   available.

  \item[\texttt{quit}] -- exits ftp.
...
\begin{verse}
\texttt{get} \textit{filename}\, \texttt{"$|$ more"}
\end{verse}

\textit{Documentation:}

ftp is described in Chapter Six of \textit{The Whole Internet
...
\textbf{United Kingdom} & \textbf{archie.doc.ic.ac.uk}  & \textbf{146.169.11.3}\\
...
made available using anonymous ftp or \texttt{telnet}, but it does make
...
In response to the `login:' prompt reply \texttt{gopher}; no password is
required.
...
\textbf{Europe} & \textbf{gopher.sunet.se} & \textbf{192.36.125.2} & \textbf{gopher} \\
...
entry called something like \texttt{Other Gopher and Information Servers}.
...
Gopher is described in Chapter Eleven of \textit{The Whole Internet
User's Guide and Catalog}\, (see Section~\ref{FURTHER}).

It is also described by F. Anklesaria and M. McCahill in Chapter Nine
of \textit{Intelligent Information Retrieval: The Case of Astronomy and
Related Space Sciences}\, (see Section~\ref{FURTHER}).
...
`not'\footnote{Standard WAIS clients such as \texttt{xwais} do not support
...
\nopagebreak
WAIS is described in Chapter Twelve of \textit{The Whole Internet
User's Guide and Catalog}\, (see Section~\ref{FURTHER}).

It is also described by J. Fullton in Chapter Eight of \textit{Intelligent
Information Retrieval: The Case of Astronomy and Related Space
Sciences}\, (see Section~\ref{FURTHER}).
...
The Web is described in Chapter Thirteen of \textit{The Whole Internet
User's Guide and Catalog}\, (see Section~\ref{FURTHER}).

It is also described by B. White in Chapter Ten of \textit{Intelligent
Information Retrieval: The Case of Astronomy and Related Space
Sciences}\, (see Section~\ref{FURTHER}).

A list of frequently asked questions (FAQs in INTERNET jargon) about
the Web is available by anonymous ftp from STEIS (see
Section~\ref{STEIS_FTP}). It is held as file \texttt{.WWW-FAQ} in the
top level directory. Note that because the file name begins with a dot
it will not appear in normal Unix directory listings.

\textit{Getting Started with NCSA Mosaic}\, by M. Andreessen, May 1993 is
an introductory article about the \texttt{xmosaic} client. However, it is
...
\textit{Hypertext and Hypermedia} by J. Nielsen, 1990 (Academic Press,
San Francisco). This book is about hypertext in general, rather than
being specifically about the Web.

The issue of \textit{Communications of the ACM}\, for July 1998 contained a
...
\begin{description}
  ...
  \item[Gopher] represents each item of information as either a menu,
   a document, an index or a \texttt{telnet} connection.
...
H. -M. Adorf's paper in \textit{The ESO Messenger}. Dr.~B.G.~Marsden
kindly supplied useful information about the IAU Circulars. Ed Krol's
\textit{The Whole Internet User's Guide and Catalog}\, explained the
