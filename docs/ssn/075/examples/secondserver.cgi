...
#     secondserver.cgi
#  Purpose:
#     Simple example HTTP server with query and error logging.
#  Language:
#     Perl 5 / CGI script.
#  Description:
...
#
#     Queries received and error messages are written to log files.
#  Algorithm:
...
#
#  Edit the following lines so that queryExe is set to wherever
#  program genfield has been installed and logDir is set to the
...
#
#  Append an entry for the query to the log file.
...
#
# Append an entry corresponding to the query to the log file.
...
{  if ($status == 0)
   {  $date = `date`;
      ...
      $openStatus = open (LOG, ">>$logDir/query.LOG");
      if ($openStatus != 0)
      {  print LOG  "$\'$date\'  ";
      ...
      {  $status = 1;
         $statusMessage = "Failed to open the log file.";
      }
   }
   $status;
}
...
#
# Append an entry the error log file.
#
# Note that this routine attempts to execute irrespective of the running
# status.

sub AppendToErrorLog
{  $date = `date`;
   chop($date);

   $openStatus = open (ERRLOG, ">>$logDir/error.LOG");
   if ($openStatus != 0)
   {  print ERRLOG  "$\'$date\'  ";

      print ERRLOG  "$query ";
      print ERRLOG  "$statusMessage\n";

      close (ERRLOG);
   }
   else
   {  print "Failed to open the error log file.";
   }
}
