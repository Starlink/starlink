...
/* If a child process has terminated (but not if it has simply
   ...
   and store the associated status information. */
         if ( ( pid > (pid_t) 0 ) && !WIFSTOPPED( stat_val ) ) {
            ...
            task->terminated = 1;
#if PRINT
            (void) printf( "HandleSIGCHLD: %s process %ld terminated%s.\n",
...
/* If we reach the end of the list without obtaining any status
   ...
   handler, as if we hadn't interfered. */
      if ( task == active_list ) {
#if PRINT
      (void) printf( "HandleSIGCHLD: Unrecognised process terminated; passing "
...
#if PRINT
   (void) printf( "RemoveTask: Removing %s process %ld from active task "
                  "list.\n",
                  task->detached ? "detached" : "attached",
...
#if PRINT
         (void) printf( "Kill: Killing %s process %ld with signal %s.\n",
                        task->detached ? "detached" : "attached",
                        (unsigned long) pid,
...
#if PRINT
         (void) printf( "WaitK: Waiting for %s process %ld to terminate.\n",
                        task->detached ? "detached" : "attached",
                        (unsigned long) pid );
#endif
         while ( 1 ) {
...
/* If the wait succeeded, remove the task from the active task list. */
         if ( *status == SAI__OK ) {
#if PRINT
            (void) printf( "WaitK: %s process %ld terminated after %g "
                           "seconds.\n",
                           task->detached ? "Detached" : "Attached",
...
/* Note initialisation has been done. */
      init = 1;
#if PRINT
      (void) printf( "InitClosedown: Task termination handling "
                     "initialised.\n" );
#endif
   }
}
...
/* Report any error. */
         if ( *status != SAI__OK ) {
            ...
            (void) printf( "InitAMS: Initialised message system with master "
                           "task name \"%s\".\n", master );
#endif
         }
#if PRINT
      } else {                   /* ICL_TASK_NAME is set */
         (void) printf( "InitAMS: Master task name defined by "
                         "\"ICL_TASK_NAME=%s\".\n",
...
/* If OK, try to establish a path to the slave task, to see if it is
   ...
   if ( *status == SAI__OK ) {
      ...
      if ( loaded ) {
         pid = (pid_t) 0;
#if PRINT
         (void) printf( "LoadW: Task \"%s\" is already loaded.\n", name );
      } else {
         (void) printf( "LoadW: Task \"%s\" is not already loaded.\n", name );
#endif
      }
   }
   ...
   those we are going to set ourselves. */
         for ( nn = n = 0; environ[ n ]; n++ ) {
            if ( strncmp( environ[ n ], "ICL_TASK_NAME=", 14 ) &&
                 strncmp( environ[ n ], "ICL_TASK_TYPE=", 14 ) ) nn++;
         }
...
/* If OK, copy the pointers to the existing environment strings to the
   new array, again omitting the variables we want to set. */
            ...
            for ( nn = n = 0; environ[ n ]; n++ ) {
               if ( strncmp( environ[ n ], "ICL_TASK_NAME=", 14 ) &&
                    strncmp( environ[ n ], "ICL_TASK_TYPE=", 14 ) ) {
...
/* Loop while the child process starts, waiting to establish a message
   path to it. */
#if PRINT
               (void) printf( "LoadW: Waiting for %s task \"%s\" "
...
/* Attempt to obtain an ADAM message system path to the slave process
   and quit looping if successful. */
                  ...
                  if ( istat == SAI__OK ) {
                     task->connected = 1;
#if PRINT
                     (void) printf( "LoadW: Message connection to %s task "
...
/* If not successful, block SIGCHLD signals and then check if the
   ...
   connection. Report an appropriate error message if it has. */
                  if ( task->terminated ) {
#if PRINT
                     (void) printf( "LoadW: %s task \"%s\" (process %d) died "
...
/* Check how long we have been waiting to make the connection. If this
   exceeds the timeout period, then report an error. */
                  ...
                  if ( ( timeout > 0 ) && ( delay > (double) timeout ) ) {
#if PRINT
                     (void) printf( "LoadW: %s task \"%s\" (process %ld) "
...
/* If there are terminated tasks that have not been waited for
   (zombies), we must remove their entries from the list. */
      if ( task->terminated ) {
#if PRINT
         (void) printf( "Closedown: %s process %ld is a zombie slave.\n",
                        task->detached ? "Detached" : "Attached",
                        (unsigned long) task->pid );
#endif
...
#if PRINT
   (void) printf( "Closedown: %d active task(s) must be closed down.\n",
                  nkill );
#endif
...
/* Loop to kill each task. */
   for ( i = 0; i < nkill; i++ ) {
#if PRINT
         (void) printf( "Closedown: Closing down attached process %ld.\n",
                        (unsigned long) task->pid );
#endif
      Kill( pid[ i ], &status );
   }
...
#if PRINT
   (void) printf( "ErrFlush: Flushing error messages...\n" );
#endif
...
#if PRINT
   (void) printf( "ErrFlush: Error messages flushed.\n" );
#endif
...
/* If there is no association with a master parameter and this is the
   ...
   using the "accept" keyword). */
            if ( !master ) {
#if PRINT
               (void) printf( "HandleParamReq: Slave parameter \"%s\" "
...
#if PRINT
                  (void) printf( "HandleParamReq: Cancelling previous "
...
#if PRINT
                  (void) printf( "HandleParamReq: Setting a dynamic default "
...
#if PRINT
                  (void) printf( "HandleParamReq: Flushing pending error "
...
/* Mark the error stack and attempt to obtain a value for the master
   parameter. */
               if ( *status == SAI__OK ) {
#if PRINT
                  (void) printf( "HandleParamReq: Requesting a value for the "
                                 "master \"%s\" parameter.\n", master );
#endif
                  ems_mark_c();
                  STATUS = *status;
...
/* Classify the returned status. If it indicates a "null" state for
   ...
   was available. */
                  if ( *status == PAR__NULL ) {
#if PRINT
                     (void) printf( "HandleParamReq: Master parameter state "
                                    "is \"null\".\n" );
#endif
                     ems_annul_c( status );
                     parvalue = "!";
...
/* If there has been no error, send the parameter value to the slave
   task. */
      if ( *status == SAI__OK ) {
#if PRINT
         (void) printf( "HandleParamReq: Sending parameter reply \"%s=%s\" "
                        "to slave task.\n", parname, parvalue );
#endif
...
#if PRINT
   (void) printf( "ObeyW: Sent action request \"%s%s%s\" to task \"%s\".\n",
...
/* Detect confirmation of the action from the task. */
...
#if PRINT
         {
            ...
            (void) printf( "ObeyW: Action \"%.*s%s%.*s\" confirmed by "
                           "task \"%s\".\n",
...
/* Detect successful completion of the action by the task. */
...
#if PRINT
         {
            int len = msg_length;
            while ( len >= 0 && ( msg_value[ len - 1 ] == ' ' ) ) len--;
            (void) printf( "ObeyW: Action \"%.*s %.*s\" completed by "
                           "task \"%s\".\n",
...
#if PRINT
         (void) printf( "ObeyW: Request for parameter \"%.*s\" from task "
...
F77_INTEGER_FUNCTION(slv_obeyw)( CHARACTER(TASK),
   ...
   if ( status != SAI__OK ) {
      ems_setc_c( "ACTION", action );
      ems_setc_c( "TASK", task );
