...
\stardocnumber      {2.8}
\stardocauthors     {D.S.\ Berry \\ M.B.\ Taylor}
\stardocdate        {11th October 2012}
\stardoctitle       {NDG \\ [\latex{1ex}]
...
\xref{SUN/150}{sun150}{}), which contains the names of a group of \emph{existing} NDFs to be used as inputs by the application\footnote{The routine
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         are flushed so that the user can see the details (\texttt{"}verbose\texttt{"} mode).
         Otherwise, they are annulled and a general \texttt{"}Cannot access file xyz\texttt{"}
      ...
      \sstsubsection{
         ...
         If the group expression was terminated by the GRP \texttt{"}flag
         character\texttt{"}, then FLAG is returned .TRUE. Otherwise it is
   ...
   \sstnotes{
      \sstitemlist{
         ...
         substitution and pipes - but pipe characters \texttt{"}$|$\texttt{"} need to be escaped
         using a backslash \texttt{"}$\backslash$\texttt{"} to avoid them being interpreted as GRP
         editing characters).
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         a new NDF (\texttt{"}verbose\texttt{"} mode). Otherwise, they are annulled and
         a general \texttt{"}Cannot access file xyz\texttt{"} message is displayed before
         re-prompting.
      }
      \sstsubsection{
         MODE = CHARACTER $*$ ( $*$ ) (Given)
      }{
         Type of NDF access required: \texttt{'}READ\texttt{'}, \texttt{'}UPDATE\texttt{'} or \texttt{'}WRITE\texttt{'}.
      }
      \sstsubsection{
...
\sstroutine{
   ...
   \sstnotes{
	 ...
	 and pipes - but pipe characters \texttt{"}$|$\texttt{"} need to be
	 escaped using a backslash \texttt{"}$\backslash$\texttt{"} to avoid
	 ...
	 basenames \texttt{"}fred1\texttt{"} or \texttt{"}fred2\texttt{"}.
         ...
         expensive data conversion. So, for instance, \texttt{"}$*$.fit\texttt{"} could pick up
...
\sstroutine{
   ...
   \sstdescription{
      ...
      \texttt{"}supplemental information\texttt{"} associated with them that holds further
      ...
      NDG\texttt{'}s supplemental information is stored in a chain of \texttt{"}slave
      groups\texttt{"} that are attached to each other using the facilities of
      GRP (e.g. see GRP\_SOWN). The supplied group is the lowest level
      \texttt{"}slave\texttt{"} in this chain.
   }
   \sstinvocation{
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         Full data type of the NDF\texttt{'}s DATA component (e.g. \texttt{'}\_DOUBLE\texttt{'} or
         \texttt{'}COMPLEX\_REAL\texttt{'}).
      }
      \sstsubsection{
         NDIM = INTEGER (Given)
      }{
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            group expression is \texttt{"}$*$\_a\texttt{"}, then an HDS container file called
            \texttt{"}fred\_a.sdf\texttt{"} is created by copying fred.sdf and then deleting all
         ...
         groups created \texttt{"}by hand\texttt{"} using GRP directly can also be used
         (i.e. without the supplemental groups created by NDG). In
         this case, there are no defaults for directory path, file type,
         or HDS component path, and the basis token (\texttt{"}$*$\texttt{"}) in the group
      ...
      \sstsubsection{
         ...
         If the group expression was terminated by the GRP \texttt{"}flag\texttt{"}
   ...
   \sstnotes{
      \sstitemlist{
         ...
         this is \texttt{"}$*$\texttt{"} then the file type is copied from the corresponding input
...
\sstroutine{
   ...
   \sstarguments{
      \sstsubsection{
         ...
         will often be created using NDG\_ASSOC, but groups created \texttt{"}by
         hand\texttt{"} using GRP directly (i.e. without the supplemental groups
...
\sstroutine{
   ...
   \sstarguments{
      \sstsubsection{
         ...
         A GRP identifier for a group holding the names of NDFs. This
         will often be created using NDG\_CREAT, but groups created \texttt{"}by
         hand\texttt{"} using GRP directly can also be used.
      }
      \sstsubsection{
         INDEX = INTEGER (Given)
      }{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      \texttt{"}owner-slave\texttt{"} relationships. They will be deleted automatically when
...
\sstroutine{
   ...
   \sstdescription{
      ...
      since it stores no extra information (\texttt{"}MORE\texttt{"}) with any of the
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         is arbitrary, but the form \texttt{"}PACKAGE:COMMAND\texttt{"} is recommended.
      }
      \sstsubsection{
         NNDF = INTEGER (Given)
      }{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      the AUTOPROV environment variable. If AUTOPROV is set to \texttt{'}1\texttt{'} then
      the input NDFs are added to the provenance information in the
      output NDF. If AUTOPROV is set to anything other than \texttt{'}1\texttt{'} then the
      ...
      to \texttt{'}1\texttt{'} (this is to avoid the potentially costly process of format
      conversion).
   }
   \sstinvocation{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      The returned KeyMap has an entry with key \texttt{"}0\texttt{"} that describes the NDF
      from which the provenance was read. It also has an entry describing
      each ancestor NDF. These entries have keys \texttt{"}1\texttt{"}, \texttt{"}2\texttt{"}, \texttt{"}3\texttt{"}, etc, up to
      ...
      \texttt{"}ID\texttt{"} - the integer index within the ancestors array (zero for the
             main NDF).

      \texttt{"}PATH\texttt{"} - The full path or base name for the NDF (see \texttt{"}base\texttt{"}).

      \texttt{"}DATE\texttt{"} - The date of creation of the NDF.

      \texttt{"}CREATOR\texttt{"} - The software item that created the NDF.

      \texttt{"}PARENTS\texttt{"} - A comma-separated list of indices into the ancestors
             array that identifies the direct parents of the NDF.

      \texttt{"}MORE\texttt{"} - A summary of the contents of the MORE structure associated
             with the NDF.

      \texttt{"}HISTORY\texttt{"} - A vector entry holding one or more KeyMaps. Each
      ...
      \sstitemlist{
         ...
         \sstitem
                \texttt{"}USER\texttt{"}: The user name that performed the action
                          (e.g. \texttt{"}dsb\texttt{"}).
      ...
      Finally, the returned KeyMap has an entry with key \texttt{"}MXLEN\texttt{"} that is
      again a pointer to another KeyMap with the same entries listed
      above (except that it has no \texttt{"}HISTORY\texttt{"} entry). However, this time
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
            \texttt{"}PATH\texttt{"}: A string holding the path of the ancestor NDF.

            \sstitem
            \texttt{"}DATE\texttt{"}: A string holding the formatted UTC date and time at
              which the provenance information for the ancestor NDF was
              recorded.

            \sstitem
            \texttt{"}CREATOR\texttt{"}: A string identifying the software that created the
              ancestor NDF.

            \sstitem
            \texttt{"}PARENTS\texttt{"}: A 1D vector of integers that are the indices of the
              immediate parents of the ancestor.

            \sstitem
            \texttt{"}MORE\texttt{"}: A KeyMap containing any extra information that has
...
\sstroutine{
   ...
   \sstdescription{
      This function flags a specified ancestor as \texttt{"}hidden\texttt{"}. The only
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
            \texttt{"}MORE\texttt{"}: A KeyMap containing extra information to store with
              the ancestor.

         }
         If the \texttt{"}DATE\texttt{"}, \texttt{"}CREATOR\texttt{"} or \texttt{"}MORE\texttt{"} components are missing then
         ...
         has no \texttt{"}PATH\texttt{"} entry. Note, the PARENTS list and HISTORY information
         stored with the specified ancestor cannot be modified (any \texttt{"}PARENTS\texttt{"}
         or \texttt{"}HISTORY\texttt{"} component in the supplied HDS structure will be
         ignored).
      }
      \sstsubsection{
         STATUS = INTEGER (Given and Returned)
...
\sstroutine{
   ...
   \sstdescription{
      This function reads the information stored in the \texttt{"}PROVENANCE\texttt{"}
...
\sstroutine{
   ...
   \sstdescription{
      ...
      all History records inherited from the \texttt{"}primary\texttt{"} NDF (i.e. the NDF
      ...
      to include a \texttt{"}creator\texttt{"} hash code for the History record that describes
...
\sstroutine{
   ...
   \sstdescription{
      ...
      \texttt{"}hidden\texttt{"}. See NDG\_HIDEPROV and NDG\_COPYPROV.
   }
   \sstinvocation{
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         returned. A value of zero will result in information being returned
         that describes the NDF from which the provenance information was
         read. Otherwise, the \texttt{"}ianc\texttt{"} value is used as an index into the
         ANCESTORS array. No error is reported if \texttt{"}ianc\texttt{"} is too large, but a
   ...
   \sstreturnedvalue{
      \sstsubsection{
         A pointer to an AST KeyMap
      }{
         The keymap contains entries with the following
         keys and values:
         \sstitemlist{
         \sstitem \texttt{"}PATH\texttt{"}: A string holding the path of the ancestor NDF.
         \sstitem \texttt{"}DATE\texttt{"}: A string holding the formatted UTC date and time at
         which the provenance information for the ancestor NDF was
         recorded.
         \sstitem \texttt{"}CREATOR\texttt{"}: A string identifying the software that created the
         ancestor NDF.
         \sstitem \texttt{"}PARENTS\texttt{"}: A 1D vector of integers that are the indices of
         the immediate parents of the ancestor.
         \sstitem \texttt{"}MORE\texttt{"}: A KeyMap containing any extra information that has been
         stored with the ancestor.
         \sstitem \texttt{"}HISTORY\texttt{"}: A vector entry holding one or more KeyMaps. Each
         KeyMap contains items that describe an action performed on
         the ancestor. The actions are stored in chronological order
         within the vector entry. The last KeyMap in the vector
         describes the action that created the ancestor NDF. Any
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         read. Otherwise, the \texttt{"}ianc\texttt{"} value is used as an index into the
         ANCESTORS array. An error is reported if \texttt{"}ianc\texttt{"} is too large.
      }
      \sstsubsection{
         km
      }{
...
\sstroutine{
   ...
   \sstarguments{
      \sstsubsection{
         ...
         \texttt{"}$<$GROUP name=\texttt{"}PROVENANCE\texttt{"} utype=\texttt{"}hds\_type:PROVENANCE\texttt{"}$>$\texttt{"}
      ...
      \sstsubsection{
         ...
         form \texttt{"}PACKAGE:COMMAND\texttt{"} is recommended. This value is only used if
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         The length of the \texttt{"}anc\texttt{"} array.
      }
      \sstsubsection{
         anc
      }{
...
\sstroutine{
   ...
   \sstdescription{
      This function ensures that a given ancestor is not flagged as
      \texttt{"}hidden\texttt{"}. See ndgHideProv and ndgCopyProv.
   }
   \sstinvocation{
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         information will not yet have been recorded, resulting in
         incorrect information being stored in the PROVENANCE extension.
         For this reason, the \texttt{"}whdef\texttt{"} argument is supplied. If it is set
         ...
         information, should supply a zero value for \texttt{"}whdef\texttt{"} and should
...
\sstroutine{
   ...
   \sstreturnedvalue{
      \sstsubsection{
         ...
         hold structures. The \texttt{"}utype\texttt{"} attributes are used to hold the
...
\begin{itemize}
   \item A bug has been fixed that resulted in the NDF AUTOHISTORY facility
    ...
    environment variable NDF\_AUTO\_HISTORY.
