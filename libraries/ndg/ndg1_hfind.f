      SUBROUTINE NDG1_HFIND( LOC1, NAME, MODE, LOC2, STATUS )
*+
*  Name:
*     NDG1_HFIND
*     
...
*  Invocation:
*     CALL NDG1_HFIND( LOC1, NAME, MODE, LOC2, STATUS )
...
*        checks elsewhere.
*     15-APR-1994 (RFWS):
*        Added extra arguments to NDG1_FPARX call.
*     15-FEB-1998 (DSB):
...
*  its file name and HDS path fields and open the container file.
      IF ( LOC1 .EQ. DAT__ROOT ) THEN
         CALL NDG1_HSPLT( NAME, F1, F2, I1, IEND, STATUS )
...
*  bounds expression).
               ELSE
                  F = F + I1 - 1
                  L = L + I1 - 1
                  CALL NDG1_FPARX( NAME( F : L ), '(', ')', LP, RP )
                  IF ( LP .LT. RP ) THEN
                     LP = LP + F - 1
                     RP = RP + F - 1
...
*  name).
     ...
     :                    ( DOTTED .OR. ( NFIELD .NE. 1 ) ) ) THEN
                        STATUS = NDF__NOCMP
                        CALL MSG_SETC( 'FIELD', NAME( F : L ) )
                        CALL ERR_REP( 'NDG1_HFIND_MSN',
     :                                'Missing name in HDS ' //
...
*  necessary.
                  IF ( STATUS .EQ. SAI__OK ) THEN
                     IF ( ( LP .LT. RP ) .AND. ( LP .LE. F ) ) THEN
                        CALL NDG1_HCUT( LOC2, NAME( LP : RP ), LOC,
     :                                  STATUS )
                        IF ( LOC1 .EQ. DAT__ROOT ) THEN
...
*  the required component exists within the current HDS structure.
                     ELSE
                        CALL NDG1_CHSCN( NAME( F : INAME ), STATUS )
...
*  Report an error if the component does not exist.
                           IF ( .NOT. THERE ) THEN
                              STATUS = NDF__CNMIN
                              CALL MSG_SETC( 'NAME', NAME( F : INAME ) )
                              CALL DAT_MSG( 'STRUCT', LOC2 )
                              CALL ERR_REP( 'NDG1_HFIND_NF',
     :                                      'There is no ''^NAME'' ' //
...
*  necessary.
                           IF ( STATUS .EQ. SAI__OK ) THEN
                              IF ( LP .LT. RP ) THEN
                                 CALL NDG1_HCUT( LOC2, NAME( LP : RP ),
     :                                           LOC, STATUS )
                                 IF ( LOC1 .EQ. DAT__ROOT ) THEN
                                    CALL DAT_PRMRY( .TRUE., LOC, .TRUE.,
     :                                              STATUS )
