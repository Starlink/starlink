...
*  Invocation:
*     (from Fortran)
*     FOUND = ONE_FIND_FILE (FILESPEC,FILENAME,CONTEXT,STATUS)
...
F77_LOGICAL_FUNCTION(one_find_file)( CHARACTER(FileSpec), CHARACTER(FileName),
                            POINTER(Context), INTEGER(Status)
                            TRAIL(FileSpec) TRAIL(FileName) )
{
  ...
  */
  ...
  if (*Context == 0) {
    ContextPtr = (ContextStruct *) cnf_malloc(2 * sizeof(ContextStruct));
      ...
      if (pipe(Fdptr) < 0) {
        *Status = ONE__PIPEERR;
        ...
        if ((STATUS = fork()) == 0) {
          ...
          (void) close (Fdptr[1]);
          *Context = (F77_POINTER_TYPE) cnf_fptr( ContextPtr );
        }
      }
    }
  }
  ...
  ContextPtr = (ContextStruct *) cnf_cptr( *Context );
  Fdptr = ContextPtr->Fds;
  if (NameLength < 1) {
    *Status = ONE__LENERR;
  } else {
 ...
 *  Call:
 *    (from Fortran)
 *    STATUS = ONE_FIND_FILE_END (CONTEXT)
 * 
 *  Parameters:
 *
 ...
 *                  the Adam conventions where an odd number represents OK status,
 ...
 *  RTP:     25-Sep-2000: Changed for Odds and Ends Library
 *                        Uses cnf pointers.
