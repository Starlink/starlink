...
*  Release the work space used to hold the pixel mask.        
      CALL PSX_FREE( IPMASK, STATUS )                           [16]
                                                              
*  Delete the group used to hold the ARD description.         
      CALL GRP_DELET( IGRP, STATUS )                            [17]
                                                              
*  Annul the AST FrameSet identifier.                                  
      CALL AST_ANNUL( IWCS, STATUS )                            [18]
                                                              
*  Annul the NDF identifier.                                  
      CALL NDF_ANNUL( INDF, STATUS )                            [19]
...
description, then it is assumed that positions are supplied in the
current Frame of the application FrameSet (so the calling application
should make sure that the FrameSet has an appropriate current Frame
before calling ARD\_WORK). The exception to this is if any of the
depracated statements COEFFS, OFFSET, TWIST, STRETCH, SCALE are included
in the ARD description, in which case positions are interpreted according
to the old, version 1 system described below.
