...
\newcommand{\stardocsource}    {sun183.3}
\newcommand{\stardocnumber}    {183.3}
\newcommand{\stardocauthors}   {D.S. Berry}
\newcommand{\stardocdate}      {21st March 1996}
...
in pixel co-ordinates. The ARD language allows positions and distances to be
given in any co-ordinate system which is linearly related to pixel co-ordinates.
Thus for instance, positions within a 2-dimensional image could be given in
arc-seconds north and east from a given position on the sky (assuming the image
is sufficiently small to be able to ignore non-linearities in the
...
This example shows the use of comments in an ARD description. It also shows the
use of \htmlref{statements}{SEC:STAT} to establish a user co-ordinate system which is different to
that expected by the application. Another feature demonstrated by this example
is the ability of ARD to recognise implicit \htmlref{.OR.}{OP:OR} operators. The list of
\htmlref{PIXEL}{KEY:PIX}
keywords specifies a set of individual pixels. Since these keywords have no
intervening operators, ARD assumes that a \htmlref{.OR.}{OP:OR} operator is to be inserted
...
This section presents Fortran code for an ADAM application which obtains a data
array (in the form of an NDF structure, see \xref{SUN/33}{sun33}{}) and an ARD description from
the environment, converts the ARD description into a pixel mask, and then finds
and displays the data sum within the region specified by the ARD description.
The application will deal with NDFs of any dimensionality up to the limit
imposed by the NDF\_ system. 
...
*  Indicate that this application expects positions and       
*  distances within the ARD description to be specified in    
*  pixel co-ordinates.                                        
      TRCOEF( 1 ) = VAL__BADR                                   [10]
...
*  pixels.                                                    
      CALL ARD_WORK( IGRP, NDIM, LBND, UBND, TRCOEF, .FALSE.,   [11]
...
*  Display the data sum.                                      
      ...
      CALL MSG_OUT( 'ARD_TEST_MSG1', '  Data sum: ^SUM',        [13]
     :               STATUS )                                 
                                                              
*  Release the work space used to hold the pixel mask.        
      CALL PSX_FREE( IPMASK, STATUS )                           [14]
                                                              
*  Delete the group used to hold the ARD description.         
      CALL GRP_DELET( IGRP, STATUS )                            [15]
                                                              
*  Annul the NDF identifier.                                  
      CALL NDF_ANNUL( INDF, STATUS )                            [16]
...
``VAL\_\_'' (see \xref{SUN/39}{sun39}{}), and GRP\_PAR defines constants starting with
``GRP\_\_'' (see \xref{SUN/150}{sun150}{}). 
...
\item This application expects the user to supply positions and distances in the
pixel co-ordinate system of the NDF, and indicates this by assigning the
``special'' value given by the symbolic constant VAL\_\_BADR to the first
element of the TRCOEF array. It is possible for an application to specify some
other co-ordinate system, so long as the co-ordinate system is linearly related
to pixel co-ordinates. In this case TRCOEF would be used to store the
co-efficients of the mapping from the new ``application'' co-ordinate
system, to the pixel co-ordinate system. See Section \ref{SEC:COORD}.
...
\item \htmlref{{\em Statement}}{SEC:STAT} fields: These are fields which effect the way that the 
keyword fields are interpreted, e.g. setting up the dimensionality of the system 
(\htmlref{DIMENSION}{ST:DIM}), the user co-ordinate system (\htmlref{COEFFS}{ST:COE}), etc.
...
There are three co-ordinate systems recognised by ARD:

\begin{enumerate}

\item {\em Pixel Co-ordinates} - These refer to the pixel mask returned by
\htmlref{ARD\_WORK}{ARD_WORK}. The position of the origin of pixel co-ordinates within the mask is
fixed by the upper and lower bounds of the mask supplied to \htmlref{ARD\_WORK}{ARD_WORK}. The
conventions for pixel co-ordinates used by Starlink software are described in
