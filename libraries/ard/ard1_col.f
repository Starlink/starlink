      SUBROUTINE ARD1_COL( RINDEX, NDIM, D, LBND, UBND, MSKSIZ,
...
*  Purpose:
*     Initialise an array to hold a COLUMN region.
...
*  Report an error if the number of dimensions is not 2.
      IF( NDIM .NE. 2 ) THEN
         ...
         CALL ERR_REP( 'ARD1_COL_ERR1', 'Wrong no. of dimensions '//
     :                 '(^ND) supplied in ARD1_COL (programming '//
     :                 'error).', STATUS )
         GO TO 999
      END IF
...
*  Store the components of a unit direction vector along the column.
      VX = D( 3 )
      VY = D( 6 )
      ...
      ELSE
         STATUS = ARD__INTER
         CALL ERR_REP( 'ARD1_COL_ERR3', 'Null direction vector '//
     :                 'supplied to ARD1_COL (programming error).',
     :                 STATUS )
         GO TO 999

      END IF

*  Loop round each of the columns.
      DO ICOL = 1, NPAR

*  Find the pixel co-ordinates corresponding to position (PAR,0).
         X = D( 1 ) + D( 2 )*PAR( ICOL )
         Y = D( 4 ) + D( 5 )*PAR( ICOL )
...
*  with the current column and which extends beyond the boundaries of the
*  mask.
