      SUBROUTINE ARD1_BXEQV( NDIM, LBND, UBND, MSKSIZ, A, LBOX,
     :                       UBOX, B, STATUS )
*+
*  Name:
*     ARD1_BXEQV
...
*  Purpose:
*     Perform an EQV operation within a box region.

*  Language:
*     Starlink Fortran 77

*  Invocation:
*     CALL ARD1_BXEQV( NDIM, LBND, UBND, MSKSIZ, A, LBOX, UBOX,
*                      B, STATUS )

*  Description:
*     The area of arrays A and B within the supplied bounding box are
*     EQV'ed together and the results stored back in B. All elements of
*     B outside the box are left unchanged. A general N-d algorithm is
*     used.
*
*     Values equal to 0 are treated as .FALSE., and values greater than
*     0 are treated as .TRUE. 1 is stored in the output if both input
*     pixels are .FALSE.. If both input pixels are .TRUE., then the
*     maximum of the two input pixels is stored in the output.
...
*        The first input array (in vector form).
...
*        The the second input array (in vector form). The results are
*        returned in this array, over-writing the supplied values within
*        the box region.
...
*  Loop round every mask pixel.
         DO VA = 1, MSKSIZ

*  If both input pixels are exterior, the output pixel is an
*  interior background pixel.
            IF( B( VA ) .LE. 0 .AND.
     :          A( VA ) .LE. 0 ) THEN
               B( VA ) = 1

*  If both input pixels are interior, then the output pixel is also
*  interior, in which case use the maximum of the two input values.
            ELSE IF( B( VA ) .GT. 0 .AND.
     :               A( VA ) .GT. 0 ) THEN
               IF( B( VA ) .LT. A( VA ) ) B( VA ) = A( VA )

*  If one input pixel is interior and the other is exterior, the output
*  pixel is exterior.
            ELSE
               B( VA ) = 0
...
*  Assign a value to the current pixel (as above).
            IF( B( VA ) .LE. 0 .AND.
     :          A( VA ) .LE. 0 ) THEN
               B( VA ) = 1
