      SUBROUTINE ARD1_STORD( VALUE, SIZE, NEXT, PNTR, STATUS )
*+
*  Name:
*     ARD1_STORD

*  Purpose:
*     Store a _double value in a dynamic array

*  Language:
*     Starlink Fortran 77

*  Invocation:
*     CALL ARD1_STORD( VALUE, SIZE, NEXT, PNTR, STATUS )
...
*        A pointer to the DOUBLE PRECISION array.
...
*  If the array is not big enough increase its size.
      IF( NEXT .GT. SIZE ) THEN
         ...
         CALL PSX_REALLOC( VAL__NBD*SIZE, PNTR, STATUS )
      END IF

*  Store the value.
      CALL ARD1_PUTD( VALUE, SIZE, NEXT, %VAL( CNF_PVAL( PNTR ) ),
     :                STATUS )
