      SUBROUTINE ARD1_BXAND( NDIM, LBND, UBND, MSKSIZ, A, LBOX,
     :                       UBOX, B, STATUS )
*+
*  Name:
*     ARD1_BXAND
...
*  Purpose:
*     Perform an AND operation within a box region.

*  Language:
*     Starlink Fortran 77

*  Invocation:
*     CALL ARD1_BXAND( NDIM, LBND, UBND, MSKSIZ, A, LBOX, UBOX,
*                      B, STATUS )

*  Description:
*     The area of arrays A and B within the supplied bounding box are
*     AND'ed together and the results stored back in B. All elements of
*     B outside the box are left unchanged. A general N-d algorithm is
*     used.
*
*     Values less than or equal to 0 are treated as .FALSE., and all
*     other values are treated as .TRUE. The values stored in the
*     output for .TRUE. pixels is given by the maximum of the input
*     values.
...
*  Loop round all pixels in the mask.
         DO VA = 1, MSKSIZ

*  Assign a value to the current pixel.
            IF( B( VA ) .GT. 0 ) THEN

               IF( A( VA ) .LE. 0 ) THEN
                  B( VA ) = 0

               ELSE IF( B( VA ) .LT. A( VA ) ) THEN
                  B( VA ) = A( VA )
...
*
*  The pixels forming the box will have continuous vector addresses
