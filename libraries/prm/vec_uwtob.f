      SUBROUTINE VEC_UWTOB( BAD, N, ARGV, RESV, IERR, NERR, STATUS )
*+
*  Name:
*     VEC_UWTOB

*  Purpose:
*     Convert vectorised UNSIGNED WORD values to BYTE.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL VEC_UWTOB( BAD, N, ARGV, RESV, IERR, NERR, STATUS )

*  Description:
*     The routine performs type conversion on a vectorised array ARGV
*     of UNSIGNED WORD values, converting them to the equivalent BYTE
*     values.  If numerical errors occur, the value VAL__BADB is
...
*        receive the BYTE results.  The value VAL__BADB will be
...
*  If not, then put a value of VAL__BADB in the corresponding
