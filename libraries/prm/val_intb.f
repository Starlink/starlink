      BYTE FUNCTION VAL_INTB( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_INTB

*  Purpose:
*     Evaluate the BYTE truncation to integer function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_INTB( BAD, ARG, STATUS )
...
*     ARG = BYTE (Given)
*        The BYTE argument of the truncation to integer function.
...
*  Returned Value:
*     VAL_INTB = BYTE
*        Returns the evaluated truncation to integer function result as a BYTE
