      SUBROUTINE VEC_LG10D( BAD, N, ARGV, RESV, IERR, NERR, STATUS )
*+
*  Name:
*     VEC_LG10D

*  Purpose:
*     Vectorised DOUBLE PRECISION common logarithm (base 10) function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL VEC_LG10D( BAD, N, ARGV, RESV, IERR, NERR, STATUS )
...
*  increment the numerical error count.
            ELSE IF( .NOT. ( NUM_GTD( ARG, VAL__ZD ) ) ) THEN
               RESV( I ) = VAL__BADD
               NERR = NERR + 1
...
*  increment the error count.
            IF( .NOT. ( NUM_GTD( ARG, VAL__ZD ) ) ) THEN
               RESV( I ) = VAL__BADD
               NERR = NERR + 1
