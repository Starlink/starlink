      REAL FUNCTION VAL_RTOR( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_RTOR
...
*  Invocation:
*     RESULT = VAL_RTOR( BAD, ARG, STATUS )

*  Description:
*     The routine copies a value of type REAL.  It forms part of the
...
*     ARG = REAL (Given)
*        The REAL value to be copied.
*     STATUS = INTEGER (Given)
*        This should be set to SAI__OK on entry, otherwise the routine
*        returns immediately with the result VAL__BADR.  This routine
*        cannot produce numerical errors, so the STATUS argument will
*        not be changed.

*  Returned Value:
*     VAL_RTOR = REAL
*        Returns the copied REAL value.  The value VAL__BADR will
