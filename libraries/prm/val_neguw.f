      INTEGER*2 FUNCTION VAL_NEGUW( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_NEGUW

*  Purpose:
*     Evaluate the UNSIGNED WORD negation function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_NEGUW( BAD, ARG, STATUS )

*  Description:
*     The routine evaluates the negation function for a single
...
*     ARG = INTEGER*2 (Given)
*        The UNSIGNED WORD argument of the negation function.
...
*  Returned Value:
*     VAL_NEGUW = INTEGER*2
*        Returns the evaluated negation function result as a UNSIGNED WORD
...
*  result VAL__BADUW and set a STATUS value.
      ELSE IF( .NOT. ( NUM_EQUW( ARG, VAL__ZUW ) ) ) THEN
         VAL_NEGUW = VAL__BADUW
         STATUS = PRM__INTOF
