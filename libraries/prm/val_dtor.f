      REAL FUNCTION VAL_DTOR( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_DTOR

*  Purpose:
*     Convert a value from DOUBLE PRECISION to REAL.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_DTOR( BAD, ARG, STATUS )

*  Description:
*     The routine performs type conversion on a value of type DOUBLE PRECISION,
*     converting it to the equivalent REAL value.  If a numerical
*     error occurs, the value VAL__BADR is returned and a STATUS
*     value is set.
...
*  Returned Value:
*     VAL_DTOR = REAL
*        Returns the converted REAL value.  The value VAL__BADR
...
*  VAL__BADR and set a STATUS value.
      ELSE IF( ( .TRUE. ) .AND.
