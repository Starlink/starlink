      REAL FUNCTION VAL_SQRTR( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_SQRTR

*  Purpose:
*     Evaluate the REAL square root function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_SQRTR( BAD, ARG, STATUS )
...
*     ARG = REAL (Given)
*        The REAL argument of the square root function.
...
*  Returned Value:
*     VAL_SQRTR = REAL
*        Returns the evaluated square root function result as a REAL
...
*  result VAL__BADR and set a STATUS value.
      ELSE IF( .NOT. ( NUM_GER( ARG, VAL__ZR ) ) ) THEN
         VAL_SQRTR = VAL__BADR
         STATUS = PRM__SQRNG
