      DOUBLE PRECISION FUNCTION VAL_ASNDD( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_ASNDD

*  Purpose:
*     Evaluate the DOUBLE PRECISION arc-sine (degrees) function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_ASNDD( BAD, ARG, STATUS )

*  Description:
*     The routine evaluates the arc-sine (degrees) function for a single
...
*        The DOUBLE PRECISION argument of the arc-sine (degrees) function.
...
*  Returned Value:
*     VAL_ASNDD = DOUBLE PRECISION
*        Returns the evaluated arc-sine (degrees) function result as a DOUBLE PRECISION
...
*  result VAL__BADD and set a STATUS value.
      ELSE IF( .NOT. ( NUM_GED( ARG, VAL__M1D ) .AND. NUM_LED( ARG, VAL_
     :_P1D ) ) ) THEN
         VAL_ASNDD = VAL__BADD
         STATUS = PRM__ARGIN
