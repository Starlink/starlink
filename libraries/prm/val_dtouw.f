      INTEGER * 2 FUNCTION VAL_DTOUW( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_DTOUW

*  Purpose:
*     Convert a value from DOUBLE PRECISION to UNSIGNED WORD.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_DTOUW( BAD, ARG, STATUS )

*  Description:
*     The routine performs type conversion on a value of type DOUBLE PRECISION,
*     converting it to the equivalent UNSIGNED WORD value.  If a numerical
*     error occurs, the value VAL__BADUW is returned and a STATUS
*     value is set.
...
*  Returned Value:
*     VAL_DTOUW = INTEGER * 2
*        Returns the converted UNSIGNED WORD value.  The value VAL__BADUW
...
*  VAL__BADUW and set a STATUS value.
      ELSE IF( ( .TRUE. ) .AND.
