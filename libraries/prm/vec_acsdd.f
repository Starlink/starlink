      SUBROUTINE VEC_ACSDD( BAD, N, ARGV, RESV, IERR, NERR, STATUS )
*+
*  Name:
*     VEC_ACSDD

*  Purpose:
*     Vectorised DOUBLE PRECISION arc-cosine (degrees) function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL VEC_ACSDD( BAD, N, ARGV, RESV, IERR, NERR, STATUS )

*  Description:
*     The routine evaluates the arc-cosine (degrees) function for a vectorised
...
*        argument values for the arc-cosine (degrees) function.
...
*  increment the numerical error count.
            ELSE IF( .NOT. ( NUM_GED( ARG, VAL__M1D ) .AND. NUM_LED( ARG
     :, VAL__P1D ) ) ) THEN
               RESV( I ) = VAL__BADD
               NERR = NERR + 1
...
*  If the argument value is acceptable, then evaluate the arc-cosine (degrees)
*  function.
            ELSE
               RESV( I ) = NUM_ACSDD( ARG )
...
*  increment the error count.
            IF( .NOT. ( NUM_GED( ARG, VAL__M1D ) .AND. NUM_LED( ARG, VAL
     :__P1D ) ) ) THEN
               RESV( I ) = VAL__BADD
               NERR = NERR + 1
