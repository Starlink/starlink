      INTEGER FUNCTION VAL_UBTOI( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_UBTOI

*  Purpose:
*     Convert a value from UNSIGNED BYTE to INTEGER.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_UBTOI( BAD, ARG, STATUS )

*  Description:
*     The routine performs type conversion on a value of type UNSIGNED BYTE,
*     converting it to the equivalent INTEGER value.  If a numerical
*     error occurs, the value VAL__BADI is returned and a STATUS
*     value is set.
...
*  Returned Value:
*     VAL_UBTOI = INTEGER
*        Returns the converted INTEGER value.  The value VAL__BADI
...
*  VAL__BADI and set a STATUS value.
      ELSE IF( ( .FALSE. ) .AND.
