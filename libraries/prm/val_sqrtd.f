      DOUBLE PRECISION FUNCTION VAL_SQRTD( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_SQRTD

*  Purpose:
*     Evaluate the DOUBLE PRECISION square root function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_SQRTD( BAD, ARG, STATUS )
...
*        The DOUBLE PRECISION argument of the square root function.
...
*  Returned Value:
*     VAL_SQRTD = DOUBLE PRECISION
*        Returns the evaluated square root function result as a DOUBLE PRECISION
...
*  result VAL__BADD and set a STATUS value.
      ELSE IF( .NOT. ( NUM_GED( ARG, VAL__ZD ) ) ) THEN
         VAL_SQRTD = VAL__BADD
         STATUS = PRM__SQRNG
