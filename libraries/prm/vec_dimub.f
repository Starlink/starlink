      SUBROUTINE VEC_DIMUB( BAD, N, ARGV1, ARGV2, RESV, IERR, NERR,
     :                          STATUS )
*+
*  Name:
*     VEC_DIMUB
...
*  Purpose:
*     Vectorised UNSIGNED BYTE Fortran DIM (positive difference) function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL VEC_DIMUB( BAD, N, ARGV1, ARGV2, RESV, IERR, NERR,
*                         STATUS )

*  Description:
*     The routine evaluates the Fortran DIM (positive difference) function for two vectorised
*     arrays ARGV1 and ARGV2 of UNSIGNED BYTE argument values.  If numerical
*     errors occur, the value VAL__BADUB is returned in appropriate
...
*        of UNSIGNED BYTE argument values for the Fortran DIM (positive difference) function.
...
*
*           RESV( I ) = DIM( ARGV1( I ), ARGV2( I ) )
*

*        for I = 1 to N.  The value VAL__BADUB will be set in
...
*  Arguments Given:
      ...
      BYTE ARGV1( * )          ! First function argument array
      BYTE ARGV2( * )          ! Second function argument array
...
*  Local Variables:
      ...
      BYTE ARG1                ! Temporary variable for argument 1
      BYTE ARG2                ! Temporary variable for argument 2
...
*  value of VAL__BADUB in the corresponding element of the result
*  array.
            IF( ARG1 .EQ. VAL__BADUB .OR.
...
*  If the argument values are acceptable, then evaluate the Fortran DIM (positive difference)
*  function.
            ELSE
               RESV( I ) = NUM_DIMUB( ARG1, ARG2 )

*  If an error handler is established, check if the numerical error
*  flag is set.  If so, put a value of VAL__BADUB in the corresponding
