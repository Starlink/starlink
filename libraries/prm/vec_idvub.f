      SUBROUTINE VEC_IDVUB( BAD, N, ARGV1, ARGV2, RESV, IERR, NERR,
     :                          STATUS )
*+
*  Name:
*     VEC_IDVUB
...
*  Purpose:
*     Vectorised UNSIGNED BYTE integer divide operation.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL VEC_IDVUB( BAD, N, ARGV1, ARGV2, RESV, IERR, NERR,
*                         STATUS )

*  Description:
*     The routine performs an arithmetic integer divide operation between
*     two vectorised arrays ARGV1 and ARGV2 of UNSIGNED BYTE values.  If
*     numerical errors occur, the value VAL__BADUB is returned in
...
*        of UNSIGNED BYTE argument values for the integer divide operation.
...
*  array.
            IF( ARGV1( I ) .EQ. VAL__BADUB .OR.
     :          ARGV2( I ) .EQ. VAL__BADUB ) THEN
               RESV( I ) = VAL__BADUB

*  If the argument values are not bad, perform the integer divide
*  operation.
            ELSE
               RESV( I ) = NUM_IDVUB( ARGV1( I ), ARGV2( I ) )

*  Check if the numerical error flag is set.  If so, put a value of
*  VAL__BADUB in the corresponding element of the result array and
