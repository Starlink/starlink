      INTEGER*2 FUNCTION VAL_SQRTW( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_SQRTW

*  Purpose:
*     Evaluate the WORD square root function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_SQRTW( BAD, ARG, STATUS )
...
*     ARG = INTEGER*2 (Given)
*        The WORD argument of the square root function.
...
*  Returned Value:
*     VAL_SQRTW = INTEGER*2
*        Returns the evaluated square root function result as a WORD
...
*  result VAL__BADW and set a STATUS value.
      ELSE IF( .NOT. ( NUM_GEW( ARG, VAL__ZW ) ) ) THEN
         VAL_SQRTW = VAL__BADW
         STATUS = PRM__SQRNG
