      SUBROUTINE VEC_TAND( BAD, N, ARGV, RESV, IERR, NERR, STATUS )
*+
*  Name:
*     VEC_TAND

*  Purpose:
*     Vectorised DOUBLE PRECISION tangent function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL VEC_TAND( BAD, N, ARGV, RESV, IERR, NERR, STATUS )

*  Description:
*     The routine evaluates the tangent function for a vectorised
...
*        argument values for the tangent function.
...
*  If the argument value is acceptable, then evaluate the tangent
*  function.
            ELSE
               RESV( I ) = NUM_TAND( ARG )
