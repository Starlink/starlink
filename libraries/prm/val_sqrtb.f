      BYTE FUNCTION VAL_SQRTB( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_SQRTB

*  Purpose:
*     Evaluate the BYTE square root function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_SQRTB( BAD, ARG, STATUS )
...
*     ARG = BYTE (Given)
*        The BYTE argument of the square root function.
...
*  Returned Value:
*     VAL_SQRTB = BYTE
*        Returns the evaluated square root function result as a BYTE
...
*  result VAL__BADB and set a STATUS value.
      ELSE IF( .NOT. ( NUM_GEB( ARG, VAL__ZB ) ) ) THEN
         VAL_SQRTB = VAL__BADB
         STATUS = PRM__SQRNG
