      SUBROUTINE VEC_SIND( BAD, N, ARGV, RESV, IERR, NERR, STATUS )
*+
*  Name:
*     VEC_SIND

*  Purpose:
*     Vectorised DOUBLE PRECISION sine function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL VEC_SIND( BAD, N, ARGV, RESV, IERR, NERR, STATUS )

*  Description:
*     The routine evaluates the sine function for a vectorised
...
*        argument values for the sine function.
...
*  If the argument value is acceptable, then evaluate the sine
*  function.
            ELSE
               RESV( I ) = NUM_SIND( ARG )
