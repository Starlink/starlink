      SUBROUTINE VEC_UBTOD( BAD, N, ARGV, RESV, IERR, NERR, STATUS )
*+
*  Name:
*     VEC_UBTOD

*  Purpose:
*     Convert vectorised UNSIGNED BYTE values to DOUBLE PRECISION.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL VEC_UBTOD( BAD, N, ARGV, RESV, IERR, NERR, STATUS )

*  Description:
*     The routine performs type conversion on a vectorised array ARGV
*     of UNSIGNED BYTE values, converting them to the equivalent DOUBLE PRECISION
*     values.  If numerical errors occur, the value VAL__BADD is
...
*        receive the DOUBLE PRECISION results.  The value VAL__BADD will be
...
*  Arguments Returned:
      DOUBLE PRECISION RESV( * )          ! Array of result values
...
*  If not, then put a value of VAL__BADD in the corresponding
