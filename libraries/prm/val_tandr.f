      REAL FUNCTION VAL_TANDR( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_TANDR

*  Purpose:
*     Evaluate the REAL tangent (degrees) function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_TANDR( BAD, ARG, STATUS )

*  Description:
*     The routine evaluates the tangent (degrees) function for a single
...
*     ARG = REAL (Given)
*        The REAL argument of the tangent (degrees) function.
...
*  Returned Value:
*     VAL_TANDR = REAL
*        Returns the evaluated tangent (degrees) function result as a REAL
