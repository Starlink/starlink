      SUBROUTINE VEC_BTOUW( BAD, N, ARGV, RESV, IERR, NERR, STATUS )
*+
*  Name:
*     VEC_BTOUW

*  Purpose:
*     Convert vectorised BYTE values to UNSIGNED WORD.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL VEC_BTOUW( BAD, N, ARGV, RESV, IERR, NERR, STATUS )

*  Description:
*     The routine performs type conversion on a vectorised array ARGV
*     of BYTE values, converting them to the equivalent UNSIGNED WORD
*     values.  If numerical errors occur, the value VAL__BADUW is
...
*        receive the UNSIGNED WORD results.  The value VAL__BADUW will be
...
*  VAL__BADUW in the corresponding element of the result array.
            IF( ARGV( I ) .EQ. VAL__BADB ) THEN
               RESV( I ) = VAL__BADUW

*  If the conversion can potentially fail, then test if the argument
*  value lies within its allowed bounds.
            ELSE IF( ( .TRUE. ) .AND.
     :               ( NUM_LTB( ARGV( I ), LO ) .OR.
     :                 NUM_GTB( ARGV( I ), HI ) ) ) THEN

*  If not, then put a value of VAL__BADUW in the corresponding
