      SUBROUTINE VEC_INTUB( BAD, N, ARGV, RESV, IERR, NERR, STATUS )
*+
*  Name:
*     VEC_INTUB

*  Purpose:
*     Vectorised UNSIGNED BYTE truncation to integer function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL VEC_INTUB( BAD, N, ARGV, RESV, IERR, NERR, STATUS )

*  Description:
*     The routine evaluates the truncation to integer function for a vectorised
*     array ARGV of UNSIGNED BYTE values.  If numerical errors occur, the
*     value VAL__BADUB is returned in appropriate elements of the
...
*        argument values for the truncation to integer function.
...
*  VAL__BADUB in the corresponding element of the result array.
            IF( ARG .EQ. VAL__BADUB ) THEN
               RESV( I ) = VAL__BADUB

*  Check if the argument value is acceptable.  If not, then put a value
*  of VAL__BADUB in the corresponding element of the result array and
...
*  If the argument value is acceptable, then evaluate the truncation to integer
*  function.
            ELSE
               RESV( I ) = NUM_INTUB( ARG )
