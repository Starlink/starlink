      BYTE FUNCTION VAL_DIMB( BAD, ARG1, ARG2, STATUS )
*+
*  Name:
*     VAL_DIMB

*  Purpose:
*     Evaluate the BYTE Fortran DIM (positive difference) function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_DIMB( BAD, ARG1, ARG2, STATUS )
...
*        The two BYTE arguments of the Fortran DIM (positive difference) function.
...
*  Returned Value:
*     VAL_DIMB = BYTE
*        Returns the evaluated Fortran DIM (positive difference) function result as a BYTE
