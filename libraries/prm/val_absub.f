      BYTE FUNCTION VAL_ABSUB( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_ABSUB

*  Purpose:
*     Evaluate the UNSIGNED BYTE Fortran ABS function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_ABSUB( BAD, ARG, STATUS )
...
*     ARG = BYTE (Given)
*        The UNSIGNED BYTE argument of the Fortran ABS function.
...
*  Returned Value:
*     VAL_ABSUB = BYTE
*        Returns the evaluated Fortran ABS function result as a UNSIGNED BYTE
