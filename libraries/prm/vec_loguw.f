      SUBROUTINE VEC_LOGUW( BAD, N, ARGV, RESV, IERR, NERR, STATUS )
*+
*  Name:
*     VEC_LOGUW

*  Purpose:
*     Vectorised UNSIGNED WORD natural logarithm function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL VEC_LOGUW( BAD, N, ARGV, RESV, IERR, NERR, STATUS )

*  Description:
*     The routine evaluates the natural logarithm function for a vectorised
...
*        argument values for the natural logarithm function.
...
*  increment the numerical error count.
            ELSE IF( .NOT. ( NUM_GTUW( ARG, VAL__ZUW ) ) ) THEN
               RESV( I ) = VAL__BADUW
               NERR = NERR + 1
...
*  If the argument value is acceptable, then evaluate the natural logarithm
*  function.
            ELSE
               RESV( I ) = NUM_LOGUW( ARG )
...
*  increment the error count.
            IF( .NOT. ( NUM_GTUW( ARG, VAL__ZUW ) ) ) THEN
               RESV( I ) = VAL__BADUW
               NERR = NERR + 1
