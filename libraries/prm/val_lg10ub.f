      BYTE FUNCTION VAL_LG10UB( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_LG10UB

*  Purpose:
*     Evaluate the UNSIGNED BYTE common logarithm (base 10) function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_LG10UB( BAD, ARG, STATUS )
...
*     ARG = BYTE (Given)
*        The UNSIGNED BYTE argument of the common logarithm (base 10) function.
...
*  Returned Value:
*     VAL_LG10UB = BYTE
*        Returns the evaluated common logarithm (base 10) function result as a UNSIGNED BYTE
...
*  result VAL__BADUB and set a STATUS value.
      ELSE IF( .NOT. ( NUM_GTUB( ARG, VAL__ZUB ) ) ) THEN
         VAL_LG10UB = VAL__BADUB
         STATUS = PRM__LOGZN
