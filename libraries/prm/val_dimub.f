      BYTE FUNCTION VAL_DIMUB( BAD, ARG1, ARG2, STATUS )
*+
*  Name:
*     VAL_DIMUB

*  Purpose:
*     Evaluate the UNSIGNED BYTE Fortran DIM (positive difference) function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_DIMUB( BAD, ARG1, ARG2, STATUS )
...
*        The two UNSIGNED BYTE arguments of the Fortran DIM (positive difference) function.
...
*  Returned Value:
*     VAL_DIMUB = BYTE
*        Returns the evaluated Fortran DIM (positive difference) function result as a UNSIGNED BYTE
