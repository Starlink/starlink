      INTEGER FUNCTION VAL_SQRTI( BAD, ARG, STATUS )
*+
*  Name:
*     VAL_SQRTI

*  Purpose:
*     Evaluate the INTEGER square root function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_SQRTI( BAD, ARG, STATUS )
...
*     ARG = INTEGER (Given)
*        The INTEGER argument of the square root function.
...
*  Returned Value:
*     VAL_SQRTI = INTEGER
*        Returns the evaluated square root function result as a INTEGER
...
*  result VAL__BADI and set a STATUS value.
      ELSE IF( .NOT. ( NUM_GEI( ARG, VAL__ZI ) ) ) THEN
         VAL_SQRTI = VAL__BADI
         STATUS = PRM__SQRNG
