      SUBROUTINE VEC_UWTOR( BAD, N, ARGV, RESV, IERR, NERR, STATUS )
*+
*  Name:
*     VEC_UWTOR

*  Purpose:
*     Convert vectorised UNSIGNED WORD values to REAL.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL VEC_UWTOR( BAD, N, ARGV, RESV, IERR, NERR, STATUS )

*  Description:
*     The routine performs type conversion on a vectorised array ARGV
*     of UNSIGNED WORD values, converting them to the equivalent REAL
*     values.  If numerical errors occur, the value VAL__BADR is
...
*        receive the REAL results.  The value VAL__BADR will be
...
*  Arguments Returned:
      REAL RESV( * )          ! Array of result values
...
C               DLO = DLO - 0.5D0 * NUM_RTOD( VAL__EPSR )
C            ELSE
C               DLO = DLO - 0.5D0 * ( NUM_RTOD( VAL__EPSR ) -
C     :                               NUM_UWTOD( VAL__EPSUW ) )
C            END IF
C            IF ( DHI .GT. 0.0D0 ) THEN
C               DHI = DHI + 0.5D0 * ( NUM_RTOD( VAL__EPSR ) -
C     :                               NUM_UWTOD( VAL__EPSUW ) )
C            ELSE
C               DHI = DHI + 0.5D0 * NUM_RTOD( VAL__EPSR )
C            END IF
...
*  VAL__BADR in the corresponding element of the result array.
            IF( ARGV( I ) .EQ. VAL__BADUW ) THEN
               RESV( I ) = VAL__BADR

*  If the conversion can potentially fail, then test if the argument
*  value lies within its allowed bounds.
            ELSE IF( ( .FALSE. ) .AND.
     :               ( NUM_LTUW( ARGV( I ), LO ) .OR.
     :                 NUM_GTUW( ARGV( I ), HI ) ) ) THEN

*  If not, then put a value of VAL__BADR in the corresponding
