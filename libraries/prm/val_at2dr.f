      REAL FUNCTION VAL_AT2DR( BAD, ARG1, ARG2, STATUS )
*+
*  Name:
*     VAL_AT2DR

*  Purpose:
*     Evaluate the REAL Fortran ATAN2 (in degrees) function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_AT2DR( BAD, ARG1, ARG2, STATUS )

*  Description:
*     The routine evaluates the Fortran ATAN2 (in degrees) function for a pair of
...
*        The two REAL arguments of the Fortran ATAN2 (in degrees) function.
...
*  Returned Value:
*     VAL_AT2DR = REAL
*        Returns the evaluated Fortran ATAN2 (in degrees) function result as a REAL
...
*  result VAL__BADR and set a STATUS value.
      ELSE IF( .NOT. ( ( ARG1 .NE. VAL__ZR ) .OR. ( ARG2 .NE. VAL__ZR )
     :) ) THEN
         VAL_AT2DR = VAL__BADR
         STATUS = PRM__ARGIN
