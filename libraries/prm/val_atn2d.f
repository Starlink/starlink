      DOUBLE PRECISION FUNCTION VAL_ATN2D( BAD, ARG1, ARG2, STATUS )
*+
*  Name:
*     VAL_ATN2D

*  Purpose:
*     Evaluate the DOUBLE PRECISION Fortran ATAN2 function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     RESULT = VAL_ATN2D( BAD, ARG1, ARG2, STATUS )

*  Description:
*     The routine evaluates the Fortran ATAN2 function for a pair of
...
*        The two DOUBLE PRECISION arguments of the Fortran ATAN2 function.
...
*  Returned Value:
*     VAL_ATN2D = DOUBLE PRECISION
*        Returns the evaluated Fortran ATAN2 function result as a DOUBLE PRECISION
...
*  result VAL__BADD and set a STATUS value.
      ELSE IF( .NOT. ( ( ARG1 .NE. VAL__ZD ) .OR. ( ARG2 .NE. VAL__ZD )
     :) ) THEN
         VAL_ATN2D = VAL__BADD
         STATUS = PRM__ARGIN
