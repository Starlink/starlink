      SUBROUTINE VEC_IDVD( BAD, N, ARGV1, ARGV2, RESV, IERR, NERR,
     :                          STATUS )
*+
*  Name:
*     VEC_IDVD
...
*  Purpose:
*     Vectorised DOUBLE PRECISION integer divide operation.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL VEC_IDVD( BAD, N, ARGV1, ARGV2, RESV, IERR, NERR,
*                         STATUS )

*  Description:
*     The routine performs an arithmetic integer divide operation between
*     two vectorised arrays ARGV1 and ARGV2 of DOUBLE PRECISION values.  If
*     numerical errors occur, the value VAL__BADD is returned in
...
*        of DOUBLE PRECISION argument values for the integer divide operation.
...
*  Arguments Given:
      ...
      DOUBLE PRECISION ARGV1( * )          ! First argument array
      DOUBLE PRECISION ARGV2( * )          ! Second argument array

*  Arguments Returned:
      DOUBLE PRECISION RESV( * )           ! Result array
...
*  array.
            IF( ARGV1( I ) .EQ. VAL__BADD .OR.
     :          ARGV2( I ) .EQ. VAL__BADD ) THEN
               RESV( I ) = VAL__BADD

*  If the argument values are not bad, perform the integer divide
*  operation.
            ELSE
               RESV( I ) = NUM_IDVD( ARGV1( I ), ARGV2( I ) )

*  Check if the numerical error flag is set.  If so, put a value of
*  VAL__BADD in the corresponding element of the result array and
