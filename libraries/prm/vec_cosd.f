      SUBROUTINE VEC_COSD( BAD, N, ARGV, RESV, IERR, NERR, STATUS )
*+
*  Name:
*     VEC_COSD

*  Purpose:
*     Vectorised DOUBLE PRECISION cosine function.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL VEC_COSD( BAD, N, ARGV, RESV, IERR, NERR, STATUS )

*  Description:
*     The routine evaluates the cosine function for a vectorised
...
*        argument values for the cosine function.
...
*  If the argument value is acceptable, then evaluate the cosine
*  function.
            ELSE
               RESV( I ) = NUM_COSD( ARG )
