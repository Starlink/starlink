...
#endif
#ifndef FC_HAVE_F95_HEX_NUMBERS
#define FC_HAVE_F95_HEX_NUMBERS 0
#endif
...
"\n",
                ...
                (FC_HAVE_F95_HEX_NUMBERS  ? "DOES" : "DOES NOT"),
    ...
    if (FortranOutput) {
	    ...
	    N.nbytes = size;
            fprintf(FortranOutput,
                    ...
                    "*     [In decimal: %d]\n\n",
                    type, name, name, tohex(&N), val);
        }
        else if (FC_HAVE_F95_HEX_NUMBERS && size > 0) {
            Number N;
            N.v.i = val;
	    N.nbytes = size;
            fprintf(FortranOutput,
                    FLINE("%s %s")
                    FLINE("PARAMETER ( %s = X'%s' )")
                    "*     [In decimal: %d]\n\n",
                    type, name, name, tohex(&N), val);
        } else {
            fprintf(FortranOutput,
...
#elif FC_HAVE_F95_HEX_NUMBERS
        assumption(sizeof(double) <= sizeof(int64_t),
                   "Number type assumes double <= 64 bits");
	fprintf(FortranOutput,
		"      %s %s\n      PARAMETER ( %s = X'%s' )\n\n",
		type, name, name, tohex(&N));
#else /* !FC_HAVE_HEX_NUMBERS of any kind */
	if (issingle) {
	    fprintf(FortranOutput,
		    "      %s %s\n      PARAMETER ( %s = %.*E )\n\n",
