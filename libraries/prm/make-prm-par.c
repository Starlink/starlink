...
#if TEST_CODE
const char *tolongint(const int size, Number *n)
{
    static char b[100];
    char sign;
    int64_t i = (size == 4 ? n-> i : n->i8);
    int parts[3];
    int mult;
    ...
    mult = 0x00ffffff;
    parts[2] = (i & 0x0000000000ffffffL);
    parts[1] = ((i >> 24) & 0x0000000000ffffffL);
    parts[0] = (i >> 48);
...
#if TEST_CODE
    if (TestOutput) {
        int precision = (size == 1 ? float_precision : double_precision);
        const char *varname;
        const char *testint;
        const char *t;
