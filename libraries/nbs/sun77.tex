...
%                      can be generated.
%    9-Dec-2014 - TIMJ - convert to new Starlink latex class. Remove man.sty
%
%  Notes:
%
%    To generate the
...
\stardocinitials    {SUN}
\stardocnumber      {77.8}
\stardocauthors     {D.J. Allan}
\stardocdate        {11 September 1995}
\stardoctitle       {NBS \\ [1ex] The Noticeboard System}
\stardocversion     {Version 2.5.5}
\stardocsource      {sun\stardocnumber}
\stardocmanual      {Programmer's Manual}
\stardocabstract  {This document describes Version 2.5.5 of the Noticeboard System. It
...
\providecommand{\noteroutine}[3]{\item[\htmlref{#1}{#2}:]\mbox{} #3}
\providecommand{\mansection}[2]{\subsection{#1 --- #2}}
...
\texttt{AUTOFIB} control software. The notation used is the same as that used by
the \texttt{NBTRACE} (Trace Noticeboard) program (see Section~\ref
...
the maximum number of dimensions (\texttt{maxd}) is greater than zero. The three
slash-separated numbers are actual number of bytes, maximum number of bytes and
modified count.

The result of running \texttt{NBTRACE} on this noticeboard might be:
...
\newpage
\section {\texttt{NBS} Routines Listed by Category}
...
\mansection {{\texttt{NBS\_TUNE}}}{      Routines used to alter global %
noticeboard system parameters}

      Two types of parameters can be altered. The first type is global to a
      process and not to a noticeboard (they are essentially Fortran {\texttt{%
COMMON}}
      ...
      original {\texttt{NBS\_TUNE}} routine is used to alter parameters of the %
first type
      and the newer {\texttt{NBS\_TUNE\_NOTICEBOARD}} is used to tune %
parameters of the
...
\noteroutine{NBS\_RESTORE\_DEFINITION}{NBS_RESTORE_DEFINITION}
      {Restore a noticeboard definition from file
      and create the noticeboard}
\noteroutine{NBS\_RESTORE\_NOTICEBOARD}{NBS_RESTORE_NOTICEBOARD}
      {Restore a noticeboard definition and data from file
      and create the noticeboard}
\noteroutine{NBS\_SAVE\_NOTICEBOARD}{NBS_SAVE_NOTICEBOARD}
      {Save a noticeboard to its noticeboard definition file}
\end{mansectionroutines}
\mansection {{\texttt{NBS\_FIND}}}{      Routines that find noticeboards and %
items within noticeboards}
      ...
      non {\texttt{NIL}} pointers are relocated by the virtual address of the %
start of
...
\begin{mansectionroutines}
\noteroutine{NBS\_FIND\_NOTICEBOARD}{NBS_FIND_NOTICEBOARD}
      {Find a named noticeboard and return an identifier to it}
\noteroutine{NBS\_FIND\_ITEM}{NBS_FIND_ITEM}
      ...
      \texttt{ID}}
\noteroutine{NBS\_FIND\_NTH\_ITEM}{NBS_FIND_NTH_ITEM}
      {Find the Nth item contained in a structure
      associated with a specified identifier and return the located item's \texttt{ID}}
\end{mansectionroutines}
\mansection {{\texttt{NBS\_LOSE}}}{      Routines that lose noticeboards and %
items within noticeboards}

      The noticeboard system maintains counts of how many items have been
      found using {\texttt{NBS\_FIND\_ITEM}} and {\texttt{NBS\_FIND\_NTH\_ITEM}}%
. The routines in this
      ...
      chance of using an item {\texttt{ID}} to access a non-existent %
noticeboard), but
      this can be overridden.

\begin{mansectionroutines}
\noteroutine{NBS\_LOSE\_NOTICEBOARD}{NBS_LOSE_NOTICEBOARD}
      {Unmap a specified noticeboard}
\noteroutine{NBS\_LOSE\_ITEM}{NBS_LOSE_ITEM}
      {Declare an intention never again to use a specified item}
\end{mansectionroutines}
\mansection {{\texttt{NBS\_PUT}}}{      Routines that write information to a %
noticeboard}
      ...
      {\texttt{ADD\_INTERLOCKED}} instruction. The noticeboard's modified %
count is
...
of these
      ...
      1. {\texttt{WORLD\_WRITE}} is {\texttt{FALSE}} by default and this %
prevents processes other
         than the noticeboard's owner from writing to it. If set {\texttt{TRUE}%
}, this
         process will be allowed to write to the noticeboard even if it does
         not own it. If set {\texttt{TRUE}} then it is possible for a reader %
to read data
         ...
         {\texttt{WORLD\_WRITE}} flag.

      2. {\texttt{INCREMENT\_MODIFY}} is {\texttt{TRUE}} by default and this %
causes the item's
         ...
         set {\texttt{FALSE}}, neither count is altered. Clearly, if this is %
done, other
      ...
      modified count or a primitive item's modified count. This \emph{must} be
      called according to the same conventions as are applied in {\texttt{NBS\_%
PUT\_VALUE}}.
...
\noteroutine{NBS\_PUT\_TRIGGER}{NBS_PUT_TRIGGER}
      {Specify a routine to be called whenever a primitive item is updated}
\end{mansectionroutines}
\mansection {{\texttt{NBS\_GET}}}{      Routines that read information from a %
noticeboard}
...
\noteroutine{NBS\_GET\_POINTER}{NBS_GET_POINTER}
      {Return a pointer to the first byte of the data of a primitive item
      associated with the specified identifier}
\noteroutine{NBS\_GET\_NAME}{NBS_GET_NAME}
      {Get the name of an item
      associated with the specified identifier}
\noteroutine{NBS\_GET\_TYPE}{NBS_GET_TYPE}
      {Get the type of an item
      associated with the specified identifier}
\noteroutine{NBS\_GET\_SIZE}{NBS_GET_SIZE}
      {Get the maximum and actual sizes of a primitive item
      associated with the specified identifier}
\noteroutine{NBS\_GET\_PRIMITIVE}{NBS_GET_PRIMITIVE}
      {Determine whether or not an item is primitive}
\noteroutine{NBS\_GET\_PARENT}{NBS_GET_PARENT}
      {Get the identifier of an item's parent structure}
\noteroutine{NBS\_GET\_CHILDREN}{NBS_GET_CHILDREN}
      {Get the number of children of a structured item}
\noteroutine{NBS\_GET\_INFO}{NBS_GET_INFO}
      {Get general non-character information on a given noticeboard}
\noteroutine{NBS\_GET\_CINFO}{NBS_GET_CINFO}
...
are the \emph{lowest} measured values of \texttt{CPU} microseconds:
...
These are fairly consistent with the times reported for the \texttt{MON} system.
...
code in V2.3.1 onwards. The speed of the \texttt{UNIX} machine is particularly
...
\appendix
\newpage
\section {Description of Individual \texttt{NBS} Routines}
...
\begin{tabbing}
\texttt{NBS\_\_XXXXXXXXXXXXXXXXXXX} \= \kill
\texttt{NBS\_\_BADOPTION} \= Illegal parameter name \\
\end{tabbing}
      }
   }
   \sstdiytopic{
...
\texttt{NBS\_\_NILID} \> NIL ID \\
\texttt{NBS\_\_BADOPTION} \> Illegal parameter name \\
\texttt{NBS\_\_DEFINING  } \> Already defining a noticeboard \\
\texttt{NBS\_\_INITALLOCFAILED} \> Storage allocation failed \\
\texttt{NBS\_\_NOMOREROOM} \> Storage area is full up \\
\end{tabbing}
      }
   }
}
...
\begin{tabbing}
\texttt{NBS\_\_XXXXXXXXXXXXXXXXXXX} \= \kill
\texttt{NBS\_\_NOTDEFINING} \> Not currently defining a noticeboard \\
\texttt{NBS\_\_NILSID} \> NIL static ID \\
\texttt{NBS\_\_PRIMITIVE} \> Prospective parent is primitive \\
\texttt{NBS\_\_NOMOREROOM} \> Storage area is full up \\
\end{tabbing}
      }
   }
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         Maximum number of bytes in this item\texttt{'}s value
      }
      \sstsubsection{
         SID = INTEGER (Returned)
      }{
...
\texttt{NBS\_\_NILSID	   } \> NIL static ID \\
\texttt{NBS\_\_PRIMITIVE } \> Prospective parent is primitive \\
\texttt{NBS\_\_TOOMANYDIMS} \> Too many dimensions \\
\end{tabbing}
      }
   }
}
\sstroutine{
   ...
   \sstdescription{
      ...
      calling process\texttt{'} id to the global section to denote ownership and mark
...
\texttt{NBS\_\_XXXXXXXXXXXXXXXXXXX} \= \kill
\texttt{NBS\_\_NOTDEFINING} \> Not currently defining a noticeboard \\
\texttt{NBS\_\_CANTOPEN  } \> Can't create the definition file \\
\texttt{NBS\_\_CANTWRITE } \> Can't write the definition file \\
\texttt{NBS\_\_SECTIONEXISTED} \> Noticeboard of this name already \\
                          \> existed. \\
 \> \\
\texttt{SS\$\_*} \> System service codes from \texttt{SYS\$CRMPSC} \\
\end{tabbing}
      }
   }
}
\sstroutine{
   ...
   \sstdescription{
      ...
      Write the calling process\texttt{'} id to the noticeboard to denote ownership and
...
\texttt{NBS\_\_XXXXXXXXXXXXXXXXXXX} \= \kill
\texttt{NBS\_\_CANTOPEN  } \> Can't create the definition file \\
\texttt{NBS\_\_DATASAVED } \> Noticeboard data was saved to the \\
      \> definition file --- cannot restore only defn \\
\texttt{NBS\_\_CANTREAD  } \> Can't read the definition file \\
\texttt{NBS\_\_BADVERSION} \> Wrong version in definition file \\
\texttt{NBS\_\_SECTIONEXISTED} \> Noticeboard of this name already \\
                          \> existed. \\
 \> \\
\texttt{SS\$\_*} \> System service codes from \texttt{SYS\$CRMPSC} \\
...
\texttt{NBS\_\_SECTIONEXISTED} \> Noticeboard of this name already \\
                          \> existed. \\
\texttt{NBS\_\_DATANOTSAVED} \> Noticeboard data not saved to the \\
			    \> definition file, so not restored \\
 \> \\
\texttt{SS\$\_*} \> System service codes from \texttt{SYS\$CRMPSC} or \\
 \> \texttt{SYS\$DELTVA} (VMS only).
\end{tabbing}
      }
   }
   \sstdiytopic{
...
\texttt{NBS\_\_XXXXXXXXXXXXXXXXXXX} \= \kill
\texttt{NBS\_\_NILID	   } \> NIL ID \\
\texttt{NBS\_\_CANTOPEN  } \> Can't open the definition file \\
\texttt{NBS\_\_NOTOWNER  } \> Caller does not own the noticeboard \\
\texttt{NBS\_\_DATANOTRESTORED} \> Noticeboard data was not restored \\
...
\sstroutine{
   ...
   \sstdescription{
      ...
      Get the process\texttt{'} id for future checking against the owner\texttt{'}s process id. \\
      If this is the owner process and it hasn\texttt{'}t already been done, unmap the
      copy mapped earlier.
...
\begin{tabbing}
\texttt{NBS\_\_XXXXXXXXXXXXXXXXXXX} \= \kill
\texttt{NBS\_\_SECTIONNOTFOUND} \> No section called NAME existed \\
\texttt{NBS\_\_TIMEOUT   } \> Timeout awaiting valid noticeboard \\
\texttt{NBS\_\_NOMOREROOM} \> Failed to allocate private memory area \\
\texttt{NBS\_\_BADVERSION} \> Wrong version in noticeboard \\
 \> \\
\texttt{SS\$\_*} \> System service codes from \texttt{SYS\$CRMPSC} or \\
 \> \texttt{SYS\$DELTVA} (VMS only).
\end{tabbing}
...
\sstroutine{
   ...
   Find an item with a specified name contained in a structure
   associated with a specified identifier and return the located item\texttt{'}s
   ID
}{
   \sstdescription{
      Check that the environment ID is not NIL and does not pertain to a
      primitive item. \\
      Search for an item of the required name (don\texttt{'}t assume that they are in
...
\texttt{NBS\_\_XXXXXXXXXXXXXXXXXXX} \= \kill
\texttt{NBS\_\_NILID	 } \> NIL ID \\
\texttt{NBS\_\_PRIMITIVE	 } \> Parent is primitive \\
\texttt{NBS\_\_ITEMNOTFOUND} \> No item of this name exists \\
\end{tabbing}
      }
   }
}
\sstroutine{
   ...
   Find the Nth item contained in a structure
   associated with a specified identifier and return the located item\texttt{'}s ID
}{
   \sstdescription{
      Check that the environment ID is not NIL and does not pertain to a
      primitive item. \\
...
\sstroutine{
   ...
   \sstdescription{
      ...
      noticeboard (not if the \texttt{"}FORCE\texttt{"} option is specified). \\
...
\texttt{NBS\_\_XXXXXXXXXXXXXXXXXXX} \= \kill
\texttt{NBS\_\_NILID	 } \> NIL ID \\
\texttt{NBS\_\_NOTTOPLEVEL } \> ID is not a top-level identifier \\
\texttt{NBS\_\_HASIDS	 } \> Noticeboard has identifiers derived \\
				  \> from it \\
 \> \\
\texttt{SS\$\_*} \> System service codes from \texttt{SYS\$DELTVA} (VMS only). \\
\end{tabbing}
      }
   }
}
\sstroutine{
   ...
   \sstdescription{
      ...
      Check that at least one item is derived from this item\texttt{'}s parent. \\
      Check that no items are currently derived from this item (not if the
      \texttt{"}FORCE\texttt{"} option is specified). \\
      Decrement the parent\texttt{'}s count of derived items.
...
\texttt{NBS\_\_NILID	 } \> NIL ID \\
\texttt{NBS\_\_NOTTOPLEVEL } \> ID is not a top-level identifier \\
\texttt{NBS\_\_NEVERFOUND  } \> Item was never found (or more items \\
...
\sstroutine{
   ...
   \sstdescription{
      ...
      values. \\
      Increment the item\texttt{'}s modified count. \\
      Update the item size (maintain a high-water mark). \\
      Copy the values to the noticeboard. \\
      Increment the item\texttt{'}s modified count again. \\
...
\texttt{NBS\_\_XXXXXXXXXXXXXXXXXXX} \= \kill
\texttt{NBS\_\_NILID	 } \> NIL ID \\
\texttt{NBS\_\_NOTPRIMITIVE} \> Item is not primitive \\
\texttt{NBS\_\_NOTOWNER 	 } \> Caller does not own the noticeboard \\
\texttt{NBS\_\_BADOFFSET	 } \> Negative offset specified \\
\texttt{NBS\_\_TOOMANYBYTES} \> Not room to put all the data \\
\end{tabbing}
      }
   }
   \sstnotes{
...
\texttt{NBS\_\_NOTPRIMITIVE} \> Item is not primitive \\
\texttt{NBS\_\_NOTOWNER 	 } \> Caller does not own the noticeboard \\
\texttt{NBS\_\_TOOMANYDIMS } \> NDIMS is greater than item max dims \\
\end{tabbing}
      }
   }
}
\sstroutine{
   ...
   \sstdescription{
      ...
      Increment the item\texttt{'}s modified count. \\
      Update the internal record of the item\texttt{'}s size. \\
      Increment the item\texttt{'}s modified count again. \\
      Increment the noticeboard modified count.
...
\texttt{NBS\_\_NOTPRIMITIVE} \> Item is not primitive \\
\texttt{NBS\_\_NOTOWNER 	 } \> Caller does not own the noticeboard \\
\texttt{NBS\_\_TOOMANYBYTES} \> NBYTES is greater than item size \\
\end{tabbing}
      }
   }
}
\sstroutine{
   ...
   \sstdescription{
      ...
      If the item is primitive increment the item\texttt{'}s modified count.

      Note that this is a very dangerous routine when called on behalf of
      primitive items. Calls to it \emph{must} be paired. Any reader of an item
...
\sstroutine{
   ...
   \sstdescription{
      ...
      In this context \texttt{"}update\texttt{"} means any change to the item\texttt{'}s shape, data, size
...
\sstroutine{
   ...
   \sstdescription{
      ...
      XXX \= \kill
        ...
        \> Copy as many bytes as there is room for in the user\texttt{'}s buffer
      ...
      If CHECK\_MODIFY is FALSE, the item\texttt{'}s modified count is not checked
      at all and a timeout cannot occur.

      If the specified offset is greater than the current size of the item\texttt{'}s
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         Size in bytes of the user\texttt{'}s buffer.
      }
      \sstsubsection{
         BYTE\_ARRAY = BYTE($*$) (Returned)
      }{
         User\texttt{'}s buffer into which bytes will be got.
      }
      \sstsubsection{
         ...
         will be copied into the user\texttt{'}s buffer.
      }
      \sstsubsection{
         STATUS = INTEGER (Given and returned)
      }{
...
\texttt{NBS\_\_NOTPRIMITIVE} \> Item is not primitive \\
\texttt{NBS\_\_BADOFFSET	 } \> Negative offset specified \\
\texttt{NBS\_\_TIMEOUT	 } \> Timeout awaiting valid data \\
\end{tabbing}
      }
   }
}
...
\sstroutine{
   ...
   \sstdescription{
      ...
      XXX \= \kill
        \> \{ \\
             \> Read the modified count for this item. \\
             \> Copy as many dimensions as there is room for in the user\texttt{'}s
...
\sstroutine{
   ...
   \sstdescription{
      ...
      If the item is primitive get the item\texttt{'}s modified count.
      ...
      An \texttt{"}update\texttt{"} is an update of an item\texttt{'}s value, shape or size.
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         The current value of the noticeboard or item\texttt{'}s modified count.
      }
      \sstsubsection{
         STATUS = INTEGER (Given and returned)
      }{
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         The address of the item\texttt{'}s modified count.
      }
      \sstsubsection{
         STATUS = INTEGER (Given and returned)
      }{
...
\sstroutine{
   ...
   \sstdescription{
      Check that the ID is not NIL and that it pertains to a primitive item. \\
      Return the address of the start of the item\texttt{'}s noticeboard data.
   }
   \sstinvocation{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         The address of the first byte of this item\texttt{'}s noticeboard data.
      }
      \sstsubsection{
         STATUS = INTEGER (Given and returned)
      }{
...
\sstroutine{
   ...
   \sstdescription{
      Check that the ID is not NIL and that it pertains to a primitive item. \\
      Return the maximum and actual sizes of the item\texttt{'}s noticeboard data.
   }
   \sstinvocation{
...
\texttt{NBS\_\_XXXXXXXXXXXXXXXXXXX} \= \kill
\texttt{NBS\_\_NILID	 } \> NIL ID \\
\texttt{NBS\_\_NOPRIMITIVE } \> Item is not primitive \\
\end{tabbing}
      }
   }
   \sstdiytopic{
...
\sstroutine{
   ...
   Get the identifier of an item\texttt{'}s parent structure
}{
   \sstdescription{
      Check that the ID is not NIL. \\
      Return the identifier of the item\texttt{'}s parent.
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         Identifier of item\texttt{'}s parent. If the item has no parent then
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         The item\texttt{'}s value. Declared as pointer to integer, but may be
...
\texttt{NBS\_\_XXXXXXXXXXXXXXXXXXX} \= \kill
\texttt{NBS\_\_NILID	 } \> NIL ID \\
\texttt{NBS\_\_BADOPTION	 } \> Illegal item name \\
\end{tabbing}
      }
   }
   \sstdiytopic{
...
\newpage
\section {\texttt{NBS} Error Codes}

This is edited from the source file that is processed by the \texttt{MESSAGE}
utility and the (slightly modified) \texttt{ADAM} \texttt{ERRGEN} utility to generate
the C (\texttt{NBS\_DIR:\-NBS\_ERR.H}) and Fortran (\texttt{NBS\_DIR:\-NBS\_ERR.INC}) versions of the error code \texttt{INCLUDE} files.
...
Error           & \tt DEFINING          & Currently defining noticeboard contents \\
                ...
                & \tt NILSID            & \texttt{NIL} static \texttt{ID} \\
                & \tt NILID             & \texttt{NIL} item \texttt{ID} \\
