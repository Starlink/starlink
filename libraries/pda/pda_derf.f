...
C***BEGIN PROLOGUE  PDA_DERF
C***PURPOSE  Compute the error function.
C***LIBRARY   SLATEC (FNLIB)
C***CATEGORY  C8A, L5A1E
C***TYPE      DOUBLE PRECISION (ERF-S, PDA_DERF-D)
C***KEYWORDS  ERF, ERROR FUNCTION, FNLIB, SPECIAL FUNCTIONS
C***AUTHOR  Fullerton, W., (LANL)
C***DESCRIPTION
C
C PDA_DERF(X) calculates the double precision error function for double
...
C
C***REFERENCES  (NONE)
C***ROUTINES CALLED  PDA_D1MACH, PDA_DCSEVL, PDA_DERFC, PDA_INITDS
...
C   950425  Implement status.  (HME)
C***END PROLOGUE  PDA_DERF
      INTEGER STATUS
      DOUBLE PRECISION X, ERFCS(21), SQEPS, SQRTPI, XBIG, Y, PDA_D1MACH,
     1  PDA_DCSEVL, PDA_DERFC
      LOGICAL FIRST
      EXTERNAL PDA_DERFC
      ...
      DATA FIRST /.TRUE./
C***FIRST EXECUTABLE STATEMENT  PDA_DERF
      IF (FIRST) THEN
         NTERF = PDA_INITDS (ERFCS, 21, 0.1*REAL(PDA_D1MACH(3)), STATUS)
         XBIG = SQRT(-LOG(SQRTPI*PDA_D1MACH(3)))
         SQEPS = SQRT(2.0D0*PDA_D1MACH(3))
      ENDIF
      FIRST = .FALSE.
C
      Y = ABS(X)
...
C
      IF (Y.LE.SQEPS) PDA_DERF = 2.0D0*X*X/SQRTPI
      IF (Y.GT.SQEPS) PDA_DERF = X*(1.0D0 + PDA_DCSEVL (2.D0*X*X-1.D0,
     1  ERFCS, NTERF, STATUS))
      RETURN
C
C ERF(X) = 1.0 - ERFC(X) FOR ABS(X) .GT. 1.0
C
 20   IF (Y.LE.XBIG) PDA_DERF = SIGN (1.0D0-PDA_DERFC(Y,STATUS), X)
      IF (Y.GT.XBIG) PDA_DERF = SIGN (1.0D0, X)
C
      RETURN
      END
