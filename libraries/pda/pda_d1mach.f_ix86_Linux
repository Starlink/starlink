...
C***BEGIN PROLOGUE  PDA_D1MACH
C***PURPOSE  Return floating point machine dependent constants.
C***LIBRARY   SLATEC
C***CATEGORY  R1
C***TYPE      DOUBLE PRECISION (R1MACH-S, PDA_D1MACH-D)
...
C***DESCRIPTION
C
C   PDA_D1MACH can be used to obtain machine-dependent parameters for the
...
C   discussed below.
C
C   PDA_D1MACH( 1) = B**(EMIN-1), the smallest positive magnitude.
C   PDA_D1MACH( 2) = B**EMAX*(1 - B**(-T)), the largest magnitude.
C   PDA_D1MACH( 3) = B**(-T), the smallest relative spacing.
C   PDA_D1MACH( 4) = B**(1-T), the largest relative spacing.
C   PDA_D1MACH( 5) = LOG10(B)
C
...
C   EMIN .LE. E .LE. EMAX.
C
C   The values of B, T, EMIN and EMAX are provided in PDA_I1MACH as
C   follows:
C   PDA_I1MACH(10) = B, the base.
C   PDA_I1MACH(14) = T, the number of base-B digits.
C   PDA_I1MACH(15) = EMIN, the smallest exponent E.
C   PDA_I1MACH(16) = EMAX, the largest exponent E.
C
C   To alter this function for a particular environment, the desired
C   set of DATA statements should be activated by removing the C from
C   column 1.  Also, the values of PDA_D1MACH(1) - PDA_D1MACH(4) should be
...
C   950404  If index out of range, return value zero, but return.
C           (HME).
C   960123  Added Linux support (BKM).
C***END PROLOGUE  PDA_D1MACH
C
      INTEGER SMALL(4)
      INTEGER LARGE(4)
      INTEGER RIGHT(4)
...
C
C     MACHINE CONSTANTS FOR THE IX86 RUNNING LINUX
C
...
C
C
C***FIRST EXECUTABLE STATEMENT  PDA_D1MACH
