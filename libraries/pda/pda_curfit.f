...
c  numbers w(i),i=1,2,...,m,subroutine pda_curfit determines a smooth spline
c  approximation of degree k on the interval xb <= x <= xe.
c  if iopt=-1 pda_curfit calculates the weighted least-squares spline
...
c
c  calling sequence:
c     call pda_curfit(iopt,m,x,y,w,xb,xe,k,s,nest,n,t,c,fp,wrk,
c    * lwrk,iwrk,ier)
c
c  parameters:
...
c   time if pda_curfit is called repeatedly for different values of s.
...
c   pda_curfit once more with the selected value for s but now with iopt=0.
c   indeed, pda_curfit may then return an approximation of the same quality
...
c we partition the working space and determine the spline approximation.
  40  ifp = 1
      ...
      ig = ib+nest*k2
      iq = ig+nest*k2
      call pda_fpcurf(iopt,x,y,w,m,xb,xe,k,s,nest,tol,maxit,k1,k2,n,t,
