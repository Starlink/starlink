...
C  Minor update by Horst Meyerdierks, UoE, Starlink:
C     1. Make the function to be optimised an argument rather than using
C        a constant name 'FCN'. This is the new first argument.
C
C  Synopsis:
...
C  Software.
C
C  A very quick (perhaps too quick) overview of PDA_SA:
C     PDA_SA tries to find the global optimum of an N dimensional function.
...
C  Thus, as T declines, VM falls and PDA_SA focuses upon the most promising
C  area for optimization.
C
C  The importance of the parameter T:
C     The parameter T is crucial in using PDA_SA successfully. It influences
...
C    IPRINT - controls printing inside PDA_SA. (INT)
...
C    ISEED1 - The first seed for the random number generator PDA_RANMAR.
C             0 .LE. ISEED1 .LE. 31328. (INT)
C    ISEED2 - The second seed for the random number generator PDA_RANMAR.
...
C             to test the results of PDA_SA. (INT)
C
...
C         important (i.e. is the value is off, PDA_SA adjusts VM to the
C         correct value). (DP(N))
C    STATUS - Should be given as zero. The value is unchanged, unless
C             an error occurs in PDA_RMARIN. In that case the return value
C             is one.
C
C  Output Parameters:
...
C       RWK1 (DP(NEPS))  (FSTAR in PDA_SA)
C       RWK2 (DP(N))     (XP    "  " )
C       IWK  (INT(N))    (NACP  "  " )
C
C  Required Functions (included):
C    PDA_EXPREP - Replaces the function EXP to avoid under- and overflows.
C             It may have to be modified for non IBM-type main-
C             frames. (DP)
C    PDA_RMARIN - Initializes the random number generator PDA_RANMAR.
C    PDA_RANMAR - The actual random number generator. Note that
C             PDA_RMARIN must run first (PDA_SA does this). It produces uniform
...
C    PDA_PRTVEC - Prints vectors.
C    PDA_PRT1 ... PDA_PRT10 - Prints intermediate output.
C
C  Machine Specific Features:
C    1. PDA_EXPREP may have to be modified if used on non-IBM type main-
C       frames. Watch for under- and overflows in PDA_EXPREP.
C    2. Some FORMAT statements use G25.18; this may be excessive for
C       some machines.
C    3. PDA_RMARIN and PDA_RANMAR are designed to be protable; they should not
C       cause any problems.
C
C  Modification:
C    Use the new STATUS argument for the case that the seeds are out of
C    range.  (HME)
...
C  Type all functions.
      DOUBLE PRECISION  PDA_EXPREP
      REAL  PDA_RANMAR

C  Initialize the random number generator PDA_RANMAR.
      CALL PDA_RMARIN(ISEED1,ISEED2,STATUS)
...
C  to eliminate any possible confusion for the user.
      ...
      FOPT = F
      FSTAR(1) = F
      IF(IPRINT .GE. 1) CALL PDA_PRT2(MAX,N,X,F)
...
C  If XP is out of bounds, select a point in bounds for the trial.
                  IF((XP(I) .LT. LB(I)) .OR. (XP(I) .GT. UB(I))) THEN
                    XP(I) = LB(I) + (UB(I) - LB(I))*PDA_RANMAR()
                    LNOBDS = LNOBDS + 1
                    NOBDS = NOBDS + 1
                    IF(IPRINT .GE. 3) CALL PDA_PRT3(MAX,N,XP,X,FP,F)
                  END IF
110            CONTINUE
...
C  Evaluate the function with the trial point XP and return as FP.
               ...
               IF(.NOT. MAX) FP = -FP
               NFCNEV = NFCNEV + 1
               IF(IPRINT .GE. 3) CALL PDA_PRT4(MAX,N,XP,X,FP,F)
...
C  acceptance or rejection.
               ELSE
                  ...
                  PP = PDA_RANMAR()
                  IF (PP .LT. P) THEN
                     IF(IPRINT .GE. 3) CALL PDA_PRT6(MAX)
                     DO 140, I = 1, N
                        X(I) = XP(I)
140                  CONTINUE
                     F = FP
                  ...
                  ELSE
                     NREJ = NREJ + 1
                     IF(IPRINT .GE. 3) CALL PDA_PRT7(MAX)
                  END IF
               END IF
...
400   CONTINUE
      ...
      IF(IPRINT .GE. 1) THEN
         CALL PDA_PRT9(MAX,N,T,XOPT,VM,FOPT,NUP,NDOWN,NREJ,LNOBDS,NNEW)
      END IF
...
C  Terminate PDA_SA if appropriate.
      IF (QUIT) THEN
         DO 420, I = 1, N
            X(I) = XOPT(I)
420      CONTINUE
