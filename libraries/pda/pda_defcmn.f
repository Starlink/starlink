*DECK PDA_DEFCMN
      SUBROUTINE PDA_DEFCMN (NDATA, XDATA, YDATA, SDDATA, NORD, NBKPT,
     +   BKPTIN, MDEIN, MDEOUT, COEFF, BF, XTEMP, PTEMP, BKPT, G, MDG,
     +   W, MDW, LW, STATUS)
C***BEGIN PROLOGUE  PDA_DEFCMN
C***SUBSIDIARY
C***PURPOSE  Subsidiary to PDA_DEFC
C***LIBRARY   SLATEC
C***TYPE      DOUBLE PRECISION (EFCMN-S, PDA_DEFCMN-D)
C***AUTHOR  Hanson, R. J., (SNLA)
C***DESCRIPTION
C
C     This is a companion subprogram to PDA_DEFC( ).
C     This subprogram does weighted least squares fitting of data by
C     B-spline curves.
C     The documentation for PDA_DEFC( ) has complete usage instructions.
C
C***SEE ALSO  PDA_DEFC
C***ROUTINES CALLED  PDA_DBNDAC, PDA_DBNDSL, PDA_DCOPY, PDA_DFSPVN, PDA_DSCAL, PDA_DSORT, PDA_XERMSG
...
C   900510  Convert XERRWV calls to PDA_XERMSG calls.  (RWC)
C   900604  DP version created from SP version.  (RWC)
C   950403  Implement status.  (HME)
C   950517  Return immediately if PDA_DBNDAC or PDA_DBNDSL return a status. (HME)
C***END PROLOGUE  PDA_DEFCMN
      INTEGER STATUS
     ...
     *   YDATA(*)
C
      EXTERNAL PDA_DBNDAC, PDA_DBNDSL, PDA_DCOPY, PDA_DFSPVN,
     *   PDA_DSCAL, PDA_DSORT, PDA_XERMSG
C
...
C
C***FIRST EXECUTABLE STATEMENT  PDA_DEFCMN
C
...
C
      CALL PDA_DCOPY (N, 0.D0, 0, COEFF, 1)
      MDEOUT = -1
      IF (NORD.LT.1 .OR. NORD.GT.20) THEN
         CALL PDA_XERMSG ('SLATEC', 'PDA_DEFCMN',
     +      'IN PDA_DEFC, THE ORDER OF THE B-SPLINE MUST BE 1 THRU 20.',
     +      3, 1, STATUS)
         RETURN
      ENDIF
C
      IF (NBKPT.LT.2*NORD) THEN
         CALL PDA_XERMSG ('SLATEC', 'PDA_DEFCMN',
     +      'IN PDA_DEFC, THE NUMBER OF KNOTS MUST BE AT LEAST ' //
     +      'TWICE THE B-SPLINE ORDER.', 4, 1, STATUS)
         RETURN
      ENDIF
C
      IF (NDATA.LT.0) THEN
         CALL PDA_XERMSG ('SLATEC', 'PDA_DEFCMN',
     +      'IN PDA_DEFC, THE NUMBER OF DATA POINTS ' //
     +      'MUST BE NONNEGATIVE.',
     +      5, 1, STATUS)
         RETURN
      ENDIF
C
     ...
     +     2*MAX(NBKPT,NDATA) + NBKPT + NORD**2
      IF (LW .LT. NB) THEN
         ...
         WRITE (XERN2, '(I8)') LW
         CALL PDA_XERMSG ('SLATEC', 'PDA_DEFCMN',
     *      'IN PDA_DEFC, INSUFFICIENT STORAGE FOR W(*).  CHECK ' //
     *      'FORMULA THAT READS LW.GE. ... .  NEED = ' // XERN1 //
     *      ' GIVEN = ' // XERN2, 6, 1, STATUS)
         MDEOUT = -1
         RETURN
      ENDIF
C
      IF (MDEIN.NE.1 .AND. MDEIN.NE.2) THEN
         CALL PDA_XERMSG ('SLATEC', 'PDA_DEFCMN',
     +      'IN PDA_DEFC, INPUT VALUE OF MDEIN MUST BE 1-2.',
     +      7, 1, STATUS)
         RETURN
      ENDIF
C
...
C
      CALL PDA_DCOPY (NBKPT, BKPTIN, 1, BKPT, 1)
      CALL PDA_DSORT (BKPT, DUMMY, NBKPT, 1, STATUS)
C
...
C
      CALL PDA_DCOPY (NDATA, XDATA, 1, XTEMP, 1)
      DO 100 I = 1,NDATA
         PTEMP(I) = I
  100 CONTINUE
C
      IF (NDATA.GT.0) THEN
         CALL PDA_DSORT (XTEMP, PTEMP, NDATA, 2, STATUS)
...
C
  ...
  120 CONTINUE
C
C     Initialize parameters of banded matrix processor, PDA_DBNDAC( ).
C
      MT = 0
      IP = 1
      IR = 1
...
C
         IF (XVAL.GE.BKPT(ILEFT+1)) THEN
            CALL PDA_DBNDAC (G, MDG, NORD, IP, IR, MT, ILEFT-NORDM1,
     *         STATUS)
...
C
                  CALL PDA_DCOPY (NORDP1, W(INTSEQ,1), MDW,
     *               G(IR,1), MDG)
                  CALL PDA_DBNDAC (G, MDG, NORD, IP, IR, 1,
     *               INTSEQ, STATUS)
                  IF (STATUS .NE. 0) RETURN
                  INTSEQ = INTSEQ + 1
               ENDIF
...
C
  140    CALL PDA_DFSPVN (BKPT, NORD, 1, XVAL, ILEFT, BF)
C
C        Move row into place.
C
         IROW = IR + MT
         MT = MT + 1
         CALL PDA_DCOPY (NORD, BF, 1, G(IROW,1), MDG)
         G(IROW,NORDP1) = YDATA(L)
C
C        Scale data if uncertainty is nonzero.
C
         IF (SDDATA(L).NE.0.D0) CALL PDA_DSCAL (NORDP1, 1.D0/SDDATA(L),
     +                               G(IROW,1), MDG)
C
C        When staging work area is exhausted, process rows.
C
         IF (IROW.EQ.MDG-1) THEN
            CALL PDA_DBNDAC (G, MDG, NORD, IP, IR, MT,
     *         ILEFT-NORDM1, STATUS)
            IF (STATUS .NE. 0) RETURN
            MT = 0
         ENDIF
...
C
      CALL PDA_DBNDAC (G, MDG, NORD, IP, IR, MT, ILEFT-NORDM1, STATUS)
      IF (STATUS .NE. 0) RETURN
C
...
C
      IF (MDEIN.EQ.2) THEN
         DO 160 I = INTSEQ,NP1
            CALL PDA_DCOPY (NORDP1, W(I,1), MDW, G(IR,1), MDG)
            CALL PDA_DBNDAC (G, MDG, NORD, IP, IR, 1, MIN(N,I), STATUS)
            IF (STATUS .NE. 0) RETURN
  160    CONTINUE
      ENDIF
C
...
C
      CALL PDA_DCOPY (NORDP1, 0.D0, 0, G(IR,1), MDG)
      CALL PDA_DBNDAC (G, MDG, NORD, IP, IR, 1, NP1, STATUS)
      IF (STATUS .NE. 0) RETURN
C
...
C
      DO 170 I = 1,NP1
         CALL PDA_DCOPY (NORDP1, G(I,1), MDG, W(I,1), MDW)
  170 CONTINUE
C
...
C
      CALL PDA_DBNDSL (1, G, MDG, NORD, IP, IR, COEFF, N, RNORM, STATUS)
      IF (STATUS .NE. 0) RETURN
      MDEOUT = 1
      RETURN
      END
