...
c  function pda_splint calculates the integral of a spline function s(x)
c  of degree k, which is given in its normalized b-spline representation
c
c  calling sequence:
c     aint = pda_splint(t,n,c,k,a,b,wrk)
c
c  input parameters:
...
c
c  other subroutines required: pda_fpintb.
c
c  references :
...
c  ni,k+1(x), i=1,2,...nk1.
      call pda_fpintb(t,n,wrk,nk1,a,b)
c  calculate the integral of s(x).
      pda_splint = 0.
      do 10 i=1,nk1
        pda_splint = pda_splint+c(i)*wrk(i)
  10  continue
      return
      end
