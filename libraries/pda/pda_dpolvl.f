...
C            call to PDA_DPLINT.
C***LIBRARY   SLATEC
C***CATEGORY  E3
C***TYPE      DOUBLE PRECISION (POLYVL-S, PDA_DPOLVL-D)
C***KEYWORDS  POLYNOMIAL EVALUATION
C***AUTHOR  Huddleston, R. E., (SNLL)
C***DESCRIPTION
C
C     Abstract -
C        Subroutine PDA_DPOLVL calculates the value of the polynomial and
C     its first NDER derivatives where the polynomial was produced by
C     a previous call to PDA_DPLINT.
C        The variable N and the arrays X and C must not be altered
C     between the call to PDA_DPLINT and the call to PDA_DPOLVL.
C
...
C     X    - *       to PDA_DPLINT and the call to PDA_DPOLVL.
C     C    - *****
C
C
C     *****  Output Parameters
...
C             PDA_DPOLVL.  It must be dimensioned by at least 2*N if NDER is
