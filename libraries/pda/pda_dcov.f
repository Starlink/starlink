...
C
C
C       STATUS is an INTEGER error status. Set to zero on entry.
C              If an error has occurred and has been reported then
C              this will be non-zero on exit.
C
C***REFERENCES  (NONE)
...
C
     ...
     1 WA4(*)
      INTEGER STATUS
      EXTERNAL FCN
      DOUBLE PRECISION ONE,SIGMA,TEMP,ZERO,PDA_DENORM
      LOGICAL SING
      SAVE ZERO, ONE
...
100   CALL PDA_DFDJC3(FCN,M,N,X,FVEC,R,LDR,IFLAG,ZERO,WA4)
110   IF (IFLAG.LT.0) GO TO 300
C
C     COMPUTE THE QR DECOMPOSITION
      CALL PDA_DQRFAC(M,N,R,LDR,.FALSE.,IDUM,1,WA1,WA1,WA1)
      DO 120 I=1,N
120   R(I,I)=WA1(I)
      GO TO 225
C
...
210   CONTINUE
      ...
      IF (IFLAG.LT.0) GO TO 300
      TEMP=FVEC(I)
      CALL PDA_DWUPDT(N,R,LDR,WA1,WA2,TEMP,WA3,WA4)
220   CONTINUE
C
...
300   CONTINUE
      ...
      IF (SING) INFO=2
      IF (INFO .LT. 0) CALL PDA_XERMSG ('SLATEC', 'DCOV',
     +   'EXECUTION TERMINATED BECAUSE USER SET IFLAG NEGATIVE.',
     +   1, 1, STATUS)
      IF (INFO .EQ. 0) CALL PDA_XERMSG ('SLATEC', 'DCOV',
     +   'INVALID INPUT PARAMETER.', 2, 1, STATUS)
      IF (INFO .EQ. 2) CALL PDA_XERMSG ('SLATEC', 'DCOV',
     +   'SINGULAR JACOBIAN MATRIX, COVARIANCE MATRIX CANNOT BE ' //
     +   'CALCULATED.', 1, 1, STATUS)
      RETURN
      END
