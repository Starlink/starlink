*DECK PDA_DBSQAD
      SUBROUTINE PDA_DBSQAD (T, BCOEF, N, K, X1, X2, BQUAD, WORK,
     *   STATUS)
C***BEGIN PROLOGUE  PDA_DBSQAD
C***PURPOSE  Compute the integral of a K-th order B-spline using the
C            B-representation.
C***LIBRARY   SLATEC
C***CATEGORY  H2A2A1, E3, K6
C***TYPE      DOUBLE PRECISION (BSQAD-S, PDA_DBSQAD-D)
...
C
C         PDA_DBSQAD computes the integral on (X1,X2) of a K-th order
...
C         PDA_DBSQAD is the smaller of 18 and the number of digits
...
C                 December 1979.
C***ROUTINES CALLED  PDA_DBVALU, PDA_DINTRV, PDA_XERMSG
...
C
C***FIRST EXECUTABLE STATEMENT  PDA_DBSQAD
      BQUAD = 0.0D0
...
C
   ...
   20 CONTINUE
      ILO = 1
      INBV = 1
      CALL PDA_DINTRV(T, NPK, AA, ILO, IL1, MFLAG)
      CALL PDA_DINTRV(T, NPK, BB, ILO, IL2, MFLAG)
      IF (IL2.GE.NP1) IL2 = N
      DO 40 LEFT=IL1,IL2
        TA = T(LEFT)
        TB = T(LEFT+1)
        ...
        DO 30 M=1,MF
          C1 = BMA*GPTS(JF+M)
          GX = -C1 + BPA
          Y2 = PDA_DBVALU(T,BCOEF,N,K,0,GX,INBV,WORK,STATUS)
          GX = C1 + BPA
          Y1 = PDA_DBVALU(T,BCOEF,N,K,0,GX,INBV,WORK,STATUS)
...
C
   60 CONTINUE
      CALL PDA_XERMSG ('SLATEC', 'PDA_DBSQAD',
     +   'X1 OR X2 OR BOTH DO NOT SATISFY T(K).LE.X.LE.T(N+1)', 2,
     +   1, STATUS)
      RETURN
   65 CONTINUE
      CALL PDA_XERMSG ('SLATEC', 'PDA_DBSQAD',
     +   'K DOES NOT SATISFY 1.LE.K.LE.20', 2, 1, STATUS)
      RETURN
   70 CONTINUE
      CALL PDA_XERMSG ('SLATEC', 'PDA_DBSQAD',
