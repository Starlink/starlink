*DECK PDA_DH12
      SUBROUTINE PDA_DH12 (MODE, LPIVOT, L1, M, U, IUE, UP, C, ICE, ICV,
     +   NCV)
C***BEGIN PROLOGUE  PDA_DH12
C***SUBSIDIARY
C***PURPOSE  Subsidiary to DHFTI, DLSEI and DWNNLS
C***LIBRARY   SLATEC
C***TYPE      DOUBLE PRECISION (H12-S, PDA_DH12-D)
C***AUTHOR  (UNKNOWN)
C***DESCRIPTION
C
...
C***ROUTINES CALLED  PDA_DAXPY, PDA_DDOT, PDA_DSWAP
...
C   900911  Added PDA_DDOT to DOUBLE PRECISION statement.  (WRB)
C***END PROLOGUE  PDA_DH12
      INTEGER I, I2, I3, I4, ICE, ICV, INCR, IUE, J, KL1, KL2, KLP,
     *     L1, L1M1, LPIVOT, M, MML1P2, MODE, NCV
      DOUBLE PRECISION B, C, CL, CLINV, ONE, UL1M1, SM, U, UP, PDA_DDOT
      DIMENSION U(IUE,*), C(*)
C     BEGIN BLOCK PERMITTING ...EXITS TO 140
C***FIRST EXECUTABLE STATEMENT  PDA_DH12
         ONE = 1.0D0
C
C     ...EXIT
...
C     ......EXIT
   ...
   60    CONTINUE
         ...
         IF (MML1P2 .LE. 20) GO TO 80
            ...
            U(1,L1M1) = UP
            IF (LPIVOT .NE. L1M1)
     *         CALL PDA_DSWAP(NCV,C(KL1),ICV,C(KLP),ICV)
            DO 70 J = 1, NCV
               SM = PDA_DDOT(MML1P2,U(1,L1M1),IUE,C(KL1),ICE)
               SM = SM*B
               CALL PDA_DAXPY(MML1P2,SM,U(1,L1M1),IUE,C(KL1),ICE)
               KL1 = KL1 + ICV
   70       CONTINUE
            U(1,L1M1) = UL1M1
C     ......EXIT
