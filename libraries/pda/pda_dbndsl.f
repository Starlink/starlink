*DECK PDA_DBNDSL
      SUBROUTINE PDA_DBNDSL (MODE, G, MDG, NB, IP, IR, X, N, RNORM,
     1   STATUS)
C***BEGIN PROLOGUE  PDA_DBNDSL
...
C***LIBRARY   SLATEC
C***CATEGORY  D9
C***TYPE      DOUBLE PRECISION (BNDSOL-S, PDA_DBNDSL-D)
...
C     are MT by JT-1.
C
C     The user of the subroutine PDA_DBNDAC provides MT,JT,C and F for
...
C     executing the statement CALL PDA_DBNDSL(1,...). The sequence of
...
C     MU can be as small as N+2.  The subprogram PDA_DBNDAC processes the
...
C
C     CALL PDA_DBNDAC(...)  Introduce new blocks of data.
C
C     CALL PDA_DBNDSL(1,...)Compute solution vector and length of
C                       residual vector.
C
C     CALL PDA_DBNDSL(2,...)Given any row vector H solve YR = H for the
C                       row vector Y.
C
C     CALL PDA_DBNDSL(3,...)Given any column vector W solve RZ = W for
...
C     G(MDG,NB+1)
C
C     Description of calling sequence for PDA_DBNDAC..
C
C     The entire set of parameters for PDA_DBNDAC are
C
...
C                       first call to PDA_DBNDAC.  Its subsequent value
C                       is controlled by PDA_DBNDAC to set up for the
C                       next call to PDA_DBNDAC.
C
C     IR                Index of the row of G(*,*) where the user is
C                       the user to the value 1 before the first call
C                       to PDA_DBNDAC.  Its subsequent value is controlled
C                       by PDA_DBNDAC. A value of IR .GT. MDG is considered
C                       an error.
C
...
C                       matrix which has been passed to PDA_DBNDAC.
C
C     IP,IR             The values of these arguments are advanced by
C                       PDA_DBNDAC to be ready for storing and processing
...
C
C     Description of calling sequence for PDA_DBNDSL..
C
C     The user must dimension the arrays appearing in the call list..
C
C     G(MDG,NB+1), X(N)
C
C     The entire set of parameters for PDA_DBNDSL are
C
C     Input..
C
...
C      NB,IP,IR         contents as following the last call to PDA_DBNDAC.
C
...
C                       zero the subroutine PDA_DBNDSL prints an
...
C     JT=N+1
C
C     CALL PDA_DBNDAC(G,MDG,NB,IP,IR,MT,JT)
C
...
C***END PROLOGUE  PDA_DBNDSL
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER STATUS
      DIMENSION G(MDG,*),X(*)
C***FIRST EXECUTABLE STATEMENT  PDA_DBNDSL
      ZERO=0.D0
C
      RNORM=ZERO
      GO TO (10,90,50), MODE
...
C
  130 CONTINUE
      NERR=1
      IOPT=2
      CALL PDA_XERMSG ('SLATEC', 'PDA_DBNDSL',
     +   'A ZERO DIAGONAL TERM IS IN THE N BY N UPPER TRIANGULAR ' //
     +   'MATRIX.', NERR, IOPT, STATUS)
      RETURN
      END
