      DOUBLE PRECISION FUNCTION PDA_V11(N, IFAULT)
*+
*  Name:
*     PDA_V11
...
c
      ...
      if (n .gt. 370) then
         x = (x**mpt15 - one) / mpt15
         pda_v11 = exp(b0 + x*(b1 + x*(b2 + x*(b3 + x*b4))))
      else if (n .le. 100) then
         x = (x**pt09 - one) / pt09
         pda_v11 = exp(c0 + x*(c1 + x*(c2 + x*(c3 + x*(c4 + x*(c5 + x*
     +        (c6 + x*(c7 + x*(c8 + x*c9)))))))))
      else if (n .le. 200) then
         x = log(a0 + x)
         pda_v11 = exp(a1 + x*(a2 + x*(a3 + x*(a4 + x*(a5 + x*a6)))))
      else
         x = log(d0 + x)
