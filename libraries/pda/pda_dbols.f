...
C***TYPE      DOUBLE PRECISION (SBOLS-S, PDA_DBOLS-D)
...
C     for PDA_DBOLSM( ). Most of the time this feature can be ignored by
...
C     MESSAGES for PDA_DBOLS( ). AN approximate solution will be returned
C     to the user only when max. iterations is reached, MODE=-22.
C     Values for MODE=-37,...,-22 come from the low-level subprogram
C     PDA_DBOLSM(). See the section ERROR MESSAGES for PDA_DBOLSM() in the
C     documentation for PDA_DBOLSM().
C
C    ------
C    STATUS
C    ------
...
C     solution with one final call to subprogram PDA_DBOLS( ). The value of
...
C
C     The user now calls PDA_DBOLS( ) in a loop. The value of IOPT(LP+1)
...
C     by PDA_DBOLS() against their actual requirements for this problem.
...
C     low-level subprogram PDA_DBOLSM(). If LP is the processing pointer
C     for IOPT(*),
C
C        IOPT(LP)=5
C        IOPT(LP+1)= Position in IOPT(*) where option array
C                    data for PDA_DBOLSM() begins.
C         .
C        CALL PDA_DBOLS()
C
C     Use of this option adds 2 to the required length of IOPT(*).
...
C       IOPT(3)=99
C
C    ERROR MESSAGES for PDA_DBOLS()
C    ----- -------- --- -------
C
C WARNING IN...
C PDA_DBOLS(). MDW=(I1) MUST BE POSITIVE.
C           IN ABOVE MESSAGE, I1=         0
C ERROR NUMBER =         2
C (NORMALLY A RETURN TO THE USER TAKES PLACE FOLLOWING THIS MESSAGE.)
C
C WARNING IN...
C PDA_DBOLS(). NCOLS=(I1) THE NO. OF VARIABLES MUST BE POSITIVE.
C           IN ABOVE MESSAGE, I1=         0
C ERROR NUMBER =         3
C (NORMALLY A RETURN TO THE USER TAKES PLACE FOLLOWING THIS MESSAGE.)
C
C WARNING IN...
C PDA_DBOLS(). FOR J=(I1), IND(J)=(I2) MUST BE 1-4.
...
C
C WARNING IN...
C PDA_DBOLS(). FOR J=(I1), BOUND BL(J)=(R1) IS .GT. BU(J)=(R2).
C           IN ABOVE MESSAGE, I1=         1
...
C
C WARNING IN...
C PDA_DBOLS(). ISCALE OPTION=(I1) MUST BE 1-3.
C           IN ABOVE MESSAGE, I1=         0
C ERROR NUMBER =         7
C (NORMALLY A RETURN TO THE USER TAKES PLACE FOLLOWING THIS MESSAGE.)
C
C WARNING IN...
C PDA_DBOLS(). OFFSET PAST X(NCOLS) (I1) FOR USER-PROVIDED  COLUMN SCALING
C MUST BE POSITIVE.
C           IN ABOVE MESSAGE, I1=         0
C ERROR NUMBER =         8
C (NORMALLY A RETURN TO THE USER TAKES PLACE FOLLOWING THIS MESSAGE.)
C
C WARNING IN...
C PDA_DBOLS(). EACH PROVIDED COL. SCALE FACTOR MUST BE POSITIVE.
...
C
C WARNING IN...
C PDA_DBOLS().THE ROW DIMENSION OF W(,)=(I1) MUST BE .GE.THE NUMBER OF ROWS=
C (I2).
C           IN ABOVE MESSAGE, I1=         0
...
C
C WARNING IN...
C PDA_DBOLS(). THE COLUMN DIMENSION OF W(,)=(I1) MUST BE .GE. NCOLS+1=(I2).
C           IN ABOVE MESSAGE, I1=         0
...
C
C WARNING IN...
C PDA_DBOLS().THE DIMENSIONS OF THE ARRAYS BL(),BU(), AND IND()=(I1) MUST BE
C .GE. NCOLS=(I2).
C           IN ABOVE MESSAGE, I1=         0
C           IN ABOVE MESSAGE, I2=         1
C ERROR NUMBER =        13
...
C
C WARNING IN...
C PDA_DBOLS(). THE DIMENSION OF X()=(I1) MUST BE .GE. THE REQD. LENGTH=(I2).
C           IN ABOVE MESSAGE, I1=         0
C           IN ABOVE MESSAGE, I2=         2
C ERROR NUMBER =        14
C (NORMALLY A RETURN TO THE USER TAKES PLACE FOLLOWING THIS MESSAGE.)
C
C WARNING IN...
C PDA_DBOLS(). THE DIMENSION OF RW()=(I1) MUST BE .GE. 5*NCOLS=(I2).
C           IN ABOVE MESSAGE, I1=         0
...
C
C WARNING IN...
C PDA_DBOLS() THE DIMENSION OF IW()=(I1) MUST BE .GE. 2*NCOLS=(I2).
C           IN ABOVE MESSAGE, I1=         0
C           IN ABOVE MESSAGE, I2=         2
C ERROR NUMBER =        16
C (NORMALLY A RETURN TO THE USER TAKES PLACE FOLLOWING THIS MESSAGE.)
C
C WARNING IN...
C PDA_DBOLS() THE DIMENSION OF IOPT()=(I1) MUST BE .GE. THE REQD. LEN.=(I2).
C           IN ABOVE MESSAGE, I1=         0
C           IN ABOVE MESSAGE, I2=         1
C ERROR NUMBER =        17
C (NORMALLY A RETURN TO THE USER TAKES PLACE FOLLOWING THIS MESSAGE.)
...
C***ROUTINES CALLED  PDA_DBOLSM, PDA_DCOPY, PDA_DNRM2, PDA_DROT, PDA_DROTG,
C                    PDA_IDAMAX, PDA_XERMSG
...
C   970428  Add type declaration for PDA_IDAMAX function as INTEGER (BLY).
C***END PROLOGUE  PDA_DBOLS
C
...
C     CHANGE THIS SUBPROGRAM NAME TO PDA_DBOLS AND THE STRINGS
C     /SCOPY/ TO /PDA_DCOPY/, /SBOL/ TO /DBOL/,
C     /SNRM2/ TO /PDA_DNRM2/, /ISAMAX/ TO /PDA_IDAMAX/,
C     /SROTG/ TO /PDA_DROTG/, /SROT/ TO /PDA_DROT/, /E0/ TO /D0/,
...
C     THIS VARIABLE SHOULD REMAIN TYPE REAL.
      ...
      DATA IGO/0/
C***FIRST EXECUTABLE STATEMENT  PDA_DBOLS
      NERR = 0
      MODE = 0
      IF (IGO.EQ.0) THEN
...
C     SEE THAT MDW IS .GT.0. GROSS CHECK ONLY.
          IF (MDW.LE.0) THEN
              WRITE (XERN1, '(I8)') MDW
              CALL PDA_XERMSG ('SLATEC', 'PDA_DBOLS', 'MDW = '
     *           // XERN1 // ' MUST BE POSITIVE.', 2, 1, STATUS)
...
C     SEE THAT NUMBER OF UNKNOWNS IS POSITIVE.
          IF (NCOLS.LE.0) THEN
              ...
              CALL PDA_XERMSG ('SLATEC', 'PDA_DBOLS', 'NCOLS = '
     *           // XERN1 // ' THE NO. OF VARIABLES MUST BE POSITIVE.',
     *           3, 1, STATUS)
C     DO(RETURN TO USER PROGRAM UNIT)
              GO TO 190
          ENDIF
...
C     SEE THAT CONSTRAINT INDICATORS ARE ALL WELL-DEFINED.
          DO 10 J = 1,NCOLS
              IF (IND(J).LT.1 .OR. IND(J).GT.4) THEN
                  ...
                  CALL PDA_XERMSG ('SLATEC', 'PDA_DBOLS', 'IND('
     *               // XERN1 // ') = ' // XERN2 // ' MUST BE 1-4.',
     *               4, 1, STATUS)
C     DO(RETURN TO USER PROGRAM UNIT)
                  GO TO 190
              ENDIF
          ...
          DO 20 J = 1,NCOLS
              IF (IND(J).EQ.3) THEN
                  IF (BL(J).GT.BU(J)) THEN
                      ...
                      CALL PDA_XERMSG ('SLATEC', 'PDA_DBOLS',
...
C     CYCLE FOREVER
              GO TO 30
C
C     IN THIS OPTION AN OPTION ARRAY IS PROVIDED TO PDA_DBOLSM().
...
C     NO VALID OPTION NUMBER WAS NOTED. THIS IS AN ERROR CONDITION.
          ELSE
              ...
              CALL PDA_XERMSG ('SLATEC', 'PDA_DBOLS',
     *           'THE OPTION NUMBER = ' //
     *           XERN1 // ' IS NOT DEFINED.', 6, 1, STATUS)
C     DO(RETURN TO USER PROGRAM UNIT)
              GO TO 190
          ENDIF
   50     CONTINUE
...
C     DO(RETURN TO USER PROGRAM UNIT)
     ...
     *               ' MUST BE .GE. NCOLS+1 = ' // XERN2, 12, 1, STATUS)
                  GO TO 190
              ENDIF
...
C     DO(RETURN TO USER PROGRAM UNIT)
     ...
     *               ' MUST BE .GE. 5*NCOLS = ' // XERN2, 15, 1, STATUS)
C     DO(RETURN TO USER PROGRAM UNIT)
                  GO TO 190
              ENDIF
     ...
     *               ' MUST BE .GE. 2*NCOLS = ' // XERN2, 16, 1, STATUS)
C     DO(RETURN TO USER PROGRAM UNIT)
                  GO TO 190
              ENDIF
     ...
     *               ' MUST BE .GE. THE REQUIRED LEN = ' // XERN2,
     *               17, 1, STATUS)
C     DO(RETURN TO USER PROGRAM UNIT)
                  GO TO 190
              ENDIF
...
C     PIVOT FOR INCREASED STABILITY.
              ...
              CALL PDA_DROT(NCOLS+1-J,W(IBIG,J+1),MDW,W(I,J+1),
     *           MDW,SC,SS)
              W(I,J) = ZERO
   70     CONTINUE
   80 CONTINUE
...
C     TO =J,AND =1, FOR USE IN PDA_DBOLSM( ).
      DO 170 J = 1,NCOLS
          IW(J) = J
          IW(J+NCOLS) = 1
          RW(3*NCOLS+J) = BL(J)
          RW(4*NCOLS+J) = BU(J)
  170 CONTINUE
      CALL PDA_DBOLSM(W,MDW,MROWS,NCOLS,RW(3*NCOLS+1),RW(4*NCOLS+1),IND,
     .            IOPT(LOPT),X,RNORM,MODE,RW(NCOLS+1),RW(2*NCOLS+1),RW,
     .            IW,IW(NCOLS+1),STATUS)
C     END PROCEDURE
      IGO = 0
  180 CONTINUE
      RETURN
