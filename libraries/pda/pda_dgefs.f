*DECK PDA_DGEFS
      SUBROUTINE PDA_DGEFS (A, LDA, N, V, ITASK, IND, WORK, IWORK,
     *   STATUS)
C***BEGIN PROLOGUE  PDA_DGEFS
C***PURPOSE  Solve a general system of linear equations.
C***LIBRARY   SLATEC
C***CATEGORY  D2A1
C***TYPE      DOUBLE PRECISION (SGEFS-S, PDA_DGEFS-D, CGEFS-C)
C***KEYWORDS  COMPLEX LINEAR EQUATIONS, GENERAL MATRIX,
C             GENERAL SYSTEM OF LINEAR EQUATIONS
C***AUTHOR  Voorhees, E. A., (LANL)
C***DESCRIPTION
C
C    Subroutine PDA_DGEFS solves a general NxN system of double
C    precision linear equations using LINPACK subroutines PDA_DGECO
C    and PDA_DGESL.  That is, if A is an NxN double precision matrix
C    and if X and B are double precision N-vectors, then PDA_DGEFS
...
C    ing factorization (ITASK=1).  IND will not be changed by PDA_DGEFS
C    in this case.
C
...
C                      designed to be handled by PDA_XERMSG in which
C                      LEVEL=1 (recoverable) and IFLAG=2 .  LEVEL=0
C                      for warning error messages from PDA_XERMSG.  Unless
C                      the user provides otherwise, an error message
C                      will be printed followed by an abort.
C
C***REFERENCES  J. J. Dongarra, J. R. Bunch, C. B. Moler, and G. W.
C                 Stewart, LINPACK Users' Guide, SIAM, 1979.
C***ROUTINES CALLED  PDA_D1MACH, PDA_DGECO, PDA_DGESL, PDA_XERMSG
...
C***END PROLOGUE  PDA_DGEFS
C
      INTEGER STATUS
      INTEGER LDA,N,ITASK,IND,IWORK(*)
      DOUBLE PRECISION A(LDA,*),V(*),WORK(*),PDA_D1MACH
      DOUBLE PRECISION RCOND
      CHARACTER*8 XERN1, XERN2
C***FIRST EXECUTABLE STATEMENT  PDA_DGEFS
      IF (LDA.LT.N) THEN
         IND = -1
         WRITE (XERN1, '(I8)') LDA
         WRITE (XERN2, '(I8)') N
         CALL PDA_XERMSG ('SLATEC', 'PDA_DGEFS', 'LDA = ' // XERN1 //
     *      ' IS LESS THAN N = ' // XERN2, -1, 1, STATUS)
         RETURN
      ENDIF
C
      IF (N.LE.0) THEN
         IND = -2
         WRITE (XERN1, '(I8)') N
         CALL PDA_XERMSG ('SLATEC', 'PDA_DGEFS', 'N = ' // XERN1 //
     *      ' IS LESS THAN 1', -2, 1, STATUS)
         RETURN
      ENDIF
C
      IF (ITASK.LT.1) THEN
         ...
         CALL PDA_XERMSG ('SLATEC', 'PDA_DGEFS', 'ITASK = ' // XERN1 //
     *      ' IS LESS THAN 1', -3, 1, STATUS)
         RETURN
      ENDIF
C
      IF (ITASK.EQ.1) THEN
...
C
         CALL PDA_DGECO(A,LDA,N,IWORK,RCOND,WORK)
C
C        CHECK FOR COMPUTATIONALLY SINGULAR MATRIX
C
         IF (RCOND.EQ.0.0D0) THEN
            IND = -4
            CALL PDA_XERMSG ('SLATEC', 'PDA_DGEFS',
     *         'SINGULAR MATRIX A - NO SOLUTION', -4, 1, STATUS)
            RETURN
         ENDIF
C
...
C
         IND = -LOG10(PDA_D1MACH(4)/RCOND)
         IF (IND.LE.0) THEN
            IND=-10
            CALL PDA_XERMSG ('SLATEC', 'PDA_DGEFS',
     *         'SOLUTION MAY HAVE NO SIGNIFICANCE', -10, 0, STATUS)
         ENDIF
      ENDIF
C
C     SOLVE AFTER FACTORING
C
      CALL PDA_DGESL(A,LDA,N,IWORK,V,0)
      RETURN
      END
