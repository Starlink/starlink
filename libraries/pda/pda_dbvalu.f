...
C         PDA_DBVALU is the BVALUE function of the reference.
C
C         PDA_DBVALU evaluates the B-representation (T,A,N,K) of a B-spline
...
C         spline is defined on T(K) .LE. X .LE. T(N+1).  PDA_DBVALU returns
...
C                    to 1 the first time PDA_DBVALU is called.
C
C         Output     WORK,PDA_DBVALU are double precision
...
C      PDA_DBVALU  - value of the IDERIV-th derivative at X
...
C                 pp. 441-472.
C***ROUTINES CALLED  PDA_DINTRV, PDA_XERMSG
...
C
     ...
     1 IP1, IP1MJ, J, JJ, J1, J2, K, KMIDER, KMJ, KM1, KPK, MFLAG, N
      ...
      DIMENSION T(*), A(*), WORK(*)
C***FIRST EXECUTABLE STATEMENT  PDA_DBVALU
      PDA_DBVALU = 0.0D0
      IF(K.LT.1) GO TO 102
      IF(N.LT.K) GO TO 101
...
C     GIVEN ITS RELEVANT B-SPLINE COEFF. IN AJ(1),...,AJ(K-IDERIV).
   ...
   80   CONTINUE
   90 CONTINUE
  100 PDA_DBVALU = WORK(1)
      RETURN
C
C
  101 CONTINUE
      CALL PDA_XERMSG ('SLATEC', 'PDA_DBVALU',
     +   'N DOES NOT SATISFY N.GE.K', 2, 1, STATUS)
      RETURN
  102 CONTINUE
      CALL PDA_XERMSG ('SLATEC', 'PDA_DBVALU',
  ...
  110 CONTINUE
      CALL PDA_XERMSG ('SLATEC', 'PDA_DBVALU',
     +   'IDERIV DOES NOT SATISFY 0.LE.IDERIV.LT.K', 2, 1, STATUS)
      RETURN
  120 CONTINUE
      CALL PDA_XERMSG ('SLATEC', 'PDA_DBVALU',
     +   'X IS N0T GREATER THAN OR EQUAL TO T(K)', 2, 1, STATUS)
      RETURN
  130 CONTINUE
      CALL PDA_XERMSG ('SLATEC', 'PDA_DBVALU',
     +   'X IS NOT LESS THAN OR EQUAL TO T(N+1)', 2, 1, STATUS)
      RETURN
  140 CONTINUE
      CALL PDA_XERMSG ('SLATEC', 'PDA_DBVALU',
