...
C   960916  Renamed PDA_DNLS1 and added STATUS argument. (PWD)
C***END PROLOGUE  DNLS1
     ...
     2     TEMP1,TEMP2,XNORM,ZERO
      DOUBLE PRECISION PDA_D1MACH,PDA_DENORM,ERR,CHKLIM
      CHARACTER*8 XERN1
      CHARACTER*16 XERN3
...
C
            MODECH = 1
            CALL PDA_DCKDER(M,N,X,FVEC,FJAC,LDFJAC,WA1,WA4,MODECH,ERR)
C
...
C
            ...
            DO 350 I = 1, M
               MODECH = 2
               CALL PDA_DCKDER(1,N,X,FVEC(I),FJAC(I,1),LDFJAC,WA1,
     1              WA4(I),MODECH,ERR)
               IF (ERR .LT. CHKLIM) THEN
                  WRITE (XERN1, '(I8)') I
                  WRITE (XERN3, '(1PE15.6)') ERR
               CALL PDA_XERMSG ('SLATEC', 'DNLS1', 'DERIVATIVE OF ' //
     *              'FUNCTION ' // XERN1 // ' MAY BE WRONG, ERR = ' //
     *               XERN3 // ' TOO CLOSE TO 0.', 7, 0, STATUS)
               ENDIF
  350       CONTINUE
         ENDIF
C
...
C
410      IFLAG = 1
         CALL PDA_DFDJC3(FCN,M,N,X,FVEC,FJAC,LDFJAC,IFLAG,EPSFCN,WA4)
         NFEV = NFEV + N
  420    IF (IFLAG .LT. 0) GO TO 300
C
C        COMPUTE THE QR FACTORIZATION OF THE JACOBIAN.
C
         CALL PDA_DQRFAC(M,N,FJAC,LDFJAC,.TRUE.,IPVT,N,WA1,WA2,WA3)
C
...
495         CONTINUE
            MODECH = 2
            CALL PDA_DCKDER(1,N,X,FVEC(I),WA3,1,WA1,WA4(I),MODECH,ERR)
            IF (ERR .LT. CHKLIM) THEN
               WRITE (XERN1, '(I8)') I
               WRITE (XERN3, '(1PE15.6)') ERR
           CALL PDA_XERMSG ('SLATEC', 'DNLS1', 'DERIVATIVE OF FUNCTION '
     *            // XERN1 // ' MAY BE WRONG, ERR = ' // XERN3 //
     *            ' TOO CLOSE TO 0.', 7, 0, STATUS)
            ENDIF
498         CONTINUE
C
            TEMP = FVEC(I)
            CALL PDA_DWUPDT(N,FJAC,LDFJAC,WA3,QTF,TEMP,WA1,WA2)
  500       CONTINUE
         NJEV = NJEV + 1
C
...
C
         ...
         DO 510 J = 1, N
            ...
            IPVT(J) = J
            WA2(J) = PDA_DENORM(J,FJAC(1,J))
  510       CONTINUE
         IF (.NOT.SING) GO TO 560
         CALL PDA_DQRFAC(N,N,FJAC,LDFJAC,.TRUE.,IPVT,N,WA1,WA2,WA3)
         DO 550 J = 1, N
...
C
            CALL PDA_DMPAR(N,FJAC,LDFJAC,IPVT,DIAG,QTF,DELTA,PAR,WA1,
     1                     WA2,WA3,WA4)
C
...
C
      ...
      IF (INFO .LT. 0) CALL PDA_XERMSG ('SLATEC', 'DNLS1',
     +   'EXECUTION TERMINATED BECAUSE USER SET IFLAG NEGATIVE.',
     +   1, 1, STATUS )
      IF (INFO .EQ. 0) CALL PDA_XERMSG ('SLATEC', 'DNLS1',
     +   'INVALID INPUT PARAMETER.', 2, 1, STATUS)
      IF (INFO .EQ. 4) CALL PDA_XERMSG ('SLATEC', 'DNLS1',
     +   'THIRD CONVERGENCE CONDITION, CHECK RESULTS BEFORE ACCEPTING.',
     +   1, 1, STATUS)
      IF (INFO .EQ. 5) CALL PDA_XERMSG ('SLATEC', 'DNLS1',
     +   'TOO MANY FUNCTION EVALUATIONS.', 9, 1, STATUS)
      IF (INFO .GE. 6) CALL PDA_XERMSG ('SLATEC', 'DNLS1',
