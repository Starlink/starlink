      subroutine pda8_curfit(iopt,m,x,y,w,xb,xe,k,s,nest,n,t,c,fp,
     * wrk,lwrk,iwrk,ier)
c  given the set of data points (x(i),y(i)) and the set of positive
c  numbers w(i),i=1,2,...,m,subroutine pda8_curfit determines a smooth spline
c  approximation of degree k on the interval xb <= x <= xe.
c  if iopt=-1 pda8_curfit calculates the weighted least-squares spline
...
c  subroutine pda_splev.
c
c  calling sequence:
c     call pda8_curfit(iopt,m,x,y,w,xb,xe,k,s,nest,n,t,c,fp,wrk,
c    * lwrk,iwrk,ier)
c
c  parameters:
...
c           unchanged on exit.
c   m     : integer*8. on entry m must specify the number of data points.
...
c   lwrk  : integer*8. on entry,lwrk must specify the actual dimension of
...
c   time if pda8_curfit is called repeatedly for different values of s.
...
c   pda8_curfit once more with the selected value for s but now with iopt=0.
c   indeed, pda8_curfit may then return an approximation of the same quality
c   of fit but with fewer knots and therefore better if data reduction
c   is also an important objective for the user.
c
c  other subroutines required:
c    pda_fpback,pda_fpbspl,pda_fpchec,pda_fpcurf,pda_fpdisc,
c    pda_fpgivs,pda_fpknot,pda_fprati,pda_fprota
c
c  references:
...
c  ..scalar arguments..
      real xb,xe,s,fp
      integer*8 m,lwrk
      integer iopt,k,nest,n,ier
c  ..array arguments..
      real x(m),y(m),w(m),t(nest),c(nest),wrk(lwrk)
      integer iwrk(nest)
c  ..local scalars..
      real tol
      integer i,ia,ib,ifp,ig,iq,iz,j,k1,k2,maxit,nmin
      integer*8 lwest
c  ..
...
c  are invalid, control is immediately repassed to the calling program.
  ...
  10  continue
      ...
      do 20 i=1,k1
         ...
         j = j-1
  20  continue
      call pda8_fpchec(x,m,t,n,k,ier)
      if(ier) 50,40,50
...
c we partition the working space and determine the spline approximation.
  40  ifp = 1
      ...
      ig = ib+nest*k2
      iq = ig+nest*k2
      call pda8_fpcurf(iopt,x,y,w,m,xb,xe,k,s,nest,tol,maxit,k1,k2,n,t,
     * c,fp,wrk(ifp),wrk(iz),wrk(ia),wrk(ib),wrk(ig),wrk(iq),iwrk,ier)
  50  return
      end
