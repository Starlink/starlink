...
C   960918  Renamed PDA_DNLS1E and added STATUS argument (PWD)
C***END PROLOGUE  DNLS1E
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER M,N,NPRINT,INFO,LWA,IOPT,STATUS
      INTEGER INDEX,IW(*)
...
C
      ...
      MODE = 1
      INDEX = 5*N+M
      CALL PDA_DNLS1(FCN,IOPT,M,N,X,FVEC,WA(INDEX+1),M,FTOL,XTOL,GTOL,
     1           MAXFEV,EPSFCN,WA(1),MODE,FACTOR,NPRINT,INFO,NFEV,NJEV,
     2           IW,WA(N+1),WA(2*N+1),WA(3*N+1),WA(4*N+1),WA(5*N+1),
     3           STATUS)
      IF (INFO .EQ. 8) INFO = 4
   10 CONTINUE
      IF (INFO .EQ. 0) CALL PDA_XERMSG ('SLATEC', 'DNLS1E',
