*DECK PDA_DBNDAC
      SUBROUTINE PDA_DBNDAC (G, MDG, NB, IP, IR, MT, JT, STATUS)
C***BEGIN PROLOGUE  PDA_DBNDAC
...
C***LIBRARY   SLATEC
C***CATEGORY  D9
C***TYPE      DOUBLE PRECISION (BNDACC-S, PDA_DBNDAC-D)
...
C***ROUTINES CALLED  PDA_DH12, PDA_XERMSG
...
C***END PROLOGUE  PDA_DBNDAC
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER STATUS
      DIMENSION G(MDG,*)
C***FIRST EXECUTABLE STATEMENT  PDA_DBNDAC
      ZERO=0.D0
C
...
C
      ...
      NERR=1
      IOPT=2
      CALL PDA_XERMSG ('SLATEC', 'PDA_DBNDAC',
     +   'MDG.LT.IR, PROBABLE ERROR.', NERR, IOPT, STATUS)
      RETURN
    5 CONTINUE
C
C                                             ALG. STEP 5
...
C                                             ALG. STEP 20
      DO 80 I=1,KH
        CALL PDA_DH12 (1,I,MAX(I+1,IR-IP+1),MH,G(IP,I),1,RHO,
