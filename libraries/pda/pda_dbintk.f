...
C***BEGIN PROLOGUE  PDA_DBINTK
C***PURPOSE  Compute the B-representation of a spline which interpolates
C            given data.
C***LIBRARY   SLATEC
C***CATEGORY  E1A
C***TYPE      DOUBLE PRECISION (BINTK-S, PDA_DBINTK-D)
...
C
C         PDA_DBINTK is the PDA_SPLINT routine of the reference.
C
C         PDA_DBINTK produces the B-spline coefficients, BCOEF, of the
C         B-spline of order K with knots T(I), I=1,...,N+K, which
C         takes on the value Y(I) at X(I), I=1,...,N.  The spline or
C         any of its derivatives can be evaluated by calls to PDA_DBVALU.
C
...
C         The banded system is then solved by a call to PDA_DBNFAC (which
C         constructs the triangular factorization for A and stores it
C         again in Q), followed by a call to PDA_DBNSLV (which then
C         obtains the solution BCOEF by substitution).  PDA_DBNFAC does no
...
C                         CALL PDA_DBNSLV (Q,2K-1,N,K-1,K-1,BCOEF)
...
C                 1978.
C***ROUTINES CALLED  PDA_DBNFAC, PDA_DBNSLV, PDA_DBSPVN, PDA_XERMSG
...
C***FIRST EXECUTABLE STATEMENT  PDA_DBINTK
      IF(K.LT.1) GO TO 100
      IF(N.LT.K) GO TO 105
      JJ = N - 1
      IF(JJ.EQ.0) GO TO 6
...
C        FOLLOWING
   30   CALL PDA_DBSPVN(T, K, K, 1, XI, LEFT, BCOEF, WORK, IWORK,
     1     STATUS)
...
C        PDA_DBNFAC). IN THE PRESENT PROGRAM, WE TREAT  Q  AS AN EQUIVALENT
...
C     ***OBTAIN FACTORIZATION OF  A  , STORED AGAIN IN  Q.
      CALL PDA_DBNFAC(Q, K+KM1, N, KM1, KM1, IFLAG)
      GO TO (60, 90), IFLAG
C     *** SOLVE  A*BCOEF = Y  BY BACKSUBSTITUTION
   60 DO 70 I=1,N
        BCOEF(I) = Y(I)
   70 CONTINUE
      CALL PDA_DBNSLV(Q, K+KM1, N, KM1, KM1, BCOEF)
      RETURN
C
C
   80 CONTINUE
      CALL PDA_XERMSG ('SLATEC', 'PDA_DBINTK',
     +   'SOME ABSCISSA WAS NOT IN THE SUPPORT OF THE CORRESPONDING ' //
     +   'BASIS FUNCTION AND THE SYSTEM IS SINGULAR.', 2, 1, STATUS)
      RETURN
   90 CONTINUE
      CALL PDA_XERMSG ('SLATEC', 'PDA_DBINTK',
