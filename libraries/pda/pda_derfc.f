...
C***BEGIN PROLOGUE  PDA_DERFC
C***PURPOSE  Compute the complementary error function.
C***LIBRARY   SLATEC (FNLIB)
C***CATEGORY  C8A, L5A1E
C***TYPE      DOUBLE PRECISION (ERFC-S, PDA_DERFC-D)
C***KEYWORDS  COMPLEMENTARY ERROR FUNCTION, ERFC, FNLIB,
C             SPECIAL FUNCTIONS
C***AUTHOR  Fullerton, W., (LANL)
C***DESCRIPTION
C
C PDA_DERFC(X) calculates the double precision complementary error function
...
C***END PROLOGUE  PDA_DERFC
      INTEGER STATUS
      DOUBLE PRECISION X, ERFCS(21), ERFCCS(59), ERC2CS(49), SQEPS,
     1  SQRTPI, XMAX, TXMAX, XSML, Y, PDA_D1MACH, PDA_DCSEVL
      LOGICAL FIRST
     ...
     1 NTERFC, NTERC2, XSML, XMAX, SQEPS, FIRST
      ...
      DATA FIRST /.TRUE./
C***FIRST EXECUTABLE STATEMENT  PDA_DERFC
      IF (FIRST) THEN
         ETA = 0.1*REAL(PDA_D1MACH(3))
         NTERF = PDA_INITDS  (ERFCS,  21, ETA, STATUS)
         NTERFC = PDA_INITDS (ERFCCS, 59, ETA, STATUS)
         NTERC2 = PDA_INITDS (ERC2CS, 49, ETA, STATUS)
C
         XSML = -SQRT(-LOG(SQRTPI*PDA_D1MACH(3)))
         TXMAX = SQRT(-LOG(SQRTPI*PDA_D1MACH(1)))
...
C
      IF (Y.LT.SQEPS) PDA_DERFC = 1.0D0 - 2.0D0*X/SQRTPI
      IF (Y.GE.SQEPS) PDA_DERFC = 1.0D0 - X*(1.0D0
     1  + PDA_DCSEVL (2.D0*X*X-1.D0, ERFCS, NTERF, STATUS))
      RETURN
C
C ERFC(X) = 1.0 - ERF(X)  FOR  1.0 .LT. ABS(X) .LE. XMAX
C
 30   Y = Y*Y
      IF (Y.LE.4.D0) PDA_DERFC = EXP(-Y)/ABS(X) * (0.5D0 + PDA_DCSEVL (
     1  (8.D0/Y-5.D0)/3.D0, ERC2CS, NTERC2, STATUS) )
      IF (Y.GT.4.D0) PDA_DERFC = EXP(-Y)/ABS(X) * (0.5D0 + PDA_DCSEVL (
     1  8.D0/Y-1.D0, ERFCCS, NTERFC, STATUS) )
      IF (X.LT.0.D0) PDA_DERFC = 2.0D0 - PDA_DERFC
      RETURN
C
 40   CALL PDA_XERMSG ('SLATEC', 'PDA_DERFC',
     :   'X SO BIG ERFC UNDERFLOWS', 1, 1, STATUS)
      PDA_DERFC = 0.D0
      RETURN
C
      END
