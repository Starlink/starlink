*DECK PDA_DEFC
      SUBROUTINE PDA_DEFC (NDATA, XDATA, YDATA, SDDATA, NORD,
     +   NBKPT, BKPT, MDEIN, MDEOUT, COEFF, LW, W, STATUS)
C***BEGIN PROLOGUE  PDA_DEFC
...
C***TYPE      DOUBLE PRECISION (EFC-S, PDA_DEFC-D)
...
C                         Internal to PDA_DEFC( ) the extreme end knots may
...
C                         by the user.
C
C                         =1  The first time that PDA_DEFC( ) has been
C                         entered.  There are NDATA points to process.
C
C                         =2  This is another entry to PDA_DEFC().  The sub-
C                         program PDA_DEFC( ) has been entered with MDEIN=1
...
C                         of the curve fit.
C
C                         =-1  A usage error of PDA_DEFC( ) occurred.  The
C                         offending condition is noted with the SLATEC
C                         library error processor, PDA_XERMSG( ).  In case
...
C                         curve returned by PDA_DEFC( ), the constrained
C                         least squares curve fitting subprogram DFC( )
C                         may be required.  The work done within PDA_DEFC( )
...
C                         to PDA_DEFC( ) with values of MDEIN=1,2,2,... .
...
C                         use the function subprogram PDA_DBVALU( ).
C
C                         F = PDA_DBVALU(BKPT,COEFF,NBKPT-NORD,NORD,IDER,
C                                      XVAL,INBV,WORKB)
C
C                         The output of this subprogram will not be
C                         defined unless an output value of MDEOUT=1
C                         was obtained from PDA_DEFC( ), XVAL is in the data
C                         interval, and IDER is nonnegative and .LT.
C                         NORD.
C
C                         The first time PDA_DBVALU( ) is called, INBV=1
C                         must be specified.  This value of INBV is the
C                         overwritten by PDA_DBVALU( ).  The array WORKB(*)
C                         must be of length at least 3*NORD, and must
C                         not be the same as the W(*) array used in the
C                         call to PDA_DEFC( ).
C
C                         PDA_DBVALU( ) expects the breakpoint array BKPT(*)
C                         to be sorted.
C
...
C                 1978.
C***ROUTINES CALLED  PDA_DEFCMN
C***REVISION HISTORY  (YYMMDD)
C   800801  DATE WRITTEN
C   890531  Changed all specific intrinsics to generic.  (WRB)
C   890531  REVISION DATE from Version 3.2
C   891214  Prologue converted to Version 4.0 format.  (BAB)
C   900510  Change Prologue comments to refer to PDA_XERMSG.  (RWC)
C   900607  Editorial changes to Prologue to make Prologues for EFC,
C           PDA_DEFC, FC, and DFC look as much the same as possible.  (RWC)
C   920501  Reformatted the REFERENCES section.  (WRB)
C   950403  Implement status.  (HME)
C***END PROLOGUE  PDA_DEFC
C
C      SUBROUTINE           FUNCTION/REMARKS
C
C      PDA_DBSPVN( )          COMPUTE FUNCTION VALUES OF B-SPLINES.  FROM
C                         THE B-SPLINE PACKAGE OF DE BOOR NOTED ABOVE.
C
C      PDA_DBNDAC( ),         BANDED LEAST SQUARES MATRIX PROCESSORS.
C      PDA_DBNDSL( )          FROM LAWSON-HANSON, SOLVING LEAST
C                         SQUARES PROBLEMS.
C
C      PDA_DSORT( )           DATA SORTING SUBROUTINE, FROM THE
C                         SANDIA MATH. LIBRARY, SAND77-1441.
C
C      PDA_XERMSG( )          ERROR HANDLING ROUTINE
C                         FOR THE SLATEC MATH. LIBRARY.
C                         SEE SAND78-1189, BY R. E. JONES.
C
C      PDA_DCOPY( ),PDA_DSCAL( )  SUBROUTINES FROM THE BLAS PACKAGE.
C
...
C
C***FIRST EXECUTABLE STATEMENT  PDA_DEFC
C     LWW=1               USAGE IN PDA_DEFCMN( ) OF W(*)..
C     LWW,...,LG-1        W(*,*)
C
C     LG,...,LXTEMP-1     G(*,*)
C
...
C
C     LBKPT,...,LBF       BKPT(*) (LOCAL TO PDA_DEFCMN( ))
C
C     LBF,...,LBF+NORD**2 BF(*,*)
C
      MDG = NBKPT+1
      ...
      LBF = LBKPT + NBKPT
      CALL PDA_DEFCMN(NDATA,XDATA,YDATA,SDDATA,
     1         NORD,NBKPT,BKPT,
     2         MDEIN,MDEOUT,
     3         COEFF,
     4         W(LBF),W(LXTEMP),W(LPTEMP),W(LBKPT),
     5         W(LG),MDG,W(LWW),MDW,LW,STATUS)
      RETURN
      END
