      double precision function pda_dpmpar(i)
      integer i
c     **********
c
c     function pda_dpmpar
c
...
c
c       double precision function pda_dpmpar(i)
c
c     where
c
...
c
c         pda_dpmpar(1) = b**(1 - t), the machine precision,
c
c         pda_dpmpar(2) = b**(emin - 1), the smallest magnitude,
c
c         pda_dpmpar(3) = b**emax*(1 - b**(-t)), the largest magnitude.
c
c     Argonne National Laboratory. MINPACK Project. June 1983.
c     Burton S. Garbow, Kenneth E. Hillstrom, Jorge J. More
c
c  Modified 13 Jul 1995 by hme:
c     Comment out all code except "return". This routine now uses D1MACH
c     rather than its own constants. D1MACH seems to exist in a newer
c     version with larger variety of machines.
c     This routine was acquired with the VAX-11 constants enabled. My
c     commenting out uses C rather than the c used otherwise.
c     Comparing the prologues of this routine and D1MACH, it is quite
c     obvious that given arguments 1,2,3 correspond to 4,1,2 in D1MACH.
c
c  Modified 13 Jul 1995 by hme:
c     Added an explicit declaration for function pda_d1mach.
c
c     **********
...
c
C     pda_dpmpar = dmach(i)
      if      ( i .eq. 1 ) then
         pda_dpmpar = pda_d1mach(4)
      else if ( i .eq. 2 ) then
         pda_dpmpar = pda_d1mach(1)
      else if ( i .eq. 3 ) then
         pda_dpmpar = pda_d1mach(2)
      else
         pda_dpmpar = 0d0
      end if
      return
c
c     Last card of function pda_dpmpar.
c
      end
