...
C***LIBRARY   SLATEC
C***CATEGORY  E1B
C***TYPE      DOUBLE PRECISION (POLCOF-S, PDA_DPOLCF-D)
C***KEYWORDS  COEFFICIENTS, POLYNOMIAL
C***AUTHOR  Huddleston, R. E., (SNLL)
C***DESCRIPTION
C
C     Abstract
C        Subroutine PDA_DPOLCF computes the coefficients of the polynomial
C     fit (including Hermite polynomial fits ) produced by a previous
C     call to PDA_DPLINT.  The coefficients of the polynomial, expanded
C     about XX, are stored in the array D. The expansion is of the form
C     P(Z) = D(1) + D(2)*(Z-XX) +D(3)*((Z-XX)**2) + ... +
C                                                  D(N)*((Z-XX)**(N-1)).
C     Between the call to PDA_DPLINT and the call to PDA_DPOLCF the variable N
...
C     X    - *     call to PDA_DPLINT and the call to PDA_DPOLCF.
C     C    - ****
C
C     *****  OUTPUT PARAMETER
C      *** All TYPE REAL variables are DOUBLE PRECISION ***
...
C     by PDA_DPLINT. You may call PDA_DPOLVL to perform the task, or you may
C     call PDA_DPOLCF to obtain the coefficients of the Taylor expansion and
...
C     coefficients produced by PDA_DPLINT, much more accuracy may be
C     expected by calling PDA_DPOLVL as opposed to writing your own scheme.
C
C***REFERENCES  (NONE)
