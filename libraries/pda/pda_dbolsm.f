*DECK PDA_DBOLSM
      SUBROUTINE PDA_DBOLSM (W, MDW, MINPUT, NCOLS, BL, BU, IND, IOPT,
     +   X, RNORM, MODE, RW, WW, SCL, IBASIS, IBB, STATUS)
C***BEGIN PROLOGUE  PDA_DBOLSM
C***SUBSIDIARY
C***PURPOSE  Subsidiary to DBOCLS and PDA_DBOLS
C***LIBRARY   SLATEC
C***TYPE      DOUBLE PRECISION (SBOLSM-S, PDA_DBOLSM-D)
C***AUTHOR  (UNKNOWN)
C***DESCRIPTION
C
...
C     abnormal completion codes see below: ERROR MESSAGES for PDA_DBOLSM
...
C       IOPT(3)=99
C
C    Error Messages for PDA_DBOLSM
C    ----- -------- --- ---------
...
C
C***SEE ALSO  DBOCLS, PDA_DBOLS
C***ROUTINES CALLED  PDA_D1MACH, PDA_DAXPY, PDA_DCOPY, PDA_DDOT, PDA_DMOUT, PDA_DNRM2, PDA_DROT,
C                    PDA_DROTG, PDA_DSWAP, PDA_DVOUT, PDA_IVOUT, PDA_XERMSG
...
C     CHANGE THE SUBPROGRAM NAME TO PDA_DBOLSM AND THE STRINGS
C     /SAXPY/ TO /PDA_DAXPY/, /SCOPY/ TO /PDA_DCOPY/,
C     /SDOT/ TO /PDA_DDOT/, /SNRM2/ TO /PDA_DNRM2/,
C     /SROT/ TO /PDA_DROT/, /SROTG/ TO /PDA_DROTG/, /R1MACH/ TO /PDA_D1MACH/,
C     /SVOUT/ TO /PDA_DVOUT/, /SMOUT/ TO /PDA_DMOUT/,
C     /SSWAP/ TO /PDA_DSWAP/, /E0/ TO /D0/,
...
C***FIRST EXECUTABLE STATEMENT  PDA_DBOLSM
C
C     Verify that the problem dimensions are defined properly.
C
      IF (MINPUT.LE.0) THEN
          WRITE (XERN1, '(I8)') MINPUT
          CALL PDA_XERMSG ('SLATEC', 'PDA_DBOLSM',
     *       'THE NUMBER OF ROWS = ' //
     *       XERN1 // ' MUST BE POSITIVE.', 31, 1, STATUS)
          MODE = -31
          RETURN
      ENDIF
C
     ...
     *       XERN1 // ' MUST BE POSITIVE.', 32, 1, STATUS)
          MODE = -32
          RETURN
      ENDIF
C
...
C
      DO 10 J = 1,NCOLS
          IF (IND(J).LT.1 .OR. IND(J).GT.4) THEN
              ...
              CALL PDA_XERMSG ('SLATEC', 'PDA_DBOLSM',
     *           'FOR J = ' // XERN1 //
     *           ' THE CONSTRAINT INDICATOR MUST BE 1-4', 34, 1, STATUS)
              MODE = -34
              RETURN
          ENDIF
   10 CONTINUE
...
C
     ...
     *           ' IS NOT BETWEEN 1 AND NCOLS = ' // XERN2,
     *           36, 1, STATUS)
              MODE = -36
              RETURN
          ENDIF
C
     ...
     *           ' MUST BE POSITIVE.$$NOW = ' // XERN2, 37, 1, STATUS)
              MODE = -37
              RETURN
          ENDIF
   30 CONTINUE
...
C
              ...
              IF (TOLIND.LT.PDA_D1MACH(4)) THEN
                  ...
                  WRITE (XERN4, '(1PD15.6)') PDA_D1MACH(4)
                  CALL PDA_XERMSG ('SLATEC', 'PDA_DBOLSM',
...
C
     ...
     *               'THE OFFSET = ' //
     *               XERN1 // ' BEYOND POSITION NCOLS = ' // XERN2 //
     *               ' MUST BE POSITIVE FOR OPTION NUMBER 3.',
     *               26, 1, STATUS)
                  MODE = -26
                  RETURN
              ENDIF
...
C
     ...
     *               'THE OFFSET = ' //
     *               XERN1 // ' BEYOND POSITION NCOLS = ' // XERN2 //
     *               ' MUST BE POSITIVE FOR OPTION NUMBER 5.',
     *               29, 1, STATUS)
                  MODE = -29
                  RETURN
              ENDIF
C
     ...
     *               'NEGATIVE.$$NOW = ' // XERN3, 30, 0, STATUS)
                  MODE = -30
                  RETURN
              ENDIF
          ENDIF
...
C
      ...
      ELSE
          ...
          CALL PDA_XERMSG ('SLATEC', 'PDA_DBOLSM',
     *       'THE OPTION NUMBER = ' //
     *       XERN1 // ' IS NOT DEFINED.', 23, 1, STATUS)
          MODE = -23
          RETURN
      ENDIF
      GO TO 590
  ...
  470 IF (FAC*MINPUT.GT.NCOLS) THEN
          DO 490 J = 1,NCOLS+1
              DO 480 I = MINPUT,J+MVAL+1,-1
                  ...
                  W(I,J) = ZERO
                  CALL PDA_DROT(NCOLS-J+1,W(I-1,J+1),MDW,W(I,J+1),
     *               MDW,SC,SS)
  480         CONTINUE
  490     CONTINUE
          MROWS = NCOLS + MVAL + 1
...
C
      CALL PDA_DCOPY(NCOLS,ZERO,0,X,1)
C
...
C
      CALL PDA_DCOPY(NCOLS,ZERO,0,WW,1)
...
C
      IF (IPRINT.GT.0) THEN
          ...
          CALL PDA_IVOUT(NCOLS,IBASIS,
     *       '('' INTERNAL VARIABLE ORDER'')',-4)
...
C
      ...
      DO 220 J = NSETB+1,NCOLS
          ...
          ITEMP = IBASIS(J)
          JCOL = ABS(ITEMP)
          T1 = PDA_DNRM2(MVAL-NSETB,W(INEXT(NSETB),J),1)
          IF (ITEMP.LT.0) THEN
...
C
      IF (MVAL.LE.NSETB) THEN
          CL1 = PDA_DNRM2(MVAL,W(1,JBIG),1)
          CL2 = ABS(WT)*PDA_DNRM2(NSETB-MVAL,W(INEXT(MVAL),JBIG),1)
          CL3 = ABS(WT)*PDA_DNRM2(MROWS-NSETB,W(INEXT(NSETB),JBIG),1)
          CALL PDA_DROTG(CL1,CL2,SC,SS)
          COLABV = ABS(CL1)
          COLBLO = CL3
      ELSE
          CL1 = PDA_DNRM2(NSETB,W(1,JBIG),1)
          CL2 = PDA_DNRM2(MVAL-NSETB,W(INEXT(NSETB),JBIG),1)
          CL3 = ABS(WT)*PDA_DNRM2(MROWS-MVAL,W(INEXT(MVAL),JBIG),1)
          COLABV = CL1
          CALL PDA_DROTG(CL2,CL3,SC,SS)
          COLBLO = ABS(CL2)
      ENDIF
C
...
C
      IF (MROWS.GT.NSETB) THEN
          DO 230 I = MROWS,NSETB+1,-1
              ...
              W(I,NSETB) = ZERO
              CALL PDA_DROT(NCOLS-NSETB+1,W(I-1,NSETB+1),
     *                  MDW,W(I,NSETB+1),MDW,SC,SS)
  230     CONTINUE
C
...
C
  270 CALL PDA_DCOPY(NSETB,W(1,NCOLS+1),1,RW,1)
      DO 280 J = NSETB,1,-1
          RW(J) = RW(J)/W(J,J)
          JCOL = ABS(IBASIS(J))
          T = RW(J)
...
C
  400 CALL PDA_DSWAP(MROWS,W(1,I),1,W(1,JBIG),1)
...
C
      IGOPR = 2
  130 CALL PDA_DCOPY(NSETB,X,1,RW,1)
      CALL PDA_DCOPY(NCOLS,ZERO,0,X,1)
      DO 140 J = 1,NSETB
          JCOL = ABS(IBASIS(J))
...
C
      I = MAX(NSETB,MVAL)
      RNORM = PDA_DNRM2(MROWS-I,W(INEXT(I),NCOLS+1),1)
C
