 ...
 *  Purpose:
 *     Defines all routines that obtain modification access to a 1-D input
 *     image.
 ...
 *  Invocation:
 *     imgMod1[x]( param, nx, ip, status )

 *  Description:
 *     This routine creates all the imgMod1[x] routines from the
 *     generic stubs.
...
#define XIMG_MOD1(type)  F77_SUBROUTINE(img_mod1 ## type)
#define IMG_MOD1(type)   XIMG_MOD1(type)

#define XIMGMOD1(type)  void imgMod1 ## type
#define IMGMOD1(type)   XIMGMOD1(type)

#define XIMGMOD1_CALL(type)  F77_CALL(img_mod1 ## type)
#define IMGMOD1_CALL(type)   XIMGMOD1_CALL(type)
