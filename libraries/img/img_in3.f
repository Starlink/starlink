      SUBROUTINE IMG_IN3( PARAM, NX, NY, NZ, IP, STATUS )
*+
*  Name:
*     IMG_IN3

*  Purpose:
*     Obtains access to a 3-dimensional input image.
...
*  If an error occurred, then report a contextual message.
      IF ( .NOT. IMG1_OK( STATUS ) ) THEN
         IF ( INDEX( PARAM, ',' ) .NE. 0 ) THEN
            CALL ERR_REP( 'IMG_IN3_ERR',
     :           'IMG_IN3: Error obtaining access to ' //
     :           '3-dimensional input images.', STATUS )
         ELSE
            CALL ERR_REP( 'IMG_IN3_ERR',
     :           'IMG_IN3: Error obtaining access to a ' //
