      SUBROUTINE IMG_NEW2( PARAM, NX, NY, IP, STATUS )
*+
*  Name:
*     IMG_NEW2

*  Purpose:
*     Creates a new 2-dimensional image.
...
*  If an error occurred, then report a contextual message.
      IF ( .NOT. IMG1_OK( STATUS ) ) THEN
         IF ( INDEX( PARAM, ',' ) .NE. 0 ) THEN
            CALL ERR_REP( 'IMG_NEW2_ERR',
     :           'IMG_NEW2: Error creating new 2-dimensional images.',
     :           STATUS )
         ELSE
            CALL ERR_REP( 'IMG_NEW2_ERR',
     :           'IMG_NEW2: Error creating a new 2-dimensional image.',
     :           STATUS )
         END IF
      END IF
