      SUBROUTINE IMG1_HSPLT( NAME, F1, F2, P1, P2, STATUS )
*+
*  Name:
*     IMG1_HSPLT
...
*  Invocation:
*     CALL IMG1_HSPLT( NAME, F1, F2, P1, P2, STATUS )
...
*  Algorithm:
*     -  Initialise and find the first and last non-blank characters in
*     the object name.
*     -  If the name is blank, then report an error.
*     -  If the first non-blank character is a quote, then search for
*     the closing quote.
...
*     -  If there have been no errors, then attempt to find the
*     position of the path name string.
*     -  If the file name extends to the end of the object name string,
*     then there is no path name (i.e. it is a top-level object), so
*     return a "null" position.
*     -  Otherwise, find the first and last non-blank character
*     positions in whatever follows the file name.
*     -  If this candidate path name does not start with a '.' or a
*     '(', then there is a top-level name present. This must be ignored
*     (the actual file name fills this role).
*     -  Find the first following occurrence of a '.' or '('.
*     -  Move the starting position to the character found, or beyond
*     the last non-blank character if a '.' or '(' was not found.
...
*     -  If the file name is not quoted then its end must be detected.
*     First determine which operating system is in use, as this
*     determines the file name format.
*     -  If we are on VMS, then search for a ']' or '>' character,
*     which may mark the end of an explicit directory specification
*     (note the latter may result if a file name is entered on a DCL
*     command line).
*     -  If there is no explicit directory reference, then search for a
*     ':' which may mark the end of a logical name representing the
*     directory.  Also search for a '('. The ':' can be used only if it
*     occurs before the '(', otherwise it is part of a subscript
*     expression.
*     -  Correct for the starting position of the search.
*     -  If we are not on VMS, then assume POSIX file name format.
*     Search for the last '/' character which delimits the final field
*     of the file's path name.
*     -  Having located the end of the directory specification (if
*     any), now search for a '.' or a '(' which marks the end of the
*     first field in the object path name.
*     -  If no such terminating character exists, then use the whole
*     name string as the file name. Otherwise, select the first path
*     name field for use as the file name.
*     -  If a '.' occurs in the first character position, then report
*     an error.
*     -  Otherwise, note the file name position, omitting any trailing
*     blanks.
*     -  If no errors have occurred, then attempt to locate the path
*     name string.
*     -  If the file name extends to the end of the object name string,
*     then there is no path name (i.e. it is a top-level object), so
*     return a "null" position.
*     -  Otherwise, eliminate any leading blanks from the candidate
*     path name which follows the file name and return the path name
*     limits.
...
*     28-NOV-1994 (PDRAPER):
*        This is the routine NDF1_HSPLT. A copy has been introduced into
*        IMG to help with determining the input NDF file protections.
*        NDF specific errors have been removed or changed to IMG ones.
...
*  If the name is blank, then report an error.
      IF ( F .GT. L ) THEN
         STATUS = IMG__PARIN
         CALL ERR_REP( 'IMG1_HSPLT_BLNK', 'Blank image name supplied.',
     :                 STATUS )
...
*  If the closing quote is missing, then report an error.
         IF ( IEND .EQ. 0 ) THEN
            ...
            CALL ERR_REP( 'IMG1_HSPLT_QTE',
     :                    'Missing quote in the image name ''^NAME''.',
     :                    STATUS )
...
*  If the quotes are consecutive, then report an error.
         ELSE IF ( IEND .EQ. F + 1 ) THEN
            STATUS = IMG__PARIN
            CALL MSG_SETC( 'NAME', NAME( F : L ) )
            CALL ERR_REP( 'IMG1_HSPLT_NON',
     :                 'File name absent in the image name ''^NAME''.',
     :                 STATUS )
...
*  If the file name is blank, then report an error.
            IF ( F1 .GT. F2 ) THEN
               STATUS = IMG__PARIN
               CALL MSG_SETC( 'NAME', NAME( F : L ) )
               CALL ERR_REP( 'IMG1_HSPLT_BLQ',
     :                       'Quoted filename is blank in the image ' //
     :                       'name ''^NAME''.',
     :                       STATUS )
...
*  error.
            IF ( IEND .LT. F ) THEN
               STATUS = IMG__PARIN
               CALL MSG_SETC( 'NAME', NAME( F : L ) )
               CALL ERR_REP( 'IMG1_HSPLT_MSF',
     :                    'Missing field in the image name ''^NAME''.',
     :                    STATUS )
