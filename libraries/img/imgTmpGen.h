 ...
 *  Purpose:
 *     Creates a temporary input image using a specific type.
 ...
 *     10-JUN-1996 (PDRAPER):
 *        Changed to use more C-like routine names.
 *     21-APR-1999 (PDRAPER):
...
IMG_TMP( IMG_F77_TYPE ) ( CHARACTER(param),
                          INTEGER(nx),
                          INTEGER(ny),
                          POINTER_ARRAY(ip),
                          INTEGER(status)
...
IMGTMP( IMG_SHORT_C_TYPE ) ( char *param,
                             int nx,
                             int ny,
                             IMG_FULL_C_TYPE **ip,
                             int *status )
{
  ...
  IMGTMP_CALL( IMG_F77_TYPE ) ( CHARACTER_ARG(fparam),
                                INTEGER_ARG(&nx),
                                INTEGER_ARG(&ny),
