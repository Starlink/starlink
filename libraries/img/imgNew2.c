 ...
 *  Purpose:
 *     Defines default routines that create a new 2-D image.

 *  Language:
 *     ANSI C

 *  Invocation:
 *     imgNew2[x]( param, nx, ny, ip, status )
...
#define XIMG_NEW2(type)  F77_SUBROUTINE(img_new2 ## type)
#define IMG_NEW2(type)   XIMG_NEW2(type)

#define XIMGNEW2(type)  void imgNew2 ## type
#define IMGNEW2(type)   XIMGNEW2(type)

#define XIMGNEW2_CALL(type)  F77_CALL(img_new2 ## type)
#define IMGNEW2_CALL(type)   XIMGNEW2_CALL(type)
