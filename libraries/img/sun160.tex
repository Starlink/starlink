...
\newcommand{\stardocsource}    {sun160.4}
\newcommand{\stardocnumber}    {160.4}
\newcommand{\stardocauthors}   {P.W. Draper \\
                                R.F. Warren-Smith}
\newcommand{\stardocdate}      {09 June 1998}
...
% ? New section for abstract if used.
  \section{\xlabel{abstract}Description}
...
variance, quality and axes. It also allows you to access NDF
extensions (our ``sources'' of header information) in more
...
Where the parts in {\tt "[]"} are optional or not always available.
{\tt [N]} indicates the number of dimensions of the data you want to access
and the necessary {\tt nx}, {\tt ny} and {\tt nz} arguments should be passed.
\begin{small}
\begin{center}
\begin{tabular}{||l|l||}
\hline
Number of dimensions {\tt [N]}    &    {\tt nx}, {\tt ny} \& {\tt nz} required \\
\hline
     {\tt 1}                      &    {\tt nx}       \\
     {\tt 2}                      &    {\tt nx} \& {\tt ny} \\
     {\tt 3}                      &    {\tt nx}, {\tt ny} \& {\tt nz} \\
...
element say {\tt [y][x]} is
{\tt nx}~$\times$~{\tt y}~$+$~{\tt x}. In 3-D this becomes 
{\tt [z][y][x]} --- 
{\tt nx}~$\times$~{\tt ny}~$\times$~{\tt z}~$+$~{\tt nx}~$\times$~{\tt y}~$+$~{\tt x}.

\label{IMGCdatatypes}
The data type of the image is specified by replacing the {\tt [X]} part
of the name by one of the following character codes:
\begin{small}
\begin{center}
\begin{tabular}{||l|l|l||}
\hline
Character code {\tt [X]} & C data type ({\tt TYPE}) &  Description \\
\hline
 {\tt F}       & {\tt float}          &   \\
 {\tt D}       & {\tt double}         &  \\
 {\tt I}       & {\tt int}            &  \\
 ...
 {\tt B}       & {\tt signed char}    &  Signed byte      \\
 {\tt UB}      & {\tt unsigned char}  &  Unsigned byte    \\
\hline
\end{tabular}
\end{center}
\end{small}
You will also need to replace {\tt TYPE} by the related data type.
The default data type is {\tt float} (in which case do not replace
{\tt [X]} by anything and set {\tt TYPE} to {\tt float}).
...
\end{rawhtml}
\label{HDRCdatatypes}
Where the {\tt "[X]"} indicate the data type of the header items. 
The possible values of {\tt [X]} are:
\begin{small}
\begin{center}
\begin{tabular}{||l|l|l||}
\hline
Character code[X] & C data type ({\tt TYPE)} &  Description      \\
\hline
 {\tt C}          & {\tt char}               &  Character string \\
 {\tt D}          & {\tt double}             &  \\
 {\tt I}          & {\tt int}                &  \\
 {\tt L}          & {\tt int}                &  Boolean          \\
 {\tt F}          & {\tt float}              &  \\
...
\hline
 {\tt VAL\_\_BADF}   & {\tt float}          \\
 {\tt VAL\_\_BADD}   & {\tt double}         \\
 {\tt VAL\_\_BADI}   & {\tt int}            \\
 {\tt VAL\_\_BADS}   & {\tt short}          \\
 {\tt VAL\_\_BADUS}  & {\tt unsigned short} \\
 {\tt VAL\_\_BADB}   & {\tt signed char}    \\
 {\tt VAL\_\_BADUB}  & {\tt unsigned char}  \\
\hline
\end{tabular}
\end{center}
\end{small}
...
\begin{enumnotes}
% 1
\item Note the extra arguments ({\tt 30} \& {\tt 80}) that pass the
...
arrays of pointers to {\tt char} forming a ragged array. When
using a character array you should pass a pointer to the first element
not the actual array (as the HDR routines expect to see {\tt char *}).
...
\begin{enumnotes}
% 1
\item The address to the first element of the {\tt RA} and {\tt DEC}
arrays is passed. This could have been written as 
{\tt \&RA[0][0]} and {\tt \&DEC[0][0]}.
\end{enumnotes}
...
NDF\_ANNUL). A new routine for copying sources of header 
information (\htmlref{HDR\_COPY}{HDR\_COPY}) has been added.
