...
\stardocauthors   {P.W. Draper \\
                                R.F. Warren-Smith}
\stardocdate        {3 March 2003}
\stardoctitle     {IMG \\ [1ex]
                                Simple Image Data Access}
\stardocversion     {Version 1.3}
\stardocmanual      {Subroutine Library}
\stardocabstract  {%
...
\slhyperref{this later section}{appendix }{}{IMGCinterface}.
...
\begin{enumnotes}
% 1
\item Programs that use IMG should \emph{always} have a main subroutine
...
% 2
\item The call to \htmlref{\myverb{IMG\_IN}}{IMG_IN[n][x]} gets the input
...
\slhyperref{given elsewhere}{in \S}{}{IMGCOMPILING}.

\textbf{To recap:} The most important IMG concepts to remember
from this example are:

\begin{enumerate}
\item \emph{Images are associated with a label known as a ``parameter''.}

\item \emph{Image data is accessed by a ``pointer'' that needs to be passed
           to a subroutine using the \myverb{\%VAL} mechanism.}

\item \emph{It is necessary to call \htmlref{\myverb{IMG\_FREE}}{IMG_FREE} to free
...
\begin{enumnotes}
%1
\item The call to \htmlref{\myverb{IMG\_NEW}}{IMG_NEW[n][x]} creates the new image dataset
...
\mynote
\begin{enumnotes}
\item The call to \htmlref{\myverb{IMG\_OUT}}{IMG_OUT[x]} creates a new output image
...
the ``Flexible Image Transport System'' is the \emph{de facto} standard
...
\end{terminalv}
\end{code}
The call to \htmlref{\myverb{HDR\_IN}}{HDR_IN[x]} specifies the image by using its
...
which are described
\slhyperref{elsewhere}{later -- \S}{}{IMGNAMEDEXTENSIONS}).
...
\mynote
\begin{enumnotes}
\item The call to \htmlref{\myverb{HDR\_OUT}}{HDR_OUT[x]} specifies the image by using its
...
as you cannot write items to images accessed by \htmlref{\myverb{IMG\_IN}}{IMG_IN[n][x]}.
...
\mynote
\begin{enumnotes}
\item The call to \htmlref{\myverb{IMG\_TMP}}{IMG_TMP[n][x]} creates a temporary image of
the size you specify (in this case the same size as the input image).
You will \emph{not} be prompted for a file name for a temporary image,
and \emph{no } entry is needed for it in the program's interface file.
...
\htmlref{\myverb{IMG\_IN}}{IMG_IN[n][x]} (2 dimensions are assumed if you do not say
otherwise).
...
(see \slhyperref{elsewhere}{\S}{}{IMGREGIONS}).
...
\slhyperref{here}{\S}{}{IMGDIFFERENTDATATYPES}.  So, for instance, with
...
% 3
\item
\htmlref{\myverb{HDR\_IN}}{HDR_IN[x]} returns the value of the header item as a
character string.
...
\htmlref{\myverb{HDR\_IN}}{HDR_IN[x]} subroutine specifies the
occurrence of the item to be read, and
\htmlref{\myverb{HDR\_OUT}}{HDR_OUT[x]} will append (rather than
...
\slhyperref{elsewhere}{\S}{}{IMGBADDATA}).  A complete description of
...
\slhyperref{elsewhere}{\S}{}{IMGMEAN}.
\begin{code}
\begin{terminalv}
#include <stdio.h>
#include "img.h"                                              [1]
...
as the eventual program \emph{except} for the trailing underscore. The
...
of these is shown \slhyperref{elsewhere}{in \S}{}{IMGCdatatypes}. Among
...
described \slhyperref{here}{in \S}{}{HDRCdatatypes}. Among the
possible calls are:
\begin{code}
\begin{terminalv}
   float bscale;
...
described
(\slhyperref{in the Fortran section}{in \S}{}{IMGCOMPILING}), except
...
calls to \htmlref{\myverb{IMG\_IN}}{IMG_IN[n][x]}, has been fixed.
