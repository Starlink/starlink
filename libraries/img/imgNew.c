 ...
 *  Purpose:
 *     Defines default routines that create a new image.
...
#define XIMG_NEW(type)  F77_SUBROUTINE(img_new ## type)
#define IMG_NEW(type)   XIMG_NEW(type)

#define XIMGNEW(type)  void imgNew ## type
#define IMGNEW(type)   XIMGNEW(type)

#define XIMGNEW_CALL(type)  F77_CALL(img_new ## type)
#define IMGNEW_CALL(type)   XIMGNEW_CALL(type)
