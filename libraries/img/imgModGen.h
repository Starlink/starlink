 ...
 *  Purpose:
 *     Obtains modification access to an input image using a specific type.

 *  Language:
 *     ANSI C

 *  Invocation:
 *     imgMod?( param, nx, ny, ip, status )

 *  Description:
 *     This C function sets up the required arguments and calls the
 *     Fortran subroutine img_mod[x].
...
IMG_MOD( IMG_F77_TYPE ) ( CHARACTER(param),
                          INTEGER(nx),
                          INTEGER(ny),
                          POINTER_ARRAY(ip),
                          INTEGER(status)
...
IMGMOD( IMG_SHORT_C_TYPE ) ( char *param,
                             int *nx,
                             int *ny,
                             IMG_FULL_C_TYPE **ip,
                             int *status )
{
  ...
  IMGMOD_CALL( IMG_F77_TYPE ) ( CHARACTER_ARG(fparam),
                                INTEGER_ARG(nx),
                                INTEGER_ARG(ny),
