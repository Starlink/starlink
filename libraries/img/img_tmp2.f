      SUBROUTINE IMG_TMP2( PARAM, NX, NY, IP, STATUS )
*+
*  Name:
*     IMG_TMP2

*  Purpose:
*     Creates a temporary 2-dimensional image.
...
*  If an error occurred, then report a contextual message.
      IF ( STATUS .NE. SAI__OK ) THEN
         IF ( INDEX( PARAM, ',' ) .NE. 0 ) THEN
            CALL ERR_REP( 'IMG_TMP2_ERR',
     :           'IMG_TMP2: Error creating temporary ' //
     :           '2-dimensional images.', STATUS )
         ELSE
            CALL ERR_REP( 'IMG_TMP2_ERR',
     :           'IMG_TMP2: Error creating a temporary ' //
     :           '2-dimensional image.', STATUS )
         END IF
      END IF
