      SUBROUTINE CVG_BT2FT( FUNIT, EXTNAM, EXTVER, EXTLEVEL, TABLE,
     :                      STATUS )
*+
*  Name:
*     CVG_BT2FT
...
*  Local Variables:
      CHARACTER CARD*(CVG__HEDLEN)! A formatted header card
...
*  FitsChan.
         CALL CVG_HD2FC( FUNIT, HEADER, STATUS )
...
*  Report an error if anything went wrong in a FITSIO routine.
      IF( FSTAT .GT. CVG__FITSOK .AND. STATUS .EQ. SAI__OK ) THEN
         CALL CVG_FIOER( FSTAT, 'CVG_BT2FT_ERR', ' ', 'Failed to '//
