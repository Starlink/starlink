      SUBROUTINE CVG_FIOER( FSTAT, PARAM, ROUTIN, MESSGE, STATUS )
*+
*  Name:
*     CVG_FIOER
...
*     12-NOV-2013 (DSB):
*        Renamed from COF_FIOER and moved to CVG from CONVERT.
...
*  Local Variables:
      CHARACTER FITBUF*(CVG__HEDLEN)! Tranlated FITSIO error message
      CHARACTER OPSTR*(CVG__HEDLEN) ! Buffer for supplied message
...
*  and exit.
      IF ( FSTAT .LE. 0 ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'CVG_FIOER',
     :     'CVG_FIOER: No FITSIO error to report (programming error).',
     :     STATUS )
         GOTO 999
      END IF
