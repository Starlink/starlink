      SUBROUTINE ARY1_CVFUW( BAD, N, ARRAY, TYPE, PNTR, DCE, STATUS )
*+
*  Name:
*     ARY1_CVFUW
 
*  Purpose:
*     Convert a vectorised array from UNSIGNED WORD.
 
*  Language:
*     Starlink Fortran 77
 
*  Invocation:
*     CALL ARY1_CVFUW( BAD, N, ARRAY, TYPE, PNTR, DCE, STATUS )
 
*  Description:
*     The routine converts a vectorised array from UNSIGNED WORD to any
...
*        The input UNSIGNED WORD vectorised array containing the values to be
*        converted.
...
*  appropriate conversion routine.
      IF ( TYPE .EQ. '_BYTE' ) THEN
         CALL VEC_UWTOB( BAD, N, ARRAY, %VAL( CNF_PVAL( PNTR ) ), IERR,
     :                   NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_UBYTE' ) THEN
         CALL VEC_UWTOUB( BAD, N, ARRAY, %VAL( CNF_PVAL( PNTR ) ), IERR,
     :                    NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_DOUBLE' ) THEN
         CALL VEC_UWTOD( BAD, N, ARRAY, %VAL( CNF_PVAL( PNTR ) ), IERR,
     :                   NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_INTEGER' ) THEN
         CALL VEC_UWTOI( BAD, N, ARRAY, %VAL( CNF_PVAL( PNTR ) ), IERR,
     :                   NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_REAL' ) THEN
         CALL VEC_UWTOR( BAD, N, ARRAY, %VAL( CNF_PVAL( PNTR ) ), IERR,
     :                   NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_WORD' ) THEN
         CALL VEC_UWTOW( BAD, N, ARRAY, %VAL( CNF_PVAL( PNTR ) ), IERR,
     :                   NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_UWORD' ) THEN
         CALL VEC_UWTOUW( BAD, N, ARRAY, %VAL( CNF_PVAL( PNTR ) ), IERR,
     :                    NERR, STATUS )
...
*  Report an error if the output data type specified was not valid.
      IF ( STATUS .EQ. SAI__OK ) THEN
         IF( .NOT. TYPOK ) THEN
            STATUS = ARY__FATIN
            CALL MSG_SETC( 'ROUTINE', 'ARY1_CVFUW' )
            CALL MSG_SETC( 'BADTYPE', TYPE )
            CALL ERR_REP( 'ARY1_CVFUW_TYPE',
...
*  Call error tracing routine and exit.
      IF ( STATUS .NE. SAI__OK ) CALL ARY1_TRACE( 'ARY1_CVFUW',
     :                                             STATUS )
