      SUBROUTINE ARY1_GT1UW( BAD, LOC, TYPE, EL, RESULT, DCE, STATUS )
*+
*  Name:
*     ARY1_GT1UW
 
*  Purpose:
*     Read a 1-dimensional array of UNSIGNED WORD values from an HDS object.
 
*  Language:
*     Starlink Fortran 77
 
*  Invocation:
*     CALL ARY1_GT1UW( BAD, LOC, TYPE, EL, RESULT, DCE, STATUS )
 
*  Description:
*     The routine reads a 1-dimensional array of UNSIGNED WORD values from an
...
*  directly.
      IF ( TYPE .EQ. '_UWORD' ) THEN
         DCE = .FALSE.
         CALL DAT_GET( LOC, '_UWORD', 1, DIM, RESULT, STATUS )
...
*  data.
      ELSE
         ...
         CALL ARY1_CVTUW( BAD, EL, TYPE, PNTR, RESULT, DCE, STATUS )
         CALL ARY1_HUNMP( LOC, STATUS )
      END IF
 
*  Call error tracing routine and exit.
      IF ( STATUS .NE. SAI__OK ) CALL ARY1_TRACE( 'ARY1_GT1UW',
     :STATUS )
