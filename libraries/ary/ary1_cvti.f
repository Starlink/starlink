      SUBROUTINE ARY1_CVTI( BAD, N, TYPE, PNTR, RESULT, DCE, STATUS )
*+
*  Name:
*     ARY1_CVTI
...
*  Invocation:
*     CALL ARY1_CVTI( BAD, N, TYPE, PNTR, RESULT, DCE, STATUS )
...
*  conversion routine.
      IF ( TYPE .EQ. '_BYTE' ) THEN
         CALL VEC_BTOI( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                  NERR, STATUS )

      ELSE IF ( TYPE .EQ. '_UBYTE' ) THEN
         CALL VEC_UBTOI( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                   NERR, STATUS )

      ELSE IF ( TYPE .EQ. '_DOUBLE' ) THEN
         CALL VEC_DTOI( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                  NERR, STATUS )

      ELSE IF ( TYPE .EQ. '_INTEGER' ) THEN
         CALL VEC_ITOI( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                  NERR, STATUS )

      ELSE IF ( TYPE .EQ. '_REAL' ) THEN
         CALL VEC_RTOI( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                  NERR, STATUS )

      ELSE IF ( TYPE .EQ. '_WORD' ) THEN
         CALL VEC_WTOI( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                  NERR, STATUS )

      ELSE IF ( TYPE .EQ. '_UWORD' ) THEN
         CALL VEC_UWTOI( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                   NERR, STATUS )
...
*  Report an error if the input data type specified was not valid.
      IF ( STATUS .EQ. SAI__OK ) THEN
         IF( .NOT. TYPOK ) THEN
            STATUS = ARY__FATIN
            CALL MSG_SETC( 'ROUTINE', 'ARY1_CVTI' )
            CALL MSG_SETC( 'BADTYPE', TYPE )
            CALL ERR_REP( 'ARY1_CVTI_TYPE',
...
*  Call error tracing routine and exit.
      IF ( STATUS .NE. SAI__OK ) CALL ARY1_TRACE( 'ARY1_CVTI',
     :STATUS )
