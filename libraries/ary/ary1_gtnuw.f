      SUBROUTINE ARY1_GTNUW( BAD, HTYPE, LOC, NDIM, LBNDD, UBNDD, LSUB,
...
*  Purpose:
*     Get a UNSIGNED WORD n-dimensional subregion of an HDS object.
 
*  Language:
*     Starlink Fortran 77
 
*  Invocation:
*     CALL ARY1_GTNUW( BAD, LOC, NDIM, LBNDD, UBNDD, LSUB, USUB, LBNDA,
...
*     a UNSIGNED WORD Fortran array, making use of lower and upper bounds
...
*     ARRAY( * ) = INTEGER*2 (Returned)
*        Output UNSIGNED WORD array.
...
*        messages.
*     13-MAR-1990 (RFWS):
*        Changed to call ARY1_BADUW instead of VEC_BADUW.
*     22-MAR-1990 (RFWS):
...
*  string supplied is too long.
      ...
      ELSE
         STATUS = ARY__FATIN
         CALL MSG_SETC( 'ROUTINE', 'ARY1_GTNUW' )
         CALL MSG_SETC( 'BADHTYPE', HTYPE )
         CALL ERR_REP( 'ARY1_GTNUW_TYP',
...
*  which is not to receive data with bad values (if required).
         IF ( I .LE. DCONTG ) THEN
            IF ( PAD .AND. ( EA .GE. SA ) ) THEN
               CALL ARY1_BADUW( EA - SA + 1, ARRAY( SA ), STATUS )
               IF ( STATUS .NE. SAI__OK ) GO TO 9999
            END IF
...
*  annul the slice. Note if a data conversion error occurred.
            CALL ARY1_GT1UW( BAD, SLICE, TYPE, NCONTG, ARRAY( EA + 1 ),
     :      DCESLC, STATUS )
...
*  Call error tracing routine and exit.
      IF ( STATUS .NE. SAI__OK ) CALL ARY1_TRACE( 'ARY1_GTNUW',
     :STATUS )
