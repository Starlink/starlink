      SUBROUTINE ARY1_CVFB( BAD, N, ARRAY, TYPE, PNTR, DCE, STATUS )
*+
*  Name:
*     ARY1_CVFB

*  Purpose:
*     Convert a vectorised array from BYTE.

*  Language:
*     Starlink Fortran 77

*  Invocation:
*     CALL ARY1_CVFB( BAD, N, ARRAY, TYPE, PNTR, DCE, STATUS )

*  Description:
*     The routine converts a vectorised array from BYTE to any
...
*     ARRAY( N ) = BYTE (Given)
*        The input BYTE vectorised array containing the values to be
*        converted.
...
*  appropriate conversion routine.
      IF ( TYPE .EQ. '_BYTE' ) THEN
         CALL VEC_BTOB( BAD, N, ARRAY, %VAL( CNF_PVAL( PNTR ) ), IERR,
     :                  NERR, STATUS )

      ELSE IF ( TYPE .EQ. '_UBYTE' ) THEN
         CALL VEC_BTOUB( BAD, N, ARRAY, %VAL( CNF_PVAL( PNTR ) ), IERR,
     :                   NERR, STATUS )

      ELSE IF ( TYPE .EQ. '_DOUBLE' ) THEN
         CALL VEC_BTOD( BAD, N, ARRAY, %VAL( CNF_PVAL( PNTR ) ), IERR,
     :                  NERR, STATUS )

      ELSE IF ( TYPE .EQ. '_INTEGER' ) THEN
         CALL VEC_BTOI( BAD, N, ARRAY, %VAL( CNF_PVAL( PNTR ) ), IERR,
     :                  NERR, STATUS )

      ELSE IF ( TYPE .EQ. '_REAL' ) THEN
         CALL VEC_BTOR( BAD, N, ARRAY, %VAL( CNF_PVAL( PNTR ) ), IERR,
     :                  NERR, STATUS )

      ELSE IF ( TYPE .EQ. '_WORD' ) THEN
         CALL VEC_BTOW( BAD, N, ARRAY, %VAL( CNF_PVAL( PNTR ) ), IERR,
     :                  NERR, STATUS )

      ELSE IF ( TYPE .EQ. '_UWORD' ) THEN
         CALL VEC_BTOUW( BAD, N, ARRAY, %VAL( CNF_PVAL( PNTR ) ), IERR,
     :                   NERR, STATUS )
...
*  Report an error if the output data type specified was not valid.
      IF ( STATUS .EQ. SAI__OK ) THEN
         IF( .NOT. TYPOK ) THEN
            STATUS = ARY__FATIN
            CALL MSG_SETC( 'ROUTINE', 'ARY1_CVFB' )
            CALL MSG_SETC( 'BADTYPE', TYPE )
            CALL ERR_REP( 'ARY1_CVFB_TYPE',
...
*  Call error tracing routine and exit.
      IF ( STATUS .NE. SAI__OK ) CALL ARY1_TRACE( 'ARY1_CVFB',
     :                                             STATUS )
