...
#define GRP__OK     0
#define GRP__WNMF 101         
#define GRP__WPER 102
#define GRP__WLER 103
#define GRP__WMER 104
...
F77_INTEGER_FUNCTION(grp1_wild)( CHARACTER(FileSpec), CHARACTER(FileName),
...
*     a blank string as the file name and an error code (GRP__WNMF)
*     as the function value. Finally, a call to GRP1_EWILD with the
*     last returned value of Context will close down any files opened or
*     memory allocated by GRP1_WILD.

*  Language:
*     C, designed to be called from Fortran.

*  Invocation:
*     STATUS = GRP1_WILD( FILESPEC, FILENAME, CONTEXT )
...
/* If no characters were read, set status */
         if( Ichar == 0 ) Status = GRP__WNMF;

/* If Status is bad exit while loop */
         if (Status != GRP__OK ) break;
...
F77_INTEGER_FUNCTION(grp1_ewild)( POINTER(Context) ) {
/*
*  Name:
*     GRP1_EWILD

*  Purpose:
*     Terminate a sequence of GRP1_WILD calls.

*  Description:
*     This routine should be called after a sequence of calls to GRP1_WILD
