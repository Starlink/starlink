      SUBROUTINE FTS1_MANDH( FIRST, NCARD, HEADER, SCARD, BITPIX, NDIM,
...
*  Invocation:
*     CALL FTS1_MANDH( FIRST, NCARD, HEADER, SCARD, BITPIX, NDIM, AXIS,
...
*  Arguments Given:
      LOGICAL FIRST
      INTEGER NCARD
      CHARACTER * 80 HEADER( NCARD ) ! FITS tape buffer
      INTEGER SCARD
...
*  Check the tape is simple FITS.
     ...
     :        HEADER( 1 )( 30:30 ) .NE. 'T' ) THEN
            ...
            STATUS = SAI__ERROR
            CALL ERR_REP( 'FTS1_MANDH_SIMPLE',
...
*  bits per pixel.
     ...
     :                 COMENT, NKC, STATUS )
      ...
      IF ( STATUS .NE. SAI__OK ) THEN
         CALL ERR_REP( 'FTS1_MANDH_FBITPI',
...
*  See if there was a BITPIX header card.
      IF ( .NOT. THERE ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'FTS1_MANDH_BITPIX',
     :     'BITPIX not present', STATUS )
         GOTO 990
      END IF

*  BITPIX can currently only have values of 8, 16 and 32.  Later -32
*  will probably mean 32-bit floating point.
      IF ( BITPIX .NE. 8 .AND. BITPIX .NE. 16 .AND.
     :     BITPIX .NE. 32  .AND. BITPIX .NE. -32 .AND.
     :     BITPIX .NE. -64 ) THEN

         STATUS = SAI__ERROR
         CALL ERR_REP( 'FTS1_MANDH_IBITPI',
...
*  Now obtain the number of dimensions.
     ...
     :                 COMENT, NKC, STATUS )
      ...
      IF ( STATUS .NE. SAI__OK ) THEN
         CALL ERR_REP( 'FTS1_MANDH_INAXIS',
     :     'Error converting NAXIS', STATUS )
      ...
      IF ( .NOT. THERE ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'FTS1_MANDH_NAXIS',
     :     'NAXIS not present', STATUS )
         GOTO 990
      END IF

*  Zero dimension means there is no main data array.  Upper limit is
...
*  Get value of NAXISn.
     ...
     :                       AXIS( N ), COMENT, NKC, STATUS )
            ...
            IF ( STATUS .NE. SAI__OK ) THEN
               CALL MSG_SETI( 'AXN', N )
               CALL ERR_REP( 'FTS1_MANDH_FAXISN',
     :              'Error converting AXIS^AXN value', STATUS )
            ...
            IF ( .NOT. THERE ) THEN
               CALL MSG_SETI( 'AXN', N )
               STATUS = SAI__ERROR
               CALL ERR_REP( 'FTS1_MANDH_NAXISn',
...
*  Check that it is physical or does not have an integer value.
            IF ( AXIS( N ) .LT. 1 .AND. ( .NOT. NONSDA ) ) THEN
               ...
               CALL ERR_REP( 'FTS1_MANDH_AXISIZ',
