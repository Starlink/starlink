      SUBROUTINE FTS1_PCADC( IPROV, FUNIT, STATUS )
*+
*  Name:
*     FTS1_PCADC
...
*        NDG.
*     12-NOV-2013 (DSB):
*        Renamed from COF_PCADC to FTS1_PCADC and moved from CONVERT to
*        KAPLIBS, so that CUPID can use it. The old NDF argument has
...
*  Issue warning if OBSIDSS is absent.
            IF ( .NOT. OBIPRS ) THEN
               ...
               CALL MSG_OUTIF( MSG__NORM, 'FTS1_PCADC_NOOBSIDSS',
...
*  reserved for non-fatal warnings.
      IF ( FSTAT .GT. FITSOK ) THEN
         CALL FTS1_FIOER( FSTAT, 'FTS1_PCADC_ERR', 'FTPKYJ',
