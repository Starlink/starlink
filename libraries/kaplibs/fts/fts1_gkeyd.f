      SUBROUTINE FTS1_GKEYD( NCARD, BUFFER, SCARD, NAME, NOCCUR,
...
*  Arguments Returned:
      LOGICAL THERE              ! Is card containing the keyword
                                 ! present?
      DOUBLE PRECISION VALUE               ! Value of the keyword
...
*  records).
                  IF ( NCHV .GT. 0 ) THEN
                     CALL ERR_MARK
                     CALL CHR_CTOD( CHRVAL( :NCHV ), VALUE, STATUS )
...
*  Report the error context.
                  IF ( STATUS .EQ. SAI__ERROR ) THEN
                     ...
                     CALL ERR_REP( 'FTS1_GKEYD_TYPCNV',
...
*  records).
                     IF ( NCHV .GT. 0 ) THEN
                        CALL ERR_MARK
                        CALL CHR_CTOD( CHRVAL( :NCHV ), VALUE,
     :                                   STATUS )
                        IF ( STATUS .NE. SAI__OK ) THEN
                           CALL ERR_ANNUL( STATUS )
                           STATUS = SAI__ERROR
