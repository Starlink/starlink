...
*  Purpose:
*     Implement the KAPPA grf module using the PGPLOT graphics system.

*  Description:
*     This file implements the low level graphics functions required
*     by AST, by calling suitable PGPLOT functions (the FORTRAN PGPLOT 
*     interface is used).
...
#include "cnf.h"                 /* C to FORTRAN interface functions */
#include "ast.h"                 /* AST constants */
#include "grf.h"                 /* GRF constants */
...
/* Fill in any missing parts of the justification string. */
      if( just ){
         if( just[ 0 ] == 'T' || just[ 0 ] == 'C' || just[ 0 ] == 'B' ){
            ...
            fprintf( stderr, "astGText: Justification string '%s' is "
                      "invalid.", just );
            return 0;
         }
         ...
         if( just[ 1 ] == 'L' || just[ 1 ] == 'C' || just[ 1 ] == 'R' ){
            ...
            fprintf( stderr, "astGText: Justification string '%s' "
                      "is invalid.", just );
            return 0;
         }
...
/* Normalise the up-vector in world coordinates. */
         ...
         if( uplen > 0.0 ){ 
            ...
            fprintf( stderr, "astGText: Zero length up-vector supplied.");
            return 0;
         }
...
/* Find the conversion factors between increment in world coordinate axes, 
   ...
   if( wx2 != wx1 && wy2 != wy1 && 
      ...
      *beta = ( ny2 - ny1 ) / ( wy2 - wy1 );
      ...
      fprintf( stderr, "astGAxScale: The graphics window or viewport has zero size." );
      ret = 0;
   }
...
/* Fill in any missing parts of the justification string. */
      if( just ){
         if( just[ 0 ] == 'T' || just[ 0 ] == 'C' || just[ 0 ] == 'B' ){
            ...
            fprintf( stderr, "astGTxExt: Justification string '%s' is "
                      "invalid.", just );
            return 0;
         }
...
/* Convert the text height from normalised device coordinates into world 
   ...
   if( vx1 != vx2 ){
      *chv *= ( wx2 - wx1 )/( vx2 - vx1 );
      ...
      fprintf( stderr, "astGQch: The graphics viewport has zero size "
                "in the X direction.");
      return 0;
   }   
...
/* If required retrieve the current character size, and set a new size. 
   ...
   default character size. The KAPPA version uses the existing PGPLOT character
   size scaling factor as the default character size. This is stored in 
   static variable "origsize". */
      ...
      if( old_value ) {
         if( origsize < 0.0 ) {
            ...
            *old_value = (double) ( rval/origsize );
         }
      }
      ...
      if( value != AST__BAD ){
         ...
         if( origsize < 0.0 ) {
            if( value != 1.0 ) {
               ccpgsch( rval* (float) value );
               origsize = rval;
            }
         } else {
            ccpgsch( origsize* (float) value );
            if( value == 1.0 ) origsize = -1.0;
         }
...
/* Give an error message for any other attribute value. */
      ...
      fprintf( stderr, "astGAttr: Unknown graphics attribute '%d' "
                "requested.", attr );
      return 0;
   }
...
F77_SUBROUTINE(grf_gattr)( INTEGER(ATTR), LOGICAL(USE), DOUBLE(VALUE), 
                           DOUBLE(OLDVAL), INTEGER(PRIM), INTEGER( ISTAT ) ) {
   GENPTR_INTEGER(ATTR)
   GENPTR_LOGICAL(USE)
   GENPTR_DOUBLE(VALUE)
   GENPTR_DOUBLE(OLDVAL)
   GENPTR_INTEGER(PRIM)
   GENPTR_INTEGER(ISTAT)

   if( F77_ISTRUE( *USE ) ) {
      *ISTAT = astGAttr( *ATTR, *VALUE, OLDVAL, *PRIM );
   } else {
