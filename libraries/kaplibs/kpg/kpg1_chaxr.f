      SUBROUTINE KPG1_CHAXR( INDF, NDIM, DIM, DATAVL, SCALE, OFFSET,
     :                         STATUS )
*+
*  Name:
*     KPG1_CHAXx
...
*  Local Variables:
      ...
      REAL DALBND( NDF__MXDIM ) ! AXIS lower bounds
      REAL DAUBND( NDF__MXDIM ) ! AXIS upper bounds
...
*  Map the axis centre values for this dimension as an array of _REAL
*  values.
            CALL NDF_AMAP( INDF, 'Centre', AXIS, '_REAL', 'READ',
     :                     APNTR, AEL, STATUS )
...
*  co-ordinates.
               CALL ERR_MARK
               CALL KPG1_MONOR( .TRUE., AEL, %VAL( CNF_PVAL( APNTR ) ), 
     :                          MONOTO,
     :                           STATUS )
...
*  messages are issued for each non-linear axis.
      IF ( DATAVL ) THEN
         CALL KPG1_DCLIR( NDIM, INDF, DALBND, DAUBND, SCALE, OFFSET,
     :                     DATAVL, STATUS )
...
*  in the transformation from world to data co-ordinates derived below.
         IF ( .NOT. DATAVL ) THEN
            ...
            DO I = 1, NDIM
               AXIS = DIM( I )
               SCALE( I ) = ( DAUBND( I ) - DALBND( I ) ) / NUM_ITOR
