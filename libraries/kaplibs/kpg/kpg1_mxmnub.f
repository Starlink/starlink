      SUBROUTINE KPG1_MXMNUB( BAD, EL, ARRAY, NINVAL, MAXMUM, MINMUM,
...
*  Watch for a bad first value.
         IF ( ARRAY( 1 ) .EQ. VAL__BADUB ) THEN
 
*  Search for the first non-bad value.
            I = 2
            DO WHILE ( I .LE. EL  .AND.
     :                 ARRAY( MIN( I, EL ) ) .EQ. VAL__BADUB )
               I = I + 1
            END DO
            NINVAL = I - 1
...
*  Use signed integers for the comparisons.
         IMAX = NUM_UBTOI( MAXMUM )
         IMIN = NUM_UBTOI( MINMUM )
...
*  Convert the integers back to the unsigned-integer type for export.
         MAXMUM = NUM_ITOUB( IMAX )
         MINMUM = NUM_ITOUB( IMIN )
      END IF
