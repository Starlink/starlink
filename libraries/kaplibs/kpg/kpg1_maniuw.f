      SUBROUTINE KPG1_MANIUW( NDIMI, DIMI, IN, NDIMO, DIMO, AXES, 
...
*  Invocation:
*     CALL KPG1_MANIUW( NDIMI, DIMI, IN, NDIMO, DIMO, AXES, COLOFF,
*                        EXPOFF, OUT, STATUS )
...
*  Local Variables:
      ...
      INTEGER*2 PIXI                ! Value of the current inupt pixel
      INTEGER*2 PIXO                ! Value of the new output pixel
...
*  collapsed dimensions to form the output pixel.
         ELSE
            ...
            DO IC = 1, NCOLL
               ...
               IF ( PIXI .NE. VAL__BADUW ) THEN
                  SUM = SUM + NUM_UWTOD( PIXI )
                  NVAL = NVAL + 1
               END IF
            END DO
            IF ( NVAL .GT. 0 ) THEN
               PIXO = NUM_DTOUW( SUM / DBLE( NVAL ) )
            ELSE
               PIXO = VAL__BADUW
            END IF
         END IF
