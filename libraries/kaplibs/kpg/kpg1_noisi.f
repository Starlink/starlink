      SUBROUTINE KPG1_NOISI( BAD, EL, VARNCE, ARRAY, STATUS )
*+
*  Name:
*     KPG1_NOISx
...
*  noise cannot be added to the data array, so set it bad.
            IF ( ARRAY( I ) .EQ. VAL__BADI .OR.
     :           VARNCE( I ) .EQ. VAL__BADI ) THEN
               ARRAY( I ) = VAL__BADI

            ELSE

*  Find the  of the Normal distribution at the element.
               SIGMA = SQRT( NUM_ITOR( VARNCE( I ) ) )

*  Generate the random noise.  There is no offset.
               ARRAY( I ) = ARRAY( I ) +
     :                      NUM_RTOI( PDA_RNNOR( 0.0, SIGMA ) )
            END IF
         END DO
