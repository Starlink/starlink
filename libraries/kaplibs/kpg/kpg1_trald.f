      SUBROUTINE KPG1_TRALD( NDIMI, IDIMS, LBND, UBND, TRID, IEL,
...
*  Arguments Returned:
      DOUBLE PRECISION COIN( IEL, NDIMI )
      DOUBLE PRECISION COOUT( IEL, NDIMO )
...
*  routine, as the transformation is applied row by row.
      DO I = 1, IDIMS( 1 ) + 1
         COIN( I, 1 ) = LBND( 1 ) + NUM_ITOD( I - 1 ) * STEP( 1 )
      END DO
...
*   subroutine call.
         CALL TRN_TRND( .TRUE., IEL, NCOUT, IDIMS( 1 ) + 1, COIN,
...
*   the output co-ordinate system.
            DO I = 1, IDIMS( 1 ) + 1
               IF ( COOUT( I, IAXIS ) .NE. VAL__BADD ) THEN
