      SUBROUTINE KPG1_AINBD( LBND, UBND, AXIS, EL, VALUE, INDEX,
     :                        STATUS )
*+
*  Name:
*     KPG1_AINBx
...
*  reached or there are insufficient axis values remaining.
     ...
     :           ( UBND - LBC ) .GT. 0 )
         INCREM = NUM_DTOD( AXIS( UBND ) - AXIS( LBC ) )
...
*  co-ordinates.
               ...
               SLOPE = NUM_DTOD( AXIS( UB ) - AXIS( LB ) ) /
     :                 DBLE( UB - LB )
...
*  rather than the difference in axis values.
               IF ( RESID / SLOPE .LT. 0.0D0 ) THEN
                  ...
                  DIFF = NUM_DTOD( AXIS( UB - 1 ) - AXIS( UB ) )
                  IF ( ABS( DIFF ) .LT. NUM_DTOD( VAL__EPSD ) ) THEN
                     DIFF = SLOPE
                  END IF
                  FRAC = NUM_DTOD( VALUE( I ) - AXIS( UB ) ) / DIFF
 
                  LOOP = NINT( ABS( FRAC ) ) .GE. 2
 
*  Evaluate the required index.
                  IF ( .NOT. LOOP )
     :              INDEX( I ) = NUM_DTOD( FRAC ) + NUM_ITOD( UB )
...
*  rather than the difference in axis values.
               ELSE
                  ...
                  DIFF = NUM_DTOD( AXIS( LB + 1 ) - AXIS( LB ) )
                  IF ( ABS( DIFF ) .LT. NUM_DTOD( VAL__EPSD ) ) THEN
                     DIFF = SLOPE
                  END IF
                  FRAC = NUM_DTOD( VALUE( I ) - AXIS( LB ) ) / DIFF
 
                  LOOP = NINT( ABS( FRAC ) ) .GE. 2
 
*  Evaluate the required index.
                  IF ( .NOT. LOOP )
     :              INDEX( I ) = NUM_DTOD( FRAC ) + NUM_ITOD( LB )
               END IF
            END DO
         END IF
      END DO
