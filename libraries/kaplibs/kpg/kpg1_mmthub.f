      SUBROUTINE KPG1_MMTHUB( BAD, EL, ARRAY, THRESH, NINVAL, MAXMUM,
...
*  Test for valid pixel.
     ...
     :           NUM_GTUB( ARRAY( I ), OTHRES( 1 ) ) .AND.
     :           NUM_LTUB( ARRAY( I ), OTHRES( 2 ) ) ) THEN
 
*  Check current maximum against current pixel value.
               IF ( NUM_GTUB( ARRAY( I ), MAXMUM ) ) THEN
                  MAXMUM = ARRAY( I )
                  MAXPOS = I
               END IF
 
*  Check current minimum against current pixel value.
               IF ( NUM_LTUB( ARRAY( I ), MINMUM ) ) THEN
                  MINMUM = ARRAY( I )
                  MINPOS = I
               END IF
            ELSE
...
*  Loop round all the elements of the array.
         DO I = 1, EL
            ...
            IF ( NUM_GTUB( ARRAY( I ), OTHRES( 1 ) ) .AND.
     :           NUM_GTUB( ARRAY( I ), OTHRES( 2 ) ) ) THEN
