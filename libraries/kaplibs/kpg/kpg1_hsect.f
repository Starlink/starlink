      SUBROUTINE KPG1_HSECT( LOC1, NDIM, LBND, UBND, LOC2, NAME,
     :                       STATUS )
*+
*  Name:
*     KPG1_HSECT
...
*  Local Variables:
      ...
      INTEGER CLEN
      INTEGER DIMIN( DAT__MXDIM )
      INTEGER DIMOUT( DAT__MXDIM )
      INTEGER EL
      INTEGER I
      INTEGER IPIN
      INTEGER IPOUT
      INTEGER LBNDI( DAT__MXDIM )
      INTEGER NDIMIN
...
*  array.
      DO I = 1, NDIM
         ...
         IF( DIMOUT( I ) .LT. 1 .AND. STATUS .EQ. SAI__OK ) THEN
            CALL MSG_SETI( 'A', I )
            CALL MSG_SETI( 'L', LBND( I ) )
            CALL MSG_SETI( 'U', UBND( I ) )

            STATUS = SAI__ERROR
            CALL ERR_REP( ' ', 'KPG1_HSECT: Supplied lower bound on '//
...
*  Map it as a vector.
      CALL DAT_MAPV( LOC1, TYPE, 'READ', IPIN, EL, STATUS )

*  Create the required new component, and get a locator to it.
      CALL DAT_NEW( LOC2, NAME, TYPE, NDIM, DIMOUT, STATUS )
...
*  for each data type.
      IF( TYPE .EQ. '_INTEGER' ) THEN
         CALL KPG1_CPNDI( NDIM, LBNDI, UBNDI, %VAL( CNF_PVAL( IPIN ) ),
     :                    LBND, UBND, %VAL( CNF_PVAL( IPOUT ) ), EL,
     :                    STATUS )

      ELSE IF( TYPE .EQ. '_REAL' ) THEN
         CALL KPG1_CPNDR( NDIM, LBNDI, UBNDI, %VAL( CNF_PVAL( IPIN ) ),
     :                    LBND, UBND, %VAL( CNF_PVAL( IPOUT ) ), EL,
     :                    STATUS )
     ...
     :                    STATUS )
      ...
      ELSE IF( TYPE .EQ. '_BYTE' ) THEN
         CALL KPG1_CPNDB( NDIM, LBNDI, UBNDI, %VAL( CNF_PVAL( IPIN ) ),
     :                    LBND, UBND, %VAL( CNF_PVAL( IPOUT ) ), EL,
     :                    STATUS )

      ELSE IF( TYPE .EQ. '_UBYTE' ) THEN
         CALL KPG1_CPNDUB( NDIM, LBNDI, UBNDI, %VAL( CNF_PVAL( IPIN ) ),
     :                    LBND, UBND, %VAL( CNF_PVAL( IPOUT ) ), EL,
     :                    STATUS )

      ELSE IF( TYPE .EQ. '_INT64' ) THEN
         CALL KPG1_CPNDK( NDIM, LBNDI, UBNDI, %VAL( CNF_PVAL( IPIN ) ),
     :                    LBND, UBND, %VAL( CNF_PVAL( IPOUT ) ), EL,
     :                    STATUS )
     ...
     :                    %VAL( CNF_CVAL( CLEN ) ) )
      ...
      ELSE IF( STATUS .EQ. SAI__OK ) THEN
         ...
         STATUS = SAI__ERROR
         CALL ERR_REP( ' ', 'KPG1_HSECT: Unsupported data type ^T',
     :                 STATUS )
      END IF
