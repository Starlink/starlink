      SUBROUTINE KPG1_CPNTW( PNNDF, PNTIT, NDIM, DIMS, ARRAY, NULL,
     :                         STATUS )
*+
*  Name:
*     KPG1_CPNTx
...
*    Create a new NDF.
      ...
      CALL NDF_CREP( PNNDF, '_WORD', NDIM, DIMS, NDFO, STATUS )
 
*    Map the data array.  Wrap to prevent line overflow when the token
*    is expanded.
 
      CALL KPG1_MAP( NDFO, 'Data', '_WORD', 'WRITE', OPNTR, EL,
     :              STATUS )
...
*    conversion errors so they are not checked.
      ...
      CALL VEC_WTOW( .FALSE., EL, ARRAY, %VAL( CNF_PVAL( OPNTR( 1 ) ) ),
     :                   IERR, NERR, STATUS )
