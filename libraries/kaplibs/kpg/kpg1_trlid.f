      SUBROUTINE KPG1_TRLID( NDIMI, IDIMS, INARR, VAR, INVAR, TRID,
...
*  Invocation:
*     CALL KPG1_TRLIx( NDIMI, IDIMS, INARR, VAR, INVAR, TRID, FLUX,
...
*     KPG1_TDLIx for double precision.
...
*  Arguments Returned:
      ...
      REAL COIN( OEL, NDIMI )
      REAL COOUT( OEL, NDIMO )
      REAL INDICE( OEL, NDIMI )
...
*  Local Variables:
      ...
      REAL  FRAC( NDF__MXDIM, 2 ) ! Interpolation fractions
      ...
      INTEGER NCOUT            ! Number of values in the forward
                               ! transformation
      DOUBLE PRECISION NEIGH( 2 ** NDF__MXDIM ) ! Vector of input data values
                               ! about a transformed point
      INTEGER OFFSET           ! Offset within the first dimension
                               ! of the output array
      REAL ORIGIN( NDF__MXDIM ) ! Origins of each axis
      ...
      INTEGER STRIDO( NDF__MXDIM ) ! Dimension strides for output arrays
                               ! and co-ordinates
      DOUBLE PRECISION VNEIGH( 2 ** NDF__MXDIM ) ! Vector of input variance
...
*  routine, as the transformation is applied row by row.
      ORIGIN( 1 ) = REAL( OLBND( 1 ) )
      DO I = 1, ODIMS( 1 )
         COOUT( I, 1 ) = ORIGIN( 1 ) + REAL( I - 1 ) - 0.5
      END DO
      AXJUMP( 1 ) = 1
...
*  Initialise the input co-ordinates for any higher dimensions.
      IF ( NDIMO .GT. 1 ) THEN
         DO IAXIS = 2, NDIMO
            ORIGIN( IAXIS ) = REAL( OLBND( IAXIS ) )
            AXJUMP( IAXIS ) = AXJUMP( IAXIS - 1 ) + IDIMS( IAXIS - 1 )
            DO I = 1, ODIMS( 1 )
               COOUT( I, IAXIS ) = ORIGIN( IAXIS ) - 0.5
            END DO
         END DO
      END IF
...
*   of the input array.
            CALL TRN_TRNR( .TRUE., OEL, NCOUT, ODIMS( 1 ), COOUT,
...
*   onto the next output element.
         ...
         DO IAXIS = 1, NDIMI
            I = MOD( K - 1, ODIMS( 1 ) ) + 1
            IF ( INDICE( I, IAXIS ) .EQ. VAL__BADR ) THEN
               OUTARR( K ) = VAL__BADD
               IF ( VAR ) OUTVAR( K ) = VAL__BADD
               GO TO 80
...
*   defined to be 1, and KPG1_AINBR returns a bad value for indices
*   less than the lower bound.
            ELSE
               VO = VO + INT( INDICE( I, IAXIS ) - 0.5 ) *
     :              STRIDI( IAXIS )
            END IF
         END DO
...
*   Loop around the neighbourhood elements either side of the point.
            DO J = 1, 2 ** NDIMI
               IF ( NEIGH( J ) .NE. VAL__BADD ) THEN
 
*   Form the weight for the current element.
                  W = 1.0D0
                  DO I = 1, NDIMI
                     W = W * DBLE( FRAC( I, MOD( ( J - 1 ) / ESTR( I ),
     :                   2 ) + 1 ) )
                  END DO
 
*   Integrate the weights and the weighted value.
                  WSUM = WSUM + W
                  ASUM = ASUM + NUM_DTOD( NEIGH( J ) ) * W
               END IF
            END DO
