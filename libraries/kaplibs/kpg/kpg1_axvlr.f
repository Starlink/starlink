      SUBROUTINE KPG1_AXVLR( LBND, UBND, AXIS, EL, PIXCO, DATCO,
     :                       STATUS )
*+
*  Name:
*     KPG1_AXVLx
...
*  Local Variables:
      ...
      REAL PIXIND             ! Input co-ordinate as a pixel index
...
*  Check the pixel co-ordinate does not lie outside the bounds.
         IF ( PIXCO( I ) .LT. NUM_ITOR( LBND ) - 1.0E0 .OR.
     :        PIXCO( I ) .GT. NUM_ITOR( UBND ) ) THEN
            STATUS = SAI__ERROR
            CALL MSG_SETR( 'PIXCO', PIXCO( I ) )
...
*  not between pixels, and so cannot be interpolated.
         ELSE IF ( PIXCO( I ) .LE. NUM_ITOR( LBND ) -
     :             NUM_RTOR( 0.5 ) ) THEN
            DATCO( I ) = AXIS( LBND )
 
*  Deal with a co-ordinate within the axis upper-bound element, but
*  not between pixels, and so cannot be interpolated.
         ELSE IF ( PIXCO( I ) .GE. NUM_ITOR( UBND ) -
     :             NUM_RTOR( 0.5 ) ) THEN
            DATCO( I ) = AXIS( UBND )
