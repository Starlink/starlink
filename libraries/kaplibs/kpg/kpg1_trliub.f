      SUBROUTINE KPG1_TRLIUB( NDIMI, IDIMS, INARR, VAR, INVAR, TRID,
...
*   Test for a bad input data value.
         IF ( INARR( VO ) .EQ. VAL__BADUB ) THEN
            OUTARR( K ) = VAL__BADUB
            VALDET = .TRUE.
         END IF
...
*   Test for a bad input variance value.
         IF ( VAR ) THEN
            IF ( INVAR( VO ) .EQ. VAL__BADUB ) THEN
               OUTVAR( K ) = VAL__BADUB
               VARDET = .TRUE.
            END IF
         ELSE
...
*   Loop around the neighbourhood elements either side of the point.
            DO J = 1, 2 ** NDIMI
               IF ( VNEIGH( J ) .NE. VAL__BADUB ) THEN
...
*   Integrate the weights and the weighted value.
                  WSUM = WSUM + W
                  ASUM = ASUM + NUM_UBTOD( VNEIGH( J ) ) * W
               END IF
            END DO
