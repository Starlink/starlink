      SUBROUTINE KPG1_MONOR( BAD, EL, ARRAY, MONOTO, STATUS )
*+
*  Name:
*     KPG1_MONOx
...
*  Find the first two non-bad values.  Exclude any bad data.
         ...
         END DO
         HIGH = MIN( LOW + 1, EL )
         DO WHILE ( ARRAY( HIGH ) .EQ. VAL__BADR .AND. HIGH .LT. EL )
            HIGH = HIGH + 1
         END DO
...
*  appropriate test for monotonicity depending on the polarity.
               IF ( ARRAY ( I + 1 ) .NE. VAL__BADR ) THEN
                  IF ( INCREA ) THEN
...
*  Watch for zero increase, and flag as non-monotonic.
         IF ( .NOT. INCREA ) THEN
            MONOTO = ABS( ARRAY( 2 ) - ARRAY( 1 ) ) .GT. VAL__EPSR
         END IF
