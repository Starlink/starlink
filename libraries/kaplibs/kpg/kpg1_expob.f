      SUBROUTINE KPG1_EXPOB( BAD, EL, INARR, VAR, INVAR, BASE, OUTARR,
...
*  Local Variables:
      ...
      BYTE MAXVAL             ! Maximum valid pixel value allowed
                                 ! for exponentiation
      DOUBLE PRECISION MAXV      ! Maximum value for the data type
      BYTE MAXVAR             ! Maximum valid variance value allowed
                                 ! for exponentiation
      DOUBLE PRECISION MINV      ! Minimum value for the data type
      BYTE MINVAL             ! Minimum valid pixel value allowed
                                 ! for exponentiation
...
*  Define a couple of useful variables.
      MINV = NUM_BTOD( VAL__MINB )
      MAXV = NUM_BTOD( VAL__MAXB )
...
*  of cases, and saves testing pixel by pixel.
         MINVAL = NUM_DTOB( MIN( MAXV, MAX( MINV,
     :            LOG( MINV ) / LOG( BASE ) ) ) )
...
*  and bases close to 1, but it should protect the vast majority
*  of cases, and saves testing pixel by pixel.
         MAXVAL = NUM_DTOB( MIN( MAXV, MAX( MINV,
     :            LOG( MAXV ) / LOG( BASE ) ) ) )
...
*  This is only approximate because it depends on the output value too.
         MAXVAR = NUM_DTOB( MIN( MAXV, MAX( MINV,
...
*  Propagate a bad pixel.
                  IF ( INVAR( J ) .EQ. VAL__BADB .OR.
     :                  OUTARR( J ) .EQ. VAL__BADB ) THEN
                     OUTVAR( J ) = VAL__BADB
