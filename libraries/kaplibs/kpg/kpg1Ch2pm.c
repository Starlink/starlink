...
/* Check the supplied locator is for a scalar. */
   ...
   if( size > 1 && *status == SAI__OK) {
      *status = SAI__ERROR;
      errRep( " ", "kpg1Ch2pm: Supplied HDS object is not scalar "
...
/* 2D Chebyshev polynomials are defined on a square (-1:1,-1:1), which is 
   ...
   Sn = ( 2*AXISn - ( TMAX(n) + TMIN(n) ) )/( TMAX(n) - TMIN(n) ) */
      ...
      datFind( loc, "TMIN", &tminloc, status );
      datMapV( tminloc, "_DOUBLE", "Read", (void *) &tmin, &nel, status );
      if( nel != ndim && *status == SAI__OK) {
         msgSeti( "N", nel );
         msgSeti( "M", ndim );
         *status = SAI__ERROR;
         errRep( " ", "kpg1Ch2pm: Corrupt POLYNOMIAL structure supplied: TMIN "
                 "has length ^N but DATA_ARRAY has ^M axes.", status );
      }

      datFind( loc, "TMAX", &tmaxloc, status );
      datMapV( tmaxloc, "_DOUBLE", "Read", (void *) &tmax, &nel, status );
      if( nel != ndim && *status == SAI__OK) {
         msgSeti( "N", nel );
         msgSeti( "M", ndim );
         *status = SAI__ERROR;
         errRep( " ", "kpg1Ch2pm: Corrupt POLYNOMIAL structure supplied: TMAX "
                 "has length ^N but DATA_ARRAY has ^M axes.", status );
      }
...
/* Allocate workspace. */
         ...
         coeffs_f = astMalloc( 3*nc*sizeof( double ) );
...
/* Store the coefficients in a form suitable for use with the AST PolyMap
   constructor. */
         ncoeff_f = 0;
         p = coeffs_f;
         q = w2;
...
/* Create the PolyMap. */
         result = astPolyMap( 1, 1, ncoeff_f, coeffs_f, 0, NULL, " " );
...
/* Allocate workspace. */
         w1 = astMalloc( nc*sizeof( double ) );
         coeffs_f = astMalloc( 4*nc*sizeof( double ) );
...
/* Create the PolyMap. */
         result = astPolyMap( 2, 1, ncoeff_f, coeffs_f, 0, NULL, " " );
...
/* Currently, we can only handle 1D or 2D Chebyshev polynomials. */
         ...
         msgSeti( "N", ndim );
         *status = SAI__ERROR;
         errRep( " ", "kpg1Ch2pm: Supplied polynomial is ^N-dimensional. "
...
/* Now handle SIMPLE polynomials */
/* ============================= */
   } else if( !strcmp( variant, "SIMPLE" ) && *status == SAI__OK ) {

/* Allocate workspace. */
      coeffs_f = astMalloc( ( ndim + 2 )*nc*sizeof( double ) );
...
/* Loop round all the coefficients in POLYNOMIAL.DATA_ARRAY. */
      ncoeff_f = 0;
      p = coeffs_f;
      q = data;
      for( i = 0; i < nc; i++,q++ ) {

/* If the coefficient is non-zero, add it to the array used by the
   PolyMap constructor. */
         if( *q != 0.0 ) {
            ncoeff_f++;
            *(p++) = *q;
            *(p++) = 1.0;
            for( j = 0; i < ndim; j++ ) *(p++) = power[ j ];
         }
...
/* Create the PolyMap. */
      result = astPolyMap( ndim, 1, ncoeff_f, coeffs_f, 0, NULL, " " );
...
/* Report an error for an unknown polynomial variant. */
      ...
      msgSetc( "V", variant );
      *status = SAI__ERROR;
      errRep( " ", "kpg1Ch2pm: Unsupported POLYNOMIAL variant '^V' "
...
/* If no error has occurred, return the new PolyMap pointer. */
   if( result ) {
      if( *status == SAI__OK ) {
         *polymap = result;
      } else {
