      SUBROUTINE KPG1_MXMER( BAD, EL, ARRAY, ERROR, NSIGMA, THRESH,
...
*  Local Variables:
      REAL AMINUS             ! Data value less error
      REAL APLUS              ! Data value plus error
      REAL DELTA              ! Error-bar offsets
      INTEGER I                  ! Counter
      REAL OTHRES( 2 )        ! Thresholds where first element is the
...
*  Add some defensive code in case the thresholds are reversed.
      OTHRES( 1 ) = NUM_MINR( THRESH( 1 ), THRESH( 2 ) )
      OTHRES( 2 ) = NUM_MAXR( THRESH( 1 ), THRESH( 2 ) )
...
*  Test for valid pixel.
            IF ( ARRAY( I ) .NE. VAL__BADR .AND.
     :           ERROR( I ) .NE. VAL__BADR ) THEN
...
*  types.
               DELTA = NUM_RTOR( ABS( NUM_RTOR( ERROR( I ) ) *
     :                 NSIGMA ) )
               APLUS = NUM_ADDR( ARRAY( I ), DELTA )
               AMINUS = NUM_SUBR( ARRAY( I ), DELTA )
 
*  Test that the value is between the thresholds.
               IF ( NUM_GTR( AMINUS, OTHRES( 1 ) ) .AND.
     :              NUM_LTR( APLUS, OTHRES( 2 ) ) ) THEN
 
*  Check current maximum against current pixel value.
                  IF ( NUM_GTR( APLUS, MAXMUM ) ) THEN
                     MAXMUM = APLUS
                     MAXPOS = I
                  END IF
 
*  Check current minimum against current pixel value.
                  IF ( NUM_LTR( AMINUS, MINMUM ) ) THEN
                     MINMUM = AMINUS
                     MINPOS = I
                  END IF
