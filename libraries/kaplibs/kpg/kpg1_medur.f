      SUBROUTINE KPG1_MEDUR( BAD, EL, ARRAY, MEDIAN, NELUSE, STATUS )
*+
*  Name:
*     KPG1_MEDUx
...
*  Local Variables:
      INTEGER CURR               ! Index to current smallest value
                                 ! during sorting
      REAL CURVAL             ! Current smallest value during sorting
...
*  convert to double precision to evaluate the average.
         IF ( MOD( NELUSE, 2 ) .EQ. 0 ) THEN
            MEDIAN = NUM_DTOR( ( NUM_RTOD( ARRAY( TEST - 1 ) ) +
     :               NUM_RTOD( ARRAY( TEST -2 ) ) ) * 0.5D0 )
         ELSE
            MEDIAN = ARRAY( TEST - 1 )
         END IF
