      SUBROUTINE KPG1_MXMED( BAD, EL, ARRAY, ERROR, NSIGMA, THRESH,
...
*  Local Variables:
      DOUBLE PRECISION AMINUS             ! Data value less error
      DOUBLE PRECISION APLUS              ! Data value plus error
      DOUBLE PRECISION DELTA              ! Error-bar offsets
      INTEGER I                  ! Counter
...
*  types.
               DELTA = NUM_RTOD( ABS( NUM_DTOR( ERROR( I ) ) *
     :                 NSIGMA ) )
               APLUS = NUM_ADDD( ARRAY( I ), DELTA )
               AMINUS = NUM_SUBD( ARRAY( I ), DELTA )
 
*  Test that the value is between the thresholds.
               IF ( NUM_GTD( AMINUS, OTHRES( 1 ) ) .AND.
     :              NUM_LTD( APLUS, OTHRES( 2 ) ) ) THEN
 
*  Check current maximum against current pixel value.
                  IF ( NUM_GTD( APLUS, MAXMUM ) ) THEN
                     MAXMUM = APLUS
                     MAXPOS = I
                  END IF
 
*  Check current minimum against current pixel value.
                  IF ( NUM_LTD( AMINUS, MINMUM ) ) THEN
                     MINMUM = AMINUS
                     MINPOS = I
                  END IF
