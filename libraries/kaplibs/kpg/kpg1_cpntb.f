      SUBROUTINE KPG1_CPNTB( PNNDF, PNTIT, NDIM, DIMS, ARRAY, NULL,
     :                         STATUS )
*+
*  Name:
*     KPG1_CPNTx
...
*    Create a new NDF.
      ...
      CALL NDF_CREP( PNNDF, '_BYTE', NDIM, DIMS, NDFO, STATUS )
 
*    Map the data array.  Wrap to prevent line overflow when the token
*    is expanded.
 
      CALL KPG1_MAP( NDFO, 'Data', '_BYTE', 'WRITE', OPNTR, EL,
     :              STATUS )
...
*    conversion errors so they are not checked.
      ...
      CALL VEC_BTOB( .FALSE., EL, ARRAY, %VAL( CNF_PVAL( OPNTR( 1 ) ) ),
     :                   IERR, NERR, STATUS )
