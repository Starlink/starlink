      SUBROUTINE KPG1_GAUFD( DATA, LBND, UBND, NITER, TOLL, AMP, X0,
     :                        SIGMA, BACK, STATUS )
*+
*  Name:
*     KPG1_GAUFx
...
*    are too many) to use as an initial background estimate.
      ...
      CALL KPG1_NTHMD( .TRUE., DATA( LBND ), NINT( POINTS ),
     :                    NQUART, STAK, STATUS )
 
*    Watch for all bad pixels.
 
      IF ( STAK( 1 ) .EQ. VAL__MAXD ) THEN
         STATUS = SAI__ERROR
     ...
     :     STATUS )
      ...
      X( 4 ) = VAL_DTOR( .FALSE., STAK( 1 ), STATUS )
