      SUBROUTINE KPG1_MANIUB( NDIMI, DIMI, IN, NDIMO, DIMO, AXES, 
...
*  Invocation:
*     CALL KPG1_MANIUB( NDIMI, DIMI, IN, NDIMO, DIMO, AXES, COLOFF,
*                        EXPOFF, OUT, STATUS )
...
*  Local Variables:
      ...
      BYTE PIXI                ! Value of the current inupt pixel
      BYTE PIXO                ! Value of the new output pixel
...
*  collapsed dimensions to form the output pixel.
         ELSE
            ...
            DO IC = 1, NCOLL
               ...
               IF ( PIXI .NE. VAL__BADUB ) THEN
                  SUM = SUM + NUM_UBTOD( PIXI )
                  NVAL = NVAL + 1
               END IF
            END DO
            IF ( NVAL .GT. 0 ) THEN
               PIXO = NUM_DTOUB( SUM / DBLE( NVAL ) )
            ELSE
               PIXO = VAL__BADUB
            END IF
         END IF
