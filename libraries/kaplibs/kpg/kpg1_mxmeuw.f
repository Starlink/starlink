      SUBROUTINE KPG1_MXMEUW( BAD, EL, ARRAY, ERROR, NSIGMA, THRESH,
...
*  types.
               DELTA = NUM_RTOUW( ABS( NUM_UWTOR( ERROR( I ) ) *
     :                 NSIGMA ) )
               APLUS = NUM_ADDUW( ARRAY( I ), DELTA )
               AMINUS = NUM_SUBUW( ARRAY( I ), DELTA )
 
*  Test that the value is between the thresholds.
               IF ( NUM_GTUW( AMINUS, OTHRES( 1 ) ) .AND.
     :              NUM_LTUW( APLUS, OTHRES( 2 ) ) ) THEN
 
*  Check current maximum against current pixel value.
                  IF ( NUM_GTUW( APLUS, MAXMUM ) ) THEN
                     MAXMUM = APLUS
                     MAXPOS = I
                  END IF
 
*  Check current minimum against current pixel value.
                  IF ( NUM_LTUW( AMINUS, MINMUM ) ) THEN
                     MINMUM = AMINUS
                     MINPOS = I
                  END IF
