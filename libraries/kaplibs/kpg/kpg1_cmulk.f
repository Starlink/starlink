      SUBROUTINE KPG1_CMULK( BAD, EL, A, CONST, B, NBAD, STATUS )
*+
*  Name:
*     KPG1_CMULK

*  Purpose:
*     Multiplies each element of a vectorised 64-bit integer array by
*     a constant.
...
*  Description:
*     The routine multiplies each element of a vectorised 64-bit-
*     integer array by a constant to produce a new 64-bit-integer
...
*  Notes:
*     -  This routine is intended for processing 64-bit integer data
*     only. There is a related generic routine for processing other data
*     types, except double precision which has its own routine like this
*     one.
...
*                   Councils.
*     Copyright (C) 2012 Science & Technology Research Council.
*     All Rights Reserved.
...
*     2012 May 9 (MJC):
*        Made a variant of KPG1_CMULD for 64-bit integers.  This cannot
*        be combined into a generic kpg1_cmul.gdk because there would
*        be two kpg1_cmul.f files created at instantiation.  
