      SUBROUTINE KPG1_FLCOD( PNAME, NDIM, POSCOD, NPOINT, IPCO,
     :                         LBND, UBND, STATUS )
*+
*  Name:
*     KPG1_FLCOx
...
*  Arguments Returned:
      INTEGER NPOINT
      INTEGER IPCO
      DOUBLE PRECISION LBND( NDIM )
      DOUBLE PRECISION UBND( NDIM )
...
*  Obtain dynamic memory space for the positions.
      CALL PSX_CALLOC( NDIM * NPOINT, '_DOUBLE', IPCO, STATUS )
...
*  reading them all.
      COUNT = 1
      CALL KPG1_RFCOD( FD, NDIM, NPOINT, POSCOD, COUNT, 
     :                 %VAL( CNF_PVAL( IPCO ) ),
