      SUBROUTINE KPG1_MXMNW( BAD, EL, ARRAY, NINVAL, MAXMUM, MINMUM,
...
*  Use signed integers for the comparisons.
         IMAX = NUM_WTOI( MAXMUM )
         IMIN = NUM_WTOI( MINMUM )
...
*  Convert the integers back to the unsigned-integer type for export.
         MAXMUM = NUM_ITOW( IMAX )
         MINMUM = NUM_ITOW( IMIN )
      END IF
