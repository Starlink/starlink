      SUBROUTINE KPG1_LOGAR( BAD, EL, INARR, VAR, INVAR, BASE, OUTARR,
...
*  Input array pixel is good.  Compute the output value.
                  OUTARR( J ) = NUM_DTOR( FACTOR *
     :                          LOG( NUM_RTOD( INARR( J ) ) ) )
...
*  denominator, so check for zeroes.
                  ELSE IF ( INVAR( J ) .LE. 0.0E0 .OR.
     :                      INARR( J ) .EQ. 0.0E0 ) THEN
