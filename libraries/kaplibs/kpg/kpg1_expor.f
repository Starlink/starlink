      SUBROUTINE KPG1_EXPOR( BAD, EL, INARR, VAR, INVAR, BASE, OUTARR,
...
*  Local Variables:
      ...
      REAL MAXVAL             ! Maximum valid pixel value allowed
                                 ! for exponentiation
      DOUBLE PRECISION MAXV      ! Maximum value for the data type
      REAL MAXVAR             ! Maximum valid variance value allowed
                                 ! for exponentiation
      DOUBLE PRECISION MINV      ! Minimum value for the data type
      REAL MINVAL             ! Minimum valid pixel value allowed
                                 ! for exponentiation
...
*  Define a couple of useful variables.
      MINV = NUM_RTOD( VAL__MINR )
      MAXV = NUM_RTOD( VAL__MAXR )
...
*  of cases, and saves testing pixel by pixel.
         MINVAL = NUM_DTOR( MIN( MAXV, MAX( MINV,
     :            LOG( MINV ) / LOG( BASE ) ) ) )
...
*  and bases close to 1, but it should protect the vast majority
*  of cases, and saves testing pixel by pixel.
         MAXVAL = NUM_DTOR( MIN( MAXV, MAX( MINV,
     :            LOG( MAXV ) / LOG( BASE ) ) ) )
...
*  This is only approximate because it depends on the output value too.
         MAXVAR = NUM_DTOR( MIN( MAXV, MAX( MINV,
...
*  Propagate a bad pixel.
                  IF ( INVAR( J ) .EQ. VAL__BADR .OR.
     :                  OUTARR( J ) .EQ. VAL__BADR ) THEN
                     OUTVAR( J ) = VAL__BADR
