      SUBROUTINE KPG1_MANIC( NDIMI, DIMI, IN, NDIMO, DIMO, AXES, 
...
*  Invocation:
*     CALL KPG1_MANIC( NDIMI, DIMI, IN, NDIMO, DIMO, AXES, COLOFF,
*                        EXPOFF, OUT, STATUS )
...
*  Local Variables:
      ...
      CHARACTER*(*) PIXI                ! Value of the current inupt pixel
      CHARACTER*(*) PIXO                ! Value of the new output pixel
...
*  collapsed dimensions to form the output pixel.
         ELSE
            ...
            DO IC = 1, NCOLL
               ...
               IF ( PIXI .NE. VAL__BADC ) THEN
                  SUM = SUM + NUM_CTOD( PIXI )
                  NVAL = NVAL + 1
               END IF
            END DO
            IF ( NVAL .GT. 0 ) THEN
               PIXO = NUM_DTOC( SUM / DBLE( NVAL ) )
            ELSE
               PIXO = VAL__BADC
            END IF
         END IF
