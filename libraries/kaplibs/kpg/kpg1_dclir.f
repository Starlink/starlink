      SUBROUTINE KPG1_DCLIR( NDIM, NDF, DALBND, DAUBND, SCALE, OFFSET,
     :                       LINEAR, STATUS )
*+
*  Name:
*     KPG1_DCLIx
...
*  is linear.
            CALL KPG1_AXLIR( AEL, %VAL( AXPNTR( 1 ) ), DALBND( I ),
...
*  Determine whether the two co-ordinate systems are identical.
               IDENTY = IDENTY .AND. (
     :               ABS( SCALE( I ) - 1.0E0 ) .LT. VAL__EPSR .AND.
     :               ABS( OFFSET( I ) - 0.0E0 ) .LT. VAL__EPSR )
...
*  the bounds-finding routine.
         ELSE
            CALL KPG1_AXBNR( AEL, %VAL( AXPNTR( 1 ) ), DALBND( I ),
     :                         DAUBND( I ), STATUS )
            SCALE( I ) = 0.0E0
            OFFSET( I ) = DALBND( I )
            IDENTY = .FALSE.
