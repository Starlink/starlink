      SUBROUTINE KPG1_RMAPW( EL, INARR, VALMAX, VALMIN, NUMBIN, MAP,
     :                         OUTARR, STATUS )
*+
*  Name:
*     KPG1_RMAPx
...
*  unsigned integer data types.
      DSML = NUM_WTOD( VAL__SMLW )
      DLRG = NUM_WTOD( VAL__MAXW )
      DVMIN = NUM_WTOD( VALMIN )
      DVMAX = NUM_WTOD( VALMAX )
      DLOWER = 0.5D0 * DVMIN
      DUPPER = 0.5D0 * DVMAX
...
*  Therefore a messy section of code follows
         STATUS = SAI__ERROR
         IF ( 'W' .EQ. 'I' .OR. 'W' .EQ. 'B' .OR. 'W' .EQ. 'UB'
     :       .OR. 'W' .EQ. 'W' .OR. 'W' .EQ. 'UW' ) THEN
            CALL MSG_SETI( 'MIN', NUM_WTOI( VALMIN ) )
            CALL MSG_SETI( 'MAX', NUM_WTOI( VALMAX ) )
         ELSE IF ( 'W' .EQ. 'R' ) THEN
...
*  Make a double-precision copy of the data value.
         TEMP = NUM_WTOD( INARR( I ) )
 
*  Test for a valid element.
         IF ( INARR( I ) .EQ. VAL__BADW ) THEN
...
*  transformed bin.
            OUTARR( I ) = NUM_DTOW( ( ( DBLE( NEWBIN ) - 0.5 )
     :                    / RNUMB ) * SCALE + DVMIN )
