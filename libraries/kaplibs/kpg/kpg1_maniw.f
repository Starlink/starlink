      SUBROUTINE KPG1_MANIW( NDIMI, DIMI, IN, NDIMO, DIMO, AXES, 
...
*  Invocation:
*     CALL KPG1_MANIW( NDIMI, DIMI, IN, NDIMO, DIMO, AXES, COLOFF,
*                        EXPOFF, OUT, STATUS )
...
*  collapsed dimensions to form the output pixel.
         ELSE
            ...
            DO IC = 1, NCOLL
               ...
               IF ( PIXI .NE. VAL__BADW ) THEN
                  SUM = SUM + NUM_WTOD( PIXI )
                  NVAL = NVAL + 1
               END IF
            END DO
            IF ( NVAL .GT. 0 ) THEN
               PIXO = NUM_DTOW( SUM / DBLE( NVAL ) )
            ELSE
               PIXO = VAL__BADW
            END IF
         END IF
