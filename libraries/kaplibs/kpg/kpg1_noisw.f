      SUBROUTINE KPG1_NOISW( BAD, EL, VARNCE, ARRAY, STATUS )
*+
*  Name:
*     KPG1_NOISx
...
*  noise cannot be added to the data array, so set it bad.
            IF ( ARRAY( I ) .EQ. VAL__BADW .OR.
     :           VARNCE( I ) .EQ. VAL__BADW ) THEN
               ARRAY( I ) = VAL__BADW

            ELSE

*  Find the  of the Normal distribution at the element.
               SIGMA = SQRT( NUM_WTOR( VARNCE( I ) ) )

*  Generate the random noise.  There is no offset.
               ARRAY( I ) = ARRAY( I ) +
     :                      NUM_RTOW( PDA_RNNOR( 0.0, SIGMA ) )
            END IF
         END DO
