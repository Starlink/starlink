      SUBROUTINE KPG1_AXLII( EL, ARRAY, LVAL, UVAL, LINEAR, STATUS )
*+
*  Name:
*     KPG1_AXLIx
...
*  Arguments Given:
      ...
      INTEGER     ARRAY( EL )   ! Array to be tested.
 
*  Arguments Returned:
      LOGICAL    LINEAR        ! True if array is linear
      INTEGER     LVAL          ! Value of first array element.
      INTEGER     UVAL          ! Value of last array element.
...
*    the ends of the array.
      ...
      LOW = 1
      DO WHILE ( ARRAY( LOW ) .EQ. VAL__BADI .AND. LOW .LT. EL )
         LOW = LOW + 1
      END DO
      HIGH = EL
      DO WHILE ( ARRAY( HIGH ) .EQ. VAL__BADI .AND. HIGH .GT. LOW )
         HIGH = HIGH - 1
      END DO
...
*    Evaluate the linear increment.
      ...
      INCREM = ( NUM_ITOD( ARRAY( HIGH ) )
     :         - NUM_ITOD( ARRAY( LOW ) ) ) / NUM_ITOI( HIGH - LOW )
...
*    Find the biggest absolute value in the array.
      ...
      DO I = LOW, HIGH
         IF ( ARRAY( I ) .NE. VAL__BADI ) THEN
            MAXVAL = MAX( MAXVAL, ABS( NUM_ITOD( ARRAY( I ) ) ) )
         END IF
      END DO
...
*    allow for rounding errors and a full decade of values.
      ...
      IF ( VAL__EPSI .EQ. 1  ) THEN
         DIFLIM = 0.5D0
      ELSE
         DIFLIM = 11  * VAL__EPSI * MAXVAL
      END IF
...
*    linear array.
      ...
      IF ( ARRAY( 1 ) .EQ. VAL__BADI ) THEN
         LVAL = NUM_DTOI( NUM_ITOD( ARRAY( LOW ) ) -
     :          INCREM * DBLE( LOW - 1 ) )
      END IF
 
      IF ( ARRAY( EL ) .EQ. VAL__BADI ) THEN
         UVAL = NUM_DTOI( NUM_ITOD( ARRAY( HIGH ) ) +
     :          INCREM * DBLE( EL - HIGH ) )
      END IF
