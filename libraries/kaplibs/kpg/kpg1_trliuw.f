      SUBROUTINE KPG1_TRLIUW( NDIMI, IDIMS, INARR, VAR, INVAR, TRID,
...
*  Local Variables:
      ...
      INTEGER NCOUT            ! Number of values in the forward
                               ! transformation
      INTEGER*2 NEIGH( 2 ** NDF__MXDIM ) ! Vector of input data values
      ...
      INTEGER STRIDO( NDF__MXDIM ) ! Dimension strides for output arrays
                               ! and co-ordinates
      INTEGER*2 VNEIGH( 2 ** NDF__MXDIM ) ! Vector of input variance
...
*   Test for a bad input data value.
         IF ( INARR( VO ) .EQ. VAL__BADUW ) THEN
            OUTARR( K ) = VAL__BADUW
            VALDET = .TRUE.
         END IF
...
*   Test for a bad input variance value.
         IF ( VAR ) THEN
            IF ( INVAR( VO ) .EQ. VAL__BADUW ) THEN
               OUTVAR( K ) = VAL__BADUW
               VARDET = .TRUE.
            END IF
         ELSE
...
*   Loop around the neighbourhood elements either side of the point.
            DO J = 1, 2 ** NDIMI
               IF ( VNEIGH( J ) .NE. VAL__BADUW ) THEN
...
*   Integrate the weights and the weighted value.
                  WSUM = WSUM + W
                  ASUM = ASUM + NUM_UWTOD( VNEIGH( J ) ) * W
               END IF
            END DO
