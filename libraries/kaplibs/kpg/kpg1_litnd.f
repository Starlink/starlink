      SUBROUTINE KPG1_LITND( NDIM, ILBND, IUBND, OLBND, OUBND,
     :                         ITOO, OTOI, STATUS )
*+
*  Name:
*     KPG1_LITNx
...
*        least of 26 + 2 * VAL__SZD characters.
...
*  Arguments Given:
      INTEGER NDIM
      DOUBLE PRECISION ILBND( NDIM )
      DOUBLE PRECISION IUBND( NDIM )
      DOUBLE PRECISION OLBND( NDIM )
      DOUBLE PRECISION OUBND( NDIM )
...
*  Local Variables:
      ...
      DOUBLE PRECISION OFFSET              ! Offset of a linear transformation
      DOUBLE PRECISION SCALE               ! Scale of a linear transformation
...
*  Substitute the actual scales for the tokens.
         CALL TRN_STOKD( 'scale', SCALE, OTOI( I ), NSUBS, STATUS )
         CALL TRN_STOKD( 'scale', SCALE, ITOO( I ), NSUBS, STATUS )
 
*  Substitute the actual offsets for the tokens.
         CALL TRN_STOKD( 'offset', OFFSET, OTOI( I ), NSUBS, STATUS )
         CALL TRN_STOKD( 'offset', OFFSET, ITOO( I ), NSUBS, STATUS )
      END DO
