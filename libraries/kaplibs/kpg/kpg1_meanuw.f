      SUBROUTINE KPG1_MEANUW( N, DATA, MEAN, STATUS )
*+
*  Name:
*     KPG1_MEANx
...
*        The data array.
*     MEAN = ? (Returned)
*        The mean value.  Set to VAL__BADUW if no good data are found.
...
*  keeping a count of the number of good values found.
      DO I = 1, N
         ...
         IF ( DATA( I ) .NE. VAL__BADUW ) THEN
            SUM = SUM + NUM_UWTOD( DATA( I ) )
            NSUM = NSUM + 1
         END IF
