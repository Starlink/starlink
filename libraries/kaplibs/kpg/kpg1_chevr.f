      SUBROUTINE KPG1_CHEVR( XMIN, XMAX, NCOEF, CHCOEF, NPTS, X,
     :                         EVAL, STATUS )
*+
*  Name:
*     KPG1_CHEVx
...
*  Local Variables:
      ...
      REAL XN                 ! Normalised argument
      REAL XN2                ! Twice the normalised argument
      REAL XR                 ! Argument range
...
*  Validate the data limits.
      IF ( XMAX .LE. XMIN ) THEN
         STATUS = SAI__ERROR
         CALL MSG_SETR( 'MX', XMAX )
         CALL MSG_SETR( 'MN', XMIN )
...
*  coefficient in the Chebyshev series.
               EVAL( I ) = DP1 * XN - DP2 + CHCOEF( 1 ) / 2.0E0
            END IF
         END DO
      END IF
