...
*        An optional AST KeyMap containing data for extra columns to add
*        to the catalogue. Each entry in the KeyMap should be a vector
*        with length equal to NPOS. The key for the entry is used as the 
*        column name in the table. No extra columns are added if a value 
*        of AST__NULL is supplied for KEYMAP. This keyMap can be used
