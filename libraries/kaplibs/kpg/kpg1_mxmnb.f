      SUBROUTINE KPG1_MXMNB( BAD, EL, ARRAY, NINVAL, MAXMUM, MINMUM,
...
*  Watch for a bad first value.
         IF ( ARRAY( 1 ) .EQ. VAL__BADB ) THEN
 
*  Search for the first non-bad value.
            I = 2
            DO WHILE ( I .LE. EL  .AND.
     :                 ARRAY( MIN( I, EL ) ) .EQ. VAL__BADB )
               I = I + 1
            END DO
            NINVAL = I - 1
...
*  Use signed integers for the comparisons.
         IMAX = NUM_BTOI( MAXMUM )
         IMIN = NUM_BTOI( MINMUM )
...
*  Convert the integers back to the unsigned-integer type for export.
         MAXMUM = NUM_ITOB( IMAX )
         MINMUM = NUM_ITOB( IMIN )
      END IF
