      SUBROUTINE KPG1_EXPOD( BAD, EL, INARR, VAR, INVAR, BASE, OUTARR,
...
*  Local Variables:
      ...
      DOUBLE PRECISION MAXVAL             ! Maximum valid pixel value allowed
                                 ! for exponentiation
      DOUBLE PRECISION MAXV      ! Maximum value for the data type
      DOUBLE PRECISION MAXVAR             ! Maximum valid variance value allowed
                                 ! for exponentiation
      DOUBLE PRECISION MINV      ! Minimum value for the data type
      DOUBLE PRECISION MINVAL             ! Minimum valid pixel value allowed
                                 ! for exponentiation
...
*  Define a couple of useful variables.
      MINV = NUM_DTOD( VAL__MIND )
      MAXV = NUM_DTOD( VAL__MAXD )
...
*  of cases, and saves testing pixel by pixel.
         MINVAL = NUM_DTOD( MIN( MAXV, MAX( MINV,
     :            LOG( MINV ) / LOG( BASE ) ) ) )
...
*  and bases close to 1, but it should protect the vast majority
*  of cases, and saves testing pixel by pixel.
         MAXVAL = NUM_DTOD( MIN( MAXV, MAX( MINV,
     :            LOG( MAXV ) / LOG( BASE ) ) ) )
...
*  This is only approximate because it depends on the output value too.
         MAXVAR = NUM_DTOD( MIN( MAXV, MAX( MINV,
