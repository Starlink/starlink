      SUBROUTINE KPG1_MANIL( NDIMI, DIMI, IN, NDIMO, DIMO, AXES, 
...
*  Invocation:
*     CALL KPG1_MANIL( NDIMI, DIMI, IN, NDIMO, DIMO, AXES, COLOFF,
*                        EXPOFF, OUT, STATUS )
...
*  Local Variables:
      ...
      LOGICAL PIXI                ! Value of the current inupt pixel
      LOGICAL PIXO                ! Value of the new output pixel
...
*  collapsed dimensions to form the output pixel.
         ELSE
            ...
            DO IC = 1, NCOLL
               ...
               IF ( PIXI .NE. VAL__BADL ) THEN
                  SUM = SUM + NUM_LTOD( PIXI )
                  NVAL = NVAL + 1
               END IF
            END DO
            IF ( NVAL .GT. 0 ) THEN
               PIXO = NUM_DTOL( SUM / DBLE( NVAL ) )
            ELSE
               PIXO = VAL__BADL
            END IF
         END IF
