      SUBROUTINE KPG1_MXMEB( BAD, EL, ARRAY, ERROR, NSIGMA, THRESH,
...
*  Local Variables:
      BYTE AMINUS             ! Data value less error
      BYTE APLUS              ! Data value plus error
      BYTE DELTA              ! Error-bar offsets
      INTEGER I                  ! Counter
      BYTE OTHRES( 2 )        ! Thresholds where first element is the
...
*  Add some defensive code in case the thresholds are reversed.
      OTHRES( 1 ) = NUM_MINB( THRESH( 1 ), THRESH( 2 ) )
      OTHRES( 2 ) = NUM_MAXB( THRESH( 1 ), THRESH( 2 ) )
...
*  Test for valid pixel.
            IF ( ARRAY( I ) .NE. VAL__BADB .AND.
     :           ERROR( I ) .NE. VAL__BADB ) THEN
...
*  types.
               DELTA = NUM_RTOB( ABS( NUM_BTOR( ERROR( I ) ) *
     :                 NSIGMA ) )
               APLUS = NUM_ADDB( ARRAY( I ), DELTA )
               AMINUS = NUM_SUBB( ARRAY( I ), DELTA )
 
*  Test that the value is between the thresholds.
               IF ( NUM_GTB( AMINUS, OTHRES( 1 ) ) .AND.
     :              NUM_LTB( APLUS, OTHRES( 2 ) ) ) THEN
 
*  Check current maximum against current pixel value.
                  IF ( NUM_GTB( APLUS, MAXMUM ) ) THEN
                     MAXMUM = APLUS
                     MAXPOS = I
                  END IF
 
*  Check current minimum against current pixel value.
                  IF ( NUM_LTB( AMINUS, MINMUM ) ) THEN
                     MINMUM = AMINUS
                     MINPOS = I
                  END IF
