      SUBROUTINE KPG1_MANII( NDIMI, DIMI, IN, NDIMO, DIMO, AXES, 
...
*  Invocation:
*     CALL KPG1_MANII( NDIMI, DIMI, IN, NDIMO, DIMO, AXES, COLOFF,
*                        EXPOFF, OUT, STATUS )
...
*  Local Variables:
      ...
      INTEGER PIXI                ! Value of the current inupt pixel
      INTEGER PIXO                ! Value of the new output pixel
...
*  collapsed dimensions to form the output pixel.
         ELSE
            ...
            DO IC = 1, NCOLL
               ...
               IF ( PIXI .NE. VAL__BADI ) THEN
                  SUM = SUM + NUM_ITOD( PIXI )
                  NVAL = NVAL + 1
               END IF
            END DO
            IF ( NVAL .GT. 0 ) THEN
               PIXO = NUM_DTOI( SUM / DBLE( NVAL ) )
            ELSE
               PIXO = VAL__BADI
            END IF
         END IF
