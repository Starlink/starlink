      SUBROUTINE KPG1_RMAPI( EL, INARR, VALMAX, VALMIN, NUMBIN, MAP,
     :                         OUTARR, STATUS )
*+
*  Name:
*     KPG1_RMAPx
...
*  Therefore a messy section of code follows
         STATUS = SAI__ERROR
         IF ( 'I' .EQ. 'I' .OR. 'I' .EQ. 'B' .OR. 'I' .EQ. 'UB'
     :       .OR. 'I' .EQ. 'W' .OR. 'I' .EQ. 'UW' ) THEN
            CALL MSG_SETI( 'MIN', NUM_ITOI( VALMIN ) )
            CALL MSG_SETI( 'MAX', NUM_ITOI( VALMAX ) )
         ELSE IF ( 'I' .EQ. 'R' ) THEN
...
*  Make a double-precision copy of the data value.
         TEMP = NUM_ITOD( INARR( I ) )
 
*  Test for a valid element.
         IF ( INARR( I ) .EQ. VAL__BADI ) THEN
...
*  transformed bin.
            OUTARR( I ) = NUM_DTOI( ( ( DBLE( NEWBIN ) - 0.5 )
     :                    / RNUMB ) * SCALE + DVMIN )
