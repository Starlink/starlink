      SUBROUTINE KPG1_AXLIUW( EL, ARRAY, LVAL, UVAL, LINEAR, STATUS )
*+
*  Name:
*     KPG1_AXLIx
...
*    the ends of the array.
      ...
      LOW = 1
      DO WHILE ( ARRAY( LOW ) .EQ. VAL__BADUW .AND. LOW .LT. EL )
         LOW = LOW + 1
      END DO
      HIGH = EL
      DO WHILE ( ARRAY( HIGH ) .EQ. VAL__BADUW .AND. HIGH .GT. LOW )
         HIGH = HIGH - 1
      END DO
...
*    Evaluate the linear increment.
      ...
      INCREM = ( NUM_UWTOD( ARRAY( HIGH ) )
     :         - NUM_UWTOD( ARRAY( LOW ) ) ) / NUM_ITOUW( HIGH - LOW )
...
*    Find the biggest absolute value in the array.
      ...
      DO I = LOW, HIGH
         IF ( ARRAY( I ) .NE. VAL__BADUW ) THEN
            MAXVAL = MAX( MAXVAL, ABS( NUM_UWTOD( ARRAY( I ) ) ) )
         END IF
      END DO
...
*    allow for rounding errors and a full decade of values.
      ...
      IF ( VAL__EPSUW .EQ. 1  ) THEN
         DIFLIM = 0.5D0
      ELSE
         DIFLIM = 11  * VAL__EPSUW * MAXVAL
      END IF
...
*    linear array.
      ...
      IF ( ARRAY( 1 ) .EQ. VAL__BADUW ) THEN
         LVAL = NUM_DTOUW( NUM_UWTOD( ARRAY( LOW ) ) -
     :          INCREM * DBLE( LOW - 1 ) )
      END IF
 
      IF ( ARRAY( EL ) .EQ. VAL__BADUW ) THEN
         UVAL = NUM_DTOUW( NUM_UWTOD( ARRAY( HIGH ) ) +
     :          INCREM * DBLE( EL - HIGH ) )
      END IF
