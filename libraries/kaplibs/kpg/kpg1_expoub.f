      SUBROUTINE KPG1_EXPOUB( BAD, EL, INARR, VAR, INVAR, BASE, OUTARR,
...
*  of cases, and saves testing pixel by pixel.
         MINVAL = NUM_DTOUB( MIN( MAXV, MAX( MINV,
     :            LOG( MINV ) / LOG( BASE ) ) ) )
...
*  and bases close to 1, but it should protect the vast majority
*  of cases, and saves testing pixel by pixel.
         MAXVAL = NUM_DTOUB( MIN( MAXV, MAX( MINV,
     :            LOG( MAXV ) / LOG( BASE ) ) ) )
...
*  This is only approximate because it depends on the output value too.
         MAXVAR = NUM_DTOUB( MIN( MAXV, MAX( MINV,
...
*  Propagate a bad pixel.
                  IF ( INVAR( J ) .EQ. VAL__BADUB .OR.
     :                  OUTARR( J ) .EQ. VAL__BADUB ) THEN
                     OUTVAR( J ) = VAL__BADUB
