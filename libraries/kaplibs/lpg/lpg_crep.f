      SUBROUTINE LPG_CREP( PARAM, FTYPE, NDIM, UBND, INDF, STATUS )
*+
*  Name:
*     LPG_CREP
...
*  If looping is currently disabled, just call NDF_CREAP and exit.
      IF( DISAB ) THEN
...
*  Report an error if all slots are in use.
         ...
         IF( NPAR .GT. LPG__MXPAR .AND. STATUS .EQ. SAI__OK ) THEN
            ...
            CALL ERR_REP( 'LPG_CREP_ERR1', 'Too many data file '//
...
*  flag character.
     ...
     :                      FLAG, STATUS )
            IF( FLAG ) THEN
               ...
               CALL MSG_OUT( 'LPG_CREP_MSG1', 'Please supply more '//
...
*  result in the user being prompted for a new parameter value.
         ...
         IF( STATE .EQ. SUBPAR__CANCEL ) THEN
            CALL NDG_CREP1( PARAM, FTYPE, NDIM, UBND, INDF, NAME, 
     :                      STATUS )
...
*  use it on all invocations of the application.
         IF( SIZE( IPAR ) .EQ. 1 ) THEN
            CALL NDG_NDFCP( IGRP( IPAR ), 1, FTYPE, NDIM, UBND, INDF, 
     :                      STATUS )
         ELSE
            CALL NDG_NDFCP( IGRP( IPAR ), NRUN, FTYPE, NDIM, UBND, INDF, 
     :                      STATUS )
         END IF
...
*  already been reported.
         IF( VERB .AND. .NOT. REP( IPAR ) ) THEN
            CALL NDF_MSG( 'N', INDF )
            CALL MSG_SETC( 'P', UPAR )
            CALL MSG_OUT( 'LPG_CREP_MSG2', '%^P = ^N', STATUS )
            REP( IPAR ) = .TRUE.
         END IF
