...
\item KPG1\_BBOX$<$T$>$ added. It obtains the pixel bounding box of a given value
...
\item kaplibs\_link script now works without attempting to load ADAM
     symbols
\item Can now be built as a shared library
\item Added kpg1\_astcmn to provide access to KPG\_AST private common block
\item The HLP helper routines are no longer part of kaplibs. Use SHL
     library instead.
\item kpg1\_scrsz no longer present. Please use one\_scrsz instead.
...
\item Added KPG1\_MXMNX, which provides a single interface to the
      existing generic KPG1\_MXMN$<$T$>$ routines.
\item Added KPG1\_SCALX, which provides a single interface to the
      new doubly generic KPG1\_SCL$<$T$>$$<$T$>$ routines, for scaling
