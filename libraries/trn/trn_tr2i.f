      SUBROUTINE TRN_TR2I( BAD, NXY, XIN, YIN, IDT, XOUT, YOUT,
     :                       STATUS )
...
*  Arguments Given:
      ...
      INTEGER YIN( * )           ! Input Y coordinate values

      INTEGER IDT               ! ID for the compiled transformation to
                                ! be applied

*  Arguments Returned:
      INTEGER XOUT( * )          ! Array for transformed X coordinate
                                ! values

      INTEGER YOUT( * )          ! Array for transformed Y coordinate
                                ! values
...
*  Local Variables:
      ...
      INTEGER WRK1( NWRK, 2 )    ! Workspace for input coordinate values

      INTEGER WRK2( NWRK, 2 )    ! Workspace for output coordinate values
...
*   workspace.
        CALL VEC_ITOI( BAD, IBATCH, XIN( ISTART ), WRK1( 1, 1 ),
     :                     IERRL, NERRL, NSTATL )
        CALL VEC_ITOI( BAD, IBATCH, YIN( ISTART ), WRK1( 1, 2 ),
     :                     IERRL, NERRL, NSTATL )


*   Transform the data points.
        CALL TRN_TRNI( BAD, NWRK, 2, IBATCH, WRK1, IDT, NWRK, 2, WRK2,
     :                   STATUS )


*   Copy the transformed coordinate values into the output arrays.
        CALL VEC_ITOI( BAD, IBATCH, WRK2( 1, 1 ), XOUT( ISTART ),
     :                     IERRL, NERRL, NSTATL )
        CALL VEC_ITOI( BAD, IBATCH, WRK2( 1, 2 ), YOUT( ISTART ),
     :                     IERRL, NERRL, NSTATL )
