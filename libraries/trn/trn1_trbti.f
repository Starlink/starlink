      SUBROUTINE TRN1_TRBTI( BAD, ND1, NVIN, DATA, ISTART, NPROC,
...
*  Purpose:
*     apply a transformation to a batch of INTEGER data.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL TRN1_TRBTI( BAD, ND1, NVIN, DATA, ISTART, NPROC,
*                        NMOD, PINDEX, NVAR, IPRC, MXWRK, NR1,
*                        NVOUT, RESULT, WRKI, WRKR, WRKD, STATUS )
...
*     coordinate list for a subset (batch) of INTEGER data points.
...
*  Arguments Given:
      ...
      INTEGER DATA( ND1, NVIN )  ! Array containing coordinate values for
                                ! each input data point
...
*   transformation.  Report an error if necessary.
     ...
     :    ( NVOUT .NE. NVAR( NMOD + 1 ) ) ) THEN
        ...
        CALL TRN1_ERROR( 'TRN1_TRBTI', ' ', STATUS )
...
*   ...convert to integer:
        IF( PRICOD .EQ. TRN_PR_I ) THEN
          DO IVAR = 1, NVIN
            CALL VEC_ITOI( BADL, NPROC, DATA( ISTART, IVAR ),
...
*   ...convert to real:
        ELSE IF( PRICOD .EQ. TRN_PR_R ) THEN
          DO IVAR = 1, NVIN
            CALL VEC_ITOR( BADL, NPROC, DATA( ISTART, IVAR ),
...
*   ...convert to double precision:
        ELSE IF( PRICOD .EQ. TRN_PR_D ) THEN
          DO IVAR = 1, NVIN
            CALL VEC_ITOD( BADL, NPROC, DATA( ISTART, IVAR ),
...
*   ...convert to integer:
          IF( PRLAST .EQ. TRN_PR_I ) THEN
            DO IVAR = 1, NVOUT
              NSTATL = SAI__OK
              CALL VEC_ITOI( BADL, NPROC, WRKI( 1, IVAR ),
     :                         RESULT( ISTART, IVAR ),
     :                         IERRL, NERRL, NSTATL )
            ENDDO
...
*   ...convert to real:
          ELSE IF( PRLAST .EQ. TRN_PR_R ) THEN
            DO IVAR = 1, NVOUT
              NSTATL = SAI__OK
              CALL VEC_RTOI( BADL, NPROC, WRKR( 1, IVAR ),
     :                         RESULT( ISTART, IVAR ),
     :                         IERRL, NERRL, NSTATL )
            ENDDO
...
*   ...covert to double precision:
          ELSE IF( PRLAST .EQ. TRN_PR_D ) THEN
            DO IVAR = 1, NVOUT
              NSTATL = SAI__OK
              CALL VEC_DTOI( BADL, NPROC, WRKD( 1, IVAR ),
     :                         RESULT( ISTART, IVAR ),
     :                         IERRL, NERRL, NSTATL )
            ENDDO
          ENDIF
