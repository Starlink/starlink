      SUBROUTINE TRN_TRNI( BAD, ND1, NCIN, NDAT, DATA, IDT, NR1,
     :                       NCOUT, RESULT, STATUS )
*+
*  Name:
*     TRN_TRNI
...
*  Purpose:
*     apply general transformation to INTEGER data.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL TRN_TRNI( BAD, ND1, NCIN, NDAT, DATA, IDT, NR1, NCOUT,
*                      RESULT, STATUS )
...
*       process.
*     - Call TRN1_TRDTI to transform the data using the compiled
...
*  Arguments Given:
      ...
      INTEGER DATA( ND1, NCIN )  ! Array containing coordinates for each
                                ! input data point
...
*   Apply the transformation to the data points.
      CALL TRN1_TRDTI( BAD, ND1, NCIN, DATA, NDAT,
