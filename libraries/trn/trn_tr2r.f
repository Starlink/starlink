      SUBROUTINE TRN_TR2R( BAD, NXY, XIN, YIN, IDT, XOUT, YOUT,
     :                       STATUS )
...
*  Arguments Given:
      ...
      REAL YIN( * )           ! Input Y coordinate values

      INTEGER IDT               ! ID for the compiled transformation to
                                ! be applied

*  Arguments Returned:
      REAL XOUT( * )          ! Array for transformed X coordinate
                                ! values

      REAL YOUT( * )          ! Array for transformed Y coordinate
                                ! values
...
*  Local Variables:
      ...
      REAL WRK1( NWRK, 2 )    ! Workspace for input coordinate values

      REAL WRK2( NWRK, 2 )    ! Workspace for output coordinate values
...
*   workspace.
        CALL VEC_RTOR( BAD, IBATCH, XIN( ISTART ), WRK1( 1, 1 ),
     :                     IERRL, NERRL, NSTATL )
        CALL VEC_RTOR( BAD, IBATCH, YIN( ISTART ), WRK1( 1, 2 ),
     :                     IERRL, NERRL, NSTATL )


*   Transform the data points.
        CALL TRN_TRNR( BAD, NWRK, 2, IBATCH, WRK1, IDT, NWRK, 2, WRK2,
     :                   STATUS )


*   Copy the transformed coordinate values into the output arrays.
        CALL VEC_RTOR( BAD, IBATCH, WRK2( 1, 1 ), XOUT( ISTART ),
     :                     IERRL, NERRL, NSTATL )
        CALL VEC_RTOR( BAD, IBATCH, WRK2( 1, 2 ), YOUT( ISTART ),
     :                     IERRL, NERRL, NSTATL )
