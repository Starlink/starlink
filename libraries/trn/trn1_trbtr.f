      SUBROUTINE TRN1_TRBTR( BAD, ND1, NVIN, DATA, ISTART, NPROC,
...
*  Purpose:
*     apply a transformation to a batch of REAL data.

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL TRN1_TRBTR( BAD, ND1, NVIN, DATA, ISTART, NPROC,
*                        NMOD, PINDEX, NVAR, IPRC, MXWRK, NR1,
*                        NVOUT, RESULT, WRKI, WRKR, WRKD, STATUS )
...
*     coordinate list for a subset (batch) of REAL data points.
...
*  Arguments Given:
      ...
      REAL DATA( ND1, NVIN )  ! Array containing coordinate values for
                                ! each input data point
...
*   transformation.  Report an error if necessary.
     ...
     :    ( NVOUT .NE. NVAR( NMOD + 1 ) ) ) THEN
        ...
        CALL TRN1_ERROR( 'TRN1_TRBTR', ' ', STATUS )
...
*   ...convert to integer:
        IF( PRICOD .EQ. TRN_PR_I ) THEN
          DO IVAR = 1, NVIN
            CALL VEC_RTOI( BADL, NPROC, DATA( ISTART, IVAR ),
...
*   ...convert to real:
        ELSE IF( PRICOD .EQ. TRN_PR_R ) THEN
          DO IVAR = 1, NVIN
            CALL VEC_RTOR( BADL, NPROC, DATA( ISTART, IVAR ),
...
*   ...convert to double precision:
        ELSE IF( PRICOD .EQ. TRN_PR_D ) THEN
          DO IVAR = 1, NVIN
            CALL VEC_RTOD( BADL, NPROC, DATA( ISTART, IVAR ),
...
*   ...convert to integer:
          IF( PRLAST .EQ. TRN_PR_I ) THEN
            DO IVAR = 1, NVOUT
              NSTATL = SAI__OK
              CALL VEC_ITOR( BADL, NPROC, WRKI( 1, IVAR ),
     :                         RESULT( ISTART, IVAR ),
     :                         IERRL, NERRL, NSTATL )
            ENDDO
...
*   ...convert to real:
          ELSE IF( PRLAST .EQ. TRN_PR_R ) THEN
            DO IVAR = 1, NVOUT
              NSTATL = SAI__OK
              CALL VEC_RTOR( BADL, NPROC, WRKR( 1, IVAR ),
     :                         RESULT( ISTART, IVAR ),
     :                         IERRL, NERRL, NSTATL )
            ENDDO
...
*   ...covert to double precision:
          ELSE IF( PRLAST .EQ. TRN_PR_D ) THEN
            DO IVAR = 1, NVOUT
              NSTATL = SAI__OK
              CALL VEC_DTOR( BADL, NPROC, WRKD( 1, IVAR ),
     :                         RESULT( ISTART, IVAR ),
     :                         IERRL, NERRL, NSTATL )
            ENDDO
          ENDIF
