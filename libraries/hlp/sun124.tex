...
\stardocinitials    {SUN}
\stardocsource      {sun124.10}
\stardocnumber      {124.10}
\stardocauthors     {P.\,T.\,Wallace}
\stardocdate        {13th June 1995}
\stardoctitle       {HLP --- Interactive Help System}
\stardocversion     {v3.3}
...
be abbreviated \fstring{A\_G\_N} (or \fstring{A} \textit{etc}).
...
application's input routine (by means of the \texttt{END=} feature
of the \texttt{READ} statement), followed by simulation of multiple
...
The files
\texttt{helpic} and \texttt{comic}
(on the Unix platforms -- \texttt{HELPIC.FOR} and \texttt{COMIC.FOR} on the VAX,
\texttt{HELPIC} and \texttt{COMIC} on the PC) contain definitions and
...
The \texttt{comic} INCLUDE is used only by the \fstring{HLP\_COMSTR}
...
\end{terminalv}
HLP\_ERRMES translates a HLP system error code \texttt{J}
into a message string \texttt{MES}.  See Section~4.6.

\begin{terminalv}
SUBROUTINE HLP_CREH(NAMETR,LUIN,SOURCE,LUOUT,LIB,LUERR,EOS,JSTAT)
\end{terminalv}
HLP\_CREH translates a file of help source \texttt{SOURCE},
on I/O unit \texttt{LUIN}, into a help library \texttt{LIB}, subject
to name translation by the routine \texttt{NAMETR}, on I/O
unit \texttt{LUOUT}.  Any error messages go to I/O unit \texttt{LUERR}.
\texttt{EOS} is the end-of-string character, conventionally \texttt{CHAR(0)}.
\texttt{JSTAT} is the status.  See Section~3.2.
