      SUBROUTINE NDF1_ADRST( IAX, IACB, STATUS )
*+
*  Name:
*     NDF1_ADRST
...
*  Description:
*     The routine resets an axis data array to an undefined state by
*     deleting it if it exists.

*  Arguments:
*     IAX = INTEGER (Given)
*        Number of the NDF axis whose data array is to be reset.
*     IACB = INTEGER (Given)
...
*  Notes:
*     -  An axis data array cannot be reset with this routine while
*     mapped access to any part of it is in effect.
*     -  This routine may only be used to reset an axis data array
*     for a base NDF. It returns without action if the NDF supplied is
*     a section.
*     -  This routine should only be used internally as part of the
*     process of resetting the entire axis structure, or of making
*     other scheduled changes to the axis structure. Resetting of an
*     individal axis data array under user control is not a valid
*     operation.
...
*     -  If the axis data array exists, then obtain its storage form
...
*  History:
*     19-OCT-1990 (RFWS):
*        Original version, derived from the NDF1_AVRST routine.
*     23-OCT-1990 (RFWS):
*        Added status check before ARY_DELET and noted whether the DCB
*        array information is correct afterwards.
*     30-OCT-1990 (RFWS):
*        Removed the status check again after re-consideration.
...
*  Call error tracing routine and exit.
      IF ( STATUS .NE. SAI__OK ) CALL NDF1_TRACE( 'NDF1_ADRST', STATUS )
