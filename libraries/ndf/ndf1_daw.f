      SUBROUTINE NDF1_DAW( IAX, IDCB, STATUS )
*+
*  Name:
*     NDF1_DAW

*  Purpose:
*     Ensure that information about an NDF's axis width array is
...
*  Description:
*     The routine ensures that information about an NDF's axis width
...
*     system identifier for the axis width array is stored in the DCB.
...
*     the axis width array identifier remains null.
*     -  See if the axis width array component exists. If not, then its
...
*     axis width array and the pixel index bounds of the main NDF data
*     array.
*     -  Check that the axis width array does not hold complex values.
*     Report an error if it does.
*     -  Check that the dimensionality of the axis width array is 1.
...
*     -  Set the default axis width type to be _REAL and the default
*     axis width storage form to match the NDF default storage form
...
*  History:
*     15-OCT-1990 (RFWS):
*        Original version, derived from the NDF1_DAV routine.
*     {enter_changes_here}
...
*  Local Variables:
      ...
      INTEGER LBNDW( NDF__MXDIM ) ! Axis width array lower bounds
      INTEGER NDIM               ! Number of NDF dimensions
      INTEGER NDIMW              ! Number of width array dimensions
      INTEGER UBND( NDF__MXDIM ) ! NDF upper bounds
      INTEGER UBNDW( NDF__MXDIM ) ! Axis width array upper bounds
      LOGICAL CMPLXW             ! Whether width array is complex
      LOGICAL THERE              ! Whether width array exists
...
*  identifier remains null.
               CALL DAT_THERE( DCB_ALOC( IAX, IDCB ), 'WIDTH', THERE,
     :                         STATUS )
               IF ( STATUS .EQ. SAI__OK ) THEN
                  IF ( THERE ) THEN
...
*  Import the axis width array into the ARY_ system, storing the
*  identifier in the DCB.
                     CALL ARY_FIND( DCB_ALOC( IAX, IDCB ), 'WIDTH',
...
*  width array and the pixel index bounds of the main NDF data array.
                     CALL ARY_CMPLX( DCB_AWID( IAX, IDCB ), CMPLXW,
     :                               STATUS )
                     CALL ARY_BOUND( DCB_AWID( IAX, IDCB ), NDF__MXDIM,
     :                               LBNDW, UBNDW, NDIMW, STATUS )
...
*  Report an error if it does.
                        IF ( CMPLXW ) THEN
                           ...
                           CALL DAT_MSG( 'AXIS',
     :                                   DCB_ALOC( IAX, IDCB ) )
                           CALL ERR_REP( 'NDF1_DAW_CMPLX',
     :                     'The WIDTH array in the NDF axis ' //
     :                     'structure ^AXIS holds illegal complex ' //
     :                     'values.', STATUS )

*  Check that the dimensionality of the axis width array is 1.  Report
*  an error if it is not.
                        ELSE IF ( NDIMW .NE. 1 ) THEN
                           STATUS = NDF__NDMIN
                           CALL DAT_MSG( 'AXIS',
     :                                   DCB_ALOC( IAX, IDCB ) )
                           CALL MSG_SETI( 'BADNDIM', NDIMW )
                           CALL ERR_REP( 'NDF1_DAW_NDIMW',
     :                     'The WIDTH array in the NDF axis ' //
     :                     'structure ^AXIS is ' //
...
*  width array match the bounds of the corresponding NDF dimension.
*  Report an error if they do not.
                        ELSE IF ( ( LBNDW( 1 ) .NE. LBND( IAX ) ) .OR.
     :                            ( UBNDW( 1 ) .NE. UBND( IAX ) ) ) THEN
                           STATUS = NDF__BNDIN
                           CALL DAT_MSG( 'AXIS',
     :                                   DCB_ALOC( IAX, IDCB ) )
                           CALL MSG_SETI( 'LBNDW', LBNDW( 1 ) )
                           CALL MSG_SETI( 'UBNDW', UBNDW( 1 ) )
                           CALL MSG_SETI( 'LBND', LBND( IAX ) )
                           CALL MSG_SETI( 'UBND', UBND( IAX ) )
                           CALL ERR_REP( 'NDF1_DAW_BNDD',
     :                     'The pixel-index bounds (^LBNDW:^UBNDW) ' //
     :                     'of the WIDTH array in the NDF axis ' //
...
*  may have been allocated.
                     IF ( STATUS .NE. SAI__OK ) THEN
                        CALL ARY_ANNUL( DCB_AWID( IAX, IDCB ), STATUS )
                     END IF
                  END IF
               END IF
            END IF
...
*  from the data array.
         IF ( STATUS .EQ. SAI__OK ) THEN
            DCB_AWTYP( IAX, IDCB ) = '_REAL'
            DCB_AWFRM( IAX, IDCB ) = DCB_DEFRM( IDCB )
         END IF
...
*  Call error tracing routine and exit.
      IF ( STATUS .NE. SAI__OK ) CALL NDF1_TRACE( 'NDF1_DAW', STATUS )
