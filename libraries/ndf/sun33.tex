...
\newcommand{\stardocnumber}    {33.8}
\newcommand{\stardocauthors}   {R.F.~Warren-Smith}
\newcommand{\stardocdate}      {10th July 2006}
...
\latexonly{(SUN/92)} should be consulted for details of the range and
...
\subsection{\xlabel{using_axis_coordinates_to_specify_sections}\label{ss:axissections}Using Axis Coordinates to Specify Sections}
...
\subsection{Logging in for NDF\_ Development}
...
\newcommand{\sstinvocation}[1]{\item[Invocation:]\hspace{0.4em}\hi{#1}}
...
\sstroutine{
   ...
   \sstarguments{
      \sstsubsection{
         ...
         The history update mode required: \hi{'}DISABLED\hi{'}, \hi{'}QUIET\hi{'},
         \hi{'}NORMAL\hi{'} or \hi{'}VERBOSE\hi{'}. This value may be abbreviated, to no
...
\sstroutine{
   ...
   \sstdescription{
      The routine creates a new NDF data structure through the \xref{ADAM}{sg4}{}
      ...
      based on a existing \hi{"}template\hi{"} NDF, and the values of components
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         argument. Thus CLIST=\hi{'}DATA,QUALITY\hi{'} would cause the new NDF to
         ...
         name with the prefix \hi{'}NO\hi{'}. Thus CLIST=\hi{'}DATA,NOHISTORY\hi{'} would
         ...
         using \hi{'}NOEXTENSION()\hi{'} as one of the items in the CLIST argument;
         a list of the extensions to be suppressed should appear between
         the parentheses. Thus CLIST=\hi{'}AXIS,NOEXTENSION(IRAS,ASTERIX)\hi{'}
         ...
         \hi{'}EXTENSION()\hi{'} in a similar manner at a later point in the CLIST
         value.
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         the \hi{"}Component Propagation\hi{"} section for further details.
      }
      \sstsubsection{
         PLACE = INTEGER (Given and Returned)
      }{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         CLIST argument. Thus CLIST=\hi{'}DATA,QUALITY\hi{'} would cause the DATA
         ...
         by supplying a component name with the prefix \hi{'}NO\hi{'}. Thus
         CLIST=\hi{'}DATA,NOHISTORY\hi{'} would propagate the DATA component, but
...
\sstroutine{
   ...
   \sstinvocation{
      CALL NDF\_SSZx( SCALE, ZERO, INDF, COMP, STATUS )
   }
   \sstarguments{
      \sstsubsection{
         SCALE = ? (Given)
...
\sstroutine{
   ...
   \sstnotes{
      ...
      \sstitemlist{
         ...
         \sstitem
         \hi{'}DOCVT\hi{'}: Controls whether to convert foreign format
data files to and from native NDF format for access (using the
facilities described in \xref{SSN/20}{ssn20}{}). If DOCVT is set to 1
(the default), and the other necessary steps described in SSN/20 have
been taken, then such conversions will be performed whenever they are
necessary to gain access to data stored in a foreign format. If DOCVT
is set to 0, no such conversions will be attempted and all data will
be accessed in native NDF format only.
...
\xref{SSN/20}{ssn20}{}). If SHCVT
         ...
         \sstitem
         \hi{'}TRACE\hi{'}: Controls the reporting of additional error messages
         ...
         \sstitem
         \hi{'}WARN\hi{'}: Controls the issuing of warning messages when certain
...
\subsection{\xlabel{list_of_most_recent_changes}Changes Introduced in V1.6}
