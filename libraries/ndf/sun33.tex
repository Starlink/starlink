...
\sstroutine{
   ...
   \sstarguments{
      \sstsubsection{
         ...
         NDF parameters that have been accessed using NDF\_ASSOC or
         NDF\_EXIST are cancelled.
      }
      \sstsubsection{
         STATUS = INTEGER (Given and Returned)
      }{
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \sstitem
         The behaviour of this routine is identical to PAR\_CANCL, except
         ...
         unaffected by this routine. It{\tt '}s only affect is to cause NDF\_ASSOC
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         Name of the NDF array component; {\tt '}DATA{\tt '}, {\tt '}QUALITY{\tt '} or {\tt '}VARIANCE{\tt '}.
      }
      \sstsubsection{
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         extension names. Thus {\tt '}NOEXTENSION($*$),EXTENSION(IRAS){\tt '}
...
\sstroutine{
   ...
   \sstnotes{
      ...
      \sstitemlist{
         ...
         \sstitem
         {\tt '}AUTO\_HISTORY{\tt '}; Controls whether to include an empty
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         These may be selected from {\tt '}DATA, {\tt '}VARIANCE{\tt '} or {\tt '}QUALITY{\tt '}.
         Additionally, if {\tt '}$*${\tt '} is supplied all three components will be
         compressed.
      }
      \sstsubsection{
         MINRAT = REAL (Given)
      ...
      \sstsubsection{
         ...
         array values. This must be one of {\tt '}\_BYTE{\tt '}, {\tt '}\_WORD{\tt '} or {\tt '}\_INTEGER{\tt '}.
   ...
   \sstnotes{
      \sstitemlist{
         ...
         will be reported if the input NDF has a complex type, or if {\tt "}TYPE{\tt "}
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         Numeric type of the output NDF{\tt '}s DATA component (e.g. {\tt '}\_REAL{\tt '} or
         {\tt '}\_INTEGER{\tt '}).
      }
      \sstsubsection{
         ...
         {\tt "}Notes:{\tt "} below. On exit, any supplied values will be rounded to
   ...
   \sstnotes{
      \sstitemlist{
         ...
         reported if TYPE is {\tt '}\_DOUBLE{\tt '}.
...
\subsection{\xlabel{list_of_most_recent_changes}Changes Introduced in V1.12}

The following describes the most significant changes which occurred in
the NDF\_ system between versions V1.11 and V1.12 (the current version):
