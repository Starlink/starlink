...
void ndf1Bpp( const char *type, size_t el, void *pntr, int *bad, int *status ) {
/*
*+
*  Name:
*     ndf1Bpp
...
*  Synopsis:
*     void ndf1Bpp( const char *type, size_t el, void *pntr, int *bad,
*                   int *status )
...
/* ...byte. */
      if( !strcmp( utype, "_BYTE" ) ){
         ndf1BppB( el, pntr, bad, status );

/* ...unsigned byte. */
      } else if( !strcmp( utype, "_UBYTE" ) ){
         ndf1BppUB( el, pntr, bad, status );

/* ...double precision. */
      } else if( !strcmp( utype, "_DOUBLE" ) ){
         ndf1BppD( el, pntr, bad, status );

/* ...integer. */
      } else if( !strcmp( utype, "_INTEGER" ) ){
         ndf1BppI( el, pntr, bad, status );

/* ...real. */
      } else if( !strcmp( utype, "_REAL" ) ){
         ndf1BppF( el, pntr, bad, status );

/* ...word. */
      } else if( !strcmp( utype, "_WORD" ) ){
         ndf1BppW( el, pntr, bad, status );

/* ...unsigned word. */
      } else if( !strcmp( utype, "_UWORD" ) ){
         ndf1BppUW( el, pntr, bad, status );

/* ...64-bit integer. */
      } else if( !strcmp( utype, "_INT64" ) ){
         ndf1BppK( el, pntr, bad, status );
...
/* If the TYPE argument was not valid, then report an error. */
   if( !typok && *status == SAI__OK ){
      *status = NDF__FATIN;
      ...
      errRep( " ", "Routine ndf1Bpp called with an invalid TYPE argument "
              "of '^BADTYPE' (internal programming error).", status );
   }

/* Call error tracing routine and exit. */
   if( *status != SAI__OK ) ndf1Trace( "ndf1Bpp", status );
