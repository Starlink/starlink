...
void ndf1Avfrm( int iax, NdfACB *acb, char *form, size_t form_length,
                int *status ){
/*
*+
*  Name:
...
*  Description:
*     This function returns the storage form of an NDF axis variance array
...
*        holding axis variance array storage form (upper case).
*     form_length
...
/* Otherwise, obtain the default storage form from the DCB. */
         ...
         ndf1Ccpy( dcb->avfrm[ iax ], form, form_length, status );
      }
   }

/* Call error tracing function and exit. */
   if( *status != SAI__OK ) ndf1Trace( "ndf1Avfrm", status );
