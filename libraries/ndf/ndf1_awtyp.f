      SUBROUTINE NDF1_AWTYP( IAX, IACB, TYPE, STATUS )
*+
*  Name:
*     NDF1_AWTYP
...
*  Invocation:
*     CALL NDF1_AWTYP( IAX, IACB, TYPE, STATUS )

*  Description:
*     The routine returns the numeric type of an NDF axis width array
...
*        Axis width array numeric type (upper case).
...
*     -  If the axis width array exists, then determine its numeric
*     type directly.
...
*  directly.
         IF ( DCB_AWID( IAX, IDCB ) .NE. ARY__NOID ) THEN
            CALL ARY_TYPE( DCB_AWID( IAX, IDCB ), TYPE, STATUS )

*  Otherwise, obtain the numeric type from the DCB.
         ELSE
            CALL NDF1_CCPY( DCB_AWTYP( IAX, IDCB ), TYPE, STATUS )
         END IF
      END IF

*  Call error tracing routine and exit.
      IF ( STATUS .NE. SAI__OK ) CALL NDF1_TRACE( 'NDF1_AWTYP', STATUS )
