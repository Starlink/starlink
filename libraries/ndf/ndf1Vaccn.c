...
void ndf1Vaccn( const char *ccomp, int *iccomp, int *status ){
/*
*+
*  Name:
*     ndf1Vaccn
...
*  Description:
*     This function checks that the name of an NDF axis character component
...
*        (one of the symbolic constants NDF__ALAB or NDF__AUNI).
*     *status
*        The global status.
...
/* If the component name was not recognised, then report an error. */
      ...
      *status = NDF__CNMIN;
      msgSetc( "BADCOMP", ccomp );
      errRep( " ", "Invalid axis character component name '^BADCOMP' "
              "specified (possible programming error).", status );
   }

/* Call error tracing function and exit. */
   if( *status != SAI__OK ) ndf1Trace( "ndf1Vaccn", status );
