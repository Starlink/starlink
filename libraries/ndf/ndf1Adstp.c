...
void ndf1Adstp( const char *type, int iax, NdfACB *acb, int *status ){
/*
*+
*  Name:
*     ndf1Adstp
...
*  Description:
*     This function sets a new numeric type for an NDF's axis data array.
...
*        (case insensitive).
*     iax
*        Zero-based index of the NDF axis whose data array numeric type is
*        to be modified.
*     acb
...
*  Notes:
*     -  The numeric type of an axis data array cannot be changed with this
*     function while mapped access to any part of it is in effect.
*     -  This function may only be used to set a numeric type for the axis
*     data array of a base NDF. It returns without action if the NDF
...
/* If the axis data array exists, then set a new numeric type for it. */
            ...
            aryStype( type, dcb->adid[ iax ], status );

/* Otherwise, set a new default value for its numeric type in the DCB
   (in upper case). */
         } else {
            star_strlcpy( dcb->adtyp[ iax ], type, sizeof( dcb->adtyp[ iax ] ) );
            astChrCase( NULL, dcb->adtyp[ iax ], 1, 0 );
         }
      }
   }

/* Call error tracing function and exit. */
   if( *status != SAI__OK ) ndf1Trace( "ndf1Adstp", status );
