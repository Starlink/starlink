...
void ndf1Typ( NdfACB *acb, const char *comp, int *itype, int *status ){
/*
*+
*  Name:
*     ndf1Typ
...
*  Purpose:
*     Obtain the numeric data type of an NDF array component identified by
*     its ACB entry.
...
*  Description:
*     This function returns the numeric data type of one of the array
*     components of an NDF as an upper case character string.  The NDF is
*     identified by its entry in the ACB.
...
/* Call error tracing function and exit. */
   if( *status != SAI__OK ) ndf1Trace( "ndf1Typ", status );
