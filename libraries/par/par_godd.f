      SUBROUTINE PAR_GODD( PARAM, DEFAUL, VMIN, VMAX, NULL, VALUE,
     :                     STATUS )
...
*     acceptable limits and odd.  If it is not report the fact and the
*     limits.
...
*  range.
     ...
     :                 .OR. ( ABS( MOD( VALUE, 2 ) ) .NE. 1 )
            ELSE
               NOTOK = ( VALUE .LT. VMIN ) .OR. ( VALUE .GT. VMAX ) .OR.
     :                 ( ABS( MOD( VALUE, 2 ) ) .NE. 1 )
            END IF
...
*  including full information using tokens.
            IF ( NOTOK ) THEN
               ...
               CALL ERR_REP( 'PAR_GODD_OUTR',
     :           '^VALUE is not permitted for ^PARAM.  Give an odd '/
...
*  Inform the user what has happened.
               ...
               CALL MSG_SETC( 'PARAM', PARAM )
               CALL MSG_OUTIF( MSG__VERB, 'PAR_GODD_DEFA',
