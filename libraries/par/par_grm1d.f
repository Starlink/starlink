      SUBROUTINE PAR_GRM1D( PARAM, NVALS, DEFAUL, VMIN, VMAX, NULL,
     :                        VALUES, STATUS )
...
*  Actually set the suggested values.
      IF ( SUGDEF ) CALL PAR_DEF1D( PARAM, NVALS, DEFAUL, STATUS )
...
*  Get exactly NVALS values for the parameter.
         CALL PAR_EXACD( PARAM, NVALS, VALUES, STATUS )
...
*  including full information using tokens.
               IF ( NOTOK ) THEN
                  ...
                  CALL MSG_SETC( 'PARAM', PARAM )
                  CALL MSG_SETI( 'EN', I )
                  CALL MSG_SETD( 'MIN', VMIN( I ) )
                  CALL MSG_SETD( 'MAX', VMAX( I ) )
                  IF ( EXCLUD ) THEN
...
*  Reset the dynamic defaults in the parameter system.
               IF ( SUGDEF )
     :           CALL PAR_DEF1D( PARAM, NVALS, DEFAUL, STATUS )
...
*  Inform the user what has happened.
               CALL MSG_SETD( 'DEFAULT', DEFAUL )
               CALL MSG_SETC( 'PARAM', PARAM )
               CALL MSG_OUTIF( MSG__VERB, 'PAR_GDR1D_DEFA',
