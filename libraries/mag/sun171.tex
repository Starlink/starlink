...
\newcommand{\stardoctitle}     {MAG \\ [1ex]
...
are also described\latexonly{ (in Appendix \ref{vax})}.
...
invocations\latexonly{ (see Sections \ref{positions} and \ref{dataset_intro})}.
...
tape devices in an 
\xref{HDS}{sun92}{abstract}\latexonly{\footnote{Hierarchical Data System 
...
directory\latexonly{ (see SUN/144)}. 
...
\htmlref{different on VAX/VMS}{vax}\latexonly{ (see Appendix \ref{vax})}.
...
system \latexonly{ (see SUN/104)}, and an appropriate STATUS value set. 
...
\htmlref{MAG\_ALOC}{MAG_ALOC} --   Allocate tape device \\
\htmlref{MAG\_ANNUL}{MAG_ANNUL} --   Annul tape descriptor, releasing any associated tape drive \\
\htmlref{MAG\_ASSOC}{MAG_ASSOC} --   Open a tape device defined by a
parameter and return a descriptor \\
\htmlref{MAG\_CANCL}{MAG_CANCL} --   Close a tape device defined by a parameter\\
\htmlref{MAG\_CLOSE}{MAG_CLOSE} --   Close a tape device, given a descriptor \\
\htmlref{MAG\_DEACT}{MAG_DEACT} --   Annul all tape descriptors \\
\htmlref{MAG\_DEAL}{MAG_DEAL} --   De-allocate a tape device \\
\htmlref{MAG\_DISM}{MAG_DISM} --   Dismount tape from drive \\
\htmlref{MAG\_JEOV}{MAG_JEOV} --   Jump over an EOV condition (2 consecutive tape marks) \\
\htmlref{MAG\_JUMP}{MAG_JUMP} --   Skip a specified number of physical blocks \\
\htmlref{MAG\_MOVE}{MAG_MOVE} --   Move to a specified file and block on tape \\
\htmlref{MAG\_OPEN}{MAG_OPEN} --   Open a tape device \\
\htmlref{MAG\_POS}{MAG_POS} --   Enquire current tape file/block positions \\
\htmlref{MAG\_READ}{MAG_READ} --   Read a block from tape \\
\htmlref{MAG\_REW}{MAG_REW} --   Rewind tape \\
\htmlref{MAG\_SET}{MAG_SET} --   Set current tape file/block positions \\
\htmlref{MAG\_SKIP}{MAG_SKIP} --   Skip a specified number of tape marks \\
\htmlref{MAG\_STOP}{MAG_STOP} --   Cancel all tape devices \\
\htmlref{MAG\_WRITE}{MAG_WRITE} --   Write a block to tape \\
\htmlref{MAG\_WTM}{MAG_WTM} --   Write tape mark
\end{latexonly}
...
\xref{ADAM\_USER directory}{sun144}{ADAM_USER_directory}\latexonly{ (see SUN/144)}.
...
\htmlref{device datasets}{device_datasets}\latexonly{ (Section \ref{device_datasets})}.
