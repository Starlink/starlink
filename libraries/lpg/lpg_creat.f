      SUBROUTINE LPG_CREAT( PARAM, FTYPE, NDIM, LBND, UBND, INDF,
     :                       STATUS )
*+
*  Name:
*     LPG_CREAT
...
*  Description:
*     This routine should be called in place of NDF_CREAT within
...
*  If looping is currently disabled, just call NDF_CREAT and exit.
      IF( DISAB ) THEN
         CALL NDF_CREAT( PARAM, FTYPE, NDIM, LBND, UBND, INDF,
     :                   STATUS )
         GO TO 999
      END IF
...
*  Report an error if all slots are in use.
         ...
         IF( NPAR .GT. LPG__MXPAR .AND. STATUS .EQ. SAI__OK ) THEN
            ...
            CALL ERR_REP( 'LPG_CREAT_ERR1', 'Too many data file '//
...
*  flag character.
     ...
     :                      FLAG, STATUS )
            IF( FLAG ) THEN
               ...
               CALL MSG_OUT( 'LPG_CREAT_MSG1', 'Please supply more '//
...
*  supplied, report a PAR__NULL status.
         IF( STATUS .EQ. SAI__OK .AND. SIZE( NPAR ) .EQ. 0 ) THEN
            ...
            CALL ERR_REP( 'LPG_CREAT_ERR2', 'Null NDF structure '//
...
*  associated with it.
     ...
     :            STATUS .EQ. SAI__OK ) THEN
            STATUS = PAR__NULL
            CALL MSG_SETC( 'PARAM', UPAR )
            CALL ERR_REP( 'LPG_CREAT_ERR3', 'Null NDF structure '//
     :                    'specified for the ''%^PARAM'' parameter.',
     :                    STATUS )
         END IF
...
*  already been reported.
         IF( VERB .AND. .NOT. REP( IPAR ) ) THEN
            CALL NDF_MSG( 'N', INDF )
            CALL MSG_SETC( 'P', UPAR )
            CALL MSG_OUT( 'LPG_CREAT_MSG2', '%^P = ^N', STATUS )
            REP( IPAR ) = .TRUE.
         END IF
