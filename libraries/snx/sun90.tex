...
\stardocnumber      {90.10}
\stardocauthors     {P C T Rees, M J Bly,  P T Wallace}
\stardocdate        {7 March 1995}
\stardoctitle       {SNX --- Starlink Extensions to
                                the NCAR Graphics Utilities}
\stardocversion     {Version 1.1-1}
...
the environment variables \texttt{PATH} and \texttt{LD\_LIBRARY\_PATH} (see
...
and ``$-0.5$" \emph{etc.}: it is a Starlink version of the NCAR
subroutine AGCHNL.  These routines will be automatically linked by
using \texttt{`snx\_link`} if you have called the SNX routine \texttt{SNX\_AGOP}.   If you are using SNX routines as part of an NCAR/SGS mix,
and have not called \texttt{SNX\_AGOP} to initialise the graphics system,
...
orientation, \emph{etc.}) can be achieved by setting the appropriate AUTOGRAPH
...
NCAR can plot other forms of data: \emph{e.g.}\ y array only, multiple curves,
\emph{etc.}
...
To set up one of the four predefined AUTOGRAPH labels (see \S 2.30 \emph{et
...
Extremely flexible control over font, colour, \emph{etc.}, is available via GKS
...
``\texttt{r'S'2'N'}''.
The ``\texttt{r}" is simply drawn as a lower case ``r"; the ``\texttt{'S'}" is a
function code switching to superscript; the ``\texttt{2}" is then drawn as a small
numeral in the superscript position; and finally the ``\texttt{'N'}" switches back
...
facilities and thus allows control of font, precision, \emph{etc.}
...
contains the labels, axes, grid, \emph{etc.}) to be coincident with the GKS
...
plot, including labels \emph{etc.}, is drawn), so that the AUTOGRAPH
...
coordinates (GX,GY) and the user (\emph{i.e.}\ data) coordinates (UX,UY).
...
operates in user (\emph{i.e.}\ data) coordinates, not appropriate for
...
the left edge of the leftmost character is at XG,YG" \emph{etc.}
...
X, Y (real) are user (\emph{i.e.}\ data) coordinates.
...
\emph{etc.}, may be made by direct SGS/GKS calls before SNX\_CURS is called.
...
\textbf{PATH} and \textbf{LD\_LIBRARY\_PATH} (see SUN/118), then to link an ADAM
...
the ALINK and MLINK procedures, \emph{e.g.}
...
on VAX/VMS and in the archive \texttt{/star/\-starlink/\-lib/\-snx/\-examples/\-snx-examples\_source.tar} on
...
\texttt{/star/bin/examples/snx}.  If it has been deinstalled and removed to save
...
NCAR\_DIR:XYPLOT.DAT on VAX/VMS and \texttt{/star/\-bin/\-examples/\-snx/xyplot.dat} on UNIX (if installed).  The
...
in the directory NCAR\_DIR on VAX/VMS and in the archive \texttt{/star/\-starlink/\-lib/\-snx/examples/snx-examples.tar} on UNIX.
...
and \textbf{/star/\-starlink/\-lib/\-snx/speed.dat} on UNIX) and ``Workstation?"
...
NCAR\_DIR on VAX/VMS and \textbf{/star/\-starlink/\-lib/\-snx/examples} on UNIX.
...
string to select different fonts, draw subscripts, \emph{etc.} These
function codes are the ones you will need to use if you wish to have
Greek letters, \emph{etc.}\ in your labels.  The AGPWRITX documentation
...
control codes to switch font, \emph{etc.}
...
\begin{tabular}{llll}
 ...
 & CHRS,NCHS & c,i & string and length, inclusive of codes, \emph{etc.}\\
...
character string CHRS to change font, case, \emph{etc.}\ within the plotted
string.
...
\begin{quote}
\begin{tabular}{ll}
R & Roman characters, \emph{etc.}\\
G & Greek characters, \emph{etc.}
\end{tabular}
\end{quote}
