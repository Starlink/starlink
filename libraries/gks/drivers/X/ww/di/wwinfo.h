...
#define ASKDISPLAY	040	/* XDisplay */
#define ASKSCREEN	050	/* XScreen */
/*
...
int	 gk0xddselect();
box	 gk0xemucharbox();
emupane	*gk0xemucreate();
void	 emufollow();
void	 emufree();
char	*gk0xemuget();
void	 gk0xemuprint();
void	 gk0xemurecreate();
void	 gk0xemushow();
char	*emutermcap();
int	 exread();
int	 exwrite();
...
void	 gk0xbmbox(box b,int flags);
void	 gk0xbmcircle(bitmap *bm,int x,int y,int radius,int flags);
box	 bmclip(bitmap *bm,box b);
bitmap	*gk0xbmcopy(bitmap *bm,box b,int flags);
bitmap	*gk0xbmdecode(char *pattern,int style);
void	 gk0xbmdraw(int xa,int ya,int xb,int yb,bitmap *bm,int rop);
...
void	 gk0xbmfree(bitmap *bm);
bitmap	*gk0xbmget(int width,int height);
int	 bmgetclip(bitmap *bm,box *b);
void	 bmgrey(box b,int sh);
void	 gk0xbmmove(box b,int x,int y);
void	 bmscroll(box b,int xdist,int ydist);
void	 bmslide(box b,int x,int y);
void	 gk0xbmstack(int flag);
void	 gk0xbmxbox(bitmap *bm,box b,int flags);
void	 gk0xbmxcopy(bitmap *from,box frombox,bitmap *to,box tobox,int rop);
bitmap	*gk0xbmxget(int width,int height,int colours);
box	 gk0xboxbuild(int left,int top,int right,int bottom);
int	 gk0xboxinside(box b,int x,int y);
box	 gk0xboxop(box a,box b,int flag);
box	 gk0xboxshift(box b,int x,int y);
box	 gk0xboxzoom(box b,int dist);
int	 copack(int rep,int colour,int flags);
int	 gk0xcorep(bitmap *bm,char *r,char *g,char *b,int step,int start,int count,int flags);
int	 gk0xcosize(bitmap *bm,int colours);
cursor	*gk0xcudecode(char *pattern,int style);
char	*cuencode(cursor *cp,int style,int *length);
void	 gk0xcufree(cursor *cp);
void	 gk0xcuset(cursor *cp,window *wp,int flag);
void	 gk0xcustack(cursor *cp,int flag);
void	 cuwin(cursor *cp,window *wp);
void	 gk0xcuxstack(window *win,cursor *cp,int flag);
int	 gk0xddselect(int fd,int flag);
box	 gk0xemucharbox(emupane *ep,int fromx,int tox,int y);
emupane	*gk0xemucreate(box b);
void	 emufollow(emupane *pp);
void	 gk0xemufree(emupane *ep);
char	*gk0xemuget(emupane *pp,box b);
void	 gk0xemuprint(emupane *ep,char *cp,int len);
void	 gk0xemurecreate(emupane *ep,box b);
void	 gk0xemushow(emupane *pp,box b);
char	*emutermcap(emupane *ep);
int	 exread(int file,int *style,exrep *ex);
int	 exwrite(int file,int what,int style,exrep ex);
box	 gk0xftbox(fontinfo *fp,char *str,int len,int flags);
cursor	*gk0xftcursor(int width,int height);
void	 gk0xftfree(fontinfo *fp);
fontinfo	*gk0xftload(char *file);
void	 gk0xftprint(box b,char *str,int flags);
void	 gk0xftstack(int flag);
void	 gk0xftxprint(int x,int y,char *str,int len,int rop,fontinfo *font,bitmap *bm,int flags);
void	 gk0xipset(int flag);
int	 gk0xipwait();
void	 gk0xipxset(window *wp,int flag);
int	 gk0xipxwait();
void	 gk0xline(int x,int y,int flag);
void	 gk0xlnstack(int flag);
...
int	 gk0xunportask(void *pointer,int what);
void	 gk0xwinstack(int flag);
...
int	 gk0xwwask(int what);
void	 gk0xwwcharmap(bitmap *bm,box b,unsigned char *map,int flags);
void	 gk0xwwexit();
void	 wwfnbox(box posn,char **str);
void	 gk0xwwfree(window *win);
window	*gk0xwwget(box size);
window	*gk0xwwgetscreen();
int	 wwinit();
unsigned	 wwintmap(bitmap *bm,int x,int y,unsigned set,int flags);
void	 wwnoise();
void	 gk0xwwpanic(char *s);
char	*wwshare(char *what,int length,char *filename,int *returnlen);
void	 gk0xwwstack(int flag);
window	*gk0xwwxget(box size,int colours,char *label,int flags);
void	 gk0xwwxnoise(window *win);
void	 gk0xwwxstack(window *win,int flag);
