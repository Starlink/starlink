...
DDPUBLIC window *gk0xwinalloc(size)box size;{
	...
	newwin->w_bm = wwbmget(WIDTH(size),HEIGHT(size));
...
#if defined(WWFORSUN)|defined(WWFORWC)
DDPUBLIC wwseticonpic(bm)bitmap *bm;{
...
#endif WWFORSUN
		icon = bmdecode(defaulticon,ENWWSTYLE);
#ifdef WWFORSUN
		dd->d_colours = savecols;
#endif WWFORSUN
	}
	bmxcopy(icon,icon->bm_box,bm,
	  boxshift(icon->bm_box,(bm->bm_box.b_right-icon->bm_box.b_right)/2,
	  	(bm->bm_box.b_bottom-icon->bm_box.b_bottom)/2),
	  WWCOPY);
}
#endif defined(WWFORSUN)|defined(WWFORWC)
