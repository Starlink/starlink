...
int string_len;
{
  ...
  gk0xoe(echoarea,"String device");		/* Open the echo area */
  echoarea=ddbm->bm_box;

  echoterm = gk0xemucreate(gk0xboxzoom(echoarea, -1));	/* Create terminal emulator */
  gk0xemuget(echoterm, gk0xboxbuild(0, 0, 0, 0));	/* Guarantee retention */
  ...
  if (initstr_len > 0) {			/* process it */
    ...
    gk0xemuprint(echoterm, initstr_ptr, initstr_len);	/* And display it */
    ...
    gk0xemuprint(echoterm, keyecho, 4);		/* and position cursor */
  }
  ...
  while (!stop) {				/* Loop processing input */
	...
	keyecho[0] = 0;
      gk0xemuprint(echoterm, keyecho, strlen(keyecho));	/* echo the character */
    };
  };
