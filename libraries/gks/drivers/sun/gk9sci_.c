 ...
 *  16/11/89  RMK   Created gk9sci_.c based on gk9srl_.c   - needed for
 *                  implementation of pick input.  gk9srl_.c is now obsolete.
 *  20/02/91  ACA   Made cursor ignore initial position settings and appear
 *                  where it physically is (S451).
 */
#include "../../drivers/sun/varinc/wwinfo.h"
...
locator input */
  f77_integer *keyhit;
  f77_real *fxdc, *fydc;
  f77_integer inta[];		/* Integer part of locator device state */
  f77_real reala[];		/* Real  part of locator device state*/
{
  f77_integer
  ...
  xdc = *fxdc;
  ydc = y_max - *fydc;				/* Invert Y for ww */
  ...
  xdc = dd->d_x ;		                /* Set the intitial position */
  ydc = dd->d_y ;                               /* to be as in ww */
  if (inta[KIPE-1] == GECHO)			/* Open up echo area */
    (*locator[inta[KIPPRT-1]-1].open)(reala);
  ipset(IPON);					/* Switch on input */
  do {
	  gkyerr_.kerror = 0;			/* Ignore DC -> NDC errors */
	  if (inta[KIPE-1] == GECHO)
	    (*locator[inta[KIPPRT-1]-1].echo)
	      ((int) xdc, (int) ydc,
	       (int) inixdc,
	       (int) iniydc);	                /* echo the locator position */
	  ipwait();				/* wait for an event */
	  if (inta[KIPE-1] == GECHO)
	    (*locator[inta[KIPPRT-1]-1].erase)
	      ((int) xdc, (int) ydc,
	       (int) inixdc,
  ...
  static trackingcross(x,y) int x,y; /* Display a small tracking cross */
{
  ...
  static digitalopen(reala) f77_real reala[];
		/* Open echo area for PET 6 */
{
	echoarea = boxbuild((int) reala[KIPEXL-1],
