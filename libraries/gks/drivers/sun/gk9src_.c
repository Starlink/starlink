/*
 * gk9src_.c --- Module for handling a choice tool on the SUN workstation for
 * RAL GKS.
 *
     ...
     f77_integer *choice;
{
	...
	(*choiceecb[inta[KIPPRT] - MINPET].open) ();	/* Open up echo area */
	ipset(IPON);				/* Enable input */
	do {
		(*choiceecb[inta[KIPPRT] - MINPET].echo) ();	/* Echo current i/p */
		ipwait();			/* Wait for input */
		(*choiceecb[inta[KIPPRT] - MINPET].erase) (); /* Erase the echo */
	} while (!((VALIDCHOICE && TRIGGER) || BREAK));
	gk9sbu();
	ipset(IPOFF);				/* Disable input */
	*choice = (*choiceecb[inta[KIPPRT] - MINPET].close) ();
...
buttonsopen1()
{
	...
	gk9soe(echoarea, "Choice device");	/* Open the echo area */
	...
	for (i = 0; i < NROWS; i++) {		/* Compute all the boxes */
		for (j = 0; j < NCOLS; j++) {
			choiceboxes[boxno++] = boxbuild(left, top, left + width - 1, top + height - 1);
			left += width;
		};
		top += height;
		left = 1;
	...
	for (i = 0; i < NBOXES; i++) {		/* Draw all the boxes */
		char label[5];
		bmbox(choiceboxes[i], BMEDGES | BMCLEAR); /* Draw edges of box */
		(void) sprintf(label, "%d", i + 1); /* Construct the label */
		ftprint(choiceboxes[i], label, FTCENTRE);	/* and print it */
	}
}
...
buttons()
{
	...
	if (dd->d_event != IPLEAVE &&
		...
		for (boxno = 0; (boxno < NBOXES) &&
		     !boxinside(choiceboxes[boxno], dd->d_x, dd->d_y);
		     boxno++);
		if (boxno != NBOXES && boxno != oldboxno) {
			if (oldboxno != -1)	/* Switch off old box */
			  bmbox(choiceboxes[oldboxno], BMNOT);
			bmbox(choiceboxes[boxno], BMNOT); /* Switch on new box */
...
{						/* PET 3 (Menus) */
	...
	chbox = ftbox(ftload(NULLPTR(char)),itemchars[longest],maxlen,FTPROP);
	...
	choiceboxes[0] = boxbuild(0, 0, chwd, chht);
	for (i = 1; i < nitems; i++)
	  choiceboxes[i] = boxshift(choiceboxes[i - 1], 0, chht);
	...
	for (i = 0; i < nitems; i++) {
		...
		ftprint(choiceboxes[i], itemchars[i],
  ...
  stringopen()
{
	...
	chbox = ftbox(ftload(NULLPTR(char)),itemtable[longest],maxlen,FTPROP);

	/* Must allow for prompt in width and CR at end of line */

	ptbox = ftbox(ftload(NULLPTR(char)),"MMMMMMMMM",9,FTPROP);
	...
	gk9soe(echoarea, "Choice device");	/* Open the echo area */
	ep=emucreate(ddbm->bm_box);		/* create terminal emulator */

	/* Output each item to the terminal emulator we've created */

	for (i = 0; i < nitems; i++) {
		emuprint(ep,itemtable[i],strlen(itemtable[i]));
		pos(ep,0,ep->p_y+1);
	}
}
  ...
  string()
{
	...
	emuprint(ep,"\033K",2);			/* Erase the line */
	emuprint(ep,"Choice: ",8);		/* Prompt the user */
...
{
	gk9soe(echoarea,"Choice device");	/* Open the echo area */
	ftprint(ddbm->bm_box,"Hit a printing key",FTCENTRE);
}
