************************************************************************
      ...
      SUBROUTINE AGI_RCP ( PNAME, PSTART, PICID, STATUS )
...
*     Find a picture of the given name searching backwards.
...
*   If picture is not there then indicate an error
         IF ( .NOT. FOUND ) THEN
            STATUS = AGI__PICNF
            CALL ERR_REP( 'AGI_RCP_PICNF', 'Picture not found', STATUS )
            GOTO 99
         ENDIF
...
*   If the starting picture is invalid then start from the last one
            IF ( SPICNM .LT. 1 ) THEN
               CSTART = 'L'
            ELSE
               CSTART = 'P'
...
*   Flag an error if the starting identifier is improper
         ELSE
            STATUS = AGI__IMPID
            CALL ERR_REP( 'AGI_RCP_IMPID',
...
*   same device
         IF ( WKNAME. NE. SWKNAM ) THEN
            STATUS = AGI__DIFDV
            CALL ERR_REP( 'AGI_RCP_DIFDV', 'Different devices', STATUS )
            GOTO 99
         ENDIF

*   Find a picture of the given name searching backwards
*   from the current one. Have to decrement SPICNM before starting.
         SPICNM = SPICNM - 1
         CALL AGI_1SEARP( WKNAME, PNAME, CSTART, 'B', SPICNM, OPNAME,
...
*   for another one.
     ...
     :                ( STATUS .EQ. SAI__OK ) )
               SPICNM = SPICNM - 1
               CALL AGI_1SEARP( WKNAME, PNAME, 'P', 'B', SPICNM, OPNAME,
