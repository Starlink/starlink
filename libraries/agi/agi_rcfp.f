************************************************************************
      ...
      SUBROUTINE AGI_RCFP ( PNAME, X, Y, PICID, STATUS )
...
*   Get the device coords of the current picture
         IF ( CURPID .GT. 0 ) THEN
            WKNAME = CAGIWK( CURPID )
            PICNUM = CPICNM( CURPID )
         ELSE
            STATUS = AGI__NOCUP
            CALL ERR_REP( 'AGI_RCFP_NOCUP',
...
*   Find a picture of that name starting from the first picture.
