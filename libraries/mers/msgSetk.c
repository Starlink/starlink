...
*  Purpose:
*     Assign a 64-bit integer value to a message token (concise).

*  Language:
*     Starlink ANSI C

*  Invocation:
*     msgSeti( const char * token, int64_t kvalue );
...
void msgSetk( const char * token, int64_t kvalue ) {
  emsSetk( token, kvalue );
}
