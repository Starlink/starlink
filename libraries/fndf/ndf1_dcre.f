      SUBROUTINE NDF1_DCRE( FTYPE, NDIM, LBND, UBND, IPCB, IACB,
     :                      STATUS )
*+
*  Name:
*     NDF1_DCRE
...
*  Invocation:
*     CALL NDF1_DCRE( FTYPE, NDIM, LBND, UBND, IPCB, IACB, STATUS )

*  Description:
*     The routine creates a simple NDF data object containing just a
...
*     6-DEC-1989 (RFWS):
*        Implemented initialisation of the attribute entries in the DCB
*        which are used as default values for other components.
*     12-DEC-1989 (RFWS):
*        Implemented initialisation of DCB mapping counts.
*     26-FEB-1990 (RFWS):
*        Changed NDF placeholder data type to 'NDF'.
*     1-MAR-1990 (RFWS):
*        Removed un-referenced variable.
*     22-MAR-1990 (RFWS):
*        Call HDS_TUNE to set the optimum number of NDF components.
*     26-MAR-1990 (RFWS):
*        Added initialisation of the DCB quality bad-bits override
*        flag.
*     1-AUG-1990 (RFWS):
...
*  for it in the DCB.
         ...
         CALL ARY_NEW( FTYPE, NDIM, LBND, UBND, PLACE, DCB_DID( IDCB ),
     :                 STATUS )
...
*  default values for other components.
         ...
         CALL ARY_CMPLX( DCB_DID( IDCB ), DCB_DECPX( IDCB ), STATUS )
...
*  Call error tracing routine and exit.
      IF ( STATUS .NE. SAI__OK ) CALL NDF1_TRACE( 'NDF1_DCRE', STATUS )
