      SUBROUTINE NDF1_AWSBN( LBNDW, UBNDW, IAX, IACB, STATUS )
*+
*  Name:
*     NDF1_AWSBN
...
*  Invocation:
*     CALL NDF1_AWSBN( LBNDW, UBNDW, IAX, IACB, STATUS )

*  Description:
*     The routine sets new pixel-index bounds for an NDF's axis
*     width array, retaining the existing array values (if they exist).
*     The existing values will be extrapolated if the new bounds extend
*     beyond the old ones.

*  Arguments:
*     LBNDW( 1 ) = INTEGER (Given)
*        New lower pixel-index bound for the axis.
*     UBNDW( 1 ) = INTEGER (Given)
*        New upper pixel-index bound for the axis.
*     IAX = INTEGER (Given)
*        Number of the NDF axis whose width array bounds are to be
*        changed.
*     IACB = INTEGER (Given)
...
*     width array for a base NDF. It will return without action if the
...
*     initial axis width array bounds.

*  Algorithm:
*     -  Check that the NDF is not a section. There is nothing to do if
*     it is.
*     -  Obtain an index to the data object entry in the DCB.
*     -  Ensure that information about the axis width array is
...
*     new axis width array will have to hold values which are an
*     extrapolation of the current one.
*     -  Obtain the width array's numeric type and storage form and
...
*     become the new axis width array.
*     -  Import the array into the ARY_ system and store the identifier
*     in the DCB.
...
*  History:
*     23-OCT-1990 (RFWS):
*        Original version, derived from the NDF1_AVSBN routine.
*     2-NOV-1990 (RFWS):
*        Re-structured the re-importation of the new width array.
...
*  the new bounds, if necessary.
            IF ( DCB_AWID( IAX, IDCB ) .EQ. ARY__NOID ) THEN
               CALL NDF1_CBFRM( 1, LBNDW, UBNDW, DCB_AWFRM( IAX, IDCB ),
     :                          STATUS )
...
*  necessary.
                     CALL ARY_TYPE( DCB_AWID( IAX, IDCB ), TYPE,
     :                              STATUS )
                     CALL ARY_FORM( DCB_AWID( IAX, IDCB ), FORM,
     :                              STATUS )
                     CALL NDF1_CBFRM( 1, LBNDW, UBNDW, FORM, STATUS )
...
*  Create a primitive array.
                        IF ( FORM .EQ. 'PRIMITIVE' ) THEN
                           CALL ARY_NEWP( TYPE, 1, UBNDW, PLACE, TIARY,
     :                                    STATUS )
...
*  for writing, and copy the values across.
                           CALL NDF1_AWMAP( IAX, IACBS, TYPE, 'READ',
     :                                      PNTR, EL, STATUS )
...
*  Unmap the arrays when done.
                           CALL NDF1_AWUMP( IAX, IACBS, STATUS )
...
*  Create a simple array.
                        ELSE IF ( FORM .EQ. 'SIMPLE' ) THEN
                           CALL ARY_NEW( TYPE, 1, LBNDW, UBNDW, PLACE,
     :                                   TIARY, STATUS )
...
*  new axis width array.
     ...
     :                              STATUS )
                     CALL DAT_RENAM( TLOC, 'WIDTH', STATUS )

*  Import the array into the ARY_ system and store the identifier in
*  the DCB.
                     CALL ARY_IMPRT( TLOC, DCB_AWID( IAX, IDCB ),
     :                               STATUS )
...
*  Call error tracing routine and exit.
      IF ( STATUS .NE. SAI__OK ) CALL NDF1_TRACE( 'NDF1_AWSBN', STATUS )
