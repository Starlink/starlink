...
int
dau_export_loc(struct DSC *locator, struct LCP **pntr)
...
/* If no errors have occurred, then set the current mode of HDS depending on */
/* the "extended" bit in the RCL and return a pointer to the LCP.            */
      if ( _ok( hds_gl_status ) )
      {
