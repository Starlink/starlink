*+ DAT_NEW1C - Create new vector string component, with specified precision
      subroutine dat_new1c(struct, comp, size, len, status)
*    Description :
*     Create vector structure component of type '_CHAR*<n>', with the
*     string precision, <n>, specified by the LEN parameter.
*    Invocation :
*     CALL DAT_NEW1C(LOC, NAME, LEN, SIZE; STATUS)
*    Parameters :
...
*     SIZE=INTEGER
*           Expression specifying the number of values required.
*     STATUS=INTEGER
...
*       Create the new variable
         call dat_new1(struct, comp, type, len, status)
      endif
