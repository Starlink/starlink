...
\stardoccopyright
{Copyright \copyright\ 2019 East Asian Observatory}
...
\stardocauthors   {R.F.~Warren-Smith\\ M.D.~Lawden\\
                                ...
                                D.S.~Berry}
\stardocdate        {18th September 2019}
\stardoctitle       {HDS\\ [\latex{1ex}]
                                Hierarchical Data System}
\stardocversion     {Version 7.0}
...
(version 4 or version 5) to use. By default, the new disk format
(version 5) is used. This means that, by default, files created by version
6 of the HDS library cannot be read by starlink systems that still use
version 5. However, this can be changed by setting the environment variable
``\texttt{HDS\_VERSION}'' to ``4'', or by using the HDS\_TUNE routine to
set tuning parameter ``\texttt{VERSION}'' to 4.
...
created. It may take the value 3, 4 or 5, and currently defaults to 5.
Version 4 is a Starlink-specific data format that is a development of the
original HDS data format created by the Starlink project in the 1980's.
Version 5 uses the HDF5 data format on disk to mimic the facilities of
version 4. Such files cannot be accessed by versions of the HDS library prior
to version 6, but can be examined using various publicly available HDF5 tools
- see section \htmlref{``DISK FORMATS AND HDF5''}{sect:hdf5}.  Version 3 refers
...
\subsubsection*{Parameters used only with HDS version 5 files}

\begin{description}

\item [\xlabel{HDS_LOCKCHECK_tuning_parameter}LOCKCHECK - controls lock checking:]\mbox{}\\
If non-zero, an error is reported if a V5 HDS object has not been locked
appropriately by the current thread before use. If zero, no such check is
performed. This is provided so that old software which pre-dates the
datLock function (and which presumably is explicitly designed to avoid conflicting
simultaneous access to an HDS object from separate threads) can be used
without change. Use of this tuning parameter should be seen as a
temporary stop-gap measure until such time as the software can be changed
to use the datLock function correctly.
...
\section{\xlabel{routine_variants_for_large_arrays}\label{sect:variants}ROUTINE VARIANTS FOR LARGE ARRAYS}
The HDS routines described in the reference section of this document all
use 4 byte integers arguments (i.e. a basic Fortran INTEGER) to store
array dimensions, pixel indices and pixel counts. This restricts their
usefulness to arrays that have no more than 2147483647 pixels (the
largest positive number that can be stored in an INTEGER). Using these
functions with larger arrays will cause overflow and undefined behaviour.
However, for each routine that includes one or more such arguments,
there is a variant of the routine that uses INTEGER*8 arguments for all
array dimensions, pixel indices and pixel counts. Each variant routine has the
same name as the corresponding basic routine but with the digit ``8''
(as in ``INTEGER*8'') appended to the end of the name. The available
variant routines are listed below:

\htmlref{DAT\_ALTER8}{DAT_ALTER},
\htmlref{DAT\_BASIC8}{DAT_BASIC},
\htmlref{DAT\_CELL8}{DAT_CELL},
\htmlref{DAT\_GET8}{DAT_GET},
\htmlref{DAT\_GETx8}{DAT_GETx},
\htmlref{DAT\_GET1x8}{DAT_GET1x},
\htmlref{DAT\_GETVx8}{DAT_GETVx},
\htmlref{DAT\_MAP8}{DAT_MAP},
\htmlref{DAT\_MAPx8}{DAT_MAPx},
\htmlref{DAT\_MAPN8}{DAT_MAPN},
\htmlref{DAT\_MAPV8}{DAT_MAPV},
\htmlref{DAT\_MOULD8}{DAT_MOULD},
\htmlref{DAT\_NEW8}{DAT_NEW},
\htmlref{DAT\_NEWC8}{DAT_NEWC},
\htmlref{DAT\_NEW18}{DAT_NEW1},
\htmlref{DAT\_NEW1x8}{DAT_NEW1x},
\htmlref{DAT\_PUTx8}{DAT_PUTx},
\htmlref{DAT\_PUT8}{DAT_PUT},
\htmlref{DAT\_PUT1x8}{DAT_PUT1x},
\htmlref{DAT\_PUTVx8}{DAT_PUTVx},
\htmlref{DAT\_SHAPE8}{DAT_SHAPE},
\htmlref{DAT\_SIZE8}{DAT_SIZE},
\htmlref{DAT\_SLICE8}{DAT_SLICE},
\htmlref{DAT\_TEMP8}{DAT_TEMP},
\htmlref{HDS\_NEW8}{HDS_NEW}

Each variant routine has the same argument list as the corresponding
basic routine except that any INTEGER argument used to pass array
dimensions, pixel indices or pixel counts is changed to INTEGER*8.

Note, when changing code from using basic to using variant routines,
remember that if a literal integer constant is passed to a subroutine,
it's kind must match that expected by the the subroutine. So if the old
code was:
...
The literal constant ``5\_8'' indicates a 8 byte integer constant with
the value 5. If the old literal constant ``5'' was left unchanged,
spurious behaviour such as a segmentation fault is likely to occur.
...
\textbf{\htmlref{DAT\_DSAME}{DAT_DSAME}}\st{(loc1,loc2;same,status)} -- Are two primitive data representations the same?\\
...
\sstroutine{
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \sstitem Not all combinations of data format and storage order are
                  supported.

         \sstitem This routine may only be used with objects stored using
                  HDS data formats that pre-date version 5. An error is
                  reported if the supplied object is stored using data
...
\sstroutine{
   ...
   Check if two primitive objects have the same data representation.
}{
   \sstdescription{
      The routine returns a flag indicating if the two supplied objects
      (which must both be primitive) are stored using the same data
      representation. See also DAT\_DREP.
   }
   \sstinvocation{
      CALL DAT\_DSAME( LOC1, LOC2, SAME, STATUS )
   }
   \sstarguments{
      \sstsubsection{
         LOC1 = CHARACTER \astar\ ( DAT\_\_SZLOC ) (Given)
      }{
         First primitive object \htmlref{locator}{sect:locators}.
      }
      \sstsubsection{
         LOC2 = CHARACTER \astar\ ( DAT\_\_SZLOC ) (Given)
      }{
         Second primitive object \htmlref{locator}{sect:locators}.
      }
      \sstsubsection{
         SAME = LOGICAL (Returned)
      }{
         .TRUE. if and only if the two objects use the same data
         representation.
      }
      \sstsubsection{
         STATUS = INTEGER (Given and Returned)
   ...
   \sstnotes{
      Unlike DAT\_DREP, this routine may be used on objects stored in
      using any HDS data format, including version 5 and later.
   }
}
\sstroutine{
   DAT\_ERASE
...
\subsection{\xlabel{list_of_most_recent_changes}Changes in V6.1}

\begin{enumerate}
\item The default output data format has been changed from V4 (the
classic starlink format) to V5 (the HDF5-based format). To re-establish
the old behaviour, set environment variable HDS\_VERSION to ``4''.
\end{enumerate}

\subsection{\xlabel{list_of_most_recent_changes}Changes in V7.0}

\begin{enumerate}

\item The definition of the \verb+hdsdim+ type used in the C interface
has been changed from a 4-byte integer type (\verb+int+) to an 8 byte
integer type (\verb+int64_t+). This allows HDS-based applications to
access arrays with more than 2147483647 pixels (the largest value that
can be stored in a 4-byte integer). Application code that uses the
\verb+hdsdim+ type consistently should continue to compile and run as
before. However, application code that assumes that the \verb+hdsdim+
type is 4-bytes long (e.g. by inappropriate casting between \verb+int+
and \verb+hdsdim+) may not compile or run correctly. Such code should
be changed to avoid the assumption of a 4-byte \verb+hdsdim+ type.

The classic HDS Fortran interface has not been changed - INTEGER$*$4
variables are still used for arguments that pass array dimensions, pixel
indices or pixel counts. However, there is now a variant routine for each
routine that has one or more such arguments, which uses INTEGER$*$8 variables
for array dimensions, pixel indices or pixel counts. Each variant routine
is identical to the basic routine except that the digiti ``8'' is
