      SUBROUTINE ANT_ERRC (MESSGE, STRING)
*+
*  Name:
*     ANT_ERRC
*  Purpose:
*     Report a parser error message with an associated CHARACTER string.
*  Language:
*     Fortran 77.
*  Invocation:
*     CALL ANT_ERRC (MESSGE, CODE)
*  Description:
...
*        CHARACTER string associated with the message.
*  Algorithm:
...
*    Assemble the text string containing the error report.
      ...
      IF (STRING .NE. ' ') THEN
         ...
         CALL CHR_PUTC (STRING(1 : STRLEN), BUFFER, BUFPOS)
      ELSE
         CALL CHR_PUTC ('<No string available>', BUFFER, BUFPOS)
      END IF
