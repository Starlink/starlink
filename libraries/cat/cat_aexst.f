      SUBROUTINE CAT_AEXST (CI, IAST, STATUS)
*+
*  Name:
*     CAT_AEXST
*  Purpose:
*     Get an existing AST Object from a catalogue.
*  Language:
*     Fortran 77.
*  Invocation:
*     CALL CAT_AEXST (CI; IAST; STATUS)
*  Description:
*     Get an existing AST Object from a catalogue.
*
*     If there is no AST object stored with the catalogue then a
*     null AST object is returned.  This behaviour can be used to
*     check whether the catalogue contains an AST object.
*
...
*        the catalogue does not contain an AST object or if an error
*        occurs.
...
*     2/11/99  (ACD): First stable version.
*  Bugs:
*     None known
*-
*  Type Definitions:
...
*       pointer is explicitly set to null.
         ...
         IF (STATUS .NE. SAI__OK) THEN
            ...
            CALL ERR_REP ('CAT_AEXST_ERR', 'Failed to return an AST '/
