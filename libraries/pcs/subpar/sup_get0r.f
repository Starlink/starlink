      SUBROUTINE SUBPAR_GET0R ( NAMECODE, RVALUE, STATUS )
*+
*  Name:
*     SUBPAR_GET0R
...
*  Invocation:
*     CALL SUBPAR_GET0R ( NAMECODE, RVALUE, STATUS )

*  Description:
*     Get a scalar REAL value from the storage associated with the
*     indicated parameter.
...
*     The number is extracted in the data type declared for the
*     parameter.  Any required limits checking is done and, if
*     successful, the necessary type conversion is done.
...
*
     ...
     :           STATUS )
               ...
               IF ( STATUS .EQ. SAI__OK ) THEN
                  CALL CHR_CTOR( VALUE_STRING, RVALUE, STATUS )
                  IF ( STATUS .NE. SAI__OK ) THEN
                     STATUS = SUBPAR__CONER
                     CALL EMS_SETC( 'VAL', VALUE_STRING )
                     CALL EMS_REP( 'SUP_GET0R2',
     :                'SUBPAR: Error converting ^VAL to REAL', STATUS )
                  ENDIF
               ENDIF
     ...
     :           STATUS )
               ...
               IF ( STATUS .EQ. SAI__OK ) THEN
                  RVALUE = REAL( VALUE_DOUBLE )
               ENDIF

            ELSE IF ( TYPE .EQ. SUBPAR__LOGICAL ) THEN
               STATUS = SUBPAR__CONER
               CALL EMS_REP( 'SUP_GET0R3',
     :          'SUBPAR: Attempt to convert LOGICAL to REAL',
     :           STATUS )
...
*
               CALL DAT_GETR ( LOC, 0, 0, RVALUE, STATUS )

            ENDIF
*
*          If storage was in an HDS structure, annul the locator.
*          NB - does not close the container file, or annul other cloned
*          locators which might have been previously associated with
*          this parameter. This should be good from the point of view
*          of future access speed to the same value.
...
*        If not SUBPAR probable status error, report status
     ...
     :      .AND. ( STATUS .NE. SUBPAR__CONER ) ) THEN
               ...
               CALL EMS_REP( 'SUP_GET0R4', '^MESS', STATUS )
            ENDIF
...
*        Check for try limit
            ...
            IF ( TRIES .EQ. MAXTRY ) THEN
               ...
               CALL EMS_REP( 'SUP_GET0R5', 'SUBPAR: '//
