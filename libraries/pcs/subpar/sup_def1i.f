      SUBROUTINE SUBPAR_DEF1I ( NAMECODE, NVAL, VALUES, STATUS )
      IMPLICIT NONE
...
*  Invocation:
*     CALL SUBPAR_DEF1I ( NAMECODE, NVAL, VALUES, STATUS )
...
*   are the same as previously.
     ...
     :      .AND. ((PARDYN(3,NAMECODE) .EQ. -SUBPAR__INTEGER) .OR.
     :             (PARDYN(3,NAMECODE) .EQ. SUBPAR__INTEGER)) )
     : THEN
...
*     we are allowed to overwrite an existing set default value
         ...
         PARDYN(3,NAMECODE) = SUBPAR__INTEGER
         DO J = START, FINISH
            K = J - START + 1
            INTLIST(J) = VALUES(K)
         ENDDO
      ...
      ELSE IF ( ( NVAL1 .LE. 6 )
     :    .AND. ( INTPTR + NVAL1 .LT. SUBPAR__MAXLIMS ) ) THEN
