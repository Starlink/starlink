      SUBROUTINE SUBPAR_LIMITR ( NAMECODE, VALUE, ACCEPTED, STATUS )
*+
*  Name:
*     SUBPAR_LIMITR
...
*  Invocation:
*     CALL SUBPAR_LIMITR ( NAMECODE, VALUE, ACCEPTED, STATUS )
...
*     01-OCT-1991 (AJC):
*        Add missing // operator in EMS_REP call
*      1-MAR-1993 (AJC):
*        Add INCLUDE DAT_PAR
*     10-MAR-1993 (AJC):
*        Revise for MIN/MAX
...
*  Arguments Given:
      ...
      REAL VALUE                         ! value to be tested against
...
*   Check if there is a 'one of a set' constraint.
      IF (( PARLIMS(3,NAMECODE) .EQ. SUBPAR__REAL ) .AND.
     :      .NOT. PARCONT(NAMECODE) ) THEN

*      There is - apply it
         DO J = PARLIMS(1,NAMECODE), PARLIMS(2,NAMECODE)
            IF ( VALUE .EQ. REALLIST(J) ) THEN
               ACCEPTED = .TRUE.
            ENDIF
         ENDDO
...
*      If there is a constraint, it is a range or MIN/MAX
         CALL SUBPAR_RANGER( NAMECODE, VALUE, .TRUE.,
     :                       ACCEPTED, STATUS )
