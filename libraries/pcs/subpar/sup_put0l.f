      SUBROUTINE SUBPAR_PUT0L ( NAMECODE, LVALUE, STATUS )
*+
*  Name:
*     SUBPAR_PUT0L

*  Purpose:
*     Write scalar LOGICAL parameter value.

*  Language:
*     Starlink Fortran 77

*  Invocation:
*     CALL SUBPAR_PUT0L ( NAMECODE, LVALUE, STATUS )
...
*
            IF ( .NOT. INTERNAL ) THEN
               ...
               CALL DAT_PUT0L ( LOC, LVALUE, STATUS )
            ...
            ELSE IF ( TYPE .EQ. SUBPAR__CHAR ) THEN
               ...
               CALL CHR_LTOC( LVALUE, PARVALS(NAMECODE), ITEMP )
               IF ( PARVALS(NAMECODE)(1:1) .EQ. '*' ) THEN
                  STATUS = SUBPAR__CONER
                  CALL EMS_SETC ( 'NAME', PARKEY(NAMECODE) )
                  CALL EMS_REP ( 'SUP_PUT0L1',
     :            'SUBPAR: Failed to convert LOGICAL value to '//
     :            'CHARACTER for parameter ^NAME', STATUS )
            ...
            ELSE IF ( TYPE .EQ. SUBPAR__LOGICAL ) THEN
               ...
               PARLOG ( NAMECODE ) = LVALUE

            ELSE

               STATUS = SUBPAR__IVPRTYPE
               CALL EMS_SETC ( 'NAME', PARKEY(NAMECODE) )
               CALL EMS_REP ( 'SUP_PUT0L2',
     :         'SUBPAR: Parameter ^NAME is non-primitive - '//
     :         'attempted PUT0L to it', STATUS )
