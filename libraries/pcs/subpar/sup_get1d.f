      SUBROUTINE SUBPAR_GET1D ( NAMECODE, MAXVAL, DVALUES, ACTVAL,
     :  STATUS )
*+
*  Name:
*     SUBPAR_GET1D
...
*  Invocation:
*     CALL SUBPAR_GET1D ( NAMECODE, MAXVAL, DVALUES, ACTVAL, STATUS )
...
*  Arguments Returned:
      DOUBLE PRECISION DVALUES(*)               ! Array for values
...
*              Get the data values and size
                  CALL DAT_GET1D( BOTLOC, MAXVAL, DVALUES,
     :            ACTVAL, STATUS )
               ENDIF
...
*           Get the data value
               CALL DAT_GET0D( BOTLOC, DVALUES(1), STATUS )
*           and set size = 1
               ACTVAL = 1
