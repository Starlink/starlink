      SUBROUTINE SUBPAR_RANGEC( NAMECODE, VALUE, MNMX, OK, STATUS )
*+
*  Name:
*     SUBPAR_RANGEC
...
*  Invocation:
*     CALL SUBPAR_RANGEC( NAMECODE, VALUE, MNMX, OK, STATUS )
...
*        CHARLIST( SUBPAR__MAXLIMS ) = CHARACTER*(*) (Read)
...
*  Local Variables:
      CHARACTER*(SUBPAR__STRLEN) MINVAL       ! Lower value in RANGE
      CHARACTER*(SUBPAR__STRLEN) MAXVAL       ! Upper value in RANGE
...
*  (probably illegally) point to the same value
      IF ( ( PARLIMS(3,NAMECODE) .EQ. SUBPAR__CHAR )
     :.AND. PARCONT(NAMECODE) ) THEN

*     Check against any RANGE values

*     Get min and max in right order
         IF ( LLE( CHARLIST(PARLIMS(1,NAMECODE)),
     :             CHARLIST(PARLIMS(2,NAMECODE)) ) ) THEN
            EXCLUSIVE = .FALSE.
            MINVAL = CHARLIST(PARLIMS(1,NAMECODE))
            MAXVAL = CHARLIST(PARLIMS(2,NAMECODE))
            IF ( LLT( VALUE, MINVAL )
     :      .OR. LGT( VALUE, MAXVAL ) ) OK = .FALSE.

         ELSE
            EXCLUSIVE = .TRUE.
            MINVAL = CHARLIST(PARLIMS(2,NAMECODE))
            MAXVAL = CHARLIST(PARLIMS(1,NAMECODE))
            IF ( LGT( VALUE, MINVAL )
     :      .AND. LLT( VALUE, MAXVAL ) ) OK = .FALSE.
         END IF
         ...
         IF ( .NOT. OK ) THEN
            ...
            CALL EMS_SETC ( 'L1',  MINVAL )
            CALL EMS_SETC ( 'L2',  MAXVAL )
            IF ( EXCLUSIVE ) THEN
...
*     Check against MIN value if any
     ...
     :   .AND. ( PARMIN(1,NAMECODE) .GT. 0 ) ) THEN
            ...
            IF ( LLT( VALUE, CHARLIST(PARMIN(1,NAMECODE)) ) ) THEN

*           Unless exclusive and value also < or = MAX
               IF ( EXCLUSIVE ) THEN
                  IF ( LGT( VALUE, CHARLIST(PARMAX(1,NAMECODE)) ) ) THEN
*                 Error in exclusive min/max
                     ...
                     CALL EMS_SETC ( 'VAL', VALUE )
                     CALL EMS_SETC
     :                ( 'L1',  CHARLIST(PARMIN(1,NAMECODE) ) )
                     CALL EMS_SETC
     :                ( 'L2',  CHARLIST(PARMAX(1,NAMECODE) ) )
     ...
     :               'for parameter ^NAME.', STATUS )
               ...
               ELSE
                  ...
                  CALL EMS_SETC ( 'NAME', PARKEY(NAMECODE) )
                  CALL EMS_SETC ( 'VAL', VALUE )
                  CALL EMS_SETC ( 'L1', CHARLIST(PARMIN(1,NAMECODE) ) )
...
*     If still OK, check against MAX value if any
         IF ( OK .AND. ( PARMAX(2,NAMECODE) .EQ. SUBPAR__CHAR )
     :   .AND. ( PARMAX(1,NAMECODE) .GT. 0 ) ) THEN
...
*                 Error in exclusive min/max
     ...
     :               'for parameter ^NAME.', STATUS )
               ...
               ELSE
                  ...
                  CALL EMS_SETC ( 'NAME', PARKEY(NAMECODE) )
                  CALL EMS_SETC ( 'VAL', VALUE )
                  CALL EMS_SETC ( 'L1', CHARLIST(PARMAX(1,NAMECODE) ) )
