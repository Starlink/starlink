      SUBROUTINE SUBPAR_DEF1D ( NAMECODE, NVAL, VALUES, STATUS )
      IMPLICIT NONE
...
*  Invocation:
*     CALL SUBPAR_DEF1D ( NAMECODE, NVAL, VALUES, STATUS )
...
*   are the same as previously.
     ...
     :      .AND. ((PARDYN(3,NAMECODE) .EQ. -SUBPAR__DOUBLE) .OR.
     :             (PARDYN(3,NAMECODE) .EQ. SUBPAR__DOUBLE)) )
     : THEN
...
*     we are allowed to overwrite an existing set default value
         ...
         PARDYN(3,NAMECODE) = SUBPAR__DOUBLE
         DO J = START, FINISH
            K = J - START + 1
            DOUBLELIST(J) = VALUES(K)
         ENDDO

      ELSE IF ( ( NVAL1 .LE. 6 )
     :    .AND. ( DOUBLEPTR + NVAL1 .LT. SUBPAR__MAXLIMS ) ) THEN
*     New space required and there is room
         START = DOUBLEPTR + 1
         FINISH = DOUBLEPTR + NVAL1
         DOUBLEPTR = FINISH
