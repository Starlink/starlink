*+  TASK_DECND - decode a character string as an array
      SUBROUTINE TASK_DECND ( STRING, NMAXDIMS, MAXDIMS, NDIMS, DIMS,
     :                          DVALS, STATUS )
*    Description :
...
*    Invocation :
*     CALL TASK_DECND ( STRING, NMAXDIMS, MAXDIMS, NDIMS, DIMS,
*                         DVALS, STATUS )
*    Parameters :
...
*     DVALS(1:*)=DOUBLE PRECISION (returned)
...
*    Export :
      ...
      DOUBLE PRECISION DVALS(1:*)  ! the decoded array, treated as a vector
...
*
     ...
     :                         CLENGTHS, STATUS )
         ...
         DO WHILE ( ( J .LT. COUNT ) .AND. ( STATUS .EQ. SAI__OK ) )
            J = J + 1
            CALL TASK_DEC0D ( CARRAY(J)(1:CLENGTHS(J)), DVALS(J),
     :      STATUS )
         ENDDO
      ENDIF
