      SUBROUTINE ARY1_CVTUB( BAD, N, TYPE, PNTR, RESULT, DCE, STATUS )
*+
*  Name:
*     ARY1_CVTUB
...
*  Invocation:
*     CALL ARY1_CVTUB( BAD, N, TYPE, PNTR, RESULT, DCE, STATUS )
...
*  conversion routine.
      IF ( TYPE .EQ. '_BYTE' ) THEN
         CALL VEC_BTOUB( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                   NERR, STATUS )
     ...
     :                    IERR, NERR, STATUS )
      ...
      ELSE IF ( TYPE .EQ. '_DOUBLE' ) THEN
         CALL VEC_DTOUB( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                   NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_INTEGER' ) THEN
         CALL VEC_ITOUB( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                   NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_REAL' ) THEN
         CALL VEC_RTOUB( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                   NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_WORD' ) THEN
         CALL VEC_WTOUB( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                   NERR, STATUS )
...
*  Report an error if the input data type specified was not valid.
      IF ( STATUS .EQ. SAI__OK ) THEN
         IF( .NOT. TYPOK ) THEN
            STATUS = ARY__FATIN
            CALL MSG_SETC( 'ROUTINE', 'ARY1_CVTUB' )
            CALL MSG_SETC( 'BADTYPE', TYPE )
            CALL ERR_REP( 'ARY1_CVTUB_TYPE',
...
*  Call error tracing routine and exit.
      IF ( STATUS .NE. SAI__OK ) CALL ARY1_TRACE( 'ARY1_CVTUB',
     :STATUS )
