      SUBROUTINE ARY1_CVTD( BAD, N, TYPE, PNTR, RESULT, DCE, STATUS )
*+
*  Name:
*     ARY1_CVTD
...
*  Invocation:
*     CALL ARY1_CVTD( BAD, N, TYPE, PNTR, RESULT, DCE, STATUS )
...
*  conversion routine.
      IF ( TYPE .EQ. '_BYTE' ) THEN
         CALL VEC_BTOD( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                  NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_UBYTE' ) THEN
         CALL VEC_UBTOD( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                   NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_DOUBLE' ) THEN
         CALL VEC_DTOD( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                  NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_INTEGER' ) THEN
         CALL VEC_ITOD( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                  NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_REAL' ) THEN
         CALL VEC_RTOD( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                  NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_WORD' ) THEN
         CALL VEC_WTOD( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                  NERR, STATUS )
 
      ELSE IF ( TYPE .EQ. '_UWORD' ) THEN
         CALL VEC_UWTOD( BAD, N, %VAL( CNF_PVAL( PNTR ) ), RESULT, IERR,
     :                   NERR, STATUS )
...
*  Report an error if the input data type specified was not valid.
      IF ( STATUS .EQ. SAI__OK ) THEN
         IF( .NOT. TYPOK ) THEN
            STATUS = ARY__FATIN
            CALL MSG_SETC( 'ROUTINE', 'ARY1_CVTD' )
            CALL MSG_SETC( 'BADTYPE', TYPE )
            CALL ERR_REP( 'ARY1_CVTD_TYPE',
...
*  Call error tracing routine and exit.
      IF ( STATUS .NE. SAI__OK ) CALL ARY1_TRACE( 'ARY1_CVTD',
     :STATUS )
