...
\sstroutine{
   ...
   Add axes to a Region so that it matches a given Frame
}{
   \sstdescription{
      This routine returns a new Region (normally a Prism) defined with
      the co-oridnate system specified by {\tt "}FRM{\tt "}. Any axes in FRM that are
      also present in the supplied region OLDREG inherit their coverage
      in the new region from OLDREG. Any axes in FRM that are not present
      in OLDREG are unbounded in the new region.
   }
   \sstinvocation{
      CALL ATL\_EXTREG( OLDREG, FRM, NEWREG, STATUS )
   }
   \sstarguments{
      \sstsubsection{
         IREG = INTEGER (Given)
      ...
      \sstsubsection{
         ...
         The Frame defining the axes required in the new Region.
      }
      \sstsubsection{
         NEWREG = INTEGER (Returned)
      }{
         The new Region. Returned equal to AST\_\_NULL if an error occurs.
      }
      \sstsubsection{
         STATUS = INTEGER (Given and Returned)
      }{
