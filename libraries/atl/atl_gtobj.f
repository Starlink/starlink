      SUBROUTINE ATL_GTOBJ( PARAM, CLASS, ISA, IAST, STATUS )
*+
*  Name:
*     ATL_GTOBJ
...
*  Invocation:
*     CALL ATL_GTOBJ( PARAM, CLASS, ISA, IAST, STATUS )
...
*  Report an error if we could not find an object of the correct class.
         IF( .NOT. OK .AND. STATUS .EQ. SAI__OK ) THEN
            IF( AST_ISAFRAMESET( IAST, STATUS ) ) THEN
               ...
               STATUS = SAI__ERROR
               CALL ERR_REP( 'ATL_GTOBJ_ERR1', '$^P contains a '//
     ...
     :                       '^L is required.', STATUS )
            ...
            ELSE
               ...
               CALL MSG_SETC( 'L', CLASS )
               STATUS = SAI__ERROR
               CALL ERR_REP( 'ATL_GTOBJ_ERR2', '$^P contains a ^C, '//
