*-----------------------------------------------------------------------
*+  IDSRCO - Save and recover common blocks

      SUBROUTINE IDSRCO ( DISPID, STATUS )

*    Description :
*     This saves the common blocks for the current device in a FORTRAN
*     direct access file IDI_USER:IDI_COMMS.DAT, and recovers the
*     common blocks for the device specified by the display identifier
...
*    Local variables :
      ...
      CHARACTER * ( DAT__SZNAM ) SCURRN, SGIVEN, SNAME
...
*   Save the current context if the device is open
...
*   In the next record write the characteristics, configuration,
...
*   The device names are held every 8th record
      ...
      DO WHILE ( .NOT. FOUND )
         I = I + 8
         READ( 1, REC = I, ERR= 20, IOSTAT = ISTAT ) SNAME
...
*   If the device is found then read in the common blocks
      IF ( FOUND ) THEN
         I = I + 1
