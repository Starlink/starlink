...
c+
\newcommand{\stardocnumber}    {211.10}
c-
f+
\newcommand{\stardocnumber}    {210.10}
f-
\newcommand{\stardocauthors}   {R.F. Warren-Smith \& D.S. Berry}
\newcommand{\stardocdate}      {11th September 2001}
...
routines 
f-
define the metric of the coordinate space which the Frame represents. In
...
passes at a given position angle through the starting position. For
SkyFrames, the position angle is the angle from north to the required
geodesic, with positive rotation in the same sense as rotation from north
to east. Any permutation of the axes in a SkyFrame is taken into
account when interpreting position angles. For simple Frames, axis
permutations are ignored, and the position angle is always the angle 
between the positive direction of the second Frame axis to the required
line, with positive angles being in the same sense as rotation from the 
...
If used with a 3-dimensional Frame the result is un-signed. Other
dimensionalities cause a value of AST\_\_BAD to be returned.

c+
The astBear function 
c-
f+
The AST\_BEAR routine 
f-
calculates the position angle of one point as seen from a second point
within a 2 dimensional Frame (an error is reported if the Frame is not
2 dimensional). Again, the algorithm used is appropriate to the class of
the Frame.
...
The following describes the most significant changes which have
occurred in the AST library between versions V1.7 and V1.8 (the
current version):
