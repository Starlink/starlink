...
c+
\newcommand{\stardocnumber}    {211.17}
c-
f+
\newcommand{\stardocnumber}    {210.17}
f-
\newcommand{\stardocauthors}   {R.F. Warren-Smith \& D.S. Berry}
\newcommand{\stardocdate}      {15th September 2004}
...
XML markup. Currently, the only form of markup provided by the XmlChan
class is closely related to the text format produced by the basic Channel 
class (currently, no schema or DTD is available describing this format). 
It is hoped to add more widely understood formats later. 
...
Note, AST requires the SLALIB library (\xref{SUN/67}{sun67}{}) to be 
installed.
...
does include a precision, then Digits will be used to determine the default 
precision.}. Any valid ``printf'' format specifier may be used so
...
Digits value\footnote{The exception to this rule is that if the Format value 
does include a precision, then Digits will be used to determine the default 
precision.}.  Unfortunately for Fortran programmers, this must be a C
...
using ``{\tt{.}}'' followed by an integer. If this is omitted, then a
default number of decimal places will be used which results in the total
number of digits being equal to the value of the Digits(axis) attribute.
So to supress all decimal places, it is necessary to include ``.0'' 
explicitly in the format specifier.
...
Currently, there is no DTD or schema defining the structure of data produced 
by an XmlChan. At the moment, such data is limited to be a fairly close
transliteration of the AST native format produced by the basic Channel
class. The following is a native AST representation of a simple 1-D Frame
(including comments and with the Full attribute set to zero so that some
default attribute values are included as extra comments):
...
unknown linear axes).}{Determined by the System attribute 
...
\fitskey{OBSGEO-X/Y/Z}{Used to set the GeoLon and
GeoLat attributes of the SpecFrame (the observers
...
The following describes the most significant changes which have
occurred in the AST library between versions V3.3 and V3.4 (the
current version):

\begin{enumerate}

\item A change has been made to the interpretation of Format strings which
specify how celestial axis values should be formatted for display. If
no precision specifier is included to specify the number of trailing
decimal places, then a default precision is now used which results in the
total number of digits being equal to the value of the Digits attribute.
Previously, zero decimal places were produced if the Format string
did not include a precision specifier. To get zero decimal places in the
new scheme, the Format value must specify explicitly that no decimal
places are required (\emph{e.g.} ``dms.0'' rather than ``dms'').
