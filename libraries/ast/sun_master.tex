...
c-
f+
routines AST_SETACTIVEUNIT and AST_GETACTIVEUNIT
f-
...
f+
In fact we need not have set the Unit attribute active in FRAME1
since the behaviour of AST_CONVERT is determined by its TO Frame
...
\subsubsection{\label{ss:unitsyntax}Side-effects of Changing the Unit attribute}
...
%\cleardoublepage
%\section{\label{ss:specframes}TBW - Spectral Coordinate Systems (SpecFrames)}
...
but if a different System value were set, the defaults would be
correspondingly different. For example, frequency is measured in 
units of Hertz, not $m/s$, so setting ``System=freq''
...
\begin{quote}
\begin{verbatim}
#   Uni1 = "Hz"         # Units for axis 1
\end{verbatim}
\end{quote}

Of course, if you do not like any of these defaults, you may always
over-ride them by setting explicit attribute values yourself. For
instance, you may choose to have your frequency axis expressed in ``GHz''
rather than ``Hz''. To do this simply set the attribute value as follows:

c+
\begin{quote}
\small
\begin{verbatim}
astSetC( specframe, "Unit", "GHz" );
\end{verbatim}
\normalsize
\end{quote}
c-
...
\small
\begin{verbatim}
      CALL AST_SETC( SPECFRAME, 'Unit', 'GHz', STATUS )
\end{verbatim}
\normalsize
f-
...
Some SpecFrame attributes (RefRA, RefDec and RestFreq) have no default
value. If no value is set for one of these attributes, an error will be
reported when a calculation is performed which needs a value for the
attribute. For RefRA and RefDec, this will be when converting between
different standards of rest, and for RestFreq it will be when converting
...
specframe = astSpecFrame( "System=Freq,Unit=GHz,StdOfRest=LSRK" );
...
\begin{verbatim}
      ...
      SPECFRAME = AST_SPECFRAME( 'System=Freq,Unit=GHz,StdOfRest=LSRK',
     :                           STATUS )
...
There is one potential problem with approach if you are interested in
...
c-
f+
AST_TRAN1 in place of AST_TRAN2
f-
...
been posible for foreign encodings. The WCSAXES keyword is used to store
