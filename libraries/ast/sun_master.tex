...
c+
\newcommand{\stardocnumber}    {211.12}
c-
f+
\newcommand{\stardocnumber}    {210.12}
f-
\newcommand{\stardocauthors}   {R.F. Warren-Smith \& D.S. Berry}
\newcommand{\stardocdate}      {8th October 2003}
\newcommand{\stardoctitle}     {AST\\
                                A Library for Handling\\
                                World Coordinate Systems\\
                                in Astronomy}
\newcommand{\stardoccopyright} {<COPYRIGHT_STATEMENT>}
...
\newcommand{\fitskey}[3]{{#1}&\parbox[t]{2in}{{#2}}&\parbox[t]{3in}{{#3}}\\}
...
Currently, AST supports five foreign encodings, which may be selected
...
\htmladdnormallink{http://fits.gsfc.nasa.gov/fits_wcs.html}
...
When reading a \htmlref{FrameSet}{FrameSet} from a \htmlref{FitsChan}
{FitsChan}, these conventions are used if the CTYPE\emph{i} keyword
...
The following table describes the use made by AST of each keyword
...
\fitskey{CUNIT\emph{ia}}{Used to set the \htmlref{Units}{Units} attributes
...
When reading a \htmlref{FrameSet}{FrameSet} from a \htmlref{FitsChan}
{FitsChan} the CTYPE\emph{i} values in the FitsChan are used to set the
\htmlref{Symbol}{Symbol} attributes of the corresponding WCS \htmlref{Frame}
{Frame}. The \htmlref{Label}{Label} attributes of the WCS Frame are set from 
...
each complete set of axis descriptions results in a single \htmlref{Frame} 
{Frame} being added to the final FrameSet, connected via an appropriate 
\htmlref{Mapping} {Mapping} to the base pixel Frame. The \htmlref{Ident}
{Ident} attribute of the Frame is set to hold the single alphabetical 
...
unknown linear axes).}{Determined by the \htmlref{System}{System} attribute 
...
stored as a \htmlref{PV}{PVi_m} attribute for the longitude axis of the WcsMap.}{Only
...
written if using FITS-AIPS encoding. Determined by the Equinox attribute
of the SkyFrame.}
...
azimuthal, then any values set for attributes ``\htmlref{PV\emph{i}\_3}{PVi_m}'' 
...
if supplied, are stored as the corresponding \htmlref{PV}{PVi_m} attributes 
of the WcsMap.
...
keywords, and corresponding \htmlref{PV}{PVi_m} attributes are set in the 
...
\fitskey{CUNIT\emph{ia}}{Used to set the \htmlref{Units}{Units} attribute of 
...
\fitskey{OBSGEO-X/Y/Z}{Used to set the \htmlref{GeoLon}{GeoLon} and
\htmlref{GeoLat}{GeoLat} attributes of the SpecFrame (the observers
height above sea level is ignored).}{Set from the GeoLon and GeoLat 
attributes of the SpecFrame, if they have been set explicitly (it is
...
\fitskey{VSOURCE\emph{a}}{Used to set the \htmlref{SourceVel}{SourceVel}
attribute of the SpecFrame (the \htmlref{SourceVRF}{SourceVRF} attribute
is first set to ``Topocentric'').}{Set from the SourceVel attribute of
the SpecFrame, if it has been set explicitly (the SourceVRF
attribute is set to ``Topocentric'' before getting the value of
SourceVel).}

\fitskey{ZSOURCE\emph{a}}{Used to set the \htmlref{SourceVel}{SourceVel}
attribute of the SpecFrame (the SourceVRF attribute
is first set to ``Topocentric'' and the ZSOURCE value is converted to a
topocentric apparent radial velocity).}{Never written (VSOURCE is written
instead).}

\fitskey{VELOSYS\emph{a}}{Ignored.}{Never written.}
...
SpecFrame, but only if the \htmlref{System}{System} attribute is not set to 
``WAVE'', ``VOPT'', ``ZOPT'' or ``AWAV'', and only if RestFreq has been set 
explicitly.}

\fitskey{RESTWAV\emph{a}}{Used to set the \htmlref{RestFreq}{RestFreq}
...
\htmlref{System}{System} attribute is set to ``WAVE'', ``VOPT'', ``ZOPT'' or 
``AWAV'', and only if RestFreq has been set explicitly.}

\fitskey{CNAME\emph{ia}}{Used to set the \htmlref{Label}{Label} attributes of 
...
\subsubsection{Requirements for a Successful Write Operation}
When writing a \htmlref{FrameSet}{FrameSet} in which the WCS
\htmlref{Frame}{Frame} is a \htmlref{SpecFrame}{SpecFrame} to a 
...
\item It contains a \htmlref{GrismMap}{GrismMap}, and the Mapping before the 
GrismMap (from
