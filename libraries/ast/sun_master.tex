...
tabok
\item The TabOK attribute is set non-zero in the FitsChan, and the
longitude and latitude axes are separable. In this case the Mapping will
...
\item The TabOK attribute is set non-zero in the FitsChan.
\end{enumerate}

If none of the above conditions hold, the write operation will be
unsuccessful. Note, if the FitsChan's TabOK attribute is set non-zero
then any Mapping that does not meet any of the earlier conditions will be
written out as a look-up table, using the ``-TAB'' algorithm described in
FITS-WCS paper III. If the TabOK attribute is to zero (the default) in
the FitsChan, then the write operation will be unsuccessful unless one of
the eaerlier conditions is met.
...
The following describes the most significant changes which have
occurred in the AST library between versions V5.3-2 and V5.4-0 (the
current version):
