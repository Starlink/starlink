...
\newcommand{\stardocdate}      {21st September 2009}
...
The following describes the most significant changes which have
occurred in the AST library between versions V5.2 and V5.3 (the
current version):
...
f+
AST\_FINDFRAME and AST\_CONVERT
functions will now succesfully match a Frame of a specialised class with
a Frame of its parent class, returning a UnitMap between them. All the
other restrictions on matching are unchanged (e.g. the number of axes 
must match, the Domain names must match, \emph{etc}).
