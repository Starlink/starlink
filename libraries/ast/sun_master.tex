...
f+
Note, this splitting of long strings over several cards only occurs when
writing AST Objects to a FitsChan using the AST_WRITE routine and the
{\em native} encoding. If a long string is stored in a FitsChan using
(for instance) the AST_PUTFITS routine, it will simply be truncated.
f- 
...
AST_WRITE. The AST_READ routine reverses this process by concatenating
...
FITS-IRAF, FITS-WCS, FITS-AIPS and FITS-PC encodings since they share
many keywords.}.  Adopting this approach would simply require multiple
calls to AST_WRITE, rewinding the FitsChan and changing its Encoding value
before each one.
f-
...
are represented by {tt PVi\_m} keywords instead of the {\rr PROJPm}
keywords.
...
now depracated in favour of the new PVj_m attributes. They are interpreted 
