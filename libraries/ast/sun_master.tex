...
PermMap in \secref{ss:permmapexample} and the UnitMap in
...
comprehensible. Using the FrameSet pointer ``wcsinfo'' obtained in
\secref{ss:howtoreadwcs} and a pair of world coordonates ``xw'' and
``yw'' ({\em{e.g.}}\ see \secref{ss:howtotransform}), you could
proceed as follows:
c-
f+
Once you have converted pixel coordinates into world coordinates, you
...
f+
For more information on reading coordinate values using the
AST\_UNFORMAT routine, see \secref{ss:unformattingaxisvalues}. For
details of how sexagesimal formats are handled, and the forms of input
...
\begin{quote}
\begin{description}
\item[Naxes]\begin{latexonly}\mbox{}\\ \end{latexonly}
A read-only integer giving the number of Frame axes.

\item[Title]\begin{latexonly}\mbox{}\\ \end{latexonly}
A string describing the coordinate system which the Frame represents.

\item[Label(axis)]\begin{latexonly}\mbox{}\\ \end{latexonly}
A label string for each axis.

\item[Unit(axis)]\begin{latexonly}\mbox{}\\ \end{latexonly}
A string describing the physical units on each axis.

\item[Symbol(axis)]\begin{latexonly}\mbox{}\\ \end{latexonly}
...
\item[Format(axis)]\begin{latexonly}\mbox{}\\ \end{latexonly}
...
\item[Direction(axis)]\begin{latexonly}\mbox{}\\ \end{latexonly}
...
\item[Domain]\begin{latexonly}\mbox{}\\ \end{latexonly}
...
\begin{quote}
\begin{description}
\item[Epoch]\begin{latexonly}\mbox{}\\ \end{latexonly}
...
\item[\label{ss:equinoxitem}Equinox]\begin{latexonly}\mbox{}\\ \end{latexonly}
...
\begin{quote}
\begin{description}
\item[Class]\begin{latexonly}\mbox{}\\ \end{latexonly}
Has the value ``FrameSet''.
\item[ID]\begin{latexonly}\mbox{}\\ \end{latexonly}
Identifies the particular FrameSet (not its current Frame).
\item[Nin]\begin{latexonly}\mbox{}\\ \end{latexonly}
Equals the number of axes in the FrameSet's base Frame.
\item[Invert]\begin{latexonly}\mbox{}\\ \end{latexonly}
Is independent of any of the Objects within the FrameSet.
\item[Nobject]\begin{latexonly}\mbox{}\\ \end{latexonly}
Counts the number of active FrameSets.
\item[RefCount]\begin{latexonly}\mbox{}\\ \end{latexonly}
...
\begin{verbatim}
/* 1: */
astSet( fitschan, "Card=1, Encoding=FITS-WCS" );
...
approach would simply require multiple calls to astWrite, rewinding
...
astTranP function provided by AST ({\em{q.v.}}), except that the first
parameter (the Mapping pointer) is not required and should be
...
AST\_TRANN function provided by AST ({\em{q.v.}}), except that the
first argument (the Mapping pointer) is not required and should be
...
for any other Mapping (\secref{ss:settingattributes}).
c-
f+
...
same way as for any other Mapping (\secref{ss:settingattributes}).
f-
...
astMark( plot, NMARK, NCOORD, NCOORD, in, type );
\end{verbatim}
\normalsize
\end{quote}
c-
...
these grid lines will not be geodesics, so astCurve
...
and grid lines to represent coordinate systems requires a slightly
different approach. The problem is that for some coordinate systems,
these grid lines will not be geodesics, so AST\_CURVE
(\secref{ss:plottinggeodesics}) cannot easily be used (you would have
to resort to approximating grid lines by many small elements). Lines
of constant celestial latitude provide an example of this, with the
exception of the equator which is a geodesic.
...
c+
It returns boolean (integer) value to indicate if any invalid or
...
\small
\begin{verbatim}
alink adamprog.o `ast_link_adam`
\end{verbatim}
\normalsize
\end{quote}
c-
...
c+
Note the use of backward quote characters and the pre-compilation of
the C main routine to produce the ``adamprog.o'' file which is
required when ``alink'' is used with C.
c-
f+
