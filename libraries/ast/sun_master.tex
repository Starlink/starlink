...
\cleardoublepage
\section{\label{ss:specframes}Spectral Coordinate Systems (SpecFrames)}
...
A CmpFrame has no extra attributes over and above those common to all
Frames. The default value for its Domain attribute is ``CMP'' and the
(fixed) value for its System attribute is ``Compound''. 
