...
\newcommand{\stardoctitle}     {AST\\
                                ...
                                in Astronomy}
\newcommand{\stardoccopyright} {Copyright (C) 2001 Central Laboratory of the Research Councils}
...
\newcommand{\fitsurl}[0]{http://www.gsfc.nasa.gov/fits_home.html}
...
\sstroutine{
   ...
   \sstexamples{
      ...
      \sstexamplesubsection{
         RESULT = AST\_FINDFRAME( TARGET, AST\_FRAME( 1, '\htmlref{MaxAxes}{MaxAxes}=3', STATUS ), ' ', STATUS )
      }{
...
\sstroutine{
   AST\_GRFFUN\sstlabel{AST_GRFFUN}
}{
   Register a graphics routine for use by the Plot class
}{
   \sstdescription{
      This routine can be used to select the underlying graphics
      routines to be used when the \htmlref{Plot}{Plot} class produces graphical output.
   ...
   \sstinvocation{
      CALL AST\_GRFFUN( THIS, NAME, FUN, STATUS )
   }
   \sstarguments{
      \sstsubsection{
         THIS = INTEGER (Given)
      ...
      \sstsubsection{
         ...
         Eight graphics routines are used in total by the
         ...
         eight graphics routines are not replaced in this way, the
      ...
      \sstsubsection{
         ...
         routine which invokes AST\_GRFFUN).

         Once a routine has been provided, the {\tt{"}}null{\tt{"}} routine AST\_NULL can
         be supplied in a subsequent call to astGrfFun to reset the routine
   ...
   \sstdiytopic{
      ...
      The {\tt{"}}AxScale{\tt{"}} function returns two values (one for each axis) which
      ...
      \sstitemlist{
         ...
         \sstitem
         ALPHA = REAL (Returned) - Returned holding the scale for the
            X axis (i.e. Xnorm = alpha$*$Xworld).

         \sstitem
         BETA = REAL (Returned) - Returned holding the scale for the
...
\sstroutine{
   ...
   \sstdescription{
      ...
      Each WcsMap also allows up to 99 {\tt{"}}projection parameters{\tt{"}} to be
      associated with each axis. These specify the precise form of the
      projection, and are accessed using \htmlref{PVj\_m}{PVj_m} attribute, where {\tt{"}}j{\tt{"}} is
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         here via the PVj\_m attribute (see the {\tt{"}}Examples{\tt{"}}
   ...
   \sstexamples{
      ...
      \sstexamplesubsection{
         WCSMAP = AST\_WCSMAP( AST\_\_COE, 3, 2, 3, 'PV3\_1=40.0', STATUS )
      }{
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \sstitem
         The validity of any projection parameters given via the PVj\_m
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         the {\tt{"}}FITS-WCS{\tt{"}} encoding (which uses CDi\_j keywords to describe the
         scale and rotation) should be used in preference to {\tt{"}}FITS-PC{\tt{"}}.
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         Otherwise, if the FitsChan contains a keyword of the form
         CDELTi, where {\tt{"}}i{\tt{"}} is a single digit, then FITS-PC encoding is
         used.
   ...
   \sstdiytopic{
      ...
      coordinate systems will include a {\tt{"}}sky projection{\tt{"}} (e.g. as
...
\sstroutine{
   ...
   Use Grf routines registered through AST\_GRFFUN?
}{
   \sstdescription{
      ...
      Otherwise, routines registered using \htmlref{AST\_GRFFUN}{AST_GRFFUN} are used. In this
...
\sstroutine{
   ...
   \sstdescription{
      ...
      \htmlref{PVj\_m}{PVj_m} attribute.
   }
   \sstattributetype{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      conditions when an \htmlref{Object}{Object} is read from or written to a \htmlref{FitsChan}{FitsChan}.
      The value supplied for the Warnings attribute should consist of a
      space separated list of condition names (see the \htmlref{AllWarnings}{AllWarnings}
      attribute for a list of the currently defined names). Each name
      indicates a condition which should be reported. The default
      value for Warnings is the string {\tt{"}}Tnx Zpx BadCel{\tt{"}}.
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         PVj\_m: FITS-WCS projection parameters
