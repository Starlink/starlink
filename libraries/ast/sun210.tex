...
\sstroutine{
   AST\_INTERPOLATE\sstlabel{AST_INTERPOLATE}
}{
   ...
   \sstdescription{
      ...
      give it any name you wish). Such a function may be
      passed as the UINTERP argument to the \htmlref{AST\_RESAMPLE$<$X$>$}{AST_RESAMPLEX} functions
   ...
   \sstinvocation{
      RESULT = AST\_INTERPOLATE( NDIM\_IN, LBND\_IN, UBND\_IN,
                                IN, IN\_VAR, NPOINT, OFFSET, COORDS, PARAMS,
                                FLAGS, BADVAL, OUT, OUT\_VAR, STATUS )
   }
   \sstarguments{
      \sstsubsection{
      ...
      \sstsubsection{
         ...
         The array given for the PARAMS argument of
         AST\_RESAMPLE$<$X$>$. You may use this to pass any
      ...
      \sstsubsection{
         ...
         The value given for the FLAGS argument of
         AST\_RESAMPLE$<$X$>$. You may test this value in order to provide
      ...
      \sstsubsection{
         ...
         The value given for the BADVAL argument of AST\_RESAMPLE$<$X$>$.
         This will have the same numerical type as the data being
         processed (i.e. as elements of the IN array).  It should be
         used to test for bad pixels in the input grid (but only if
         the AST\_\_USEBAD flag is set via the FLAGS argument) and for
         identifying bad output values in the OUT (and OUT\_VAR)
         array(s).
      }
      \sstsubsection{
         OUT( $*$ ) = $<$Xtype$>$ (Returned)
   ...
   \sstreturnedvalue{
      \sstsubsection{
         AST\_INTERPOLATE = INTEGER
      }{
         The number of interpolation points at which an output data
         value (and/or a variance value if relevant) equal to BADVAL
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \sstitem
         If an error occurs within this function, it should set the
         STATUS argument to an error value and return a function result
         of zero. This action will cause an immediate return from
         AST\_RESAMPLE$<$X$>$.
      }
   }
}
...
\sstroutine{
   ...
   \sstdescription{
      ...
      implements your own (see \htmlref{AST\_INTERPOLATE}{AST_INTERPOLATE} as an example).
   ...
   \sstinvocation{
      RESULT = AST\_RESAMPLE$<$X$>$( THIS, NDIM\_IN, LBND\_IN, UBND\_IN,
                                IN, IN\_VAR, INTERP, UINTERP, PARAMS,
                                TOL, MAXPIX, FLAGS, BADVAL,
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         (I'th) dimension is UBND\_IN(I)-LBND\_IN(I)$+$1. They also define
      ...
      \sstsubsection{
         ...
         should be REAL, while if using AST\_RESAMPLES, the type
         should be INTEGER$*$2).
      ...
      \sstsubsection{
         ...
         This argument specifies the algorithm to be used for
         ...
         Schemes{\tt{"}} section below.  If a value of zero is supplied, then
         the default linear interpolation scheme is used (equivalent
         to supplying the value AST\_\_LINEAR).

         Alternatively, you may supply the value AST\_\_UINTERP, which
         indicates that you will provide your own function to perform
         sub-pixel interpolation by means of the UINTERP argument.
      }
      \sstsubsection{
         UINTERP = FUNCTION (Given)
      }{
         If the value AST\_\_UINTERP is given for the INTERP argument,
         then this argument should be used to supply a function which
         implements your own sub-pixel interpolation algorithm.  This
         should have the same interface as the dummy function
         AST\_INTERPOLATE (q.v.), whose description is provided as a
         template, and should be designed to process data with the
         same numerical type as the IN array. It must also be declared
         in a Fortran EXTERNAL statement in the routine which invokes
         AST\_RESAMPLE$<$X$>$.

         If the INTERP argument has any other value, corresponding to
         a pre-defined sub-pixel interpolation algorithm, then you
         should supply the dummy routine AST\_NULL here (note only one
         underscore).  No EXTERNAL statment is required for this
         routine, so long as the AST\_PAR include file has been used.
      }
      \sstsubsection{
         PARAMS( $*$ ) = DOUBLE PRECISION (Given)
      }{
         ...
         section below (you may also use this argument to pass values
      ...
      \sstsubsection{
         ...
         The sum of a set of flag values which provide
      ...
      \sstsubsection{
         ...
         LBND\_OUT and UBND\_OUT arrays). Regions of the output grid
   ...
   \sstdiytopic{
      ...
      PRECISION data, while AST\_RESAMPLER would be used to process
      REAL data, etc.

      For compatibility with other Starlink facilities, the codes W
      and UW are also provided as synonyms for S and US respectively
      (but only in the Fortran interface to AST).
   }
   \sstdiytopic{
      Sub-Pixel Interpolation Schemes
   }{
      The following values are defined in the AST\_PAR include file and
      may be supplied via the INTERP argument in order to select a
      ...
      If none of the above values is given, INTERP should be set to
      AST\_\_UINTERP and a function should be supplied via the UINTERP
      argument to implement the required sub-pixel interpolation
      algorithm.
   }
   \sstdiytopic{
      Control Flags
      ...
      selected a set of flags, you should supply the sum of
      their values via the FLAGS argument:
   ...
   \sstdiytopic{
      ...
      calculated) may be declated bad and flagged with the BADVAL
...
\sstroutine{
   ...
   \sstdescription{
      ...
      does not result in simple straight lines. The basic plotting
      element is therefore not a straight line, but a geodesic curve
      (see \htmlref{AST\_CURVE}{AST_CURVE} and \htmlref{AST\_POLYCURVE}{AST_POLYCURVE}). A Plot also provides facilities
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         AST\_POLYCURVE: Draw a series of connected geodesic curves
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         Curves: Geodesic curves drawn using AST\_CURVE or AST\_POLYCURVE
...
A new set of functions, \htmlref{AST\_RESAMPLE$<$X$>$}{AST_RESAMPLEX}, has been introduced to
perform resampling of gridded data such as images
