...
\newcommand{\stardocnumber}    {210.2}
\newcommand{\stardocauthors}   {R.F. Warren-Smith \& D.S. Berry}
\newcommand{\stardocdate}      {2nd June 1998}
...
This programmer's manual should be of interest to anyone writing astronomical
applications which need to manipulate coordinate system data, especially
celestial coordinate systems. AST is portable and environment-independent.
}
...
PermMap in \secref{ss:permmapexample} and the \htmlref{UnitMap}{UnitMap} in
\secref{ss:unitmapexample}. The ZoomMap is used as an example
throughout \secref{ss:primer}.
...
\subsection{\ldots Build an AST Program}
...
Precisely how you extract World Coordinate \htmlref{System}{System} (WCS) information
...
(Some datasets which do not easily yield FITS headers may require a
different approach. In the case of the Starlink NDF data format, for
example, the above recipe should be replaced by a single call to the
routine \xref{NDF\_GTWCS}{sun33}{NDF_GTWCS}---see
\xref{SUN/33}{sun33}{}).
...
Once you have converted pixel coordinates into world coordinates, you
may want to format them as text before displaying them. Typically,
this would convert from (say) radians into something more
comprehensible. Using the \htmlref{FrameSet}{FrameSet} pointer WCSINFO obtained in
\secref{ss:howtoreadwcs} and a pair of world coordonates XW and YW
({\em{e.g.}}\ see \secref{ss:howtotransform}), you could proceed as
follows:
...
\begin{verbatim}
      ...
      WRITE ( *, 99 ) XTEXT, YTEXT
 99   FORMAT( 'Position = ', A, ', ', A )
\end{verbatim}
\normalsize
...
For a complete discription of the Report attribute, see its entry in
...
\htmlref{AST\_UNFORMAT}{AST_UNFORMAT} routine, see \secref{ss:unformattingaxisvalues}. For
...
``pixel coordinates'' whenever this distinction is not important. This
should not be taken to imply that the GRID convention for numbering
pixels is excluded---in fact, it is usually to be preferred (at the
level of data handling being discussed in this document) and we
recommend it.
...
\small
\begin{verbatim}
      CALL AST_CONVERT( FRAMESETA, FRAMESETB, 'SKY,PIXEL,GRID', STATUS )
      IF ( CVT .EQ. AST__NULL ) THEN
...
Currently, AST supports two foreign encodings, which may be selected
...
This encoding is particularly important because it is based on a
...
substantial changes to the FITS-WCS proposal has come as something of
a surprise to many onlookers, given that the paper by Greisen \&
Calabretta has changed very little for several years. The present
uncertain situation is clearly not satisfactory and we intend to
provide an interim solution in a forthcoming version of AST. Most
probably, this will take the form of support for additional unofficial
(but stable) encodings based on the original FITS standard.}

The main value of this encoding at present is that the WCS convention
incorporated in the original (1981) FITS standard forms a stable
subset of the new proposals. The FITS-WCS encoding is therefore able
to read information stored using this original convention, which has
been quite widely used despite its limitations.
\end{description}
\end{quote}
...
\subsection{Propagating WCS Information through Data Processing Steps}
...
Unfortunately, at present this is a slightly difficult question to
answer. Ideally, you would probably want to try a sequence of
alternative encodings, using an approach such as the following:
...
However, since the FITS-WCS encoding is not yet based on a stable FITS
standard (\secref{ss:foreignencodings}), attempting to write using
this encoding cannot currently be recommended, in case the final
standard differs significantly from the current draft.  Therefore, the
only sensible option at present is probably to omit step (A) above and
to attempt only steps (B) and (C).
...
will be able to read it. This approach is possible because currently
there is no conflict between the FITS keywords used by the different
encodings which AST supports, although conflicts between new encodings
introduced in future obviously cannot be ruled out.  Adopting this
approach would simply require multiple calls to AST\_WRITE, rewinding
the FitsChan and changing its Encoding value before each one.
...
\htmlref{AST\_TRANN}{AST_TRANN} function provided by AST ({\em{q.v.}}), except that the
first argument (the \htmlref{Mapping}{Mapping} pointer) is not required and should be
omitted. Here is a simple example of a suitable transformation routine
which transforms coordinates by squaring them:
\xlabel{SqrTran}

\small
\begin{verbatim}
      SUBROUTINE SQRTRAN( NPOINT, NCOORD_IN, INDIM, IN, FORWARD,
    :                     NCOORD_OUT, OUTDIM, OUT, STATUS )
      INTEGER NPOINT, NCOORD_IN, INDIM, NCOORD_OUT, OUTDIM, STATUS
...
the same transformation routine with several IntraMaps, since it will
...
AST to the error, causing it to abort the current operation.
...
\begin{verbatim}
      ...
      CONTACT = 'http://star-www.rl.ac.uk/cgi-bin/htxserver/' //
...
same way as for any other \htmlref{Mapping}{Mapping} (\secref{ss:settingattributes}).
...
\small
\begin{verbatim}
      SUBROUTINE POLY3TRAN( NPOINT, NCOORD_IN, INDIM, IN, FORWARD,
     :                      NCOORD_OUT, OUTDIM, OUT, STATUS
      INTEGER NPOINT, NCOORD_IN, INDIM, NCOORD_OUT, OUTDIM, STATUS,
      DOUBLE PRECISION IN( INDIM, NCOORD_IN ), OUT( OUTDIM, NCOORD_OUT )
      LOGICAL FORWARD
...
\small
\begin{verbatim}
      SUBROUTINE MAXTRAN( NPOINT, NCOORD_IN, INDIM, IN, FORWARD,
     :                     NCOORD_OUT, OUTDIM, OUT, STATUS )
      INTEGER NPOINT, NCOORD_IN, INDIM, NCOORD_OUT, OUTDIM, STATUS
      DOUBLE PRECISION IN( INDIM, NCOORD_IN ), OUT( OUTDIM, NCOORD_OUT )
      LOGICAL FORWARD
...
these grid lines will not be geodesics, so \htmlref{AST\_CURVE}{AST_CURVE}
...
using the ``$-$pgplot'' switch with ``ast\_link'', as follows:
...
\sstroutine{
   ...
   \sstdescription{
      ...
      This routine may also be used to merge two FrameSets.
   }
   \sstinvocation{
...
\sstroutine{
   ...
   \sstreturnedvalue{
      \sstsubsection{
         ...
         .FALSE. is returned if the plotting area is completely filled by
...
\sstroutine{
   ...
   \sstexamples{
      ...
      \sstexamplesubsection{
         CVT = AST\_CONVERT(\htmlref{AST\_SKYFRAME}{AST_SKYFRAME}(' '),AST\_SKYFRAME('\htmlref{Equinox}{Equinox}=2005'),' ',STATUS)
      }{
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         insensitive). However, this template may also contain {\tt{"}}field
         specifiers{\tt{"}} which are capable of matching a range of
         characters (see the {\tt{"}}Keyword Templates{\tt{"}} section for
...
\sstroutine{
   ...
   \sstexamples{
      \sstexamplesubsection{
         RESULT = AST\_FINDFRAME( TARGET, \htmlref{AST\_FRAME}{AST_FRAME}( 3, ' ' ), ' ', STATUS )
      }{
      ...
      \sstexamplesubsection{
         RESULT = AST\_FINDFRAME( TARGET, \htmlref{AST\_SKYFRAME}{AST_SKYFRAME}( ' ' ), ' ', STATUS )
      }{
      ...
      \sstexamplesubsection{
         RESULT = AST\_FINDFRAME( TARGET, AST\_SKYFRAME( '\htmlref{System}{System}=FK5' ), ' ', STATUS )
      }{
      ...
      \sstexamplesubsection{
         RESULT = AST\_FINDFRAME( TARGET, AST\_FRAME( 2, ' ' ), 'SKY,PIXEL,', STATUS )
      }{
      ...
      \sstexamplesubsection{
         RESULT = AST\_FINDFRAME( TARGET, AST\_FRAME( 1, 'Domain=WAVELENGTH' ),' ',STATUS)
      }{
         Searches for any 1-dimensional coordinate system in the
         target which has the domain {\tt{"}}WAVELENGTH{\tt{"}}.
      }
      \sstexamplesubsection{
         RESULT = AST\_FINDFRAME( TARGET, AST\_FRAME( 1, ' ' ), 'WAVELENGTH', STATUS )
      }{
      ...
      \sstexamplesubsection{
         RESULT = AST\_FINDFRAME( TARGET, AST\_FRAME( 1, '\htmlref{MaxAxes}{MaxAxes}=3' ), ' ', STATUS )
      }{
      ...
      \sstexamplesubsection{
         RESULT = AST\_FINDFRAME( TARGET, AST\_SKYFRAME( '\htmlref{Permute}{Permute}=0' ), ' ', STATUS )
      }{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      cards may be inserted (\htmlref{AST\_PUTFITS}{AST_PUTFITS}) and existing ones may be
      deleted (\htmlref{AST\_DELFITS}{AST_DELFITS}).
      ...
      data into it (e.g. using AST\_PUTFITS or AST\_WRITE). If you
   ...
   \sstarguments{
      \sstsubsection{
         ...
         stored in it (e.g. using AST\_PUTFITS).
      }
      \sstsubsection{
         SINK = SUBROUTINE (Given)
      }{
...
\sstroutine{
   ...
   \sstdescription{
      This routine draws a complete annotated set of 2-dimensional
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         required and should have an interface like \htmlref{AST\_TRANN}{AST_TRANN}, except
         that the first argument (the Mapping pointer) is not required
...
\sstroutine{
   ...
   \sstinvocation{
      RESULT = AST\_PERMAP( NIN, INPERM, NOUT, OUTPERM, CONSTANT, OPTIONS,
                           STATUS )
   }
   \sstarguments{
      \sstsubsection{
...
\sstroutine{
   ...
   \sstexamples{
      ...
      \sstexamplesubsection{
         FRAME = AST\_SKYFRAME( 'Equinox = J2005, Digits = 10', STATUS )
      }{
         Creates a SkyFrame to describe the default FK5 celestial
...
\sstroutine{
   ...
   \sstexamples{
      \sstexamplesubsection{
         WCSMAP = AST\_WCSMAP( AST\_\_MER, 2, 1, 2, ' ', STATUS )
      }{
      ...
      \sstexamplesubsection{
         WCSMAP = AST\_WCSMAP( AST\_\_COE, 3, 2, 3, 'ProjP(1)=40.0', STATUS )
      }{
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         FITS data in future. For further details, see the section {\tt{"}}The
         FITS-WCS Encoding{\tt{"}} below.
   ...
   \sstdiytopic{
      ...
      from a FitsChan (using AST\_READ) where it was already stored
      using the DSS encoding scheme.  This is because any intervening
      modification to the FrameSet (such as re-mapping its Frames -
      see \htmlref{AST\_REMAPFRAME}{AST_REMAPFRAME}) is likely to cause it to depart from the very
...
\sstroutine{
   ...
   \sstdescription{
      ...
      element of graphical output produced by a \htmlref{Plot}{Plot}, and is given in
      inches. It takes a separate value for each graphical element so
      that, for instance, the setting {\tt{"}}Width(border)=0.1{\tt{"}} causes the
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         AST\_DELFITS: Delete the current FITS card in a FitsChan

         \sstitem
         AST\_FINDFITS: Find a FITS card in a FitsChan by keyword

         \sstitem
         AST\_PUTFITS: Store a FITS header card in a FitsChan
      }
   }
}
\sstroutine{
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         AST\_CLIP: Set up or remove clipping for a Plot

         \sstitem
         AST\_CURVE: Draw a geodesic curve

         \sstitem
         AST\_GRID: Draw a set of labelled coordinate axes

         \sstitem
         AST\_GRIDLINE: Draw a grid line (or axis) for a Plot

         \sstitem
         AST\_MARK: Draw a set of markers for a Plot

         \sstitem
         AST\_TEXT: Draw a text string for a Plot
      }
   }
   \sstdiytopic{
      Graphical Elements
      ...
      \sstitemlist{
         ...
         \sstitem
         Curves: Geodesic curves drawn using AST\_CURVE
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         AST\_SLAADD: Add a celestial coordinate conversion to an SlaMap
      }
   }
}
\sstroutine{
...
\sstroutine{
   ...
   \sstexamples{
      ...
      \sstexamplesubsection{
         f77 import.f -L/star/lib `ast\_link -csla` -o import
      }{
         Compiles and links a Fortran program called ``import'' which does
         not produce any graphical output. The ``-csla'' flag indicates that
         the C version of SLALIB should be used by AST (this should be avoided
         if the main program already calls the Fortran version).
      }
      \sstexamplesubsection{
         f77 plotit.f -L. -L/star/lib `ast\_link -mygrf` -lgrf\_my -o plotit
      }{
         Compiles and links a Fortran program ``plotit''. The ``-mygrf''
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         ``-pgplot'' (or ``-pgp''): Requests that the program be linked so that
         graphical output from the AST library is displayed via the PGPLOT
         graphics package. By default, no graphics package is linked and this
         will result in an error at run time if AST routines are invoked that
...
\sstroutine{
   ...
   \sstexamples{
      \sstexamplesubsection{
         ...
         uses PGPLOT for graphical output.
      }
      \sstexamplesubsection{
         alink import.f -L/star/lib `ast\_link\_adam -csla`
      }{
         Compiles and links an ADAM Fortran program called ``import'' which
         does not produce any graphical output. The ``-csla'' flag indicates
         that the C version of SLALIB should be used by AST (this should be
         avoided if the main program already calls the Fortran version).
      }
      \sstexamplesubsection{
         alink plotit.f -L. -L/star/lib `ast\_link\_adam -mygrf` -lgrf\_my
      }{
         Compiles and links an ADAM Fortran program ``plotit''. The ``-mygrf''
         switch indicates that graphical output will be delivered through
         a graphical interface which you have implemented yourself. Here, this
         interface is supplied by means of the ``-lgrf\_my'' library reference.
      }
...
\subsection{\xlabel{list_of_most_recent_changes}Changes Introduced in V1.1}

The following describes the most significant changes which have
occurred in the AST library between versions V1.0 and V1.1 (the
current version):
...
a compound \htmlref{Mapping}{Mapping} which is being simplified using \htmlref{AST\_SIMPLIFY}{AST_SIMPLIFY}.

\item The internal structure of a \htmlref{FrameSet}{FrameSet} is now automatically tidied
...
current \htmlref{Frame}{Frame} (either by setting or clearing their values). This is
...
Applications which are statically linked will need to be re-linked in
