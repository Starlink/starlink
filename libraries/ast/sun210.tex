...
\newcommand{\stardocdate}      {18th December 1998}
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      kernel is usually regarded as {\tt{"}}separable{\tt{"}} and formed from the
      ...
      PARAMS(1) should be used to specify how many pixels are to
      ...
      In these cases, the FINTERP argument is not used:

      \sstitemlist{

         \sstitem
         AST\_\_SINC: This scheme uses the sinc(x) kernel, which
         sometimes features as an {\tt{"}}optimal{\tt{"}} interpolation kernel in books
         ...
         \sstitem
         AST\_\_SINCSINC: This scheme uses a rather better kernel, of the
         form sinc(x).sinc(x/2) and uses only two neighbouring pixels on
         each side of the interpolation point (the PARAMS array is
