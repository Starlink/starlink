...
\newcommand{\stardocdate}      {11th June 1998}
...
\secref{ss:howtoreadwcs} and a pair of world coordinates XW and YW
...
\subsection{\ldots Modify a WCS Calibration}
...
see \secref{ss:remapframe} and \secref{ss:wcsprocessingexample}.

\subsection{\ldots Display a Graphical Coordinate Grid}

A common requirement when displaying image data is to plot an
associated coordinate grid ({\em{e.g.}}\ Figure~\ref{fig:gridplot})
over the displayed image. The use of AST in such circumstances is
independent of the underlying graphics system, so starting up the
graphics system, setting up a coordinate system, displaying the image,
and closing down afterwards can all be done using the graphics
routines you would normally use.
...
the coordinate system used by an underlying computer graphics system
...
\subsection{\xlabel{changes}\xlabel{list_of_most_recent_changes}Changes Introduced in V1.1}
