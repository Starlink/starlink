...
\newcommand{\stardocdate}      {8th January 2000}
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      In addition, the experimental IRAF {\tt{"}}ZPX{\tt{"}} sky projection will be
      accepted when reading, but will never be written (the corresponding
      FITS {\tt{"}}ZPN{\tt{"}} projection being used instead). However, longitude and
      latitude correction surfaces used by this {\tt{"}}ZPX{\tt{"}} projection (appearing
      as {\tt{"}}lngcor{\tt{"}} or {\tt{"}}latcor{\tt{"}} terms in the IRAF-specific {\tt{"}}WAT{\tt{"}} keywords) are
      ...
      FitsChan as a set of cards using the keyword {\tt{"}}ASTWARN{\tt{"}}.
