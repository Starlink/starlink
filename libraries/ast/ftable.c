...
F77_INTEGER_FUNCTION(ast_columntype)( INTEGER(THIS),
                                      CHARACTER(COLUMNNAME),
                                      INTEGER(STATUS)
                                      TRAIL(COLUMNNAME) ) {
   GENPTR_INTEGER(THIS)
   ...
   astAt( "AST_COLUMNTYPE", NULL, 0 );
   astWatchSTATUS(
      columnname = astString( COLUMNNAME, COLUMNNAME_length );
      RESULT = astColumnType( astI2P( *THIS ), columnname );
      astFree( columnname );
   )
   return RESULT;
}
...
F77_INTEGER_FUNCTION(ast_columnndim)( INTEGER(THIS),
   ...
   astWatchSTATUS(
      columnname = astString( COLUMNNAME, COLUMNNAME_length );
      RESULT = astColumnNdim( astI2P( *THIS ), columnname );
      astFree( columnname );
   )
   return RESULT;
}
