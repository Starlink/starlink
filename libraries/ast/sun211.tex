...
(for instance) the \htmlref{astPutFits}{astPutFits} function, it will simply be truncated.
...
\sstroutine{
   astGrfFun\sstlabel{astGrfFun}
}{
   Register a graphics function for use by the Plot class
}{
   \sstdescription{
      This function can be used to select the underlying graphics
      functions to be used when the \htmlref{Plot}{Plot} class produces graphical output.
   ...
   \sstsynopsis{
      void astGrfFun( AstPlot $*$this, const char $*$name, void ($*$ fun)() )
   }
   \sstparameters{
      \sstsubsection{
         this
      ...
      \sstsubsection{
         ...
         Eight graphics functions are used in total by the
         ...
         eight graphics functions are not replaced in this way, the
      ...
      \sstsubsection{
         ...
         functionality indicated by parameter name.

         Once a function has been provided, a null pointer can be supplied
         in a subsequent call to astGrfFun to reset the function to the
   ...
   \sstdiytopic{
      ...
      int Attr( int attr, double value, double $*$old\_value, int prim )
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         alpha - A pointer to the location at which to return the scale for the
            X axis (i.e. Xnorm = alpha$*$Xworld).

         \sstitem
         beta - A pointer to the location at which to return the scale for the
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         chv- A pointer to the double which is to receive the height of
            characters drawn with a vertical baseline. This will be an
            increment along the X axis, in X axis units.

         \sstitem
         chh - A pointer to the double which is to receive the height of
   ...
   \sstdiytopic{
      ...
      int Text( const char $*$text, float x, float y, const char $*$just,
                float upx, float upy )
   ...
   \sstdiytopic{
      ...
      int TxExt( const char $*$text, float x, float y, const char $*$just,
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      either side). A value of zero or less may be given for {\tt{"}}params[0]{\tt{"}}
      ...
      \sstitemlist{
         ...
         central point.  Hence a block of (2 $*$ params[0] $+$ 1)$\wedge$ndim\_in
         ...
         value of params[0] may be (R - 1)/2.
...
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         {\tt{"}}flags{\tt{"}} parameter) and for identifying bad output values in
      ...
      \sstsubsection{
         ...
         relevant) equal to {\tt{"}}badval{\tt{"}} has been assigned because no
...
\sstroutine{
   ...
   \sstexamples{
      ...
      \sstexamplesubsection{
         wcsmap = astWcsMap( AST\_\_COE, 3, 2, 3, {\tt{"}}PV3\_1=40.0{\tt{"}} );
      }{
...
\sstroutine{
   ...
   Use Grf functions registered through astGrfFun?
}{
   \sstdescription{
      ...
      Otherwise, functions registered using \htmlref{astGrfFun}{astGrfFun} are used. In this
