      SUBROUTINE IRQ_SETQM( LOCS, BAD, QNAME, SIZE, MASK, SET, STATUS )
*+
*  Name:
*     IRQ_SETQM
...
*  Check that write access is available to the NDF.
      ...
      IF( .NOT. WRITE .AND. STATUS .EQ. SAI__OK ) THEN
         STATUS = IRQ__NOWRT
         CALL ERR_REP( 'IRQ_SETQM_ERR1',
     :           'IRQ_SETQM: Write access is not available to the NDF.',
     :            STATUS )
      END IF
...
*  Check that the supplied mask has the same size as the NDF.
         IF( NEL .NE. SIZE ) THEN
            STATUS = IRQ__INCOM
            CALL ERR_REP( 'IRQ_SETQM_ERR2',
     :      'IRQ_SETQM: Supplied mask and NDF have different sizes.',
     :                    STATUS )
         END IF
...
*  If an error occur, give context information.
 999  CONTINUE
      IF( STATUS .NE. SAI__OK ) THEN
         ...
         CALL ERR_REP( 'IRQ_SETQM_ERR3',
     :          'IRQ_SETQM: Unable to assign values for quality name '//
