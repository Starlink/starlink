      SUBROUTINE IRQ1_TEMP8( TYPE, DIM, LOC, STATUS )
*+
*  Name:
*     IRQ1_TEMP8
...
*  Invocation:
*     CALL IRQ1_TEMP8( TYPE, DIM, LOC, STATUS )
...
*     with calling DAT_TEMP8 if the objects created must subsequently be
*     erased.
...
*  Decide the number of dimensions to use.
      IF( DIM .EQ. 0 ) THEN
         NDIM = 0
      ELSE
         NDIM = 1
...
*  structure.
      IF ( COUNT .EQ. 1 ) THEN
         TMPLOC = ' '
         CALL DAT_TEMP8( 'IRQ1_TEMP8', 0, DUMMY, TMPLOC, STATUS )
...
*  to it.
         CALL DAT_NEW8( TMPLOC, NAME, TYPE, NDIM, DIM, STATUS )
...
*  elements.
      IF ( STATUS .NE. SAI__OK ) THEN
         IF( NDIM .EQ. 0 ) THEN
            CALL MSG_SETI( 'EL', 1 )
         ELSE
            CALL MSG_SETK( 'EL', DIM )
         END IF
         CALL MSG_SETC( 'TYPE', TYPE )
         CALL ERR_REP( 'IRQ1_TEMP8_NOTOBT',
     :            'IRQ1_TEMP8: Error obtaining ^EL elements of ^TYPE '//
