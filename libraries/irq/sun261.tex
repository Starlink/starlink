...
\sstroutine{
   ...
   \sstdescription{
      ...
      that can be used by IRQ\_SBAD. The compiled quality expression is
      identified by the returned IRQ identifier which should be
      released using \htmlref{IRQ\_ANNUL}{IRQ_ANNUL} when no longer needed. If any error is
   ...
   \sstarguments{
	 ...
	 leading blanks are removed.
      ...
      \sstsubsection{
         ...
         be annulled using routine \htmlref{IRQ\_ANNUL}{IRQ_ANNUL} or IRQ\_CLOSE when it is no
...
\sstroutine{
   ...
   \sstdescription{
      ...
      5 HDS locators is returned which is needed when calling other
      IRQ routines. The first locator points to a temporary object
      which holds a cloned identifier for the NDF, the other 4 point
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         are annuled by calling \htmlref{IRQ\_RLSE}{IRQ_RLSE}.
      }
      \sstsubsection{
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         If FIXED is {\tt false}, then BIT holds the corresponding bit number
         in the QUALITY component. The least significant bit is called
         Bit 1 (not Bit 0). If FIXED is {\tt true}, then BIT is
         indeterminate.
      }
      \sstsubsection{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      to compile succesfully (using \htmlref{IRQ\_COM}{IRQ_COM}), it must not 
...
within the NDF. The components of this structure are shown in 
\latexhtml{Table~\ref {TAB:QN}}{the table below}.
...
The set of five HDS locators returned by routines \htmlref{IRQ\_NEW}{IRQ_NEW}
and \htmlref{IRQ\_FIND}{IRQ_FIND} include
