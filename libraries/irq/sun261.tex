...
\sstroutine{
   ...
   \sstdescription{
      ...
      \htmlref{IRQ\_SETQL}{IRQ_SETQL} or \htmlref{IRQ\_SETQM}{IRQ_SETQM}
      is made. Note, the string \texttt{ANY} cannot be used as a quality
      name. Also, quality names may not contain any full stops.
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         If \texttt{true}, then by default all pixels are assumed to hold the
         quality specified by QNAME. If \texttt{false}, then it is assumed that
...
\sstroutine{
   ...
   \sstdescription{
      ...
      name specified by LOCS. If it is found, THERE is returned \texttt{true}.
      Otherwise THERE is returned \texttt{false}.
   }
   \sstinvocation{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         If \texttt{true}, then the quality name is defined within the NDF
         specified by LOCS. If \texttt{false}, then the quality name is
         undefined.
      }
      \sstsubsection{
         STATUS = INTEGER (Given and Returned)
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         Returned \texttt{.TRUE.} if the specified quality name had a fixed bit
         number on entry to this routine, and \texttt{.FALSE.} otherwise.
      }
      \sstsubsection{
         STATUS = INTEGER (Given and Returned)
      }{
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         If \texttt{true}, then the quality is either held by all pixels, or by
         no pixels. In this case the quality may not have a
         corresponding bit in the QUALITY component. If \texttt{false}, then
      ...
      \sstsubsection{
         ...
         If FIXED is \texttt{true}, then VALUE specifies whether all pixels hold
         the quality (VALUE = .TRUE.), or whether no pixels hold the
         quality (VALUE = .FALSE.). If FIXED is \texttt{false}, then VALUE is
         indeterminate.
      }
      \sstsubsection{
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         The quality name to remove, or \texttt{'ANY'} if all quality names are
         to be removed.
      }
      \sstsubsection{
         STATUS = INTEGER (Given and Returned)
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         If \texttt{true}, then the quality is removed from all pixels included
         in the list given by LIST.  If \texttt{false}, then the quality is
...
\sstroutine{
   ...
   \sstdescription{
      ...
      which either do (or, if BAD is \texttt{false}, do not) correspond to \emph{bad}
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         If \texttt{true}, then the quality is removed from all NDF pixels
         corresponding to `bad' pixels in the mask.  If \texttt{false}, then the
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         If \texttt{true}, then the read-only flag for the quality name will be
...
\sstroutine{
   ...
   \sstdescription{
      ...
      found. If HELD is \texttt{true}, then the corresponding pixels in VEC are
      set to the `bad' value (other pixels are left unaltered).
      If HELD is \texttt{false}, the corresponding pixels in VEC are left as
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         If \texttt{true} then those VEC pixels that hold a quality satisfying
      ...
      \sstsubsection{
         ...
         Returned \texttt{true} if all pixels in VEC are returned with `bad'
         values, and \texttt{false} if any returned pixel values are not `bad'.
      }
      \sstsubsection{
         NOBAD = LOGICAL (Returned)
      }{
         Returned \texttt{true} if no pixels in VEC are returned with `bad'
         values. It is \texttt{false} if any `bad' pixel values are returned.
      }
      \sstsubsection{
         STATUS = INTEGER (Given and Returned)
      }{
   ...
   \sstnotes{
      \sstitemlist{
         \sstitem
         There is a routine for each numeric data type: replace \texttt{"x"} in
...
\sstroutine{
   ...
   \sstdescription{
      ...
      (or, if LISTED is \texttt{false}, not included) in the supplied list of
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         If \texttt{true}, then the quality is assigned to all NDF pixels
         corresponding to `bad' pixels in the mask.  If \texttt{false}, then the
...
\sstroutine{
   ...
   \sstarguments{
      ...
      \sstsubsection{
         ...
         If any of the STATUS values IRQ\_\_BADSY( \texttt{"Unrecognised logical
         operator or constant"}), IRQ\_\_MSOPT ( \texttt{"Missing operator"}) or
         IRQ\_\_MSOPD ( \texttt{"Missing operand"}) are returned, then ERRPNT
         returns the offset within the quality expression at which the
         error was detected. Note, the offset refers to the returned
         form of QEXP, not the given form. These will be different if
         the given form of QEXP has any leading blanks. An offset of
