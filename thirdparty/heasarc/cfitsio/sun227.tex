...
\newcommand{\stardocversion}   {v2.4.40b}
...
\begin{quote}
\texttt{/star/sources/cfitsio/changes.txt}
\end{quote}

Users are advised to read the release information file at each new
release.

The Starlink release wraps the standard CFISTIO build system so that it
can be manipulated within the Starlink Software collection.  It has been
test built on Alpha/Digital Unix, PC/Linux and SPARC/Solaris.  The 
build system also handles MIPS/Ultrix and SPARC/SunOS though these have
not been tested.  The standard CFITSIO distribution may be built on many
more system types not relevant to Starlink.
