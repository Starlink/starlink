...
#
# RCS: @(#) $Id: tcl.m4,v 1.15 2006/02/01 22:10:36 abrighto Exp $
...
AC_DEFUN(TEA_PATH_TCLCONFIG, [
	...
	AC_CACHE_VAL(ac_cv_c_tclconfig,[
		...
		for i in `ls -d ${exec_prefix}/lib 2>/dev/null` \
			...
			`ls -d ${TCLTK_ROOT}/lib 2>/dev/null` \
...
AC_DEFUN(TEA_MAKE_LIB, [
	...
	if test "${SHARED_BUILD}" = "1" ; then
	    ...
	    SHLIB_LD_LIBS="${SHLIB_LD_LIBS} \"`${CYGPATH} ${TCL_BIN_DIR}/${TCL_LIB_FILE}`\""
	    if test x"${TK_BIN_DIR}" != x ; then
		SHLIB_LD_LIBS="${SHLIB_LD_LIBS} \"`${CYGPATH} ${TK_BIN_DIR}/${TK_LIB_FILE}`\""
	    fi
