C*GVDRIV -- PGPLOT Genicom printer driver -- PORTRAIT MODE
C+
      SUBROUTINE GVDRIV (IFUNC, RBUF, NBUF, CHR, LCHR)
...
C
C This driver is basically a copy of pxdriver.for with changes for
C setting the genicom mode and scale.
C Version 1.0  - 1990 Feb 12 - J. H. Trice.
C=======================================================================
C
C Supported device: Genicom 4410 dot-matrix printer.
C
C Device type code: /VGENICOM.
C
C Default device name: PGPLOT.PRPLOT.
C
C Default view surface dimensions: 7.8in (horizontal) by 10.25in
C (vertical).
C
C Resolution: 140 (x) x 144 (y) pixels/inch.
C
...
C%    type *,'Begin picture',NPICT
      ...
      CALL GRGV03(BX*BY, %val(BITMAP), 'C0'X)
      RETURN
C
C--- IFUNC=12, Draw line -----------------------------------------------
C
  120 CONTINUE
      CALL GRGV01(1, RBUF, IC, BX, BY, %val(BITMAP))
      RETURN
C
C--- IFUNC=13, Draw dot ------------------------------------------------
C
  130 CONTINUE
      CALL GRGV01(0, RBUF, IC, BX, BY, %val(BITMAP))
      RETURN
C
...
C%    type *,'End picture  ',NPICT
      CALL GRGV02(UNIT, BX, BY, %val(BITMAP))
      RETURN
C
...
C*GRGV01 -- PGPLOT Genicom printer driver, draw line
C+
      SUBROUTINE GRGV01 (LINE,RBUF,ICOL, BX, BY, BITMAP)
      INTEGER LINE
      REAL RBUF(4)
      INTEGER ICOL, BX, BY
      BYTE BITMAP(BX,BY)
...
C*GRGV02 -- PGPLOT Genicom driver, copy bitmap to output file
C+
      SUBROUTINE GRGV02 (UNIT, BX, BY, BITMAP)
      INTEGER UNIT, BX, BY
      BYTE BITMAP(BX,BY)
C
C Arguments:
...
C*GRGV03 -- fill buffer with a specified character
C+
      SUBROUTINE GRGV03 (BUFSIZ,BUFFER,FILL)
C
