...
/* Generated on Sat Jul  1 22:37:10 EDT 2006 */

#include "codelet-dft.h"

#ifdef HAVE_FMA

/* Generated by: ../../../genfft/gen_twidsq_c -fma -reorder-insns -schedule-for-pipeline -simd -compact -variables 4 -pipeline-latency 8 -n 8 -dif -name q1bv_8 -include q1b.h -sign 1 */

/*
 * This function contains 264 FP additions, 192 FP multiplications,
 * (or, 184 additions, 112 multiplications, 80 fused multiply/add),
 * 121 stack variables, and 128 memory accesses
 */
/*
 * Generator Id's : 
...
static const R *q1bv_8(R *ri, R *ii, const R *W, stride is, stride vs, INT m, INT dist)
{
     ...
     R *x;
     x = ii;
     for (i = 0; i < m; i = i + VL, x = x + (VL * dist), W = W + (TWVL * 14), MAKE_VOLATILE_STRIDE(is), MAKE_VOLATILE_STRIDE(vs)) {
	       ...
	       V T2O, T3C, T3i, T3k;
	       {
		    V T3d, T32, T3e, T3o, T3p, T33;
		    {
			 V T2v, T2w, T3V, T46, T3W;
			 {
			      ...
			      Tc = LD(&(x[WS(is, 2)]), dist, &(x[0]));
			      Td = LD(&(x[WS(is, 6)]), dist, &(x[0]));
			      T1C = LD(&(x[WS(vs, 3)]), dist, &(x[WS(vs, 3)]));
			      T1D = LD(&(x[WS(vs, 3) + WS(is, 4)]), dist, &(x[WS(vs, 3)]));
			      T1N = LD(&(x[WS(vs, 3) + WS(is, 2)]), dist, &(x[WS(vs, 3)]));
			      T1O = LD(&(x[WS(vs, 3) + WS(is, 6)]), dist, &(x[WS(vs, 3)]));
			      {
				   ...
				   T2w = VADD(T2k, T2l);
				   T2m = VSUB(T2k, T2l);
				   T3V = LD(&(x[WS(vs, 7) + WS(is, 2)]), dist, &(x[WS(vs, 7)]));
				   T46 = VADD(T3K, T3L);
				   T3M = VSUB(T3K, T3L);
				   T3W = LD(&(x[WS(vs, 7) + WS(is, 6)]), dist, &(x[WS(vs, 7)]));
			      }
			 }
			 {
			      V T15, TU, T16, T1g, TV, T1h;
			      {
				   ...
				   TJ = LD(&(x[WS(vs, 1) + WS(is, 2)]), dist, &(x[WS(vs, 1)]));
				   T2x = VSUB(T2v, T2w);
				   T2D = VADD(T2v, T2w);
				   TK = LD(&(x[WS(vs, 1) + WS(is, 6)]), dist, &(x[WS(vs, 1)]));
				   T47 = VADD(T3V, T3W);
				   T3X = VSUB(T3V, T3W);
				   ...
				   T1g = LD(&(x[WS(vs, 2) + WS(is, 2)]), dist, &(x[WS(vs, 2)]));
				   TL = VSUB(TJ, TK);
				   TV = VADD(TJ, TK);
				   T48 = VSUB(T46, T47);
				   T4e = VADD(T46, T47);
				   T1h = LD(&(x[WS(vs, 2) + WS(is, 6)]), dist, &(x[WS(vs, 2)]));
			      }
			      {
				   ...
				   T12 = VADD(TU, TV);
				   TW = VSUB(TU, TV);
				   T2R = LD(&(x[WS(vs, 5) + WS(is, 2)]), dist, &(x[WS(vs, 5)]));
				   T1i = VSUB(T1g, T1h);
				   T1s = VADD(T1g, T1h);
				   T2S = LD(&(x[WS(vs, 5) + WS(is, 6)]), dist, &(x[WS(vs, 5)]));
				   T3d = LD(&(x[WS(vs, 6)]), dist, &(x[WS(vs, 6)]));
				   T2I = VSUB(T2G, T2H);
				   T32 = VADD(T2G, T2H);
				   T3e = LD(&(x[WS(vs, 6) + WS(is, 4)]), dist, &(x[WS(vs, 6)]));
				   T3o = LD(&(x[WS(vs, 6) + WS(is, 2)]), dist, &(x[WS(vs, 6)]));
				   T3p = LD(&(x[WS(vs, 6) + WS(is, 6)]), dist, &(x[WS(vs, 6)]));
				   T1z = VADD(T1r, T1s);
				   T1t = VSUB(T1r, T1s);
				   T33 = VADD(T2R, T2S);
				   T2T = VSUB(T2R, T2S);
...
static const ct_desc desc = { 8, "q1bv_8", twinstr, &GENUS, {184, 112, 80, 0}, 0, 0, 0 };
...
/* Generated by: ../../../genfft/gen_twidsq_c -simd -compact -variables 4 -pipeline-latency 8 -n 8 -dif -name q1bv_8 -include q1b.h -sign 1 */
...
static const ct_desc desc = { 8, "q1bv_8", twinstr, &GENUS, {264, 128, 0, 0}, 0, 0, 0 };
