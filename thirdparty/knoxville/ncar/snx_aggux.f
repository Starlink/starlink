      ...
      REAL FUNCTION snx_AGGUX (GX)
*+
*  Name:
*     AGGUX

*  Purpose:
*     Convert grid X-coordinate GX into user X-coordinate.
...
*     Check validity of XLUW and XRUW.
         IF (XLUW.LE.0.0.OR.XRUW.LE.0.0) THEN

*        The value of either XLUW or XRUW is less than or equal to zero,
*        so abort.
            CALL SETER('SNX_AGGUX: UNABLE TO CONVERT GRID COORDINATES',
     :                 104,2)
            GO TO 999
         END IF

         XL=ALOG10(XLUW)
         X=10**(XL+GX*(ALOG10(XRUW)-XL))
      ELSE
         XL=XLUW
         X=XL+GX*(XRUW-XL)
      END IF
