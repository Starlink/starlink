...
*     Check validity of XLUW, XRUW and UX.
         IF (XLUW.LE.0.0.OR.XRUW.LE.0.0.OR.UX.LE.0.0) THEN

*        The value of XLUW, XRUW or UX is less than or equal to zero,
*        so abort.
            CALL SETER('SNX_AGUGX: UNABLE TO CONVERT USER COORDINATES',
     :                 106,2)
            GO TO 999
         END IF

         XL=ALOG10(XLUW)
         X=(ALOG10(UX)-XL)/(ALOG10(XRUW)-XL)
      ELSE
         XL=XLUW
         X=(UX-XL)/(XRUW-XL)
      END IF
