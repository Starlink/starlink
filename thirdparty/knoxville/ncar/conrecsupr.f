      ...
      DIMENSION       VWPRT(4)   ,WNDW(4)    ,LASF(13)
...
C
      ...
      IF (ILAB .EQ. 0) NML = 0
C
C SAVE CURRENT NORMALIZATION TRANS NUMBER
C
...
C
C SET TEXT INTENSITY TO LOW, AND WRITE TITLE USING NORMALIZATION TRANS 0
C
      CALL GSTXCI (ISPRTX)
      CALL GSELNT (0)
...
C
      CALL GSELNT (0)
      CALL WTSTR (.09765,.48825,IWORK(1:36),3,0,-1)
  127 CONTINUE
C
...
C
     ...
     -             WNDW(1),WNDW(2),WNDW(3),WNDW(4),IOLLS)
      ...
      CALL GSPLCI (IPLCI)
C
C SELECT ORIGINAL TRANS NUMBER NTORIG, AND RESTORE ASF
C
      CALL GSELNT (NTORIG)
      LASF(3)  = LSV3
      LASF(10) = LSV10
