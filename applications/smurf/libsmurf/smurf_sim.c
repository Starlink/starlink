...
*     Sim combines the functionality of a number of executables
...
void smurf_sim( int *status ) {
  ...
  pars[0] = (char *) malloc ( LEN__METHOD );
  pars[1] = (char *) malloc ( LEN__METHOD );
  pars[2] = (char *) malloc ( LEN__METHOD );
  pars[3] = (char *) malloc ( LEN__METHOD );
  strcpy ( pars[0], "sim" );
  ...
  if ( mode == stare || mode == pong || mode == dream ) {
      ...
      head = ( struct sc2head * )malloc( sizeof( struct sc2head[maxwrite] ) );
	...
	for ( bol=0; bol<nbol; bol++ ) {
     ...
     output = (double *)calloc ( numsamples, sizeof(double) );
     heatptr = (double *)calloc ( numsamples, sizeof(double) );
     dbuf = (double *)calloc ( numsamples*nbol, sizeof(double) );
     digits = (int *)calloc ( numsamples*nbol, sizeof(int) );
     dksquid = (int *)calloc ( numsamples*inx.nboly, sizeof(int) );
     head = (struct sc2head *)calloc ( inx.numsamples, 
