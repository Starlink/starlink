...
void smurf_makemap( int * status ) {
  ...
  msgOut("smurf_makemap","Inside MAKEMAP", status );
  ...
  if ( *status != SAI__OK ) {
    if ( nin != nout) {
      *status = SAI__ERROR;
      msgSeti( "NIN", nin);
      msgSeti( "NOUT", nout);
      errRep( "smurf_makemap", "Number of input pixels not equal to the number of output pixels (^NIN != ^NOUT)",status);
    }
  }
  ...
  if (!astOK) {
    if (*status == SAI__OK) {
      *status = SAI__ERROR;
      errRep( "makemap", "Error from AST", status);
    }
  }
