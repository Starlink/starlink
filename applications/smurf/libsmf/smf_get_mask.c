...
*  Arguments:
*     mtype = (Given)
*        The type of model (COM or AST) for which the mask is required.
   ...
   if( mtype == SMF__COM ) {
      ...
      *status = SAI__ERROR;
      errRepf( " ", "smf_get_mask: Unsupported model type %d supplied - "
               "must be COM or AST.", status, mtype );
   }
   subkm = NULL;
...
/* Report masking info. */
                     ndfMsg( "N", indf2 );
                     msgSetc( "M", modname );
                     msgOutif( MSG__DEBUG, " ", "smf_get_mask: The ^M "
...
/* Report masking info. */
               if( !have_mask ) {
                  msgOutiff( MSG__DEBUG, " ", "smf_get_mask: The %s model "
                             "will be masked on each iteration using an "
                             "SNR limit of %g.", status, modname, zero_snr );
               }
...
/* Report masking info. */
                  msgOutiff( MSG__DEBUG, " ", "smf_get_mask: The %s model "
                             "will be masked on each iteration using a "
                             "smoothed form of the final mask created with "
                             "the previous map.", status, modname );
               }
            }
         }
