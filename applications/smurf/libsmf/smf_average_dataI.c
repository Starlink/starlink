...
*  Purpose:
*     Average SCUBA-2 timestream data _INTEGERs
...
*  Invocation:
*     smf_average_dataI( const smfData *data, int start,  int nslice, 
...
*  Notes:
*     - Should only be used on _INTEGER (uncompressed) data.
...
void smf_average_dataI( const smfData *data, int start,  int nslice, 
  ...
  int64_t sum;               /* Sum of input data - use larger size */
  int *tstream = NULL;   /* Pointer to input data stream */
  ...
  if (data->dtype != SMF__INTEGER) {
    ...
    *status = SAI__ERROR;
    ...
    errRep(" ", FUNC_NAME ": This function averages _INTEGER only not ^TYP",
           status );
    return;
  }
