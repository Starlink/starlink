...
/* Description :
    Check to make sure the required common inx and sinx structure
    components exist.
   ...
   History :
    12Jun2006:  original (jb)
*/

{
  //  if ( !StatusOkP(status) ) return;
   
  /* Check the dxml_struct components */
  /*  if ( inx->dataname == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "dataname not specified in obsXMLfile.\n" );
  } else if ( inx->dec == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "dec not specified in obsXMLfile.\n" );  
  } else if ( inx->lambda == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "lambda not specified in obsXMLfile.\n" ); 
  } else if ( inx->mjdaystart == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "mjdaystart not specified in obsXMLfile.\n" ); 
  } else if ( inx->nbolx == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "nbolx not specified in obsXMLfile.\n" ); 
  } else if ( inx->nboly == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "nboly not specified in obsXMLfile.\n" ); 
  } else if ( inx->obsmode == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "obsmode not specified in obsXMLfilet.\n" ); 
  } else if ( inx->ra == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "ra not specified in obsXMLfile.\n" ); 
  } else if ( inx->sample_t == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "sample_t not specified in obsXMLfile.\n" ); 
  } else if ( inx->obsmode == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "target_pow not specified in obsXMLfile.\n" ); 
  }
*/
  /* Check the dxml_sim_struct components */
  /*  if ( sinx->add_atm == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "add_atm not specified in simXMLfile.\n" );
  } else if ( sinx->add_fnoise == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "add_fnoise not specified in simXMLfile.\n" );       
  } else if ( sinx->add_pns == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "add_pns not specified in simXMLfile.\n" );     
  } else if ( sinx->airmass == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "airmass not specified in simXMLfile.\n" );   
  } else if ( sinx->aomega == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "aomega not specified in simXMLfile.\n" );   
  } else if ( sinx->astname == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "astname not specified in simXMLfile.\n" );
  } else if ( sinx->atmname == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "atmname not specified in simXMLfile.\n" );
  } else if ( sinx->atmxvel == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "atmxvel not specified in simXMLfile.\n" );
  } else if ( sinx->atmyvel == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "atmyvel not specified in simXMLfile.\n" );
  } else if ( sinx->atmzerox == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "atmzerox not specified in simXMLfile.\n" );
  } else if ( sinx->atmzeroy == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "atmzeroy not specified in simXMLfile.\n" );
  } else if ( sinx->atend == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "atend not specified in simXMLfile.\n" );
  } else if ( sinx->atstart == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "atstart not specified in simXMLfile.\n" );
  } else if ( sinx->bandGHz == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "bandGHz not specified in simXMLfile.\n" );
  } else if ( sinx->flux2cur == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "flux2cur not specified in simXMLfile.\n" );
  } else if ( sinx->meantatm == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "meanatm not specified in simXMLfile.\n" );
  } else if ( sinx->pwvzen == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "pwvzen not specified in simXMLfile.\n" );
  } else if ( sinx->subname == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "subname not specified in simXMLfile.\n" );
  } else if ( sinx->telemission == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "telemission not specified in simXMLfile.\n" );
  }
*/
}//checkCommon
...
/* Description :
    Check to make sure the required DREAM-specific inx and sinx structure
    components exist.
...

{
  ...
  /* Check the dxml_struct components */
   /*  if ( inx->conv_shape == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "conv_shape not specified in obsXMLfile.\n" );
  } else if ( inx->conv_sig == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "conv_sig not specified in obsXMLfile.\n" );  
  } else if ( inx->jig_step_x == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "jig_step_x not specified in obsXMLfile.\n" ); 
  } else if ( inx->jig_step_y == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "jig_step_y not specified in obsXMLfile.\n" ); 
  } else if ( inx->jig_vert == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "jig_vert not specified in obsXMLfile.\n" ); 
  } else if ( inx->nvert == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "nvert not specified in obsXMLfile.\n" ); 
  } else if ( inx->smu_move == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "smu_move not specified in obsXMLfile.\n" ); 
  } else if ( inx->smu_offset == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "smu_offset not specified in obsXMLfile.\n" ); 
  } else if ( inx->smu_samples == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "smu_samples not specified in obsXMLfile.\n" ); 
  } 
*/
  /* Check the dxml_sim_struct components */
  /*  if ( sinx->ncycle == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "ncycle not specified in simXMLfile.\n" );
  } else if ( sinx->smu_terr == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "smu_terr not specified in simXMLfile.\n" );       
  } 
*/
}//checkDream
...
/* Description :
    Check to make sure the required HEATRUN-specific inx and sinx structure
    components exist.
...

{
  ...
  /* Check the dxml_struct components */
  /*  if ( inx->flatname == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "flatname not specified in obsXMLfile.\n" );
  } else if ( inx->heatnum == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "heatnum not specified in obsXMLfile.\n" );  
  } else if ( inx->heatstart == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "heatstart not specified in obsXMLfile.\n" ); 
  } else if ( inx->numsamples == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "numsamples not specified in obsXMLfile.\n" ); 
  } else if ( inx->jig_vert == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "jig_vert not specified in obsXMLfile.\n" ); 
  } else if ( inx->nvert == NULL ) {
...
*/
  /* Check the dxml_sim_struct components */
  /*  if ( sinx->add_hnoise == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "add_hnoise not specified in simXMLfile.\n" );
  } 
*/
}//checkHeat
...
/* Description :
    Check to make sure the required PONG-specific inx and sinx structure
    components exist.
...

{
  ...
  /* Check the dxml_struct components */
  /*  if ( inx->pongspacing == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "pongspacing not specified in obsXMLfile.\n" );
  } else if ( inx->pong_vmax == NULL ) {
      *status = DITS__APP_ERROR;
      printf ( "pong_vmax not specified in obsXMLfile.\n" );
  } 
*/
}//checkPong
...
/* Description :
    Check to make sure the required STARE-specific inx and sinx structure
    components exist.
