...
/* 2 term not damped. */
      ...
      mxv = sin ( PI / 4.0 ) + sin ( 3.0 * PI / 4.0 ) / 3.0;
      theta = ( -PI / 4.0 ) + offset * PI / vertex_t;
...
/* 3 term not damped. */
      ...
      mxv = sin ( PI / 6.0 ) + sin ( 3.0 * PI / 6.0 ) / 3.0 + 
        sin ( 5.0 *PI / 6.0 ) / 5.0; 

      theta = ( -PI / 6.0 ) + offset * PI / vertex_t;
...
/* 4 term not damped. */
      ...
      mxv = sin ( PI / 8.0 ) + sin ( 3.0 * PI / 8.0 ) / 3.0 + 
        sin ( 5.0 * PI / 8.0 ) / 5.0 + sin ( 7.0 * PI / 8.0 ) / 7.0; 
      theta = (-PI/8.0) + offset * PI / vertex_t;
      frac = 1.104 * ( ( sin(theta) + sin ( 3.0 * theta ) / 3.0 + 
...
/* 3 term flat end. */
      ...
      mxv = sin(PI/6.0) + sin(3.0*PI/6.0) / 3.0 + sin(5.0*PI/6.0) / 5.0;
      theta = (-PI/6.0) + offset * PI / vertex_t;
      if ( theta < PI/6.0 )
      {
...
/* Experimental. 
   ...
   Now it is a cosine waveform from 0 to 1 in the full time. */
      ...
      frac = 0.5 * ( 1.0 - cos ( PI * offset / vertex_t ) );
      *x = frac * ( vertxy[jend][0] - vertxy[jstart][0] ) + vertxy[jstart][0];
      *y = frac * ( vertxy[jend][1] - vertxy[jstart][1] ) + vertxy[jstart][1];
...
/* Experimental. 
   ...
   Now it is a cosine waveform from 0 to 1 in the full time. */
      ...
      for ( i=0; i<npts; i++ )
      {
         a[i] = ( cos ( PI * ( (double)i - popepi ) / popepi ) + 1.0 ) / 2.0;
      }
