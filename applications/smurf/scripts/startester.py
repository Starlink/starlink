...
*        In addition, TILES can be set to "?" if it is not yet known
*        which tiles will be generated. In this case, the used list of
*        tiles consists of whatever files matching "tile_*.sdf" are
*        generated. This should only be used as an initial means to derive
*        the tile list. The TDF file should later be modified to include the
*        real tile list.
...
#  Run the command, and store the standard output generated by it.
      stdoutText = self._runCmd( cmd )

#  Fix up the list of output NDFs.
      self._fixOuts( testVals )

#  Return the standard output generated by the command.
      return stdoutText
...
#  -------------------------------
   def _fixOuts( self, testVals ):
      if testVals[ "OUT" ] == "?":
         outs = None
         for tile in glob.glob("tile_*sdf"):
            if outs:
               outs = "{0},{1}".format( outs, tile )
            else:
               outs = tile
         testVals[ "OUT" ] = outs
