...
%    2010-09-16 (DSB):
%        Update to describe new step finder.
...
\newcommand{\autophotom}{\xref{\task{autophotom}}{sun95}{AUTOPHOTOM}}
...
% sc2clean s4a20091214_00015_0002_con.sdf clean2 \
   config='"order=0,dcfitbox=30,dcthresh=25,dcsmooth=50,fillgaps=1"'
\end{verbatim}
\end{myquote}

In addition to removing the mean as in the previous example, the extra
parameters starting with `dc' instruct \clean\ to smooth the data with a
median filter of width 50 samples, and then look for steps in the
smoothed data in excess of 25-$\sigma$. The heights of steps found in
this way are measured by fitting straight lines to the smoothed data on
either side of the jump using 30 samples in each fit. The corrections
thus determiend are applied to the original bolometer data.  Finally,
...
(smooth) realisation of noise to avoid introducing spikes into the data
stream (these ranges are ultimately ignored when producing final maps). A
map produced from `clean2.sdf' is shown in Fig.~\ref{fig:map_dc_mean}.
...
slices. When `badmasked.sdf' is subsequently viewed with \gaia, none
...
with good QUALITY has dropped from 7664503 to 7606521 (about an 0.8
per cent decrease) as additional samples were flagged in each
iteration.
...
\item[\texttt{dimmconfig\_bright\_compact.lis}]\quad This
  ...
  iterations is increased, and as in
...
\item{It then runs the \Kappa\ \beamfit\ task on the specified point
  ...
  \Kappa\ \autophotom. Flux densities for calibrators such as Uranus,
