...
\caption{Initial \gaia\ windows displayed upon loading a data cube
  ...
  broken isolated bolometers, all indicated in grey. {\bf Right:} The
...
\caption{{\sl Left:} Gain image: black $<$ 0.75, white $>$ 1.25; 
{\sl Middle:} Common-mode subtracted median: black $< -$5e-04, white $>$ 5e-04
{\sl Right:} Common-mode subtracted rms: white $>$ 6e-03 (twice the median).}
\label{fig:masks}
\end{center}
\end{figure}
...
\caption{
{\sl Top left:} Map of the concatinated data; 
{\sl Top right:} After removing a linear baseline from the time series;
{\sl Bottom left:} After linear baseline, steps, and spike removal;
{\sl Bottom right:} Iterative map maker with dimmconfig\_bright\_compact.lis
}
\label{fig:sampleastres}
\end{center}
\end{figure}
