...
first and last observations, which were dark observations, have nearly
identical lengths. Similarly files 2 to 14 also have identical sizes
as they each contain 30\,s worth of the on-source integration. File 15
is shorter as the end of the integration was shorter than 30\,s.  As
noted in the introduction, the first two files containing data from
...
scale raw data to units proportional to pW (double precision floating
...
(see the HDS extension in MORE.SCUBA2.FLATCAL) or can be calculated
dynamically from the flatfield ramps bracketing each science
observation. The DIMM will always use a flatfield ramp if
...
\caption{Initial \gaia\ windows displayed upon loading a data cube
  ...
  the `File' tab under `Startup options'). {\bf Left:} The main
  window, after clicking the `Z' button a number of times to zoom-in,
  shows a map of detector values at a fixed sample in time. Note that
  these data came from an array with a number of broken columns and
  broken isolated detectors, all indicated in grey. {\bf Right:} The
...
in \texttt{
  \$STARLINK\_DIR/share/smurf/s4a20091214\_00015\_000*.sdf}. All of
...
MAKEMAP: Map-maker will use 38671 MBytes of memory
...
BADBOL:    4920000 (32.03%),         410 bolos
DCJUMP:      27500 ( 0.18%),
 NOISE:     504000 ( 3.28%),          42 bolos
Total samples available for map:   10412658, 67.79% of max (867.721 bolos)
...
BADBOL:    5184000 (33.75%),         432 bolos  ,change     264000 (+5.37%)
DCJUMP:      27500 ( 0.18%),                    ,change          0 (+0.00%)
  APOD:    3635200 (23.67%),        2840 tslices,change    3635200 (+inf%)
   COM:     364579 ( 2.37%),                    ,change     364579 (+inf%)
 NOISE:     504000 ( 3.28%),          42 bolos  ,change          0 (+0.00%)
Total samples available for map:    7664503, 65.37% of max (836.736 bolos)
     Change from last report:   -2748155, -26.39% of previous
smf_iteratemap: Iteration 2 / 5 ---------------
smf_iteratemap: Calculate time-stream model components
smf_iteratemap: Rebin residual to estimate MAP
smf_iteratemap: *** CHISQUARED = 4.72918514955082 for chunk 1
...
\item[\texttt{dimmconfig.lis}]\quad Before the iterative solution
  ...
  from the water vapour monitor by default), and then \texttt{FLT}
...
CSO(225GHz) opacity, $\tau_{225}$, to SCUBA opacity terms were
determined for the different SCUBA filters. It was assumed for
commissioning and S2SRO that the new SCUBA-2 filters were sufficiently
similar to the wide-band SCUBA filters that these terms could be used
for extinction correction. In the form $\tau_{\lambda} = a \times
(\tau_{225} - b)$, the original SCUBA corrections were:
\begin{equation}
...
$\tau_{225}$. The WVM (not the CSO 225GHz tipper) data was used for
this analysis.
...
450$\mu$m filter, in particular, significantly narrower than its SCUBA
counterpart. The SCUBA-2 filter characteristics are described in
detail at
\htmladdnormallink{http://www.jach.hawaii.edu/JCMT/continuum/scuba2/filter/}{http://www.jach.hawaii.edu/JCMT/continuum/scuba2/filter/}.
...
the specifically designed dimmconfig\_bright\_compact.lis. The maps
produced from this are then analysed using tailor-made PICARD
recipes. PICARD is a post-processing and data combination tool that
using the same infrastructure as ORAC-DR, but is designed to be used
after the initial reduction with the DIMM is complete. Details of the
PICARD recipes and how to use them can be found on
\htmladdnormallinkfoot{the ORAC-DR web
  page}{http://www.oracdr.org/oracdr/PICARD}.


A map reduced by the mapmaker has units of pW. To calibrate the data
into units of Janskys, a set of bright, point-source objects with well
known flux properties are observed regularly to provide a flux
...
calibrators, the FCFs and NEFD's have been calculated as follows:
\begin{enumerate}
\item{The PICARD recipe SCUBA2\_FCFNEFD takes the reduced map, crops
    ...
    changable in the PICARD parameter file.}
\item{It then runs the Kappa \beamfit\ program on the specified
    point source. Fluxes for calibrators such as CRL618, HLTAU, Uranus and Mars
    ...
    parameter file with the known flux: FLUX\_450 = 0.050 (Jy) or FLUX\_850
    = 0.005 (Jy) for example. \beamfit\ will calculate the peak flux, the
...
\item {\textbf{$FCF_{arcsec}$}}

\begin{equation}
\label{eq:fcf_arcsec}
FCF_{arcsec} = \frac{S_{tot}}{P_{int} \times A_{pix}}
\end{equation}

where $S_{tot}$ is the total flux of the calibrator, $P_{int}$ is the
integrated sum of the source in the map (in pW) and $A_{pix}$ is the pixel
area in arcsec$^2$, producing an FCF in Jy/arcsec$^2$/pW. This
$FCF_{arcsec}$ is the number to multiply your map by when you wish to use
the calibrated map to do aperture photometry.

\item{\textbf{$FCF_{beam}$}}

\begin{equation}
\label{eq:fcf_beam}
FCF_{beam} = \frac{S_{peak}}{P_{peak}}
\end{equation}
producing an FCF in units of Jy/beam/pW.

The measured peak signal here is derived from the Gaussian fit applied
...
particularly at 450$\mu$m. $FCF_{beam}$ is the number to multiply your
map by when you wish to measure absolute peak fluxes of discrete
sources. The peak value in the map is then the total flux of the point
source. You should not integrate over the source after calibrating in
this fashion, as this will give an overestimate of the flux.

To overcome the problems encountered as a result of the peak errors, a
third FCF method has been derived, where the $FCF_{arcsec}$ is taken and
...
\item{\textbf{$FCF_{beamequiv}$}}
\begin{equation}
\label{eq:fcf_beamequiv}
FCF_{beamequiv}  = \frac{S_{tot} \times 1.133 \times {FWHM_{beam}}^2}{P_{int} \times A_{pix}}
\end{equation}

 or more conveniently:

\begin{equation}
FCF_{beamequiv} = FCF_{arcsec} \times 1.133 \times {FWHM_{beam}}^2
\end{equation}
where FWHM is 7.5'' and 14.0'' for the 450$\mu$m and 850$\mu$m
respectively. This produces an FCF in units of Jy/beam/pW.

The $FCF_{beamequiv}$ and $FCF_{beam}$ should agree with each other,
however this is often not the case when the source is distorted for
the reasons mentioned above. $FCF_{beamequiv}$ has been found to provide
more consistent results and it is advised to use this value when
possible, in the same way as $FCF_{beam}$. It is advised, when running
the matched filter on data, to use the $FCF_{beamequiv}$ for calibration.
...
noise that is not effectively modelled/removed by the map-maker
...
PICARD recipe called \texttt{SCUBA2\_MATCHED\_FILTER}.
...
also be supplied using the \texttt{PSF\_MATCHFILTER} recipe
parameter).
...
dip is about 10\% of the peak signal, and corresponds to attenuation
of the peak signal. However, the normalization applies a positive
correction to preserve peak flux densities, which results in an
...
by the larger noise where there was less data.
...
\caption{Top row: Orion22.sdf (left) and Orion23.sdf (right) maps from
  ...
  maps following processing with the PICARD script REMOVE\_BACKGROUND
...
\hline
\rule[-1ex]{0pt}{3.5ex} Source & RA(J2000) & DEC(J2000) & 850$\mu$m flux & 450$\mu$m flux & Ref  \\
\hline
...
450$\mu$m $FCF_{beamequiv}$ values for all calibrator observations taken
during the S2SRO period. The resulting mean FCF's in each waveband
are as follows:

\begin{equation}
FCF_{450} = 400 \pm 90 ~\text{Jy/beam/pW}
\end{equation}
\begin{equation}
FCF_{850} = 500 \pm 90 ~\text{Jy/beam/pW}
\end{equation}
...
\caption{Histograms of the 450$\mu$m (left) and 850$\mu$m (right)
  $FCF_{beamequiv}$ values calculated for the secondary calibrators observed
...
It is the recommendation to use the mean $FCF_{beamequiv}$ values
...
\item{Use PICARD's recipe SCUBA2\_FCFNEFD on your reduced calibration
    ...
    calculate the NEFD. (450$\mu$m: 400 Jy/beam/pW and 850$\mu$m: 500
