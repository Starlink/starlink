...
%        for this version 1.0 release coinciding with shared risk observing
...
\newcommand{\stardocnumber}    {19}
\newcommand{\stardocauthors}   {Edward Chapin \& Douglas Scott}
\newcommand{\stardocdate}      {18 January 2009}
...
\begin{myquote}
\begin{verbatim}
% s4a20091214_00015_0002_con.sdf uranus method=rebin
\end{verbatim}
\end{myquote}
...
\begin{myquote}
\begin{verbatim}
% sc2clean s4a20091214_00015_0002_con.sdf clean2 order=0 dcbox=50 \
...
indicates that {\em all} the detectors will have steps repaired at
that time slice when a step is found in {\em any individual} bolometer
(since there is significant crosstalk and large steps tend to be
correlated with small steps in many other detectors). Finally,
...
flagged `\texttt{BADSAM}' by the data acquisition system. Since each
bolometer produced about 14000 samples (including 2000 for padding),
this corresponds to 4416000/14000 $\sim$ 315 detectors that were not
operational (out of 1280). Since there are $40\times32=1280$
bolometers, and a total of 100 samples are flagged (50 before, and 50
after) at the locations of DC steps ({\texttt DCJUMP})in {\em all}
detectors, there are $1024000/(1280*100)=8$ DC steps that were
identified. Similarly, $1280 \mathrm{bolometers} \times 2000 =
...
During map-making flagged samples will not be used. However, for
...
in {\texttt \$STARLINK/share/smurf/s4a20091214\_00015\_000*.sdf}. All
...
\begin{myquote}
\begin{verbatim}
% makemap '$STARLINK/share/smurf/s4a20091214_00015_000?.sdf' uranus 2 \
...
Note that the quality and variance for the data become are stored as
...
\begin{figure}
\begin{center}
\includegraphics[width=\linewidth]{iter_ast} \\
\includegraphics[width=\linewidth]{iter_com} \\
\includegraphics[width=\linewidth]{iter_flt} \\
\includegraphics[width=\linewidth]{iter_res} \\
\includegraphics[width=\linewidth]{iter_qua} \\
