...
data file (see the HDS extension in MORE.SCUBA2.FLATCAL) or can be
...
override these values by setting \verb|ext.taurelation.filtname| in
...
defaults are listed in \verb|$SMURF_DIR/smurf_extinction.def|. We have
...
\begin{enumerate}
\item{The PICARD recipe \texttt{SCUBA2\_FCFNEFD} takes the reduced
...
\item{It then runs the \Kappa\ \beamfit\ task on the specified point
  source. Flux densities for calibrators such as CRL618, HLTau, Uranus
...
\item{\textbf{$\mathrm{FCF_{arcsec}}$} (surface brightness calibration)}
...
FCF in Jy/arcsec$^2$/pW. This $\mathrm{FCF_{arcsec}}$ is the number to
multiply your map by when you wish to have surface brightness units,
and to be able to carry out aperture photometry.

\item{\textbf{$\mathrm{FCF_{beam}}$} (point source calibration)}
...
at 450\,\micron. $\mathrm{FCF_{beam}}$ is the number to multiply your
...
third FCF method has been derived, where the $\mathrm{FCF_{arcsec}}$
...
\item{\textbf{$F\mathrm{CF_{beamequiv}}$} (improved point source calibration)}
\begin{equation}
...
The $\mathrm{FCF_{beamequiv}}$ and $\mathrm{FCF_{beam}}$ should agree
with each other. However, this is often not the case when the source
is distorted, for the reasons mentioned
above. $\mathrm{FCF_{beamequiv}}$ has been found to provide more
consistent results and it is advisable to use this value when
available, instead of $\mathrm{FCF_{beam}}$. It is also advisable,
when running the matched filter on data (see
Section~\ref{sec:cosmology}), to use the $\mathrm{FCF_{beamequiv}}$
for calibration.
