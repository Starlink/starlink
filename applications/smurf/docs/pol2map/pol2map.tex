...
object using the same mask. This suggests a two stage process in which all
available observations are processed initially using the auto-masking
method. These initial ``auto-masked'' maps are then co-added to produce a
map with higher signal-to-noise. A new mask is then defined by applying an
SNR cut to this co-add. All observations are then re-processed by
\texttt{makemap} a second time using this
static mask to define the source regions for all observations. The
resulting ``externally-masked'' maps are then co-added to produce the
final co-add.
