...
normalised map change between iterations falls below 0.2
(\texttt{pca.zero\_niter = 0.2}, \texttt{com.zero\_niter = 0.2},
\texttt{flt.zero\_niter = 0.2}). By this point, most of the astronomical
...
If auto-masking is used, the strongest 50 PCA components are removed on
each iteration. If external-masking is used, the strongest 150 PCA components
are removed on each iteration. The reasoning for this difference is that speed
is more important than accuracy when forming the initial auto-masked map, since
the auto-masked map is only used to create the masks used to create subsequent
external-masked maps.
