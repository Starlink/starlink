...
\stardocabstract  {
  ...
  low-level tasks are used within automated \ORACDR\ pipelines that
  produce calibrated data products. This document is intended for users
  who wish to run the low-level tasks themselves, to explore
...
contains details of {\em all} available command parameters and configuration
...
wild-cards, dollars, commas, {\em etc}, then they must be protected in
...
individual configuration parameters in detail.
\end{description}
...
Note that if you are providing a text file containing {\em output} file names,
...
for raw SCUBA-2 data,  ``pW'' for flat-fielded SCUBA-2 data, {\em etc}).
\end{itemize}
...
moving sources will use a coordinate system that represents {\em offsets}
...
See Chapin {\em et. al.} \cite{dimm} for a full description and analysis of the
...
necessary to create a plain text file containing all the required
...
Appendix~\ref{par:full} gives details of {\em all} the available
