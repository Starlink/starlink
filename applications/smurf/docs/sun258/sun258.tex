...
observation. We note that in the particular case of SCUBA-2 data a
...
outlined by the steps in Section \ref{se:dsworkflow} and
...
are generally not of ``science grade'' quality. A second approach is
to use the Dynamic Iterative Map Maker, described in Section
...
command. While the processing time may be larger, the reduced maps are
of superior quality.
...
%WHAT ELSE CAN WE SAY ABOUT THE RESPONSIVITY?
...
\item JCMT Water Vapour Monitor: The JCMT Water Vapour Monitor (WVM)
  ...
  line-of-site, and then converting this to an effective $\tau_{\rm
...
pixels. \SMURF\ supports two methods of creating a map:
\begin{enumerate}
\item A simple rebinning algorithm to assign data to output map pixels;
...
\item a single-step approach which applies of the necessary
...
\begin{latexonly}
   ...
   \begin{center}
     ...
     \caption{Image reconstructed from simulated data using the
       ...
       sources and the numerous spikes.}
     \label{fig:rebinmap}
   \end{center}
   \end{figure}
\end{latexonly}
\begin{htmlonly}
   \label{fig:rebinmap}
   \htmladdimg{sun258_rebinmap.gif}

   Figure ??: Image reconstructed from simulated data using the
   \aparam{REBIN} method. Note the deep wells around bright sources
   and the numerous spikes.

\end{htmlonly}

\subsubsection{\xlabel{iter}Iterative map-maker\label{se:iter}}
...
\ref{se:rebin} separately, the DIMM is capable of performing all
...
\begin{htmlonly}
   ...
   Figure ??: Simulation of bolometer time-series showing some of the
...
\begin{latexonly}
   ...
   \begin{center}
     ...
     \caption{Image reconstructed from simulated data using the
       ...
       sources are no longer present, and there are no spikes.}
     \label{fig:itermap}
   \end{center}
   \end{figure}
\end{latexonly}
