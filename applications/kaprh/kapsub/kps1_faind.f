      SUBROUTINE KPS1_FAIND( BAD, DIM1, DIM2, INARR, SIGRNG, LOW,
...
*    data type.
      ...
      LOWER = NUM_DTOD( MAX( MEAN + SIGRNG( 1 ) * SIGMA,
     :                    NUM_DTOD( VAL__MIND ) ) )
      UPPER = NUM_DTOD( MIN( MEAN + SIGRNG( 2 ) * SIGMA,
     :                    NUM_DTOD( VAL__MAXD ) ) )

*    Report the scaling limits.

      CALL MSG_SETD( 'MINVAL', NUM_DTOD( LOWER ) )
      CALL MSG_SETD( 'MAXVAL', NUM_DTOD( UPPER ) )
