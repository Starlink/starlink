...
\newcommand{\stardocdate}      {2 July 1998}
...
\latexonly{(see SG/5)} command language, and the IRAF CL.
...
the \xref{\texttt{hdstrace}}{sun102}{} command. For instance, to check the
...
space. POLKA attempts to create files within your \emph{ADAM directory},
and this message may appear if file creation fails. Your ADAM directory is
usually a sub-directory named \texttt{adam} within your home directory.
This may be changed by assigning an alternative directory path to the 
...
\section{Running POLPACK from the IRAF CL}
...
\dash\ in the area of graphics for instance. Some general guidance on running
Starlink applications from IRAF CL is given in \xref{SUN/217}{sun217}{}.
A description, intended for programmers, of the way Starlink packages are
set up for use with IRAF, and some technical detail about how the system
works is in \xref{SSN/35}{ssn35}{}.
...
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         list, or any of the other forms described in the help on \texttt{"}Group
         Expressions\texttt{"}.
      }
      \sstsubsection{
...
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         help on \texttt{"}Group Expressions\texttt{"}. Note, the input frames cannot be
      ...
      \sstsubsection{
         ...
         cursor must be over the POLKA window to see the \texttt{"}correct\texttt{"} colours.
         [FALSE]
      }
      \sstsubsection{
...
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         also be given by supplying a group of \texttt{"}name=value\texttt{"} strings where
         \texttt{"}name\texttt{"} is the name of an AST Frame attribute (see
         \xref{SUN/210}{sun210}{}), and
         \texttt{"}value\texttt{"} is the value to assign to it. In addition, IRAS90 Sky
         Coordinate System (SCS) values (such as \texttt{"}EQUATORIAL(J2000)\texttt{"},
         \texttt{"}GALACTIC\texttt{"}, \emph{etc} - see \xref{SUN/163}{sun163}{}) can be given. Domains \texttt{"}WORLD\texttt{"} and
         \texttt{"}DATA\texttt{"} are taken as synonyms for \texttt{"}PIXEL\texttt{"} and \texttt{"}AXIS\texttt{"} (unless the
         catalogue contains explicit definitions for the \texttt{"}WORLD\texttt{"} and \texttt{"}DATA\texttt{"}
      ...
      \sstsubsection{
         ...
         should be a group of comma-separated name=value strings where \texttt{"}name\texttt{"}
         is the name of a Plot attribute and \texttt{"}value\texttt{"} is the value to assign
         to the attribute. The text in the key is drawn as Plot \texttt{"}Strings\texttt{"}
         ...
         as a Plot \texttt{"}Curve\texttt{"} (using attributes COLOUR(CURVES), \emph{etc} - the
      ...
      \sstsubsection{
         ...
         a group of comma-separated name=value strings where \texttt{"}name\texttt{"} is the
         name of a Plot attribute, and \texttt{"}value\texttt{"} is the value to assign to
         the attribute. Vectors are drawn as \texttt{"}Curves\texttt{"} but may also be
         referred to using the synonym \texttt{"}Vectors\texttt{"} when specifying Plot
...
\hspace*{1in}\texttt{width(border)=0.05}\\
      ...
      \sstexamplesubsection{
         ...
         is specified by columns \texttt{"}ra\texttt{"} and \texttt{"}dec\texttt{"}. The annotated axes give
...
\xref{\texttt{hdstrace}}{sun102}{} command.
\begin{latexonly}
  \begin{figure}[htb]
  \begin{center}
