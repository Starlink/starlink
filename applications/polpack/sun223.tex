...
\newcommand{\stardocdate}      {24 March 1999}
...
\footnote{At the moment, circular polarization can only be measured when
using dual-beam data.} polarization of extended astronomical objects. Data 
...
\item extraction of $O$ and $E$ images from a single frame of dual-beam
data.
...
another. That is, the linear mapping between positions in any two target 
...
taken into account, if you have known values for these quantities.

\item Estimates of the variance in the input data can be made if necessary.
This is useful if your data does not have usable variance information
associated with it. Variances on the reduced quantities (Stokes vectors,
polarization vectors, \emph{etc}) can be made on the basis either of these
input variance estimates or of the variances supplied with the input data.

\item The input intensity Frames must all have the same normalization.
...
\item Variances can only be estimated for the reduced quantities (Stokes
vectors, polarization vectors, \emph{etc}) if the input data has usable
variances associates with it (\emph{i.e.} input variances cannot be estimated for
dual-beam data).
...
\item A single fixed analyser passes light polarized parallel to its axis.
A \emph{half-wave plate} is placed in the light path in front of the
analyser which rotates the plane of polarization of the incoming light
...
This form of polarimeter is shown diagramatically in Figure~\ref{fig:singopt}.
...
\begin{htmlonly}
   ...
   \begin{figure}[bhtp]
   \label{fig:singopt}
   \includegraphics{sun223_figures/singopt_htx.eps}
   \caption{The main optical components in a typical single-beam imaging polarimeter.}
   \end{figure}
   \end{quote}
\end{htmlonly}
...
orientation of the analyser (or half-wave plate) is specified by giving the
angle between this reference direction and the analyser (or half-wave
...
direction. This angle is referred to as $ANGROT$, and is specified in
degrees.
...
This is the angle from the first image axis to a hypothetical analyser
...
\begin{htmlonly}
   ...
   \begin{figure}[bhtp]
   \label{fig:effan}
   \includegraphics{sun223_figures/effan_htx.eps}
    \caption{The effective analyser position for a rotating half-wave plate.}
   \end{figure}
   \end{quote}
\end{htmlonly}
...
would have a value of $1.0$ for both. A perfectly \emph{bad}
analyser such as a piece of high quality glass, would have a transmission
of $2.0$ and an efficiency of zero. If you know the transmission and
efficiency of your analysers, then POLPACK can take account of them when
estimating the values of the Stokes parameters, $I$, $Q$ and $U$. If you
do not known them, then don't worry... just use values of $1.0$ for both.
...
analyser transmission and efficiency. This will give you some idea of
how sensitive the final results are to the values used.
...
There are many similarities between the reduction of single and dual beam
imaging polarimetry data. The basic difference is that each target
exposure contains only a single image of the sky (instead of the two
images produced by a dual-beam polarimeter). The various steps involved
are summarised below. For more details see \hyperref{go here}{see section }
{}{SEC:DBRED}. Details of how POLPACK can be used to implement these ideas 
in practice are given in later sections of this document. For a
mathematical description of the reduction of single-beam imaging
polarimeter data, see Sparks \& Axon (\emph{P.A.S.P.} in prep.).

\begin{description}
\item [Detector corrections] - The objective is to correct your data values
so that they are proportional to the combined sky and object intensity
transmitted by the analyser (plus noise). The details will depend on your
detector. Ideally, the noise in each data value should be independant of
the noise in the neighbouring data values. 

\item [Image alignment] - The images need to be aligned so that a given
pixel corresponds to the same point on the sky in all images. This
alignment can involved a shift of origin, magnification and rotation.
Note, if the images are rotated, the reference direction must also be
rotated.

\item [Sky subtraction] - Just the same as for dual-beam reduction.

\item [Estimation of the Stokes parameters] - For dual-beam data, the
Stokes parameters are derived from the \emph{difference} between two
images recorded simultaneously on the same detector. Thus drifts in zero 
point offsets in the detector, \emph{etc}, are cancelled out. However, 
for single-beam data the Stokes values depend on the absolute data values
in each target exposure, and so are more susceptible to these effects.
 
\item [Binning of the Stokes parameters] - Just the same as for dual-beam reduction.
\item [Calculation of the polarization vectors] - Just the same as for dual-beam reduction.
\item [Displaying the results] - Just the same as for dual-beam reduction.
...
Whether you are using dual or single-beam data, \emph{do not forget to
...
vectors (see the WEIGHTS parameter for application \htmlref{POLCAL}{POLCAL}). 
...
{}{APP:SNGBM} for details of the algorithm used to estimate the input
variances.
...
will be updated by any POLPACK application which causes the orientation
of the reference direction within the image to change. This will happen
...
using applications from another package, then be aware that the Frame
defining the reference direction may be lost or invalidated. Basically,
...
addition, the separate intensity images on which the Stokes parameters
were based may be saved once the processing is complete.

The reference direction (see \hyperref{this section}{section }
{}{SEC:SBPOLARIM}) for the Stokes vectors in the output cube will
not in general be the same as in the input intensity images. The
output reference direction is chosen as follows:
...
longitude/latitude, etc, then the reference direction in the output cube
will be north within the specific celestial co-ordinate system found.
...
\item [OUT]	- A list of the output images to receive the aligned, 
sky-subtracted images. If a null value (!) is supplied, these images are 
not saved. Only used in single-beam mode, or for non-polarimetric data.
...
of vectors in the final map. The bins used in yjis example are 8 by 
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      values. This process is repeated a number of times given by
      parameter NITER. Input values are rejected if they differ by more than
...
\sstroutine{
   ...
   \sstnotes{
      \sstitemlist{
         ...
         Frame is formed by rotating the pixel co-ordinates Frame so that the
...
\hspace*{1in}\makebox[1in][l]\texttt{STOKES?}  \texttt{PPCKSTOK}
   ...
   \sstnotes{
      \sstitemlist{
         ...
         Domain \texttt{"}POLANAL\texttt{"}. This Frame is formed by rotating the pixel co-ordinates
...
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         be insufficient free colours in the standard colour map (\emph{i.e.} if
...
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         vector map, showing the co-ordinate Frame specified by parameter
      ...
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
               ...
               corresponding pixel co-ordinates.
      ...
      \sstsubsection{
         ...
         The co-ordinates to put at the lower left corner of the plotting
         area, in the co-ordinates system specified by parameters COLX and
      ...
      \sstsubsection{
         ...
         The co-ordinates to put at the top right corner of the plotting
         area, in the co-ordinates system specified by parameters COLX and
         COLY. If a null value is supplied then an area is used which just
         encloses all the data in the supplied catalogue. [!]
      }
   ...
   \sstexamples{
      \sstexamplesubsection{
         ...
         the co-ordinates of each vector from columns X and Y.
      }
      \sstexamplesubsection{
      ...
      \sstexamplesubsection{
         ...
         equatorial (RA/DEC) co-ordinates referred to the equinox of B1950.
         ...
         displayed data if possible (\emph{i.e.} the FrameSet associated with the
         existing picture in the AGI database contains a sky co-ordinate
         Frame). If this is not possible, then the vector map will be
         aligned in pixel or grid co-ordinates. A message is displayed
      ...
      \sstexamplesubsection{
         ...
         in terms of any of the common sky co-ordinate systems, then the
   ...
   \sstnotes{
      \sstitemlist{
         ...
         co-ordinates in the {\tt "}Base Frame{\tt "} of the WCS information stored as
         ...
         are assumed to be equatorial sky co-ordinates with epoch and equinox
         ...
         picture has sky co-ordinate information stored with it).
      }
   }
}
\newpage
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
               X  -- The pixel X co-ordinate at the tabulated point.

            \sstitem
               Y  -- The pixel Y co-ordinate at the tabulated point.
         ...
         The co-ordinates contained in columns X and Y refer to pixel
         co-ordinates after any binning. For this reason it is usually better to avoid
         binning the Stokes vectors in this application (see parameter BOX).
         Information describing the mappings between pixel co-ordinates and
         any other known co-ordinate Frames will be stored in the catalogue
...
Given a set if input intensity values, and corresponding variance values,
...
\htmlref{POLBIN}{POLBIN} do not as yet use these co-variances\Neither do
they use the improved de-biassing technique described by Sparks \&
...
value is then found. If this residual is larger than a specified multiple
\footnote{Specified by POLCAL parameter NSIGMA.} of the standard deviation 
associated with the observed intensity value, $\sigma_k$, then the observed 
intensity value is rejected. The Stokes vectors are calculated again,
excluding all the rejected intensity values. This process is repeated a
number of times, as specified by POLCAL parameter NITER. The number of
pixels rejected in each image at each iteration can be displayed by
setting POLCAL parameter ILEVEL to 3.

If no usable variances are available for the observed data values, the
simplest option is to assume a constant value of $1.0$ for $\sigma^2_{k}$ 
...
Another option is to estimate the variances associated with the input data 
by looking at the deviations of the observed intensity values from the 
best fitting sine curve given by equation \ref{EQN:IEXP}. This allows
variances for the output Stokes vectors to be created, and also allows
the iterative rejection of bad data. This option can be used by setting 
POLCAL parameter WEIGHTS to 3. The technique is described in the
following pseudo-code, and is illustrated diagramatically in Figure
Figure~\ref{fig:varest}:

\begin{enumerate}

\item \label{STEP1}  \verb;  Fill the variance image with the value 1.0;
\item \label{STEP2}  \vspace{-3mm} \verb;  Calculate Stokes vectors;
\item \label{STEP3}  \vspace{-3mm} \verb;  Set ITER = 0;
\item \label{STEP4}  \vspace{-3mm} \verb;  While ( ITER < NITER ) do...;
\item \label{STEP5}  \vspace{-3mm} \verb;     Reduce the noise in the Stokes vectors;
\item \label{STEP6}  \vspace{-3mm} \verb;     Calculate expected intensity values implied by the smoothed I, Q and;
                     \\            \verb;     U values;
\item \label{STEP7}  \vspace{-3mm} \verb;     Find the squared residuals between these intensity estimates and the;
                     \\            \verb;     observed intensity values;
\item \label{STEP8}  \vspace{-3mm} \verb;     Find the mean squared residual at each pixel position;
\item \label{STEP9}  \vspace{-3mm} \verb;     Smooth the image holding these mean squared residual values;
\item \label{STEP10} \vspace{-3mm} \verb;     Copy this smoothed image to the current variance image;
\item \label{STEP11} \vspace{-3mm} \verb;     Copy the input intensity values, excluding values for which the;
                     \\            \verb;     squared residual found above is too large;
\item \label{STEP12} \vspace{-3mm} \verb;     Calculate I, Q and U again using only the copied input values, and;
                     \\            \verb;     the current variance image;
\item \label{STEP13} \vspace{-3mm} \verb;     Set ITER = ITER + 1;
\item \label{STEP14} \vspace{-3mm} \verb;  End do;
...
\xref{EQN:CHI}, no matter which input image it comes from. If this were
...
in step \ref{STEP1}. All input data is used.
...
parameter SMBOX. Setting SMBOX to 1 (or zero) results in no smoothing
being performed. 

As mentioned above, the likely error in an observed intensity value can be 
estimated by comparing it with the best fitting sine curve defined by the
current Stokes vectors. However, another handle on the errors can be
obtained by requiring the Stokes vectors to be spatially smooth over some
small scale. The smoothing introduced in this step, for instance, makes
it possible for the variance estimation process to work even if only
three input images are available. Without this smoothing, the best
fitting sine curve would always be an exact fit to any set of three 
observed intensity values, no matter what the noise on them may be.
Without smoothing this would result in the variance estimates always
being zero. The smoothing results in the $I$, $Q$ and $U$ values being
modified slightly so that they no longer correspond exactly to the
observed intensity values. Thus the residuals between the expected and
observed intensities are no longer zero, and a non-zero variance can be
obtained.


Why is thsi necessary?
...
\begin{latexonly}
  ...
  \begin{figure}[htb]
  \begin{center}
  \includegraphics[clip,scale=0.5]{sun223_figures/varest.eps}
...
\begin{htmlonly}
   ...
   \begin{figure}[bhtp]
   \label{fig:varest}
   \includegraphics{sun223_figures/varest.eps}
   \caption{The estimation of input variances. The observed and expected
            intensity images are combined to form a single variance image.}
   \end{figure}
   \end{quote}
...
Only *one* variance image.

Variance for *each* image is displayed if ILEVEL = 3.
...
Estimation of input variances.

Rejection of bad input values.
