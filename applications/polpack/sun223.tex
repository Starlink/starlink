...
space in your \emph{ADAM directory}. Your ADAM directory is
usually a sub-directory named \texttt{adam} within your home directory.
It may be changed to a new directory by assigning an alternative directory 
path to the environment variable ADAM\_USER.
...
\sstroutine{
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \sstitem
         Intermediate files created during the execution of POLKA are
	 stored in a separate directory created each time POLKA is run,
	 and deleted when POLKA exits. The directory will have a name of
	 the form \texttt{polka_temp_<nnn>} where \texttt{<nnn>} is some
	 number. This directory will be created within the directory
	 specified by the \texttt{HDS_SCRATCH} environment variable. If
	 \texttt{HDS_SCRATCH} is not defined then it will be created
