...
circular\footnote{At the moment, circular polarization can only be
measured when using dual-beam data.} polarization of extended
astronomical objects. Data from both single and dual beam polarimeters
can be processed.
...
(Stokes vectors, polarization vectors, \emph{etc}) can then be made on the
basis of these estimated variances.
...
dual-beam data. The basic difference is that each target exposure
contains only a single image of the sky, instead of the two images
produced by a dual-beam polarimeter. The various steps involved are
...
Details of how Stokes parameters are calculated in single-beam mode
are presented \hyperref{here}{in appendix }{}{APP:SNGBM}.
...
\item [OUT]	- If you have single-beam, or non-polarimetrix data, this
...
input NDFs. Then the structure of the output container files will be
...
``\verb+*_A+'' then an output file named \verb+o66_int_A.sdf+ will be created.
...
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         defined by columns {\tt "}X{\tt "} and {\tt "}Y{\tt "} and will usually be in units of pixels.
      }
      \sstsubsection{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      ({\tt "}imported{\tt "}) from FITS keywords stored in the files. Such keywords
...
\hspace*{1in}  \texttt{ANGROT?}   \texttt{PPCKANGR}\\
\hspace*{1in}  \texttt{ANLANG?}   \texttt{PPCKANLA}\\
\hspace*{1in}  \texttt{EPS?}      \texttt{PPCKEPS}\\
\hspace*{1in}  \texttt{FILTER?}   \texttt{PPCKFILT}\\
\hspace*{1in}  \texttt{IMGID?}    \texttt{PPCKIMID}\\
\hspace*{1in}  \texttt{RAY?}      \texttt{PPCKRAY}\\
\hspace*{1in}  \texttt{STOKES?}   \texttt{PPCKSTOK}\\
\hspace*{1in}  \texttt{T?}        \texttt{PPCKT}\\
\hspace*{1in}  \texttt{VERSION?}   \texttt{PPCKVERS}\\
\hspace*{1in}  \texttt{WPLATE?}   \texttt{PPCKWPLT}
...
\hspace*{1in}  \texttt{WPLATE} \texttt{HWP}
...
\hspace*{1in}  \texttt{WPLATE} \texttt{45.0}
...
\hspace*{1in}  \texttt{IMGID} \texttt{"}M51\_PLATEB\texttt{"}
...
\hspace*{1in}  \texttt{Data-type} \texttt{FITS-keyword}
...
\hspace*{1in}  \texttt{\_REAL} \texttt{HWP}\\
\hspace*{1in}  \texttt{WPLATE} \texttt{57.29578$*$HWP}
...
\hspace*{1in}  \texttt{\_INTEGER} \texttt{OBSNUM}\\
\hspace*{1in}  \texttt{\_INTEGER} \texttt{IDATE}\\
\hspace*{1in}  \texttt{IMGID} \texttt{OBSNUM//IDATE}
...
\hspace*{1in}  \texttt{WPLATE} \texttt{POLPLATE  48.0=0.0 -}\\
\hspace*{2in}  {}           \texttt{138.0=45.0}
      ...
      signs ({\tt "}48.0{\tt "} and {\tt "}138.0{\tt "}), If a match is found (ignoring leading
      and trailing blanks), it assigns the value from the right hand side
      of the equals sign ({\tt "}0.0{\tt "} or {\tt "}45.0{\tt "}) to the WPLATE component in the
      POLPACK extension. An error is reported if no match is found. The {\tt "}-{\tt "}
...
\hspace*{1in}  \texttt{RAY?} \texttt{OLDRAY}\\
\hspace*{1in}  \texttt{RAY?} \texttt{PPCKRAY}
...
\hspace*{1in} X\_out = C1  $+$  C2 $*$ X\_in  $+$  C3 $*$ Y\_in

\hspace*{1in} Y\_out = C4  $+$  C5 $*$ X\_in  $+$  C6 $*$ Y\_in
...
\hspace*{1in}\texttt{title=My favorite colours} \\
\hspace*{1in}\texttt{grid=1} \\
\hspace*{1in}\texttt{minticklen=0} \\
\hspace*{1in}\texttt{colour(border)=green} \\
\hspace*{1in}\texttt{colour(grid)=blue} \\
\hspace*{1in}\texttt{colour(vec)=red} \\
\hspace*{1in}\texttt{width(border)=0.05}
...
Given a set if observed intensity values, and corresponding variance values,
...
\begin{htmlonly}
   ...
   \begin{figure}[bhtp]
   \label{fig:varest}
   \includegraphics{sun223_figures/varest_htx.eps}
   \caption{The estimation of input variances. The observed and expected
            intensity images are combined to form a single variance image.}
   \end{figure}
   \end{quote}
...
performed independantly for each pixel. However, unless your images are
...
\begin{latexonly}
  ...
  \begin{figure}[htb]
  \begin{center}
  \includegraphics[clip,scale=0.75]{sun223_figures/quadfit.eps}
  \caption{A 1-dimensional representation of the smoothing performed by
...
\begin{htmlonly}
   ...
   \begin{figure}[bhtp]
   \label{fig:quadfit}
   \includegraphics{sun223_figures/quadfit_htx.eps}
   \caption{A 1-dimensional representation of the smoothing performed by
            POLCAL in single-beam mode. In this example, each pixel is 
            replaced by the value of a quadratic surface fitted to the 
            nearest 5 data values.}
...
values and the corresonding observed intensity values are found, and squared.
...
The size of the box used is the same as in step \ref{STEP5}. So setting
...
step \ref{STEP7} will represent samples from the noise distribution. The
...
$N$ is larger than 3, then the residuals produced in \ref{STEP7} will already
...
setting parameter WEIGHTS to 4 in order to use these variances. This
...
intensity images analagous to the templates, but filled with the expected
...
component by POLIMP or POLEXT. If data written by POLACK V2.0 or later 
