...
\newcommand{\stardocdate}      {13th May 1999}
...
% Caption for front cover picture.
\begin{latexonly}
\newpage
\begin{center}
\vspace*{3in}
...
\begin{htmlonly}
\htmlfig{fig:quadfit}{quadfit.gif}{
A 1-dimensional representation of the smoothing performed by
POLCAL in single-beam mode. In this example, each pixel is 
replaced by the value of a quadratic surface fitted to the 
nearest 5 data values.}
\end{htmlonly}
...
\htmlref{POLCAL}, \htmlref{POLBIN} and \htmlref{POLVEC} has been changed.
Prior to V2.0 the reference direction was the WPLATE=0 position ({\emph i.e.} 
the direction of the fixed analyser). As of V2.0, the reference direction
will be north if there is WCS information available to define north, or
the positive Y axis ({\emph i.e.} the second pixel axis) otherwise.
\htmlref{POLVEC} and \htmlref{POLBIN} can still read data sets created by
earlier versions of POLPACK which use the previous convention, but will 
write data sets using the new convention.

\item The angles produced by \htmlref{POLBIN} and \htmlref{POLVEC} which
...
the same reference direction as the Stokes parameters. \htmlref{POLPLOT}
...
\htmlref{POLIMP} or \htmlref{POLEXT} is run. V2.0 will read ANGROT values
