...
% ? Add picture here if required for the LaTeX version.
   \begin{center}
   \includegraphics[scale=0.6]{sun223_figures/map2.eps}
   \end{center}
% ? End of picture
...
% ? Add picture here if required.
   \begin{center}
   \includegraphics[scale=0.7]{sun223_figures/map2.eps}
   \end{center}
% ? End of picture
...
the form of a {\em polarization map} (see the first page of this document
...
\begin{latexonly}
  \begin{figure}[htpb]
  \begin{center}
  \includegraphics[scale=0.7]{sun223_figures/dataflow.eps}
...
accessing parameters, producing graphics, reporting errors, etc. They
...
data in the form of an NDF {\em VARIANCE component}. This is an array
...
have been created by the data aquisition system). For other data formats,
...
degrees, and assign the result to the item ANGROT in the POLPACK extension.
The first line informs POLIMP that the keyword \verb+ROTA+ should be
treaed as a single precision floating point value. The recognized data
types are:

\begin{myquote}
\begin{description}
\item[\_BYTE]		- Single byte signed integer values.
\item[\_CHAR]		- Character strings.
\item[\_DOUBLE]		- Double precision floating point values.
\item[\_INTEGER]	- Single precision integer values.
\item[\_REAL]		- Single precision floating point values.
\item[\_WORD]		- Two byte signed integer values.
\end{description}
\end{myquote}
...
the \xref{\tt hdstrace}{sun102}{} command. For instance, to check the
...
The POLKA command activates a standard Starlink A-task which aquires
...
~\ref{fig:polka}). Once the required features have been supplied, POLKA
commences the processing of the supplied data frames. This may take a
significant length of time depending on your hardware, and the size and
number of your images. A window displays the progress being made during
this phase.

\begin{latexonly}
  \begin{figure}[htpb]
  \begin{center}
  \includegraphics[scale=0.9]{sun223_figures/polka.eps}
...
\begin{htmlonly}
   ...
   \begin{figure}[bhtp]
   \label{fig:polka}
   \includegraphics[scale=0.9,width=8in]{sun223_figures/polka.eps}
   \caption{The Graphical User Interface presented by POLKA.}
   \end{figure}
   \end{quote}
\end{htmlonly}
...
output catalogue in the same format, but containing fewer rows (i.e.
...
\item \label{STEP:DEVICE} Select the graphics device and the image
display device (both are set to an X windows display in this example):
\begin{myquote}
\begin{verbatim}
   % gdset xwindows
   % idset xwindows
...
CONTOUR, POLPLOT, etc, produce separate output PostScript files which
then need to be combined into a single file using the 
\xref{\tt psmerge}{sun164}{} command. In step \ref{STEP:DEVICE},
...
\verb+gks74.ps.1+, \verb+gks74.ps.2+, etc\footnote{The sequence number
...
examine the individual files using {\tt ghostview}.}.
...
The text file {\tt style.dat} specifies the style with which to draw the 
...
The text file {\tt keystyle.dat} specifies the style with which to draw the 
...
It is sometimes convenient to store the strings specifing the objects to
...
list of explicit values. This is done by preceeding the name of the text
...
\end{myquote}
\vspace{-3mm}
This means ``Use \verb+hh1_b1s1_ds.sdf+, \verb+hh1_b1s2_ds.sdf+, etc''.
...
\sstroutine{
   ...
   \sstdescription{
      ...
      as the {\tt "}F-factor{\tt "} and is based on the redundancy provided by using four
   ...
   \sstexamples{
      \sstexamplesubsection{
         ...
         corresponding I, Q and U values in the 3d cube {\tt "}stokes{\tt "}.
      }
   }
   \sstnotes{
      \sstitemlist{
...
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         list, or any of the other forms described in the help on {\tt "}Group
         Expressions{\tt "}.
      }
      \sstsubsection{
   ...
   \sstdiytopic{
      ...
      stored in the keyword is shown in parentheses (see 
      \htmlref{POLIMP}{POLIMP} for a description of these extension items):
      \sstitemlist{
...
\hspace*{1in}\makebox[1in][l]{\tt ANGROT}   {\tt PPCKANGR}\\
\hspace*{1in}\makebox[1in][l]{\tt FILTER}   {\tt PPCKFILT}\\
\hspace*{1in}\makebox[1in][l]{\tt IMGID}    {\tt PPCKIMID}\\
\hspace*{1in}\makebox[1in][l]{\tt WPLATE}   {\tt PPCKWPLT}\\
\hspace*{1in}\makebox[1in][l]{\tt RAY?}     {\tt PPCKRAY}\\
\hspace*{1in}\makebox[1in][l]{\tt STOKES?}  {\tt PPCKSTOK}
...
\hspace*{1in}\makebox[1in][l]{\tt ANGROT} {\tt ANROTA}\\
...
\hspace*{1in}\makebox[1in][l]{\tt ANGROT} {\tt 45.0}\\
...
\hspace*{1in}\makebox[1in][l]{\tt IMGID} {\tt "}M51\_PLATEB{\tt "}\\
...
\hspace*{1in}\makebox[1in][l]{\tt Data-type} {\tt FITS-keyword}\\
...
\hspace*{1in}\makebox[1in][l]{\tt \_REAL} {\tt ROTA}\\
\hspace*{1in}\makebox[1in][l]{\tt ANGROT} {\tt 57.29578$*$ROTA}\\

       The function may use any of the usual Fortran operators; $+$, -, $*$, /,
      $*$$*$ and built-in functions (SIN, COS, TAN, LOG, etc). See 
...
\hspace*{1in}\makebox[1in][l]{\tt \_INTEGER} {\tt OBSNUM}\\
\hspace*{1in}\makebox[1in][l]{\tt \_INTEGER} {\tt IDATE}\\
\hspace*{1in}\makebox[1in][l]{\tt IMGID} {\tt OBSNUM//IDATE}\\
...
\hspace*{1in}\makebox[1in][l]{\tt WPLATE} {\tt WAVEPLT  A=0.0 -}\\
\hspace*{1in}\makebox[1in][l]{}           {\tt B=22.5 -}\\
\hspace*{1in}\makebox[1in][l]{}           {\tt C=45.0 -}\\
\hspace*{1in}\makebox[1in][l]{}           {\tt D=67.5}\\
...
\hspace*{1in}\makebox[1in][l]{\tt RAY?} {\tt OLDRAY}\\
\hspace*{1in}\makebox[1in][l]{\tt RAY?} {\tt PPCKRAY}\\
...
\sstroutine{
   ...
   \sstdescription{
      ...
      bit. The GUI makes use of various applications from POLPACK, \xref{KAPPA}{sun95}{} and
      \xref{CCDPACK}{sun139}{} to perform all these tasks.
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         \hspace*{5mm} X\_out = C1  $+$  C2 $*$ X\_in  $+$  C3 $*$ Y\_in\\
         \hspace*{5mm} Y\_out = C4  $+$  C5 $*$ X\_in  $+$  C6 $*$ Y\_in\\
      ...
      \sstsubsection{
         ...
         help on {\tt "}Group Expressions{\tt "}. Note, the input frames cannot be
...
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         supported by the CAT library (see \xref{SUN/181}{sun181}{}). A 
         file type of .FIT is assumed if no file type is supplied.
      }
      \sstsubsection{
         CLEAR = \_LOGICAL (Read)
      }{
      ...
      \sstsubsection{
         ...
         {\tt "}name{\tt "} is the name of an AST Frame attribute (see
         \xref{SUN/210}{sun210}{}), and
         {\tt "}value{\tt "} is the value to assign to it. In addition, IRAS90 Sky
         Coordinate System (SCS) values (such as {\tt "}EQUATORIAL(J2000){\tt "},
         {\tt "}GALACTIC{\tt "}, etc - see \xref{SUN/163}{sun163}{}) can be given. Domains {\tt "}WORLD{\tt "} and
         {\tt "}DATA{\tt "} are taken as synonyms for {\tt "}PIXEL{\tt "} and {\tt "}AXIS{\tt "} (unless the
         catalogue contains explicit definitions for the {\tt "}WORLD{\tt "} and {\tt "}DATA{\tt "}
      ...
      \sstsubsection{
         ...
         referred to using the synonym {\tt "}Vectors{\tt "} when specifying Plot
...
\sstroutine{
   ...
   \sstdescription{
      ...
      created by \htmlref{POLKA}{POLKA} or \htmlref{POLCAL}{POLCAL}). These 
      calculated values may be stored either in
      a series of 2D NDFs, or in a single catalogue which may be
      examined and manipulated using the \xref{CURSA}{sun190}{} package
      {\latexonly (see SUN/190)}.
   }
   \sstusage{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         is usually better to use the \htmlref{POLBIN}{POLBIN} application 
         to bin the Stokes vectors. [1]
      }
      \sstsubsection{
         CAT = LITERAL (Read)
      }{
...
\xref{\tt hdstrace}{sun102}{} command.
\begin{latexonly}
  \begin{figure}[htb]
  \begin{center}
