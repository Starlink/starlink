...
\newcommand{\stardocdate}      {12 May 1998}
...
{\em Note, the same flat-field should be used to flat-field all target
...
POLPACK to produce a vector map from dual-beam polarimetry data which
conforms to the model outlines in the previous section. An over-view of
the data reduction process is given in Figure~\ref{fig:dataflow}.

\begin{latexonly}
  \begin{figure}[htpb]
  \begin{center}
  \includegraphics[scale=0.8]{sun223_figures/dataflow.eps}
...
\begin{htmlonly}
   ...
   \begin{figure}[bhtp]
   \label{fig:dataflow}
   \includegraphics[scale=0.8,width=8in]{sun223_figures/dataflow.eps}
   \caption{The main steps in the production of a polarization map.}
   \end{figure}
   \end{quote}
\end{htmlonly}
...
\xref{CONVERT}{sun55}{} package provides facilities for converting to and
...
The first command initialise the commands required to use the CONVERT
...
same file names, but a file typ of \verb+.sdf+\footnote{The double quotes
...
\verb+fits2ndf+.}. The \verb+.sdf+ files can then be given as inputs to
any application from POLPACK, CCDPACK or KAPPA.

\item You can rely on the facilities of the NDF library to perform
...
contrary), and you should undertand these as referring to your own
non-NDF data files.
...
of sensitivity across the detector, zero point offsets, etc, before
...
will usually be able to perform these corrections,
...
$E$ ray channels. If the {\em same} flat-field is used for all target
...
{\em Do not forget to use the same flat-field to correct {\em all} target
...
master flat-field. This is important since any noise in the flat-field
gets passed directly into the final results.
...
of your detector, you may be able to store this information with your data in 
the form of an NDF {\em VARIANCE component}. This is an array holding an 
estimate of the variance at every pixel in your data. If present, POLPACK
will process this information to obtain estimates of the uncertainty in
the final polarisation parameters. If you use CCDPACK to calibrate your
data, then VARIANCE components are created automatically if you supply the
required information.

\subsection{\xlabel{startup}Starting up POLPACK}
...
This command must be issued before attempting to use any POLPACK command.
...
orientation of the reference direction, etc. Different instruments may
...
item, each line of the control table contains a description of where to
...
tells POLIMP to find the value of the FITS keyword HWPANG and store it as 
item ``WPLATE'' in the POLPACK extension (this is the half-wave plate 
position).

If your data is supplied in the form of a set of FITS files, then the
HWPANG value is simply obtained from the primary header. If your data is
supplied in the form of a set of NDF structures, then the value is read
from the FITS extension in the NDF structure (which should have been
created by the data aquisition system). For other data formats, the
origin of the keyword value depends on how the data conversion is
performed. \xref{SUN/55}{sun55}{} contains details of how FITS keywords are
created from the foreign data file.
...
convention. See the description of the \htmlref{POLIMP}{POLIMP}
application for further details of the syntax of the control table.
...
The contents of the POLPACK extension within an NDF can be examined using
the \xref{\verb+hdstrace+}{sun102} command. For instance, to check the
contents of the POLPACK extension in the NDF \verb+partproc.sdf+, do:
...
The POLKA command activates a standard Starlink a-atask which aquires
parameter values using the normal methods employed by most Starlink
applications. The a-task then activates a script written in a language
called Tk/Tcl which performs the processing. This script requires you
...
and controls to select any of the above features. Once the required
features have been supplied, POLKA commences the processing of the
supplied data frames. This may take a significant length of time depending
on your hardware, and the size and number of your images. A window
display the progress being made during this phase.
...
linear polarisation, or V for circular polarization. In addition, the
separate intensity images on which the Stokes parameters were based may
be saved once the processing is complete.

The POLKA application has many parameters. The values provided for some of
these may be changed from within the GUI. However, some may not be
changed and should always be assigned appropriate values when invoking POLKA.
...
\item [MODE] - Can be CIRCULAR or LINEAR, and indicates what sort of
polarization you are measuring.
\item [OUT\_E] - A list of the output images to recieve the extracted,
...
OUT (parameters OUT\_S, OUT\_E and OUT\_O are not used). In this mode,
...
are new to POLKA, it is  a good idea to work your way through the tutorial.
...
\begin{description}
\item [\htmlref{POLVEC}{POLVEC}] - This converts a 3-D cube containing Stokes parameters
...
For cicular polarimetry, the Q and U columns are replaced by a single V
...
POLVEC can also produce a set of 2-D images holding the above quantities.

\item [\htmlref{POLBIN}{POLBIN}] - This accepts a catalogue containing Stokes parameters
as input (such as produced by POLVEC), and creates a new output catalogue
in the same format. The output catalogue is formed by binning the Stokes 
vectors in the input catalogue within a grid of equally sized
...
creates a new catalogue called \verb+selcat.FIT+ which contains all the
...
\item Convert the 3-D NDF containing Stokes vector in file \verb+cube.sdf+ 
...
\item Bin the catalogue to reduce the noise, and to decrease the density
of vectors in the final map. The bins used are 8 by 8 pixels (measured in
the Stokes parameter NDF \verb+cube.sdf+), and the Stokes parameters are
combined using a median. The binned catalogue is stored in file \verb+bincat.FIT+:
...
\item \label{STEP:DEVICE} Select the graphics device and the image display device (both set to an X 
windows display in this case):
\begin{myquote}
\begin{verbatim}
   % gdset xwindows
   % idset xwindows
...
\item \label{STEP:PICDEF} Create a ``picture'' to contain a greyscale display of the total
...
greyscale image, replace step \ref{STEP:DISPLAY} with:
...
CONTOUR and POLPLOT produce separate output PostScript files which then
need to be combined into a single file. In step \ref{STEP:DEVICE},
...
\verb+gks74.ps.1+, \verb+gks74.ps.2+, etc. To produce a contour map with
overlaid vectors, replace steps \ref{STEP:DISPLAY} and \ref{STEP:POLPLOT}
with:
...
\begin{verbatim}
   ...
   % psmerge contour.ps polplot.ps > total.ps
\end{verbatim}
\end{myquote}

Note, CONTOUR produces two PostScript files (\verb+gks74.ps+ and
\verb+gks74.ps.1+). Make sure you use \verb+gks74.ps.1+.
...
{\em groups} (or {\em lists}) of objects. For instance, the 
...
group of plotting attributes. A {\em group expression} is the string
...
character. For instance, the group expression ``\^{}{\small style.dat}''
would result in the file {\small style.dat} being opened and the strings
...
{\small style.dat} contained the following two lines: 
...
The the strings \verb+grid=1+, \verb+colour(grid)=red+, \verb+border=1+
and \verb+colour(border)=red+ would be returned to the 
application, and in addition the file {\small labels.dat} would be 
...
{\small file2\_ds}, the resulting group would be {\small new\_file1\_imb}
and {\small new\_file2\_imb}. Note, this facility is only available if
...
This means ``Use the NDFs stored in files b1.sdf, b2.sdf, b3.sdf and
b4.sdf''. If automatic format conversion is being used (see 
\hyperref{here}{section}{}{SEC:CONVERT}), then an example such as 
...
ask the user for  more data files''.
...
\end{myquote}
\vspace{-3mm}
This means ``Use all accessable data files in the current directory''. 
...
\end{myquote}
\vspace{-3mm}
This means ``Use all accessable data files contained in the directory 
../data''.
...
detector), and $E_{\alpha}$ is an {\em exposure factor} which takes into
account any differences in exposure time, sky transparency, etc.
...
the measured signals in the flat-field exposure will be proportional to
...
$K_{e}$, then the measured signals in the flat-field images can be
written as:
...
exposure. Thus, the measured intensities after the flat-field correction
...
The target exposure $T_{45}$ is taken with an analyser angle 
of 90\dgs\ (accomplished by rotating the half-wave plate by 45\dgs), and
...
$T_{0}$, and $s_{5}$ and $s_{6}$ are the corresponding signals from
exposure $T_{45}$. In order to calculate the polarisation we need signals
...
{\em Note, each pair of target exposures must be flat-fielded using
{\em the same} master flat-field frame.}

\section{\label{APP:POLEXT}\xlabel{thepolpackndfextension}The POLPACK NDF Extension}
Figure Figure~\ref{fig:polext} lists the items which may be stored in
the POLPACK extension within a data file. The \htmlref{POLEXP}{POLEXP}
and \htmlref{POLIMP}{POLIMP} applications may be used to transfer these items
to and from a set of corresponding FITS keywords (see 
\hyperref{here}{section}{}{SEC:IMPORT}). The contents of the POLPACK
extension within an NDF may be examined using the
\xref{\verb+hdstrace+}{sun102} command.
\begin{latexonly}
  \begin{figure}[htb]
  \begin{center}
  \includegraphics[scale=0.8]{sun223_figures/polext.eps}
...
\begin{htmlonly}
   ...
   \begin{figure}[bhtp]
   \label{fig:polext}
   \includegraphics[scale=0.8,width=8in]{sun223_figures/polext.eps}
   \caption{The allowed items within the POLPACK extension.}
   \end{figure}
   \end{quote}
\end{htmlonly}
