...
/* Get a unique temporary file name. This file is used to pass values to
   the Polka Tcl script. All this complication is needed
   to avoid the warning message generated by the linker on RH 7 Linux 
   resulting from the use of the simpler "tmpnam" function. */
   strcpy( file_name, "doplka_fileXXXXXX" );
   ifd = mkstemp( file_name );
   if( ifd == -1 ){
      *STATUS = SAI__ERROR;
      Error( "Unable to create a temporary \"doplka_file\" file name.", 
              STATUS );
      return;
   } else {
      close( ifd );
...
/* Get another unique temporary file name. This file is used to collect
   ...
   strcpy( outfile_name, "doplka_file2XXXXXX" );
   ifd = mkstemp( outfile_name );
   if( ifd == -1 ){
      *STATUS = SAI__ERROR;
      Error( "Unable to create a temporary \"doplka_file2\" file name.", 
              STATUS );
      return;
   } else {
      close( ifd );
