...
data files are created. Once initiated (using the \htmlref{{\em
Save}}{POLKA_SAVE} item in the \htmlref{{\em File}}{POLKA_FILE_MENU}
...
\htmlref{{\em Remove Sky}}{POLKA_REMOVE_SKY} item in the \htmlref{{\em
Options}}{POLKA_OPTIONS_MENU} menu within the GUI. If sky subtraction is
...
The \htmlref{{\em Dump}}{POLKA_DUMP} command (available in the
\htmlref{{\em File}}{POLKA_FILE_MENU} menu) can be used to save
...
\htmlref{{\em Restore}}{POLKA_RESTORE} command. This facility enables
...
labelled \htmlref{{\em Current:}}{POLKA_CURRENT}. This will cause default
...
use the \htmlref{{\em Delete}}{POLKA_DELETE} button to delete the suspect
...
\htmlref{{\em Mapping Types}}{POLKA_MAP_TYPES} entry in the \htmlref{{\em
Options}}{POLKA_OPTIONS_MENU} menu.
...
you can simply press the \htmlref{{\em Accept}}{POLKA_ACCEPT} button to
...
\subsubsection {\mylabel{POLKA_DUMP}{\em Dump}} 
...
\htmlref{{\em Restore}}{POLKA_RESTORE} command. There are three
options:
...
\htmlref{{\em Restore}}{POLKA_RESTORE} command to restore the conditions 
...
\subsubsection {\mylabel{POLKA_RESTORE}{\em Restore}} 
This restores features lists, masks, mappings, etc, from a text file
created earlier using the \htmlref{{\em Dump}}{POLKA_DUMP} command.
\htmlref{{\em Restore}}{POLKA_RESTORE} command. There are three
options:
...
corresponding option in the \htmlref{{\em Dump}}{POLKA_DUMP} command. 
...
\subsubsection {\mylabel{POLKA_COPY}{\em Copy}} Stores copies of any
...
the \htmlref{{\em Paste}}{POLKA_PASTE} entry in the {\em Edit} menu.
...
required image will cause the \htmlref{{\em Edit a
mapping}}{POLKA_EDIT_MAPPING_DIALOG} dialog box to be displayed.

\subsubsection {\mylabel{POLKA_PASTE}{\em Paste}} Pastes any previously
copied polygons onto the display. This entry is disabled unless one or
more polygons have been copied using the \htmlref{{\em Copy}}{POLKA_COPY}
...
\subsubsection {\mylabel{POLKA_MAP_TYPES}{\em Mapping Types}} This displays a 
...
\subsubsection {\mylabel{POLKA_POLMODE}{\em polarimetry Mode}} This is used 
...
this item will result in the \htmlref{{\em Transfer features}}
...
\subsubsection {\mylabel{POLKA_ALIGN_EFFECT}{\em Align} } 
...
\subsubsection {\mylabel{POLKA_FILL_EFFECT}{\em Fill} }
...
\subsubsection {\mylabel{POLKA_FILTER_EFFECT}{\em Filter} } 
...
\subsubsection {\mylabel{POLKA_FITSKY_EFFECT}{\em Fit Sky} } 
...
options values (such as \htmlref{{\em Sky Order}}{POLKA_SKYORDER}). 

\subsubsection {\mylabel{POLKA_LOG_EFFECT}{\em Log} } 
Takes the log (base 10) of the difference between each pixel value and the
minimum pixel value in the displayed image.

\subsubsection {\mylabel{POLKA_MATHS_EFFECT}{\em Maths} } 
...
\subsubsection {\mylabel{POLKA_NEGATE_EFFECT}{\em Negate} } 
Multiplies every pixel value in the displayed image by minus one.

\subsubsection {\mylabel{POLKA_SMOOTH_EFFECT}{\em Smooth} }
Applies Gaussian smoothing to the displayed image.

\subsubsection {\mylabel{POLKA_STATS_EFFECT}{\em Stats} } 
...
\subsubsection {\mylabel{POLKA_THRESHOLD_EFFECT}{\em Threshold} } 
...
\subsubsection {\mylabel{POLKA_SHOW_EFFECTS}{\em Show Effects} }
Creates a dialog box containing a list of the effects applied to the 
currently displayed image.

\subsubsection {\mylabel{POLKA_UNDO_EFFECT}{\em Undo} } 
Removes the most recently applied effect from the currently displayed
image.

\subsubsection {\mylabel{POLKA_UNDO_ALL_EFFECTS}{\em Undo All} } 
...
\htmlref{{\em Search for...}}{POLKA_HSEARCH} entry in the {\em Help} menu.

\subsubsection {\mylabel{POLKA_HSEARCH}{\em Search for...}}
...
\htmlref{{\em Status Items}}{POLKA_STATUS_ITEMS} entry in the
\htmlref{{\em Options}}{POLKA_OPTIONS_MENU} menu. It can also be removed
entirely from the GUI by de-selecting the \htmlref{{\em Display Status
...
\subsection {\mylabel{POLKA_TRANSFER_DIALOG}The {\em Transfer features} Dialog Box}
...
selected image (using the current value of the \htmlref{{\em Feature
Size}}{POLKA_FEATURE_SIZE} option from the \htmlref{{\em Options}}
...
The \htmlref{{\em \% not white}}{POLKA_WHITE} and \htmlref{{\em \% black}}
{POLKA_BLACK} controls in the \htmlref{{\em Display Controls}}
...
\htmlref{{\em Lock Scaling}}{POLKA_LOCK_SCALING} button. If this is done,
...
Several operations (such as the \htmlref{{\em Zoom}}{POLKA_ZOOM} and
\htmlref{{\em Delete}}{POLKA_DELETE} buttons, and the \htmlref{{\em
...
changed using the \htmlref{{\em Options}}{POLKA_OPTIONS_MENU} menu). 

Any operation which uses a selection box will remove it upon completion,
but it may also be removed by pressing the \htmlref{{\em Cancel}}{POLKA_CANCEL}
button in the \htmlref{{\em Display Controls}}{POLKA_DISPLAY_CONTROLS} area
...
feature position easier to identify. If the \htmlref{{\em Feature
Size}}{POLKA_FEATURE_SIZE} has been set to a non-zero value in the
the \htmlref{{\em Options}} {POLKA_OPTIONS_MENU} menu, then the supplied
...
then use the \htmlref{{\em Dump}}{POLKA_DUMP}/{\em Mask Only} and
\htmlref{{\em Restore}}{POLKA_RESTORE}/{\em Mask Only} commands in the
\htmlref{{\em File}}{POLKA_FILE_MENU} menu. {\em Dump} creates a text file
...
\htmlref{{\em Dump}} {POLKA_DUMP}/{\em Displayed Image} command in the
\htmlref{{\em File}}{POLKA_FILE_MENU} menu. Then carry on to identify the 
required features and masks in the other images, and create the
aligned extracted images using the \htmlref{{\em File}}
...
from the saved file using the \htmlref{{\em Restore}}{POLKA_RESTORE}/{\em
Displayed Image} command in the \htmlref{{\em File}}{POLKA_FILE_MENU}
...
\htmlref{{\em File}} {POLKA_FILE_MENU}/{\em Exit} command as usual. Note,
...
Use the \htmlref{{\em Dump}}{POLKA_DUMP}/{\em Everything} and 
\htmlref{{\em Restore}}{POLKA_RESTORE}/{\em Everything} commands in the 
\htmlref{{\em File}}{POLKA_FILE_MENU} menu. {\em Dump} creates a text file
containing all the features positions, masks, mappings, etc, which you
have supplied so far. After using {\em Dump}, you can use \htmlref{{\em
...
use the {\em Transfer} command in the \htmlref{{\em Images}}{POLKA_IMAGES_MENU}
...
area} buttons is selected in the ``\htmlref{{\em Current:}}{POLKA_CURRENT}'' 
