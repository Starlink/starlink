...
*  type.
            CALL GRP_GET( WRDGRP( 1 ), I, 1, NAME1, STATUS )
            CALL GRP_GET( WRDGRP( 2 ), I, 1, NAME2, STATUS )
            CALL POL1_KNEXT( NAME1, YES, NAME3, STATUS )
            IF ( YES ) THEN
                     
*  Ok item is known, is it typing correct?
               IF ( NAME2 .NE. NAME3 ) THEN
                     
*  No, better supersede this.
                  CALL MSG_SETC( 'ITEM', NAME1 )
                  CALL MSG_SETC( 'OLDTYPE', NAME2 )
                  CALL MSG_SETC( 'NEWTYPE', NAME3 )
                  CALL MSG_OUT( ' ', '  Warning - data type of'//
     :' extension item ^ITEM incorrect (^OLDTYPE) superseded by '//
     :'(^NEWTYPE)', STATUS )
                     
*  Overwrite the old type.
                  CALL GRP_PUT( WRDGRP( 2 ), 1, NAME3, I, STATUS )
                  NAME2 = NAME3
               END IF
            ELSE IF ( STATUS .EQ. SAI__OK ) THEN 
...
*  first word which is the name of a FITS-keyword or a literal constant. 
*  Just check if this exists, if not add it to the list.       
               CALL GRP_GRPSZ( FITGRP( 1 ), NFITS, STATUS )
               LVAL = .FALSE.               
               DO 7 J = 1, NFITS            
                  CALL GRP_GET( FITGRP( 1 ), J, 1, NAME4, STATUS )
                  WORDS( 1 ) = NAME4
                  IF( START( 1 ) .LE. STOP( 1 ) ) THEN
...
*  It's a FITS-keyword! Enter into group. Otherwise, it will be assumed it 
