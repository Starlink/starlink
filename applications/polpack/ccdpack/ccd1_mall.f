...
*        block is exceeded.
*     5-DEC-1997 (DSB):
*        Return -2 if an error has already occurred. CCD1_MFREE also 
*        changed to ignore such pointer values. Thus, CCD1_MFREE will 
*        now not try to free memory if an error occurred before allocating 
*        it.
*     {enter_changes_here}
