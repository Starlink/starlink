...
*  installed version string.
      IF( STATUS .EQ. PAR__NULL ) THEN
         ...
         CALL MSG_SETC( 'V', BUF( : NC ) )
         CALL MSG_OUT( 'VERSION_MSG1', '^V', STATUS )
...
*  Report a context message if anything went wrong.
         IF( STATUS .NE. SAI__OK ) THEN
            ...
            CALL ERR_REP( 'VERSION_ERR4', 'The version string ''^V'','//
...
*  Write out the result.
         ...
         CALL MSG_OUT( 'VERSION_MSG2', '^I', STATUS )
...
*  Add a context report if anything went wrong.
      IF( STATUS .NE. SAI__OK ) THEN
         CALL ERR_REP( 'VERSION_ERR', 'VERSION: Failed to check the'//
