...
*  Purpose:
*     Find the centroid of a single position in an image.
...
*  Local Variables:
      ...
      REAL XIN                   ! Original guess at input position
      REAL XOUT                  ! Accurate position
      REAL YIN                   ! Original guess at input position
      REAL YOUT                  ! Accurate position
...
*  The initial position guess.
      CALL PAR_GET0R( 'XIN', XIN, STATUS )
      CALL PAR_GET0R( 'YIN', YIN, STATUS )
...
*  Transform the initial position guess to pixel indices.
      XPOS = DBLE( XIN - REAL( LBND( 1 ) ) + 1.5 )
      YPOS = DBLE( YIN - REAL( LBND( 2 ) ) + 1.5 )
...
*  Transform this position back to input coordinates.
      XOUT = REAL( XACC + DBLE( LBND( 1 ) ) - 1.5D0 ) 
      YOUT = REAL( YACC + DBLE( LBND( 2 ) ) - 1.5D0 )

*  Write out the results parameters.
      WRITE( TEXT, * ) XOUT, YOUT
      CALL MSG_OUT( ' ', TEXT, STATUS )
      CALL PAR_PUT0C( 'XYOUT', TEXT( : CHR_LEN( TEXT ) ), STATUS )
      CALL PAR_PUT0R( 'XOUT', XOUT, STATUS )
      CALL PAR_PUT0R( 'YOUT', YOUT, STATUS )
