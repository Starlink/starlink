C+
C               D S A _ _ E X P O S U R E _ N A M E
C
C  Routine name:
C     DSA__EXPOSURE_NAME
C
C  Function:
C     Returns the name of the data item holding an exposure time value.
C
C  Description:
C     This routine, given the reference slot number for a structure,
C     returns the DTA system name of the data item to hold an exposure
C     value for the observation from which the main data array
...
C
C  Call:
C     CALL DSA__EXPOSURE_NAME (REF_SLOT,NAME,LENGTH)
C
...
C                        exposure value for the structure.
...
C+
      SUBROUTINE DSA__EXPOSURE_NAME (REF_SLOT,NAME,LENGTH)
C
      IMPLICIT NONE
C
C     Parameters
...
C
     ...
     :                                     '.MORE.FIGARO.TIME'
      ...
      ELSE
         NAME=OBJ_NAMES(REF_SLOT)(:OBJ_LEN(REF_SLOT))//'.OBS.TIME'
