...
C
         ...
         CALL DSA_UNFLAG_DATA(NELM,.FALSE.,DATA_TYPE,DYNAMIC_MEM(DPTR),
     :                            DYNAMIC_MEM(QPTR),NFLAGGED,STATUS)
C
...
C
         IF (QUAL_TYPE.NE.'BYTE') THEN
            QPTR=DYN_ELEMENT(QUAL_ADDR)
            IF (QUAL_TYPE.EQ.'DOUBLE') THEN
               CALL VEC_BTOD(.FALSE.,NELM,DYNAMIC_MEM(TPTR),
     :            DYNAMIC_MEM(QPTR),IERR,NBAD,VECSTA)
            ELSE IF (QUAL_TYPE.EQ.'SHORT'.OR.QUAL_TYPE.EQ.'WORD') THEN
               CALL VEC_BTOW(.FALSE.,NELM,DYNAMIC_MEM(TPTR),
     :            DYNAMIC_MEM(QPTR),IERR,NBAD,VECSTA)
            ELSE IF (QUAL_TYPE.EQ.'INT') THEN
               CALL VEC_BTOI(.FALSE.,NELM,DYNAMIC_MEM(TPTR),
     :            DYNAMIC_MEM(QPTR),IERR,NBAD,VECSTA)
            ELSE IF (QUAL_TYPE.EQ.'FLOAT'.OR.QUAL_TYPE.EQ.'REAL') THEN
               CALL VEC_BTOR(.FALSE.,NELM,DYNAMIC_MEM(TPTR),
     :            DYNAMIC_MEM(QPTR),IERR,NBAD,VECSTA)
            ELSE IF (QUAL_TYPE.EQ.'USHORT'.OR.QUAL_TYPE.EQ.'UWORD') THEN
               CALL VEC_BTOUW(.FALSE.,NELM,DYNAMIC_MEM(TPTR),
     :            DYNAMIC_MEM(QPTR),IERR,NBAD,VECSTA)
            ELSE
               STATUS=DSA__INVTYP
            END IF
