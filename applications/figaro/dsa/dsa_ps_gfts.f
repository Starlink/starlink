C+
C                    D S A _ P O S T _ G E T _ F I T S
C
C  Routine name:
C     DSA_POST_GET_FITS
C
C  Function:
C     Performs post processing for the DSA_GET_FITS_{x} routines.
C
C  Description:
C     This routine is called by the DSA_GET_FITS_{x} routines,
C     and does little more than test the status of the DTA call
C     used to read the value.
C
C  Language:
C     FORTRAN
C
C  Call:
C     CALL DSA_POST_GET_FITS (DTA_STATUS,NAME,STATUS)
C
C  Parameters:   (">" input, "!" modified, "W" workspace, "<" output)
C
C     (>) DTA_STATUS       (Integer,ref) The DTA_ status returned by
C                          the call to read the FITS value.
...
C+
      SUBROUTINE DSA_POST_GET_FITS (DTA_STATUS,NAME,STATUS)
C
      IMPLICIT NONE
C
C     Parameters
...
C
      IF (DTA_STATUS.NE.0) THEN
         CALL DSA_WRUSER ('Error reading FITS data from ')
