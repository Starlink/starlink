C+
C               D S A _ _ C R E A T E _ E X T R A
C
C  Routine name:
C     DSA__CREATE_EXTRA
C
C  Function:
C     Creates the extension substructure for a data structure.
C
C  Description:
...
C
C  Call:
C     CALL DSA__CREATE_EXTRA (REF_SLOT,DTA_STATUS)
C
...
C     12th Mar  1990.   Original version was really creating the structure used
C                       for items connected with the main data array and not the
C                       more general extra structure. Original function is now
C                       performed by DSA__CREATE_DATA_EXTRA.  KS/AAO.
...
C+
      SUBROUTINE DSA__CREATE_EXTRA (REF_SLOT,DTA_STATUS)
C
      IMPLICIT NONE
C
C     Parameters
