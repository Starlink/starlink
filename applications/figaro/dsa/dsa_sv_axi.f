C+
C                        D S A _ S A V E _ A X I S
C
C  Routine name:
C     DSA_SAVE_AXIS
C
C  Function:
C     Saves an axis structure for possible restoration later.
C
C  Description:
C     Some Figaro applications transform the main data array in such a
C     way that the axis information is no longer applicable.  These could
C     just delete the old axis (using DSA_DELETE_AXIS), but it may be
C     that a subsequent application might reverse the transform and would
C     then want to restore the old axis information.  (FFT and BFFT are
C     an obvious example.)  This routine saves the information from the
C     axis of a structure in such a way that DSA_RESTORE_AXIS can restore
C     it later.  If the axis does not exist, then this routine does nothing.
C
C  Language:
C     FORTRAN
C
C  Call:
C     CALL DSA_SAVE_AXIS (REF_NAME,AXIS,STATUS)
C
...
C     DSA_AXIS_NAME      Get DTA name for axis structure
C     DSA_CHECK_MAPPING  Close down any inactive mappings for a structure
...
C+
      SUBROUTINE DSA_SAVE_AXIS (REF_NAME,AXIS,STATUS)
C
      IMPLICIT NONE
C
C     Parameters
...
C
C        It does exist, so we'll have to save it.
C        Here's a messy little aspect to moving stuff around like this.
...
C
     ...
     :                           'save the axis structure ',STATUS)
         IF (STATUS.NE.0) GO TO 500    ! Error exit
C
C        Now attempt to save the axis data
C
         CALL DSA__SAVE_AXIS (REF_SLOT,AXIS,'SAVE',FROM,TO,DTA_STATUS)
