C+
C                  D S A _ S E E K _ I M A G I N A R Y
C
C  Routine name:
C     DSA_SEEK_IMAGINARY
C
C  Function:
C     Determines whether or not an imaginary data array exists.
C
C  Description:
...
C     one whose size does not match that of the main data array.  It
C     will treat a non-matching imaginary array as not existing.
C
C  Language:
C     FORTRAN
C
C  Call:
C     CALL DSA_SEEK_IMAGINARY (REF_NAME,EXIST,STATUS)
C
C  Parameters:   (">" input, "!" modified, "W" workspace, "<" output)
C
C     (>) REF_NAME     (Fixed string,descr) The reference name associated
C                      with the structure.
C     (<) EXIST        (Logical,ref) True if there is an imaginary part
C                      of the main data array available.
...
C     DSA_WRUSER, DSA__IMAGINARY_NAME, ICH_LEN
C
...
C     DSA__IMAGINARY_NAME Get name of imaginary data array.
C     ICH_LEN          Position of last non-blank char in string.
C
C  Note:
C     This version supports both the original Figaro data structures, and
C     Starlink's NDF format.
C
C  History:
C     2nd Feb 1989.   Original version.  KS / AAO.
C     1st May 1990.   Modified to support additional data formats.  KS/AAO.
C     21st Aug 1992   Automatic portability modifications
C                     ("INCLUDE" syntax etc) made. KS/AAO
C     29th Aug 1992   "INCLUDE" filenames now upper case. KS/AAO
C+
      SUBROUTINE DSA_SEEK_IMAGINARY (REF_NAME,EXIST,STATUS)
C
      IMPLICIT NONE
C
C     Parameters
...
C
      IF ((STATUS.EQ.0).AND.EXIST) THEN
         CALL DSA_ARRAY_SIZE(OBJ_NAME(:LENGTH),10,NDIM,DIMS,
     :                                                  ERROR,STATUS)
     ...
     :                                                        STATUS)
         IF (STATUS.NE.0) THEN
            ...
            CALL DSA_WRUSER(
     :        'Unable to check dimensions of the imaginary part of the'
     :        //' data array against those of the real part in ')
  ...
  340          CONTINUE
            ...
            IF (.NOT.MATCH) THEN
               ...
               CALL DSA_WRUSER(STRUCTURE(:ICH_LEN(STRUCTURE)))
               CALL DSA_WRUSER(
     :            ' has a data array in which the real and imaginary '
     :            //'parts have different dimensions.')
               CALL DSA_WRFLUSH
               EXIST=.FALSE.
            END IF
         END IF
