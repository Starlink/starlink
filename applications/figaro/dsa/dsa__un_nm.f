C+
C                   D S A _ _ U N I T S _ N A M E
C
C  Routine name:
C     DSA__UNITS_NAME
C
C  Function:
C     Returns the name of the units string for the main data in a structure.
C
C  Description:
C     This routine, given the refernece slot number for a structure,
C     returns the DTA system name of the units string associated with
C     the structure data, should it contain one.  Whether or not the string
...
C
C  Call:
C     CALL DSA__UNITS_NAME (REF_SLOT,NAME,LENGTH)
C
...
C                        units string in the structure.
...
C+
      SUBROUTINE DSA__UNITS_NAME (REF_SLOT,NAME,LENGTH)
C
      IMPLICIT NONE
C
C     Parameters
...
C
      IF (NDF_FORMAT(REF_SLOT)) THEN
         NAME=OBJ_NAMES(REF_SLOT)(:OBJ_LEN(REF_SLOT))//'.UNITS'
         LENGTH=OBJ_LEN(REF_SLOT)+6
      ELSE
         NAME=OBJ_NAMES(REF_SLOT)(:OBJ_LEN(REF_SLOT))//'.Z.UNITS'
