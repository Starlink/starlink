C+
C                   D S A _ _ L A B E L _ N A M E
C
C  Routine name:
C     DSA__LABEL_NAME
C
C  Function:
C     Returns the name of the label string for the data in a structure.
C
C  Description:
C     This routine, given the reference slot number for a structure,
C     returns the DTA system name of the label string for the main
C     data array, should it contain one.  Whether or not the string
...
C
C  Call:
C     CALL DSA__LABEL_NAME (REF_SLOT,NAME,LENGTH)
C
...
C                        label string in the structure.
...
C+
      SUBROUTINE DSA__LABEL_NAME (REF_SLOT,NAME,LENGTH)
C
      IMPLICIT NONE
C
C     Parameters
...
C
      IF (NDF_FORMAT(REF_SLOT)) THEN
         NAME=OBJ_NAMES(REF_SLOT)(:OBJ_LEN(REF_SLOT))//'.LABEL'
         LENGTH=OBJ_LEN(REF_SLOT)+6
      ELSE
         NAME=OBJ_NAMES(REF_SLOT)(:OBJ_LEN(REF_SLOT))//'.Z.LABEL'
