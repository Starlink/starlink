C+
C                   D S A _ _ A X I S _ L A B E L _ N A M E
C
C  Routine name:
C     DSA__AXIS_LABEL_NAME
C
C  Function:
C     Returns the name of the label string for an axis in a structure.
C
C  Description:
C     This routine, given the refernece slot number for a structure,
C     returns the DTA system name of the label string for the
C     specified axis, should it contain one.  Whether or not the string
...
C
C  Call:
C     CALL DSA__AXIS_LABEL_NAME (REF_SLOT,AXIS,NAME,LENGTH)
C
...
C                        axis label string in the structure.
...
C     27th Feb  1990.   Support for NDF format files added. KS/AAO.
...
C+
      SUBROUTINE DSA__AXIS_LABEL_NAME (REF_SLOT,AXIS,NAME,LENGTH)
C
      IMPLICIT NONE
C
C     Parameters
...
C
     ...
     :                                           CHAR1//'].LABEL'
         LENGTH=OBJ_LEN(REF_SLOT)+14
      ELSE
         NAME=OBJ_NAMES(REF_SLOT)(:OBJ_LEN(REF_SLOT))//'.'//
     :                            AXIS_NAMES(AXIS:AXIS)//'.LABEL'
