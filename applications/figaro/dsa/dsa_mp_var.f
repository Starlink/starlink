C+
C                   D S A _ M A P _ V A R I A N C E
C
C  Routine name:
C     DSA_MAP_VARIANCE
C
C  Function:
C     Maps the variance data array in a structure.
C
C  Description:
C     This routine maps the variance data array in a structure,
C     returning the address of the mapped array.  The whole array is
C     mapped.  If there is in fact no variance data array, and no
C     alternative array that can be processed to supply the same
C     information (such as an error array) then an array of zeros is
...
C
C  Call:
C     CALL DSA_MAP_VARIANCE (REF_NAME,MODE,TYPE,ADDRESS,SLOT,STATUS)
C
...
C     DSA_SEEK_VARIANCE See if a variance data array exists.
C     DSA_ERR_TO_VARIANCE  Converts an error array to a variance array.
C     DSA_GET_WORK_ARRAY   Get workspace of specified size and type.
C     DSA__VARIANCE_NAME Get name of variance data object in a structure.
C     DSA__CREATE_ARRAY Creates an array of a given type and size.
C
C  History:
C     27th Jun 1989  Original version.  KS / AAO.
C     12th Mar 1990  Completely reworked to handle the case where the
C                    data structure actually has a variance array, and
C                    not rely on processing an uncertainty array mapped
C                    by DSA_MAP_ERRORS.  KS/AAO.
...
C+
      SUBROUTINE DSA_MAP_VARIANCE (REF_NAME,MODE,TYPE,ADDRESS,
     :                                                   SLOT,STATUS)
C
      IMPLICIT NONE
C
...
C
      ...
      CHARACTER NAME*128                    ! Name of dummy or variance array
...
C     is an exact mirror image of DSA_MAP_ERRORS.
C
...
C
C        The variance array either is required to exist or does exist.
...
C
     ...
     :                                            DIMS,TYPE,DTA_STATUS)
            IF (DTA_STATUS.NE.0) THEN
               ...
               CALL DSA_WRUSER(
     :                 'Error trying to create variance array in ')
               IGNORE=0
...
C           errors are all zero, so are the variances. Otherwise we have
C           to convert.
C
            IF (CHR.EQ.'W'.OR.(.NOT.EXIST)) THEN
               CALL DSA_ZFILL_ARRAY (NELM,WORK_ADDRESS,TYPE_UC,STATUS)
            ELSE
               CALL DSA_ERR_TO_VARIANCE (NELM,TYPE,ADDRESS,
     :                                         WORK_ADDRESS,ERRORS)
               IF (ERRORS.NE.0) THEN
     ...
     :                              'converting the error array in ')
                  IGNORE=0
                  CALL DSA_GET_ACTUAL_NAME (REF_NAME,STRUCTURE,
     :                                                        IGNORE)
                  CALL DSA_WRUSER (STRUCTURE(:ICH_LEN(STRUCTURE)))
                  CALL DSA_WRUSER (' into a variance array.')
                  CALL DSA_WRFLUSH
               END IF
            END IF
...
C
         ...
         NAME='variance array in '//STRUCTURE
