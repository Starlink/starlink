C+
C               D S A _ _ C R E A T E _ O B S _ E X T R A
C
C  Routine name:
C     DSA__CREATE_OBS_EXTRA
C
C  Function:
C     Creates the observation extension substructure for a data structure.
C
C  Description:
...
C    with the observation responsible for it (the exposure time, for example),
...
C
C  Call:
C     CALL DSA__OBS_DATA_EXTRA (REF_SLOT,DTA_STATUS)
C
...
C+
      SUBROUTINE DSA__CREATE_OBS_EXTRA (REF_SLOT,DTA_STATUS)
C
      IMPLICIT NONE
C
C     Parameters
...
C
      ...
      ELSE
C
C        In the original Figaro format, all observation information
C        should go in the .OBS substructure.  We see if that exists, and
C        attempt to create it if it doesn't.
C
         NAME=OBJ_NAMES(REF_SLOT)(:OBJ_LEN(REF_SLOT))//'.OBS'
         LENGTH=OBJ_LEN(REF_SLOT)+4
         CALL DTA_TYVAR (NAME,TYPE,DTA_STATUS)
         IF (DTA_STATUS.NE.0) THEN
