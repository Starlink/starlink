C+
C                   D S A _ _ A X I S _ W I D T H _ N A M E
C
C  Routine name:
C     DSA__AXIS_WIDTH_NAME
C
C  Function:
C     Returns the name of the width array for an axis in a structure.
C
C  Description:
C     This routine, given the reference slot number for a structure,
C     returns the DTA system name of the axis width data object for the
...
C
C  Call:
C     CALL DSA__AXIS_WIDTH_NAME (REF_SLOT,AXIS,NAME,LENGTH)
C
...
C                        width data object in the structure.
...
C+
      SUBROUTINE DSA__AXIS_WIDTH_NAME (REF_SLOT,AXIS,NAME,LENGTH)
C
      IMPLICIT NONE
C
C     Parameters
