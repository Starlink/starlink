      SUBROUTINE DSA_MAP_ERRORS( DSAREF, MODE, TYPE,
...
*  Purpose:
*     Map the error array in an NDF.

*  Language:
*     Starlink Fortran 77

*  Invocation:
*     CALL DSA_MAP_ERROR( DSAREF, MODE, TYPE, ADDRESS, MSLOT, STATUS )

*  Description:
*     This routine maps the error data array in an NDF If there is in
*     fact no variance array, then an array
...
*        Original version.
*     20 Jul 1988 (ks):
*        Call to DSA_MAP_ARRAY modified.
*     08 Sep 1989 (ks):
*        Call to DSA_MAP_ARRAY modified - propagation flag set false.
*     15 Dec 1989 (ks):
*        Now uses DSA__ERROR_NAME to get name of array.
*     17 Jan 1990 (ks):
*        Change to calling sequence for DSA__ERROR_NAME.
*     12 Mar 1990 (ks):
*        Now allows for the possibility that the error array is a
*        variance array, not an uncertainty array.
*     21 Aug 1992 (ks):
...
*     Write access or update access to absent variance.
     ...
     :         DSA__REFID1(SLOT), 'VARIANCE', STATUS )
            CALL NDF_MAP( DSA__REFID1(SLOT), 'ERROR', NDFTYP,
...
*     Read access to absent variance.
         ELSE IF ( MODEUC .EQ. 'R' .AND. .NOT. VTHERE ) THEN
            IF ( DSA__REFID3(SLOT) .NE. NDF__NOID ) THEN
               ...
               CALL ERR_REP( 'FDA_E019', 'DSA_MAP_ERRORS: Cannot ' //
     :            'access error array for reference ^FDA_T001, ' //
