      SUBROUTINE DSA_MATCH_SIZES( DSARE1, DSARE2, STATUS )
*+
*  Name:
*     DSA_MATCH_SIZES

*  Purpose:
*     Check that two NDFs match in all significant dimensions.

*  Language:
*     Starlink Fortran 77

*  Invocation:
*     CALL DSA_MATCH_SIZES( DSARE1, DSARE2, STATUS )

*  Description:
*     This routine is usually called prior to an operation performed
*     pixel-to-pixel on the main data arrays of two structures. It
*     checks that they have the same dimensions, issuing an error
*     message and returning a bad status value if they do not match.
*
*     Contrary to earlier implementations this routine gives no warning
*     any more if the formal dimensionalities differ, but the actual
*     dimensionalities are the same. This is the case where e.g. two
*     spectra might be given, but one is formally an NX-by-1 image.
...
*  Check for match.
      MATCH = NDIM1 .EQ. NDIM2
      IF ( MATCH ) THEN
         DO 3 I = 1, NDIM1
            IF ( DIM1(I) .NE. DIM2(I) ) MATCH = .FALSE.
 3       CONTINUE
      END IF

*  Set status and give message.
      IF ( .NOT. MATCH ) THEN
         STATUS = SAI__ERROR
         CALL MSG_SETC( 'FDA_T001', DSA__REFNAM(SLO1) )
         CALL MSG_SETC( 'FDA_T007', DSA__REFNAM(SLO2) )
         CALL ERR_REP( 'FDA_E033', 'DSA_MATCH_SIZES: The two ' //
