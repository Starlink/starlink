      SUBROUTINE DSA_FMTCON( BAD, CODE1, CODE2, ARRAY1,
     :   ARRAY2, NELM, NBAD )
*+
*  Name:
*     DSA_FMTCON
...
*  Purpose:
*     Convert an array from one data type to another.
...
*     another. The types are identified by integer codes, Conversion is
*     done by the VEC_xTOx routines.
...
*     ARRAY1( NELM ) = TYPE(CODE1) (Given)
*        The input array.
*     ARRAY2( NELM ) = TYPE(CODE2) (Returned)
*        The output array.
*     NELM = INTEGER (Given)
...
*        Original version.
*     24 Feb 1993 (KS):
*        Added definition of type 6 (UBYTE) and changed call so that
*        arrays are passed as such, rather than as pointers. This makes
*        the routine more suitable for calling from conventional
*        Fortran programs. Support for UBYTE arrays still has to be
*        added.
*     19 Feb 1996 (hme):
*        Use an error context and flush error reports.
*     {enter_changes_here}
...
*  Begin error context and initialise status.
      CALL ERR_MARK
      STATUS = SAI__OK
