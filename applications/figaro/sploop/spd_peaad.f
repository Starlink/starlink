      SUBROUTINE SPD_PEAAD( BAD, NELM, ARRAY, MINVAL, MAXVAL, STATUS )
*+
*  Name:
*     SPD_PEAA{DR}
...
*  Invocation:
*     CALL SPD_PEAAD( BAD, NELM, ARRAY, MINVAL, MAXVAL, STATUS )
...
*     MINVAL = DOUBLE PRECISION (Returned)
*        Minimum value found. 0 if not found.
*     MAXVAL = DOUBLE PRECISION (Returned)
...
*     27 Apr 1994 (hme):
*        Adapt from RANGER, make generic, copy to SPLOOP.
*     {enter_changes_here}
...
*     Return values in case of failure
         IF ( .NOT. FOUND ) THEN
            STATUS = SAI__ERROR
            CALL ERR_REP( 'SPD_PEAA_E01', 'SPD_PEAA: Error ' //
     :         'finding extrema. Given array has only bad values.',
     :         STATUS )
            GO TO 500
         END IF
