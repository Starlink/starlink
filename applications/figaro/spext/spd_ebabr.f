      SUBROUTINE SPD_EBABR( MAXDIM, SPAXIS,
...
*     SPVALS( DIM1, DIM2, ..., DIM7 ) = REAL (Given)
*        The input data, i.e. the array of pixel centres.
*     SPWIDS( DIM1, DIM2, ..., DIM7 ) = REAL (Returned)
...
*  Arguments Given:
      ...
      REAL SPVALS( DIM1, DIM2, DIM3,
     :   DIM4, DIM5, DIM6, DIM7 )

*  Arguments Returned:
      REAL SPWIDS( DIM1, DIM2, DIM3,
     :   DIM4, DIM5, DIM6, DIM7 )
...
*  Local Variables:
      ...
      REAL DELTA               ! Actual base for deriving width
...
*  Check MAXDIM.
      IF ( MAXDIM .NE. LMXDIM  ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPABJ_MXDIM', 'SPD_EBABR: Error: ' //
...
*  defined.
      ...
      IF ( DN(SPAXIS) .LE. 1 ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPABJ_UNDEF', 'SPD_EBABR: Error: ' //
