      SUBROUTINE SPD_EBAAR( MAXDIM, SPAXIS, ODIM, INELM, ONELM,
     :   VECTOR, ARRAY, STATUS )
*+
*  Name:
*     SPD_EBAA{DR}
...
*  Invocation:
*     CALL SPD_EBAAR( MAXDIM, SPAXIS, ODIM, INELM, ONELM,
*        VECTOR, ARRAY, STATUS )

*  Description:
*     This routine is a simplified version of SPABHR, which can grow
...
*     VECTOR( INELM ) = REAL (Given)
*        Source data vector.
*     ARRAY( ONELM ) = REAL (Returned)
...
*     24 Feb 1994 (hme):
*        Simplify SPABHR for 1-D input that fills the whole output
*        array.
...
*  Check MAXDIM.
      IF ( MAXDIM .NE. LMXDIM  ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPABH_MXDIM', 'SPD_EBAAR: Error: ' //
...
*  Check ODIM(SPAXIS).
      IF ( ODIM(SPAXIS) .NE. INELM ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPD_EBAA_E01', 'SPD_EBAAR: Error: Vector' //
