      SUBROUTINE SPD_EAGE( NDF, XLOC, ACCESS, TYPE,
...
*  Purpose:
*     Access existing covariance row sums.

*  Language:
*     Starlink Fortran 77

*  Invocation:
*     CALL SPD_EAGE( NDF, XLOC, ACCESS, TYPE, PNTR, ONDF, NELM, STATUS )
...
*     access a section with the same bounds as the main NDF.
...
*        The global status. This is set if the main NDF is a base NDF
*        and its shape does not match the Extension NDF.
...
*  Get the NDFs' bounds.
      ...
      CALL NDF_FIND( XLOC, XCMP8, ONDF, STATUS )
...
*  If main NDF is base, check bounds.
 ...
 1       CONTINUE
         IF ( .NOT. MATCH ) THEN
            STATUS = SAI__ERROR
            CALL ERR_REP( 'SPE_INCSHP', 'SPD_EAGE: Error: ' //
...
*  Map the array.
      IF ( ACCESS(:5) .EQ. 'WRITE' ) THEN
         CALL NDF_MAP( ONDF, 'DATA', TYPE, 'WRITE/ZERO',
     :      PNTR, NELM, STATUS )
      ELSE
