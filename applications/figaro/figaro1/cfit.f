...
C
      ...
      INTEGER CMPTAB(0:7),I,IER,IFAIL,IFAIL2,IOFF,IR(MAXPTS)
      INTEGER IPT,MNPT,NPT,NSTAT,INVB,NDEG
      REAL X(MAXPTS),XMAX,XMIN,Y(MAXPTS),W(MAXPTS)
      DOUBLE PRECISION A(4,4),AK(4),C(MAXPTS+4),DVAL,DX(MAXPTS)
      DOUBLE PRECISION DY(MAXPTS),K(MAXPTS+4),S(4),WRK(7*MAXPTS+8),EPS
