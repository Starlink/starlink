...
C     2nd Sep 1987.  DJA/ AAO.  Revised DSA_ routines - some specs changed.
C                    Now uses DYN routines for dynamic memory handling. Added
C                    logical unit as parameter to TRANSFER subroutine.
...
C                               ! of the output spectrum is 65536 pixels
C
      INTEGER  EOF
      PARAMETER (EOF=(-1))
C
...
C
      ...
      REAL         X1           !
      REAL         X2           !
      INTEGER      XPTR         ! Dynamic-memory pointer to output axis array
...
C
      CALL TRANSFER(LU,DYNAMIC_MEM(XPTR),DYNAMIC_MEM(OPTR),NX,X1,X2)
      WRITE(STRING,6) NX
    ...
    7 FORMAT('First X value = ',F9.3,'  Last X value = ',F9.3)
      CALL PAR_WRUSER(STRING,IGNORE)
      IF (.NOT.FIG_SCRCHK(NX,DYNAMIC_MEM(XPTR))) THEN
         CALL PAR_WRUSER(
