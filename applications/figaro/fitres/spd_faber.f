      SUBROUTINE SPD_FABER( A_MNDF, LINNAM, LABFRQ, CMPTYP, NPARA,
     :   PARTYP, COMP, STATUS )
*+
*  Name:
*     SPD_FABE{DR}
...
*  Invocation:
*     CALL SPD_FABER( MNDF, LINNAM, LABFRQ, CMPTYP, NPARA, PARTYP,
*        COMP, STATUS )
...
*  Find the slot.
 ...
 1    CONTINUE
      IF ( SLOT .LT. 1 ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPD_FABER_E01', 'SPD_FABER: Error: ' //
...
*  Check that the type is correct.
      IF ( TYPE(2,SLOT) .NE. TYPER ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPD_FABER_E02', 'SPD_FABER: Error: ' //
...
*     Component type.
         TESTC = SPD_FDABC( %VAL( CNF_PVAL(CPNTR(3,SLOT)) ), COMP,
     :                      STATUS, %VAL(CNF_CVAL(XCLEN)) )
         IF ( .NOT. CHR_SIMLR( CMPTYP, 'unknown function' ) .AND.
     :        .NOT. CHR_SIMLR( CMPTYP, TESTC ) ) GO TO 4

*     Laboratory frequency.
         IF ( LABFRQ .NE. VAL__BADR ) THEN
            TESTR = SPD_FDABR( %VAL( CNF_PVAL(CPNTR(2,SLOT)) ), COMP,
     :                         STATUS )
            IF ( TESTR .EQ. VAL__BADR ) THEN
               GO TO 4
            ELSE
               IF ( ABS( LABFRQ-TESTR ) .GT.
     :            TOLR * ABS( LABFRQ+TESTR ) ) GO TO 4
            END IF
         END IF
