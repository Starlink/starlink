      SUBROUTINE SPD_FACBR( A_MNDF, COMP, ROW, NPARA,
     :   DATA, VAR, STATUS )
*+
*  Name:
*     SPD_FACB{DR}
...
*  Invocation:
*     CALL SPD_FACBR( MNDF, COMP, ROW, NPARA, DATA, VAR, STATUS )
...
*  Find the slot.
 ...
 1    CONTINUE
      IF ( SLOT .LT. 1 ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPD_FACB_E01', 'SPD_FACBR: Error: ' //
     :      'No result structure accessed for that main NDF.', STATUS )
         GO TO 500
      END IF

*  Check that the type is correct.
      IF ( TYPE(1,SLOT) .NE. TYPER ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPD_FACB_E02', 'SPD_FACBR: Error: ' //
...
*  Check component exists and number of prameters matches.
      IF ( COMP .LT. 1 .OR. COMP .GT. NCOMP(SLOT) ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPD_FACB_E03', 'SPD_FACBR: Error: ' //
...
*  Check row number.
      IF ( ROW .LT. 1 .OR. ROW .GT. DNELM(SLOT)/TNPAR(SLOT) ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPD_FACB_E05', 'SPD_FACBR: Error: ' //
