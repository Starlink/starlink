      SUBROUTINE SPD_FABBD( A_MNDF, COMP, LINNAM, LABFRQ, CMPTYP,
     :   NPARA, PARTYP, STATUS )
*+
*  Name:
*     SPD_FABB{DR}
...
*  Check that NPARA is not negative.
      IF ( NPARA .LT. 0 ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPD_FABB_E01', 'SPD_FABBD: Error: ' //
...
*  Find the slot.
 ...
 1    CONTINUE
      IF ( SLOT .LT. 1 ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPD_FABB_E02', 'SPD_FABBD: Error: ' //
...
*  Check that the type is correct.
      IF ( TYPE(2,SLOT) .NE. TYPED ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPD_FABB_E03', 'SPD_FABBD: Error: ' //
...
*     deleted. That should get rid of it in the container file as well.
         CALL ERR_REP( 'SPD_FABB_E04', 'SPD_FABBD: Error ' //
