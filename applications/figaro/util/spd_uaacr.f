      SUBROUTINE SPD_UAACR( VARUSE, IDIM, IELM, OELM,
...
*  Invocation:
*     CALL SPD_UAACR( VARUSE, IDIM, IELM, OELM, BADVAL, IDIMS, ODIMS,
...
*        argument should be given as VAL__BADR.
...
*     INDAT( IELM ) = REAL (Given)
*        Data array to extract from.
*     INVAR( IELM ) = REAL (Given)
...
*  This loop is coded separately for VARUSE and .NOT.VARUSE.
     ...
     :                 VAR .NE. BADVAL .AND. VAR .NE. ZEROR ) THEN
                    OUTDAT(OIND) = OUTDAT(OIND) + DATA / VAR
                    OUTVAR(OIND) = OUTVAR(OIND) +  ONER / VAR
                 ENDIF
                 IIND = IIND + 1
                 OIND = OIND + OINC(1)
 4              CONTINUE
...
*  variances. This loop is coded separately for VARUSE and .NOT.VARUSE.
      IF ( VARUSE ) THEN
         DO 24 OIND = 1, OELM
            IF ( OUTVAR(OIND) .NE. ZEROR ) THEN
               OUTDAT(OIND) = OUTDAT(OIND) / OUTVAR(OIND)
               OUTVAR(OIND) =          ONER / OUTVAR(OIND)
            ELSE
               OUTDAT(OIND) = BADVAL
               OUTVAR(OIND) = BADVAL
               BADDAT       = .TRUE.
