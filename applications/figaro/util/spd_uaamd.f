      SUBROUTINE SPD_UAAMD( INDAT, OUTDAT, STAPIX, ENDPIX,
...
*  Invocation:
*     CALL SPD_UAAMD( INDAT, OUTDAT, STAPIX, ENDPIX,
*        MAXDIM, LDIM, ODIM, INELM, ONELM, STATUS )
...
*  Arguments:
*     INDAT( INELM ) = DOUBLE PRECISION (Given)
...
*     OUTDAT( ONELM ) = DOUBLE PRECISION (Returned)
...
*  Arguments Given:
      ...
      INTEGER LDIM( MAXDIM )
      INTEGER ODIM( MAXDIM )
      DOUBLE PRECISION INDAT( INELM )

*  Arguments Returned:
      DOUBLE PRECISION OUTDAT( ONELM )
...
*  Check MAXDIM.
      IF ( MAXDIM .NE. LMXDIM  ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPD_UAAM_MXDIM', 'SPD_UAAMD: Error: ' //
...
*  Check STAPIX, ENDPIX, LDIM.
 ...
 1    CONTINUE
      IF ( ERROR ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPD_UAAM_INCON', 'SPD_UAAMD: Error: ' //
