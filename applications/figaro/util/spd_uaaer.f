      SUBROUTINE SPD_UAAER( NX, X, NPAR, MODPAR, Y, STATUS )
*+
*  Name:
*     SPD_UAAE{DR}
...
*  Invocation:
*     CALL SPD_UAAER( NX, X, NPAR, MODPAR, Y, STATUS )
...
*        SPFFDT.
*     09 May 1994 (hme):
*        Adapted to become SPD_UAAER. Use Fortran function for Gauss.
...
*  Internal Referneces:
      REAL SPD_UAADR         ! Gauss function value
...
*  Outer loop is for vector element. Inner loop is for component.
      DO 2 I = 1, NX
         ...
         DO 1 C = 2, NPAR, 3
            Y(I) = Y(I) +
     :         SPD_UAADR( MODPAR(C), MODPAR(C+1), MODPAR(C+2), X(I) )
 1       CONTINUE
 2    CONTINUE
