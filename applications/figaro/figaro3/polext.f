...
C     2nd  Apr 1996.  BKM / RAL, Starlink. Correct bug due to uninitialised
C                     variable NDUD.    
...
C
      ...
      INTEGER    DLU           ! Logical unit number for dud fibres file
      LOGICAL    EOF           ! True when end of file reached
      REAL       EX            ! Extraction width
      LOGICAL    EXIST         ! True if IMAGE AXIS(1) structure exists
      LOGICAL    FAULT         ! True if fault discovered by non DSA methods
      ...
      INTEGER    IPDP          ! Dynamic memory element for poly. coeff.
      INTEGER    IPIN          ! Dynamic memory element for IMAGE data
      INTEGER    IPOUT         ! Dynamic memory element for OUTPUT data
      INTEGER    IPX           ! Dynamic memory element for TO DP X(NPTS)
      INTEGER    IPYL          ! Dynamic memory element for TO YLEVEL(NPTS)
      INTEGER    IPYR          ! Dynamic memory element for TO YRUN(NX)
      INTEGER    LENGTH        ! Used to format user messages
      INTEGER    NDEAD         ! Index for dud fibres
      INTEGER    NDIM          ! Number of dimensions
      INTEGER    NDUD          ! Number of dud fibres
      INTEGER    NELM          ! Number of elements in structure - ignored
      ...
      INTEGER    PLU           ! Logical unit number for polynomial file
      INTEGER    SLOT          ! Slot number for mapped data - ignored
      INTEGER    STATUS        ! Running status for DSA routines
      CHARACTER  TEXT*80       ! Used to format informational messages
      CHARACTER  TEXTL*132     ! Used to read first line from .pol file
...
C
      ...
      NPLUS1=NORDER+1
      BYTES=ID1(1)*BFLOAT+NPTS*BFLOAT+NPTS*BDOUB+2*NPLUS1*BDOUB
      CALL DSA_GET_WORKSPACE(BYTES,ADDRESS,SLOT,STATUS)
C      IPYR=DYN_ELEMENT(ADDRESS)
C      IF(STATUS.NE.0)GOTO 500
C      IPYL=IPYR+ID1(1)*BFLOAT
C      IPX=IPYL+NPTS*BFLOAT
C      IPDP=IPX+NPTS*BDOUB
      IF(STATUS.NE.0)GOTO 500
      IPX=DYN_ELEMENT(ADDRESS)
      IPDP=IPX+NPTS*BDOUB
      IPYR=IPDP+2*NPLUS1*BDOUB
      IPYL=IPYR+ID1(1)*BFLOAT
C
C     Do the actual extraction
C
      CALL POLEXT_WORK(ID1(1),ID1(2),DYNAMIC_MEM(IPIN),ID2(2),
     :                 DYNAMIC_MEM(IPOUT),DYNAMIC_MEM(IPYR),
     :                 DYNAMIC_MEM(IPYL),DYNAMIC_MEM(IPX),PLU,
     :                 NPLUS1,DYNAMIC_MEM(IPDP),NUMFIB,NPTS,NORDER,EX,
     :                 FIBOK,VERSION)
  500 CONTINUE
C
...
C     Parameters -
C
C     (>) FIBOK    (Logical array FIBOK(MAXFIB)) Flags if the fibres are OK.
