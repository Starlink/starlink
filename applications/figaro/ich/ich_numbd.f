C+
      INTEGER FUNCTION ICH_NUMBD(STRING,IST,DELIMS,VALUE,NSFIG,NEXT)
C
C     I C H _ N U M B D
C
...
C
C     (<) VALUE       (Double precision) The value of the number decoded.
C
C     (<) NSFIG       (Integer) The number of significant figures in
C                     the number - ie in the integer and fractional
C                     parts, excluding leading zeros in the integer part
C                     only.  This can be used to decide if the number
C                     needs to be maintained in double precision.
C
...
C     Modified:
C
C     29th Dec 1987  KS / AAO  Now accepts 'D' as well as 'E' for a
C                    decimal exponent.
C+
      IMPLICIT NONE
C
