C+
      SUBROUTINE DTA_MUVARS (NAME,NITEM,POINTER,STATUS)
C
C     D T A _ M U V A R S
C
C     Obtains a pointer to the start of a range of virtual
C     addresses which contains a named data object.  This
C     is generally by far the most efficient means of
...
C     the generic routine DTA_MUVAR, which maps data of type
C     SHORT.
C
...
C
C     DTA_MUVAR   (DTA_ package) Map data from data structure.
C
C                                      KS / CIT 5th Nov 1982
C     Modified:
C
...
C
C     TYP_DSSHORT   Code for SHORT data type
C
      INCLUDE 'DTATCON'
C
      CALL DTA_MUVAR(NAME,NITEM,TYP_DSSHORT,POINTER,STATUS)
C
      END
