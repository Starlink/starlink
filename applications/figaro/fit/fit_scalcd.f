C+
      SUBROUTINE FIT_SCALCD (DATA,NELM,CHECK,VMIN,VMAX,SCALES,
     :                                             ZEROS,ERRORS)
C
C     F I T _ S C A L C D
C
...
C     (<) SCALES   (Double pecision array SCALES(3)) Gives the
...
C
      LOGICAL CHECK
      INTEGER NELM
      REAL DATA(NELM),VMIN,VMAX,ERRORS(3)
      DOUBLE PRECISION SCALES(3),ZEROS(3)
C
C     Local variables
C
      INTEGER I,INTVAL,J
      REAL RVALUE,VALUE
      DOUBLE PRECISION RANGE,RSCALE(3)
C
      ...
      IF (CHECK) THEN
         ...
         DO I=1,NELM
            ...
            DO J=1,3
               ...
               RVALUE=(DBLE(INTVAL)*SCALES(J))+ZEROS(J)
