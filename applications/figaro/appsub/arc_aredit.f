C+
      SUBROUTINE ARC_AREDIT(ARC1,ARC2,ARC3,ARCS,NLARCS,CHANS,
...
C
      ...
      DO WHILE (EDIT)
         ...
         IF (EDIT) THEN
            LINE=ABS(VALUE)
            REPLY='Current wavelength is '
            INVOKE=ICH_ENCODE(REPLY,WAVES(LINE),23,6,NEXT)
            CALL PAR_WRUSER(REPLY(:NEXT-1),STATUS)
            CALL ARC_ARGETW(ARC1,ARC2,ARC3,ARCS,NLARCS,0.,0.,FORGET,
     :                                                     WAVEL)
            IF (PAR_ABORT()) RETURN
            IF (FORGET) THEN
C
...
C
      ...
      DO LINE=NLI,1,-1
         IF (WAVES(LINE).LT.0.) THEN
            CALL ARC_ARDELE(LINE,CHANS,WAVES,WEIGHTS,CLASS,NLID)
         END IF
      END DO
C
      RETURN
