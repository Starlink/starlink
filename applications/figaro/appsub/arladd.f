C+
      SUBROUTINE ARLADD (CENT,WAVEL,NX,ZVALS,SIGMA,NLMAX,CHANS,
...
C     11 Mar 1988  KS / AAO.  Now uses GKD_ routines for dialogue.
C     18 Feb 1991  KS / AAO.  Now uses GEN_SIMILAR to test for matching
C                  vavelengths. (Test to within 0.05 was not
C                  suitable for wavelengths in microns.)
C     23 Jul 1993  HME / UoE, Starlink.  Disuse PAR_Q*.
C+
      IMPLICIT NONE
C
C     Parameters -
...
C
      IF (NLID.GE.NLMAX) THEN
         CALL GKD_WRITE_LINE('Line tables full. Cannot add line')
         FORGET=.TRUE.
      ELSE
C
...
C
         ...
         MATCH=0
         DO I=1,NLID
            IF (GEN_SIMILAR(WAVES(I),WAVEL)) THEN
               REPLY='Line already identified at channel '
               STATUS=ICH_ENCODE(REPLY,CHANS(I),36,2,NEXT)
...
C
            REPLY='Re-analyse line centers assuming a '
            REPLY='Shift: re-analyse line centers assuming a '
            SHIFT=CENT-CHANS(MATCH)
            STATUS=ICH_ENCODE(REPLY,SHIFT,43,2,NEXT)
