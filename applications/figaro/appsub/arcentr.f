C+
      SUBROUTINE ARCENTR(XVALS,ZVALS,NX,IX,CENT,XCENT)
C
C     A R C E N T R
C
...
C     GKD_WRITE_LINE  ( "      "   ) Output string to user.
C
C                                               KS / CIT 13th June 1983
C     Modified:
C
C     11th Mar 1988  KS / AAO.  Now uses GKD_ routines.
...
C
      LOGICAL OK
      INTEGER I,IXEN,IXST,KX,KX1,KX2
...
C
     ...
     :            'Select 2 points with the cursor giving the X range')
         CALL GKD_WRITE_LINE('The lowest point gives the Y level')
         X1=XVALS(IX)
         Y1=(HIGH+LOW)*.5
...
C
         ...
         END DO
         IF (SUMY.LE.0.) THEN
            CALL GKD_WRITE_LINE('No data in line')
         ELSE
