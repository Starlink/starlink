C+
      SUBROUTINE ARC_ARAUTO(ZVALS,NX,SIGMA,ARC1,ARC2,ARC3,NLARCS,NLMAX,
     :           NCOEFF,COEFFS,PARMS,NLID,CHANS,WAVES,WEIGHTS,CLASS)
C
C     A R A U T O
C
...
C
C     WFL / AAO 31st May 1988. Attempt to avoid crash if NLID is too small.
C+
      IMPLICIT NONE
C
C     Parameters
C
      INTEGER NX, NLARCS, NLMAX, NLID, NCOEFF, CLASS(NLMAX)
...
C
      IF (NLID.LE.2) THEN
         RMS = 0.0
      ELSE
         CALL ARC_ARFITX(0,CHANS,WAVES,WEIGHTS,NLID,NCOEFF,RMS)
      END IF
      IF (RMS.LE.0.) THEN
         ICEN=NX/2
         WDELTA=(GEN_EPOLYD(DBLE(ICEN),COEFFS,NCOEFF)
     :                -GEN_EPOLYD(DBLE(ICEN+1),COEFFS,NCOEFF))*0.5
      ELSE
         WDELTA=RMS*PARMS(2)
      END IF
...
C
                  IF (FOUND) THEN
                     STRING='Line at '
                     INVOKE=ICH_ENCODE(STRING,WAVEL,9,6,NEXT)
                     STRING(NEXT:)=' found, channel '
                     INVOKE=ICH_ENCODE(STRING,CENTER,NEXT+16,4,NEXT)
