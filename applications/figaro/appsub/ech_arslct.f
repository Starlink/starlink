C+
      SUBROUTINE ECH_ARSLCT(XVALS,ZVALS,NX,ARC1,ARC2,ARC3,NLARCS,
     :                       HIGH,LOW,SIGMA,NLMAX,ARCS,IXST,IXEN,
     :                  NCHAN,NC,COEFFS,NCOEFF,FITTED,SHOWRMS,XS,
     :                  CHANS,WAVES,WEIGHTS,CLASS,NLID,COMPLETE)
C
...
C
C     Stolen & Modified --> ECH_ARSLCT:     JKM / CIT 11th Dec 1986
C
C+
      IMPLICIT NONE
C
...
C
      ...
      INTEGER INVOKE,NX,NLARCS,NLMAX,IXST,IXEN,NCHAN,NLID,NCOEFF,NC
      INTEGER CLASS(NLMAX)
...
C
      LOGICAL PAR_ABORT
      INTEGER ICH_ENCODE,ICH_FOLD,GEN_BSEARCH
      INTEGER ARC_ARID
...
C
         ...
         IF (GOTRMS) THEN
            ...
            IF (NLID.GE.NCOEFF) THEN
               REPLY(NEXT:)=' Order of fit = '
               INVOKE=ICH_ENCODE(REPLY,FLOAT(NCOEFF-1),NEXT+16,0,NEXT)
            ELSE
               REPLY(NEXT:)=' Specified order = '
               INVOKE=ICH_ENCODE(REPLY,FLOAT(NCOEFF-1),NEXT+19,0,NEXT)
               REPLY(NEXT:)=', order used = '
               INVOKE=ICH_ENCODE(REPLY,FLOAT(NLID-1),NEXT+15,0,NEXT)
            END IF
            CALL PAR_WRUSER(REPLY(:NEXT),STATUS)
         END IF
         CALL PAR_WRUSER('Use cursor to select an arc line',STATUS)
         CALL PGCURSE(X,Y,CH)
         LENGTH=ICH_FOLD(CH)
C
...
C
            WANTED=.FALSE.
            CENT=IX
            ID=ARC_ARID(CHANS,NLID,CENT,VALUE)
            IF ((ABS(VALUE-CENT).GT.(3.*SIGMA)).OR.(ID.EQ.0)) THEN
               CALL PAR_WRUSER('No identified line at that position',
     :                                                   STATUS)
            ELSE
C
...
C
               ...
               IF (.NOT.XS) THEN
                  IF (CLASS(ID).NE.0) THEN
                     CHARS='___ ('
                     INVOKE=ICH_ENCODE(CHARS,WAVES(ID),6,6,NEXT)
                     CHARS(NEXT:NEXT)=')'
                     NEXT=NEXT+1
                  ELSE
                     CHARS='___ '
                     INVOKE=ICH_ENCODE(CHARS,WAVES(ID),5,6,NEXT)
                  END IF
               END IF
               CHLEN=NEXT-1
               IF (WAVED) THEN
     ...
     :                                   (XVALS(IX+1)-XVALS(IX))
               ...
               IF (XS) THEN
                  ...
                  END IF
               ELSE
                  CALL PGPTEXT(XV,YP,90.0,0.0,CHARS(:CHLEN))
               END IF
               CALL PGSCI(1)
...
C
                  CALL ARC_ARDELE(ID,CHANS,WAVES,WEIGHTS,CLASS,NLID)
                  CHANGE=.TRUE.
               ELSE
C
C                 OOps! Wrong line.  Restore identification.
C
                  CALL PAR_WRUSER('Sorry',STATUS)
                  IF (XS) THEN
                     IF (CLASS(ID).EQ.0) THEN
                        CALL PGPOINT(1,XV,YP,ICHAR('X'))
                     ELSE
...
C
            ...
            CALL PAR_RDVAL('ORDERFIT',0.,10.,FLOAT(NCOEFF-1),' ',VALUE)
            IF (PAR_ABORT()) RETURN
            NCOEFF=VALUE+1
            CHANGE=.TRUE.
            WANTED=.FALSE.
...
C
            CALL ARC_ARCENTR(XVALS,ZVALS,NX,IX,CENT,XCENT)
            IF (PAR_ABORT()) RETURN
            FOUND=.TRUE.
            DONE=.TRUE.
            WANTED=.FALSE.
...
C
            CALL ARC_ARPOSN(XVALS,ZVALS,NX,IX,CENT,XCENT)
            FOUND=.TRUE.
            DONE=.TRUE.
            SHOWRMS=.TRUE.
            WANTED=.FALSE.
...
C
            ...
            END IF
            IF (.NOT.FOUND) THEN
               CALL PAR_WRUSER('No obvious line near cursor position',
     :                                                      STATUS)
            ELSE
C
...
C
            ...
            IF (NLID.GE.2) THEN
               REPLY(NEXT:)=' ( interp: '
               CALL ARC_ARGUESS(CHANS,WAVES,NLID,CENT,VINT)
               STATUS=ICH_ENCODE(REPLY,VINT,NEXT+12,6,NEXT)
               REPLY(NEXT:)=')'
            END IF
            CALL PAR_WRUSER(REPLY,STATUS)
            CALL ARC_ARGETW(ARC1,ARC2,ARC3,ARCS,NLARCS,VINT,VFIT,
     :                                            FORGET,WAVEL)
            IF (PAR_ABORT()) RETURN
C
...
C
            IF (.NOT.FORGET) THEN
               CALL ARC_ARLADD(CENT,WAVEL,NX,ZVALS,SIGMA,NLMAX,CHANS,
...
C
               ...
               IF (WANTED) THEN
                  ...
                  CALL PGSCI(1)
                  REPLY(1:)='___ '
                  INVOKE=ICH_ENCODE(REPLY,WAVEL,5,6,NEXT)
                  CALL PGPTEXT(XV,YP,90.0,0.0,REPLY(:NEXT-1))
               END IF
            END IF
         END IF
C
...
C
         IF (CHANGE.AND.(NLID.GT.2)) THEN
            CALL ARC_ARIFIT(CHANS,WAVES,WEIGHTS,NLID,COEFFS,
     :                                       MIN(NLID,NCOEFF),RMS)
            GOTRMS=.TRUE.
            FITTED=.TRUE.
         ELSE
