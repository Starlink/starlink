C+
      SUBROUTINE FIG_FIXCOLSQ (DATA,QUAL,NX,NY,LX1,LX2,LY1,LY2,NCOL,
     :                                      ICOLS,NCOEFF)
C
C     F I G _ F I X C O L S Q
C
...
C
C     FIG_HORIZONTALQ (FIG_ package) Fix a single element by a horiz fit.
C
C     Adapted 1998 from FIG_FIXCOLS.
C
...
C
            ...
            LIMX1=COL-2*NCOEFF
            DO IY=LY1,LY2
               CALL FIG_HORIZONTALQ(DATA,QUAL,NX,NY,IY,COL,COL,LIMX1,
     :                             LIMX2,NCOEFF,MAXPT,X,Z,STATUS)
            END DO
         END IF
      END DO
C
