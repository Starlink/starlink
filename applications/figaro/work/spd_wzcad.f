      SUBROUTINE SPD_WZCAD( BIAS, NELM, XDATA, YDATA, VAR,
     :   MOMENT, MVAR, STATUS )
*+
*  Name:
*     SPD_WZCA{DR}
...
*  Invocation:
*     CALL SPD_WZCAD( BIAS, NELM, XDATA, YDATA, VAR,
*        MOMENT, MVAR, STATUS )
...
*        handled properly.
*     VAR( NELM ) = DOUBLE PRECISION (Given)
...
*     MVAR( 15 ) = DOUBLE PRECISION (Returned)
...
*  Arguments Given:
      ...
      DOUBLE PRECISION   VAR( NELM )

*  Arguments Returned:
      DOUBLE PRECISION MOMENT( 15 )
      DOUBLE PRECISION   MVAR( 15 )
...
*     ignore x with bad y or bad var(y)
         IF ( LMOMEN(1) .EQ. VAL__BADD ) THEN
            LMOMEN(1) = XDATA(I)
         ELSE IF ( YDATA(I) .NE. VAL__BADD .AND.
     :               VAR(I) .NE. VAL__BADD       ) THEN
...
*  retaining bad values.
      ...
      CALL VEC_DTOD( .TRUE., 15, LMVAR,  MVAR,   IERR, NERR, STATUS )
