      SUBROUTINE SPD_WZPDR( VARUSE, KMAX, LMAX, IK, VK,
     :   MAT, IL, STATUS )
*+
*  Name:
*     SPD_WZPD{DR}
...
*  Invocation:
*     CALL SPD_WZPDR( VARUSE, KMAX, LMAX, IK, VK, MAT, IL, STATUS )
...
*     MAT( KMAX, LMAX ) = REAL (Given and Returned)
*        On entry this is the weight matrix. On exit it is the
*        coefficient matrix.
*     IL( LMAX ) = REAL (Returned)
...
*  Local Variables:
      ...
      REAL WEIGHT              ! Weight
      REAL SUMK                ! Sum of weights
      REAL VALK                ! Input data value
      REAL VALL                ! Output data value
...
*        the matrix.
            DO 1 K = 1, KMAX
               VALK   = IK(K)
               WEIGHT = MAT(K,L)
               IF (   VALK .EQ. VAL__BADR .OR.
     :               VK(K) .EQ. VAL__BADR ) THEN
                  MAT(K,L) = 0.
               ELSE
                  SUMK = SUMK + WEIGHT
