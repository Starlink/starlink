      SUBROUTINE SPD_WZDC( LOGUSE, FILENO, MSKDIM, FITPAR, FITDIM,
     :   IN, OELM,
...
*  Purpose:
*     Report FITTRI settings/results to screen or file.

*  Language:
*     Starlink Fortran 77

*  Invocation:
*     CALL SPD_WZDC( LOGUSE, FILENO, MSKDIM, FITPAR, FITDIM,
*        IN, OELM,
*        VARUSE, MSKUSE, MSKELM, XMIN, XMAX, MASK,
*        NCOMP, CONT, CFLAGS, PFLAGS, SFLAGS, CENTRE, PEAK, FWHM,
*        COVAR, CHISQR, STATUS )
...
*  Description:
*     Report current settings of FITTRI to screen or log file.
...
*     NCOMP = INTEGER (Given)
*        The value of NCOMP is the number of triangle components.
...
*        Half power width for each component. This routine will however
...
*  Local Variables:
      ...
      REAL FLUX( MAXTRI )        ! Line fluxes
      REAL VARIAN( 4*MAXTRI )    ! Variances of the parameters
      REAL VARSCL                ! CHISQR or 1.
      REAL COVPW                 ! Covariance between peak and FWHM
      CHARACTER * ( 10 )
     :   PARNAM( 3*MAXTRI )      ! Names of the free parameters
      CHARACTER * ( 10 )
...
*  This part only if there are triangles to be reported.
      IF ( NCOMP .GT. 0 ) THEN
...
*        We leave the message in here, hoping to never see it again.
            IF ( VARIAN(4*I) .LT. 0. ) THEN
               VARIAN(4*I) = 0.
               CALL MSG_SETC(   'MESS',
     :            'FITTRI: Negative variance. Set to 0.' )
               CALL MSG_OUT ( 'FITTRI_NEGVAR', ' ', STATUS )
            END IF
...
*     General: Title, files.
         CALL MSG_SETC( 'MESS', ' ' )
         CALL MSG_OUT ( 'FITTRI_REPORT', '^MESS', STATUS )
...
*        Fit results.
     ...
     :            SQRT(VARIAN(4*I-2)), SQRT(VARIAN(4*I-1)),
     :            SQRT(VARIAN(4*I))
               CALL MSG_SETC( 'MESS', STRING )
               CALL MSG_OUT ( 'FITTRI_REPORT', '^MESS', STATUS )
 10         CONTINUE
