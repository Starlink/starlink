      SUBROUTINE SPD_WAAAD( VARUSE, COVRSX, INELM, MSKDIM,
...
*  Invocation:
*     CALL SPD_WAAAD( VARUSE, COVRSX, INELM, MSKDIM,
*        INX, INDAT, INVAR, INCVRS,
*        MSKUSE, MASK, MSKELM, MXDWC, STATUS )
...
*     MXDWC( 4*INELM ) = DOUBLE PRECISION (Returned)
...
*     22 Apr 1994 (hme):
*        Adapt from SPABXD. Simplify: no ADAM parameters, no
...
*  Arguments Returned:
      INTEGER MSKELM
      DOUBLE PRECISION MXDWC( 4*INELM )
...
*  Error handling.
      IF ( MSKELM .LT. 1 ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPD_WAAA_E01', 'SPD_WAAAD: ' //
