      SUBROUTINE SPD_WZPFD( PROPCO, LMAX, INL, CNLM,
...
*  Invocation:
*     CALL SPD_WZPFD( PROPCO, LMAX, INL, CNLM,
*        VECI, SIL, SILSQ, SCLM, STATUS )
...
*     INL( LMAX ) = DOUBLE PRECISION (Given)
*        The vector to be included in the weighted sum.
*     CNLM( LMAX, LMAX ) = DOUBLE PRECISION (Given)
...
*     SIL( LMAX ) = DOUBLE PRECISION (Given and Returned)
*        The sum vector.
*     SILSQ( LMAX ) = DOUBLE PRECISION (Given and Returned)
*        The vector of the sums of squared components.
*     SCLM( LMAX, LMAX ) = DOUBLE PRECISION (Given and Returned)
*        The sum matrix.
...
*  Arguments Given:
      LOGICAL PROPCO
      INTEGER LMAX
      DOUBLE PRECISION INL( LMAX )
      DOUBLE PRECISION CNLM( LMAX, LMAX )

*  Arguments Given and Returned:
      INTEGER VECI( LMAX )
      DOUBLE PRECISION SIL( LMAX )
      DOUBLE PRECISION SILSQ( LMAX )
...
*  Local Variables:
      ...
      DOUBLE PRECISION VALINL              ! Data value
...
*        be entered either.
            VALINL = INL(L)
            VALCLL = CNLM(L,L)
            IF (  VALINL .NE. VAL__BADD .AND.
     :            VALCLL .NE. VAL__BADD .AND.
     :            VALCLL .NE. 0. ) THEN
...
*           diagonal elements.
               VALCMM = CNLM(M,M)
               VALCLM = CNLM(M,L)
               IF (  VALCLM .NE. VAL__BADD .AND.
     :               VALCLL .NE. VAL__BADD .AND.
     :               VALCMM .NE. VAL__BADD .AND.
