      SUBROUTINE SPD_WZPED( VARUSE, KMAX, LMAX, VK, COEFF,
     :   CLM, STATUS )
*+
*  Name:
*     SPD_WZPE{DR}
...
*  Invocation:
*     CALL SPD_WZPED( VARUSE, KMAX, LMAX, VK, COEFF, CLM, STATUS )
...
*        The given vector.
*     COEFF( KMAX, LMAX ) = DOUBLE PRECISION (Given)
*        The given matrix.
*     CLM( LMAX, LMAX ) = DOUBLE PRECISION (Returned)
*        The result matrix.
...
*  Arguments Given:
      ...
      DOUBLE PRECISION COEFF( KMAX, LMAX )
...
*  Local Variables:
      ...
      DOUBLE PRECISION SUMK                ! Sum over a coefficient row
      DOUBLE PRECISION WEIGHT              ! Addend from a row element
      DOUBLE PRECISION VALLM               ! Value for returned matrix
