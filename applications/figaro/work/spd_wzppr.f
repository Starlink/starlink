      SUBROUTINE SPD_WZPPR( INFO, VARUSE, USEEXT, KMAX, LMAX, DIM2,
...
*  Invocation:
*     CALL SPD_WZPPR( INFO, VARUSE, USEEXT, KMAX, LMAX, DIM2,
*        XK, WK, IK, VK, XL, WL, OKL, CLM, IL, VL, CRSL, STATUS )
...
*  Arguments Given:
      ...
      INTEGER LMAX
      INTEGER DIM2
      REAL XK( KMAX, DIM2 )     ! May actually be KMAX by 1
      REAL WK( KMAX, DIM2 )     ! May actually be KMAX by 1
      REAL IK( KMAX, DIM2 )
      REAL VK( KMAX, DIM2 )
      REAL XL( LMAX )
      REAL WL( LMAX )
...
*           input and output pixels.
               CALL SPD_WZPCR( INFO, KMAX, LMAX, XK(1,I), WK(1,I),
     :            XL, WL, OKL, STATUS )
...
*           The switch NEEDC is ignored and assumed true.
               CALL SPD_WZPKR( LMAX, CLM, CRSL(1,I),
     :            NEEDC, STATUS )

*           Progress report.
               IF ( INFO ) THEN
                  CALL MSG_SETI( 'SPD_WZPPR_T01', I )
                  CALL MSG_OUT( 'SPD_WZPPR_M01',
     :               '^SPD_WZPPR_T01 rows processed so far.', STATUS )
               END IF
 2          CONTINUE
...
*           Calculate the matrix of overlaps between pixels of i-th
*           input and output pixels.
               CALL SPD_WZPCR( INFO, KMAX, LMAX, XK, WK,
     :            XL, WL, OKL, STATUS )
...
*           Resample data values of i-th input.
               CALL SPD_WZPDR( .FALSE., KMAX, LMAX, IK(1,I), ZEROR,
     :            OKL, IL(1,I), STATUS )
