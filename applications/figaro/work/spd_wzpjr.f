      SUBROUTINE SPD_WZPJR( INFO, LMAX, VECI, IL,
...
*  Invocation:
*     CALL SPD_WZPJR( INFO, LMAX, VECI, IL, VEC, MAT, GOODC, GOODV,
*        STATUS )
...
*  Local Variables:
      ...
      REAL VALIL               ! Data value
      REAL VALVEC              ! Vector component
      REAL VALMLL              ! Matrix diagonal element
      REAL VALMMM              ! Matrix diagonal element
      REAL VALMLM              ! Matrix element
      REAL SUML1               ! A sum over L
      REAL SUML2               ! Another sum over L
      REAL V0                  ! The global input variance
...
*     If this pixel can make a contribution to variance.
     ...
     :         VALIL  .NE. VAL__BADR .AND.
     :         VALVEC .NE. VAL__BADR .AND.
     :         VALMLL .NE. VAL__BADR .AND.
     :         VALMLL .NE. VAL__ZEROR ) THEN

*        Output variance.
*        This must not be negative.
            VEC(L) = ( VALVEC / VALMLL - VALIL * VALIL  )
     :               / ( COUNTL - 1. )
            VEC(L) = MAX( VEC(L), VAL__ZEROR )
            GOODV  = .TRUE.
...
*  covariance.
      IF ( SUML1 .GT. VAL__ZEROR ) THEN
         GOODC = .TRUE.
         V0    = SUML2 / SUML1
         IF ( INFO ) THEN
            CALL MSG_SETR( 'SPD_WZPJR_T01', V0 )
            CALL MSG_OUT( 'SPD_WZPJR_M01', 'The global variance ' //
     :         'in the input NDFs was ^SPD_WZPJR_T01.',
     :         STATUS )
         END IF
      ELSE IF ( INFO ) THEN
         GOODC = .FALSE.
         CALL MSG_OUT( 'SPD_WZPJR_M02',
...
*           (Bad diagonal may be signalled by zero counter.)
     ...
     :               VALMMM .EQ. VAL__BADR .OR.
     :               VALMLL .EQ. VAL__BADR ) THEN
                  MAT(M,L) = 0.
