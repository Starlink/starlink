      SUBROUTINE SPD_CZPHD( INFO, VARUSE, KMAX, XNK, DTYPE, ATYPE,
...
*  Invocation:
*     CALL SPD_CZPHD( INFO, VARUSE, KMAX, XNK, DTYPE, ATYPE, INDF, ONDF,
...
*        axis 1 is mapped.
*     17 May 1993 (hme):
*        Use MSG_SETD rather than always the one for real.
*     17 Sep 1993 (hme):
...
*  Get start, end and size of input NDF. Report it to the user.
      ...
      IF ( INFO ) THEN
         CALL MSG_SETD( 'SPD_CZPHD_T01', XSTART )
         CALL MSG_SETD( 'SPD_CZPHD_T02', XEND   )
         CALL MSG_SETI( 'SPD_CZPHD_T03', KMAX   )
         CALL MSG_OUT(  'SPD_CZPHD_M01', 'Abscissa grid in first ' //
     :      'input row is from ^SPD_CZPHD_T01 ' //
     :      'to ^SPD_CZPHD_T02 in ^SPD_CZPHD_T03 pixels.', STATUS )
      END IF
