      SUBROUTINE SPD_CZCBD( XISND, VARUSE, MNDF, COMP,
...
*  Invocation:
*     CALL SPD_CZCBD( XISND, VARUSE, MNDF, COMP, XNELM, NROWS, ROWLEN,
...
*        constant).
*     XDATA( XNELM ) = DOUBLE PRECISION (Given)
*        The array of x values. This can be a vector of length ROWLEN or
*        an array of size ROWLEN*NROWS.
*     YDATA( ROWLEN, NROWS ) = DOUBLE PRECISION (Given)
*        The array of y values.
*     VAR( ROWLEN, NROWS ) = DOUBLE PRECISION (Given)
...
*  Arguments Given:
      ...
      DOUBLE PRECISION XDATA( XNELM )
      DOUBLE PRECISION YDATA( ROWLEN, NROWS )
      DOUBLE PRECISION VAR(   ROWLEN, NROWS )
...
*  Local Variables:
      ...
      DOUBLE PRECISION MDATA( NPARA )      ! The moments for a row
      DOUBLE PRECISION MVAR(  NPARA )      ! Their variances
...
*  COMP-th component of the result structure of the main MNDF.
      IF ( XISND .AND. VARUSE ) THEN
         ...
         INDEX = 1
         DO 1 I = 1, NROWS
            CALL SPD_WZCAD( BIAS, ROWLEN, XDATA(INDEX), YDATA(1,I),
     :         VAR(1,I), MDATA, MVAR, STATUS )
            CALL SPD_FACBD( MNDF, COMP, I, NPARA, MDATA, MVAR, STATUS )
            INDEX = INDEX + ROWLEN
 1       CONTINUE
 ...
 2       CONTINUE
         DO 3 I = 1, NROWS
            CALL SPD_WZCBD( BIAS, ROWLEN, XDATA(INDEX), YDATA(1,I),
     :         MDATA, STATUS )
            CALL SPD_FACBD( MNDF, COMP, I, NPARA, MDATA, MVAR, STATUS )
            INDEX = INDEX + ROWLEN
 3       CONTINUE
      ...
      ELSE IF ( VARUSE ) THEN
         ...
         DO 4 I = 1, NROWS
            CALL SPD_WZCAD( BIAS, ROWLEN, XDATA(1), YDATA(1,I),
     :         VAR(1,I), MDATA, MVAR, STATUS )
            CALL SPD_FACBD( MNDF, COMP, I, NPARA, MDATA, MVAR, STATUS )
 4       CONTINUE
 ...
 5       CONTINUE
         DO 6 I = 1, NROWS
            CALL SPD_WZCBD( BIAS, ROWLEN, XDATA(1), YDATA(1,I),
     :         MDATA, STATUS )
            CALL SPD_FACBD( MNDF, COMP, I, NPARA, MDATA, MVAR, STATUS )
 6       CONTINUE
