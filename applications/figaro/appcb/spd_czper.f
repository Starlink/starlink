      SUBROUTINE SPD_CZPER( INFO, COVRS, KMAX, XNK, DTYPE, ATYPE,
     :                      INDF, AXIS, LMAX, XL, ONDF, XNDF,
     :                      MESSAG, STATUS )
*+
*  Name:
*     SPD_CZPE{DR}
...
*  Invocation:
*     CALL SPD_CZPER( INFO, COVRS, KMAX, XNK, DTYPE, ATYPE, INDF,
...
*     17 May 1993 (hme):
*        Use MSG_SETR instead of the one for real.
*     17 Sep 1993 (hme):
...
*  Get start, end and size of input NDF. Report it to the user.
      ...
      IF ( INFO ) THEN
         CALL MSG_SETR( 'SPD_CZPER_T01', XSTART )
         CALL MSG_SETR( 'SPD_CZPER_T02', XEND   )
         CALL MSG_SETI( 'SPD_CZPER_T03', KMAX   )
         CALL MSG_OUT(  'SPD_CZPER_M01',
     :      'Abscissa grid in first input is from ^SPD_CZPER_T01 ' //
     :      'to ^SPD_CZPER_T02 in ^SPD_CZPER_T03 pixels.', STATUS )
      END IF
