...
* Invocation:
*    CALL LMPAR(N,R,LDR,IPVT,DIAG,QTB,DELTA,PAR,X,SDIAG,WA1,WA2)
...
*        is an input array of length n which must contain the
*         diagonal elements of the matrix d.
*       QTB(N) = DOUBLE PRECISION ARRAY (Given)
*        is an input array of length n which must contain the first
*         n elements of the vector (q transpose)*b.
*       DELTA (d) is a positive input variable which specifies an upper
*         bound on the euclidean norm of d*x.
*       PAR = DOUBLE PRECISION (Given and returned)
*        is a nonnegative variable. on input par contains an
*         initial estimate of the levenberg-marquardt parameter.
*         on output par contains the final estimate.
*       X(N) = DOUBLE PRECISION ARRAY (Returned)
*        is an output array of length n which contains the least
*         squares solution of the system a*x = b, sqrt(par)*d*x = 0,
*         for the output par.
*       SDIAG(N) = DOUBLE PRECISION ARRAY (Returned)
*        is an output array of length n which contains the
