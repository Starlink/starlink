      SUBROUTINE KPG1_LITRD( SCALE, OFFSET, STATUS )
*+
*  Name:
*     KPG1_LITRx
...
*    Substitute the actual scales for the tokens.
      ...
      CALL TRN_STOKD( 'x_scale', SCALE( 1 ), WTOD( 1 ), NSUBS,
     :                  STATUS )
      CALL TRN_STOKD( 'y_scale', SCALE( 2 ), WTOD( 2 ), NSUBS,
     :                  STATUS )
      CALL TRN_STOKD( 'x_scale', SCALE( 1 ), DTOW( 1 ), NSUBS,
     :                  STATUS )
      CALL TRN_STOKD( 'y_scale', SCALE( 2 ), DTOW( 2 ), NSUBS,
     :                  STATUS )
 
*    Substitute the actual offsets for the tokens.
 
      CALL TRN_STOKD( 'x_offset', OFFSET( 1 ), WTOD( 1 ), NSUBS,
     :                  STATUS )
      CALL TRN_STOKD( 'y_offset', OFFSET( 2 ), WTOD( 2 ), NSUBS,
     :                  STATUS )
      CALL TRN_STOKD( 'x_offset', OFFSET( 1 ), DTOW( 1 ), NSUBS,
     :                  STATUS )
      CALL TRN_STOKD( 'y_offset', OFFSET( 2 ), DTOW( 2 ), NSUBS,
     :                  STATUS )
