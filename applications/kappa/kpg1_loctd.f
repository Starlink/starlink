      SUBROUTINE KPG1_LOCTD( NDIM, LBND, UBND, ARRAY, INIT, SEARCH,
...
*  Invocation:
*     CALL KPG1_LOCTD( NDIM, LBND, UBND, ARRAY, INIT, SEARCH, POSTIV,
...
*  Check the dimensionality.
      IF ( NDIM .LT. 2 .AND. NDIM .GT. NDF__MXDIM ) THEN
         ...
         CALL ERR_REP( 'KPG1_LOCTD_INVDIM',
...
*  Check that the bounds are acceptable.
      IF ( SMLDIM ) THEN
         STATUS = SAI__ERROR
         CALL MSG_SETI( 'NDIM', NDIM )
         CALL ERR_REP( 'KPG1_LOCTD_SMLDIM',
...
*  individual values in WORK1.
                     PUTAT = NAV( MO, M ) + 1
                     NAV( MO, M ) = PUTAT
                     WORK1( PUTAT, MO, M ) = NUM_DTOR( ARRAY( VO ) )
...
*  Check for potential errors that might cause a divde by zero later.
            IF ( BACK( I ) .EQ. VAL__MAXR ) THEN
               STATUS = SAI__ERROR
               CALL ERR_REP( 'KPG1_LOCTD_UNDEFBGD',
