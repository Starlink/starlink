      SUBROUTINE KPG1_TRPID( NDIMI, IDIMS, TRID, AXES, OEL, NDIMO,
...
*  Arguments Returned:
      ...
      DOUBLE PRECISION FRIND( OEL, NDIMI )
      INTEGER INDICE( * )
...
*  routine, as the transformation is applied row by row.
      ORIGIN( 1 ) = NUM_ITOD( OLBND( 1 ) )
      DO I = 1, ODIMS( 1 )
         COOUT( I, 1 ) = ORIGIN( 1 ) + NUM_ITOD( I -1 )
     :                   - 1.0D0 / 2.0D0
      END DO
      AXJUMP( 1 ) = 1
...
*  Initialise the input co-ordinates for any higher dimensions.
      IF ( NDIMO .GT. 1 ) THEN
         DO IAXIS = 2, NDIMO
            ORIGIN( IAXIS ) = NUM_ITOD( OLBND( IAXIS ) )
            AXJUMP( IAXIS ) = AXJUMP( IAXIS - 1 ) + IDIMS( IAXIS - 1 )
            DO I = 1, ODIMS( 1 )
               COOUT( I, IAXIS ) = ORIGIN( IAXIS ) - 1.0D0 / 2.0D0
            END DO
         END DO
      END IF
...
*   context.
            IF ( STATUS .EQ. SAI__OK ) THEN
               DO IAXIS = 1, NDIMI
                  CALL ERR_MARK
                  CALL KPG1_AINBD( 1, IDIMS( IAXIS ), AXES(
...
*   co-ordinate, set the index to be bad.
         ...
         DO IAXIS = 1, NDIMI
            I = MOD( K - 1, ODIMS( 1 ) ) + 1
            IF ( FRIND( I, IAXIS ) .EQ. VAL__BADD ) THEN
               INDICE( K ) = VAL__BADI
               GO TO 100
...
*   strides to be applied to the vector offset.
            ELSE IF ( FRIND( I, IAXIS ) .LT. 0.0D0 ) THEN
               VOFF = VOFF + INT( FRIND( I, IAXIS ) -
     :                3.0D0 / 2.0D0 ) * STRIDI( IAXIS )
            ELSE
               VOFF = VOFF + INT( FRIND( I, IAXIS ) -
     :                1.0D0 / 2.0D0 ) * STRIDI( IAXIS )
            END IF
         END DO
         INDICE( K ) = VOFF
  100    CONTINUE
