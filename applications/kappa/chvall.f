      SUBROUTINE CHVALL( DIM, INARR, OLDVAL, NEWVAL, OUTARR, STATUS )
*
*    Description :
*
...
*    Invocation :
*
*     CALL CHVALL( DIM, INARR, OLDVAL, NEWVAL, OUTARR, STATUS )
*
*    Arguments :
*
*     DIM = INTEGER( READ )
*         The dimension of the input array
*     INARR( DIM ) = LOGICAL( READ )
*         The input data array
*     OLDVAL = LOGICAL( READ )
*         Value to be replaced
*     NEWVAL = LOGICAL( READ )
*         New value to be substituted for the %OLDVAL.
*     OUTARR( DIM ) = LOGICAL( READ )
...
*    External references :
      ...
      DOUBLE PRECISION
     :  VAL_LTOD             ! Data-conversion external function
...
*    minimum detectable difference.
      ...
      FPOLD = 0.5 * VAL_LTOD( .FALSE., OLDVAL, STATUS )
      DIFF = ABS( FPOLD * VAL__EPSD )
      DO  I = 1, DIM, 1
 
*       The halving is done to prevent overflows.
 
         IF ( ABS( ( VAL_LTOD( .FALSE., INARR( I ), STATUS ) * 0.5 )
     :        - FPOLD ) .LE. DIFF ) THEN
