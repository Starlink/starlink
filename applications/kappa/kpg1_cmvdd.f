      SUBROUTINE KPG1_CMVDD( NDIM, DIMS, INARR, INVAR, COMPRS, NLIM,
     :                         NORMAL, OUTARR, OUTVAR, SUM, NUM,
     :                         STATUS )
*+
*  Name:
*     KPG1_CMVDx
...
*     dimensions of the input arrays.  Determine the normalisation
...
*        o  Calculate the normalised output-array elements for each of
...
*  Check the dimensionality.
      IF ( NDIM .LT. 2 .OR. NDIM .GT. NDF__MXDIM ) THEN
         ...
         CALL ERR_REP( 'KPG1_CMVDx_INVDIM',
...
*  if an error is encountered.
      DO I = 1, NDIM
         IF ( COMPRS( I ) .LT. 1 .OR. COMPRS( I ) .GT. DIMS( I ) ) THEN
            ...
            IF ( COMPRS( I ) .GT. DIMS( I ) ) THEN
               CALL ERR_REP( 'KPG1_CMVDx_INVCMP',
     ...
     :           STATUS )
            ELSE
               CALL ERR_REP( 'KPG1_CMVDx_INVCMP2',
     :           'Unable to compress an array since the compression '/
