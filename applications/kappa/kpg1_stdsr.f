      SUBROUTINE KPG1_STDSR( NDIM, EL, NGOOD, DMIN, MINP, MINC,
...
*  Purpose:
*     Display statistics generated by KPG1_STATx and KPG1_HSTAx.
 
*  Language:
*     Starlink Fortran 77.
 
*  Invocation:
*     CALL KPG1_STDSx( NDIM, EL, NGOOD, DMIN, MINP, MINC,
*                      DMAX, MAXP, MAXC, SUM, MEAN, STDEV,
*                      MEDIAN, MODE, NUMPER, PERCNT, PERVAL, STATUS )
 
*  Description:
*     This routine diplays the statistics of pixel data as generated by
*     the routines KPG1_STATx and KPG1_HSTAx.
...
*  Display the pixel sum.
      ...
      CALL MSG_SETR( 'SUM', NUM_DTOR( SUM ) )
...
*  Display the mean pixel value.
      IF ( NGOOD .GT. 0 ) THEN
         IF ( MEAN .NE. VAL__BADD ) THEN
            CALL MSG_SETR( 'MEAN', NUM_DTOR( MEAN ) )
...
*  Display the standard deviation.
         IF ( STDEV .NE. VAL__BADD ) THEN
            CALL MSG_SETR( 'STDEV', NUM_DTOR( STDEV ) )
...
*  Display the minimum value found.
         IF ( DMIN .NE. VAL__BADD ) THEN
            CALL MSG_SETR( 'MINIMUM', NUM_DTOR( DMIN ) )
...
*  Display the minimum pixel's co-ordinates.
            CALL MSG_SETC( 'CMIN', BUF2( : NC2 ) )
...
*  Display the maximum pixel value found.
         IF ( DMAX .NE. VAL__BADD ) THEN
            CALL MSG_SETR( 'MAXIMUM', NUM_DTOR( DMAX ) )
...
*  Display the maximum pixel's co-ordinates.
            CALL MSG_SETC( 'CMAX', BUF2( : NC2 ) )
...
*  Display the median value.
         IF ( MEDIAN .NE. VAL__BADD ) THEN
            CALL MSG_SETR( 'MEDIAN', NUM_DTOR( MEDIAN ) )
...
*  Display the mode.
         IF ( MODE .NE. VAL__BADD ) THEN
            CALL MSG_SETR( 'MODE', NUM_DTOR( MODE ) )
         ...
         IF ( PERCNT( 1 ) .NE. VAL__BADR ) THEN
            DO 3 I = 1, NUMPER
               IF ( PERVAL( I ) .NE. VAL__BADD ) THEN
                  ...
                  END IF
                  CALL MSG_SETR( 'PERVAL', NUM_DTOR( PERVAL( I ) ) )
