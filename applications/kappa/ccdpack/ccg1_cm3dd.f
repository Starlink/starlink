...
*     line and and does NOT propagate it.  All work is done in double
*     precision.  Note that the output array is in the processing
*     precision.  The array NCON holds the actual numbers of pixels
*     which were used in deriving the output value plus any values
*     already present in the array; thus a cumulative sum of
...
*  Forming the weighted median.
         CALL CCG1_MDD3D( STACK, NPIX, NLINES, VARS, MINPIX,
...
*  Forming trimmed mean.
          CALL CCG1_TMD3D( ALPHA, STACK, NPIX, NLINES, MINPIX,
     :                       RESULT, WRK1, NCON, POINT, USED, STATUS )

      ELSE IF ( IMETH .EQ. 5 ) THEN

*  Forming the mode.
         CALL CCG1_MOD3D( NSIGMA, NITER, STACK, NPIX, NLINES,
...
*  Forming threshold trimmed mean.
          CALL CCG1_TCD3D( RMIN, RMAX, STACK, NPIX, NLINES, VARS,
...
*  Forming Min-Max exclusion mean.
         CALL CCG1_MMD3D( STACK, NPIX, NLINES, MINPIX, RESULT, WRK1,
     :                      NCON, POINT, USED, STATUS )

      ELSE IF ( IMETH .EQ. 9 ) THEN

*  Forming broadened median,
         CALL CCG1_BMD3D( STACK, NPIX, NLINES, MINPIX, RESULT, WRK1,
     :                      NCON, POINT, USED, STATUS )

      ELSE IF ( IMETH .EQ. 10 ) THEN

*  Forming sigma clipped mean.
         CALL CCG1_SMD3D( NSIGMA, STACK, NPIX, NLINES, VARS, MINPIX,
     :                      RESULT, WRK1, WRK2, NCON, POINT, USED,
     :                      STATUS )
...
*  Forming fast median (no weights).
         CALL CCG1_FMD3D( STACK, NPIX, NLINES, MINPIX, RESULT, 
...
*  gaps.
         CALL CCG1_WCWID( NPIX, NLINES, COORDS, WIDTHS, STATUS )
...
*  Forming intensity-weighted co-ordinate dispersion.
         CALL CCG1_IWC3D( NPIX, NLINES, STACK, COORDS, MINPIX,
     :                    RESULT, NCON, STATUS )

      ELSE IF ( IMETH .EQ. 23 ) THEN

*  Forming intensity-weighted co-ordinate dispersion.
         CALL CCG1_IWD3D( NPIX, NLINES, STACK, COORDS, 
     :                    MINPIX, RESULT, NCON, STATUS )

      ELSE IF ( IMETH .EQ. 24 ) THEN

*  Forming mean absolute deviation.
         CALL CCG1_RMS3D( NPIX, NLINES, STACK, MINPIX, RESULT,
     :                    NCON, STATUS )

      ELSE IF ( IMETH .EQ. 25 ) THEN

*  Forming mean absolute deviation.
         CALL CCG1_AD3D( NPIX, NLINES, STACK, MINPIX, RESULT, NCON,
     :                   STATUS )
...
*  the RESULT array.
         CALL CCG1_I2WCD( NPIX, NLINES, COIND, COORDS, RESULT, NBAD,
     :                    STATUS )
