      SUBROUTINE CCG1_FMD1D( STACK, NPIX, NLINES, VARS, MINPIX, 
...
*  Purpose:
*     Combines data lines using a fast median (double precision version).

*  Language:
*     Starlink Fortran 77

*  Invocation:
*     CALL CCG1_FMD1D( STACK, NPIX, NLINES, VARS, MINPIX, COVEC, NMAT, 
...
*
*     The output medians are returned in the array double precision
*     array RESULT.
...
*  Select the two values needed for median, update the used counters.
                  CALL CCG1_KTHD( NGOOD, IMED, WRK1, POINT, VAL,
     :                            STATUS )
                  NCON( POINT( IMED ) ) = NCON( POINT( IMED ) ) + 1.0D0

                  IMED = IMED - 1
                  CALL CCG1_KTHD( NGOOD, IMED, WRK1, POINT, DVAL,
     :                            STATUS )
                  NCON( POINT( IMED ) ) = NCON( POINT( IMED ) ) + 1.0D0
