...
*  Invocation:
*      CALL CCG1_WMD3R( ORDDAT, WEIGHT, NVAL, PBAD, NITER, TOLL,
...
*  Determine deviation from mean and weight it.
            DATA = NUM_RTOD( ORDDAT( I ) )
            DEV = DATA - OMODE
            DW = NUM_RTOD( WEIGHT( I ) )
            DEV2 = DEV * DEV * DW
...
*  Determine weight associated with this value.
            DEV = NUM_RTOD( ORDDAT( J ) ) - XMODE
            DW = NUM_RTOD( WEIGHT( J ) )
            DEV2 = DW * DEV * DEV
            EX = EXP( - DATA * DEV2 )
            PROB = EX / ( PBNORM + EX )
            W1 = DW * PROB
...
*  Determine weight associated with this value.
                  DEV = NUM_RTOD( ORDDAT( K ) ) - XMODE
                  DW = NUM_RTOD( WEIGHT( K ) )
                  DEV2 = DW * DEV * DEV
                  EX = EXP( - DATA  * DEV2 )
                  PROB = EX / ( PBNORM + EX )
                  W2 = DW * PROB
