      SUBROUTINE CCG1_WTM2D( ORDDAT, WEIGHT, NENT, USED, RESULT,
     :                          STATUS )
*+
*  Name:
*     CCG1_WTM2D
...
*  Invocation:
*     CALL CCG1_WTM2D( ARR, WEIGHT, NENT, USED, RESULT, STATUS )
...
* Search for median weight.
      ...
      DO 2 I = 1, NENT
         ...
         ELSE
            WTINC = ( NUM_DTOD( WEIGHT( I ) + WEIGHT( I-1 )) )* 0.5D0
         END IF
         WTSUM = WTSUM + WTINC
...
* Extract adjacent data values
      IF ( I .EQ. 1 ) THEN
         D1 = NUM_DTOD( ORDDAT( I ) )
         D2 = D1

      ELSE
         D1 = NUM_DTOD( ORDDAT( I - 1 ) )
