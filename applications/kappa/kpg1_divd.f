      SUBROUTINE KPG1_DIVD( BAD, VAR, EL, A, VA, B, VB, C, VC, NBAD,
     :                      STATUS )
*+
*  Name:
*     KPG1_DIVx
...
*  Check for bad values and zero denominator.
               IF( A( I ) .NE. VAL__BADD .AND. B( I ) .NE. VAL__BADD 
     :            .AND. B( I ) .NE. 0.0D0 ) THEN
...
*  Check for bad variances. Store the resulting variance if there are none.
                  IF( VA( I ) .NE. VAL__BADD .AND. 
     :                VB( I ) .NE. VAL__BADD ) THEN
                     VC( I ) = ( VA( I ) + ( VB( I )*A( I )*A( I ) ) /
     :                                     ( B( I )*B( I ) ) ) /
     :                         ( B( I )*B( I ) )
                  ELSE
