      SUBROUTINE IRM_TABLE( TITLE, NROW, NCOL, TABLE, FREQ, X1, X2, 
...
*  Invocation:
*     CALL IRM_TABLE( TITLE, NROW, NCOL, TABLE, FREQ, X1, X2, Y1, Y2, 
*                     COLOUR, FONT, YBTTM, STATUS )

*  Description:
*     This routine draw a table in the current SGS zone at the specified
*     position. The X extent of the table is specified by calling
*     routine while the Y extent will be determined by number of rows
*     of the table, with the maximum limit given by the calling
*     routine.
*
*     The columns in the table are separated by vertical lines.  The
*     rows are separated by a space of a text height except the first
...
*        The title of the table, a blank string will supress the title.
*     NROW = INTEGER (Given)
...
*     FREQ = INTEGER (Given)
*        The frequency at which the table is displayed. FREQ=1 causes
*        the whole table to be plotted. FREQ=2 causes the even-numbered
*        data to be plotted, and so on.  The first row are the headers
*        and these are always plotted.
*     X1 = REAL (Given)
*        The X lower extent of the table in world co-ordinates.
*     X2 = REAL (Given)
*        The X upper extent of the table in world co-ordinates.
*     Y1 = REAL (Given)
...
*        Added FREQ argument and changed the the default aspect ratio of
*        the text from 0.6667 to 0.75.
...
*  Local Constants:
      ...
      PARAMETER ( ASPCT = 0.75 )
      INTEGER MAXCOL                  ! Max. number of columns
      PARAMETER ( MAXCOL = 100 )

*  Local Variables:
      INTEGER EFFROW                  ! Effective number of rows
...
*  Derive the effective number of rows.
      IF ( FREQ .LE. 1 ) THEN
         EFFROW = NROW
      ELSE
         EFFROW = NROW / FREQ
...
*  Calculate the maximum item length in each column.
      ROWLN = 0.0
      DO I = 1, NCOL
         MITMCL( I ) = 0
         DO J = 1, NROW
...
*  If this column is empty, set status to error, report end exit.
         IF ( MITMCL( I ) .EQ. 0 ) THEN
            STATUS = SAI__ERROR
            CALL ERR_REP( 'IRM1_TAB_ERR1', 'An empty column', 
     :                     STATUS )
            GOTO 999
         END IF
...
*  Calculate the height of the text from the height of the table. 
      HI2 = ( Y2 - Y1 ) / ( FLOAT( EFFROW + 1 ) +
     :      0.5 * FLOAT( EFFROW ) )

*  Get the text height from above two possible heights.
      TXTHI = MIN( HI1, HI2 ) 
