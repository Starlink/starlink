      SUBROUTINE KPG1_RMAPB( EL, INARR, VALMAX, VALMIN, NUMBIN, MAP,
     :                         OUTARR, STATUS )
*+
*  Name:
*     KPG1_RMAPx
...
*  unsigned integer data types.
      ...
      DLRG = NUM_BTOD( VAL__MAXB )
      DVMIN = NUM_BTOD( VALMIN )
      DVMAX = NUM_BTOD( VALMAX )
      DLOWER = 0.5D0 * DVMIN
      DUPPER = 0.5D0 * DVMAX
...
*  Therefore a messy section of code follows
         STATUS = SAI__ERROR
         IF ( 'B' .EQ. 'I' .OR. 'B' .EQ. 'B' .OR. 'B' .EQ. 'UB'
     :       .OR. 'B' .EQ. 'W' .OR. 'B' .EQ. 'UW' ) THEN
            CALL MSG_SETI( 'MIN', NUM_BTOI( VALMIN ) )
            CALL MSG_SETI( 'MAX', NUM_BTOI( VALMAX ) )
         ELSE IF ( 'B' .EQ. 'R' ) THEN
...
*  Make a double-precision copy of the data value.
         TEMP = NUM_BTOD( INARR( I ) )
 
*  Test for a valid element.
         IF ( INARR( I ) .EQ. VAL__BADB ) THEN
...
*  transformed bin.
            OUTARR( I ) = NUM_DTOB( ( ( DBLE( NEWBIN ) - 0.5 )
     :                    / RNUMB ) * SCALE + DVMIN )
