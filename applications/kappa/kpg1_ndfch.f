      SUBROUTINE KPG1_NDFCH( START, GID, FNF, STATUS )
*+
*  Name:
*     KPG1_NDFCH
...
*  Invocation:
*     CALL KPG1_NDFCH( START, GID, FNF, STATUS )
...
*     environmental variable NDF_FORMATS_IN).  Files with types of
*     ".sdf" are also checked to see if they are container files for
*     top-level NDFs.  Others are returned unchecked.
...
*        The group identifier, created by a call to GRP_GROUP.  If the
...
*        have been removed.  In this case, the supplied group
*        identifier is annulled.
*     FNF = LOGICAL (Returned)
*        It is .TRUE. if any specified files could not be found, and
*        .FALSE. otherwise.
...
*     1997 May 19 (MJC):
*        Rebadged and edited for KAPPA.  Renamed some calls to other
*        renamed routines.  Used GRP.
...
*  Global Constants:
      ...
      INCLUDE 'IRG_ERR'          ! IRG__ error constants
...
*  Local References:
      ...
      INTEGER IRG1_WILD
      EXTERNAL IRG1_WILD         ! Lists files (including wildcards)

      INTEGER IRG1_EWILD
      EXTERNAL IRG1_EWILD        ! Ends file listing

*  Local Variables:
      CHARACTER FILE * ( GRP__SZNAM ) ! File spec. which matches NAME
      CHARACTER FTYPE * ( 10 )  ! File type of matching file
      CHARACTER NAME * ( GRP__SZNAM ) ! Current group name
      CHARACTER SLICE * ( GRP__SZNAM ) ! Any slice specification which
                                 ! was included in the original name
      ...
      LOGICAL FOUND              ! A matching .SDF file found?
      LOGICAL VALID              ! Filename is valid?
*.
...
*  Get the initial group size.
      CALL GRP_GRPSZ( GID, LAST, STATUS )
...
*  Extract and remove any slice specification from the name (i.e. any
*  string occurring between opening and closing parentheses at the end
*  of the name).
            CALL KPG1_SLICE( NAME, SLICE, NAMEND, STATUS )
...
*  If the file type is ".sdf" then test it to see if it really is an
*  NDF.
               CALL KPG1_FSPEC( FILE, ' ', 'TYPE', FTYPE, STATUS )
               IF ( FTYPE .EQ. '.sdf' .AND.
...
*  name.  Otherwise, add the file name to the end of the group.
                  IF ( .NOT. FOUND ) THEN
                     CALL GRP_PUT( GID, 1, FILE, I, STATUS )
                  ELSE
                     CALL GRP_PUT( GID, 1, FILE, 0, STATUS )
                  END IF
                  FOUND = .TRUE.
               END IF
...
*  If no further files can be found which match the current name.  If
*  no matching file was found, give a warning message and set FNF to
*  indicate this.
               IF ( .NOT. FOUND ) THEN
                  IF ( .NOT. FNF ) THEN
...
*  If an error was detected report it appropriately.
            ELSE
               STATUS = SAI__ERROR
               CALL MSG_SETC( 'NAME', NAME )
               CALL ERR_REP( 'KPG1_NDFCH_ERR1',
     :           'KPG1_NDFCH: error while searching for names '/
     :           /'"^NAME": ^ERR', STATUS )

               IF ( ISTAT .EQ. IRG__WPER ) THEN
                  CALL ERR_REP( 'KPG1_NDFCH_ERR2',
     :               'KPG1_NDFCH: Error getting pipe from forked '/
     :               /'process.', STATUS )

               ELSE IF ( ISTAT .EQ. IRG__WMER ) THEN
                  CALL ERR_REP( 'KPG1_NDFCH_ERR4',
     :              'KPG1_NDFCH: Insufficient memory.', STATUS )

               ELSE IF ( ISTAT .EQ. IRG__WSER ) THEN
                  CALL ERR_REP( 'KPG1_NDFCH_ERR5',
     :              'KPG1_NDFCH: Failed to locate irg_wild script.',
     :              STATUS )
