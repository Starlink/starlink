      SUBROUTINE KPG1_NUMBUB( BAD, VABS, ABOVE, EL, INARR, VALUE,
     :                         NUMBER, STATUS )
*+
*  Name:
*     KPG1_NUMBx
...
*  Internal References:
      ...
      INCLUDE 'NUM_DEC_UB'      ! Declarations of other routines
      INCLUDE 'NUM_DEF_CVT'      ! Definitions of conversion routines
      INCLUDE 'NUM_DEF_UB'      ! Definitions of other routines
...
*  NUM routine.  VAL is an order of magnitude slower.
                     IF ( NUM_ABSUB( INARR( I ) ) .GT. VALUE ) THEN
...
*  element's validity.  Note no ABS function for byte data so must call
*  NUM routine.  VAL is an order of magnitude slower.
                     IF ( NUM_ABSUB( INARR( I ) ) .LT. VALUE ) THEN
