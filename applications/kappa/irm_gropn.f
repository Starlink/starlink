...
*     the user. This new xone is stroed in the AGI database as the
...
*        If the zone can be partily cleared, it is true. Otherwise, it
*        is false.
...
*  Local Variables:
      REAL BLUE                  ! Blue intensity
      INTEGER COL                ! 1 if colour available on the device,
                                 ! 0 otherwise
      INTEGER COLREP             ! 1 if colour representation is
                                 ! changeable, 0 otherwise
      INTEGER CONID              ! GKS connection identifier
      REAL DEFX                  ! Default x size
      REAL DEFY                  ! Default y size
      INTEGER FABUN              ! Dummy for GQDWKA
      REAL GREEN                 ! Green intensity
      INTEGER GSTAT              ! GKS status
      INTEGER I                  ! Do loop index
      INTEGER NCOL               ! Number of colours available
      INTEGER NCPI               ! Number of predefined colour indices
      INTEGER PAREP              ! Dummy for GQDWKA 
      INTEGER PLBUN              ! Dummy for GQDWKA
      INTEGER PMBUN              ! Dummy for GQDWKA
      REAL RED                   ! Red intensity
      INTEGER TSTAT              ! Temporary status
      INTEGER TXBUN              ! Dummy for GQDWKA
...
*  Check status. If error, report and exit.
      IF ( STATUS .NE. SAI__OK ) THEN
         CALL ERR_REP( 'GRAOPN_ERR1', 'Unable to open a graphic '/
     :                /'device properly.', STATUS )
         GOTO 999
      END IF
...
*  If error happened, report and exit.
      IF ( STATUS .NE. SAI__OK ) THEN
         CALL ERR_REP( 'GRAOPN_ERR2', 'Unable to obtain graphic '/
     :                /'workstation type.', STATUS )
         GOTO 999
      END IF
...
*  Inquire whether the colour representation is changeable.
...
*  If error, report and exit.
         IF ( STATUS .NE. SAI__OK ) THEN
            CALL ERR_REP( 'GRAOPN_ERR3', 'Error while inquiring '/
     :                   /'workstation dynamic attributes.', STATUS )
            GOTO 999  
         END IF
      

*  If the colour representation is changeable, set up the standard SGS
*  pen colours.
         IF ( COLREP .EQ. 1 ) THEN
            DO I = 0, MIN( NSGSP, NCPI - 1 )

*  Inquire the predefined coloru representation.
               CALL GQPCR( WTYPE, I, GSTAT, RED, GREEN, BLUE )
                    
*  Reset the colour representation
               CALL GSCR( WKID, I, RED, GREEN, BLUE )
            END DO
         END IF
...
*  If error, report and exit.
         IF ( STATUS .NE. SAI__OK ) THEN
            CALL ERR_REP( 'GRAOPN_ERR4', 'Unable to set up standard '/
     :                   /'SGS pen number.', STATUS )
            GOTO 999
         END IF
