      SUBROUTINE KPG1_AXLID( EL, ARRAY, LVAL, UVAL, LINEAR, STATUS )
*+
*  Name:
*     KPG1_AXLIx
...
*    the ends of the array.
      ...
      END DO
      HIGH = EL
      DO WHILE ( ARRAY( HIGH ) .EQ. VAL__BADD .AND. HIGH .GT. LOW )
         HIGH = HIGH - 1
      END DO
...
*    Evaluate the linear increment.
      ...
      INCREM = ( NUM_DTOD( ARRAY( HIGH ) )
     :         - NUM_DTOD( ARRAY( LOW ) ) ) / NUM_ITOD( HIGH - LOW )
...
*    Find the biggest absolute value in the array.
      ...
      DO I = LOW, HIGH
         IF ( ARRAY( I ) .NE. VAL__BADD ) THEN
            MAXVAL = MAX( MAXVAL, ABS( NUM_DTOD( ARRAY( I ) ) ) )
         END IF
      END DO
...
*    allow for rounding errors and a full decade of values.
      ...
      IF ( VAL__EPSD .EQ. 1.0D0 ) THEN
         DIFLIM = 0.5D0
      ELSE
         DIFLIM = 11.0D0 * VAL__EPSD * MAXVAL
      END IF
...
*    linear array.
      ...
      IF ( ARRAY( 1 ) .EQ. VAL__BADD ) THEN
         LVAL = NUM_DTOD( NUM_DTOD( ARRAY( LOW ) ) -
     :          INCREM * DBLE( LOW - 1 ) )
      END IF
 
      IF ( ARRAY( EL ) .EQ. VAL__BADD ) THEN
         UVAL = NUM_DTOD( NUM_DTOD( ARRAY( HIGH ) ) +
     :          INCREM * DBLE( EL - HIGH ) )
      END IF
