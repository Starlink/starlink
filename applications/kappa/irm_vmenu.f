      SUBROUTINE IRM_VMENU( NITEM, MENU, NROW, DEFNO, X1, X2, Y1, Y2,
     :                      TITLE, COLOUR, FONT, TXTHT, GTITM, ITEMNO,
     :                      STATUS )
*+
*  Name:
*     IRM_VMENU
...
*  Invocation:
*     CALL IRM_VMENU( NITEM, MENU, NROW, DEFNO, X1, X2, Y1, Y2, TITLE, 
*                     COLOUR, CURSOR, FONT, TXTHT, GTITM, ITEMNO, STATUS )
...
*     lines when writting the content of an item into the box. So 
*     the calling routine must specifies approximately how many
*     lines the content of an item should be split into when writting
...
*        each item should be split when writting it into its menu box.
*     DEFNO = INTEGER (Given)
...
*  then set the status, report the error and exit.
     ...
     :                       'Not enought items given in menu list.',
     :                        STATUS )
               GOTO 999
            END IF
         END IF
