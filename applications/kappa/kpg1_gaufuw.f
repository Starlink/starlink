      SUBROUTINE KPG1_GAUFUW( DATA, LBND, UBND, NITER, TOLL, AMP, X0,
     :                        SIGMA, BACK, STATUS )
*+
*  Name:
*     KPG1_GAUFx
...
*    are too many) to use as an initial background estimate.
      ...
      CALL KPG1_NTHMUW( .TRUE., DATA( LBND ), NINT( POINTS ),
     :                    NQUART, STAK, STATUS )
 
*    Watch for all bad pixels.
 
      IF ( STAK( 1 ) .EQ. VAL__MAXUW ) THEN
         STATUS = SAI__ERROR
     ...
     :     STATUS )
      ...
      X( 4 ) = VAL_UWTOR( .FALSE., STAK( 1 ), STATUS )
...
*       Do not count negative or bad data.
         ...
         IF ( DATA( I ) .NE. VAL__BADUW ) THEN
            D = MAX( 0.0, NUM_UWTOR( DATA( I ) ) - X( 4 ) )
            SUM1 = SUM1 + D
            D = D * I
            SUM2 = SUM2 + D
            D = D * I
