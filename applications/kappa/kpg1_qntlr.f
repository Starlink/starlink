      SUBROUTINE KPG1_QNTLR( USEWT, INTERP, FRACT, EL, X, W, IP, Q,
     :                         STATUS )
*+
*  Name:
*     KPG1_QNTLx
...
*     lying immediately above and below the required quantile.
*     Instead, these must be obtained by inverting the cumulative
*     weight versus rank function as iterations proceed.  As a result,
*     a different top-level algorithm is needed in the case where
*     weights are supplied (recursive invocations made by this
*     top-level algorithm continue as normal).  This is implemented by
*     having two code branches; one handles the `weight-less' and
*     recursive cases, while the other handles top-level invocations
*     for the weighted case alone.  A return to the start of the
*     algorithm in order to perform another iteration can be made from
*     the end of either branch, with appropriate updating and
*     termination conditions being used in each case.
*
...
*     each side.  In addition, a new estimate of the target rank is
*     made at each iteration by linearly interpolating between the most
...
*        data.
*     20-DEC-1994 (DSB):
*        Renamed from CCD1_QNTL to KPG1_QNTLx.
...
*  sum of weights.  Initialise the target rank to this value.
      KZERO = NINT( EL * MIN( MAX ( 0.0E0, FRACT ), 1.0E0 ) )
      K = KZERO
...
*  elements.
            I1 = MAX( 1, L - 1 )
            I2 = MIN( R + 1, EL )
            ALPHA = ( EL * MIN( MAX( 0.0E0, FRACT ), 1.0E0 ) ) -
     :              ( I1 - HALF )
            ALPHA = MIN( MAX( 0.0E0, ALPHA ), 1.0E0 )
 
*  To obtain the result, either interpolate between the adjacent
*  values...
            IF ( INTERP ) THEN
               Q = ( 1.0E0 - ALPHA ) * X( IP( I1 ) ) +
     :             ALPHA * X( IP( I2 ) )
...
*  for all elements smaller than the final result should have.
         WTARG = MIN( MAX( 0.0E0, FRACT ), 1.0E0 ) *
     :           ( WLEFT + WL + WR + WRIGHT )
