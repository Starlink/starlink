      SUBROUTINE KPG1_XYZWR( DIM1, DIM2, ARRAY, XAXIS, YAXIS, BAD,
...
*  arrays. (Note that the input arrays are REAL but the output
*  arrays are DOUBLE PRECISION, so a conversion is needed).
                  NGOOD = NGOOD + 1
                  X( NGOOD ) = XAXIS( IX )
                  Y( NGOOD ) = YAXIS( IY )
                  Z( NGOOD ) = NUM_RTOD( ARRAY( IX, IY ) )
...
*  Check the variance is not bad.
                     IF ( VAR( IX, IY ) .NE. VAL__BADR ) THEN
