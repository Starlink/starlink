      SUBROUTINE KPG1_NOISR( BAD, EL, VARNCE, ARRAY, STATUS )
*+
*  Name:
*     KPG1_NOISx
...
*  noise cannot be added to the data array, so set it bad.
            IF ( ARRAY( I ) .EQ. VAL__BADR .OR.
     :           VARNCE( I ) .EQ. VAL__BADR ) THEN
               ARRAY( I ) = VAL__BADR

            ELSE

*  Find the  of the Normal distribution at the element.
               SIGMA = SQRT( NUM_RTOR( VARNCE( I ) ) )

*  Generate the random noise.  There is no offset.
               ARRAY( I ) = ARRAY( I ) +
     :                      NUM_RTOR( PDA_RNNOR( 0.0, SIGMA ) )
            END IF
         END DO
