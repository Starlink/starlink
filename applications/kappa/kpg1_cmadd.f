      SUBROUTINE KPG1_CMADD( NDIM, DIMS, INARR, COMPRS, NLIM, NORMAL,
...
*  Invocation:
*     CALL KPG1_CMADx( NDIM, DIMS, INARR, COMPRS, NLIM, NORMAL, OUTARR,
*                      SUM, NUM, STATUS )
...
*     dimensions of the input array.  Determine the normalisation
*     factor.  Constrain the minimum number of good values within each
*     box to be sensible.  Initialise the edges of the first box.
...
*        o  Calculate the normalised output-array element for each of
...
*  Local Variables:
      ...
      INTEGER K                  ! Counter
      INTEGER M                  ! Counter
      DOUBLE PRECISION NORM               ! Normalisation factor
...
*  Check the dimensionality.
      IF ( NDIM .LT. 2 .OR. NDIM .GT. NDF__MXDIM ) THEN
         ...
         CALL ERR_REP( 'KPG1_CMADx_INVDIM',
...
*  if an error is encountered.
      DO I = 1, NDIM
         IF ( COMPRS( I ) .LT. 1 .OR. COMPRS( I ) .GT. DIMS( I ) ) THEN
            ...
            IF ( COMPRS( I ) .GT. DIMS( I ) ) THEN
               CALL ERR_REP( 'KPG1_CMADx_INVCMP',
     ...
     :           STATUS )
            ELSE
               CALL ERR_REP( 'KPG1_CMADx_INVCMP2',
     :           'Unable to compress an array since the compression '/
...
*  Determine in which bin within the output line of data
*  the input value should be inserted.
                  M = ( J - 1 ) / COMPRS( 1 ) + 1
...
*  element to be bad.  Otherwise normalise the sum to the number of
*  elements in the box.
