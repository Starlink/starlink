      SUBROUTINE KPG1_MTHER( BAD, EL, N, DAT, VFLAG, VAR, IMAP, QUICK,
...
*  Local Variables:
      REAL DL                  ! Lower output perturbation
      REAL DU                  ! Upper output perturbation
...
*  appropriate elements of the output data array.
         CALL TRN_TRNR( BAD, MXBAT, N, ( IEND - ISTART + 1 ), WRK,
...
*  used for this purpose and note that bad values are present.
                  IF ( ( VRES( I ) .EQ. VAL__BADR ) .OR.
     :                 ( VAR( I, IVAR ) .LT. 0.0E0 ) .OR.
     :                 ( VAR( I, IVAR ) .EQ. VAL__BADR ) ) THEN
                     WRK( IWRK, IERR ) = VAL__BADR
                     WRK( IWRK, IIN ) = VAL__BADR
                     LBAD = .TRUE.
...
*  data, storing the results in the IPLUS row of the work array.
               CALL TRN_TRNR( ( BAD .OR. LBAD ), MXBAT, N,
...
*  bad and note that these bad values are present.
                     IF ( WRK( IWRK, IPLUS ) .EQ. VAL__BADR ) THEN
                        VRES( I ) = VAL__BADR
                        BADVR = .TRUE.
...
*  input value and note that bad values are present.
                     IF ( WRK( IWRK, IERR ) .EQ. VAL__BADR ) THEN
                        WRK( IWRK, IIN ) = VAL__BADR
                        LBAD = .TRUE.
...
*  and note that bad values are present.
                     IF ( ( WRK( IWRK, IPLUS ) .EQ. VAL__BADR ) .OR.
     :                    ( WRK( IWRK, IMINUS ) .EQ. VAL__BADR ) )
     :                    THEN
                        VRES( I ) = VAL__BADR
                        BADVR = .TRUE.
...
*  caused by the two input perturbations applied.
                     ELSE
                        ...
                        IF ( DU .LT. 0.0E0 ) DU = - DU
                        DL = RES( I ) - WRK( IWRK, IMINUS )
                        IF ( DL .LT. 0.0E0 ) DL = - DL
