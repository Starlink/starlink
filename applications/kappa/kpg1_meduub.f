      SUBROUTINE KPG1_MEDUUB( BAD, EL, ARRAY, MEDIAN, NELUSE, STATUS )
*+
*  Name:
*     KPG1_MEDUx
...
*  Local Variables:
      INTEGER CURR               ! Index to current smallest value
                                 ! during sorting
      BYTE CURVAL             ! Current smallest value during sorting
...
*  If the pixel is valid then increment the number-of-values counter.
               IF ( ARRAY( INDEX ) .NE. VAL__BADUB ) THEN
                  NELUSE = NELUSE + 1
...
*  convert to double precision to evaluate the average.
         IF ( MOD( NELUSE, 2 ) .EQ. 0 ) THEN
            MEDIAN = NUM_DTOUB( ( NUM_UBTOD( ARRAY( TEST - 1 ) ) +
     :               NUM_UBTOD( ARRAY( TEST -2 ) ) ) * 0.5D0 )
         ELSE
            MEDIAN = ARRAY( TEST - 1 )
         END IF
