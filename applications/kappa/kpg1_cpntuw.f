      SUBROUTINE KPG1_CPNTUW( PNNDF, PNTIT, NDIM, DIMS, ARRAY, NULL,
     :                         STATUS )
*+
*  Name:
*     KPG1_CPNTx
...
*    Create a new NDF.
      ...
      CALL NDF_CREP( PNNDF, '_UWORD', NDIM, DIMS, NDFO, STATUS )
 
*    Map the data array.  Wrap to prevent line overflow when the token
*    is expanded.
 
      CALL KPG1_MAP( NDFO, 'Data', '_UWORD', 'WRITE', OPNTR, EL,
     :              STATUS )
...
*    conversion errors so they are not checked.
      ...
      CALL VEC_UWTOUW( .FALSE., EL, ARRAY, %VAL( OPNTR( 1 ) ),
     :                   IERR, NERR, STATUS )
