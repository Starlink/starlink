      SUBROUTINE KPG1_GAUSD( SIGMA, IBOX, SAMBAD, WLIM, NX, NY, BAD,
...
*  Arguments Returned:
      ...
      LOGICAL BADOUT
      DOUBLE PRECISION WEIGHT( -IBOX : IBOX )
      DOUBLE PRECISION AMAR( NX )
      DOUBLE PRECISION WMAR( NX )
 
*  Status:
      INTEGER STATUS
 
*  Local Variables:
      DOUBLE PRECISION DENOM               ! Denominator for result calculation
      DOUBLE PRECISION OFFS                ! Offset from Gaussian centre
      DOUBLE PRECISION SUM                 ! Sum of weighted array values
      DOUBLE PRECISION WSUM                ! Sum of weights
      DOUBLE PRECISION WT1                 ! Pixel weight
      DOUBLE PRECISION WT2                 ! Weight for summing pixels
...
*  appropriate weight.
            IF ( BAD ) THEN
               DO 4 IX = 1, NX
                   IF ( A( IX, ILINE ) .NE. VAL__BADD ) THEN
