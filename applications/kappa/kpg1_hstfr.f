      SUBROUTINE KPG1_HSTFR( NUMBIN, HISTOG, MAXMUM, MINMUM, NFRAC,
     :                         FRAC, VALUES, STATUS )
*+
*  Name:
*     KPG1_HSTFx
...
*  Arguments Given:
      REAL MAXMUM              ! Maximum value used to evaluate the
                                 ! histogram
      REAL MINMUM              ! Maximum value used to evaluate the
                                 ! histogram
...
*  Arguments Returned:
      REAL VALUES( NFRAC )     ! The data values at the
                                 ! given fractions of the histogram
*  Status:
      INTEGER  STATUS            ! Global status
 
*  Local Variables:
      REAL BIN                ! Histogram bin width
      INTEGER I                  ! Counter
      ...
      INTEGER PREV               ! Number of pixels summed up to previous
                                 ! histogram bin
      REAL THRESH             ! Threshold for identical-limits test
...
*  arithmetic overflow for data with a very large dynamic range.
      THRESH = ( ABS( MAXMUM ) / 2.0E0 + ABS( MINMUM ) / 2.0E0 ) *
     :         VAL__EPSR * NUM_ITOR( NUMBIN )
 
*  If the maximum effectively equals minimum, o ordered statistics are
*  meaningless, so abort.
      IF ( ABS( MAXMUM / 2.0E0 - MINMUM / 2.0E0 ) .LT. THRESH )
     :  THEN
         STATUS = SAI__ERROR
...
*  Compute the binsize.
      BIN = ( MAXMUM - MINMUM ) / NUM_ITOR( NUMBIN )
      TOTAL = REAL( NVALUE )
