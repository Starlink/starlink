      SUBROUTINE KPG1_NUMBD( BAD, VABS, ABOVE, EL, INARR, VALUE,
     :                         NUMBER, STATUS )
*+
*  Name:
*     KPG1_NUMBx
...
*  Internal References:
      ...
      INCLUDE 'NUM_DEC_D'      ! Declarations of other routines
      INCLUDE 'NUM_DEF_CVT'      ! Definitions of conversion routines
      INCLUDE 'NUM_DEF_D'      ! Definitions of other routines
...
*  NUM routine.  VAL is an order of magnitude slower.
                     IF ( NUM_ABSD( INARR( I ) ) .GT. VALUE ) THEN
...
*  element's validity.  Note no ABS function for byte data so must call
*  NUM routine.  VAL is an order of magnitude slower.
                     IF ( NUM_ABSD( INARR( I ) ) .LT. VALUE ) THEN
