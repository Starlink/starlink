...
*     stright line and have no tick marks and numeric label on it. 
*
...
*
*     Warning:
*     After useing this routine the world coordinates of the SGS zone 
...
*        When it is true, all the curves to be ploted will have the 
...
*        fales, it should be a 2-dimensional array with its second 
...
*        The SGS pen number used to draw foure axes.
...
*        or equal to zero means using NCAR default setting. Tiltle of 
...
*        the x and y axes. A negative value forces the NCAR to coumpute
...
*        within range ( 0.0, 0.05 ). A vaule outside this range means 
...
*  Local Variables:
      INTEGER NPATN              ! Number of different patterns
...
*  Set the phase of curves. If number of lines over 26, set all of 
*  them as solid lines.
      NPATN = NLINE
      CALL IRM_STPHA( NLINE, INLAB, INLBPS, NPATN, STATUS )

*  If no error happens so far, draw the plot.
      IF ( STATUS .EQ. SAI__OK ) THEN

*  Draw the curve and flush it.
         NPNT = NX
...
*  Otherwise, give an error message
      ELSE
         CALL ERR_REP( 'MLINE_ERR1',
     :                 'Error when setting NCAR parameters', STATUS )
      END IF
