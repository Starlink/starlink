      SUBROUTINE KPG1_MXMEI( BAD, EL, ARRAY, ERROR, NSIGMA, THRESH,
...
*  Local Variables:
      INTEGER AMINUS             ! Data value less error
      INTEGER APLUS              ! Data value plus error
      INTEGER DELTA              ! Error-bar offsets
      INTEGER I                  ! Counter
      INTEGER OTHRES( 2 )        ! Thresholds where first element is the
...
*  Add some defensive code in case the thresholds are reversed.
      OTHRES( 1 ) = NUM_MINI( THRESH( 1 ), THRESH( 2 ) )
      OTHRES( 2 ) = NUM_MAXI( THRESH( 1 ), THRESH( 2 ) )
...
*  types.
               DELTA = NUM_RTOI( ABS( NUM_ITOR( ERROR( I ) ) *
     :                 NSIGMA ) )
               APLUS = NUM_ADDI( ARRAY( I ), DELTA )
               AMINUS = NUM_SUBI( ARRAY( I ), DELTA )
 
*  Test that the value is between the thresholds.
               IF ( NUM_GTI( AMINUS, OTHRES( 1 ) ) .AND.
     :              NUM_LTI( APLUS, OTHRES( 2 ) ) ) THEN
 
*  Check current maximum against current pixel value.
                  IF ( NUM_GTI( APLUS, MAXMUM ) ) THEN
                     MAXMUM = APLUS
                     MAXPOS = I
                  END IF
 
*  Check current minimum against current pixel value.
                  IF ( NUM_LTI( AMINUS, MINMUM ) ) THEN
                     MINMUM = AMINUS
                     MINPOS = I
                  END IF
