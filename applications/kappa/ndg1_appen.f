...
*  Description:
*     All files matching the supplied file template, and which are also 
*     recognised foreign NDFs, are appended to IGRP1. For each such 
*     file, a copy of REST is also appended to IGRP2.
...
*  If a system error was detected by NDG1_WILD, report it.
     ...
     :                       'Cannot allocate memory', STATUS )
            ...
            ELSE IF ( ISTAT .EQ. NDG__WSER ) THEN
               STATUS = SAI__ERROR
               CALL ERR_REP( 'NDG1_APPEN_ERR3', 'NDG1_APPEN: '//
