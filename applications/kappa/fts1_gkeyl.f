      SUBROUTINE FTS1_GKEYL( NCARD, BUFFER, SCARD, NAME, NOCCUR,
...
*  Arguments Returned:
      LOGICAL THERE              ! Is card containing the keyword
                                 ! present?
      LOGICAL VALUE               ! Value of the keyword
...
*  records).
                  IF ( NCHV .GT. 0 ) THEN
                     CALL ERR_MARK
                     CALL CHR_CTOL( CHRVAL( :NCHV ), VALUE, STATUS )
...
*  Report the error context.
                  IF ( STATUS .EQ. SAI__ERROR ) THEN
                     ...
                     CALL ERR_REP( 'FTS1_GKEYL_TYPCNV',
...
*  records).
                     IF ( NCHV .GT. 0 ) THEN
                        CALL ERR_MARK
                        CALL CHR_CTOL( CHRVAL( :NCHV ), VALUE,
     :                                   STATUS )
                        IF ( STATUS .NE. SAI__OK ) THEN
                           CALL ERR_ANNUL( STATUS )
                           STATUS = SAI__ERROR
