      SUBROUTINE KPG1_AXLIW( EL, ARRAY, LVAL, UVAL, LINEAR, STATUS )
*+
*  Name:
*     KPG1_AXLIx
...
*  Arguments Given:
      ...
      INTEGER*2     ARRAY( EL )   ! Array to be tested.
 
*  Arguments Returned:
      LOGICAL    LINEAR        ! True if array is linear
      INTEGER*2     LVAL          ! Value of first array element.
      INTEGER*2     UVAL          ! Value of last array element.
...
*    the ends of the array.
      ...
      LOW = 1
      DO WHILE ( ARRAY( LOW ) .EQ. VAL__BADW .AND. LOW .LT. EL )
         LOW = LOW + 1
      END DO
      HIGH = EL
      DO WHILE ( ARRAY( HIGH ) .EQ. VAL__BADW .AND. HIGH .GT. LOW )
         HIGH = HIGH - 1
      END DO
...
*    Evaluate the linear increment.
      ...
      INCREM = ( NUM_WTOD( ARRAY( HIGH ) )
     :         - NUM_WTOD( ARRAY( LOW ) ) ) / NUM_ITOW( HIGH - LOW )
...
*    Find the biggest absolute value in the array.
      ...
      DO I = LOW, HIGH
         IF ( ARRAY( I ) .NE. VAL__BADW ) THEN
            MAXVAL = MAX( MAXVAL, ABS( NUM_WTOD( ARRAY( I ) ) ) )
         END IF
      END DO
...
*    allow for rounding errors and a full decade of values.
      ...
      IF ( VAL__EPSW .EQ. 1  ) THEN
         DIFLIM = 0.5D0
      ELSE
         DIFLIM = 11  * VAL__EPSW * MAXVAL
      END IF
...
*    linear array.
      ...
      IF ( ARRAY( 1 ) .EQ. VAL__BADW ) THEN
         LVAL = NUM_DTOW( NUM_WTOD( ARRAY( LOW ) ) -
     :          INCREM * DBLE( LOW - 1 ) )
      END IF
 
      IF ( ARRAY( EL ) .EQ. VAL__BADW ) THEN
         UVAL = NUM_DTOW( NUM_WTOD( ARRAY( HIGH ) ) +
     :          INCREM * DBLE( EL - HIGH ) )
      END IF
