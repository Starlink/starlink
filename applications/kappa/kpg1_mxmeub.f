      SUBROUTINE KPG1_MXMEUB( BAD, EL, ARRAY, ERROR, NSIGMA, THRESH,
...
*  Add some defensive code in case the thresholds are reversed.
      OTHRES( 1 ) = NUM_MINUB( THRESH( 1 ), THRESH( 2 ) )
      OTHRES( 2 ) = NUM_MAXUB( THRESH( 1 ), THRESH( 2 ) )
...
*  types.
               DELTA = NUM_RTOUB( ABS( NUM_UBTOR( ERROR( I ) ) *
     :                 NSIGMA ) )
               APLUS = NUM_ADDUB( ARRAY( I ), DELTA )
               AMINUS = NUM_SUBUB( ARRAY( I ), DELTA )
 
*  Test that the value is between the thresholds.
               IF ( NUM_GTUB( AMINUS, OTHRES( 1 ) ) .AND.
     :              NUM_LTUB( APLUS, OTHRES( 2 ) ) ) THEN
 
*  Check current maximum against current pixel value.
                  IF ( NUM_GTUB( APLUS, MAXMUM ) ) THEN
                     MAXMUM = APLUS
                     MAXPOS = I
                  END IF
 
*  Check current minimum against current pixel value.
                  IF ( NUM_LTUB( AMINUS, MINMUM ) ) THEN
                     MINMUM = AMINUS
                     MINPOS = I
                  END IF
