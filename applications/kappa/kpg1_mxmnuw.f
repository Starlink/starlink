      SUBROUTINE KPG1_MXMNUW( BAD, EL, ARRAY, NINVAL, MAXMUM, MINMUM,
...
*  Watch for a bad first value.
         IF ( ARRAY( 1 ) .EQ. VAL__BADUW ) THEN
 
*  Search for the first non-bad value.
            I = 2
            DO WHILE ( I .LE. EL  .AND.
     :                 ARRAY( MIN( I, EL ) ) .EQ. VAL__BADUW )
               I = I + 1
            END DO
            NINVAL = I - 1
...
*  Use signed integers for the comparisons.
         IMAX = NUM_UWTOI( MAXMUM )
         IMIN = NUM_UWTOI( MINMUM )
...
*  Convert the integers back to the unsigned-integer type for export.
         MAXMUM = NUM_ITOUW( IMAX )
         MINMUM = NUM_ITOUW( IMIN )
      END IF
