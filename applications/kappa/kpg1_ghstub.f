      SUBROUTINE KPG1_GHSTUB ( BAD, DIM, ARRAY, NUMBIN,
...
*  Assign some useful variables.
      DVMIN = NUM_UBTOD( VALMIN )
      DVMAX = NUM_UBTOD( VALMAX )
      TNUMB = DBLE( NUMBIN )
...
*  arithmetic overflow for data with a very large dynamic range.
      IF ( VAL__EPSUB .LT. 1 ) THEN
         THRESH = ( ABS( DVMAX ) / 2.0D0 + ABS( DVMIN ) / 2.0D0 ) *
     :            NUM_UBTOD( VAL__EPSUB ) * TNUMB
