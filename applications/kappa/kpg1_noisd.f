      SUBROUTINE KPG1_NOISD( BAD, EL, VARNCE, ARRAY, STATUS )
*+
*  Name:
*     KPG1_NOISx
...
*  noise cannot be added to the data array, so set it bad.
            IF ( ARRAY( I ) .EQ. VAL__BADD .OR.
     :           VARNCE( I ) .EQ. VAL__BADD ) THEN
               ARRAY( I ) = VAL__BADD

            ELSE

*  Find the  of the Normal distribution at the element.
               SIGMA = SQRT( NUM_DTOR( VARNCE( I ) ) )

*  Generate the random noise.  There is no offset.
               ARRAY( I ) = ARRAY( I ) +
     :                      NUM_RTOD( PDA_RNNOR( 0.0, SIGMA ) )
            END IF
         END DO
