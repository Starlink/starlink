      SUBROUTINE KPG1_AXEXD( EL, CENTRE, USEERR, ERROR, USEWID, WIDTH,
...
*  Local Variables:
      DOUBLE PRECISION MAXE               ! Maximum axis centre plus error
      INTEGER MAXPOS             ! Index of maximum centre plus error
      DOUBLE PRECISION MINE               ! Minimum axis centre plus error
      INTEGER MINPOS             ! Index of minimum centre plus error
      INTEGER NINVAL             ! Number of bad centres and/or errors
      DOUBLE PRECISION THRESH( 2 )        ! Thresholds for finding the range
...
*  maximum and minimum.
      IF ( USEERR ) THEN
         THRESH( 1 ) = VAL__MIND
         THRESH( 2 ) = VAL__MAXD
         CALL KPG1_MXMED( .TRUE., EL, CENTRE, ERROR, NSIGMA, THRESH,
...
*  Include both the width and the errors in the calculation.
         IF ( USEERR .AND. USEWID ) THEN
            ASTART = MIN( CENTRE( 1 ) - ABS( WIDTH( 1 ) ) / 2.0D0,
     :               MINE )
            AEND = MAX( CENTRE( EL ) + ABS( WIDTH( EL ) ) / 2.0D0,
     :             MAXE )
...
*  Allow for the widths.
         ELSE IF ( USEWID ) THEN
            ASTART = CENTRE( 1 ) - ABS( WIDTH ( 1 ) ) / 2.0D0
            AEND = CENTRE( EL ) +  ABS( WIDTH ( EL ) ) / 2.0D0
...
*  values.
      ELSE
         IF ( USEERR .AND. USEWID ) THEN
            ASTART = MAX( CENTRE( 1 ) + ABS( WIDTH( 1 ) ) / 2.0D0,
     :               MAXE )
            AEND = MAX( CENTRE( EL ) - ABS( WIDTH( EL ) ) / 2.0D0,
     :             MINE )
