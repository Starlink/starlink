...
*  Indicate that we have not yet located the start of the HDS path.
      IPATH = 0 
...
*  Replace the directory path with the supplied directory path (which
*  will be blank if the NDF is in he current working directory). 
               PATH = ' '
               LPATH = 0
               CALL CHR_APPND( DIR, PATH, LPATH )
               CALL NDG1_LASTO( PATH2( : LPATH2 ), '/', IAT, STATUS )
               IAT = IAT + 1
               CALL CHR_APPND( PATH2( IAT : LPATH2 ), PATH, LPATH )

*  Find the start of the HDS component path in this string if this has not 
*  already been done (it will be the same for all located NDFs). The 
*  component path is any text following the file base name. The end of the
*  base name is marked by the first "." or "(" following the last "/" 
*  (which marks the end of the directory spec).
               IF( IPATH .EQ. 0 ) THEN

                  CALL NDG1_LASTO( PATH, '/', IAT, STATUS )
                  IAT = IAT + 1
