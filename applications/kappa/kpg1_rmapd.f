      SUBROUTINE KPG1_RMAPD( EL, INARR, VALMAX, VALMIN, NUMBIN, MAP,
     :                         OUTARR, STATUS )
*+
*  Name:
*     KPG1_RMAPx
...
*  unsigned integer data types.
      ...
      DLRG = NUM_DTOD( VAL__MAXD )
      DVMIN = NUM_DTOD( VALMIN )
      DVMAX = NUM_DTOD( VALMAX )
      DLOWER = 0.5D0 * DVMIN
      DUPPER = 0.5D0 * DVMAX
...
*  Therefore a messy section of code follows
         STATUS = SAI__ERROR
         IF ( 'D' .EQ. 'I' .OR. 'D' .EQ. 'B' .OR. 'D' .EQ. 'UB'
     :       .OR. 'D' .EQ. 'W' .OR. 'D' .EQ. 'UW' ) THEN
            CALL MSG_SETI( 'MIN', NUM_DTOI( VALMIN ) )
            CALL MSG_SETI( 'MAX', NUM_DTOI( VALMAX ) )
         ELSE IF ( 'D' .EQ. 'R' ) THEN
...
*  Make a double-precision copy of the data value.
         TEMP = NUM_DTOD( INARR( I ) )
 
*  Test for a valid element.
         IF ( INARR( I ) .EQ. VAL__BADD ) THEN
...
*  transformed bin.
            OUTARR( I ) = NUM_DTOD( ( ( DBLE( NEWBIN ) - 0.5 )
     :                    / RNUMB ) * SCALE + DVMIN )
