      SUBROUTINE IRM_TMENU( MENU, TITLE, PARAM, ITEMNO, STATUS )
*+
*  Name:
*     IRM_TMENU
...
*  Invocation:
*     CALL IRM_TMENU( MENU, TITLE, PARAM, ITEMNO, STATUS )
...
*        The menu list of the items sperated by comma. 
...
*  If the used length is 0, set status to error, report and exit.
      IF ( MENULN .EQ. 0 ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'IRM_TMENU', 'An blank menu', STATUS )
         GOTO 999
      END IF
