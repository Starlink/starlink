      SUBROUTINE KPG1_LUDCR( N, EL, ARRAY, PINDEX, SCALE, EVEN,
     :                         STATUS )
*+
*  Name:
*     KPG1_LUDCx
...
*  Local Variables:
      REAL COLMAX              ! Column maximum
      REAL DUMMY               ! Dummy value used for swapping
      INTEGER I                  ! Loop counter
      INTEGER J                  ! Loop counter
      INTEGER K                  ! Loop counter
      REAL MERIT               ! Degree of merit of a pivot
      INTEGER PIVOT              ! Pivot index
      REAL SUM                 ! Stores summation
...
*  prevent a division by zero below.
         IF ( ABS( ARRAY( J, J ) ) .LT. VAL__EPSR ) THEN
            ARRAY( J, J ) = VAL__EPSR
         END IF
