...
*        STYPE="Absolute". Offsets for celestial longitude and latitude
*        axes should be specified in arc-seconds. Offsets for all other
*        types of axes should be given directly in the units of the axis.
*     FID = LITERAL (Read)
*        A comma separated list of formatted axis values giving the position
*        of the fiducial point in WCS co-ordinates.  The number of values 
*        supplied must match the number of WCS axes in the NDF.  It is only 
*        used if STYPE="Relative".
*     NDF = NDF (Update)
...
*  Examples:
*     wcsslide m31 [32,23]
*        The (RA,Dec) axes in the NDF m31 are shifted by 32 arc-seconds 
*        in RA and 23 arc-seconds in Dec.
...
*     Therefore, whilst the translation will be constant across the array
*     in pixel co-ordinates, it may vary in WCS co-ordinates depending on
*     the nature of the pixel->WCS transformation. The size of the
*     translation in pixel co-ordinates is chosen in order to produce the 
*     required shift in WCS co-ordinates at the OBJ position (if STYPE is
*     "relative") or at the array centre (if STYPE is "absolute").
