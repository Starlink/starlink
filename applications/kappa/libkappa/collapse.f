...
*  Description:
*     This application collapses a nominated axis of an N-dimensional
*     NDF, producing an output NDF with one fewer axes than the input
*     NDF.  A specified range of axis values can be used instead of the
*     whole axis (see parameters LOW and HIGH).
*
...
*        be compressed.  Note, HIGH and LOW should not be equal since. 
*        If a null value (!) is supplied for either HIGH or LOW, the
...
*        since if a null value (!) is supplied for either LOW or HIGH,
...
*     collapse axis, and the two end points of the projection define the
*     range of axis values to collapse.
...
*  ========================
      IF ( ESTIM .EQ. 'COMAX' .OR. ESTIM .EQ. 'COMIN' ) THEN
...
*  The following two moments are dimensionless, so leave the unit
*  undefined.
      ELSE IF ( ESTIM .EQ. 'IWC' .OR. ESTIM .EQ. 'IWD' ) THEN
         CALL NDF_RESET( INDF2, 'Units', STATUS )
