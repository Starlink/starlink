...
*        parameter is not accessed if parameter MODE is set to "File". If 
*        COLOUT is null (!), no file will be created. [!]
*     COLUMNS = LITERAL (Read)
*        A pair of X values indicating the range of columns to be replaced.
*        All columns between the supplied values will be replaced. This
*        parameter is only accessed if parameter LINCOL is set to
*        "Columns" or "Region", and parameter MODE is set to "Interface".
*        Each X value should be given as a formatted value for axis 1
...
*
*        - "Region" -- Replaces the rectangular region of pixels within the
*        X and Y bounds specified by parameters COLUMNS and LINES. The
*        edges of the box are parallel to the pixel axes.
*
*        If this parameter is specified on the command line, and parameter 
*        MODE is set to "Interface", only one area will be replaced; 
*        otherwise a series of areas will be replaced until a null (!) 
*        value is supplied for this parameter.
*     LINES = LITERAL (Read)
*        A pair of Y values indicating the range of lines to be replaced.
*        All lines between the supplied values will be replaced. This
*        parameter is only accessed if parameter LINCOL is set to
*        "Lines" or "Region", and parameter MODE is set to "Interface".
*        Each Y value should be given as a formatted value for axis 2
*        of the current co-ordinate Frame of the input NDF. The two values
*        should be separated by a comma, or by one or more spaces.
*     MARKER = INTEGER (Read)
*        This parameter is only accessed if parameter PLOT is set to
*        "Mark". It specifies the type of marker with which 
...
*        [current value]
*     MODE  =  LITERAL (Read)
*        The method used to obtain the bounds of the areas to be replaced.
*        The supplied string can be one of the following:
*
...
*        "Linear". If a TRUE value is supplied, gaussian noise is added to 
...
*        value are used. This facility is provided for cosmetic use. [FALSE]
*     OUT  =  NDF (Write)
*        The output image.
*     PLOT = LITERAL (Read)
...
*        the attributes Colour(Curves), Width(Curves), etc (the synonym
*        Lines may be used in place of Curves). The appearance of boxes 
*        is controlled by the attributes Colour(Border), Size(Border), etc
*        (the synonym Box may be used in place of Border). The appearance 
*        of markers is controlled by attributes Colour(Markers), 
*        Size(Markers), etc. [current value]
*     TITLE  =  LITERAL (Read)
...
*        input NDF has more than 2 axes. A group of two strings should be
*        supplied specifying the 2 axes spanning the plane containing the
*        areas to be replaced. Each axis can be specified either
...
*  Local Variables:
      ...
      CHARACTER MODE*10         ! Mode in which line/column co-ordinates are to be obtained
      ...
      DOUBLE PRECISION BC( 2 )  ! User-supplied Base Frame position
      DOUBLE PRECISION CC( 2 )  ! User-supplied Current Frame position
      DOUBLE PRECISION CLBND( 2 )! Double precision current Frame bounds
      DOUBLE PRECISION COLVAL( 2 )! Range of values on axis 1 to zap
      DOUBLE PRECISION CUBND( 2 )! Double precision current Frame bounds
      DOUBLE PRECISION DLBND( 2 )! Double precision GRID bounds
      DOUBLE PRECISION DP( 2, 2 )! A pair of double precision 2D positions
      ...
      INTEGER IPOUT( 2 )        ! Pointer to output DATA_ARRAY component
      ...
      INTEGER ZLBND( 2 )        ! Lower bounds of the pixels to be replaced
      INTEGER ZUBND( 2 )        ! Upper bounds of the pixels to be replaced
      ...
      LOGICAL FILE              ! Positions supplied in input text file?
      ...
      LOGICAL LCDEF             ! Parameter LINCOL defined on the command line?
      ...
      LOGICAL XYOK              ! Positions read successfully from file?
...
*  1 zap operation is performed in interface mode.
      IF( INTERF ) THEN
...
*  Frame. The current Frame in the resulting Plot will be the PIXEL Frame
*  from the original WCS FrameSet. The Base Frame will be the GRAPHICS
*  Frame.
...
*  Get the simplified Mapping from the GRAPHICS Frame to the PIXEL Frame 
*  of the NDF.
...
*  Store a Mapping from Current Frame to PIXEL Frame (i.e. the inverse of
*  CMAP).
         MAP = AST_COPY( CMAP, STATUS )
         CALL AST_INVERT( MAP, STATUS )

*  Report an error if the forward transformation (i.e. from Current to PIXEL 
...
*  In these modes, the user can supply upper and lower ranges on a single
*  specific axis. To transform these into pixel co-ords, we need to
*  specify some value for the "other" axis as well. We use the extreme
*  values covered by the whole image.
...
*  Map the output data array and variance. Check whether or not bad pixels 
*  may be present in the variance.
...
*  the defined HDS data types. Since the data type at this poin could be
*  anything, we need to use PSX_MALLOC, first finding the total number of
*  bytes required.
...
*  Obtain the type of area to zap. In FILE mode, this is specified by the
*  contents of the supplied file. In other modes, ask the user.
         IF( .NOT. FILE ) THEN
...
*  object which is to be zapped, and indicate the PGPLOT rubber band mode 
*  to be used when getting the points with a cursor.
            REGION = .FALSE.
...
*  If the graphics should adapt to the type of region, set the values which 
*  indicate what KPG1_PGCUR is to draw.
...
*  If Zapping a range of lines, just get the range on axis 2, and use the
*  full range covered by the image for the other axis.
            ELSE IF( LINE ) THEN
               CALL KPG1_GTAXV( 'LINES', 2, .TRUE., CFRM, 2, LINVAL, 
     :                          NVAL, STATUS )
               COLVAL( 1 ) = CLBND( 1 )
...
*  If Zapping a range of columns, just get the range on axis 1, and use the
*  full range covered by the image for the other axis.
            ELSE 
               CALL KPG1_GTAXV( 'COLUMNS', 2, .TRUE., CFRM, 1, COLVAL, 
     :                          NVAL, STATUS )
...
*  Transform the co-ordinates of the two positions to pixel co-ordinates.
...
*  Check the pixel co-ordinates are good.
     ...
     :                       'position does not correspond to a valid'//
     :                       ' position in pixel co-ordinates.', 
     :                       STATUS )
               GOOD = .FALSE.
...
*  Ignore these points if they are co-incident.
...
*  Make the reports of the pixel-index bounds.  Bad status is returned when 
*  the line/column limits are invalid.
...
*  Zap the region. The subroutines do not use origin information, only the
*  displacement within the array.  Therefore, subtract the origins.
...
*  routines are strictly 2-d for the time being.
            IF( ITYPE .EQ. '_REAL' ) THEN
...
*  Check the current Frame co-ordinates are good.
     ...
     :             DP( 1, 2 ) .EQ. AST__BAD ) THEN
                  ...
                  CALL MSG_OUT( 'ZAPLIN_MSG1', 'The first supplied '//
     :                       'position does not correspond to a valid'//
     :                       ' position in the current co-ordinate '//
     :                       'Frame of ''^N'' and so cannot be logged.', 
     :                       STATUS )
...
*  Add a context report if anything went wrong.
     ...
     :                 'selected regions within a 2-dimensional NDF.', 
     :                 STATUS )
      END IF
