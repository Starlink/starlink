...
F77_SUBROUTINE(cadd)( INTEGER(STATUS) ){
/*
...
/* Local Variables: */
   ...
   double cons;          /* Constant to be added */
...
/* Select the appropriate function for the data type being processed and
   ...
   if( !strcmp( itype, "_BYTE" ) ) {
      kpgCaddB( wf, bad, el, pntr1, cons, pntr2, &(nerr), STATUS );

   } else if( !strcmp( itype, "_UBYTE" ) ) {
      kpgCaddUB( wf, bad, el, pntr1, cons, pntr2, &(nerr), STATUS );

   } else if( !strcmp( itype, "_DOUBLE" ) ) {
      kpgCaddD( wf, bad, el, pntr1, cons, pntr2, &(nerr), STATUS );

   } else if( !strcmp( itype, "_INTEGER" ) ) {
      kpgCaddI( wf, bad, el, pntr1, cons, pntr2, &(nerr), STATUS );

   } else if( !strcmp( itype, "_INT64" ) ) {
      kpgCaddK( wf, bad, el, pntr1, cons, pntr2, &(nerr), STATUS );

   } else if( !strcmp( itype, "_REAL" ) ) {
      kpgCaddF( wf, bad, el, pntr1, cons, pntr2, &(nerr), STATUS );

   } else if( !strcmp( itype, "_WORD" ) ) {
      kpgCaddW( wf, bad, el, pntr1, cons, pntr2, &(nerr), STATUS );

   } else if( !strcmp( itype, "_UWORD" ) ) {
      kpgCaddUW( wf, bad, el, pntr1, cons, pntr2, &(nerr), STATUS );
...
/* If an error occurred, then report context information. */
   if( *STATUS != SAI__OK ) errRep( " ", "CADD: Error adding a scalar "
                                    "value to an NDF data structure.",
                                    STATUS );
}
