...
*  Purpose:
*     Mosaic a group of NDFs using World Co-ordinate System information.
...
*     output NDF.
*
*     The algorithm proceeds as follows: First, the output NDF is filled
*     with zeros. An associated array of weights (one weight for each output 
*     pixel) is created and is also filled with zeros. Each input NDF is then 
*     processed in turn. For each pixel in the current input NDF, the
*     corresponding transformed position in the output NDF is found (based 
*     on the WCS information in both NDFs). The input pixel value is then 
*     divided up between a small group of output pixels centred on this
*     central output position. The method used for choosing the fraction
*     of the input pixel value assigned to each output pixel is determined
*     by the METHOD and PARAMS parameters. Each of the affected output pixel 
*     values is then incremented by its allocated fraction of the input pixel
*     value. The corresponding weight values are incremented by the fractions 
...
*     given by parameter WLIM are set bad. The mean value in the weights 
*     array (excluding those values less than WLIM) is then found. The
*     output NDF is then normalised by dividing it by the weights array.
*     This normalisation of the output NDF takes account of any difference 
*     in the number of pixels contributing to each output pixel, and also 
*     removes artifacts which may be produced by aliasing between the input 
*     and output pixel grids. 
*
...
*        are generated based on the spread of input pixel values contributing 
*        to each output pixel. If false, the output variances are based
*        on the variances in the input NDFs, and the output NDF will
*        contain variances only if all input NDFs contain variances. [FALSE]
*     IN = NDF (Read)
...
*        group of neighbouring output pixels. For details on these schemes, 
*        see the description of AST_REBINx in SUN/210. METHOD can take the
*        following values.
*
...
*        offset from the transformed input pixel centre, and sinc(z)=sin(z)/z. 
...
*
*        - "Somb" -- Uses the somb(pi*x) kernel, where somb(z)=2*J1(z)/z 
*        (J1 is the first-order Bessel function of the first kind). This 
...
*        specify how many output pixels on either side of the central
*        output pixel are to receive contribution from the corresponding
*        input pixel. Typically, a value of 2 is appropriate and the minimum 
...
*        SincCos schemes, it specifies the number of output pixels at which 
*        the envelope of the function goes to zero. The minimum value is
...
*        Note, fractional values are allowed. If a value less than
*        1.0E-10 is supplied,a value of 1.0E-10 is used. [1.0E-10]
...
*        they are aligned with the first input NDF, and combines them all
*        into a single output NDF called "mosaic". The output NDF is just big 
...
*  Local Variables:
      ...
      INTEGER IPMAP              ! Pointer to array of pix_in->pix_out Mappings
      ...
      INTEGER LBND( NDF__MXDIM ) ! Indices of lower left corner of output
      ...
      INTEGER METHOD_CODE        ! Integer corresponding to spreading method 
      ...
      INTEGER UBND( NDF__MXDIM ) ! Indices of upper right corner of output
      INTEGER UBND1( NDF__MXDIM )! Indices of upper right corner of input
      LOGICAL BAD_DV             ! Any bad data/variance values in input?
      LOGICAL GENVAR             ! Use input spread to create output variance?
      LOGICAL USEVAR             ! Use input variances to create output variance?
...
*  Extract required global information describing the group of input NDF.
...
*  If required, set the dynamic defaults for PARAMS, then get new values.
      IF( NPAR .GT. 0 ) THEN
...
*  See if output variances are to be generated on the basis of the spread
*  of input data values. If so, we do not use input variances.
      CALL PAR_GET0L( 'GENVAR', GENVAR, STATUS )
      IF( GENVAR ) USEVAR = .FALSE.

*  Create the output NDF by propagation from the reference NDF. The default
*  components HISTORY, TITLE, LABEL and all extensions are propagated,
*  together with the UNITS and WCS component. The NDF is initially created with
*  the same bounds as the reference NDF.
...
*  If required, tell the user which input NDF is currently being procesed.
         IF( VERB ) THEN
...
*  Get a pointer to the Mapping from input to output pixel coords.
