...
F77_SUBROUTINE(mult)( INTEGER(STATUS) ){
/*
*+
*  Name:
*     mult
...
*  Examples:
*     mult a b c
*        This multiplies the NDF called b to the NDF called a, to make the
*        NDF called c.  NDF c inherits its title from a.
*     mult out=c in1=a in2=b title="Normalised spectrum"image"
*        This multiplies the NDF called b to the NDF called a, to make the
*        NDF called c.  NDF c has the title "Normalised spectrum".
...
/* Determine which data type to use to process the input arrays and set
   ...
   ndfMtype( "_INTEGER,_REAL,_DOUBLE", ndf1, ndf2, comp, itype, sizeof(itype),
...
/* Integer data... */
   if( !strcmp( itype, "_INTEGER" ) ) {
      kpgMulI( wf, bad, var, el, pntr1[ 0 ], pntr1[ 1 ], pntr2[ 0 ],
               pntr2[ 1 ], pntr3[ 0 ], pntr3[ 1 ], &nerr, STATUS );

/* Real data... */
   } else if( !strcmp( itype, "_REAL" ) ) {
      kpgMulF( wf, bad, var, el, pntr1[ 0 ], pntr1[ 1 ], pntr2[ 0 ],
               pntr2[ 1 ], pntr3[ 0 ], pntr3[ 1 ], &nerr, STATUS );

/* Double precision data... */
   } else if( !strcmp( itype, "_DOUBLE" ) ) {
      kpgMulD( wf, bad, var, el, pntr1[ 0 ], pntr1[ 1 ], pntr2[ 0 ],
               pntr2[ 1 ], pntr3[ 0 ], pntr3[ 1 ], &nerr, STATUS );
   }
...
/* If an error occurred, then report context information. */
   if( *STATUS != SAI__OK ) errRep( " ", "MULT: Error multiplying two NDF "
