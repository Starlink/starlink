...
static void add_work( void *job_data_ptr, int *status );
...
*  Description:
*     This function adds two NDF data structures pixel-by-pixel to produce
*     a new NDF.
...
*  Examples:
*     add a b c
*        This adds the NDF called b to the NDF called a, to make the NDF
*        called c.  NDF c inherits its title from a.
*     add out=c in1=a in2=b title="Co-added image"
*        This adds the NDF called b to the NDF called a, to make the NDF
*        called c.  NDF c has the title "Co-added image".
...
/* Local Variables: */
   AddData *job_data = NULL;       /* Array of structs used to pass info to workers */
   AddData *pdata;       /* A single struct used to pass info to a worker */
...
/* Submit a job to the workforce to process the pixels assigned to the
   worker. */
         thrAddJob( wf, 0, pdata, add_work, 0, NULL, STATUS );
      }
...
/* If an error occurred, then report context information. */
   if( *STATUS != SAI__OK ) errRep( " ", "ADD: Error adding two NDF data "
                                    "structures.", STATUS );
...
*  Invocation:
*     void add_work( void *job_data_ptr, int *status )

*  Arguments:
*     job_data_ptr = AddData * (Given)
