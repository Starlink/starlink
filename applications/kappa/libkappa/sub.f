...
*  Map the input and output data arrays.
      CALL NDF_MAP( NDF1, 'Data', ITYPE, 'READ', PNTR1, EL, STATUS )
      CALL NDF_MAP( NDF2, 'Data', ITYPE, 'READ', PNTR2, EL, STATUS )
      CALL NDF_MAP( NDF3, 'Data', ITYPE, 'WRITE', PNTR3, EL, STATUS )
...
*  subtract the data arrays.
      IF ( ITYPE .EQ. '_BYTE' ) THEN
         CALL VEC_SUBB( BAD, EL, %VAL( PNTR1( 1 ) ),
     :                  %VAL( PNTR2( 1 ) ), %VAL( PNTR3( 1 ) ),
     :                  IERR, NERR, STATUS )
 
      ELSE IF ( ITYPE .EQ. '_UBYTE' ) THEN
         CALL VEC_SUBUB( BAD, EL, %VAL( PNTR1( 1 ) ),
     :                   %VAL( PNTR2( 1 ) ), %VAL( PNTR3( 1 ) ),
     :                   IERR, NERR, STATUS )
 
      ELSE IF ( ITYPE .EQ. '_DOUBLE' ) THEN
         CALL VEC_SUBD( BAD, EL, %VAL( PNTR1( 1 ) ),
     :                  %VAL( PNTR2( 1 ) ), %VAL( PNTR3( 1 ) ),
     :                  IERR, NERR, STATUS )
 
      ELSE IF ( ITYPE .EQ. '_INTEGER' ) THEN
         CALL VEC_SUBI( BAD, EL, %VAL( PNTR1( 1 ) ),
     :                  %VAL( PNTR2( 1 ) ), %VAL( PNTR3( 1 ) ),
     :                  IERR, NERR, STATUS )
 
      ELSE IF ( ITYPE .EQ. '_REAL' ) THEN
         CALL VEC_SUBR( BAD, EL, %VAL( PNTR1( 1 ) ),
     :                  %VAL( PNTR2( 1 ) ), %VAL( PNTR3( 1 ) ),
     :                  IERR, NERR, STATUS )
 
      ELSE IF ( ITYPE .EQ. '_WORD' ) THEN
         CALL VEC_SUBW( BAD, EL, %VAL( PNTR1( 1 ) ),
     :                  %VAL( PNTR2( 1 ) ), %VAL( PNTR3( 1 ) ),
     :                  IERR, NERR, STATUS )
 
      ELSE IF ( ITYPE .EQ. '_UWORD' ) THEN
         CALL VEC_SUBUW( BAD, EL, %VAL( PNTR1( 1 ) ),
     :                   %VAL( PNTR2( 1 ) ), %VAL( PNTR3( 1 ) ),
     :                   IERR, NERR, STATUS )
      END IF
...
*  output variance arrays.
      ...
      IF ( VAR1 .AND. VAR2 ) THEN
         CALL NDF_MAP( NDF1, 'Variance', ITYPE, 'READ', PNTR1, EL,
     :                 STATUS )
         CALL NDF_MAP( NDF2, 'Variance', ITYPE, 'READ', PNTR2, EL,
     :                 STATUS )
         CALL NDF_MAP( NDF3, 'Variance', ITYPE, 'WRITE', PNTR3, EL,
     :                 STATUS )
...
*  Select the appropriate routine to add the input variance arrays.
         IF ( ITYPE .EQ. '_BYTE' ) THEN
            CALL VEC_ADDB( BAD, EL, %VAL( PNTR1( 1 ) ),
     :                     %VAL( PNTR2( 1 ) ), %VAL( PNTR3( 1 ) ),
     :                     IERR, NERR, STATUS )
 
         ELSE IF ( ITYPE .EQ. '_UBYTE' ) THEN
            CALL VEC_ADDUB( BAD, EL, %VAL( PNTR1( 1 ) ),
     :                      %VAL( PNTR2( 1 ) ), %VAL( PNTR3( 1 ) ),
     :                      IERR, NERR, STATUS )
     ...
     :                     IERR, NERR, STATUS )
         ...
         ELSE IF ( ITYPE .EQ. '_WORD' ) THEN
            CALL VEC_ADDW( BAD, EL, %VAL( PNTR1( 1 ) ),
     :                     %VAL( PNTR2( 1 ) ), %VAL( PNTR3( 1 ) ),
     :                     IERR, NERR, STATUS )
 
         ELSE IF ( ITYPE .EQ. '_UWORD' ) THEN
            CALL VEC_ADDUW( BAD, EL, %VAL( PNTR1( 1 ) ),
     :                      %VAL( PNTR2( 1 ) ), %VAL( PNTR3( 1 ) ),
     :                      IERR, NERR, STATUS )
         END IF
