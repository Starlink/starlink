...
*     of "position identifiers" (see parameters FIRST and LAST). 
*
...
*     listshow stars.dat style="digits(1)=5,digits(2)=7"
...
*        the positions list.
*     listshow stars.dat plot=3 style="colour=red,size=2"
...
*  The positions are returned in the Base Frame of this FrameSet.
      IWCS = AST__NULL
      CALL KPG1_RDLST( 'INLIST', .FALSE., IWCS, NPOS, NBAX, IPPOS, IPID,
     :                 TITLE, STATUS )
...
*  Create an output positions list if required.
      IF( TITLE .EQ. ' ' ) THEN
         CALL KPG1_WRLST( 'OUTLIST', NDISP, NDISP, NBAX, %VAL( IPW1 ),
