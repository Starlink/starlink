      SUBROUTINE KPS1_THGTB( PNTHLO, PNTHHI, PNRLO, PNRHI, THRLO,
...
*  External References:
      BYTE VAL_DTOB          ! Double to BYTE conversion
...
*  status.
      THRLO = VAL_DTOB( .FALSE., DTHRLO, STATUS )
...
*  two-byte integers.
         ...
         NEWLO = VAL_DTOB( .FALSE., DNEWLO, STATUS )
      END IF
...
*  protect against potentionally harmful values when there is a bad
*  status.
      THRHI = VAL_DTOB( .FALSE., DTHRHI, STATUS )
...
*  two-byte integers.
         ...
         NEWHI = VAL_DTOB( .FALSE., DNEWHI, STATUS )
      END IF
