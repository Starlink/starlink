      SUBROUTINE KPS1_NOM1R( EL, INARR, OLDVAL, MEAN, SIGMA, DATMIN,
...
*  Find the maximum and minimum allowable output values.
      HILIM = MIN( NUM_RTOD( VAL__MAXR ), DATMAX )
      LOLIM = MAX( NUM_RTOD( VAL__MINR ), DATMIN )
...
*  the low limit.
      IF ( HILIM .LE. LOLIM ) THEN
         ...
         IF ( 'R' .EQ. 'B' .OR. 'R' .EQ. 'W' .OR.
     :        'R' .EQ. 'UB' .OR. 'R' .EQ. 'UW' .OR.
     :        'R' .EQ. 'I' ) THEN
