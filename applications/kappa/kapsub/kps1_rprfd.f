      SUBROUTINE KPS1_RPRFD( DIM1, DIM2, ARRAY, LBND, SIG0, AXISR,
...
*  Invocation:
*     CALL KPS1_RPRFD( DIM1, DIM2, ARRAY, LBND, SIG0, AXISR, THETA,
...
*     ARRAY( DIM1, DIM2 ) = DOUBLE PRECISION (Given)
...
*  If the pixel is valid, calculate the effective radius.
                  IF ( ARRAY( IMLOCN ) .NE. VAL__BADD ) THEN
                     DX2 = DX * DX
                     DXY = DX * DY
...
*  Find the most likely value in the bin.
                  ...
                  ELSE
                     CALL KPG1_MODED( DATA, ICOUNT, PBAD, NITER, 0.1,
