      SUBROUTINE KPS1_ZPRGB( DIM1, DIM2, LBND, UBND, NOISE,
     :                         VAR, ARRAY, STATUS )
*+
*  Name:
*     KPS1_ZPRGx
...
*          The start position cannot be an invalid pixel.
            ...
            DO WHILE ( ARRAY( I, STARTM ) .EQ. VAL__BADB .AND.
     :                .NOT. SEDGE )
...
*          The finish position cannot be an invalid pixel.
            ...
            DO WHILE ( ARRAY( I, FINSHP ) .EQ. VAL__BADB .AND.
     :                 .NOT. FEDGE )
...
*                may be present.
                  ...
                  IF ( NOISE .AND. VAR( I, J ) .NE. VAL__BADB )
     :              CALL KPG1_NOISB( .TRUE., 1, VAR( I, J ),
     :                                 ARRAY( I, J ), STATUS )
               END DO
...
*             lack of factorisation to reduce the risk of an overflow.
               ...
               DIFARR = ( NUM_BTOD( ARRAY( I, FINSHP ) ) / SEPAR ) -
     :                  ( NUM_BTOD( ARRAY( I, STARTM ) ) / SEPAR )

*             Similarly for the variance.  This is strictly not correct
*             but should make little difference.

               DIFVAR = ( NUM_BTOD( VAR( I, FINSHP ) ) / SEPAR ) -
     :                  ( NUM_BTOD( VAR( I, STARTM ) ) / SEPAR )
...
*                to the starting value to interpolate into the gap.
                  ...
                  ARRAY( I, J ) = ARRAY( I, STARTM ) +
     :                            NUM_DTOB( DELARR )
                  VAR( I, J ) = VAR( I, STARTM ) + NUM_DTOB( DELVAR )
...
*          The start position cannot be an invalid pixel.
            ...
            DO WHILE ( ARRAY( STARTM, J ) .EQ. VAL__BADB .AND.
     :                 .NOT. SEDGE )
...
*          The finish position cannot be an invalid pixel.
            ...
            DO WHILE ( ARRAY( FINSHP, J ) .EQ. VAL__BADB .AND.
     :                 .NOT. FEDGE )
...
*                may be present.
                  ...
                  IF ( NOISE .AND. TEMVAR .NE. VAL__BADB )
     :              CALL KPG1_NOISB( .TRUE., 1, TEMVAR, TEMARR,
     :                                 STATUS )
...
*             lack of factorisation to reduce the risk of an overflow.
               ...
               DIFARR = ( NUM_BTOD( ARRAY( FINSHP, J ) ) / SEPAR ) -
     :                  ( NUM_BTOD( ARRAY( STARTM, J ) ) / SEPAR )

*             Similarly for the variance.  This is strictly not correct
*             but should make little difference.

               DIFVAR = ( NUM_BTOD( VAR( FINSHP, J ) ) / SEPAR ) -
     :                  ( NUM_BTOD( VAR( STARTM, J ) ) / SEPAR )
...
*                to the starting value to interpolate into the gap.
                  ...
                  TEMARR = ARRAY( STARTM, J ) + NUM_DTOB( DELARR )
                  TEMVAR = VAR( STARTM, J ) + NUM_DTOB( DELVAR )
