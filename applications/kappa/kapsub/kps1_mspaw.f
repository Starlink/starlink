      SUBROUTINE KPS1_MSPAW( DIM1, DIM2, XAXIS, XMIN, XMAX, YAXIS,
...
*  Arguments Returned:
      DOUBLE PRECISION WORK( DIM1 ) ! Workspace to store intermediate
                                 ! fitted values
      INTEGER*2 OUTARR( DIM1, DIM2 ) ! Fitted data
...
*  Internal References:
      INTEGER*2 VAL_DTOW         ! Conversion from double precision
...
*  execute zero times if there are no bad pixels).
               DO I = 1, MFIRST - 1
                  OUTARR( I, J ) = VAL__BADW
               END DO

               DO I = MFIRST, MLAST
                  OUTARR( I, J ) = VAL_DTOW( .TRUE., WORK( I ),
     :                                         STATUS )
               END DO
