      SUBROUTINE KPS1_NOM1W( EL, INARR, OLDVAL, MEAN, SIGMA, DATMIN,
...
*  Find the maximum and minimum allowable output values.
      HILIM = MIN( NUM_WTOD( VAL__MAXW ), DATMAX )
      LOLIM = MAX( NUM_WTOD( VAL__MINW ), DATMIN )
...
*  the low limit.
      IF ( HILIM .LE. LOLIM ) THEN
         ...
         IF ( 'W' .EQ. 'B' .OR. 'W' .EQ. 'W' .OR.
     :        'W' .EQ. 'UB' .OR. 'W' .EQ. 'UW' .OR.
     :        'W' .EQ. 'I' ) THEN
