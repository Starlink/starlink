      SUBROUTINE KPS1_MSPAD( DIM1, DIM2, XAXIS, XMIN, XMAX, YAXIS,
...
*  Arguments Returned:
      DOUBLE PRECISION WORK( DIM1 ) ! Workspace to store intermediate
                                 ! fitted values
      DOUBLE PRECISION OUTARR( DIM1, DIM2 ) ! Fitted data
...
*  Internal References:
      DOUBLE PRECISION VAL_DTOD         ! Conversion from double precision
...
*  execute zero times if there are no bad pixels).
               ...
               DO I = MFIRST, MLAST
                  OUTARR( I, J ) = VAL_DTOD( .TRUE., WORK( I ),
     :                                         STATUS )
               END DO
