      SUBROUTINE KPS1_PUTI( LOC, NDIM, DIMS, SIZE, NLINES, INDNTC,
...
*  Obtain a scalar.
      IF ( NDIM .EQ. 0 ) THEN
         CALL DAT_GET( LOC, '_INTEGER', NDIM, DIMS, VALUES, STATUS )
...
*  as to obtain byte and word, signed and unsigned types.
         CALL DAT_GET( SLICE, '_INTEGER', 1, MXVAL, VALUES, STATUS )
         NVALUE = MXVAL
...
*  Get the values in the slice.
               CALL DAT_GET( SLICE, '_INTEGER', 1, MXVAL, VALUES,
     :                       STATUS )
               NVALUE = MXVAL
...
*  Test for a bad value.
            IF ( VALUES( IVALUE ) .EQ. VAL__BADI ) THEN
...
*  Test for a bad value.
               IF ( VALUES( FVALUE ) .EQ. VAL__BADI ) THEN
