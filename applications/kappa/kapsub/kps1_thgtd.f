      SUBROUTINE KPS1_THGTD( PNTHLO, PNTHHI, PNRLO, PNRHI, THRLO,
...
*  Arguments Returned:
      DOUBLE PRECISION THRLO
      DOUBLE PRECISION THRHI
      DOUBLE PRECISION NEWLO
      DOUBLE PRECISION NEWHI
 
*  Status:
      INTEGER STATUS             ! Global status
 
*  External References:
      DOUBLE PRECISION VAL_DTOD          ! Double to DOUBLE PRECISION conversion
...
*  (which is not actually used due to the ppath).
      MINREP = NUM_DTOD( VAL__MIND )
      MAXREP = NUM_DTOD( VAL__MAXD )
      DEFREP = 0.0
...
*  status.
      THRLO = VAL_DTOD( .FALSE., DTHRLO, STATUS )
...
*  two-byte integers.
         ...
         NEWLO = VAL_DTOD( .FALSE., DNEWLO, STATUS )
      END IF
...
*  protect against potentionally harmful values when there is a bad
*  status.
      THRHI = VAL_DTOD( .FALSE., DTHRHI, STATUS )
...
*  two-byte integers.
         ...
         NEWHI = VAL_DTOD( .FALSE., DNEWHI, STATUS )
      END IF
