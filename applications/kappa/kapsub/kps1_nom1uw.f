      SUBROUTINE KPS1_NOM1UW( EL, INARR, OLDVAL, MEAN, SIGMA, DATMIN,
...
*  Find the maximum and minimum allowable output values.
      HILIM = MIN( NUM_UWTOD( VAL__MAXUW ), DATMAX )
      LOLIM = MAX( NUM_UWTOD( VAL__MINUW ), DATMIN )
...
*  the low limit.
      IF ( HILIM .LE. LOLIM ) THEN
         ...
         IF ( 'UW' .EQ. 'B' .OR. 'UW' .EQ. 'W' .OR.
     :        'UW' .EQ. 'UB' .OR. 'UW' .EQ. 'UW' .OR.
     :        'UW' .EQ. 'I' ) THEN
