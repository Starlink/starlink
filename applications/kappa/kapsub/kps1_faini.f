      SUBROUTINE KPS1_FAINI( BAD, DIM1, DIM2, INARR, SIGRNG, LOW,
...
*             pixels.
               ...
               IF ( INARR( J, I ) .NE. VAL__BADI ) THEN
                  DIMAGE = NUM_ITOD( INARR( J, I ) )
                  SX = SX + DIMAGE
...
*    data type.
      ...
      LOWER = NUM_DTOI( MAX( MEAN + SIGRNG( 1 ) * SIGMA,
     :                    NUM_ITOD( VAL__MINI ) ) )
      UPPER = NUM_DTOI( MIN( MEAN + SIGRNG( 2 ) * SIGMA,
     :                    NUM_ITOD( VAL__MAXI ) ) )

*    Report the scaling limits.

      CALL MSG_SETD( 'MINVAL', NUM_ITOD( LOWER ) )
      CALL MSG_SETD( 'MAXVAL', NUM_ITOD( UPPER ) )
