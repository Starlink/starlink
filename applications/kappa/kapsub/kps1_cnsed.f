      SUBROUTINE KPS1_CNSED( PNMODE, PNCONT, PNSTRT, PNSTEP, PNVAL,
...
*  Arguments Returned:
      DOUBLE PRECISION CNTLEV( MXCONT )
      REAL PERCNT( MXCONT )
      REAL AREA( MXCONT )
      INTEGER NCONT
...
*  Local Variables:
      DOUBLE PRECISION BOT                ! First fraction of area of array
      ...
      DOUBLE PRECISION MAXVAL             ! Maximum value in array
      INTEGER MINPOS( 2 )        ! Position of minimum found in array
      DOUBLE PRECISION MINVAL             ! Minimum value in array
      CHARACTER * ( 9 ) MODE     ! Method of selecting contours
      INTEGER NINVAL             ! Number of bad pixels in array
      DOUBLE PRECISION OFFSET             ! First contour height
      DOUBLE PRECISION SCALE              ! Step interval between areas
      DOUBLE PRECISION STEP               ! Interval between contour heights (may
                                 ! be logarithmic for Magnitude mode)
      DOUBLE PRECISION TOP                ! Last fraction of area of array
...
*  Find the maximum and minimum values in the array.
            CALL KPG1_MXMND( BAD, EL, ARRAY, NINVAL, MAXVAL,
...
*  reports.
     ...
     :           'Error obtaining extrema for Automatic mode', STATUS )
            ...
            ELSE IF ( ABS( MAXVAL - MINVAL ) .LT. VAL__EPSD ) THEN
               STATUS = SAI__ERROR
...
*  Derive the interval between linear contours.
                  STEP = ( MAXVAL - MINVAL ) / NUM_ITOD( NCONT )
                  OFFSET = MINVAL + STEP / 2.0D0
 
*  Evaluate contour heights.
                  DO I = 1, NCONT
                     CNTLEV( I ) = OFFSET + NUM_ITOD( I-1 ) * STEP
                  END DO
...
*  equivalent radius increases by equal increments.
               TOP = NUM_RTOD( SQRT( 0.1 / REAL( NCONT ) ) )
               BOT = NUM_RTOD( SQRT( 0.95 - 0.45 /
     :               REAL( MAX( NCONT-5, 1 ) ) ) )
               SCALE = ( BOT - TOP ) / NUM_ITOD( MAX( 1, NCONT-1 ) )
...
*  Generate an histogram of the required array.
               CALL KPG1_GHSTD( BAD, EL, ARRAY, NUMBIN, MAXVAL,
     :                            MINVAL, HISTOG, STATUS )
 
*  Find the pixel values corresponding to the areas.
               CALL KPG1_HSTFD( NUMBIN, HISTOG, MAXVAL, MINVAL,
...
*  size.
            CALL PAR_GDR0I( PNCONT, 6, 1, MXCONT, .FALSE., NCONT,
     :                      STATUS )
            CALL PAR_GDR0D( PNSTRT, 1.0, VAL__MIND, VAL__MAXD,
     :                        .FALSE., OFFSET, STATUS )
            CALL PAR_GDR0D( PNSTEP, 1.0, VAL__MIND, VAL__MAXD,
     :                        .FALSE., STEP, STATUS )
...
*  ============
         ELSE IF ( MODE( 1:2 ) .EQ. 'FR' ) THEN
            CALL PAR_GET1D( PNVAL, MXCONT, CNTLEV, NCONT, STATUS )
...
*  fractions, so the 90% percentile will be at 0.9.
                     STEP = 1 / NUM_ITOD( NCONT )
                     OFFSET = STEP / 2.0D0
 
*  Evaluate contour heights in percentiles.
                     DO I = 1, NCONT
                        PERCNT( I ) = OFFSET + NUM_ITOD( I-1 ) * STEP
                     END DO
