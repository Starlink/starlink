      SUBROUTINE KPS1_CPBI( NEL, VAR, DATREF, DATOUT, VAROUT, NBAD,
     :                      STATUS )
*+
*  Name:
*     KPS1_CPBI
...
*  Invocation:
*     CALL KPS1_CPBI( NEL, VAR, DATREF, DATOUT, VAROUT, NBAD, STATUS )

*  Description:
*     This routine is called by copybad, and is passed two integer
*     arrays, DATOUT and DATREF, which contain the data 
*     values of two NDF files. If any element in DATREF is equal to
*     VAL__BADI, and the corresponding element in DATOUT is not
*     equal to VAL__BADI, then the corresponding element in DATOUT 
*     is set to VAL__BADI. If a variance array is present in NDF OUT,
*     then the corresponding variance element is also set to VAL__BADI.
...
*     DATREF( NEL ) = INTEGER (Given)
*         Array containing the data values of NDF REF.
*     DATOUT( NEL ) = INTEGER (Given and Returned)
*         Array containing the data values of NDF OUT.
*     VAROUT( NEL ) = INTEGER (Given and Returned)
...
*  Arguments Given:
      ...
      INTEGER DATREF( NEL )       ! Pixel values from REF

*  Arguments Given and Returned:
      INTEGER DATOUT( NEL )       ! Pixel values from OUT
      INTEGER VAROUT( NEL )       ! Variance values from OUT
...
*  and VARIANCE arrays are both bad.
         IF( DATREF( COUNT ) .EQ. VAL__BADI ) THEN

            DATOUT( COUNT ) = VAL__BADI
            IF( VAR ) VAROUT( COUNT ) = VAL__BADI

         END IF

*  Increment the number of bad pixels in the output NDF.
         IF( DATOUT( COUNT ) .EQ. VAL__BADI ) NBAD = NBAD + 1
