      SUBROUTINE KPS1_CPBUW( NEL, VAR, DATREF, DATOUT, VAROUT, NBAD,
     :                       STATUS )
*+
*  Name:
*     KPS1_CPBUW
...
*  Invocation:
*     CALL KPS1_CPBUW( NEL, VAR, DATREF, DATOUT, VAROUT, NBAD, STATUS )

*  Description:
*     This routine is called by copybad, and is passed two UNSIGNED WORD
*     arrays, DATOUT and DATREF, which contain the data 
*     values of two NDF files. If any element in DATREF is equal to
*     VAL__BADUW, and the corresponding element in DATOUT is not
*     equal to VAL__BADUW, then the corresponding element in DATOUT 
*     is set to VAL__BADUW. If a variance array is present in NDF OUT,
*     then the corresponding variance element is also set to VAL__BADUW.
...
*     DATREF( NEL ) = INTEGER*2 (Given)
*         Array containing the data values of NDF REF.
*     DATOUT( NEL ) = INTEGER*2 (Given and Returned)
*         Array containing the data values of NDF OUT.
*     VAROUT( NEL ) = INTEGER*2 (Given and Returned)
...
*  Arguments Given:
      ...
      INTEGER*2 DATREF( NEL )     ! Pixel values from REF

*  Arguments Given and Returned:
      INTEGER*2 DATOUT( NEL )     ! Pixel values from OUT
      INTEGER*2 VAROUT( NEL )     ! Variance values from OUT
...
*  and VARIANCE arrays are both bad.
         IF( DATREF( COUNT ) .EQ. VAL__BADUW ) THEN

            DATOUT( COUNT ) = VAL__BADUW
            IF( VAR ) VAROUT( COUNT ) = VAL__BADUW

         END IF

*  Increment the number of bad pixels in the output NDF.
         IF( DATOUT( COUNT ) .EQ. VAL__BADUW ) NBAD = NBAD + 1
