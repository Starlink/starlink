      SUBROUTINE KPS1_THGTW( PNTHLO, PNTHHI, PNRLO, PNRHI, THRLO,
...
*  External References:
      INTEGER*2 VAL_DTOW          ! Double to WORD conversion
...
*  status.
      THRLO = VAL_DTOW( .FALSE., DTHRLO, STATUS )
...
*  two-byte integers.
         ...
         NEWLO = VAL_DTOW( .FALSE., DNEWLO, STATUS )
      END IF
...
*  protect against potentionally harmful values when there is a bad
*  status.
      THRHI = VAL_DTOW( .FALSE., DTHRHI, STATUS )
...
*  two-byte integers.
         ...
         NEWHI = VAL_DTOW( .FALSE., DNEWHI, STATUS )
      END IF
