      SUBROUTINE KPS1_TRDRD( FD, DIM1, DIM2, POSCOD, COUNT, CODATA,
...
*             Extract the value required and convert to DOUBLE PRECISION type for
*             the data value, or to real for the co-ordinates.

               NVAL = NVAL + 1
               IF ( INDPOS( NVAL ) .EQ. DIM1 + 1 ) THEN
                  CALL CHR_CTOD( BUFFER( INDEXS:INDEXE ),
     :                             VADATA( COUNT ), STATUS )
               ELSE
                  J = INDPOS( NVAL )
