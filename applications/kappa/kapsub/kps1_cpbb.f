      SUBROUTINE KPS1_CPBB( NEL, VAR, DATREF, DATOUT, VAROUT, NBAD,
     :                      STATUS )
*+
*  Name:
*     KPS1_CPBB
...
*  Invocation:
*     CALL KPS1_CPBB( NEL, VAR, DATREF, DATOUT, VAROUT, NBAD, STATUS )

*  Description:
*     This routine is called by copybad, and is passed two byte
*     arrays, DATOUT and DATREF, which contain the data 
*     values of two NDF files. If any element in DATREF is equal to
*     VAL__BADB, and the corresponding element in DATOUT is not
*     equal to VAL__BADB, then the corresponding element in DATOUT 
*     is set to VAL__BADB. If a variance array is present in NDF OUT,
*     then the corresponding variance element is also set to VAL__BADB.
...
*  and VARIANCE arrays are both bad.
         IF( DATREF( COUNT ) .EQ. VAL__BADB ) THEN

            DATOUT( COUNT ) = VAL__BADB
            IF( VAR ) VAROUT( COUNT ) = VAL__BADB

         END IF

*  Increment the number of bad pixels in the output NDF.
         IF( DATOUT( COUNT ) .EQ. VAL__BADB ) NBAD = NBAD + 1
