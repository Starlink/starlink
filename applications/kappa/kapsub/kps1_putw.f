      SUBROUTINE KPS1_PUTW( LOC, NDIM, DIMS, SIZE, NLINES, INDNTC,
...
*  Purpose:
*     Report the first and last few values of an INTEGER*2 object.
...
*  Local Variables:
      ...
      INTEGER*2 VALUES( MAXVAL )   ! Values
...
*  Obtain a scalar.
      IF ( NDIM .EQ. 0 ) THEN
         CALL DAT_GET( LOC, '_WORD', NDIM, DIMS, VALUES, STATUS )
...
*  as to obtain byte and word, signed and unsigned types.
         CALL DAT_GET( SLICE, '_WORD', 1, MXVAL, VALUES, STATUS )
         NVALUE = MXVAL
...
*  Get the values in the slice.
               CALL DAT_GET( SLICE, '_WORD', 1, MXVAL, VALUES,
     :                       STATUS )
               NVALUE = MXVAL
...
*  Test for a bad value.
            IF ( VALUES( IVALUE ) .EQ. VAL__BADW ) THEN
...
*  Test for a bad value.
               IF ( VALUES( FVALUE ) .EQ. VAL__BADW ) THEN
