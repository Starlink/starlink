      SUBROUTINE KPS1_PUTUW( LOC, NDIM, DIMS, SIZE, NLINES, INDNTC,
...
*  Obtain a scalar.
      IF ( NDIM .EQ. 0 ) THEN
         CALL DAT_GET( LOC, '_UWORD', NDIM, DIMS, VALUES, STATUS )
...
*  as to obtain byte and word, signed and unsigned types.
         CALL DAT_GET( SLICE, '_UWORD', 1, MXVAL, VALUES, STATUS )
         NVALUE = MXVAL
...
*  Get the values in the slice.
               CALL DAT_GET( SLICE, '_UWORD', 1, MXVAL, VALUES,
     :                       STATUS )
               NVALUE = MXVAL
...
*  Test for a bad value.
            IF ( VALUES( IVALUE ) .EQ. VAL__BADUW ) THEN
...
*  as integers.
               IF ( BWTYPE ) THEN
                  DUMMY = NUM_UWTOI( VALUES( IVALUE ) )
                  CALL CHR_ITOC( DUMMY, STR, NCHAR )

               ELSE IF ( 'UW' .EQ. 'I' ) THEN
...
*  Test for a bad value.
               IF ( VALUES( FVALUE ) .EQ. VAL__BADUW ) THEN
