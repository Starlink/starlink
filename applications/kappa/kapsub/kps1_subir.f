      SUBROUTINE KPS1_SUBIR( DIM1, DIM2, ARRAY, BINX, BINY, ESTIMA,
...
*  bins are not put in the list.
               ...
               IF ( STATUS .EQ. SAI__OK .AND. ( .NOT. NOSTAT ) ) THEN
                  ...
                  IF ( ESTIM .EQ. 'MODE' ) THEN
                     Z( NBIN ) = NUM_RTOR( MODE )
                  ELSE IF ( ESTIM .EQ. 'MEDI' ) THEN
                     Z( NBIN ) = NUM_RTOR( MEDIAN )
                  END IF
                  W( NBIN ) = SQRT( NUM_ITOR( NSAM ) )
               END IF
     ...
     :                    ( ESTIM .EQ. 'MEAN' ) )
               IF ( STATUS .EQ. SAI__OK .AND. ( .NOT. NOSTAT ) ) THEN
                  ...
                  IF ( ESTIM .EQ. 'MEAN' ) THEN
                     Z( NBIN ) = NUM_RTOR( MEAN )
                     W( NBIN ) = NUM_RTOR( STDDEV )
                  ELSE
                     Z( NBIN ) = NUM_RTOR( MEANCL )
                     W( NBIN ) = NUM_RTOR( STDVCL )
                  END IF
               END IF
...
*  all the valid pixels is unlikely to make much difference.
                  X( NBIN ) = NUM_ITOR( IX ) / NUM_ITOR( NPT ) +
     :                        NUM_RTOR( XMIN - 1.0 )
                  Y( NBIN ) = NUM_ITOR( IY ) / NUM_ITOR( NPT ) +
     :                        NUM_RTOR( YMIN - 1.0 )
               END IF
