      SUBROUTINE KPS1_CFF2D( NX, NY, INARR, VAR, NVAR, INVAR, BOXSZ,
...
*     of values.  The local mean is obtained by a rectangular
...
*     1991 June 30 (MJC):
*        Revised the calling sequence for just 2-d usage, removed FRACT
*        and replaced by SAMBAD and NLIM, ditto DIMS( 2 ) by NX and NY.
*        Added error reporting.  Renamed from REJCT and made generic.
*        Added range limits.
...
*  Local Constants:
      INTEGER NDIM               ! Max. dimension of image the routine
                                 ! can handle.
      PARAMETER ( NDIM = 2 )
      DOUBLE PRECISION Q0                 ! Minimum variance of a pixel can take.
...
*  Local Variables:
      ...
      DOUBLE PRECISION DIFF               ! Image difference
      DOUBLE PRECISION DIFF2              ! Square of image difference
      DOUBLE PRECISION CLIPSQ             ! Squared clipping threshold
      DOUBLE PRECISION RANGE( 2 )         ! Ordered limits between which a
                                 ! pixel's data value must lie if has a
                                 ! chance of being rejected
      DOUBLE PRECISION SIG                ! Sum of squared difference of images
      DOUBLE PRECISION THRESH             ! Threshold for rejecting algorithm
      DOUBLE PRECISION VARNCE             ! Estimated Variance of the image
...
*  rectangular neighbourhood.
         CALL KPG1_BLOCD( .TRUE., SAMBAD, .FALSE., NX, NY, CINARR,
...
*  Use only those pixels valid in both image.
               IF ( INARR( I ) .NE. VAL__BADD .AND.
     :              OUTARR( I ) .NE. VAL__BADD ) THEN
...
*  Estimate the variance of the image.
            IF ( NGOOD .GE. 1 ) THEN
               VARNCE = MAX( SIG / NUM_ITOD( NGOOD ), Q0 )
            ELSE
               VARNCE = Q0
            END IF
         END IF
...
*  For efficiency compute the square clipping threshold.
         CLIPSQ = NUM_RTOD( CLIP( ITER ) * CLIP( ITER ) )
...
*  Estimate the RMS noise per pixel of the result array.
         SIGMA = SQRT( NUM_DTOD( VARNCE ) * DBLE( NBOX ) /
     :           DBLE( MAX( 1, NBOX-1 ) ) )
 
*  If interaction level is high, report the progress of the iteration.
         IF ( ILEVEL .GE. 2 ) THEN
            CALL MSG_SETD( 'SIGMA', NUM_DTOD( SIGMA ) )
...
*  variance component exists.
      IF ( VAR ) THEN
         DO 50 I = 1, NEL
            IF ( OUTARR( I )  .NE.  VAL__BADD .AND.
     :           INVAR( I ) .NE.  VAL__BADD ) THEN
               OUTVAR( I ) = INVAR( I )
            ELSE
