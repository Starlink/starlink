      SUBROUTINE KPS1_NOM1D( EL, INARR, OLDVAL, MEAN, SIGMA, DATMIN,
...
*     Copyright (C) 1997, 1999 Central Laboratory of the Research
...
*  Find the maximum and minimum allowable output values.
      HILIM = MIN( NUM_DTOD( VAL__MAXD ), DATMAX )
      LOLIM = MAX( NUM_DTOD( VAL__MIND ), DATMIN )
...
*  the low limit.
      IF ( HILIM .LE. LOLIM ) THEN
         ...
         IF ( 'D' .EQ. 'B' .OR. 'D' .EQ. 'W' .OR.
     :        'D' .EQ. 'UB' .OR. 'D' .EQ. 'UW' .OR.
     :        'D' .EQ. 'I' ) THEN
