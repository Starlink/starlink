      SUBROUTINE KPS1_CPBD( NEL, VAR, DATREF, DATOUT, VAROUT, NBAD,
     :                      STATUS )
*+
*  Name:
*     KPS1_CPBD
...
*  Invocation:
*     CALL KPS1_CPBD( NEL, VAR, DATREF, DATOUT, VAROUT, NBAD, STATUS )

*  Description:
*     This routine is called by copybad, and is passed two double 
*     precision arrays, DATOUT and DATREF, which contain the data 
*     values of two NDF files. If any element in DATREF is equal to
*     VAL__BADD, and the corresponding element in DATOUT is not
*     equal to VAL__BADD, then the corresponding element in DATOUT 
*     is set to VAL__BADD. If a variance array is present in NDF OUT,
*     then the corresponding variance element is also set to VAL__BADD.
...
*     DATREF( NEL ) = DOUBLE PRECISION (Given)
*         Array containing the data values of NDF REF.
*     DATOUT( NEL ) = DOUBLE PRECISION (Given and Returned)
*         Array containing the data values of NDF OUT.
*     VAROUT( NEL ) = DOUBLE PRECISION (Given and Returned)
...
*  Arguments Given:
      ...
      DOUBLE PRECISION DATREF( NEL ) ! Pixel values from REF

*  Arguments Given and Returned:
      DOUBLE PRECISION DATOUT( NEL ) ! Pixel values from OUT
      DOUBLE PRECISION VAROUT( NEL ) ! Variance values from OUT
...
*  and VARIANCE arrays are both bad.
         IF( DATREF( COUNT ) .EQ. VAL__BADD ) THEN

            DATOUT( COUNT ) = VAL__BADD
            IF( VAR ) VAROUT( COUNT ) = VAL__BADD

         END IF

*  Increment the number of bad pixels in the output NDF.
         IF( DATOUT( COUNT ) .EQ. VAL__BADD ) NBAD = NBAD + 1
