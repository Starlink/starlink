      SUBROUTINE KPS1_CPBW( NEL, VAR, DATREF, DATOUT, VAROUT, NBAD,
     :                      STATUS )
*+
*  Name:
*     KPS1_CPBW
...
*  Invocation:
*     CALL KPS1_CPBW( NEL, VAR, DATREF, DATOUT, VAROUT, NBAD, STATUS )

*  Description:
*     This routine is called by copybad, and is passed two WORD
*     arrays, DATOUT and DATREF, which contain the data 
*     values of two NDF files. If any element in DATREF is equal to
*     VAL__BADW, and the corresponding element in DATOUT is not
*     equal to VAL__BADW, then the corresponding element in DATOUT 
*     is set to VAL__BADW. If a variance array is present in NDF OUT,
*     then the corresponding variance element is also set to VAL__BADW.
...
*  and VARIANCE arrays are both bad.
         IF( DATREF( COUNT ) .EQ. VAL__BADW ) THEN

            DATOUT( COUNT ) = VAL__BADW
            IF( VAR ) VAROUT( COUNT ) = VAL__BADW

         END IF

*  Increment the number of bad pixels in the output NDF.
         IF( DATOUT( COUNT ) .EQ. VAL__BADW ) NBAD = NBAD + 1
