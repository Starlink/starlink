      SUBROUTINE KPS1_NOM1B( EL, INARR, OLDVAL, MEAN, SIGMA, DATMIN,
...
*  Find the maximum and minimum allowable output values.
      HILIM = MIN( NUM_BTOD( VAL__MAXB ), DATMAX )
      LOLIM = MAX( NUM_BTOD( VAL__MINB ), DATMIN )
...
*  the low limit.
      IF ( HILIM .LE. LOLIM ) THEN
         ...
         IF ( 'B' .EQ. 'B' .OR. 'B' .EQ. 'W' .OR.
     :        'B' .EQ. 'UB' .OR. 'B' .EQ. 'UW' .OR.
     :        'B' .EQ. 'I' ) THEN
