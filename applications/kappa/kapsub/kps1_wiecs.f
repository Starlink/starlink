      SUBROUTINE KPS1_WIECS( NPIX, NLIN, DATA, SIGMA, STATUS )
*+
*  Name:
*     KPS1_WIECS
...
*  Invocation:
*     CALL KPS1_WIECS( NPIX, NLIN, DATA, SIGMA, STATUS )
...
*     pixels which differ by more than 3*sigma from their neighbours.
...
*  Local Variables:
      ...
      INTEGER  NSUM2             ! Number of values summed in SUM2
...
*  of adjacent good pixels).
      ...
      ELSE
         STATUS = SAI__ERROR
         CALL ERR_REP( 'KPS1_WIECS_ERR2', 'Insufficient good data to '/
...
*  If possible, calculate a typical sigma value.
      ...
      ELSE
         STATUS = SAI__ERROR
         CALL ERR_REP( 'KPS1_WIECS_ERR3', 'Cannot find the noise '/
