...
*  Purpose:
*     Add curve labels and zero point markers to the plot produced by
*     MLINPLOT. 
...
*  Description:
*     This routine adds curve labels and zero point markers to the plot 
*     produced by MLINPLOT. The following graphical attributes may be
*     used to control the apperance of the labels and zero point markers: 
*
...
*        LabPos(Right)   : Horizontal position of the right curve label. 
*        
...
*     
*     "LabPos" without any qualifier refers to the Left label. LabPos values
*     are floating point, with 0.0 meaning the left edge of the plotting
*     area, and 1.0 the right edge.
...
*        The zero point for each curve, in PGPLOT world co-ordinate (i.e.
*        millimetres from the bottom of the view surface).
*     IPLOT = INTEGER (Given)
...
*        left corner of the PGPLOT view surface). 
...
*        The name of the parameter to be used when determining whether or
*        not to display curve labels at the left hand end of each curve.
*     APP = CHARACTER * ( * ) (Given)
...
*  Local Variables:
      ...
      DOUBLE PRECISION DIST      ! X distance between data sample and label
      DOUBLE PRECISION GAP       ! Gap between left edge and label centre
      DOUBLE PRECISION MNDIST    ! Min. X distance found so far
      DOUBLE PRECISION POS( 2 )  ! Label position
      DOUBLE PRECISION YMAX      ! Max Y value in plot
      DOUBLE PRECISION YMIN      ! Min Y value in plot
      INTEGER I                  ! Position index
      INTEGER IAT                ! No. of characters in string
      INTEGER ICURR              ! Index of original Plot Currnt Frame
      INTEGER IGRP1              ! GRP identifier for group holding pen defn.s
      INTEGER IGRP2              ! GRP identifier for group holding labels
      ...
      INTEGER NLAB               ! No. of labels supplied in IGRP2
      INTEGER NPEN               ! No. of pen defintions supplied in IGRP1
      ...
      REAL WL                    ! Length of shorter dimension of PGPLOT window
...
*  draw in GRAPHICS (Base Frame) coordinates.
...
*  created above. Annull the error if no labels are supplied. 
...
*  during the following call to KPG1_ASSET. The length of the zero point 
*  markers is specified by "Size(zeromark)" which is a synonym for 
*  "MajTickLen". MajTickLen is used because it can take both negative and 
*  positive values. We first translate any explicit MajTickLen settings into
*  settings for the illegal attribute name DUMMY. This prevents MajTickLen 
*  settings from changing the length of the zero point markers.
      CALL KPG1_ASPSY( 'MAJTICKLEN', 'DUMMY', STATUS )

*  Now translate any Size(zeromark) settings into settings for MajTickLen.
*  Note, since this is done after the previous translation, these
*  MajTickLen settings will not get translated into settings for DUMMY.
      CALL KPG1_ASPSY( 'SIZE(ZER*OMARK)', 'MAJTICKLEN', STATUS )

*  The other attributes for the zero point markers are set equal to those 
*  of AST "axes". We first translate any explicitly supplied Axes values
*  into Dummy values so that they do not effect the appearance of the zero
*  point markers.
...
*  Now translate "LabPos(L*eft)" into "TextLabGap(1), and "LabPos(R*ight)" 
*  into "TextLabGap(2). "LabPos" is made equivalent to "LabPos(Left)".
...
*  Get the size of the markers. This is scaled by the "size(zero)" attribute 
*  which is a synonym for "size(curves)".
...
*  Otherwise, format the line pixel index, preceeding it by a "#". Store
*  the label in the group.
         ELSE
            LAB = '#'
            IAT = 1
...
*  Get the horizontal position of the label (0.0 -> left edge, 1.0 -> right
*  edge).
