      SUBROUTINE KPS1_CLNSI( LBND, UBND, XCEN, X, STATUS )
*+
*  Name:
*     KPS1_CLNSx
...
*    point as a background estimate.
      ...
      CALL KPG1_NTHMI( .TRUE., X( LBND ), NX, NQUART, STAK, STATUS )
...
*       data limit as the maximum of the last NMIN values.
         ...
         DO I = I0, IEND, IDIRN
            ...
            DO J = 2, NMIN
               IF ( NUM_GTI( SMIN( J ), UPLIM ) ) UPLIM = SMIN( J )
            END DO

*          Limit the data to the current upper limit.

            IF ( NUM_GTI( X( I ), UPLIM ) ) X( I ) = UPLIM
