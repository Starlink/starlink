      SUBROUTINE KPS1_PUTD( LOC, NDIM, DIMS, SIZE, NLINES, INDNTC,
...
*  Purpose:
*     Report the first and last few values of an DOUBLE PRECISION object.
...
*  Local Variables:
      ...
      DOUBLE PRECISION VALUES( MAXVAL )   ! Values
...
*  Obtain a scalar.
      IF ( NDIM .EQ. 0 ) THEN
         CALL DAT_GET( LOC, '_DOUBLE', NDIM, DIMS, VALUES, STATUS )
...
*  as to obtain byte and word, signed and unsigned types.
         CALL DAT_GET( SLICE, '_DOUBLE', 1, MXVAL, VALUES, STATUS )
         NVALUE = MXVAL
...
*  Get the values in the slice.
               CALL DAT_GET( SLICE, '_DOUBLE', 1, MXVAL, VALUES,
     :                       STATUS )
               NVALUE = MXVAL
...
*  Test for a bad value.
            IF ( VALUES( IVALUE ) .EQ. VAL__BADD ) THEN
...
*  Test for a bad value.
               IF ( VALUES( FVALUE ) .EQ. VAL__BADD ) THEN
