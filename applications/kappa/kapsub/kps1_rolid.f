      SUBROUTINE KPS1_ROLID( IDIM1, IDIM2, INARR, VAR, INVAR, ANGLE,
...
*  Arguments Given:
      ...
      LOGICAL VAR
      DOUBLE PRECISION INVAR( IDIM1, IDIM2 )
      REAL ANGLE
      INTEGER ODIM1
      INTEGER ODIM2
...
*  Arguments Returned:
      ...
      DOUBLE PRECISION OUTVAR( ODIM1, ODIM2 )
...
*  Local Variables:
      ...
      DOUBLE PRECISION SUM                ! Sum of weighted surrounding pixels
      DOUBLE PRECISION VAL1               ! Bottom-left value surrounding
                                 ! the point to be interpolated
      DOUBLE PRECISION VAL2               ! Bottom-right value
      DOUBLE PRECISION VAL3               ! Top-left value
      DOUBLE PRECISION VAL4               ! Top-right value
      DOUBLE PRECISION VAR1               ! Bottom-left variance surrounding
                                 ! the point to be interpolated
      DOUBLE PRECISION VAR2               ! Bottom-right variance
      DOUBLE PRECISION VAR3               ! Top-left variance
      DOUBLE PRECISION VAR4               ! Top-right variance
      DOUBLE PRECISION WEIGHT             ! Weight of a pixel for interpolation
...
*  or all four surrounding pixels or their variances are invalid.
                  IF ( INARR( IIP, IJP ) .EQ. VAL__BADD .OR.
     :                 ( ( VAL1 .EQ. VAL__BADD .OR.
     :                     VAR1 .EQ. VAL__BADD ) .AND.
     :                   ( VAL2 .EQ. VAL__BADD .OR.
     :                     VAR2 .EQ. VAL__BADD ) .AND.
     :                   ( VAL3 .EQ. VAL__BADD .OR.
     :                     VAR3 .EQ. VAL__BADD ) .AND.
     :                   ( VAL4 .EQ. VAL__BADD .OR.
     :                     VAR4 .EQ. VAL__BADD ) )  ) THEN
...
*  point.
                     XDIST = NUM_RTOD( XO - REAL( IIP ) )
                     YDIST = NUM_RTOD( YO - REAL( IJP ) )
...
*  lies within the input image and is not invalid.
                     IF ( VAL1 .NE. VAL__BADD .AND.
     :                    VAR1 .NE. VAL__BADD ) THEN
...
*  bottom right...
                     IF ( VAL2 .NE. VAL__BADD ) THEN
                        WEIGHT = XDIST * ( 1.0D0 - YDIST ) / VAR2
                        SUM = SUM + VAL2 * WEIGHT
                        WTSUM = WTSUM + WEIGHT
                     END IF
 
*  top left...
                     IF ( VAL3 .NE. VAL__BADD ) THEN
                        WEIGHT = ( 1.0D0 - XDIST ) * YDIST / VAR3
                        SUM = SUM + VAL3 * WEIGHT
                        WTSUM = WTSUM + WEIGHT
                     END IF
 
*  top right...
                     IF ( VAL4 .NE. VAL__BADD ) THEN
...
*  or all four surrounding pixels are invalid.
                  IF ( INARR( IIP, IJP ) .EQ. VAL__BADD .OR.
     :                 (  VAL1 .EQ. VAL__BADD .AND.
     :                    VAL2 .EQ. VAL__BADD .AND.
     :                    VAL3 .EQ. VAL__BADD .AND.
     :                    VAL4 .EQ. VAL__BADD )  ) THEN
