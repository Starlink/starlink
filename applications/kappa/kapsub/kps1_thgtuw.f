      SUBROUTINE KPS1_THGTUW( PNTHLO, PNTHHI, PNRLO, PNRHI, THRLO,
...
*  External References:
      INTEGER*2 VAL_DTOUW          ! Double to UNSIGNED WORD conversion
...
*  status.
      THRLO = VAL_DTOUW( .FALSE., DTHRLO, STATUS )
...
*  two-byte integers.
         ...
         NEWLO = VAL_DTOUW( .FALSE., DNEWLO, STATUS )
      END IF
...
*  protect against potentionally harmful values when there is a bad
*  status.
      THRHI = VAL_DTOUW( .FALSE., DTHRHI, STATUS )
...
*  two-byte integers.
         ...
         NEWHI = VAL_DTOUW( .FALSE., DNEWHI, STATUS )
      END IF
