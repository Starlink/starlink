      SUBROUTINE KPS1_STPAI( DIM1, DIM2, ARRAY, LBND, NXY, ISIZE, POS,
...
*    Warn the user about any unusable positions.
         IF ( NGOOD .LT. NXY ) THEN
            ...
            CALL MSG_SETI( 'N', NXY )
            CALL MSG_OUT( 'KPS1_STPAI_MSG1', '  Stars could only be '//
...
*    If no star positions could be used, report an error.
      ...
      ELSE IF( STATUS .EQ. SAI__OK ) THEN
         STATUS = SAI__ERROR
         IF( NXY .EQ. 1 ) THEN
            CALL ERR_REP( 'KPS1_STPAI_ERR1', 'No star could be '//
     :                    'found at the supplied position.', STATUS )
         ELSE
            CALL ERR_REP( 'KPS1_STPAI_ERR1', 'No stars could be '//
