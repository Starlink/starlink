...
*     VIN( LBND1:UBND1, LBND2:UBND2 ) = REAL (Given)
*        The input variance array. Only accessed if VAR is .TRUE.
*     DOUT( LBND1:UBND1, LBND2:UBND2 ) = REAL (Given and Returned)
*        The output data array. This should be supplied holding a copy of
*        DIN.
*     VOUT( LBND1:UBND1, LBND2:UBND2 ) = REAL (Given and Returned)
...
*  Arguments Given:
      ...
      REAL VIN( LBND1:UBND1, LBND2:UBND2 )

*  Arguments Given and Returned:
      REAL DOUT( LBND1:UBND1, LBND2:UBND2 )
      REAL VOUT( LBND1:UBND1, LBND2:UBND2 )
...
*  weights.
               ...
               SUM = 0.0
               DO K = 1, KTOP
                  IF( WRK1( K ) .NE. VAL__BADR .AND.
     :                WRK2( K ) .NE. VAL__BADR .AND.
     :                WRK2( K ) .NE. 0.0 ) THEN
                     NGOOD = NGOOD + 1
                     WRK1( NGOOD ) = WRK1( K )
...
*  the box.
               IF( NGOOD .LT. 3 ) THEN
                  DOUT( I0, J0 ) = VAL__BADR
                  IF( VAR ) VOUT( I0, J0 ) = VAL__BADR
