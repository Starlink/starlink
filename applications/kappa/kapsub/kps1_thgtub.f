      SUBROUTINE KPS1_THGTUB( PNTHLO, PNTHHI, PNRLO, PNRHI, THRLO,
...
*  External References:
      BYTE VAL_DTOUB          ! Double to UNSIGNED BYTE conversion
...
*  status.
      THRLO = VAL_DTOUB( .FALSE., DTHRLO, STATUS )
...
*  two-byte integers.
         ...
         NEWLO = VAL_DTOUB( .FALSE., DNEWLO, STATUS )
      END IF
...
*  protect against potentionally harmful values when there is a bad
*  status.
      THRHI = VAL_DTOUB( .FALSE., DTHRHI, STATUS )
...
*  two-byte integers.
         ...
         NEWHI = VAL_DTOUB( .FALSE., DNEWHI, STATUS )
      END IF
