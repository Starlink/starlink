      SUBROUTINE KPS1_DSCLW( BAD, DIM1, DIM2, INARR, INVERT, PARLOW,
...
*  Purpose:
*     Scales a 2-d INTEGER*2 array between user-defined
...
*     INTEGER*2 array, and is then asked for the limits between which
...
*     INARR( DIM1, DIM2 ) = INTEGER*2 (Given)
...
*         array.
*     MINV = INTEGER*2 (Given)
*        The minimum value of the image.
*     MAXV = INTEGER*2 (Given)
...
*     LOWER = INTEGER*2 (Returned)
*        The lower limit used for scaling the image.
*     UPPER = INTEGER*2 (Returned)
...
*  Local Variables:
     ...
     :  DTMAX,                  ! Maximum data value for INTEGER*2
     :  DTMIN,                  ! Minimum data value for INTEGER*2
...
*    Convert the extreme values to double precision for later testing.
      ...
      DTMIN = NUM_WTOD( VAL__MINW )
      DTMAX = NUM_WTOD( VAL__MAXW )
 
*    Set defaults depending on required polarity of the scaling.
 
      IF ( POSTIV ) THEN
         DEFMIN = NUM_WTOD( MINV )
         DEFMAX = NUM_WTOD( MAXV )
      ELSE
         DEFMIN = NUM_WTOD( MAXV )
         DEFMAX = NUM_WTOD( MINV )
      END IF
...
*       Check that there is indeed a range.
         ...
         IF ( STATUS .EQ. SAI__OK ) THEN
            ...
            IF ( ABS( DUPPER - DLOWER ) .LT. DSML ) THEN
               STATUS = SAI__ERROR
               CALL ERR_REP( 'KPG1_DSCLW_INVRNG',
...
*             Convert to the required data type.
               ...
               LOWER = NUM_DTOW( DLOWER )
               UPPER = NUM_DTOW( DUPPER )
            END IF
         END IF
