      SUBROUTINE KPS1_MSPAI( DIM1, DIM2, XAXIS, XMIN, XMAX, YAXIS,
...
*  Arguments Returned:
      DOUBLE PRECISION WORK( DIM1 ) ! Workspace to store intermediate
                                 ! fitted values
      INTEGER OUTARR( DIM1, DIM2 ) ! Fitted data
...
*  Internal References:
      INTEGER VAL_DTOI         ! Conversion from double precision
...
*  execute zero times if there are no bad pixels).
               DO I = 1, MFIRST - 1
                  OUTARR( I, J ) = VAL__BADI
               END DO

               DO I = MFIRST, MLAST
                  OUTARR( I, J ) = VAL_DTOI( .TRUE., WORK( I ),
     :                                         STATUS )
               END DO
