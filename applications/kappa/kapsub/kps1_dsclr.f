      SUBROUTINE KPS1_DSCLR( BAD, DIM1, DIM2, INARR, INVERT, PARLOW,
...
*  Purpose:
*     Scales a 2-d REAL array between user-defined
...
*     REAL array, and is then asked for the limits between which
...
*  Local Variables:
     ...
     :  DTMAX,                  ! Maximum data value for REAL
     :  DTMIN,                  ! Minimum data value for REAL
...
*    Convert the extreme values to double precision for later testing.
      ...
      DTMIN = NUM_RTOD( VAL__MINR )
      DTMAX = NUM_RTOD( VAL__MAXR )
 
*    Set defaults depending on required polarity of the scaling.
 
      IF ( POSTIV ) THEN
         DEFMIN = NUM_RTOD( MINV )
         DEFMAX = NUM_RTOD( MAXV )
      ELSE
         DEFMIN = NUM_RTOD( MAXV )
         DEFMAX = NUM_RTOD( MINV )
      END IF
...
*       Check that there is indeed a range.
         ...
         IF ( STATUS .EQ. SAI__OK ) THEN
            ...
            IF ( ABS( DUPPER - DLOWER ) .LT. DSML ) THEN
               STATUS = SAI__ERROR
               CALL ERR_REP( 'KPG1_DSCLR_INVRNG',
...
*             Convert to the required data type.
               ...
               LOWER = NUM_DTOR( DLOWER )
               UPPER = NUM_DTOR( DUPPER )
            END IF
         END IF
