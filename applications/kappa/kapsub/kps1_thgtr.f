      SUBROUTINE KPS1_THGTR( PNTHLO, PNTHHI, PNRLO, PNRHI, THRLO,
...
*  External References:
      REAL VAL_DTOR          ! Double to REAL conversion
...
*  status.
      THRLO = VAL_DTOR( .FALSE., DTHRLO, STATUS )
...
*  two-byte integers.
         ...
         NEWLO = VAL_DTOR( .FALSE., DNEWLO, STATUS )
      END IF
...
*  protect against potentionally harmful values when there is a bad
*  status.
      THRHI = VAL_DTOR( .FALSE., DTHRHI, STATUS )
...
*  two-byte integers.
         ...
         NEWHI = VAL_DTOR( .FALSE., DNEWHI, STATUS )
      END IF
