      SUBROUTINE KPS1_BAFID( NPIX, NLINES, INARR, VAR, INVAR, NITER,
...
*  Arguments Given:
      INTEGER NPIX
      INTEGER NLINES
      DOUBLE PRECISION INARR( NPIX, NLINES )
      LOGICAL VAR
      DOUBLE PRECISION INVAR( NPIX, NLINES )
      INTEGER NITER
...
*  Arguments Returned:
      ...
      INTEGER NBAD
      DOUBLE PRECISION OUTARR( NPIX, NLINES )
      DOUBLE PRECISION OUTVAR( NPIX, NLINES )
      DOUBLE PRECISION DSUM( NPIX, NLINES )
      DOUBLE PRECISION WTSUM( NPIX, NLINES )
      DOUBLE PRECISION DLAST( NPIX )
      DOUBLE PRECISION WTLAST( NPIX )

*  Status:
      INTEGER STATUS             ! Global status

*  Local Variables:
      DOUBLE PRECISION DEC                ! Weighting decrement factor
...
*  Copy the input to the output data array and count the bad values.
      DO J = 1, NLINES
         DO I = 1, NPIX
            ...
            IF ( INARR( I, J ) .EQ. VAL__BADD ) NBAD = NBAD + 1
         END DO
      END DO
...
*  If the input pixel is valid, reset the weighted sums.
                     IF ( INARR( I, J ) .NE. VAL__BADD ) THEN
                        DLAST( 1 ) = OUTARR( I, J )
                        WTLAST( 1 ) = 1.0D0
...
*  weighted sums for this line.
                        IF ( OUTARR( I, J ) .NE. VAL__BADD ) THEN
...
*  found, calculate the replacement value.
                  IF ( INARR( I, J ) .EQ. VAL__BADD .AND.
     :                 WTSUM( I, J ) .GT. 0.0D0 ) THEN
...
*  bad values if the data was good, but the variance was bad.
     ...
     :              INVAR( I, J ) .EQ. VAL__BADD ) NBAD = NBAD + 1
            END DO
         END DO
...
*  can be found, calculate the replacement value.
                  IF ( ( INARR( I, J ) .EQ. VAL__BADD .OR.
     :                   INVAR( I, J ) .EQ. VAL__BADD ) .AND.
     :                 WTSUM( I, J ) .GT. 0.0D0 ) THEN
                     NEWVAL = DSUM( I, J ) / WTSUM( I, J )
...
*  is a new value.
                     IF ( INARR( I, J ) .EQ. VAL__BADD )
     :                 OUTARR( I, J ) = NEWVAL
                     OUTVAR( I, J ) = 2.0D0 / WTSUM( I, J )
                  END IF
               END DO
            END DO
