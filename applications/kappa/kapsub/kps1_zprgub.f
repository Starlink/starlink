      SUBROUTINE KPS1_ZPRGUB( DIM1, DIM2, LBND, UBND, NOISE,
     :                         VAR, ARRAY, STATUS )
*+
*  Name:
*     KPS1_ZPRGx
...
*          The start position cannot be an invalid pixel.
            ...
            DO WHILE ( ARRAY( I, STARTM ) .EQ. VAL__BADUB .AND.
     :                .NOT. SEDGE )
...
*          The finish position cannot be an invalid pixel.
            ...
            DO WHILE ( ARRAY( I, FINSHP ) .EQ. VAL__BADUB .AND.
     :                 .NOT. FEDGE )
...
*                may be present.
                  ...
                  IF ( NOISE .AND. VAR( I, J ) .NE. VAL__BADUB )
     :              CALL KPG1_NOISUB( .TRUE., 1, VAR( I, J ),
     :                                 ARRAY( I, J ), STATUS )
               END DO
...
*             lack of factorisation to reduce the risk of an overflow.
               ...
               DIFARR = ( NUM_UBTOD( ARRAY( I, FINSHP ) ) / SEPAR ) -
     :                  ( NUM_UBTOD( ARRAY( I, STARTM ) ) / SEPAR )

*             Similarly for the variance.  This is strictly not correct
*             but should make little difference.

               DIFVAR = ( NUM_UBTOD( VAR( I, FINSHP ) ) / SEPAR ) -
     :                  ( NUM_UBTOD( VAR( I, STARTM ) ) / SEPAR )
...
*                to the starting value to interpolate into the gap.
                  ...
                  ARRAY( I, J ) = ARRAY( I, STARTM ) +
     :                            NUM_DTOUB( DELARR )
                  VAR( I, J ) = VAR( I, STARTM ) + NUM_DTOUB( DELVAR )
...
*          The start position cannot be an invalid pixel.
            ...
            DO WHILE ( ARRAY( STARTM, J ) .EQ. VAL__BADUB .AND.
     :                 .NOT. SEDGE )
...
*          The finish position cannot be an invalid pixel.
            ...
            DO WHILE ( ARRAY( FINSHP, J ) .EQ. VAL__BADUB .AND.
     :                 .NOT. FEDGE )
...
*                may be present.
                  ...
                  IF ( NOISE .AND. TEMVAR .NE. VAL__BADUB )
     :              CALL KPG1_NOISUB( .TRUE., 1, TEMVAR, TEMARR,
     :                                 STATUS )
...
*             lack of factorisation to reduce the risk of an overflow.
               ...
               DIFARR = ( NUM_UBTOD( ARRAY( FINSHP, J ) ) / SEPAR ) -
     :                  ( NUM_UBTOD( ARRAY( STARTM, J ) ) / SEPAR )

*             Similarly for the variance.  This is strictly not correct
*             but should make little difference.

               DIFVAR = ( NUM_UBTOD( VAR( FINSHP, J ) ) / SEPAR ) -
     :                  ( NUM_UBTOD( VAR( STARTM, J ) ) / SEPAR )
...
*                to the starting value to interpolate into the gap.
                  ...
                  TEMARR = ARRAY( STARTM, J ) + NUM_DTOUB( DELARR )
                  TEMVAR = VAR( STARTM, J ) + NUM_DTOUB( DELVAR )
