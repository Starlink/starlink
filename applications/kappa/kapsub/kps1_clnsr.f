      SUBROUTINE KPS1_CLNSR( LBND, UBND, XCEN, X, STATUS )
*+
*  Name:
*     KPS1_CLNSx
...
*    point as a background estimate.
      ...
      CALL KPG1_NTHMR( .TRUE., X( LBND ), NX, NQUART, STAK, STATUS )
...
*       data limit as the maximum of the last NMIN values.
         ...
         DO I = I0, IEND, IDIRN
            ...
            DO J = 2, NMIN
               IF ( NUM_GTR( SMIN( J ), UPLIM ) ) UPLIM = SMIN( J )
            END DO

*          Limit the data to the current upper limit.

            IF ( NUM_GTR( X( I ), UPLIM ) ) X( I ) = UPLIM
