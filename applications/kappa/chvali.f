      SUBROUTINE CHVALI( DIM, INARR, OLDVAL, NEWVAL, OUTARR, STATUS )
*
*    Description :
*
...
*    Invocation :
*
*     CALL CHVALI( DIM, INARR, OLDVAL, NEWVAL, OUTARR, STATUS )
*
*    Arguments :
*
*     DIM = INTEGER( READ )
*         The dimension of the input array
*     INARR( DIM ) = INTEGER( READ )
*         The input data array
*     OLDVAL = INTEGER( READ )
*         Value to be replaced
*     NEWVAL = INTEGER( READ )
*         New value to be substituted for the %OLDVAL.
*     OUTARR( DIM ) = INTEGER( READ )
...
*    External references :
      ...
      DOUBLE PRECISION
     :  VAL_ITOD             ! Data-conversion external function
...
*    minimum detectable difference.
      ...
      FPOLD = 0.5 * VAL_ITOD( .FALSE., OLDVAL, STATUS )
      DIFF = ABS( FPOLD * VAL__EPSD )
      DO  I = 1, DIM, 1
 
*       The halving is done to prevent overflows.
 
         IF ( ABS( ( VAL_ITOD( .FALSE., INARR( I ), STATUS ) * 0.5 )
     :        - FPOLD ) .LE. DIFF ) THEN
