      SUBROUTINE KPG1_MULI( BAD, VAR, EL, A, VA, B, VB, C, VC, NERR,
     :                        STATUS )
*+
*  Name:
*     KPG1_MULx
...
*  Simply multiply the two data arrays.
      IF ( .NOT. VAR ) THEN
         CALL ERR_MARK
         CALL VEC_MULI( BAD, EL, A, B, C, IERR, NERR, STATUS )
...
*  values.
               IF ( ( A( I ) .EQ. VAL__BADI ) .OR.
     :              ( B( I ) .EQ. VAL__BADI ) ) THEN
                  C( I ) = VAL__BADI
                  VC( I ) = VAL__BADI
...
*  output variance value.
                  IF ( ( VA( I ) .EQ. VAL__BADI ) .OR.
     :                 ( VB( I ) .EQ. VAL__BADI ) ) THEN
                     VC( I ) = VAL__BADI
