      SUBROUTINE KPG1_EXPOUW( BAD, EL, INARR, VAR, INVAR, BASE, OUTARR,
...
*  Local Variables:
      ...
      INTEGER*2 MAXVAL             ! Maximum valid pixel value allowed
                                 ! for exponentiation
      DOUBLE PRECISION MAXV      ! Maximum value for the data type
      INTEGER*2 MAXVAR             ! Maximum valid variance value allowed
                                 ! for exponentiation
      DOUBLE PRECISION MINV      ! Minimum value for the data type
      INTEGER*2 MINVAL             ! Minimum valid pixel value allowed
                                 ! for exponentiation
...
*  Define a couple of useful variables.
      MINV = NUM_UWTOD( VAL__MINUW )
      MAXV = NUM_UWTOD( VAL__MAXUW )
...
*  of cases, and saves testing pixel by pixel.
         MINVAL = NUM_DTOUW( MIN( MAXV, MAX( MINV,
     :            LOG( MINV ) / LOG( BASE ) ) ) )
...
*  and bases close to 1, but it should protect the vast majority
*  of cases, and saves testing pixel by pixel.
         MAXVAL = NUM_DTOUW( MIN( MAXV, MAX( MINV,
     :            LOG( MAXV ) / LOG( BASE ) ) ) )
...
*  This is only approximate because it depends on the output value too.
         MAXVAR = NUM_DTOUW( MIN( MAXV, MAX( MINV,
...
*  Propagate a bad pixel.
                  IF ( INVAR( J ) .EQ. VAL__BADUW .OR.
     :                  OUTARR( J ) .EQ. VAL__BADUW ) THEN
                     OUTVAR( J ) = VAL__BADUW
