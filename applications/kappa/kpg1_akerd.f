      SUBROUTINE KPG1_AKERD( IBOX, JBOX, WEIGHT, SAMBAD, WLIM, NX, NY,
...
*  Arguments Given:
      INTEGER IBOX
      INTEGER JBOX
      DOUBLE PRECISION WEIGHT( -IBOX : IBOX, -JBOX : JBOX )
      LOGICAL SAMBAD
...
*  Local Variables:
      DOUBLE PRECISION BWSUM               ! Sum of weights of bad pixels
...
*  Record that the output array has at least one bad value.
               IF ( SAMBAD .AND. A( IX, IY ) .EQ. VAL__BADD ) THEN
                  B( IX, IY ) = VAL__BADD
                  BADOUT =.TRUE.
               ELSE
...
*  Form the sums.
                        IF ( A( IIX, IIY ) .NE. VAL__BADD ) THEN
