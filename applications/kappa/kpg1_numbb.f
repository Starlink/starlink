      SUBROUTINE KPG1_NUMBB( BAD, VABS, ABOVE, EL, INARR, VALUE,
     :                         NUMBER, STATUS )
*+
*  Name:
*     KPG1_NUMBx
...
*  Internal References:
      ...
      INCLUDE 'NUM_DEC_B'      ! Declarations of other routines
      INCLUDE 'NUM_DEF_CVT'      ! Definitions of conversion routines
      INCLUDE 'NUM_DEF_B'      ! Definitions of other routines
...
*  NUM routine.  VAL is an order of magnitude slower.
                     IF ( NUM_ABSB( INARR( I ) ) .GT. VALUE ) THEN
...
*  element's validity.  Note no ABS function for byte data so must call
*  NUM routine.  VAL is an order of magnitude slower.
                     IF ( NUM_ABSB( INARR( I ) ) .LT. VALUE ) THEN
