      SUBROUTINE KPG1_MEDUD( BAD, EL, ARRAY, MEDIAN, NELUSE, STATUS )
*+
*  Name:
*     KPG1_MEDUx
...
*  If the pixel is valid then increment the number-of-values counter.
               IF ( ARRAY( INDEX ) .NE. VAL__BADD ) THEN
                  NELUSE = NELUSE + 1
...
*  convert to double precision to evaluate the average.
         IF ( MOD( NELUSE, 2 ) .EQ. 0 ) THEN
            MEDIAN = NUM_DTOD( ( NUM_DTOD( ARRAY( TEST - 1 ) ) +
     :               NUM_DTOD( ARRAY( TEST -2 ) ) ) * 0.5D0 )
         ELSE
            MEDIAN = ARRAY( TEST - 1 )
         END IF
