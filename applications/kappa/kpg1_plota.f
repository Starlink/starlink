...
*        assumption such as "AGI world co-ordinates are PIXEL co-ordinates". 
...
*        with Domain AGI_WORLD will be used to describe AGI world co-ordinates.
*
...
*        Returned equal to AST__NULL if an error occurs, or if there is no 
...
*  identifier for the current AGI picture.
      CALL AGP_ASSOC( 'DEVICE', MODE, ' ', .FALSE., IPIC0, STATUS )
...
*  If the new DATA picture is to be aligned with an existing DATA picture,
*  get the Plot for the existing DATA picture.
      IF( ALIGN ) THEN
...
*  Return null values if an error has occurred, or there was no alignment.
