      SUBROUTINE KPG1_NOISB( BAD, EL, VARNCE, ARRAY, STATUS )
*+
*  Name:
*     KPG1_NOISx
...
*  noise cannot be added to the data array, so set it bad.
            IF ( ARRAY( I ) .EQ. VAL__BADB .OR.
     :           VARNCE( I ) .EQ. VAL__BADB ) THEN
               ARRAY( I ) = VAL__BADB

            ELSE

*  Find the  of the Normal distribution at the element.
               SIGMA = SQRT( NUM_BTOR( VARNCE( I ) ) )

*  Generate the random noise.  There is no offset.
               ARRAY( I ) = ARRAY( I ) +
     :                      NUM_RTOB( PDA_RNNOR( 0.0, SIGMA ) )
            END IF
         END DO
