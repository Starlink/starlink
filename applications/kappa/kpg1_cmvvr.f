      SUBROUTINE KPG1_CMVVR( NDIM, DIMS, INARR, INVAR, COMPRS, NLIM,
...
*  Test for bad pixels.
                  IF ( INARR( VO ) .NE. VAL__BADR .AND.
     :                 INVAR( VO ) .GT. 0.0 .AND.
     :                 INVAR( VO ) .NE. VAL__BADR ) THEN
...
*  corresponding data value are not bad.
                     IF ( INVAR( VO ) .NE. VAL__BADR .AND.
     :                    INVAR( VO ) .GT. 0.0 ) THEN
...
*  variance too.
     ...
     :              ABS( NUM( J ) ) .LT. VAL__SMLR ) THEN
                  OUTARR( K ) = VAL__BADR
                  OUTVAR( K ) = VAL__BADR
               ELSE
                  OUTARR( K ) = SUM( J ) / NUM( J )
                  OUTVAR( K ) = 1.0E0 / NUM( J )
               END IF
            END DO
...
*  counting array just counts pixels.
               M = J + ODIMS( 1 )
               IF ( NINT( NUM( M ) ) .LT. NVAL .OR.
     :              ABS( SUM( M ) ) .LT. VAL__SMLR ) THEN
                  OUTVAR( K ) = VAL__BADR
               ELSE
                  OUTVAR( K ) = 1.0E0 / SUM( M ) 
               END IF
            END DO
         END IF
