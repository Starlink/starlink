      SUBROUTINE KPG1_RMAPUB( EL, INARR, VALMAX, VALMIN, NUMBIN, MAP,
     :                         OUTARR, STATUS )
*+
*  Name:
*     KPG1_RMAPx
...
*  unsigned integer data types.
      DSML = NUM_UBTOD( VAL__SMLUB )
      DLRG = NUM_UBTOD( VAL__MAXUB )
...
*  Therefore a messy section of code follows
         STATUS = SAI__ERROR
         IF ( 'UB' .EQ. 'I' .OR. 'UB' .EQ. 'B' .OR. 'UB' .EQ. 'UB'
     :       .OR. 'UB' .EQ. 'W' .OR. 'UB' .EQ. 'UW' ) THEN
            CALL MSG_SETI( 'MIN', NUM_UBTOI( VALMIN ) )
            CALL MSG_SETI( 'MAX', NUM_UBTOI( VALMAX ) )
...
*  Make a double-precision copy of the data value.
         TEMP = NUM_UBTOD( INARR( I ) )
 
*  Test for a valid element.
         IF ( INARR( I ) .EQ. VAL__BADUB ) THEN
...
*  transformed bin.
            OUTARR( I ) = NUM_DTOUB( ( ( DBLE( NEWBIN ) - 0.5 )
     :                    / RNUMB ) * SCALE + DVMIN )
