...
*        synonym for "Curves").  These values apply to all lines unless
*        subsequent attributes over-ride them.  Attributes for
*        individual clipping levels can be given by replacing "Curves"
*        above by a string of the form "Nsig<i>" where "<i>" is an
*        integer index into the list of clipping levels supplied for
*        Parameter NSIGMA.  Thus, "Colour(Nsig1)" will set the colour
*        for the lines associated with the first clipping level, etc.
*        The attribute settings can be restricted to one of the two
*        lines by appending either a "+" or a "-" to the "Nsig<i>"
*        string. Thus, "Width(Nsig2-)" sets the line width for the
*        lower of the two lines associated with the second clipping
*        level, and "Width(Nsig2+)" sets the width for the upper of the
*        two lines.  [current value]
...
*  Set the plotting style for the upper (positive) line.  The plus
*  prefix requests support of temporary attributes.  Also allow for
*  more than one call to KPG1_ASSET for STYLE by using the delimiter
*  suffix.
         IF ( NCLIP .EQ. 1 ) THEN
...
*  Set the plotting style for the lower (negative) line.  The plus
