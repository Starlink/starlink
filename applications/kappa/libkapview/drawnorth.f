...
*        underlying picture. [current value]
*     BLANK = _LOGICAL (Read)
...
*        value of 1.0 results in the blanked area being just large enough
...
*        graphics database. [1.05]
*     DEVICE = DEVICE (Read)
*        The plotting device. [Current graphics device]
*     FRAME = LITERAL (Read)
*        Specifies the co-ordinate Frame to which the drawn arrows refer.
*        If a null (!) value is supplied, the arrows are drawn parallel to
...
*        dimension of the underlying picture. If only one value is supplied, 
*        both arrows will be drawn with the given length. One of the
*        supplied values can be set to zero if only a single arrow is 
...
*        used to annotate the underlying picture (supplying a colon ":" 
*        will display details of this co-ordinate Frame). [CURNDC]
*     ORIGIN = LITERAL (Read)
*        The co-ordinates at which to place the origin of the arrows, in the 
*        Frame specified by parameter OFRAME. If a null (!) value is
*        supplied, OFRAME is ignored and the arrows are positions at a
*        default position near one of the corners, or at the centre. An
...
*        parameter FRAME has more than 2 axes. A group of two strings should
*        be supplied specifying the 2 axes to which the 2 drawn arrows should
*        refer. Each axis can be specified either by its integer index within 
*        the Frame (in the range 1 to the number of axes in the current Frame),
*        or by its symbol string. A list of acceptable values is displayed if 
*        an illegal value is supplied. If a null (!) value is supplied, the 
...
*  Notes:
*     -  An error is reported if there is no existing DATA picture on the
...
*  default.
         IAXIS( 1 ) = 1
         IAXIS( 2 ) = 2
         CALL KPG1_GTAXI( 'USEAXIS', IPLOT, 2, IAXIS, STATUS )
...
*  un-selected axes.
         NEWCUR = AST_PICKAXES( IPLOT, 2, IAXIS, PMAP, STATUS )

*  Add this new Frame into the FrameSet. It becomes the Current Frame.
         CALL AST_ADDFRAME( IPLOT, AST__CURRENT, PMAP, NEWCUR, 
     :                      STATUS )
...
*  Current Frame in the Plot.
      CALL MSG_SETC( 'PIC', 'picture' )
      CALL KPG1_ASFRM( 'OFRAME', 'EPOCH', IPLOT, ' ', ' ', .TRUE.,
     :                 '^PIC', STATUS )
      
*  Get the position of the arrow origin. Check first that no error has
*  occurred.
      IF( STATUS .NE. SAI__OK ) GOTO 999
      CALL KPG1_GTPOS( 'ORIGIN', IPLOT, .FALSE., CC0, GC0, STATUS )
...
*  Convert relative values into absolute values (in millimetres).
      CALL PGQWIN( X1, X2, Y1, Y2 )
      MXSIZE = MAX( ABS( X2 - X1 ), ABS( Y2 - Y1 ) )
      MNSIZE = MIN( ABS( X2 - X1 ), ABS( Y2 - Y1 ) )
      ARROW = ARROW*MXSIZE
...
*  priority over the attribute settings in kappa_drawnorth_style.def.
      ...
      IPLOT = AST_PLOT( IPLOT, GBOX, BBOX, ' ', STATUS )
...
*  Set the attributes of the Plot to give the required plotting style.     
      CALL KPG1_ASSET( 'KAPPA_DRAWNORTH', 'STYLE', IPLOT, STATUS )
...
*  DATA picture. Leave a 3mm margin.
            GC1( 1 ) = GC0( 1 ) - XX1 + X1 + 3.0
            GC1( 2 ) = GC0( 2 ) - YY2 + Y2 - 3.0
...
*  Modify the origin so that the bounding box does not extend outside the
*  DATA picture.
            GC1( 1 ) = GC0( 1 ) + X2 - XX2 - 3.0
            GC1( 2 ) = GC0( 2 ) + Y1 - YY1 + 3.0
...
*  These are returned as positions in GRAPHICS coords.
      CALL KPS1_DNRTH( .FALSE., IPLOT, LENS, ARROW, GC0, MNSIZE,
...
*  Draw the required arrows and labels.
      CALL KPS1_DNRTH( .TRUE., IPLOT, LENS, ARROW, GC0, MNSIZE, 
     :                 XX1, XX2, YY1, YY2, STATUS )
...
*  Save the Plot with the new AGI picture.
      CALL KPG1_GDPUT( IPICK, ' ', ' ', IPLOT, STATUS )
