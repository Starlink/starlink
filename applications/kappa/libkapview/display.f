...
*  Local Variables:
      ...
      INTEGER LEL              ! Number of elements in i/p and o/p LUT arrays
      ...
      REAL PCLBND( NDIM )      ! Low pixel co-ord bounds of PGPLOT window
      REAL PCUBND( NDIM )      ! High pixel co-ord bounds of PGPLOT window
...
*  If succesful, store the bounds of the viewport for the current picture
*  (in millimetres).
...
*  Get the positive magnifications required for both axes. Use a dynamic 
*  default for YMAGN equal to the value supplied for XMAGN. A magnification 
*  of 1.0 results in the whole image being displayed within the current 
...
*  Otherwise, we can achieve the requested magnification without reducing
*  the window.
      ELSE
         XVMAG = XMAGN
         XWMAG = 1.0
...
*  correspond to different axes in the supplied NDF. Store the bounds of 
*  the sction to be displayed within the supplied NDF
...
*  which falls within the DATA picture.
...
*  ==========================
*  If the array being displayed is 1-dimensional, set a dynamic default of
...
*  cleared, set the new style, and draw it.
      IF( BORDER ) THEN
         ...
         CALL KPG1_ASSET( 'DISPLAY', 'BORSTYLE', IPLOT, STATUS )
...
*  indices. If this is the case, then the array of scaled values does not
*  correspond pixel-for-pixel to the input NDF and so cannot be saved.
*  Report an error and delete the output NDF.
...
*  Set the data type of the data component to be one of the above rather 
*  than the type of the input NDF.
...
*  Move the contents from the scaled array to the NDF data component. Since 
...
*  Add a context report if anything went wrong.
     ...
     :                 'image of a 1 or 2-dimensional data set.', 
     :                 STATUS )
      END IF
