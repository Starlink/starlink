      SUBROUTINE KPG1_MIXVR( PARAM, MAXVAL, VMIN, VMAX, OPTS, VALUES,
     :                       ACTVAL, STATUS )
...
*  Invocation:
*     CALL KPG1_MIXVR( PARAM, MAXVAL, VMIN, VMAX, OPTS, VALUES, ACTVAL,
*                     STATUS )
...
*  Local Variables:
      ...
      REAL VAL                 ! A value obtained if the input is numeric
*.
...
*  Too few values requested.
         ...
         CALL ERR_REP( 'KPG1_MIXVR_TOOFEW',
...
*  Get up to MAXVAL values from the parameter system. If too many values 
*  are supplied, report an error, flush the error, cancel the parameter 
*  and loop.
         ACTVAL = MAXVAL + 1
         ...
         DO WHILE( ACTVAL .GT. MAXVAL .AND. STATUS .EQ. SAI__OK )
            ...
            IF( IGRP .NE. GRP__NOID ) THEN
               ...
               IF( MAXVAL .GT. 1 ) THEN
                  ...
                  CALL ERR_REP( 'KPG1_MIXVR_ERR1', 'Too many values '//
     :                          '(^A) supplied for parameter %^P. '//
     :                          'Please supply up to ^M values.', 
     :                          STATUS )
               ELSE
                  CALL ERR_REP( 'KPG1_MIXVR_ERR2', 'Too many values '//
     :                          '(^A) supplied for parameter %^P. '//
...
*  Make a contextual error report.
                     CALL MSG_SETC( 'PARAM', PARAM )
                     CALL ERR_REP( 'KPG1_MIXVR_INVOPT',
...
*  Warn the user that the nearest match was used.
                     IF( PENALT .NE. 0 ) THEN
                        ...
                        CALL MSG_SETI( 'I', J )
                        CALL MSG_SETC( 'PARAM', PARAM )
                        CALL MSG_OUTIF( MSG__NORM, 'KPG1_MIXVR_MISMAT',
