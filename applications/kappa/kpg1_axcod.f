      SUBROUTINE KPG1_AXCOD( LBND, UBND, AXIS, EL, VALUE, INDEX,
     :                       STATUS )
*+
*  Name:
*     KPG1_AXCOx
...
*  Arguments Given:
      ...
      DOUBLE PRECISION VALUE( EL )
 
*  Arguments Returned:
      DOUBLE PRECISION INDEX( EL )
...
*  Local Variables:
      ...
      DOUBLE PRECISION AXMAX              ! Maximum axis value
      DOUBLE PRECISION AXMIN              ! Minimum axis value
      ...
      DOUBLE PRECISION VALTH              ! Value at the threshold element
...
*  Get the axis bounds.
      AEL = UBND - LBND + 1
      CALL KPG1_AXBND( AEL, AXIS, AXMIN, AXMAX, STATUS )
...
*  does make an error report, but continue.
         IF ( VALUE( I ) .LT. AXMIN .OR. VALUE( I ) .GT. AXMAX ) THEN
            STATUS = SAI__ERROR
            CALL MSG_SETD( 'VALUE', VALUE( I ) )
            CALL MSG_SETI( 'I', I )
            CALL MSG_SETD( 'AXMIN', AXMIN )
            CALL MSG_SETD( 'AXMAX', AXMAX )
...
*  Find the index of the element just greater than the value.
            CALL KPG1_AXGVD( AEL, AXIS, VALUE( I ), ITH, VALTH,
     :                         STATUS )
