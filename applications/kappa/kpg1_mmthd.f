      SUBROUTINE KPG1_MMTHD( BAD, EL, ARRAY, THRESH, NINVAL, MAXMUM,
...
*  Local Variables:
      INTEGER I                  ! Counter
      DOUBLE PRECISION OTHRES( 2 )        ! Thresholds where first element is the
...
*  Add some defensive code in case the thresholds are reversed.
      OTHRES( 1 ) = NUM_MIND( THRESH( 1 ), THRESH( 2 ) )
      OTHRES( 2 ) = NUM_MAXD( THRESH( 1 ), THRESH( 2 ) )
...
*  Test for valid pixel.
     ...
     :           NUM_GTD( ARRAY( I ), OTHRES( 1 ) ) .AND.
     :           NUM_LTD( ARRAY( I ), OTHRES( 2 ) ) ) THEN
 
*  Check current maximum against current pixel value.
               IF ( NUM_GTD( ARRAY( I ), MAXMUM ) ) THEN
                  MAXMUM = ARRAY( I )
                  MAXPOS = I
               END IF
 
*  Check current minimum against current pixel value.
               IF ( NUM_LTD( ARRAY( I ), MINMUM ) ) THEN
                  MINMUM = ARRAY( I )
                  MINPOS = I
               END IF
            ELSE
...
*  Loop round all the elements of the array.
         DO I = 1, EL
            ...
            IF ( NUM_GTD( ARRAY( I ), OTHRES( 1 ) ) .AND.
     :           NUM_GTD( ARRAY( I ), OTHRES( 2 ) ) ) THEN
