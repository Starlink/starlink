      SUBROUTINE KPG1_MEDUUW( BAD, EL, ARRAY, MEDIAN, NELUSE, STATUS )
*+
*  Name:
*     KPG1_MEDUx
...
*  Local Variables:
      INTEGER CURR               ! Index to current smallest value
                                 ! during sorting
      INTEGER*2 CURVAL             ! Current smallest value during sorting
...
*  If the pixel is valid then increment the number-of-values counter.
               IF ( ARRAY( INDEX ) .NE. VAL__BADUW ) THEN
                  NELUSE = NELUSE + 1
...
*  convert to double precision to evaluate the average.
         IF ( MOD( NELUSE, 2 ) .EQ. 0 ) THEN
            MEDIAN = NUM_DTOUW( ( NUM_UWTOD( ARRAY( TEST - 1 ) ) +
     :               NUM_UWTOD( ARRAY( TEST -2 ) ) ) * 0.5D0 )
         ELSE
            MEDIAN = ARRAY( TEST - 1 )
         END IF
