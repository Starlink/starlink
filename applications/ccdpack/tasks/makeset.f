...
*           - ERASE     -- All Set header information is erased.
*           - LIST      -- New Sets will be formed according to the
*                          order in which NDFs are named in the IN list.
*                          In the most straightforward case, all the
*                          named files will become part of the same new
*                          Set, with a Set Name derived from the name of
*                          the first file in the list and consecutive 
*                          Set Index values 1, 2, 3, ....  The SETSIZE 
*                          and INDICES parameters can be used to create
*                          multiple sets in one invocation and to modify
*                          the ordering.
*           - CONTAINER -- One new Set will be formed for each HDS
*                          container file; thus each NDF will be grouped
*                          with the other NDFs in the same container
*                          file, being given a Set Name based on the
*                          name of the container file and a Set Index
*                          based on its position within it.  HDS
*                          container files suitable for feeding to
*                          MAKESET with MODE=CONTAINER are often
*                          the result of converting multi-extension 
*                          FITS files to NDF format.
*
*        [LIST]
*     IN = LITERAL (Read)
...
*        (if MODE=CONTAINER).  There is normally no need to use 
*        any other value.
*        [*]
...
*  What shall we do with all these NDFs?
      CALL PAR_CHOIC( 'MODE', 'LIST', 'ERASE,LIST,CONTAINER', .TRUE.,
     :                MODE, STATUS )
...
*  Getting Set information by position in container files.
      ELSE IF ( MODE .EQ. 'CONTAINER' ) THEN
