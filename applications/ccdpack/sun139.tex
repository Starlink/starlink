...
\newcommand{\secref}[2]{\latexhtml{Section \ref{#1}}{``\htmlref{#2}{#1}''}}
...
if exists.  In fact in most cases, after adding the Set header
...
\sstroutine{DRAWNDF}{
   ...
   \sstdiytopic{
      ...
      for USEEXT corresponding to the Set Index of each image will be
...
\subsection{This release 4.0-1b}
  ...
  \item[{\bf CCDPACK Sets (mosaic cameras):}]\mbox{}\\
     ...
     \end{itemize}
     Some work remains to be done in upgrading CCDPACK for Sets,
     notably the \xroutine{XREDUCE} GUI; this will be completed for the next
     release.
