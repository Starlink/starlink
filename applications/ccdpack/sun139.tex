...
command:\protect\\
\hspace*{2em}{\tt bounds r1059* clear noaxes labname labnum}}
...
\hspace*{2em}{\tt bounds "gc6352p1-?" noclear 
                  labpos=cf style="width(curves)=2"}}

Finally, \routine{BOUNDS} can display the actual image data for you.
...
drawn over it, \routine{BOUNDS} always plots on a surface in which the 
...
\routine{BOUNDS} performs no averaging or sophisticated normalisation of
...
command:\protect\\
\hspace*{2em}{\tt bounds "r10629?" image=yes}}
...
For example, you could subsequently use \xroutine{BOUNDS} to
display the outlines of the images that the above command has
selected like this:
\begin{myquote}
\begin{verbatim}
% bounds in='^showset.lis' clear lines
\end{verbatim}
\end{myquote}
...
Previewing (\xroutine{BOUNDS}), resampling (\xroutine{TRANNDF}) and
