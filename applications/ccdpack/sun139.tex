...
\newcommand{\stardocnumber}    {139.10}
...
\newcommand{\stardocversion}   {Version 2.3-1}
...
\sstroutine{ASTEXP} {
   ...
   \sstdescription{
      ...
      \xroutine{ASTIMP}.  Currently the only way these keys can be 
      generated is by use of the FITSID parameter.
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         for instance by a WCS-aware application like KAPPA's 
      ...
      \sstsubsection{
          ...
          1,2,3,\ldots  This default will normally be appropriate unless
      ...
      \sstsubsection{
         OUTDOMAIN = LITERAL (Read)
          ...
          (by the \xroutine{ASTIMP} application).  
          For this reason it is often not a
   ...
   \sstdiytopic{
      ...
      will be applied by \xroutine{ASTIMP}, their WCS components 
      should contain
...
\sstroutine{ASTIMP}{
   ...
   \sstdescription{
      ...
      written by the \xroutine{ASTEXP} application, but with care 
      frameset files
   ...
   \sstdiytopic{
      ...
      all the Base frames should normally be the same, and likewise
      for all the Current frames.  For the NDFs to which the file
      will be applied by \routine{ASTIMP}, their WCS components 
      should contain
      frames in the same domain as the AST file's Base frame.
...
\sstroutine{REGISTER} {
   ...
   \sstexamples{
      \sstexamplesubsection{
         ...
         given the domain name \qt{CCD\_REG} in the WCS component of the
         NDFs.  Actual registration of the images can then be achieved
         by aligning all the NDFs in the CCD\_REG domain.
      }
      \sstexamplesubsection{
   ...
   \sstnotes{
      ...
      \sstitemlist{
        ...
        \xref{NDFTRACE}{sun95}{NDFTRACE} or \xref{WCSSHOW}{sun95}{WCSSHOW}.
        (\xref{SUN/95}{sun95}{}).
   ...
   \sstexamples{
      \sstexamplesubsection{
         tranlist inlist=\qs{$*$} outlist=\qs{$*$.reg} trtype=wcs
                  framein=pixel frameout=ccd\_reg
      }{
         In this example all the NDFs in the current directory are
         accessed and their associated position lists are opened.
         The WCS component of each NDF is used to transform the
         coordinates in the position lists from pixel coordinates to
         coordinates in the frame with a domain \qt{CCD\_REG} (the
         domain written by default by the \xroutine{REGISTER} program).  The
...
\sstroutine{WCSEDIT}{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         This parameter specifies the \qt{basis frame}, which has the
   ...
   \sstexamples{
      ...
      \sstexamplesubsection{
         wcsreg \qt{obs1\_$*$,obs2\_$*$} outdomain=final
                domainlist=\qt{ccd\_reg,inst\_obs1,inst\_obs2}
      } {
