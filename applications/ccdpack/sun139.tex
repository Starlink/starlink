...
when it runs.  These can be deleted when you no longer need them.
...
Finally, BOUNDS can display the actual image data for you.
Unlike KAPPA's DISPLAY program, which always displays
an image as a rectangle with pixel axes horiztal and vertical, 
perhaps with with non-orthogonal image Current coordinate axes 
drawn over it, BOUNDS always plots on a surface in which the 
...
BOUNDS performs no averaging or sophisticated normalisation of
images, so that if two images overlap in their Current coordinates,
all but the last-plotted will be obscured.  
...
\myfig{sun139outim.eps}{height=0.5\textheight}{outim.gif}{OUTIM}{BOUNDS
command with IMAGE=TRUE.  This display was generated using the
command:\protect\\
{\tt\ \ \ bounds "r10629?" image=yes}}
...
the \xroutine{BOUNDS} command with IMAGE=TRUE can be used.
...
\section{\xlabel{ccdpack_sets}Grouping images by Set --- 
         mosaic cameras and worse
         \label{ccdpack_sets}}
...
which exists in files you can set it to false globally by running
\begin{myquote}
\begin{verbatim}
...
actually unique between different Sets; for instance \routine{DEBIAS}
...
\end{itemize}
\routine{MAKESET} can operate in a number of different modes,
according to how your data are arranged in the first place.
In the most straightforward case, you can use it to make a
single Set out of a group of files listed by name:
\begin{myquote}
\begin{verbatim}
...
As the output shows, the Set Name attribute of each is ``im01'' 
(the name of the first file in the list) and the Set Index attributes
...
Multi-Extension FITS file (MEFs) per observation 
you can convert these quite easily to
HDS container files using the \xref{FITS2NDF}{sun55}{FITS2NDF} 
...
% convert
   ...
   CONVERT commands are now available -- (Version 1.3-4 2000 December)
...
For full generality, the parameter SECTIONS describes how to
split up the input frame, but for the common case of dividing the
...
registration\footnote{Actually, all except \xroutine{CCDALIGN}, 
                      for which Set functionality has not yet 
                      been implemented.}%
...
% makeset '*' mode=container addwcs=yes accept
    ...
    =======
  3 input NDFs accessed using parameter IN
ASTFILE - Name of file containing exported AST framesets /!/ > 
...
make the new one.
Note that since the new CCD\_SET coordinate system is a copy
...
the old one will be overwritten without a fatal error.

\subsection{Interrogating Set headers}
...
Here is a simple invocation, showing the result of one of the
examples in the previous section:
\begin{myquote}
\begin{verbatim}
% showset 'conv-obs?' reset accept
...
\begin{myquote}
\begin{verbatim}
% showset 'conv-obs?' listby=index accept
...
select a sublist of presented files according to the values in their
Set headers.  
Using the PICKINDEX and/or PICKNAME parameters 
...
\begin{myquote}
\begin{verbatim}
% showset 'im*' pickindex=equal index='"2,3"' pickname=like namelike='im12'
...
\text{showset.lis}; in the above case that list would read:
\begin{myquote}
\begin{verbatim}
...
This section provides a brief description of what the CCDPACK
reduction tasks (\xroutine{CALCOR}, \xroutine{DEBIAS}, \xroutine{FLATCOR},
...
(\routine{DEBIAS}, \routine{FLATCOR}, \routine{CALCOR}).
...
to group the data according to the chip which produced it
and perform the make/apply calibration cycle separately for each one.
...
\item Go through all the input files with this Set Index, correct them
      using the current calibration frame, and write them out with
...
run the \text{setexercise} script (see section \ref{demos}).
...
what the origin of each image is, only how it relates to the
...
accords maximum priority to alignment withing a Set in CCD\_SET 
coordinates, and will not allow any alignment to occur which conflicts
with that.
...
\sstroutine{BOUNDS}{
   ...
   \sstdescription{
      ...
      a set of images in their Current attached coordinate system.  This
      ...
      \routine{BOUNDS} uses the \xref{AGI graphics database}{sun95}{se_agitate} in a way which is
      compatible with \xref{KAPPA}{sun95}{} applications; if the CLEAR parameter is set
      ...
      invocation of BOUNDS or by a KAPPA program such as \xref{DISPLAY}{sun95}{DISPLAY}
...
\sstroutine{CCDALIGN}{
   ...
   \sstdescription{
      This program aids the registration of images which are not
      ...
      reference image is initially displayed and you are invited to mark
...
\sstroutine{IDICURS}{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         Gives the name of the image to display and get coordinates from.
      ...
      \sstsubsection{
         ...
         image will be used.  If set false, then the input position list
...
\sstroutine{PAIRNDF} {
   Aligns NDFs graphically by drag and drop.
} {
   \sstdescription{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         These may be specified as list of comma separated names,
         using indirection if required, {\em or\/} as a single modification
...
\sstroutine{REGISTER} {
   ...
   \sstdescription{
      ...
      list of images, or a single named one.  The \xroutine{TRANNDF} program can
      use the attached coordinate systems to resample images.
   }
   \sstusage{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         OUTDOMAIN parameter with {\tt '}\_1{\tt '}, {\tt '}\_2{\tt '}, \ldots\ appended.  The image
...
\sstroutine{WCSREG}{
   ...
   \sstdescription{
      ...
      are available between different members of a set of images.  By
...
\subsection{This release 3.2-1}
  ...
  \item
     ...
     been fixed.
  \item
     \xroutine{FINDOFF} will now propagate values in trailing columns from the
