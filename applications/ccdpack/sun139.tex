...
\newcommand{\stardocdate}      {17 April 1997}
...
The necessary ease of use (it is hoped) is achieved by limiting what could
...
When using CCDPACK from the C-shell (or any other shell) care needs to
...
escape character {\small \verb+\+} or by single quotes (wildcard characters must
be protected as these are expanded internally by CCDPACK, rather than by
the shell).
...
accumilate during exposure to a light source (you see these as a
...
frames'' as they have been called so far. These are
really \xref{NDFs}{sun33}{} (a data format based on \xref{HDS}{sun92}{}
-- the hierarchical data structure system) and their names will
consequently have a file type of `\text{.sdf}'. When using these
names in the GUI do include the `\text{.sdf}' extension, this is
contrary to how you would deal with these names in ``normal'' NDF
processing programs where you would not include the file type.

\subsubsection{GUI problems}
If you have a ungraceful exit from \xroutine{XREDUCE} you
may have some CCDPACK and \xref{KAPPA}{sun95}{} processes left running.
You will now have to kill these by hand.
How you do this is system dependent.
On BSD type systems (like Digital UNIX) try:
\begin{myquote}
\begin{verbatim}
% ps ux
\end{verbatim}
...
\section{CCDPACK's data format}
CCDPACK uses data stored in the NDF format (the N-dimensional data
...
systems that CCDPACK runs on (Digital UNIX, Solaris and Linux at this time)
...
(\xref{SUN/55}{sun55}{}) contains many programs for doing this. The
KAPPA package (\xref{SUN/95}{sun95}{}) contains an application
\xref{FITSIN}{sun95}{FITSIN} that reads FITS tapes and writes the
results as NDFs. You can get a complete list of all the FITS headers of all
the files on a tape (or list of files)
using the KAPPA application \xref{FITSHEAD}{sun95}{FITSHEAD}.
...
\begin{figure}
\centering\epsfig{file=sun139geo.eps}
\caption{
...
Before you can ask CCDPACK to schedule a reduction it is necessary
to put your data frames through a process known as ``importing''. What
...
When running \routine{PRESENT} it's a good idea to check the output to make sure
that your frames are given the correct frame type. The known frame
types are:
\begin{itemize}
...
given under the ``\text{Item}'' column. These are the names of the extension
items stored in your data frames. Most of their meanings are fairly
obvious as they correspond to the \xroutine{CCDSETUP} parameters with
similar names.
\begin{itemize}
...
\end{verbatim}
\end{myquote}
assuming that only the frames to be processed where in the current
...
destination analysis package does not make use of data errors, or if
...
aways terminate the application immediately. (Note that \text{adc},
...
\begin{figure}
\centering\epsfig{file=sun139red.eps,height=0.85\textheight}
...
\routine{PAIRNDF} displays the NDFs to be registered and allows the direct
selection of image features which are common between image pairs.
This works by displaying all the NDFs in a pallette region down the
...
entered this information into the NDFs using \xroutine{REGISTER}. The choice then
...
\begin{verbatim}
% display  ndf                                     [1]
% idicurs  in=ndf outlist=ndf.approx               [2]
% findcent inlist=ndf outlist=ndf.acc              [3]
% tranlist trtype=struct inlist=ndf forward        [4]
           outlist=ndf.ref
% tranlist inlist=ndf inext=false                  [5]
           outlist=ndf.look
           transform=newndf.more.ccdpack.transform
           forward=false
\end{verbatim}
\end{myquote}
...
position list \text{ndf.approx} and associated with the NDF.
\item \xroutine{FINDCENT} centroids the positions. The new position list is
\text{ndf.acc} which is now associated with the NDF.
...
coordinate system to coordinate system of \text{newndf}, using the
inverse transform stored in \text{newndf}.
\end{enumerate}
...
and can be accessed without any further action.
...
Position lists may be associated with NDFs using the \routine{CCDEDIT}
...
file using indirection, this is exactly the same as for naming NDFs
...
the input NDF names are {\em always} used.
...
\xroutine{CCDEDIT} routines and resamples a list of NDFs. \routine{TRANNDF} will use any
...
NDFs the same as the inputs except that the string `\text{-trn}' is
...
list currently associated with an NDF.
...
capabilities. In its default mode \routine{MAKEMOS} just combines NDFs using a
...
\begin{figure}
\centering\epsfig{file=sun139reg.eps,height=0.85\textheight}
...
performed. The content of such files is made much simpler if use of
CCDPACK's NDF list accessing facility is made. The best way to do this
is to get your data files organised. This organisation can be
performed in several different ways, I prefer the first method\ldots
...
parameter \text{logto} which is set by \xroutine{CCDSETUP}. \text{logto} is a
character variable and can take one of the following values:
\begin{myquote}
\begin{description}
...
\begin{verbatim}
   ...
   IN > NGC2261_?           (all NDFs whose names begin with NGC2261,
                             followed by one extra character)
\end{verbatim}
\end{myquote}

The names of NDFs given to programs (except for \xroutine{XREDUCE})
should not include the \text{.sdf} file extension.
...
\end{verbatim}
\end{myquote}
means call all the output NDFs the same name as the associated
input NDFs, but append the string `\text{\_debias}' to the names. A
...
This will end with the NDF names having the string `\text{debias}'
...
Which in this case will take all the NDFs `\text{*\_ccd}' from one
directory and create new NDFs in the second directory replacing any
occurrences of the string
`\text{\_ccd}' with `\text{-pr}'. {\em Remember the
`\text{*}' in NDF output expressions represents only the names of
...
The CCDPACK routine \xroutine{DEBIAS} allows regions to be defined as having poor
...
and the badbits flag --- see \xref{SUN/33}{sun33}{}) or by interpreting
bad-region commands within an ordinary text file (an ASCII
region definition file --- \xref{ARD}{sun183}{} file).

Setting regions of an NDF to bad can be done in KAPPA (SUN/95) by use
of the \xref{ZAPLIN}{sun95}{ZAPLIN} or \xref{SEGMENT}{sun95}{SEGMENT}
routines or by the applications \xref{ARDGEN}{sun95}{ARDGEN} and
\xref{ARDMASK}{sun95}{ARDMASK} working in cooperation. However rather
than use this latter option you can just use the ARD file generated by
ARDGEN.

The capabilities of the ARD option (which uses considerably less disk
space than the NDF option and hence could form part of a `database')
are described below (for more details see \xref{SUN/183}{sun183}{}).
...
development of CCDPACK. David Berry is thanked for supplying the NDF
list access software. Rodney Warren-Smith developed the excellent
\xroutine{MAKEMOS} application. \xroutine{TRANNDF} uses the internals of
an early version of the KAPPA application
\xref{TRANSFORMER}{sun95}{TRANSFORMER} written by Malcolm Currie.
...
In the routines \xroutine{MAKEBIAS}, \xroutine{MAKECAL} and \xroutine{MAKEFLAT} input NDFs which have
...
\quickdes{IMPORT}{Imports FITS information into NDFs.}{IMPORT}
...
\sstroutine{FINDOBJ} {
   ...
   \sstexamples{
      \sstexamplesubsection{
         ...
         In this example \routine{FINDOBJ} processes all the NDFs in the current
...
\sstroutine{FLATCOR} {
   ...
   \sstparameters{
      \sstsubsection{
         ...
         such as \xroutine{MAKEFLAT}. The data should have a floating point HDS
...
\sstroutine{IDICURS}{
   ...
   \sstexamples{
      ...
      \sstexamplesubsection{
         ...
         the centroid routine \xroutine{FINDCENT} would then give each NDF an
...
\sstroutine{MAKEMOS} {
   ...
   \sstdescription{
      ...
      \routine{MAKEMOS} registers the set of NDFs supplied by matching their
...
\sstroutine{PRESENT} {
   ...
   MASTER\_FLASH (these are input as different groups of NDFs).
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         information using \xroutine{IMPORT}, or the NDFs have already been
   ...
   \sstexamples{
      \sstexamplesubsection{
         ...
         In this example \routine{PRESENT} processes all the NDFs in the current
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         \begin{myquote}
            OUT=$*$\qt{debiasext}
         \end{myquote}
         form the names of the NDFs output from \xroutine{DEBIAS}.
         [-db]
      }
      \sstsubsection{
      ...
      \sstsubsection{
         ...
         \begin{myquote}
            OUT=$*$\qt{flatext}
         \end{myquote}
         form the names of the NDFs output from \xroutine{FLATCOR}.
         [-flt]
      }
      \sstsubsection{
         IN = LITERAL (Read)
      ...
      \sstsubsection{
         ...
         of \xroutine{FLATCOR} each NDF is deleted in turn, so the additional disk
...
\sstroutine{TRANNDF}{
   ...
   \sstdescription{
      ...
      \xroutine{REGISTER} and \xroutine{CCDEDIT}. The output NDFs are calculated by resampling
...
\section{Notes for IRAF users}
CCDPACK is available as part of the Starlink package\ldots
...
In this release CCDPACK can now access foreign data types and is also
now available from the IRAF/CL command language.
