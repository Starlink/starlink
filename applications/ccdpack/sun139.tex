...
a fatal error.
For instance, if one of the programs tries to add a frame 
when one with the same domain already exists, the old one is deleted.
\item
...
as long as the Current coordinate frames of the images in question
are related by a simple offset then \routine{FINDOFF} can find it.
...
by unsetting the \text{usewcs} parameter in either case).
...
(see \hyperref{later}{section \S}{}{wcsdirect}).
...
\subsection{\xlabel{wcsdirect}\label{wcsdirect}Handling the WCS component 
            directly}
...
\subsection{\xlabel{astfiles}\label{astfiles}Re-use of WCS 
...
is known and stored in an AST file as 
...
% wcsedit in='"2a,2b,2c"' mode=add frame=obs2 domain=obs2-rot 
...
two commands above could be replaced by:
\begin{myquote}
\begin{verbatim}
...
% findoff inlist='"1b,1c,2b,2c"' outlist='*.off'
% register inlist='"1b,1c,2b,2c"' fittype=1
\end{verbatim}
\end{myquote}
Now the frames contained in the WCS components of the NDFs 
are (amongst others) as follows:
\begin{center}
...
adding a new frame with the \routine{WCSEDIT} program.
...
The mapping of input grid to output grid is given by the
mapping between the PIXEL-domain frame and the Current frame 
in the WCS component of the NDF; however an additional shrinking
