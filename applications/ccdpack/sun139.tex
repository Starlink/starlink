...
a single Set of data which may contain more than one frame.
...
in some other way --- there is no other header information needed.
However a CCD\_SET-domain coordinate system will only be treated as
special when the images in question have valid Set headers.
...
So if you have a number of frames from a three-chip mosaic camera 
sorted into Sets to debias, you can replace a sequence like this:
\begin{myquote}
\begin{verbatim}
% makebias in='bias-seta-*' out=master_bias-a
% makebias in='bias-setb-*' out=master_bias-b
% makebias in='bias-setc-*' out=master_bias-c
% debias in='data-seta-*' bias=master_bias-a out='debias_*'
% debias in='data-setb-*' bias=master_bias-b out='debias_*'
% debias in='data-setc-*' bias=master_bias-c out='debias_*'
\end{verbatim}
\end{myquote}
by this one, which does just the same thing:
\begin{myquote}
\begin{verbatim}
% makebias in='bias-set*-*' out=master_bias 
% debias in='data-set*-*' bias=master_bias out='debias_*'
\begin{verbatim}
\end{myquote}
...
Note that there is currently no easy way for specifying
