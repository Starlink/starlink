...
\newcommand{\stardocnumber}    {139.11}
...
components of the images.  For much of the time, and in particular
...
Some of these
labels are special, for instance a coordinate system labelled 
`SKY' indicates
...
These coordinate systems are understood by other Starlink applications
so that for instance using the KAPPA command
\begin{myquote}
\begin{verbatim}
% display myimage axes accept
\end{verbatim}
\end{myquote}
...
% the WCS component.
% The upshot of this is that when an NDF has a suitable WCS component
% its contents can be addressed in the coordinates of its Current
% frame, rather than just in pixel coordinates.
...
for adding coordinate systems to sets of NDFs, 
...
remove them, or change which one is Current for an image when 
...
between a set of images when this is possible.
\end{description}
...
for using coordinate systems associated with image files
...
Currently, this cannot be done using the normal coordinate system method,
and the older method using TRANSFORM structures must be used instead ---
see \hyperref{this section}{appendix \S}{}{transformreg}.
...
(related by a geometric {\em translation})
...
Note it is not necessary that the datasets themselves are simply 
...
The telescope was rotated and shifted between the observations so that the 
...
Current coordinate system, decreasing runtima and the likelihood
of a false match.
...
methods that CCDPACK used to use for storing coordinate transformations,
...
so that if you want to treat an
image as if it had no special coordinate systems attached you can
just run
\begin{myquote}
\begin{verbatim}
% wcsedit myimage current pixel
\end{verbatim}
\end{myquote}
...
To change the coordinate frame to be used as the Current one
...
When adding a new 
frame you must give the transformation which connects it 
...
A copy of an existing coordinate system, but with a new domain,
...
It's also possible to make fine adjustments to the the coordinate
...
\subsection{\xlabel{astfiles}\label{astfiles}Re-use of coordinate systems
...
which are aligned in their Current coordinate frame 
...
There is another example of the use of \routine{ASTIMP} in 
...
a large set of images, some of which overlap and some of which don't.
The overlapping ones can be registered using the object-matching
techniques described \hyperref{earlier}{in section \S}{}{object_matching},
but the others contain an approximate SKY coordinate system
inserted by the observing system.  In this case we would like
to use the highly accurate object matching coordinates to do the
alignment where we have them, but in their absence fall back to
...
(usually the first one listed).  Because of this, if you have
...
which are aligned in a SKY coordinate system, you can first do
\begin{myquote}
\begin{verbatim}
% wcsreg 'skyimage*' '[sky]'
\end{verbatim}
...
Note that \routine{TRANNDF} resamples into the Current coordinate frame,
so that each pixel in the output image is a $1.0 \times 1.0$ square 
in those coordinates.  Thus the coordinate frame in question must be
a suitable one for that purpose.  If the frame yields pixels of 
the wrong size, then a suitable transformation can be made by
adding a new frame with the \xroutine{WCSEDIT} or \xroutine{WCSREG} 
programs.
...
\end{itemize}
programs.  To use \routine{MAKEMOS} the images must first have been
...
does not need to operate on images which have already been resampled
...
Full descriptions of \xroutine{DRIZZLE}
and \xroutine{MAKEMOS} are given in appendix \S\ref{descriptions}.
Since \routine{DRIZZLE} does its own resampling, the remarks 
...
% 
% Control of the new style WCS coordinate information should be
% done using the \routine{WCSEDIT} program as described
% \hyperref{earlier}{in section \S}{}{wcsdirect}.
% 
...
\subsection{\xlabel{regexamples}\label{regexamples}Examples of registration}
...
Additionally, we can tell FINDOFF (by setting the RESTRICT parameter)
...
\begin{myquote}
\begin{verbatim}
% findobj in='data*' outlist='*.obj' accept
% findoff inlist='data*' outlist='*.off' restrict=true accept
% register inlist='*.off' accept
\end{verbatim}
\end{myquote}
The REGISTER program has now added a new pixel-like coordinate system
...
There is overlap between some, but not all, of the images 
...
\end{verbatim}
\end{myquote}
Note that unlike the previous example we do not use 'RESTRICT=TRUE' here,
since although the orientations are now right the positionings may not be.
...
added to these images (ignoring the ever-present GRID, PIXEL and AXIS):
\begin{center}
...
\end{verbatim}
\end{myquote}
since CCD\_REG is listed first, alignment will be made in 
these coordinates in preference to the others where possible.
...
resample and mosaic as in the previous example:
\begin{myquote}
\begin{verbatim}
% tranndf '"1?,2?"' '*-r'
% makemos '*-r' mosaic
...
including taking account of a pincushion distortion,
...
The IDTYPE=FITSID and FITSID=CHIPNAME parameters 
...
\begin{myquote}
\begin{verbatim}
% astimp 'o1059??' astfile=INT-WFC.ast reset accept
...
It's not perfect; there are still optical distortions at two
corners and alignment of CCD 2 is poor.  
...
they all have a \verb+*!+ indicator before their descriptions. You
...
\sstroutine{WCSREG}{
   ...
   \sstexamples{
      ...
      \sstexamplesubsection{
         wcsreg [obs1\_$*$,obs2\_$*$] outdomain=final
...
is also currently necessary for certain nonlinear kinds of transformations
which cannot be handled by the new system,
however this functionality will be added to the new system shortly.
...
\subsection{\xlabel{generaltransforms}General nonlinear transformations
\label{general_transforms}}

Using the TRANSFORM-based methods, CCDPACK can fit and use more
general transformations than those which can currently be encoded in 
the newer attached coordinate system method.
...
linear transformation is not good enough to register
datasets accurately.
...
When using the old TRANSFORM-based methods however this can be required,
...
\begin{itemize}
\item pairs of coordinate systems associated with an image
