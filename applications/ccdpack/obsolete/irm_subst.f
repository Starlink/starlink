      SUBROUTINE IRM_SUBST( INSTR, OLD, NEW, ALL, OUTSTR, NSUB, STATUS )
*+
*  Name:
*     IRM_SUBST
...
*  Invocation:
*     CALL IRM_SUBST( INSTR, OLD, NEW, ALL, OUTSTR, NSUB, STATUS )

*  Description:
*     This routine copies the input string to the output string,
*     replacing occurences of a given substring with another given
...
*     ALL = LOGICAL (Given)
*        If true, then all occurences of OLD will be replaced.
*        Otherwise, only the first occurence will be replaced.
...
*        than the total number of occurences of OLD within INSTR. If ALL
...
*  Local Variables:
      ...
      INTEGER OLDPOS             ! Index within the remaining part of
                                 ! INSTR at which the next occurence of
                                 ! OLD starts.
      ...
      INTEGER START              ! The index within INSTR at which the
                                 ! search for the next occurence of OLD
                                 ! will commence.
...
*  Find the start of the next occurence of the old substring within the
...
*  occurence of the old substring amd the previous occurence, to the
*  end of the output string. If there was no gap between the two
*  occurences, leave the output string as it is.
            IF( OLDPOS .GT. 1 ) THEN
...
*  Update the position at which the search for the next occurence of the
...
*  If only the first occurence is to be replaced, copy the remainder of
