 ...
 *     as the function value. Finally, a call to LIB$FIND_FILE_END with the
 *     last returned value of Context will close down any files opened or
 *     memory allocated by LIB$FIND_FILE.
 *
 *  Language:
 ...
 *  Call:
 *     (from Fortran)
 *     STATUS = LIB$FIND_FILE (FILESPEC,FILENAME,CONTEXT)
 *
 *  Parameters:
...
int lib$find_file_ (FileSpec,FileName,Context,SpecLength,NameLength)
   char *FileSpec;                /* Pointer to file specification */
   char *FileName;                /* Pointer to string to return file name in */
   void **Context;                /* Used to remember context of search */
   int SpecLength;                /* Number of bytes in FileSpec */
   int NameLength;                /* Number of bytes in FileName */

   /*  Note that this matches the way MOST UNIX Fortran compilers will 
    *  generate a call to LIB$FIND_FILE(FILESPEC,FILENAME,CONTEXT) with
    *  FILESPEC and FILENAME character strings and CONTEXT an integer.
    */
{
   /*  Local variables  */
   ...
   char Command[80];     /* 'ls' command executed */
   ...
   int MaxChars;         /* Maximum number of non-null characters in FileName */
      ...
      if ((int) ContextPtr == 0) {
         ...
         if (pipe(Fdptr) < 0) {
            ...
            if ((STATUS = fork()) == 0) {
               ...
               for (I = 3; I < (Nchars + 3); I++) Command[I] = *FileSpec++;
      ...
      while (!GotFileName) {
         ...
         if (LastChar != ' ') {
            if (LastChar == ':') {
               ...
               (void) strncat(FileName,Line,NameLength,Nchars);
               FileName[NameLength - 1] = '\0';
 ...
 *
 *  Function:
 *     Terminate a sequence of LIB$FIND_FILE calls.
 *
 *  Description:
 *     This routine should be called after a sequence of calls to LIB$FIND_FILE
 *     in order to release any resources used by LIB$FIND_FILE.  It should be
 *     passed in its Context argument the value of the Context argument 
 *     as returned by the LIB$FIND_FILE in the last call in the sequence that
...
int lib$find_file_end_ (Context)
   void **Context;
{
   char Char;                        /* Character reead from pipe */
