...
*        usage.
*     30-OCT-1996 (PDRAPER):
*        Renamed NAGMAKEMOS. Denagged version called MAKEMOS.
*     3-MAR-1997 (PDRAPER):
...
*  Start up the logging system.
      CALL CCD1_START( 'NAGMAKEMOS', STATUS )
...
*  normalise the corrections.
      IF ( GETS .OR. GETZ ) THEN
         CALL ERR_MARK
         CALL NDF_ASSOC( 'REF', 'READ', NDFREF, STATUS )
...
*  Allocate workspace.
         IF ( GETS .AND. GETZ ) THEN
            CALL PSX_CALLOC( 2 * NNDF, '_DOUBLE', WRK1, STATUS )
            CALL PSX_CALLOC( ( 2 * NCMP + 2 ) * ( 2 * NNDF + 1 ),
     :                       '_DOUBLE', WRK2, STATUS )
            CALL PSX_CALLOC( ( 2 * NNDF ) * ( 2 * NNDF + 1 ),
     :                       '_DOUBLE', WRK3, STATUS )
            CALL PSX_CALLOC( 18 * NNDF +
     :                       4 * ( 1 + NCMP + NNDF * ( NNDF + NCMP ) ),
     :                       '_DOUBLE', WRK4, STATUS )
         ELSE
...
*  additional reference NDF, if provided.
         CALL NAG1_SZSLV( GETS, GETZ, NNDF, NCMP, SCALE, DSCALE, ZERO,
     :                    DZERO, ORIGIN, %VAL( WRK1 ), %VAL( WRK2 ),
