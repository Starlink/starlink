...
proc tkConsoleBind {win} {
	...
	if {[%q tag nextrange sel 1.0 end] != ""} {
	    %q tag remove sel sel.first promptEnd
	} else {
	    if [%q compare insert < promptEnd] {
		break
	    }
	}
    }
	...
	bind $win <$left> {
	    if [%q compare insert < promptEnd] {
		tkTextSetCursor %q {insert linestart}
	    } else {
		tkTextSetCursor %q promptEnd
            }
	    break
	}
    }
	...
	bind $win <$right> {
	    tkTextSetCursor %q {insert lineend}
	    break
	}
    }
    bind $win <Control-d> {
	...
	if [%q compare insert > promptEnd] {
		...
		%q insert insert [selection get -displayof %W] {input stdin}
		%q see insert
	    }
	}
	break
...
proc tkConsoleInsert {w s} {
    ...
    if {[$q compare insert < promptEnd]} {
	$q mark set insert end	
    }
    $q insert insert $s {input stdin}
    $q see insert
}
