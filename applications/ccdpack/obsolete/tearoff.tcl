...
proc tkTearOffMenu w {
    ...
    set parent [winfo parent $w]
    set qparent [winfo command $parent]
    switch [winfo class $parent] {
	Menubutton {
	    wm title $menu [$qparent cget -text]
	}
	Menu {
	    wm title $menu [$qparent entrycget active -label]
	}
    }

    $qmenu configure -tearoff 0
    $qmenu post [winfo x $w] [winfo y $w]
    ...
    set cmd [[winfo command $w] cget -tearoffcommand]
    if {$cmd != ""} {
...
proc tkMenuDup {src dst} {
	...
	set cmd [list $dst add [$qsrc type $i]]
