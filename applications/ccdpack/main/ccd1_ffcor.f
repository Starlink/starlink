...
*  these values should not be modified.
            IF ( .NOT. SETSAT ) THEN
               CALL CCG1_RFFUB( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                         %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                         HAVFV, %VAL( IPDOUT ), %VAL( IPVOUT ),
     :                         NERR, STATUS )
            ELSE

*  Have saturated values
               CALL CCG1_RFSUB( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                          %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                          HAVFV, SATVAL, %VAL( IPDOUT ),
...
*  these values should not be modified.
            IF ( .NOT. SETSAT ) THEN
               CALL CCG1_RFFB( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                         %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                         HAVFV, %VAL( IPDOUT ), %VAL( IPVOUT ),
     :                         NERR, STATUS )
            ELSE
...
*  Have saturated values
               CALL CCG1_RFSB( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                          %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                          HAVFV, SATVAL, %VAL( IPDOUT ),
     :                          %VAL( IPVOUT ), NERR, STATUS )
            END IF
...
*  these values should not be modified.
            IF ( .NOT. SETSAT ) THEN
               CALL CCG1_RFFUW( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                         %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                         HAVFV, %VAL( IPDOUT ), %VAL( IPVOUT ),
     :                         NERR, STATUS )
            ELSE
...
*  Have saturated values
               CALL CCG1_RFSUW( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                          %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                          HAVFV, SATVAL, %VAL( IPDOUT ),
     :                          %VAL( IPVOUT ), NERR, STATUS )
            END IF
...
*  these values should not be modified.
            IF ( .NOT. SETSAT ) THEN
               CALL CCG1_RFFW( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                         %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                         HAVFV, %VAL( IPDOUT ), %VAL( IPVOUT ),
     :                         NERR, STATUS )
            ELSE
...
*  Have saturated values
               CALL CCG1_RFSW( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                          %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                          HAVFV, SATVAL, %VAL( IPDOUT ),
     :                          %VAL( IPVOUT ), NERR, STATUS )
            END IF
...
*  these values should not be modified.
            IF ( .NOT. SETSAT ) THEN
               CALL CCG1_RFFI( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                         %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                         HAVFV, %VAL( IPDOUT ), %VAL( IPVOUT ),
     :                         NERR, STATUS )
            ELSE
...
*  Have saturated values
               CALL CCG1_RFSI( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                          %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                          HAVFV, SATVAL, %VAL( IPDOUT ),
     :                          %VAL( IPVOUT ), NERR, STATUS )
            END IF
...
*  these values should not be modified.
            IF ( .NOT. SETSAT ) THEN
               CALL CCG1_RFFR( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                         %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                         HAVFV, %VAL( IPDOUT ), %VAL( IPVOUT ),
     :                         NERR, STATUS )
            ELSE
...
*  Have saturated values
               CALL CCG1_RFSR( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                          %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                          HAVFV, SATVAL, %VAL( IPDOUT ),
     :                          %VAL( IPVOUT ), NERR, STATUS )
            END IF
...
*  these values should not be modified.
            IF ( .NOT. SETSAT ) THEN
               CALL CCG1_RFFD( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                         %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                         HAVFV, %VAL( IPDOUT ), %VAL( IPVOUT ),
     :                         NERR, STATUS )
            ELSE
...
*  Have saturated values
               CALL CCG1_RFSD( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                          %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                          HAVFV, SATVAL, %VAL( IPDOUT ),
     :                          %VAL( IPVOUT ), NERR, STATUS )
            END IF
...
*  ==========================
      ...
      ELSE IF ( FTYPE .EQ. '_DOUBLE' ) THEN
         IF ( DTYPE .EQ. '_UBYTE' ) THEN
            IF ( .NOT. SETSAT ) THEN
               CALL CCG1_DFFUB( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                         %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                         HAVFV, %VAL( IPDOUT ), %VAL( IPVOUT ),
     :                         NERR, STATUS )
            ELSE
...
*  Have saturated values
               CALL CCG1_DFSUB( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                          %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                          HAVFV, SATVAL, %VAL( IPDOUT ),
     :                          %VAL( IPVOUT ), NERR, STATUS )
            END IF
         ELSE IF ( DTYPE .EQ. '_BYTE' ) THEN
            IF ( .NOT. SETSAT ) THEN
               CALL CCG1_DFFB( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                         %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                         HAVFV, %VAL( IPDOUT ), %VAL( IPVOUT ),
     :                         NERR, STATUS )
            ELSE
...
*  Have saturated values
               CALL CCG1_DFSB( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                          %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                          HAVFV, SATVAL, %VAL( IPDOUT ),
     :                          %VAL( IPVOUT ), NERR, STATUS )
            END IF
         ELSE IF ( DTYPE .EQ. '_UWORD' ) THEN
            IF ( .NOT. SETSAT ) THEN
               CALL CCG1_DFFUW( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                         %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                         HAVFV, %VAL( IPDOUT ), %VAL( IPVOUT ),
     :                         NERR, STATUS )
            ELSE
...
*  Have saturated values
               CALL CCG1_DFSUW( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                          %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                          HAVFV, SATVAL, %VAL( IPDOUT ),
     :                          %VAL( IPVOUT ), NERR, STATUS )
            END IF
         ELSE IF ( DTYPE .EQ. '_WORD' ) THEN
            IF ( .NOT. SETSAT ) THEN
               CALL CCG1_DFFW( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                         %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                         HAVFV, %VAL( IPDOUT ), %VAL( IPVOUT ),
     :                         NERR, STATUS )
            ELSE
...
*  Have saturated values
               CALL CCG1_DFSW( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                          %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                          HAVFV, SATVAL, %VAL( IPDOUT ),
     :                          %VAL( IPVOUT ), NERR, STATUS )
            END IF
         ELSE IF ( DTYPE .EQ. '_INTEGER' ) THEN
            IF ( .NOT. SETSAT ) THEN
               CALL CCG1_DFFI( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                         %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                         HAVFV, %VAL( IPDOUT ), %VAL( IPVOUT ),
     :                         NERR, STATUS )
            ELSE
...
*  Have saturated values
               CALL CCG1_DFSI( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                          %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                          HAVFV, SATVAL, %VAL( IPDOUT ),
     :                          %VAL( IPVOUT ), NERR, STATUS )
            END IF
         ELSE IF ( DTYPE .EQ. '_REAL' ) THEN
            IF ( .NOT. SETSAT ) THEN
               CALL CCG1_DFFR( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                         %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                         HAVFV, %VAL( IPDOUT ), %VAL( IPVOUT ),
     :                         NERR, STATUS )
            ELSE
...
*  Have saturated values
               CALL CCG1_DFSR( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                          %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                          HAVFV, SATVAL, %VAL( IPDOUT ),
     :                          %VAL( IPVOUT ), NERR, STATUS )
            END IF
         ELSE IF ( DTYPE .EQ. '_DOUBLE' ) THEN
            IF ( .NOT. SETSAT ) THEN
               CALL CCG1_DFFD( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                         %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                         HAVFV, %VAL( IPDOUT ), %VAL( IPVOUT ),
     :                         NERR, STATUS )
            ELSE
...
*  Have saturated values
               CALL CCG1_DFSD( BAD, EL, %VAL( IPDIN ), %VAL( IPVIN ),
     :                          %VAL( IPDFLT ), %VAL( IPVFLT ), HAVDV,
     :                          HAVFV, SATVAL, %VAL( IPDOUT ),
     :                          %VAL( IPVOUT ), NERR, STATUS )
            END IF
