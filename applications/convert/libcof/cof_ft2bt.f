      SUBROUTINE COF_FT2BT( TABLE, FUNIT, EXTNAM, STATUS )
*+
*  Name:
*     COF_FT2BT
...
*  Invocation:
*     CALL COF_FT2BT( TABLE, FUNIT, EXTNAM, STATUS )
...
*  Put this data into the FITS file. Do each data type in turn.
     ...
     :                   FSTAT )
         ...
         ELSE IF( CTYPE .EQ. AST__SINTTYPE ) THEN
            CALL FTPCLI( FUNIT, ICOL, 1, 1, TOTNEL, %VAL(CNF_PVAL(IP)),
     :                   FSTAT )

         ELSE IF( CTYPE .EQ. AST__BYTETYPE ) THEN
            CALL FTPCLB( FUNIT, ICOL, 1, 1, TOTNEL, %VAL(CNF_PVAL(IP)),
     :                   FSTAT )

         ELSE IF( CTYPE .EQ. AST__DOUBLETYPE ) THEN
            CALL FTPCLD( FUNIT, ICOL, 1, 1, TOTNEL, %VAL(CNF_PVAL(IP)),
     :                   FSTAT )

         ELSE IF( CTYPE .EQ. AST__FLOATTYPE ) THEN
            CALL FTPCLE( FUNIT, ICOL, 1, 1, TOTNEL, %VAL(CNF_PVAL(IP)),
     :                   FSTAT )

         ELSE IF( CTYPE .EQ. AST__STRINGTYPE ) THEN
            CALL FTPCLS( FUNIT, ICOL, 1, 1, TOTNEL, %VAL(CNF_PVAL(IP)),
     :                   FSTAT )
...
*  Report an error if anything went wrong in a FITSIO routine.
      IF( FSTAT .GT. FITSOK .AND. STATUS .EQ. SAI__OK ) THEN
         CALL COF_FIOER( FSTAT, 'COF_FT2BT_ERR', ' ', 'Failed to read'//
