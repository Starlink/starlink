...
*  Description:
*     This application converts an NDF to a Figaro (Vn. 2) `.DST' file.
...
*           (Similarly for AXIS(2:6) which are renamed to 
*            .Y .T .U .V or .W) 
*          
*          FIGARO extension:
*          .MORE.FIGARO.xxx   -> .xxx
...
*      Look out for an empty extension.
         IF ( NCOMP .GT. 1 .AND. STATUS .EQ. SAI__OK ) THEN

            DO I = NCOMP, 1, -1

*            Obtain a locator to the component, find its name and then
*            copy it to the Figaro structure.
...
*   Deal with axis structure.
*   =========================

      DO IAXIS=1, NDIM

*      Determine wether or not there is an axis structure in the NDF.
...
*         Create the axis.DATA structure.
            CALL DAT_NEW  (LFAX, 'DATA', TYPE, 1, DIM(IAXIS), STATUS)
            CALL DAT_FIND (LFAX, 'DATA', LFAXD, STATUS)

*         Map FIGARO axis data.
            CALL DAT_MAP (LFAXD, TYPE, 'WRITE', 1, DIM(IAXIS), FAXPTR, 
     :                    STATUS)

*         Move the NDF axis data into Figaro axis data array.
            CALL CON_TYPSZ (TYPE, NBYTES, STATUS)         
            IF ((STATUS .EQ. SAI__OK) .AND. (DIM(IAXIS) .GT. 0)) THEN
