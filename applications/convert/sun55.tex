...
\stardoccopyright   {Copyright \copyright\ 2015 Science and Technology Research Council}
\stardocauthors     {Malcolm J. Currie\\
                                G.J.Privett\\
                                A.J.Chipperfield\\
                                D.S.Berry\\
                                A.C.Davenhall}
\stardocdate        {2015 December 14}
...
between Starlink's Extensible \emph{n}-dimensional Data Format
...
from a variety of Starlink or other packages. \par Most of the
\CONVERT\ utilities may be run from the shell or \ICLref\ in the
normal way, or invoked automatically by the NDF library's `on-the-fly'
data-conversion system, but there are also \IDLref\ procedures for
handling NDFs from within IDL.}
% -----------------------------------------------------------------------------
...
there are numerous formats for storing \emph{n}-dimensional astronomical data
...
(Extensible \emph{n}-Dimensional-data format)---which most Starlink packages
are already using.
...
NDF\_FROM\_\-\emph{fmt}\_\-PARS or NDF\_TO\_\emph{fmt}\_PARS as appropriate
(where \emph{fmt} is the format name).
...
Where the `Variable' column contains a tick, the variable \emph{must} be used
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      Telescope}{http://www.eaobservatory.org/jcmt} (JCMT) in
      Hawaii.
