...
\newcommand{\stardocnumber}    {55.19}
\newcommand{\stardocauthors}   {Malcolm J. Currie\\
                                G.J.Privett\\
                                A.J.Chipperfield\\
                                D.S.Berry\\
                                A.C.Davenhall}
\newcommand{\stardocdate}      {2003 May 7}
...
\newcommand{\IDLURL}{http://www.rsinc.com/idl/index.cfm}
...
The {\em Parameters\/} section \label{app_parameters} lists the application's
...
\newpage
\latex{\subsection{The Applications}}
\sstroutine{
   ASCII2NDF
}{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      NDF.  See Section \texttt{"}Notes\texttt{"} (below) for details
      of the conversion.
   }
   \sstusage{
      ast2ndf in out
...
\sstroutine{
   ...
   \sstdescription{
      ...
      (currently IUE Final Archive, INES, ISO, and 2dF), and provides
      ...
      the general-case
      processing rules are described in the \texttt{"}Notes\texttt{"}.
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         \sstitem
            \texttt{"FITS-WCS"} --- This is the proposed FITS standard WCS
            encoding scheme described in the paper 
            \htmladdnormallink{\textit{Representation of celestial coordinates in FITS}}
            {http://www.cv.nrao.edu/fits/documents/wcs/wcs.html}\\ \latex{
            (\texttt{http://www.cv.nrao.edu/fits/documents/wcs/wcs.html})}.
            It is very similar to \texttt{"FITS-IRAF"} but supports a wider 
            range of projections and co-ordinate systems. Once the standard
            has been agreed, this encoding should be understood by any 
            \texttt{FITS-WCS} compliant software and it is likely to be
         ...
         \sstitem
            \texttt{"DSS"} --- 
            This is the system used by the Digital Sky Survey,
   ...
   \sstsubsection{
         ...
         place a \texttt{"-"} at the end of each line where a continuation line
         is desired.  \texttt{["TRUE"]}
      }
      \sstsubsection{
         IN = LITERAL (Read)
      }{
      ...
      \sstsubsection{
         ...
         parameter CONTAINER is TRUE or an EXTABLE is in use, the FITS
      ...
      \sstsubsection{
         ...
         library (see
         \xref{SUN/210}{sun210}{}).
         For instance, to force CAR projections to
         be interpreted as simple linear mappings from pixel co-ordinates to
         celestial co-ordinates (rather than the non-linear mapping implied 
         by the FITS-WCS conventions), use WCSATTRS=\texttt{"CarLin=1"}.
         A null  value (\texttt{!}) results in all attributes using default values.
         \texttt{[!]}
      }
   }
   \sstexamples{
...
\sstroutine{
   ...
   \sstdescription{
      This application converts an IRAF image to an NDF.  See the 
...
\sstroutine{
   ...
   \sstdescription{
      ...
      in IRCAM User Note 11 to one or more NDFs.  See the 
      \texttt{"}Notes\texttt{"} for a detailed list of the rules of the conversion.
   }
   \sstusage{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      tape and convert the selected tape files into FITS disk files and then using 
      \htmlref{FITS2NDF}{FITS2NDF} to
   ...
   \sstexamples{
      \sstexamplesubsection{
         \textrm{Note that []* etc must be protected from the shell.}
      }{
      }
      \sstexamplesubsection{
         ...
         \texttt{/dev/nst0} to
         an NDF called \texttt{f256}. The FITS blocking factor of the tape
         is 10.
      ...
      \sstexamplesubsection{
        ...
        \texttt{ral256\_*.fit},
         ...
         be converted to NDFs named \texttt{ral256\_*} and retained.
      }
      \sstexamplesubsection{
         mtfits2ndf
      }{
...
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         precision.  There are two other special values.  BITPIX=\texttt{0}
         will cause the output file to have the data type equivalent to
         that of the input NDF.  BITPIX=\texttt{-1} requests that the
      ...
      \sstsubsection{
         ...
         default encoding is chosen based on the contents of the NDFs FITS
      ...
      \ssthitemlist{
         ...
         \ssthitemlist{
            ...
            CUNIT\textit{n} --- are derived
              ...
              they are copied from the NDF FITS extension.
           ...
           \sstitem
              ...
              extension when PROFITS is \texttt{TRUE}, and from the NDF
   ...
   \sstdiytopic{
      ...
      \ssthitemlist{
         ...
         range of projections and co-ordinate systems.  Once the standard
         has been agreed, this encoding should be understood by any 
         FITS-WCS compliant software and it is likely to be
         adopted widely for FITS data in future. 
...
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         range of signed words ($-$32768 to 32767).  A null value (\texttt{!})
...
\sstroutine{
   ...
   \sstdescription{
      This application converts an NDF to an IRAF image.  See the
      \texttt{"}Notes\texttt{"} for details of the conversion.
   }
   \sstusage{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         The value used to replace bad pixels in the NDF's data array
         before it is copied to the IRAF file.  A null value (\texttt{!}) means
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      Figure~\ref{MAPSCHEMATIC} shows an example of a schematic.
   }
   \sstdiytopic{
...
Both READ\_NDF and WRITE\_NDF can take special action on bad
values. For a full description of their arguments see Appendix \ref{idl_specs}.
...
\texttt{IDL\_PATH} has been set (usually by `sourcing' the 
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \begin{description}
            ...
            \item[]\_WORD --$>$  integer
            \item[]\_INTEGER --$>$  longword integer
            \item[]\_LOGICAL --$>$  longword integer (with name change, see
below)
...
Added application MTFITS2NDF to convert FITS tapes to NDFs.
...
Corrected an error in the section of this document describing the easy way to
do IDL/NDF conversions. WRITE\_NDF was described as if it were a function rather
than a procedure.
