      SUBROUTINE COI_FKEYC( NAME, VALUE, CMTBGN, COMNT, COMCAR,
     :                      HEADER, STATUS )
*+
*  Name:
*     COI_FKEYC
...
*  Purpose:
*     Formats an IRAF header (FITS-like card) for a CHARACTER value
*     or a comment.
...
*
*     It is the same as CON_FKEYC, which writes FITS cards, except that
*     any trailing blanks are included in the value.
...
*  Local Variables:
      INTEGER CARDLO             ! Previous CARDLN
...
*  Allow for trailing spaces, so don't rely on the column returned by
*  CHR_APPND.
               CARDLN = CARDLO + NCHAR - STAPOS + 1
               NCHAR = CARDLN
            END IF
            GO TO 10
         END IF
