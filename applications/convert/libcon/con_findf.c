 ...
 *     as the function value.  Finally, a call to CON_FINDE with the
 *     last returned value of Context will close down any files opened or
 *     memory allocated by CON_FINDF.
 *
 *  Language:
 ...
 *  Call:
 *     (from Fortran)
 *     STATUS = CON_FINDF ( FILESPEC, FILENAME, CONTEXT )
 *
 *  Parameters:
 ...
 *         the AMS/MSP systems.
 *      1996 January 26 (MJC):
 *         Renamed to CON_FINDF for CONVERT.
 *+
 */
...
F77_INTEGER_FUNCTION(con_findf)( CHARACTER(FileSpec), CHARACTER(FileName),
 ...
 *
 *  Function:
 *     Terminate a sequence of CON_FINDF calls.
 *
 *  Description:
 *     This routine should be called after a sequence of calls to CON_FINDF
 *     in order to release any resources used by CON_FINDF.  It should be
 *     passed in its Context argument the value of the Context argument 
 *     as returned by the CON_FINDF in the last call in the sequence that
 ...
 *      1993 January 6 (MJC):
 *         Renamed to FTS1_FINDE for KAPPA FITS.  This is as a temporary
 *         measure until a Starlink standard equivalent of this routine
 *         is available.
 *      16-Mar-1993 (PDRAPER):
 *         Changed to use Starlink CNF interface.
 *      1996 January 26 (MJC):
 *         Renamed to CON_FINDE for CONVERT.
 *+
 */

F77_INTEGER_FUNCTION(con_finde)( POINTER(Context) )
{
