      SUBROUTINE CON_RGNDF( PARAM, MAXSIZ, MINSIZ, TEXT, IGRP, SIZE,  
     :                      STATUS )
*+
*  Name:
*     CON_RGNDF
...
*  Invocation:
*     CALL CON_RGNDF( PARAM, MAXSIZ, MINSIZ, TEXT, IGRP, SIZE, STATUS )
...
*  History:
*     21-AUG-2000 (DSB):
*        Original version copied from KAPPA routine KPG1_RGNDF
...
*  Local Variables:
      LOGICAL FLAG               ! Does the supplied group expression end
                                 ! with a minus sign?
...
*  Check that the argument MAXSIZ and MINSIZ are OK.
      IF( MAXSIZ .LT. MINSIZ .AND. MAXSIZ .NE. 0 ) THEN
         ...
         CALL ERR_REP( 'CON_RGNDF_ERR1', 'CON_RGNDF: Maximum '//
...
*  flag.
         IF( SIZE .GT. MAXSIZ .AND. MAXSIZ .GT. 0 ) THEN
            ...
            IF( MAXSIZ .GT. 1 ) THEN
               CALL MSG_SETI( 'MAX', MAXSIZ )
               CALL MSG_OUT( 'CON_RGNDF_MSG1',
     :       'WARNING: Only the first ^MAX NDF names can be used',
     :                          STATUS )

            ELSE
               CALL MSG_OUT( 'CON_RGNDF_MSG2',
...
*  cancelling the parameter value and telling the user what to do.
         ELSE IF( FLAG ) THEN
            IF( SIZE .LT. MAXSIZ .OR. MAXSIZ .LE. 0 ) THEN
               ...
               IF( TEXT .NE. ' ' ) CALL MSG_OUT( 'CON_RGNDF_MSG3',
     :                                           TEXT, STATUS )
