...
*  History:
*     14/8/97 (ACD): Original version.
*     8/1/98  (ACD): Changed KHz to kHz and Km to km to conform to
*       standard usage.
*  Bugs:
*     None known
*-
...
*  Local Variables:
     ...
     :  TEXTS       ! Number of lines of history text.
      CHARACTER
     :  TEXT(4)*(NDF__SZHIS),   ! Lines of history text.
     :  VCORRC*12   ! Character representation of radial velocity correction.
*.

      IF (STATUS .EQ. SAI__OK) THEN

*
*       Check whether the spectrum is expressed as frequencies or
*       radial velocities.

         IF (SPECTM(1 : 1) .EQ. 'F') THEN

*
*          The axis is a set of frequencies.  Add an appropriate line
*          to the history.
...
*
*          The axis is a set of radial velocities.  Assemble appropriate
*          lines for the history.  First the type of axis...
...
*          ... the standard of rest used.
     ...
     :           /'to any standard of rest.'
            ELSE IF (SOR .EQ. 'SOLAR') THEN
               TEXT(3) = 'They are relative to a solar (heliocentric) '/
     :           /'standard of rest.'
