...
*     CON_VCORR
*  Purpose:
*     Compute correction to adjust obs. rad. vel. to some standard of rest.
*  Language:
*     Fortran 77.
*  Invocation:
...
*  History:
*     28/7/97 (ACD): Original version.
*     12/8/97 (ACD): First stable version.
*  Bugs:
*     None known
*-
*  Type Definitions:
...
*  Local Variables:
     ...
     :  MONTH,   ! Month.
     :  DAY,     ! Day.
     :  DSTAT,   ! Local SLA status.
     :  LOOP     ! Loop index.
      CHARACTER
     :  CMONTH*3 ! Month as a three-character abbreviation.
      REAL
     :  RLAT,    ! Latitude        as a REAL.
     :  RAR,     ! Right Ascension "  "  "  .
     :  DECR,    ! Declination     "  "  "  .
     :  RLAST,   ! LAST            "  "  "  .
     :  VROT,    ! Correction for Earth's diurnal rotation (km/sec).
     :  VREV,    ! Correction for Earth's revolution about the Sun (km/sec).
     :  VSR      ! Additional correction to standard of rest (km/sec).
      DOUBLE PRECISION
     :  MJDATE,  ! Date expressed as an UT MJD.
     :  MJD,     ! Date and time expressed as an UT MJD.
     :  GMST,    ! Greenwich mean sidereal time (radians).
     :  LAST,    ! Local apparent sidereal time.
     :  HOUR,    ! Hour    of the UTC.
     :  MIN,     ! Minutes "   "   " .
     :  SEC      ! seconds "   "   " .
      DOUBLE PRECISION
     :  SCUV(3),   ! Cartesian unit vector of Right Ascension and Declination.
     :  DVH(3),    ! Heliocentric velocity of the Earth (A.U./sec).
     :  DUMMY1(3), ! Dummy argument.
     :  DUMMY2(3), !   "      "    .
     :  DUMMY3(3)  !   "      "    .
*  Local Data:
      CHARACTER
     :  MONTHS(12)*3   ! Months of the year.
...
*
*       Convert the UTC date to a modified Julian date (MJD).  First
*       extract the year, month and day of the UTC from the input
*       character string and convert them to integers.  Then attempt
*       to calculate the MJD.  Report an error of the conversion
*       fails.
...
*
*          Compute the time as an MJD by adding the UT time to the MJ
*          date.  First convert the time from a character string to
*          DOUBLE PRECISION numbers.
            ...
            MJD = MJDATE + (HOUR / 2.4D1) + (MIN / (2.4D1 * 6.0D1)) +
