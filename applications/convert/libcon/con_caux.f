...
*  Local Variables:
      CHARACTER
     :  MAPLOC*(DAT__SZLOC),  ! Locator to SPECX map structure.
     :  MMRLOC*(DAT__SZLOC),  !    "    "  input map MORE structure.
     :  CUBLOC*(DAT__SZLOC),  !    "    "  output NDF cube.
     :  CMRLOC*(DAT__SZLOC),  !    "    "  output MORE structure.
...
*             set the flag indicating that HISTORY is to be appended to
*             the output cube, copy the HISTORY component and set the
*             HISTORY update mode to NORMAL.
*
*             If the input HISTORY component is absent or not ok then
*             set the flag indicating that the output cube will not
*             contain any HISTORY.  If (and only if) a bad status has
*             been raised because the HISTORY component was absent
*             then annul the status and report a message.
