      SUBROUTINE TRA_PUTL( LOC, NDIM, DIMS, SIZE, NLINES, INDNTC,
...
*  Purpose:
*     Report the first and last few values of a logical object.

*  Language:
*     Starlink Fortran 77

*  Invocation:
*     CALL TRA_PUTL( LOC, NDIM, DIMS, SIZE, NLINES, INDNTC, LOGEXM,
*    :               FD, LINE, LENG, STATUS )
...
*          Put the value into the buffer unless it overflows the line.
            ...
            CALL TRA1_PUTS( NDIM, DIMS, IVALUE, STR( 1:NCHAR ),
...
*             Convert the numeric value to a string.
               ...
               CALL CHR_LTOC( VALUES( FVALUE ), STR, NCHAR )
...
*                      ... provided any text be output
                        ...
                        IF ( INDENT+SZELIP+4 .LT. CPOS ) THEN
                           ...
                           CALL CHR_LTOC( VALUES( FVALUE ), LINE( 1 +
     :                                    SZELIP + INDENT:CPOS ), I )
...
*                      The column position is at the indentation
*                      point.
...
*                   output last value at full precision, but without the
*                   ellipsis.

                     IF ( FVALUE .EQ. NVALUE ) THEN
                        CALL CHR_LTOC( VALUES( FVALUE ),
     :                                   LINE( INDENT:CPOS ), I )
...
*                Move the column position by the width of the added
*                string.
