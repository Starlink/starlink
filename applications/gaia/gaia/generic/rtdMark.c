...
static void		ComputeMarkBbox _ANSI_ARGS_((Tk_Canvas canvas,
                                                      MarkItem *markPtr));

static int		ConfigureMark _ANSI_ARGS_((Tcl_Interp *interp,
                            Tk_Canvas canvas, Tk_Item *itemPtr, int argc,
                            char **argv, int flags));

static int		CreateMark _ANSI_ARGS_((Tcl_Interp *interp,
			    Tk_Canvas canvas, struct Tk_Item *itemPtr,
			    int argc, char **argv));

static void		DeleteMark _ANSI_ARGS_((Tk_Canvas canvas,
			    Tk_Item *itemPtr, Display *display));

static void		ScaleMark _ANSI_ARGS_((Tk_Canvas canvas,
                            Tk_Item *itemPtr, double originX, double originY,
			    double scaleX, double scaleY));

static int		MarkCoords _ANSI_ARGS_((Tcl_Interp *interp,
			    Tk_Canvas canvas, Tk_Item *itemPtr,
			    int argc, char **argv));

static void		DisplayMark _ANSI_ARGS_((Tk_Canvas canvas,
			    Tk_Item *itemPtr, Display *display, Drawable dst,
			    int x, int y, int width, int height));

static int		MarkToArea _ANSI_ARGS_((Tk_Canvas canvas,
			    Tk_Item *itemPtr, double *areaPtr));

static double		MarkToPoint _ANSI_ARGS_((Tk_Canvas canvas,
			    Tk_Item *itemPtr, double *pointPtr));

static void		TranslateMark _ANSI_ARGS_((Tk_Canvas canvas,
			    Tk_Item *itemPtr, double deltaX, double deltaY));

static int              MarkToPostscript _ANSI_ARGS_((Tcl_Interp *interp,
                                                      Tk_Canvas
                                                      canvas,
                                                      Tk_Item *itemPtr,
                                                      int prepass));

static void             MarkCorrectAnchor _ANSI_ARGS_(( MarkItem *markPtr,
                                                        double *x,
                                                        double *y));
...
static int
CreateMark(interp, canvas, itemPtr, argc, argv)
  ...
  char **argv;                        /* Arguments describing item. */
{
   ...
   */
  if ((Tk_CanvasGetCoord(interp, canvas, argv[0], &markPtr->x) != TCL_OK) ||
      (Tk_CanvasGetCoord(interp, canvas, argv[1], &markPtr->y) != TCL_OK) ){
    return TCL_ERROR;
  }

  /*  And configure using any options */
  if (ConfigureMark(interp, canvas, itemPtr, argc-2, argv+2, 0) != TCL_OK) {
...
static int
MarkCoords(interp, canvas, itemPtr, argc, argv)
...
static int
ConfigureMark(interp, canvas, itemPtr, argc, argv, flags)
  Tcl_Interp *interp;		/* Used for error reporting. */
...
static void
DeleteMark(canvas, itemPtr, display)
...
ComputeMarkBbox(canvas, markPtr)
    ...
    MarkItem *markPtr;		        /* Item whose bbox is to be
					 * recomputed. */
{
  int sby2;
  double x, y;
...
static void
DisplayMark(canvas, itemPtr, display, drawable, xcan, ycan, width, height)
    Tk_Canvas canvas;			/* Canvas that contains item. */
    Tk_Item *itemPtr;			/* Item to be displayed. */
    Display *display;			/* Display on which to draw item. */
    Drawable drawable;			/* Pixmap or window in which to draw
					 * item. */
    int xcan, ycan, width, height;	/* Describes region of canvas that
...
static double
MarkToPoint(canvas, itemPtr, pointPtr)
  Tk_Canvas canvas;		/* Canvas containing item. */
...
static int
MarkToArea(canvas, itemPtr, areaPtr)
  Tk_Canvas canvas;		/* Canvas containing item. */
...
static void
ScaleMark(canvas, itemPtr, originX, originY, scaleX, scaleY)
...
static void
TranslateMark(canvas, itemPtr, deltaX, deltaY)
  Tk_Canvas canvas;			/* Canvas containing item. */
...
static int
MarkToPostscript(interp, canvas, itemPtr, prepass)
