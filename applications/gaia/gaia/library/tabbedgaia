...
#  Purpose:
#     Starts the GAIA image display & analysis tool using the tabbed
#     interface.
...
if test -r $GAIA_DIR/gaiaTabbedMain.tcl ; then
...
#  Add the local catalogue configuration file.
...
#  And run up the interface.
    #MALLOC_CHECK_=1;export MALLOC_CHECK_
    time $GAIA_DIR/gaia_wish $GAIA_DIR/gaiaTabbedMain.tcl ${1+"$@"}
  ...
  echo "!! Cannot start gaia; failed to locate $GAIA_DIR/gaiaTabbedMain.tcl."
  exit 1
fi
