...
#  Ensure the values in the values_ array are up to date.
      set values_(lclr) [$itk_component(lclr) get]
      set values_(vclr) [$itk_component(vclr) get]
      set values_(bgclr) [$itk_component(bgclr) get]
      set values_(bdclr) [$itk_component(bdclr) get]
...
#  -----------------
   ...
   public method setLColour {col} {set values_(lclr) $col; newVals}
   public method getLColour {} {return $values_(lclr)}

   public method setLFormat {fmt} {set values_(lfmt) $fmt; newVals}
   public method getLFormat {} {
      if { [catch { format $values_(lfmt) 0.0 }] } {
         error_dialog "Illegal format string \"$values_(lfmt)\" supplied for the key label. Reverting to default format \"%.1f\"."
         setLFormat "%.1f"
      }
      return $values_(lfmt)
   }

   public method setVColour {col} {set values_(vclr) $col; newVals}
   public method getVColour {} {return $values_(vclr)}

   public method setVValue {val} {set values_(vval) $val; newVals}
   public method getVValue {} {return $values_(vval)}

   public method setVWidth {wid} {set values_(vwid) $wid; newVals}
   public method getVWidth {} {return $values_(vwid)}

   public method setBgPad {pad} {set values_(pad) $pad; newVals}
   public method getBgPad {} {return $values_(pad)}

   public method setBgColour {clr} { set values_(bgclr) $clr; newVals}
   public method getBgColour {} {
      set ret $values_(bgclr)
      if { $ret == "(clear)" } { set ret "" }
      return $ret
   }

   public method setBdColour {clr} { set values_(bdclr) $clr; newVals}
   public method getBdColour {} {
      set ret $values_(bdclr)
      if { $ret == "(clear)" } { set ret "" }
      return $ret
   }

   public method setBdWidth {wid} {set values_(bdwid) $wid; newVals}
   public method getBdWidth {} {return $values_(bdwid)}

   public method setLFontFam {f} {set values_(ffam) $f; updateFont}
   public method setLFontSize {f} {set values_(fsize) $f; updateFont}
   public method setLFontBold {f} {set values_(fbold) $f; updateFont}
...
#  Set the hard-wired defaults.
      set values_(enable) 1
      set values_(lclr) "#fff"
      set values_(lfmt) "%.1f"
      set values_(ffam) "courier"
      set values_(fsize) 15
      set values_(fbold) 0
      set values_(vclr) "current"
      set values_(vwid) 2
      set values_(bgclr) "#000"
      set values_(pad) 0.5
      set values_(bdclr) "#fff"
      set values_(bdwid) 1
...
#  defaults.
      ...
      if { $values_(lclr) == "" } { set values_(lclr) "#fff" }
      if { $values_(ffam) == "" } { set values_(ffam) "courier" }
      if { $values_(fsize) == "" } { set values_(fsize) 15 }
      if { $values_(fbold) == "" } { set values_(fbold) 0 }
      if { $values_(vclr) == "" } { set values_(vclr) "current" }
      if { $values_(vwid) == "" } { set values_(vwid) 2 }
      if { $values_(bgclr) == "" } { set values_(bgclr) "#000" }
      if { $values_(pad) == "" } { set values_(pad) 0.5 }
      if { $values_(bdclr) == "" } { set values_(bdclr) "#fff" }
      if { $values_(bdwid) == "" } { set values_(bdwid) 1 }
...
#  Now configure the font.
      font configure $lfont_ -family $values_(ffam) \
                             -size $values_(fsize) \
		             -weight $wgt
...
#  the textual label.
	    ...
	    util::LabelEntry $w_.lfmt -text "Format string:" \
	                        -labelwidth $lwidth \
                                -textvariable [scope values_(lfmt)] \
                                -valuewidth 20 \
...
#  Create a LabelEntry to set the vector value.
	    ...
	    util::LabelEntry $w_.vval -text "Value:" \
  			        ...
  			        -labelwidth $lwidth \
                                -anchor nw \
			        -validate real \
                                -textvariable [scope values_(vval)]
	 }
...
#  Create a LabelEntry to control the width of the vector.
         itk_component add vwid {
            util::LabelEntry $w_.vwid -text "Thickness:" \
			        ...
			        -validate integer \
                                -anchor nw \
                                -textvariable [scope values_(vwid)]
         }
...
#  Create a LabelEntry to control the width of the margin.
         itk_component add pad {
            util::LabelEntry $w_.pad -text "Margin:" \
			       ...
			       -validate real \
                               -anchor nw \
                               -textvariable [scope values_(pad)]
         }
...
#  Create a LabelEntry to control the thickness of the border
         itk_component add bdwid {
            util::LabelEntry $w_.bdwid -text "Width:" \
			         ...
			         -validate integer \
                                 -anchor nw \
                                 -textvariable [scope values_(bdwid)]
         }
