...
# Peter W. Draper 19 Jan 98  Changed focus bindings. These where
#                            causing problems with people who do use
#                            focus follows mouse and with autoraise
#                            under some window managers (makes it very
#                            hard to control toplevels when entering
#                            main image raises the main window). 
...
class rtd::RtdImage {
	...
	# make frames to hold image and scrollbars
    ...
    method set_scrollregion {x0 y0 x1 y1} {
    ...
    method make_rapid_frame {popup region_id x0 y0 x1 y1} {
    ...
    method camera_pre_command {frameid} {
    }
    ...
    method camera_post_command {frameid} {
	if {$frameid == 0} {
    ...
    method set_rtd_wcs_info {frameid} {
    }
    ...
    method pixel_table {nrows ncols} {
	if {[$image_ isclear]} {
	    warning_dialog "No image is currently loaded" $w_
	    return
	}
    ...
    method picked_wcs_object {x y ra dec {equinox J2000} {fwhmX ""} {fwhmY ""} \
    ...
    # support for printing canvas image items (in Tk4.0)
	...
	# this part is tricky, due to the way the canvas is tricked
	# about the size of the image (the part you don't see is never
	# updated, but the canvas doesn't know that)
	set x0 [max [expr int([$canvas_ canvasx 0])] 0]
	set y0 [max [expr int([$canvas_ canvasy 0])] 0]
	set x1 [expr $x0+[min [winfo width $canvas_] [$image_ dispwidth]]]
	set y1 [expr $y0+[min [winfo height $canvas_] [$image_ dispwidth]]]
	...
	utilReUseWidget util::CanvasPrint $w_.print \
	    ...
	    -top_left {ESO} \
	    -top_right $name \
	    -bot_left "[id user]/[lindex [winfo name .] 0]" \
	    -bot_right [fmtclock [getclock]  {%b %d, %Y at %H:%M:%S}] \
	...
	if {"[$image_ fits get PLTRAH]" != ""} {
	    warning_dialog "Can't convert plate coefficients. \
                            Please get this image from the DSS image server." $w_
	    return
	}
    ...
    method save_region {canvas_id x0 y0 x1 y1} {
	save_as . * $x0 $y0 $x1 $y1
