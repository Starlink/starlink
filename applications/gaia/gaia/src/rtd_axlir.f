      SUBROUTINE RTD_AXLIR( EL, ARRAY, LVAL, UVAL, REFPIX, REFVAL, 
     :                      LINEAR, STATUS )
*+
*  Name:
*     RTD_AXLIR
...
*  Invocation:
*     CALL RTD_AXLIR( EL, ARRAY, LVAL, UVAL, REFPIX, REFVAL, LINEAR, 
*                     STATUS )
...
*     1996 March 7 (PWD):
*        Renamed RTD_AXLIR and no longer generic.
...
*  Arguments Returned:
      ...
      REAL REFVAL               ! Reference pixel value
...
*    the ends of the array.
      LOW = 1
 10   CONTINUE
      IF ( ARRAY( LOW ) .EQ. VAL__BADR .AND. LOW .LT. EL ) THEN
         LOW = LOW + 1
         GO TO 10
      END IF
      HIGH = EL
 11   CONTINUE
      IF ( ARRAY( HIGH ) .EQ. VAL__BADR .AND. HIGH .GT. LOW ) THEN
         HIGH = HIGH - 1
         GO TO 11
      END IF
...
*    Evaluate the linear increment.
      INCREM = DBLE( ARRAY( HIGH ) - ARRAY( LOW ) ) / DBLE( HIGH - LOW )
...
*    linear array.
      IF ( ARRAY( 1 ) .EQ. VAL__BADR ) THEN
         LVAL = REAL( DBLE( ARRAY( LOW ) ) - INCREM * DBLE( LOW - 1 ) )
      END IF

      IF ( ARRAY( EL ) .EQ. VAL__BADR ) THEN
         UVAL = REAL( DBLE( ARRAY( HIGH ) )+ INCREM* DBLE( EL - HIGH ) )
      END IF
