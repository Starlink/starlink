...
*/
static int Tcladam_Exit ( ClientData clientData, Tcl_Interp *interp, int argc,
    char *argv[]);
...
/*+   TKADAM_EXIT - exit this process invoking the Adam exit handler */

static int Tcladam_Exit
(
...

{
   ...
   if (argc == 2)
   {
      return Tcl_VarEval(interp, "exit ", argv[1], (char *)NULL);
   }
   else
   {
...
/*   get the communications path */
      ...
      if ( status == SAI__OK )
      {
         ...
         if ( status == SAI__OK )
         {
            ...
            if ( status == SAI__OK )
            {
               sprintf ( interp->result, "%d %d", path, messid );
               tcl_status = TCL_OK;
            }
            else
            {
