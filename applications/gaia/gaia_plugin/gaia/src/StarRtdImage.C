...
//       Renamed StarRtd_Init to Gaia_Init (Tcl package convention),
//
...
public:
  ...
  { "plotgrid",      &StarRtdImage::plotgridCmd,     0, 1 },
...
static Tk_ImageType starRtdImageType = {
  "rtdimage",  // changed from "starrtdimage" (ALLAN)
...
// other (presumable FITS-WCS) which is passed as an optional
...
//
//    Notes:
//       All of the grid attributes are passed in the argv array
...
int StarRtdImage::plotgridCmd( int argc, char *argv[] )
{
  ...
  if ( argc != 0 ) {
    if ( argc != 1 ) {
      error( "wrong # args: plotgrid options_list" );
      inerror = 1;
    } else {
  ...
  if ( wcs != (AstFrameSet *) NULL ) {
    ...
    // system). Note that we need to deal with the special case of
...
//
//   Purpose:
//      Debugging procedure. Writes the current local WCS system out
//      to terminal using the given encoding, plus a FITS channel
//      if requested.
//-
...
int StarRtdImage::astwriteCmd( int argc, char *argv[] )
{
  ...
  if ( newset_ ) {
    ...
    AstFitsChan *chan = (AstFitsChan *)
      astFitsChan( NULL, &write_out, "Encoding=%s", argv[0] );
    ...
    if ( argc == 2 ) {
      int slot = 0;
      if ( Tcl_GetInt( interp_, argv[1], &slot ) != TCL_OK ) {
    ...
    cout << "Sorry no local FrameSet is available" << endl;
  }
  return TCL_OK;
}
...
//     the image WCS system, return the equivalent X and Y position.
//
//  Notes:
