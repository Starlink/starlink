...
class gaia::StarAstTable {
   ...
   constructor {args} {
       ...
       itk_component add tabframe {
	   frame $w_.tabframe
       }
       itk_component add new {
	   button $itk_component(tabframe).new -text New \
		   -command [code $this enter_new_object]
       }
	   ...
	   button $itk_component(tabframe).modify -text Edit \
		   -command [code $this edit_selected_object]
       }
	   ...
	   button $itk_component(tabframe).delete -text Delete \
		   -command [code $this remove_selected]
       }
	   ...
	   button $itk_component(tabframe).grab -text Grab \
	   ...
	   button $itk_component(tabframe).centroid -text Centroid \
	   ...
	   button $itk_component(tabframe).clear -text Clear \
	   ...
	   button $itk_component(tabframe).clip -text Clip \
       ...
       pack $itk_component(tabframe) -side top -fill x -pady 3 -padx 3
       pack $itk_component(clip) -side right -expand 1 -pady 2 -padx 2
       pack $itk_component(clear) -side right -expand 1 -pady 2 -padx 2
       pack $itk_component(centroid) -side right -expand 1 -pady 2 -padx 2
       pack $itk_component(grab) -side right -expand 1 -pady 2 -padx 2
       pack $itk_component(delete) -side right -expand 1 -pady 2 -padx 2
       pack $itk_component(modify) -side right -expand 1 -pady 2 -padx 2
       pack $itk_component(new) -side right -expand 1 -pady 2 -padx 2
   ...
   protected method restore_bindings_ {} {
	   ...
	   #  Add a binding to all objects so that when they are drawn they
	   #  can be moved. Double clicking an object edits the
	   #  values associated with it. Entering an object
	   #  highlights its entry in the table. Pressing <Control-B1> over
   ...
   protected method make_markers_menu_ {m} {
      ...
      foreach {label name} {Type type Size size Width width \
       ...
       #  Add the known types.
       foreach i $marker_types_ {
	   $m.type add radiobutton \
		   -value $i \
		   -bitmap $i \
		   -command [code $this configure -mtype $i] \
   ...
   public method write_to_file {} {
      ...
      if {[$w activate]} {
         save_positions_ [$w get]
      }
      destroy $w
   }

   #  Write table contents to a named file.
   protected method save_positions_ {filename} {
   ...
   method read_positions_ {filename} {
