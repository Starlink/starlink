...
ProTclAppInit(Tcl_Interp* interp)
{
...
#if 0
    // import namespaces
    if (Tcl_Import(interp, Tcl_GetGlobalNamespace(interp), "::util::*", 1) != TCL_OK
	|| Tcl_Import(interp, Tcl_GetGlobalNamespace(interp), "::rtd::*", 1) != TCL_OK
	|| Tcl_Import(interp, Tcl_GetGlobalNamespace(interp), "::cat::*", 1) != TCL_OK
	|| Tcl_Import(interp, Tcl_GetGlobalNamespace(interp), "::skycat::*", 1) != TCL_OK) {
        return TCL_ERROR;
    }
#endif
    ...
    // return Tcl_Eval(interp, "skycat::SkyCat::startSkyCat");
    return TCL_OK;
}
