...
#  Purpose:
#     Class for querying a Cone Search server.
...
#  Invocations:
#
#        GaiaVOConeSearch object_name [configuration options]
#
#     This creates an instance of a GaiaVOConeSearch object. The return is
...
itk::usual GaiaVOConeSearch {}

itcl::class gaiavo::GaiaVOConeSearch {
   ...
   constructor {args} {
      ...
      set tempcats_ [gaia::GaiaTempName \#auto -prefix GaiaTempCone \
                        -exists 0 -type ".TAB"]

      #  Display the shortname and/or access URL.
      set lwidth 10
      set vwidth 50
      ...
      add_short_help $itk_component(server) "The Cone Search server"
   ...
   public method query {} {
      ...
      if { $querytask_ == {} } {
         set querytask_ [gaia::GaiaForeignExec \#auto \
                            -application $::gaia_dir/querycone \
   ...
   protected method query_done_ {} {
      ...
      if { ! [::file exists $votable_] } {
         warning_dialog "Failed to query Cone Search server"
         return
      }

      #  Convert to TST and do the command to display result.
      read_query $votable_
   }
