...
\htmladdnormallinkfoot{ESO}{http://www.eso.org/}. SkyCat and GAIA are 
...
\hline
 ...
 -show\_hdu\_chooser   & Automatically show the HDU chooser (default 1)
\end{tabular}
}
...
\subsection{This release - 2.6-11}

New to this release:
\begin{itemize}

   \item A number of new catalogues and image servers are now
   available. To access these delete your skycat.cfg file (found in
   \mytt{\~{}/.skycat/}) and restart GAIA. Some highlights are 2MASS
   and USNO B1.0 from CDS and GSC II from ESO.

   \item The pixel table now highlights the maximum and minimum values
   and the orientation of the Y-axis has been flipped so that the
   ...
   \item A new configuration option \mytt{-show\_hdu\_chooser} has been
   ...
   should remain inactive. This option can be switched on using the new
   \mytt{File->Startup options} window.
   ...
   selected are used whenever GAIA starts up (this removes the tedious
   need to either specify these on the command-line, or add them to
   your X resources).
   ...
   and right columns, and incorrectly formatted RA as DD:MM:SS and Dec
   as HH:MM:SS. These problems are now fixed.

   \item The direction arrows shown in the pan window could be
   displayed at the wrong size, again when North and East were
   interchanged. This problem is now fixed.

   \item Fixed shape ellipses in the aperture photometry toolbox were
