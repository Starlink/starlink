...
\newcommand{\stardocnumber}    {214.4}
\newcommand{\stardocauthors}   {Peter W. Draper}
\newcommand{\stardocdate}      {30 November 1998}
\newcommand{\stardoctitle}     {GAIA -- Graphical Astronomy and
                                Image Analysis Tool}
\newcommand{\stardocversion}   {2.2}
...
more astronomically useful ones such as aperture photometry, source
detection, arbitrary region analysis, celestial coordinate readout,
calibration and modification, grid overlays, blink comparison, defect
patching and the ability to query on-line (WWW) catalogues.}
...
interesting areas of aperture photometry, automatic source detection,
arbitrary region analysis, celestial coordinate readout, calibration
and modification, grid overlays, blink comparison, image defect
patching and the ability to connect to resources available in on-line
(WWW) catalogues and archives.
...
\begin{quote}
\mytt{\% gaiadisp image\_name 1}
\end{quote}
This displays into the window with title ``GAIA/SkyCat (1)''. If
...
command like:
\begin{quote}
\mytt{cl> gaiadisp dev\$pix.imh 0}
\end{quote}
...
\begin{itemize}
\item Aperture photometry. A highly interactive environment for
...
\item Astrometric calibration. You can either fit a new
  astrometric calibration to your image (using on-line catalogues for
  reference positions), define a new one using known information
  (such the image scale and a reference point), copy one from another
  image and finally tweak one to make it fit better.
...
the SExtractor/EXTRACTOR package.
...
``Set Plot Symbols...'' window, available under the local catalogue
menu ``Options''. Using this window you can plot multiple symbols per
...
respectively. Note that the maximum possible zoom in the main window
is limited to 32767 divided by the maximum of the image width or
height.
...
configure GAIA so that its remote catalogue access will work. To do
this you need to define an environment variable \mytt{http\_proxy} to
let GAIA know that it is to use the proxy server, rather than attempt
to access sites directly. The format of this variable is:
\begin{quote}
\begin{verbatim}
http://webcachemachine.domain:port/
\end{verbatim}
\end{quote}
So for instance if your proxy server is called
\mytt{wwwcache.dur.ac.uk} and it listens on port \mytt{8080} and you
use the C-shell (or some variant). You would need to do:
\begin{quote}
\begin{verbatim}
setenv http_proxy "http://webcache.dur.ac.uk:8080/"
\end{verbatim}
\end{quote}
before starting up GAIA (your \mytt{.login} would be an ideal place to
do this).

It is also possible to stop the proxy server being used for specific
domains (say for instance if one of the catalogue servers used by GAIA
was available to you locally). This is controlled by the variable
\mytt{http\_noproxy}. The format of this variable is:
\begin{quote}
\begin{verbatim}
domain1,domain2,domain3...
\end{verbatim}
\end{quote}
So for instance if you had local access to the main ESO server --
\mytt{archive.eso.org} -- you might like to set this variable to:
\begin{quote}
\begin{verbatim}
setenv http_noproxy "eso.org"
\end{verbatim}
\end{quote}
