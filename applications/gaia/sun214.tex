...
\newcommand{\stardocdate}      {22 November 1999}
...
\hline
 ...
 -focus\_follows\_mouse & Focus for entry areas follows mouse \\
\end{tabular}
}
...
The CDE window manager has some problems with the
\mytt{focus\_follows\_mouse} property (seen when the focus policy is
`click-to-focus' with `autoraise'). In this case try the command:
\begin{quote}
\begin{verbatim}
% gaia image_name -focus_follows_mouse 0
\end{verbatim}
\end{quote}
Add this to your X preferences if you always want this behaviour.
...
\begin{itemize}
  ...
  \item The on-line help for the photometry toolboxes should be
        available again.  

  \item A new configuration option \mytt{-focus\_follows\_mouse} has
        been added to stop problems with \mytt{autoraise} and
        \mytt{click-to-focus} when using the CDE window manager. Set
        this option to $0$ (false). 
\end{itemize}
