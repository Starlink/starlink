...
\item Display of image planes from data cubes. Using this you can step
  along an axis and display the image formed by the other two axes. 
  ...
  analysed just like any other image, but the data format should be 
  NDF for efficiency. Finally a spectrum from the hidden dimension of
  the cube can be displayed in the Starlink SPLAT program.
\end{itemize}
...
\section{This release 2.9}

There have been several minor changes in this release of GAIA.

The minor changes are:
\begin{itemize}

\item GAIA can now display in-line compressed images (such as RICE from WFCAM).
      These are converted into temporary files on the fly and can be used with
      all other tasks within GAIA.

\item When you display slices from cubes, it is now possible to get a spectral
      display by double-clicking on a display pixel. This sends the spectrum 
      to the Starlink SPLAT program.

\item The ESP toolbox now reports the mean count along the isophote and plots
      intensity profiles.

\item A rarely seen problem with foreign-file conversion deleting the input
      files has been fixed.

\item The object detection toolbox has been updated to work with SExtractor
      version 2.4.2. This include Petrosian magnitudes and some windowed
      parameters.

\end{itemize}

Internally GAIA has undergone major changes in the way that it is built and
distributed. One immediate consequence of this is that GAIA no longer
distributes any files for IRAF support. 

AUTOASTROM does not exist, so that is also missing...
