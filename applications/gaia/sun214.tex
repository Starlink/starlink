...
\newcommand{\stardocdate}      {28th April 2006}
...
\end{quote}
which may be updated from time-to-time. GAIA was mainly produced by the now
closed Starlink project, funded by the Particle Physics and Astronomy Research
Council (see links on the front page).
...
well as NDF cubes. It also features an all new, built-in, interactive
spectral readout display.
...
There are also many minor changes in this release of GAIA:
    ...
    - The 2MASS catalogue query now includes a minimum radius. Remove any
      existing $HOME/.skycat/skycat.cfg files to benefit from this change.

    - The NDF cube slice display toolbox now has options for continuously
...
uses the unified build procedures). One consequence of this is that GAIA no
longer distributes any files for IRAF support.

GAIA is no longer supported by the Starlink project, which has been
terminated. Most of this new work has been carried out for the JAC Hawaii.  If
you want work on GAIA to continue then support it by contacting any current
members of the PPARC Astronomy or Science committees and making your views
known.
