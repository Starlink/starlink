...
\newcommand{\stardocnumber}    {5.17}
\newcommand{\stardocauthors}   {P.\,T.\,Wallace}
\newcommand{\stardocdate}      {8th October 1998}
...
ASTROM is run by means of the following command (C-shell on Unix, DCL 
on VAX/VMS, MS-DOS on PC):

\begin{quote}
\texttt{astrom} [\textit{input}] [\textit{report}]
\end{quote}

The input file \textit{input} defaults to \texttt{astrom.dat}
and the report file \textit{report} defaults to \texttt{astrom.lis}.
A parameter can be defaulted either by leaving it out or by using a pair
of double-quotes.

The synopsis report, which normally appears on the command terminal,
should be monitored and any reference stars with large residuals noted.
The input file can then be edited as necessary and the job rerun.
Finally, the report file can be printed in the normal way.  An example
report file is reproduced in Appendix~\ref{example_report}.
