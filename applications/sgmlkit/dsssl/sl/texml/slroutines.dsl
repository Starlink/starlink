...
(define (process-codecollection docent)
  (let ((docelem (document-element-from-entity docent)))
      (if docelem
          ...
          (error "No codecollection docelement found"))))
...
(mode routine-ref-sst
		...
		(make-latex-environment name: "description"
			...
			(if (node-list-empty? allauthors)
			    ...
			    (make sequence
			      ...
			      (make-latex-environment name: "raggedright"
				    (make-latex-environment name: "description"
  ...
  (element routineprologue
	...
	(make-latex-environment name: "sstroutine"
  ...
  ;;      (process-children)))
  (element description
    (make-latex-environment name: "sstdescription"
	  (process-children)))
  (element moduletype
    (make-latex-environment name: "sstdiytopic"
  ...
  (element returnvalue
    (let ((none-att (attribute-string (normalize "none")))
      ...
      (make-latex-environment name: "sstreturnedvalue"
	    (make sequence
	      (make-latex-empty-command name: "item"
  ...
  (element parameterlist
    (let ((none-att (attribute-string (normalize "none"))))
      (make-latex-environment name: "sstparameters"
	    (if none-att
		(make-latex-command name: "sstsubsection"
		      (literal "No parameters"))
		(process-children)))))
  (element argumentlist
    (let ((none-att (attribute-string (normalize "none"))))
      (make-latex-environment name: "sstarguments"
	    (if none-att
  ...
  (element otherauthors
    (make sequence
      ...
      (make-latex-environment name: "itemize"
	    (process-children))))
  (element examplelist
    (make-latex-environment name: "sstexamples"
	  (process-children)))
  (element example
    (make-latex-environment name: "sstexamplesubsection"
		(make-latex-environment name: "small"
  ...
  (element examplenote
    (process-children))
  (element usage
    (make-latex-environment name: "sstusage"
	  (process-children)))
  (element invocation
    (make-latex-environment name: "sstinvocation"
	  (process-children)))
  (element implementationstatus
    (make-latex-environment name: "sstimplementationstatus"
	  (process-children)))
  (element bugs
    (make-latex-environment name: "sstbugs"
	  (process-children)))
  (element diytopic
    (let ((kids (children (current-node))))
      (make sequence
...
; 	  (make sequence
; 	    (literal " (p.")
; 	    (make-latex-empty-command name: "pageref"
; 		  parameters: `(,(string-append
; 				  "R"
; 				  (attribute-string (normalize "id")))))
; 	    (literal ")")
