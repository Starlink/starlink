...
(element ref
  (let* ((target-id (attribute-string (normalize "id")
	...
	(make element gi: "iref"
	  attributes: (list (list "target" target-id))
	  (if linktext
	      (literal linktext)
...
<codebody>
;; define dummy variables to keep slcommon.dsl happy
...
(element docxref
  (let* ((xrefent (attribute-string (normalize "doc") (current-node)))
	...
	(if xrefent
		...
		(make element gi: "xref" ; link to whole document
		      attributes:
		      (list (list "target"
...
<codebody>
(element url
  (let ((nolink (attribute-string (normalize "nolink") (current-node))))
...
(element webref
  (make element gi: "xref"
    attributes: (list (list "target" (attribute-string (normalize "url")
						       (current-node))))
    (process-children)))
