...
<p>Add "figurecontent" to the list
of elements in (chunk-element-list), and treat it specially,
extracting the names of the sysids referenced in its `image'
attribute.  Note that we extract <em/all/ the sysids, not just
the one selected for the HTML output. 
...
")
  ...
  (element figurecontent
    (let* ((entitystring (attribute-string (normalize "image")))
	   (entitylist (and entitystring
			   (tokenise-string entitystring))))
      (if entitylist
	  (let loop ((entlist entitylist)
		     (res (empty-sosofo)))
	    (if (null? entlist)
		res
		(loop (cdr entlist)
