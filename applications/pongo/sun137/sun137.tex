...
\stardocinitials    {SUN}
\stardocsource      {sun137.7}
\stardocnumber      {137.7}
\stardocauthors   {Paul Harrison, Paul Rees, \\
                                Peter Draper, Alasdair Allan }
\stardocdate        {27 March 2008}
\stardoctitle       {PONGO ---
   A Set of Applications for Interactive Data Plotting}
\stardocversion     {2.0-3}
\stardocmanual      {User's Manual}
\startitlepic{\includegraphics[width=0.5\textwidth]{sun137_cover}}
\stardocabstract  {PONGO is a suite of commands for
...
\providecommand{\xcol}{\textsf{XCOL}}
\providecommand{\excol}{\textsf{EXCOL}}
\providecommand{\ycol}{\textsf{YCOL}}
\providecommand{\eycol}{\textsf{EYCOL}}
\providecommand{\zcol}{\textsf{ZCOL}}
\providecommand{\symcol}{\textsf{SYMCOL}}
\providecommand{\labcol}{\textsf{LABCOL}}

% Types for various entities.
\providecommand{\pnam}[1]{\texttt{#1}}
\providecommand{\cnam}[1]{\texttt{#1}}

% Internal cross-reference to labels (uses text description, ignored
% by Latex).
\providecommand{\iref} [1]{\htmlref{#1}{#1}}
\providecommand{\iiref}[2]{\htmlref{#1}{#2}}
...
However, \emph{the match is case-sensitive.}
...
The data is stored internally in \emph{radians}: in this conversion any data
...
\emph{Table \ref{proj_tab}} lists the geometries available in PONGO.
...
abilities of the ICL or CL command languages to create \emph{procedures}.
...
of ICL and the ``\emph{CL Programmer's Manual}'' and
``\emph{An Introductory User's Guide to IRAF Scripts}'' for CL.
...
This procedure produces \emph{Figure \ref{fig:SPECTRUM}}, a plot of a
low resolution
...
\starfig{sun137_fig1}{[htpb]}{width=0.9\textwidth}{fig:SPECTRUM}{SPECTRUM example.}{SPECTRUM example.}
\newpage


\subsubsection{Errors}

The procedure \verb+ERRORS+ was used to plot \emph{Figure
\ref{fig:ERRORS}}.  This example demonstrates plotting simple error
...
\starfig{sun137_fig2}{[htbp]}{width=0.9\textwidth}{fig:ERRORS}{ERRORS example.}{ERRORS example.}
\newpage
...
plot \emph{Figure \ref{fig:HISTOGRAM}}, which illustrates the plotting
...
\starfig{sun137_fig3}{[htbp]}{width=0.9\textwidth}{fig:HISTOGRAM}{HISTOGRAM example.}{HISTOGRAM example.}
\newpage


\subsubsection{Ppdotdiag}

This procedure produces \emph{Figure \ref{fig:PPDOTDIAG}}, a period
...
\starfig{sun137_fig4}{[htbp]}{width=0.9\textwidth}{fig:PPDOTDIAG}{PPDOTDIAG example.}{PPDOTDIAG example.}
\newpage
...
\emph{Figure \ref{fig:ELLIPSES}}, which illustrates the use of the
\cnam{\iref{ELLIPSES}} command for plotting error ellipses.

\starfig{sun137_fig5}{[htbp]}{width=\textwidth}{fig:ELLIPSES}{ELLIPSES example.}{ELLIPSES example.}
\newpage
...
to produce \emph{Figure \ref{fig:PROJECTIONS}}.

\starfig{sun137_fig6}{[htbp]}{width=0.9\textwidth}{fig:PROJECTIONS}{PROJECTIONS example.}{PROJECTIONS example.}
\newpage
...
format.
The output from it is shown in \emph{Figure \ref{fig:RADEC}}.
The objects are some of the pulsars from
\emph{Figures \ref{fig:PROJECTIONS}} and \emph{\ref{fig:PPDOTDIAG}}.

\starfig{sun137_fig7}{[htbp]}{width=0.9\textwidth}{fig:RADEC}{RADEC example.}{RADEC example.}
\newpage
...
\starfig{sun137_fig9}{[htbp]}{width=0.9\textwidth}{fig:VECTOR}{VECTOR example.}{VECTOR example.}
\newpage
...
(\ie \texttt{Z+0.1}) to avoid very large symbols for nearby pulsars.
...
\starfig{sun137_fig8}{[htbp]}{width=\textwidth}{fig:AGI}{AGI example.}{AGI example.}
\newpage
...
AGI divides the plotting surface up into \textbf{pictures}, where a picture
...
\textbf{workstation} in Starlink graphics terminology) upon which the picture was
...
a \textbf{BASE} picture refers to the whole plotting surface and the name is
reserved only for this use;
a \textbf{FRAME} picture refers to a picture which contains other pictures;
and a \textbf{DATA} picture refers to a picture which contains a plot of the data.
...
PONGO uses the definitions of \textbf{world coordinates}, \textbf{window},
\textbf{device coordinates} and \textbf{viewport} used by the
...
It is of particular significance here that the column labels are \emph{case-sensitive}.
...
For simple cases, this behaviour is described in {\bf[} \emph{square
brackets} {\bf]}; for more complicated behaviour, a full description
is provided.  An empty set of square brackets, \ie \textbf{[]}, indicates
that a parameter value \emph{must} be specified on the command line.
...
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         values: \texttt{"}NONE\texttt{"}, \texttt{"}TAN\texttt{"}, \texttt{"}SIN\texttt{"}, \texttt{"}ARC\texttt{"}, \texttt{"}GLS\texttt{"}, \texttt{"}AITOFF\texttt{"},
         \texttt{"}MERCATOR\texttt{"}, and \texttt{"}STG\texttt{"}.

         This parameter is not specified on the command line. The value
         of the global parameter PONGO\_PROJECTN is used. If
         PONGO\_PROJECTN is not defined, the default value \texttt{"}NONE\texttt{"} is
         used.
      }
      \sstsubsection{
         RACENTRE = \_CHAR (Read)
         ...
         PROJECTION values other than \texttt{"}NONE\texttt{"}.

         This parameter is not specified on the command line. The value
         of the global parameter PONGO\_RACENTRE is used. If
         PONGO\_RACENTRE is not defined, the default value \texttt{"}0\texttt{"} is used.
      }
      \sstsubsection{
         ...
         required for PROJECTION values other than \texttt{"}NONE\texttt{"}.

         This parameter is not specified on the command line. The value
         of the global parameter PONGO\_DECCENTRE is used. If
         PONGO\_DECCENTRE is not defined, the default value \texttt{"}0\texttt{"} is used.
      }
   }
}
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         used. If there is no current value, a default value \texttt{"}NONE\texttt{"} is
         used.
      }
      \sstsubsection{
         RACENTRE = \_CHAR (Read)
...
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         The type of binning be used for the averaging. If \texttt{"}X\texttt{"}, the
         data are divided into NBIN bins over the X range.  If \texttt{"}N\texttt{"},
         bins of varying widths with each containing NBIN data points
         are formed.

         [The value will be prompted for. It has the default \texttt{"}X\texttt{"}.]
      }
      \sstsubsection{
         NBIN = \_INTEGER (Read)
      }{
         Depending upon the value of ACTION, either the number of bins
         (ACTION=\texttt{"}X\texttt{"}), or the number of points per bin (ACTION=\texttt{"}N\texttt{"}).
...
\sstroutine{
   ...
   \sstdescription{
      ...
      (using either the \texttt{"}current\texttt{"} picture or selecting using a known
   ...
   \sstparameters{
      \sstsubsection{
         ...
         If \texttt{'}B\texttt{'}, the plotting device will be cleared and the whole of
         its plotting surface used. If equal to \texttt{'}C\texttt{'}, the current picture
         will be used and a PGPLOT viewport created inside it. If \texttt{'}L\texttt{'}
...
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
               \texttt{"}A\texttt{"} -- Draw an axis (the X axis is the horizontal line
               Y=0, the Y axis is the vertical line X=0).

            \sstitem
               \texttt{"}B\texttt{"} -- Draw the bottom (X) or left (Y) edge of the frame.

            \sstitem
               \texttt{"}C\texttt{"} -- Draw the top (X) or right (Y) edge of the frame.

            \sstitem
               \texttt{"}G\texttt{"} -- Draw a grid of vertical (X) or horizontal (Y)
               lines.

            \sstitem
               \texttt{"}I\texttt{"} -- Invert the tick-marks (\ie draw them outside the
               viewport instead of inside).

            \sstitem
               \texttt{"}L\texttt{"} -- Label the axis logarithmically (see below).

            \sstitem
               \texttt{"}N\texttt{"} -- Write numeric labels in the conventional location
               below the viewport (X) or to the left of the viewport (Y).

            \sstitem
               \texttt{"}P\texttt{"} -- Extend (project) major tick-marks outside the box
               (ignored if option I is specified).

            \sstitem
               \texttt{"}M\texttt{"} -- Write numeric labels in the unconventional location
               above the viewport (X) or to the right of the viewport (Y).

            \sstitem
               \texttt{"}T\texttt{"} -- Draw major tick-marks at the major coordinate
               interval.

            \sstitem
               \texttt{"}S\texttt{"} -- Draw minor tick-marks (sub-ticks).

            \sstitem
               \texttt{"}V\texttt{"} -- Orient numeric labels vertically (this is only
               applicable to Y -- the default is to write Y-axis labels
               parallel to the axis).

            \sstitem
               \texttt{"}1\texttt{"} -- Force decimal labelling.

            \sstitem
               \texttt{"}2\texttt{"} -- Force exponential labelling.
         ...
         \sstitemlist{
            ...
            \sstitem
               \texttt{"}Z\texttt{"} for (DD) HH MM SS.S time labelling

            \sstitem
               \texttt{"}H\texttt{"} means superscript numbers with d, h, m, \& s  symbols

            \sstitem
               \texttt{"}D\texttt{"} means superscript numbers with    o, \texttt{'}, \& \texttt{'}\texttt{'} symbols

            \sstitem
               \texttt{"}F\texttt{"} causes the first label (left- or bottom-most) to
                     be omitted. Useful for sub-panels that abut each other.
                     Care is needed because first label carries sign as well.

            \sstitem
               \texttt{"}O\texttt{"} means omit leading zeros in numbers $<$ 10
         ...
         defined, the default value \texttt{"}BCNST\texttt{"} is used.]
      }
      \sstsubsection{
   ...
   \sstnotes{
      \sstitemlist{
         ...
         DD:MM:SS.SSS formats) and then set the \texttt{"}Z\texttt{"} character in the
         ...
         \texttt{"}Y\texttt{"} option is always set and that the Y axis values are
...
\sstroutine{
   ...
   \sstdescription{
      ...
      \sstitemlist{
         ...
         \sstitem
            \texttt{"}X\texttt{"} -- the \xcol\ data area,

         \sstitem
            \texttt{"}Y\texttt{"} -- the \ycol\ data area,

         \sstitem
            \texttt{"}Z\texttt{"} -- the \zcol\ data area,

         \sstitem
            \texttt{"}EX\texttt{"} -- the \excol\ error area,

         \sstitem
            \texttt{"}EY\texttt{"} -- the \eycol\ error area.
      }
   }
   \sstusage{
      ccmath
   ...
   \sstparameters{
      \sstsubsection{
         ...
         [\texttt{"}X -- \ie will cause the contents of the data area to remain
         unchanged.]
      }
      \sstsubsection{
         Y = \_CHAR (Read)
         ...
         [\texttt{"}Y\texttt{"} -- \ie will cause the contents of the data area to remain
         unchanged.]
      }
      \sstsubsection{
         Z = \_CHAR (Read)
         ...
         [\texttt{"}Z\texttt{"} -- \ie will cause the contents of the data area to remain
         unchanged.]
      }
      \sstsubsection{
         EX = \_CHAR (Read)
         ...
         [\texttt{"}EX\texttt{"} -- \ie will cause the contents of the data area to
         remain unchanged.]
      }
      \sstsubsection{
         EY = \_CHAR (Read)
         ...
         [\texttt{"}EY\texttt{"} -- \ie will cause the contents of the data area to
         remain unchanged.]
      }
   }
   \sstexamples{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      \texttt{"}L\texttt{"} option to the PONGO\_XOPT or PONGO\_YOPT global parameters, as
      appropriate.
   }
   \sstusage{
      clog action
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         updated to include the PGPLOT \texttt{"}L\texttt{"} axis option at the start.
         ...
         remove any \texttt{"}L\texttt{"} characters at the start of this string since it
         assumes that they have been put there by CLOG, and fresh data
         read in will not be logarithmic. If data are naturally
         logarithmic, the \texttt{"}L\texttt{"} should be placed other than at the start
         of the string to make an \texttt{"}L\texttt{"} option that will not be modified
         by PONGO.
...
\sstroutine{
   ...
   \sstdescription{
      ...
      \sstitemlist{
         ...
         \sstitem
            D -- DRAW
            A line is drawn from the PGPLOT \texttt{"}current position\texttt{"} to  the
         ...
         \sstitem
            ...
            label is to be drawn. Once \texttt{"}L\texttt{"} has been
            pressed, you are
         ...
         \sstitemlist{
            ...
            On pressing the \texttt{"}X\texttt{"} key, a label is prompted for. If only
         ...
         \sstitem
            V -- MOVE
            The PGPLOT \texttt{"}current position\texttt{"} will be set to the cursor
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         Allowed values: \texttt{"}NONE\texttt{"}, \texttt{"}TAN\texttt{"}, \texttt{"}SIN\texttt{"}, \texttt{"}ARC\texttt{"}, \texttt{"}GLS\texttt{"}, \texttt{"}AITOFF\texttt{"},
         \texttt{"}MERCATOR\texttt{"} and \texttt{"}STG\texttt{"}.
...
\sstroutine{
   ...
   \sstdescription{
      ...
      If a picture created by a run of a previous application is made
      (using either the \texttt{"}current\texttt{"} picture or selecting
...
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         explained in more detail in the section on projections.
         Allowed values: \texttt{"}NONE\texttt{"}, \texttt{"}TAN\texttt{"}, \texttt{"}SIN\texttt{"}, \texttt{"}ARC\texttt{"}, \texttt{"}GLS\texttt{"}, \texttt{"}AITOFF\texttt{"},
         \texttt{"}MERCATOR\texttt{"} and \texttt{"}STG\texttt{"}.This parameter is only
...
\sstroutine{
   ...
   \sstexamples{
      ...
      \sstexamplesubsection{
         drawpoly projection=aitoff deccentre=\texttt{"}12:00:00\texttt{"}
      }{
         This draws a single polygon on the current graphics display. The
         polygon is filled or shaded using the current fill-hatch styles
...
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         string \texttt{"}PONGO:\texttt{"}.
         [\texttt{"}Final viewport\texttt{"}]
      }
   }
}
\sstroutine{
...
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         \texttt{"}X\texttt{"} or \texttt{"}Y\texttt{"} depending upon which set
...
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         \cnam{\iref{READF}}
         command. The value \texttt{"}0\texttt{"} means \texttt{"}do not read these data\texttt{"}. []
      }
   }
}
\sstroutine{
...
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         \texttt{"}POLY\texttt{"} or \texttt{"}SPLINE\texttt{"}
      ...
      \sstsubsection{
         ...
         If ACTION is \texttt{"}POLY\texttt{"}, the polynomial coefficients resulting
      ...
      \sstsubsection{
         ...
         Only used if ACTION is \texttt{"}SPLINE\texttt{"}. This factor determines the
      ...
      \sstsubsection{
         ...
         \texttt{"}SPLINE\texttt{"}. This file can be used by the PLOTFUN command to
         redraw the fit.
...
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         The method of specifying the data point in question. If \texttt{"}N\texttt{"},
         interpret the VALUE parameter as specifying the index number
         of that point. If \texttt{"}C\texttt{"},  the VALUE parameter is used to try to
...
\newpage
   \sstexamples{
      \sstexamplesubsection{
         PONGO$>$ GETPOINT C \texttt{'}3C45\texttt{'} X=(XP) Y=(YP)
      }{
         This will return the X and Y coordinates of the data point
         that has the label \texttt{'}3C45\texttt{'}, if it exists, to the ICL variables
         XP and YP.
      }
   }
}
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         The type of plot to produce. This can be \texttt{"}C\texttt{"}, \texttt{"}P\texttt{"} or \texttt{"}S\texttt{"},
         with the following meanings:

         \sstitemlist{

            \sstitem
            \texttt{"}C\texttt{"} (connect) -- This action simply draws straight line
            segments between the data points.

            \sstitem
            \texttt{"}P\texttt{"} (points) -- Draw a symbol at each of the data points. The
         ...
         refers to the standard PGPLOT symbols.
            ...
            \sstitem
            \texttt{"}S\texttt{"} (sizeplot) -- This action uses the values stored in the
      ...
      \sstsubsection{
         ...
         a reasonable range of symbol sizes when ACTION=\texttt{"}S\texttt{"}.
      ...
      \sstsubsection{
         ...
         projections.  Allowed values: \texttt{"}NONE\texttt{"}, \texttt{"}TAN\texttt{"}, \texttt{"}SIN\texttt{"}, \texttt{"}ARC\texttt{"},
         \texttt{"}GLS\texttt{"}, \texttt{"}AITOFF\texttt{"}, \texttt{"}MERCATOR\texttt{"} and \texttt{"}STG\texttt{"}.
...
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         \texttt{"}NONE\texttt{"}, \texttt{"}TAN\texttt{"}, \texttt{"}SIN\texttt{"}, \texttt{"}ARC\texttt{"}, \texttt{"}GLS\texttt{"}, \texttt{"}AITOFF\texttt{"}, \texttt{"}MERCATOR\texttt{"} and
         \texttt{"}STG\texttt{"}.
...
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         When ACTION is \texttt{"}H\texttt{"} this parameter controls whether the
...
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         read by the \cnam{\iref{READF}} command. The value \texttt{"}0\texttt{"} means \texttt{"}do not read
         these data\texttt{"}. []
      }
   }
}
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         PONGO\_TITLE is not defined, the value \texttt{"} \texttt{"} is used.]
      }
      \sstsubsection{
         COLUMNS = \_LOGICAL (Read)
      }{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
               \texttt{"}T\texttt{"} -- The top edge.

            \sstitem
               \texttt{"}B\texttt{"} -- The bottom edge.

            \sstitem
               \texttt{"}L\texttt{"} -- The left-hand edge.

            \sstitem
               \texttt{"}R\texttt{"} -- The right-hand edge.

            \sstitem
               \texttt{"}LV\texttt{"} -- The left-hand edge, but with the string written
               vertically.

            \sstitem
               \texttt{"}RV\texttt{"} -- The right-hand edge, but with the string written
               vertically.
...
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         is read by the \cnam{\iref{READF}} command. The value \texttt{"}0\texttt{"} means \texttt{"}do not read
         these data\texttt{"}. []
      }
   }
}
   ...
   \sstparameters{
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
               \texttt{"}FUNC\texttt{"} -- Use a Fortran-like expression to define the
               function.

            \sstitem
               \texttt{"}POLY\texttt{"} -- Use a set of polynomial coefficients to define
               the function.

            \sstitem
               \texttt{"}SPLINE\texttt{"} -- Use a set of spline coefficients from the file
      ...
      \sstsubsection{
         ...
         The order of the polynomial: used when ACTION is \texttt{"}POLY\texttt{"}.
      ...
      \sstsubsection{
         ...
         A list of polynomial coefficients: used when ACTION is \texttt{"}POLY\texttt{"}.
   ...
   \sstparameters{
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
            \texttt{"}H\texttt{"} -- If the data in the \xcol\ data area are
            not binned, they
            ...
            \sstitem
            \texttt{"}B\texttt{"} -- If the data have already been binned, this mode will
      ...
      \sstsubsection{
         ...
         When ACTION is \texttt{"}H\texttt{"}, this parameter specifies the lower limit
         of the binning.
      ...
      \sstsubsection{
         ...
         When ACTION is \texttt{"}H\texttt{"}, this parameter specifies the upper limit of
         the binning.
      ...
      \sstsubsection{
         ...
         When ACTION is \texttt{"}H\texttt{"}, this parameter specifies the number of
      ...
      \sstsubsection{
         ...
         When ACTION is \texttt{"}H\texttt{"} this parameter specifies whether PGPLOT
      ...
      \sstsubsection{
         ...
         When ACTION is \texttt{"}B\texttt{"}, this parameter specifies whether the
   ...
   \sstparameters{
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
            \texttt{"}M\texttt{"} -- move to,

            \sstitem
            \texttt{"}D\texttt{"} -- draw line to,
...
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         detail in the section on projections.  Allowed values: \texttt{"}NONE\texttt{"},
         \texttt{"}TAN\texttt{"}, \texttt{"}SIN\texttt{"}, \texttt{"}ARC\texttt{"}, \texttt{"}GLS\texttt{"}, \texttt{"}AITOFF\texttt{"}, \texttt{"}MERCATOR\texttt{"} and \texttt{"}STG\texttt{"}.
...
\newpage
   \sstparameters{
      ...
      \sstsubsection{
         ...
         If the value is not specified on the command line, the current
         value is used. The current value is initially set to \texttt{"}!\texttt{"}.
      }
      \sstsubsection{
      ...
      \sstsubsection{
         ...
         The column number (counting from 1), or the symbolic name of a
         column, from which the X-axis data are read. The value \texttt{"}0\texttt{"}
         means \texttt{"}do not read these data\texttt{"}.

         If the value is not specified on the command line, the value
         of the global parameter PONGO\_XCOL is used. If PONGO\_XCOL is
         not defined, the current value is used. The current value is
         initially set to \texttt{"}0\texttt{"}.
      }
      \sstsubsection{
         YCOL = \_CHAR (Read and Write)
      }{
         The column number (counting from 1), or the symbolic name of a
         column, from which the Y-axis data are read. The value \texttt{"}0\texttt{"}
         means \texttt{"}do not read these data\texttt{"}.
      ...
      \sstsubsection{
         ...
         The column number (counting from 1), or the symbolic name of a
         column, from which the Z-axis data are read. The value \texttt{"}0\texttt{"}
         means \texttt{"}do not read these data\texttt{"}.
      ...
      \sstsubsection{
         ...
         The column number (counting from 1), or the symbolic name of a
         column, from which the symbolic name for each data point is
         read. The value \texttt{"}0\texttt{"} means \texttt{"}do not read these data\texttt{"}.
      ...
      \sstsubsection{
         ...
         The column number (counting from 1), or the symbolic name of a
         column, from which the PGPLOT symbol code for each data point
         is read. The value \texttt{"}0\texttt{"} means \texttt{"}do not read these data\texttt{"}.
      ...
      \sstsubsection{
         ...
         LATEX format tables by setting DELIM=\texttt{"}\&\texttt{"}.
      ...
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitemlist{
               ...
               \sstitem \texttt{"}=\texttt{"} -- equals;
               \sstitem \texttt{"}\#\texttt{"} -- not equal;
               \sstitem \texttt{"}$>$\texttt{"} -- greater than;
               \sstitem \texttt{"}$<$\texttt{"} -- less than;
               \sstitem \texttt{"}CE\texttt{"} -- equal to string;
               \sstitem \texttt{"}C\#\texttt{"} -- not equal to string;
               \sstitem \texttt{"}RA\texttt{"} -- in the range VAL1 to VAL2;
               \sstitem \texttt{"}LI\texttt{"} -- select if in the following list of values;
               \sstitem \texttt{"}EX\texttt{"} -- exclude if in the following list of values;
               \sstitem \texttt{"}IN\texttt{"} -- select if the substring is contained within the
               value;
               \sstitem \texttt{"}A$>$\texttt{"} -- absolute value greater than;
               \sstitem \texttt{"}A$<$\texttt{"} -- absolute value less than.
            }
         ...
         criterion. A value of \texttt{"}0\texttt{"} means \texttt{"}read everything\texttt{"}.
      ...
      \sstsubsection{
         ...
         The application will automatically remove any \texttt{"}L\texttt{"} characters
         ...
         given in logarithmic form, the \texttt{"}L\texttt{"} character should be
         inserted into the options strings anywhere except at the
         start.

         [The value of the global parameter PONGO\_XOPT is used. If
         PONGO\_XOPT is not defined, the default value \texttt{"}BCNST\texttt{"} is used.]
      }
      \sstsubsection{
         YOPT = \_CHAR (Read and Write)
      }{
      ...
      \sstsubsection{
         ...
         If TRUE, a \texttt{"}quick mode\texttt{"} read is performed. This mode can only
...
\sstroutine{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         \texttt{"}NONE\texttt{"}, \texttt{"}TAN\texttt{"}, \texttt{"}SIN\texttt{"}, \texttt{"}ARC\texttt{"}, \texttt{"}GLS\texttt{"}, \texttt{"}AITOFF\texttt{"}, \texttt{"}MERCATOR\texttt{"}
         and \texttt{"}STG\texttt{"}.
   ...
   \sstparameters{
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
               \texttt{"}STANDARD\texttt{"} -- The viewport is set to the standard PGPLOT
               viewport.

            \sstitem
               \texttt{"}ADJUST\texttt{"} -- The viewport is adjusted so that the scales
            ...
            \sstitem
               \texttt{"}NDC\texttt{"} -- The viewport is set by specifying its extent in
               the X and Y directions in terms of normalised device
               coordinates (\ie coordinates that run from 0 to 1 along
               the horizontal and vertical directions).

            \sstitem
               \texttt{"}INCHES\texttt{"} -- The viewport is set by specifying its extent
   ...
   \sstparameters{
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
               \texttt{"}DATA\texttt{"} -- The limits are calculated from the data limits,
               with a small border added.

            \sstitem
               \texttt{"}DATA0\texttt{"} -- The limits are calculated as for DATA, but the
               origin is included as one of the data points.

            \sstitem
               \texttt{"}GIVEN\texttt{"} -- The limits specified on the command line are
               used.

            \sstitem
               \texttt{"}RECALL\texttt{"} -- The coordinates are recalled from a previous
      ...
      \sstsubsection{
         ...
         \texttt{"}DATA\texttt{"}, \texttt{"}DATA0\texttt{"} or \texttt{"}RECALL\texttt{"}. If ACTION=\texttt{"}GIVEN\texttt{"} and no value is
      ...
      \sstsubsection{
         ...
         [If ACTION=\texttt{"}RECALL\texttt{"}, the value is prompted for.]
      }
      \sstsubsection{
         PROJECTION = \_CHAR (Read and Write)
      }{
      ...
      \sstsubsection{
         ...
         used only when ACTION = \texttt{"}DATA\texttt{"} or \texttt{"}DATA0\texttt{"}.
         [The value of the global parameter PONGO\_ERSCALE is used. If
         PONGO\_ERSCALE is not defined, the default value 1.0 is used.]
      }
   }
   ...
   \sstparameters{
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
               \texttt{"}LABLST\texttt{"} -- Write the internal list of labels out.

            \sstitem
               \texttt{"}DATA\texttt{"} -- Write out selected data.

            \sstitem
               \texttt{"}AGIPIC\texttt{"} -- Write the label, name and comment for the
      ...
      \sstsubsection{
         ...
         If the value is not specified on the command line, the current
         value is used. The current value is initially set to \texttt{"}G25.16\texttt{"}.
      }
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
               \texttt{"}DATA\texttt{"} -- Used to indicate that the AGI picture contains
               the representation of data in some graphical form (\ie a
               graph).

            \sstitem
               \texttt{"}FRAME\texttt{"} -- Used to indicate that the AGI picture contains
               a group of other plots (\ie several \texttt{"}DATA\texttt{"} pictures).
      ...
      \sstsubsection{
         ...
         If the value is not specified on the command line, the current
         value is used. The current value is set to \texttt{"}User viewport\texttt{"}.
      }
      \sstsubsection{
   ...
   \sstparameters{
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
               \texttt{"}P\texttt{"} -- Use PGPTEXT which allows the position,
               justification and angle of the text to be specified.

            \sstitem
               \texttt{"}M\texttt{"} -- Use PGMTEXT which allows the text to be written
               relative to the viewport.

            \sstitem
               \texttt{"}S\texttt{"} -- Use PGTEXT which allows only simple (x,y)
      ...
      \sstsubsection{
         ...
         If ACTION is \texttt{"}P\texttt{"} or \texttt{"}S\texttt{"}, the X coordinate of the text.  With
         the \texttt{"}M\texttt{"} action, this parameter specifies the number of
      ...
      \sstsubsection{
         ...
         If ACTION is \texttt{"}P\texttt{"} or \texttt{"}S\texttt{"}, the Y coordinate of the text.  With
         the \texttt{"}M\texttt{"} action, this parameter specifies the fraction along
      ...
      \sstsubsection{
         ...
         If ACTION=\texttt{"}M\texttt{"}, the side of the viewport where the text is to
         ...
         If the value is not specified on the command line, the current
         value is used. The current value is initially set to \texttt{"}T\texttt{"}.
      }
      \sstsubsection{
         JUSTIFICATION = \_REAL (Read and Write)
      }{
      ...
      \sstsubsection{
         ...
         If ACTION=\texttt{"}P\texttt{"}, the angle relative to the horizontal at which
...
\emph{Table \ref{mongo_same_tab}} is a list of the commands that work as
expected (with the above proviso) and \emph{Table \ref{mongo_diff_tab}} gives a
