...
*   SUBROUTINE SP1RD
*
*   Reads data in from 'SPECTRUM' format 1 files
*
*   IMPORTS:
...
*
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
       SUBROUTINE SP1RD(ASIZE1,WAVE,FLUX,NPOINT,MAXBRK,BREAK,NBREAK,
...
*
       ...
       WRITE (*,'(''   SP1RD - title:  '',A50)') TITLE(1:50)
       READ (IUNIT,'(A79)',ERR=200) IHHEAD(1:79)
       READ (IUNIT,'(20X,I6)',ERR=200) NPOINT
       IF (NPOINT.GE.ASIZE1) THEN
          NPOINT = ASIZE1 - 1
          WRITE (*,
     :    '(''   SP1RD:  too many points, only'',I5,'' read in'')')
     :    NPOINT
       ENDIF
       READ (IUNIT,'(4(F8.3,E10.3))',ERR=200)
...
*
       ...
       GOTO 300
  200  CONTINUE
       WRITE (*,'(''   SP1RD:  error reading from file'')')
       CLOSE (IUNIT)
       SUBCHK = .FALSE.
