...
(and hence typically in the CFLAGS \file{mk} environment variable).  
...
to casts and to arguments of \cc{sizeof} (in fact almost any 
...
Files may be extracted from the source archive in more places than
one in the makefile - in particular, for some reason lost in the
mists of time the {\tt do\_test} target 
usually extracts reqired files from the source archive explicitly
rather than relying on a dependency.
It will be necessary to identify all such places, and ensure that
...
The script \xroutine{extmk.pl} is provided to make the necessary 
...
\end{verbatim}
\end{squote}
which indicates that a crude check suggests a difference between
...
\file{cmp\_inscnf.pl} (and similarly for the other filters),
...
whitespace.
Note that in order to make these comparisons, the \file{cmp\_*.pl} scripts
