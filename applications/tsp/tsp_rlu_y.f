C+
      SUBROUTINE TSP_RLU_Y (LOC,LABEL,UNITS,STATUS)
C
C                               T S P _ R L U _ Y
C
C  Routine name:
...
C
C  Function:
C     Read the LABEL and UNITS of the Y axis
C
C  Description:
...
C
C  Call:
C     CALL TSP_RLU_Y (LOC,LABEL,UNITS,STATUS)
C
...
C
      ...
      IF (STATUS .EQ. SAI__OK) THEN
          ...
          CALL NDF_ACGET(ID,'LABEL',2,LABEL,STATUS)
          CALL NDF_ACGET(ID,'UNITS',2,UNITS,STATUS)
