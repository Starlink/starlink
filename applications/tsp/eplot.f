...
*  If the variance is not present create a dummy array and set it to zero
      ...
      IF (STATUS .NE. SAI__OK) THEN
          ...
          CALL TSP_EPZERO(SIZE,%VAL(CNF_PVAL(IEPTR)))
      ENDIF
      CALL ERR_RLSE
...
*  If the variance is not present create a dummy array and set it to zero
      ...
      IF (STATUS .NE. SAI__OK) THEN
          ...
          CALL TSP_EPZERO(SIZE,%VAL(CNF_PVAL(QEPTR)))
      ENDIF
      CALL ERR_RLSE
...
*  If the variance is not present create a dummy array and set it to zero
      ...
      IF (STATUS .NE. SAI__OK) THEN
          ...
          CALL TSP_EPZERO(SIZE,%VAL(CNF_PVAL(UEPTR)))
      ENDIF
      CALL ERR_RLSE
...
*  Calculate P and Theta arrays and errors
         ...
         CALL TSP_EP(SIZE,%VAL(CNF_PVAL(IPTR)),%VAL(CNF_PVAL(IEPTR)),
     :       %VAL(CNF_PVAL(QPTR)),%VAL(CNF_PVAL(UPTR)),
     :     %VAL(CNF_PVAL(QEPTR)),%VAL(CNF_PVAL(UEPTR)),
     :       %VAL(CNF_PVAL(PPTR)),%VAL(CNF_PVAL(PEPTR)),
     :       %VAL(CNF_PVAL(TPTR)),
     :     %VAL(CNF_PVAL(TEPTR)),THETA,STATUS)

*  Find maximum and minimum values

         CALL TSP_EPSCALE(SIZE,%VAL(CNF_PVAL(IPTR)),
     :       %VAL(CNF_PVAL(PPTR)),IMIN,IMAX,
     :     PMIN,PMAX,STATUS)
      ENDIF
...
*  Do the plot
      ...
      IF (STATUS .EQ. SAI__OK) THEN
        CALL TSP_EPLOT(SIZE,%VAL(CNF_PVAL(IPTR)),%VAL(CNF_PVAL(IEPTR)),
     :       %VAL(CNF_PVAL(PPTR)),
     :   %VAL(CNF_PVAL(PEPTR)),%VAL(CNF_PVAL(TPTR)),
     :       %VAL(CNF_PVAL(TEPTR)),
     :   %VAL(CNF_PVAL(LPTR)),PMIN,PMAX,IMIN,IMAX,XLABEL,YLABEL,THETA,
     :       STATUS)
      ENDIF
