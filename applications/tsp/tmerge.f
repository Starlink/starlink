...
*  Copy the time axis data
        IF (STATUS .EQ. SAI__OK) THEN
          CALL TSP_TMERGECPYD(SIZE2,NEWSIZE,%VAL(CNF_PVAL(PTR1)),
     :       %VAL(CNF_PVAL(PTR2)))
        ENDIF
...
*  Copy the intensity data
        IF (STATUS .EQ. SAI__OK) THEN
         CALL TSP_TMERGECPY(SIZE,SIZE2,NEWSIZE,%VAL(CNF_PVAL(PTR1)),
     :       %VAL(CNF_PVAL(PTR2)))
        ENDIF
     ...
     :       %VAL(CNF_PVAL(PTR2)))
        ELSE
         ...
         IF (STAT1 .EQ. SAI__OK) THEN
           CALL TSP_TMERGEZERO(SIZE,SIZE2,NEWSIZE,%VAL(CNF_PVAL(PTR1)))
         ELSE
