...
*  I, Flux or Magnitude
          ...
          IF (ITEM.EQ.'I'.OR.ITEM.EQ.'FLUX'.OR.ITEM.EQ.'MAG') THEN
              CALL TSP_PHSBIN(SIZE,%VAL(CNF_PVAL(XXPTR)),
     :       %VAL(CNF_PVAL(XDPTR)),
     :          %VAL(CNF_PVAL(IPTR)),%VAL(CNF_PVAL(YRPTR)),
     :       %VAL(CNF_PVAL(YEPTR)),BINSIZE,
     :          %VAL(CNF_PVAL(TIPTR)),%VAL(CNF_PVAL(TDPTR)),PHSTART,
     :       PHEND,NEWSIZE,STATUS)
              SIZE = NEWSIZE
...
*  Stokes Q, U OR V (%)
     ...
     :       PHEND,NEWSIZE,STATUS)
              CALL TSP_PHSBIN(SIZE,%VAL(CNF_PVAL(XXPTR)),
     :       %VAL(CNF_PVAL(XDPTR)),
     :          %VAL(CNF_PVAL(STPTR)),%VAL(CNF_PVAL(SRPTR)),
     :       %VAL(CNF_PVAL(SEPTR)),BINSIZE,
     :          %VAL(CNF_PVAL(TIPTR)),%VAL(CNF_PVAL(TDPTR)),PHSTART,
     :       PHEND,NEWSIZE,STATUS)
              SIZE = NEWSIZE
...
*  Polarization (%)
     ...
     :       PHEND,NEWSIZE,STATUS)
              CALL TSP_PHSBIN(SIZE,%VAL(CNF_PVAL(XXPTR)),
     :       %VAL(CNF_PVAL(XDPTR)),
     :          %VAL(CNF_PVAL(QPTR)),%VAL(CNF_PVAL(SRPTR)),
     :       %VAL(CNF_PVAL(SEPTR)),BINSIZE,
     :          %VAL(CNF_PVAL(TIPTR)),%VAL(CNF_PVAL(TDPTR)),PHSTART,
     :       PHEND,NEWSIZE,STATUS)
              CALL TSP_PHSBIN(SIZE,%VAL(CNF_PVAL(XXPTR)),
     :       %VAL(CNF_PVAL(XDPTR)),
     :          %VAL(CNF_PVAL(UPTR)),%VAL(CNF_PVAL(S2RPTR)),
     :       %VAL(CNF_PVAL(S2EPTR)),BINSIZE,
     :          %VAL(CNF_PVAL(TIPTR)),%VAL(CNF_PVAL(TDPTR)),PHSTART,
     :       PHEND,NEWSIZE,STATUS)
              SIZE = NEWSIZE
              CALL TSP_PHSPOL(SIZE,%VAL(CNF_PVAL(YRPTR)),
     :       %VAL(CNF_PVAL(YEPTR)),%VAL(CNF_PVAL(SRPTR)),
     :             %VAL(CNF_PVAL(SEPTR)),%VAL(CNF_PVAL(S2RPTR)),
