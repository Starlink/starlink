...
*  Do the photometry on the frames
     ...
     :       FLUXCAL,%VAL(CNF_PVAL(PTR)),%VAL(CNF_PVAL(IOPTR)))
          CALL TSP_CCDPHOT(DIMS(1),DIMS(2),DIMS(3),XO,YO,RADIUS,
     :       FLUXCAL,%VAL(CNF_PVAL(PTR)),%VAL(CNF_PVAL(IEPTR)))
...
*  Combine the two sets of photometry to give the polarization
          CALL TSP_CCDPOL(DIMS(3),%VAL(CNF_PVAL(IEPTR)),
     :       %VAL(CNF_PVAL(IOPTR)),OFFSET,
     :       %VAL(CNF_PVAL(SPTR)),%VAL(CNF_PVAL(OPTR)),
     :       %VAL(CNF_PVAL(VPTR)),%VAL(CNF_PVAL(VVPTR)),STATUS)
          CALL NDF_END(STATUS)
      ENDIF
