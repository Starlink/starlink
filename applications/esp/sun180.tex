...
Briefly, the WCS component contains several co-ordinate {\em frames},
allowing positions within the data array to be addressed in different ways.
Each frame has a label, called its {\em Domain}, which usually describes
the co-ordinate system; two important ones are GRID (which is 
always present) and SKY (which may or may not be).
At any given time one of these frames is designated the {\em Current\/} one,
...
\end{verbatim}
\end{myquote}'s 
To find out more about WCS components see \xref{SUN/95}{sun95}{se:wcsuse}.
