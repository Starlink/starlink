...
ESP (\textbf{E}xtended {\bf S}urface {\bf P}hotometry) is a package 
...
detect flatfielding faults, remove cosmics rays, median filter images,
...
\sstroutine{CORR
   ...
   \sstdescription{
      ...
      The values obtained from all the surrounding image pixels
      are summed. The total generated is divided by using a 
      normalisation value created by taking the sums of squares for 
      F1 and F2, multiplying them together and taking the square root.
   ...
   \sstparameters{
      \sstsubsection{
         ...
         The modal pixel value found in the input NDF.
      ...
      \sstsubsection{
         ...
         The number of SIGMA above the sky level
         value at which the pixel count cutoff occurs.
      }
      \sstsubsection{
         OUT = NDF (Write)
      }{
      ...
      \sstsubsection{
         ...
         The size of each image pixel. The units are arc seconds.
      }
      \sstsubsection{
         SCALE = \_REAL (Read)
      }{
         The scale length of the galaxies to be highlighted in the
         output image. The units are arc seconds.
      }
      \sstsubsection{
         SIGMA = \_REAL (Read)
      }{
         The standard deviation of the background of the pixels in the 
   ...
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{corr in=hh out=cor1 scale=8. psize=0.3 
                        back=74. useall=y mult=1.}
   }{

        Correlates image HH with a mask/template optimised for
        galaxies of 8 arc seconds scale length. The pixel size on the
        image is .3 arc second, the background value 74 and
        all the pixels on the image can be used in the calculation.
        The output image is to be named COR1.

   }
      \sstexamplesubsection{
         \parbox{143mm}{corr in=fn1 out=fn4 scale=4 psize=2 mult=1000. 
                        back=66 useall=n sigma=15 nsigma=3}
   }{

        Correlates image FN1 with a mask/template optimised for
        galaxies of 4 arc seconds scale length. The pixel size is 2
        arc seconds and the background value 66.

        Pixels that are brighter than 66$+$15$\times$3 counts are not
        included in the correlation calculations (USEALL=FALSE).
        The output image is to be named FN4.
   ...
   \sstnotes{
      ...
      The program indicates its progress by showing the percentage of
      the image processed every 50 image columns.
   }
}
\newpage
\sstroutine{ELLFOU
   ...
   \sstusage{
      ELLFOU MODE BACK SIGMA PSIZE ZEROP ARDFIL DEVICE OUT AUTOL AUTOLT 
      \newline\hspace*{2.5em} FRZORI [CURSOR] [IN] [COSYS] [ORIGIN] (FINE)  
      \newline\hspace*{2.5em} [RLIM] (LIM1) (LIM2) [SAME] [AGAIN] 
      \newline\hspace*{2.5em} [INFILE] [IMGDEV]
   }
   \sstparameters{
      \sstsubsection{
         AGAIN=\_LOGICAL (Read)
       ...
       \sstsubsection{
         ...
         Direction of increasing angle convention. True is clockwise.
      }
      \sstsubsection{
         ANGOFF = \_REAL(Read)
      }{
      ...
      \sstsubsection{
         ...
         the defined co-ordinates for a better estimate.
      }
      \sstsubsection{
         AUTOLT=\_LOGICAL (Read)
      }{
         Should a centroid (Y) or weighted mean (N) method
         be used for the centre estimation?
      }
      \sstsubsection{
         BACK=\_REAL (Read)
      }{
         The background value for the image. The units are counts.
      }
      \sstsubsection{
         COSYS=\_CHAR (Read)
      }{
      ...
      \sstsubsection{
         ...
         to remain unchanged throughout the run. The origin is free 
         to move if FRZORI=FALSE.
      }
      \sstsubsection{
         IMGDEV=DEVICE (Read)
      }{
      ...
      \sstsubsection{
         ...
         background value. If this is found to be absent the
         global background value (BACK) is substituted.
      }
      \sstsubsection{
         IN=NDF (Read)
      }{
      ...
      \sstsubsection{
         ...
         The maximum ratio that is permitted between the mean
      ...
      \sstsubsection{
         ...
         The lower limit for mean profile value. If the mean count
      ...
      \sstsubsection{
         ...
         Image indices for the galaxy origin point to be used. The units are
         pixels.
      }
      \sstsubsection{
         OUT=\_CHAR (Read)
      ...
      \sstsubsection{
         ...
         Radius at which the profiling will be stopped. The units are pixels.
      }
      \sstsubsection{
         SAME=\_LOGICAL (Read)
      }{
      ...
      \sstsubsection{
         ...
         The standard deviation of the background value. The units are
         counts.
      }
      \sstsubsection{
         ZEROP=\_REAL (Read)
      }{
         Zero point of the scale for surface brightness plots. The units are
         magnitudes per square arc seconds.
      }
   }
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{ellfou mode=t back=620 sigma=39 
             psize=1 zerop=27 ardfil=\^{ }ardf 
             device=xw out=elf autol=t frzori=t cursor=t
             same=t}
      }{
 
         Profiles are obtained for the image co-ordinates determined
         using the cursor/mouse on the DATA image currently displayed
         on device XW. The background value of that image is
         620 with an associated standard deviation of 39. The
         magnitude scale assumed has a zero point of 27, all profiles
         will be output to text file ELF, the final results will
         also be plotted on the XW device and the galaxy centre
         co-ordinates are allowed to vary.

      }
      \sstexamplesubsection{
         \parbox{143mm}{ellfou mode=y back=126 sigma=45 psize=2
             zerop=26 ardfil=\^{ }ardf device=xw out=elf2 
             autol=y frzori=y cursor=y same=n imgdev=x2w}
      }{

         Profiles are obtained for the current data image on device
         XW. The results are output onto device X2W. An ARD file 
         definition in ardf is used to identify parts of the 
         image that may not be used in the profiling operation. An 
         attempt will be made to improve the co-ordinates indicated
         via the cursor/mouse but the galaxy centre co-ordinates will
         not be allowed to vary from one profile to the next.

      }
      \sstexamplesubsection{
         \parbox{143mm}{ellfou mode=t back=620 sigma=39 psize=1 
             zerop=27 ardfil=\^{ }ardf out=e1 autol=t frzori=t
             cursor=f in=p2 cosys=w origin=[96,92] rlim=10.
             imgdev=x2w}
      }{

         Profiles for the object at world co-ordinates 96,92 on image
         p2 are obtained out to a radius of 10 pixels. The results are
         output to device X2W and to a file text file ELF. The
         background is 620 with an associated standard deviation
         of 390.

      }
      \sstexamplesubsection{
         \parbox{143mm}{ellfou mode=n infile=coord ardfil=\^{ }ardf 
             in=jet frzori=n cosys=w back=371 sigma=23 rlim=20 
             psize=1 zerop=26 autol=n}
      }{

         The program is operated in file mode where the world
         co-ordinates of the galaxies to be profiled are read from file
         COORD. An ARD file ardfile.dat is used to identify parts of
         the image that cannot be used. The global value for the
         background value is input in case the COORD file does not
...
\sstroutine{ELLPRO
   ...
   \sstdescription{
      ...
      The centre of the galaxy position may be input interactively, 
      MODE=TRUE), or, if there are many galaxies to be considered,
      may be read in from a text file (MODE=FALSE). In addition, when 
      in MODE=TRUE the galaxy can be identified using a cursor 
      (CURSOR=TRUE) as opposed to a keyboard entry of its co-ordinates.
   }
   \sstusage{
      ELLPRO MODE BACK SIGMA PSIZE ZEROP ARDFIL DEVICE OUT
      \newline\hspace*{2.5em} AUTOL AUTOLT FRZORI [CURSOR] [COSYS] [IN] 
      \newline\hspace*{2.5em} [ORIGIN] (FAST) (FINE) [RLIM] (LIM1) (LIM2) 
      \newline\hspace*{2.5em} (LIM3) (FRACT) [SAME] [AGAIN] [INFILE) 
      \newline\hspace*{2.5em} [IMGDEV]
   }
   \sstparameters{
      \sstsubsection{
         AGAIN=\_LOGICAL (Read)
      ...
      \sstsubsection{
         ...
         The maximum ratio that is permitted between the mean
         value of the two preceeding radii profiled and that of
      ...
      \sstsubsection{
         ...
         The lower limit for mean profile value (above sky).
         If the mean value for the current profile drops below
         this value the profiling operation stops. The units 
         are SIGMA.
      }
      \sstsubsection{
         LIM3=\_REAL (Read)
     ...
     The type of residual to be calculated is specified as 0, 1 or 2.
     This also 
     controls which type of statistic is returned in the final column
     of the ELLPRO output file.  It's not completely clear what is the
     best type of residual to 
     use.  The original one -- a weighted standard error, selected by
     giving this parameter the option~0 -- is rational,
     ...
     ESP maintainer would be interested to hear about it.
      ...
      \sstsubsection{
         ...
         Image indices for the origin point to be used. The units are pixels.
      }
      \sstsubsection{
         OUT=\_CHAR (Read)
      }{
      ...
      \sstsubsection{
         ...
         The standard deviation of the background value. The units are counts.
      }
      \sstsubsection{
         ZEROP=\_REAL (Read)
      }{
   ...
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{ellpro mode=t back=620 sigma=39 psize=1.
             zerop=27 ardfil=\^{ }ardf device=xw out=elp1 autol=t 
             frzori=f cursor=f cosys=d in=p2 rlim=10 origin=[96,92]}
      }{

         Performs profiling on an object positioned at (pixel
         co-ordinates) 96,92 on image p2. The profile determined will
         be output to a graphical display on device XW and to
         text file elp1. The galaxy centre provided will be
      ...
      \sstexamplesubsection{
         \parbox{143mm}{ellpro mode=y back=6200 sigma=390 psize=1 
              zerop=27 ardfil=\^{ }ardf device=xw out=elp1
              autol=n frzori=y cursor=y}
      }{
         ...
         contained in the file ardf is used to mask out bad
      ...
      \sstexamplesubsection{
         \parbox{143mm}{ellpro mode=false back=760 sigma=12 psize=.4 
              zerop=27 ardfil=\^{ }ardf in=p2 infile=p2log.dat
              out=outp cosys=w autol=t frzori=f}
      }{

         The application reads the object locations in file p2log.dat
         (taking them as world co-ordinates) and performs profiling on
         those locations on image p2. The profiling output is displayed
         on the text screen and is also placed in file outp.
...
\sstroutine{FASTMED
   ...
   Applies a square median filter of the specified size to an input image
}{
   \sstdescription{
      ...
      background value is added to the result, which is placed in the
   ...
   \sstparameters{
      \sstsubsection{
         ...
         Image background value. The units are counts.
      }
      \sstsubsection{
         IN = NDF (Read)
      }{
      ...
      \sstsubsection{
         ...
         The width of the median filter to be employed. The units are pixels.
      }
   }
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{fast in=field out=flatgal back=760. sigma=27. width=72}
      }{
         ...
         (field) had a global background value of 760. with
...
\newpage
\sstroutine{GAUFIT (original)
}{\label{des:gaufit}\xlabel{GAUFIT}
   ...
   \sstdescription{
     ...
     directions -- major axis then minor.  If the parameter FWHM is
     set true, these will be read as FWHM rather than s.d.) and the
     peak value.
...
There are two separate fitting algorithms within GAUFIT, a non-linear
least-squares routine (selectable with parameter LSQFIT=true) or the
original parameter-search routine.  This section describes the
original version, the next the updated one.
   
   }
   \sstusage{
      GAUFIT IN OUT MODE BACK SIGMA NSIGMA COSYS DEVICE
      AUTOL COLOUR INFILE MODEL MODTYP ANGCON ANGOFF 
      FWHM LSQFIT=false PSIZE NITER
      ANGINC SAINC SBINC PINC XINC YINC
   }
   \sstparameters{
      \sstsubsection{
        ANGCON = \_LOGICAL (Read)
   ...
   standard deviations?
      ...
      \sstsubsection{
        INFILE = \_CHAR (Read)
           ...
           Name of a text file containing the source co-ordinates
           to be profiled.
      }
      \sstsubsection{
        IN = NDF (Read)
   ...
   routine?  Must be LSQFIT=FALSE to use this variant of the routine.
      }
      \sstsubsection{
        MODE = \_LOGICAL (Read)
      }{
	...
	NITER = \_INTEGER (Read)
         ...
         The number of iterations the fitting routine is to use.
      }
      \sstsubsection{
        NSIGMA = \_REAL (Read)
      }{
      ...
      \sstsubsection{
        PSIZE = \_REAL (Read)
           ...
           The pixel size, in units of arcseconds.  If entered as
   null, then sizes will be displayed in units of pixels.
      }
      \sstsubsection{
        SAINC = \_REAL (Read)
      }{
   ...
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{gaufit mode=false infile=coords.dat in=image 
      ...
      \sstexamplesubsection{
         \parbox{143mm}{gaufit mode=true out=test1 modtyp=r angoff=90}
   }{
   ...
   \sstnotes{
      The residuals generated by this program have arbitrary units.
   }
}
\newpage
\sstroutine{GAUFIT (least-sq)
}{\label{des:gaufit2}
   Performs a 2-D Gaussian fit of multiple sources.
}{
   \sstdescription{
     Uses a least-squares fitting routine to determine the 2-D
     Gaussian profiles 
     of multiple sources on an NDF format image. This will be especially
     useful for those using JCMT data (see also \xref{JCMTDR}{sun132}{}).
...
least-squares routine (selectable with parameter LSQFIT=true) or the
original parameter-search routine.  This section describes the
updated version, the previous the original one.
   }
   \sstusage{
      GAUFIT IN OUT MODE BACK SIGMA CALCSD NSIGMA COSYS DEVICE
      COLOUR INFILE MODEL ANGCON ANGOFF 
      FWHM LSQFIT=true PSIZE NITER
   }
   \sstparameters{
      \sstsubsection{
        ANGCON = \_LOGICAL (Read)
      ...
      \sstsubsection{
        BACK = \_REAL (Read)
           ...
           The background value for the image.  If you give this as a
   negative value, the routine will obtain and report the best-fit background.
      }
      \sstsubsection{
   ...
   number.
      ...
      \sstsubsection{
        NSIGMA = \_REAL (Read)
           ...
           Number of sigma above sky at which pixels are considered 
           to be significant (only if BACK was specified).
      }
      \sstsubsection{
        OUT = \_CHAR (Read)
      }{
   ...
   null, then sizes will be displayed in units of pixels.
      ...
      \sstsubsection{
        SIGMA = \_REAL (Read)
           ...
           Standard deviation of the sky count (only if BACK was specified).
      }
   }
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{gaufit mode=false lsqfit=true infile=coords.dat
                               in=image cosys=w back=-1 model=imodel }
   }{
      ...
      \sstexamplesubsection{
         \parbox{143mm}{gaufit mode=true lsqfit=true out=test1
                               angoff=90}
   }{

           The sources will be identified by cursor. The resultant
           position angles will be modified by 90 degrees.

      }
   }
   \sstnotes{This variant of the GAUFIT application has appeared for
   the first time in this release, and I would be grateful to receive
   any comments on its robustness and suitability.
   }
}
\newpage
\sstroutine{GRAPHS
   ...
   \sstusage{
      GRAPHS MODE INFILE OUT RRANGE [AGAIN] [CURSOR] [DEVICE] [FITLIM]
      \newline\hspace*{2.5em} [RADISP] [WHATD]
   }
   \sstparameters{
      \sstsubsection{
         AGAIN = \_LOGICAL(Read)
      ...
      \sstsubsection{
         ...
         The range of radius values over which the scale length {\tt'}fits{\tt '}
      ...
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
            R = linear
            The radius values displayed are the semi-major axis
            dimensions when using ELLPRO/ELLFOU input files and distance
   ...
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{graphs mode=true infile=results out=scales 
            rrange=t cursor=t whatd=s radisp=r device=xw}
      }{

         The file results is examined and its contents displayed
      ...
      \sstexamplesubsection{
         \parbox{143mm}{graphs mode=t infile=profs out=lengths 
            rrange=f fitlim=1,20 cursor=f whatd=p radisp=l device=x2w}
      }{

         The results stored in file profs are read one after the
         other The profiles and {\tt '}fits{\tt '} may be observed 
         interactively as graphs on device X2W. The first display will be 
         of log(radius) versus position angle. The radius range used
         when calculating the scale length is 1 to 20 arc seconds.
         results are output to file lengths.

      }
      \sstexamplesubsection{
         \parbox{143mm}{graphs mode=f infile=elf1 out=lengths 
            rrange=t}
      }{

         The profile data required is read in from file elf1.
         Data from isophotes chosen by the application
         are selected for use in the scale length analysis.
         The results are output to file lengths.

      }
      \sstexamplesubsection{
         \parbox{143mm}{graphs mode=f infile=profs out=scales 
             rrange=f fitlim=0.5,7}
      }{

         Profile data read in from file profs is analysed to
...
\sstroutine{HISTPEAK
   ...
   \sstdescription{
      ...
      then constructs an image pixel value versus occurence
      histogram. This is used to allow count median, mode, kurtosis,
      standard deviation and skewness values to be estimated.
      ...
      \sstitemlist{
         ...
         projected mode. Calculated by extrapolating the lengths of a
           ...
           determining the pixel value at which this occurs.

         \sstitem
         interpolated mode. Calculated by assuming a Normal form
           for the histogram peak and {\tt '}fitting{\tt '} a function to it.
           The function is then used to provide both a modal value and
           its associated standard deviation.

      }
      Estimates of the standard deviation of pixel values
      and the standard deviation of the background value are generated.
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         The lowest pixel value found in the parts of the
         image that were used. The units are counts.
      }
      \sstsubsection{
         HIGH = \_REAL (Write)
      }{
         The highest pixel value found in the parts of the
         image that were used. The units are counts.
      }
      \sstsubsection{
         KURT = \_DOUBLE (Write)
      ...
      \sstsubsection{
         ...
         The mean pixel value calculated using the pixels found
         in the parts of the image used. The units are counts.
      }
      \sstsubsection{
         MEDIAN = \_DOUBLE (Write)
      }{
         The median pixel value calculated using the pixels
         found in the parts of the image used. The units are counts.
      }
      \sstsubsection{
         MODE = \_DOUBLE (Write)
      }{
      ...
      \sstsubsection{
         ...
         is present and then {\tt '}fitting{\tt '} it. The {\tt '}fit{\tt '} 
         obtained supplies the value for the background
      ...
      \sstsubsection{
         ...
         histogram peak is probably a skewed distribution.
      }
      \sstsubsection{
         MODES = \_DOUBLE (Write)
      }{
      ...
      \sstsubsection{
         ...
         The peak number of pixels found with a given value
      ...
      \sstsubsection{
         ...
         Estimates for the standard distribution of the image
         pixel values. The units are counts.
      }
      \sstsubsection{
         SIGMA = \_DOUBLE (Write)
      }{
         The standard deviation value of the background value calculated
      ...
      \sstsubsection{
         ...
         See SFACT. The units are counts.
      }
      \sstsubsection{
         SKEW = \_DOUBLE (Write)
      }{
      ...
      \sstsubsection{
         ...
         selected.
            USE={\tt '}W{\tt '} All the image pixels are used.
            USE={\tt '}A{\tt '} The image pixels are selected using an ARD file
      }
   }
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{histpeak in=galaxy sfact=3 use=w device=ikon1}
      }{
         ...
         its standard deviation is smoothed using a Gaussian filter of
         radius 3 counts.

      }
      \sstexamplesubsection{
         \parbox{143mm}{histpeak in=galaxy2 sfact=-1 use=a device=xwindows 
            ardfil=\^{ }okay.dat}
      }{

         The statistics are calculated for the image galaxy2, using 
         all the non-bad pixels on the image specified in ARD file okay.dat
         ...
         its associated standard deviation. The width of the smoothing filter is 
...
\sstroutine{HSUB
   ...
   \sstdescription{
      ...
      easily transplanted into the users ADAM applications. It establishes
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            The value returned is that actually
            employed. The units are counts.
         }
      }
      \sstsubsection{
         TYPE = \_INTEGER (Read)
         ...
         calculate the modal pixel value.
         1 = raw histogram
   ...
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{hsub in=ic3374 sfact=0 type=0}
      }{
      ...
      \sstexamplesubsection{
         \parbox{143mm}{hsub in=galaxy sfact=10 type=4}
      }{
      ...
      \sstexamplesubsection{
         \parbox{143mm}{hsub in=forn4 sfact=6 type=3}
      }{
   ...
   \sstnotes{
      ...
      This application is intended to form the basis of a user ADAM
      application requiring image statistics. The user requiring other
...
\sstroutine{LOBACK
   ...
   \sstdescription{
      ...
      The latter method is intended specifically for use with PISA, 
      RGASP{\tt '}s IMAGES or IRAF{\tt '}s FOCAS output files.

      All co-ordinates are read from a two or three columns
      ASCII text file. If two columns are present then these are
      taken as representing the image co-ordinates required for the
      regions of the image to be considered. If three columns are
      present the third column represents the width of the area
      to be sampled or the number of contiguous pixels detected there
      by PISA, FOCAS or IMAGES.
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         \sstitemlist{
            ...
            \sstitem
            $>$0 to indicate the radius of the Gaussian filter to use.
            The value returned is that employed. The units are counts.
         }
      }
      \sstsubsection{
         THIRD = \_LOGICAL (Read)
   ...
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{loback in=p2 infile=coords sfact=0 third=t 
             cosys=w out=backs wide=64}
      }{
            
         Reads the world co-ordinate data stored in text file COORDS
         and determines the background value within a 64x64 pixel
         area surrounding each of those locations. The histogram
         generated to do this will not be smoothed. The output will be
         into text file backs. Since THIRD is TRUE, the third
         column represents the number of pixels thought to make up
         the object.

      }
      \sstexamplesubsection{
         \parbox{143mm}{loback in=p2 infile=coords sfact=4 third=f 
             cosys=w out=outp width=35 }
      }{
             
         Determines the background value within a 35x35 pixel
         area surrounding each of the locations identified in
         COORDS. The histogram generated to do this will be
         smoothed using a Gaussian 4 counts wide. The output will be
         into text file outp. Since THIRD is FALSE, the third
...
\sstroutine{MASK
   ...
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{mask in=ic3374 ardfil=\^{ }ardfile.txt out=ic3374a}
      }{
...
\sstroutine{MIXUP
   ...
   \sstdescription{
      ...
      throughout the image but the pixel value statistics are retained.
      ...
      the significance of {\tt '}objects{\tt '} identified by the other 
      applications.
   }
   \sstusage{
      MIXUP IN OUT
   ...
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{mixup in=ic3374 out=ic3374m}
      }{

         The pixel positions in the input file 3374 will be
...
\sstroutine{SECTOR
   ...
   \sstdescription{
      ...
      wedge drawn outward from the galaxy origin point.
      ...
      galaxy origin. Pixel values are summed over all the pixels
      ...
      profiling applications, but merely to allow the user to obtain
   ...
   \sstusage{
      ...
      \newline\hspace*{2.5em} ZEROP OUT [IN] [DEVICE] [IMGDEV] [FITLIM] [POSANG]
      \newline\hspace*{2.5em} [ANGWID] [RLIM] [COSYS] [SAME] [AGAIN] [ORIGIN]
      \newline\hspace*{2.5em} (COLOUR)
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         Pen colour used when showing the galaxy centre.
      }
      \sstsubsection{
         COSYS = \_CHAR (Read)
      }{
      ...
      \sstsubsection{
         ...
         $y$ axis represents 0 degrees. The units are degrees.
      }
      \sstsubsection{
         PSIZE = \_REAL (Read)
      }{
         Size of the image pixels in arc seconds. The units are arc secs.
      }
      \sstsubsection{
         RADISP = \_CHAR (Read)
      }{
   ...
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{sector cursor=t ardfil=\^{ }ar1 back=620 
             sigma=39 psize=1 surf=t radisp=r mirror=t autol=t 
             zerop=27 out=x2w device=xw imgdev=xw same=f}
      }{

         Profiles an object identified on the currently displayed
         image using a cursor/mouse. The resulting profile is displayed
         as linear radius versus surface brightness. ARD file AR1 is
         used to identify parts of the image that may not be used.
         The source image is currently displayed on device XW and
         the graphs will appear on device X2W. The galaxy centre
      ...
      \sstexamplesubsection{
         \parbox{143mm}{sector cursor=f ardfil=\^{ }ardf back=76 
             sigma=2 psize=.3 surf=t radisp=q mirror=f autol=f
             zerop=26 in=ic3374 device=xw fitlim=0,20
             posang=25 angwid=5 rlim=25 cosys=d origin=47,123}
      }{
...
\sstroutine{SELFC
   ...
   \sstdescription{
      ...
      the user may elect to employ a cut out pixel value where
      ...
      (usually the sky background value - obtained via HISTPEAK),
      the background count standard deviation and the number of
      standard deviations above sky level at which the cutout should occur.
      ...
      Each pair is then considered in turn and the modal pixel value
      subtracted from each. The resultant residual pixel values
      ...
      objects detected but, despite this, appears to work well on 
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         image. The units are arc seconds.
      }
      \sstsubsection{
         IN = NDF (Read)
      }{
      ...
      \sstsubsection{
         ...
         The number of standard deviations (SIGMA) above the sky level
         value at which the pixel count cutoff occurs.
      }
      \sstsubsection{
         OUT = NDF (Write)
      ...
      \sstsubsection{
         ...
         The size of each pixel. The units are arc seconds.
      }
      \sstsubsection{
         SIGMA = \_REAL (Read)
      }{
         The standard deviation of the background value of the pixels in the
         input NDF. Should be determined using a routine such as
         HISTPEAK which ignores outliers.
      }
      \sstsubsection{
   ...
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{selfc in=search out=scorr diam=15 psize=1 
            back=727 useall=true}
      }{

         A self-correlation is carried out on image galaxies. The
         search is for an object 15 arc seconds across. The image
         pixel size is 1 arc seconds and the background level
         is 727. USEALL=TRUE ensures that no pixels are excluded from
         the correlation calculation.

      }
      \sstexamplesubsection{
         \parbox{143mm}{selfc in=plate out=plates diam=5 psize=1 
            back=6600 useall=false sigma=35 nsigma=5}
      }{

         A self-correlation is carried out on image PLATE. The search
         is for an object 5 arc seconds across. The image pixel size
         is 1 arc second and the background level is 6600.
         Since USEALL=FALSE, all pixels with a value above
         6600$+$35$\times$5 are excluded from the correlation calculation.
...
\sstroutine{SELFCW
   ...
   \sstdescription{
      ...
      value (available via HISTPEAK), the background
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         The number of background value standard deviation above the sky 
         value at which the pixel count cutoff occurs.
      }
      \sstsubsection{
         MULT = \_REAL (Read)
      ...
      \sstsubsection{
         ...
         The standard deviation of the background value for the pixels in the
         input NDF. Should be determined using a routine such as
         HISTPEAK which ignores outliers.
      }
      \sstsubsection{
   ...
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{selfcw in=p2 out=scp2 scale=15 psize=.96 
             back=1000 useall=true}
      }{
         ...
         on the image is .96 arc second and the background value
         for the source image is 1000

      }
      \sstexamplesubsection{
         \parbox{143mm}{selfcw in=ls1 out=ls2 scale=8 psize=1 back=444
         ...
         second scale length, is generated using image p2 as the input
         source image and scp2 as the output image. The pixel size
         on the image is 1. arc second and the background value
         for the source image is 444.

         All pixels with a pixel value greater than 444.$+$12.$\times$4. are
...
\sstroutine{SKEW
   ...
   \sstdescription{
      ...
      \sstitemlist{
         ...
         \sstitem
         the user may elect to employ a cut out pixel value
           where any pixel found to be above that value is ignored. The
           cutout value is determined by the user inputing a global mode
           value, its standard deviation (available via
      ...
      is multiplied by 1000 (or some other defined value) to make display
      easier.

      The modal pixel value used during the calculation is either the
      global value, or a local value calculated as required.

      The resultant value is some measure of the extent to which the
      pixel values surrounding a given pixel are not distributed
   ...
   \sstparameters{
      \sstsubsection{
         ...
         The background pixel value found in the input NDF.
         The units are counts. Only used if MODET = TRUE.
      }
      \sstsubsection{
         IN = NDF (Read)
      }{
      ...
      \sstsubsection{
         ...
         Used to indicate whether a global modal pixel value
      ...
      \sstsubsection{
         ...
         The number of background standard deviations above the sky 
         value at which the pixel count cutoff occurs.
      ...
      \sstsubsection{
         ...
         The width of each pixel. The units are arc seconds.
      }
      \sstsubsection{
         SIGMA = \_REAL (Read)
      }{
         The standard deviation of the background value of the pixels in
         the input NDF. Should be determined using a routine such as
   ...
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{skew in=ic3374 out=skewed modet=false width=10. psize=0.5
           mult=1000}
      }{
      ...
      \sstexamplesubsection{
         \parbox{143mm}{skew in=jet out=sjet modet=true width=5. psize=1.
           mult=1000. back=2010. useall=true}
      }{

         An output image SJET is generated using JET as the source
         image. The pixel size is 1 arc second, the background 
      ...
      \sstexamplesubsection{
         \parbox{143mm}{skew in=sgp27 out=result modet=t width=8 psize=1 
           mult=1000 back=4505 sigma=23 nsigma=10 useall=f}
      }{

         The output image generated is created by assuming a global
         background of 4505 and a standard deviation
         of 23 counts. All pixels of a value greater
         than 4505$+$23$\times$10 are excluded from the calculations.
...
\sstroutine{TOPPED
   ...
   \sstdescription{
      ...
      those pixels within a defined radius of the bright pixel.
   ...
   \sstparameters{
      \sstsubsection{
         ...
         The background (sky) value. The units are counts.
      }
      \sstsubsection{
         IN = NDF (Read)
      }{
      ...
      \sstsubsection{
         ...
         The number of background standard deviations above sky at which the
         cutoff occurs. 
      }
      \sstsubsection{
         OUT = NDF (Write)
      ...
      \sstsubsection{
         ...
         The standard deviation of the background pixel value. The units are counts.
      }
      \sstsubsection{
         WIDTH = \_REAL (Read)
      }{
         The width of the circle around a bright pixel within which
         pixels will be set to bad. The units are arc seconds.
      }
   }
   \sstexamples{
      \sstexamplesubsection{
         \parbox{143mm}{topped in=eggs out=scrambled width=2.5 psize=0.44 
             back=1000. sigma=23. nsigma=8. noise=false}
      }{

         Uses EGGS as the input image and finds all pixels within the
         image that have a count greater than 1000.$+$8.$\times$23. These are
         all set to the bad value. In addition, all pixels within a
         radius of 1.25 arc seconds are also set to bad.

      }
      \sstexamplesubsection{
         \parbox{143mm}{topped in=obj out=cut width=4 psize=1 back=6200
             sigma=390 nsigma=10 noise=t}
      }{

         Uses OBJ as the input image and finds all pixels within
         the image that have a value greater than 6200$+$10$\times$390.
   ...
   \sstnotes{
      ...
      particularly for low pixel values, this may not be appropriate.
   }
}
...
!! X       Y     Points  Rad(a)     Count     PA     Ellipt     Dev   PPU       
  93.2    96.4     64     5.50       139.5   -21.5   0.559       1.0  100.      
  93.0    96.2     20     1.00       211.8   -26.0   0.141       0.8  100.      
  92.9    96.2     24     2.25       209.5   -26.0   0.141       0.8  100.      
  93.2    96.2     48     4.25       164.9   -26.0   0.573       1.3  100.      
  93.2    96.4     76     6.25       121.5   -16.8   0.578       0.9  100.      
  93.4    96.3    100     8.25        81.1   -15.0   0.593       0.6  100.      
  93.2    95.9    124    10.25        57.9   -17.8   0.592       0.7  100.      
  93.5    95.9    200    16.25        21.6   -23.6   0.612       0.6  100.      
  93.3    96.1    236    19.25        14.2   -13.2   0.612       0.5  100.      
  93.2    95.9    280    22.75        14.4   -11.8   0.489       0.5   91.      
  93.2    95.9    332    26.75         4.2   -13.1   0.650       0.5   89.      
## Fourier Descriptors:
!! Rad(a)  1xSin   1xCos   2xSin   2xCos   3xSin   3xCos   4xSin   4xCos        
   5.50   -0.011   0.006  -0.009   0.011   0.043   0.054  -0.017  -0.009        
   1.00   -0.008  -0.016  -0.002  -0.015   0.000   0.004   0.000   0.005        
   2.25   -0.010  -0.020  -0.007  -0.010   0.008   0.000   0.002   0.000        
   4.25   -0.006  -0.011  -0.015   0.012   0.064   0.026  -0.003  -0.003        
   6.25   -0.019  -0.004  -0.028   0.005   0.018   0.048  -0.033  -0.034        
   8.25    0.000  -0.030   0.005  -0.013  -0.009  -0.005  -0.035  -0.038        
  10.25    0.069   0.025   0.002   0.016  -0.028  -0.020   0.022  -0.008        
  16.25   -0.078  -0.037   0.056  -0.022   0.013   0.064  -0.093  -0.035        
  19.25   -0.132  -0.061  -0.059  -0.009   0.028   0.003   0.105  -0.093        
  22.75   -0.120  -0.227  -0.172  -0.005  -0.011  -0.061   0.102   0.061        
  26.75   -0.211  -0.273  -0.058   0.123   0.323   0.293  -0.272  -0.493        
