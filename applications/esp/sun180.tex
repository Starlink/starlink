...
\newcommand{\stardocauthors}   {Norman Gray\\Grant Privett\\Mark Taylor}
\newcommand{\stardocdate}      {11 November 1999}
...
the program identifies that location in the Current co-ordinate 
system of the image.
In this example, the image's Current co-ordinate frame is in the
SKY domain, which has co-ordinates of RA and Dec.
If you wanted to have positions reported instead in a different
co-ordinate frame in the image's World Coordinate System component,
then prior to displaying it, you should select
a different frame using \xref{KAPPA}{sun95}{}'s
\xref{WCSFRAME}{sun95}{WCSFRAME} command.
For instance the following would cause all the co-ordinates to 
be reported in PIXEL co-ordinates:
\begin{myquote}
\begin{verbatim}
% wcsframe ic3374 pixel
\end{verbatim}
\end{myquote}
You can find out more about World Co-ordinate System (WCS)
components of NDFs in \xref{SUN/95}{sun95}{se:wcsuse}.
...
not allowable (e.g.\ there are illegal characters in the name or the file name
...
Right button or CTRL-C:  Quit selection.
  ...
  PIXEL frame co-ordinates:  p1 = 92.7,  p2 = 93.7
...
\begin{myquote}
\begin{verbatim}
% topped in=galaxy out=cleaned noise=false width=3. back=6200 
         sigma=23 nsigma=10
\end{verbatim}
\end{myquote}
...
As usual, if the image contains a SKY co-ordinate frame then the 
...
\item[{\tt
   \% ellfou mode=true cursor=false in=img001 origin="8:36:50.2 0:17:22"}]
  \mbox{}\\
  Permits interactive use of ELLPRO but only via the keyboard. The origin 
...
!! NOTE: Position angles are stored on file with origin upward and clockwise rot
## END                                                                          
\end{verbatim}
\end{myquote}
