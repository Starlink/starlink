...
*
*   In i1mach, I uncommented the machine constants for IEEE 
*   machines, and changed data statement to set imach(6)=1 and removed test
*   (for Fortran 77 - see comment at beginning of i1mach).
*
*   In d1mach, things are a tad more complicated: there are _two_ 
*   possibilities, for big-endian (sparc) and little-endian (Intel) 
*   IEEE floats.  Rather than resort to preprocessor gymnastics, I've
*   extracted this routine to a separate file, and distributed
*   machine-dependent versions of this (d1mach.f_machine) with the
*   appropriate uncommentings.  There are actually possibilities for a
*   large range of processor types, here and in i1mach, but I've only
*   enabled support for the two IEEE variants.  I've included the
*   pristine (except for the name change) version of d1mach below, for
*   reference.
*
...
c$$$      DOUBLE PRECISION FUNCTION GAU2_D1MACH(I)
c$$$C
...
c$$$C
c$$$C  FOR IEEE-ARITHMETIC MACHINES (BINARY STANDARD), ONE OF THE FIRST
c$$$C  TWO SETS OF CONSTANTS BELOW SHOULD BE APPROPRIATE.  IF YOU DO NOT
c$$$C  KNOW WHICH SET TO USE, TRY BOTH AND SEE WHICH GIVES PLAUSIBLE
c$$$C  VALUES.
c$$$C
c$$$C  WHERE POSSIBLE, DECIMAL, OCTAL OR HEXADECIMAL CONSTANTS ARE USED
c$$$C  TO SPECIFY THE CONSTANTS EXACTLY.  SOMETIMES THIS REQUIRES USING
c$$$C  EQUIVALENT INTEGER ARRAYS.  IF YOUR COMPILER USES HALF-WORD
c$$$C  INTEGERS BY DEFAULT (SOMETIMES CALLED INTEGER*2), YOU MAY NEED TO
c$$$C  CHANGE INTEGER TO INTEGER*4 OR OTHERWISE INSTRUCT YOUR COMPILER
c$$$C  TO USE FULL-WORD INTEGERS IN THE NEXT 5 DECLARATIONS.
c$$$C
c$$$C  COMMENTS JUST BEFORE THE END STATEMENT (LINES STARTING WITH *)
c$$$C  GIVE C SOURCE FOR D1MACH.
c$$$C
c$$$      INTEGER SMALL(2)
c$$$      INTEGER LARGE(2)
c$$$      INTEGER RIGHT(2)
...
c$$$C
c$$$C     MACHINE CONSTANTS FOR BIG-ENDIAN IEEE ARITHMETIC (BINARY FORMAT)
c$$$C     MACHINES IN WHICH THE MOST SIGNIFICANT BYTE IS STORED FIRST,
c$$$C     SUCH AS THE AT&T 3B SERIES, MOTOROLA 68000 BASED MACHINES (E.G.
c$$$C     SUN 3), AND MACHINES THAT USE SPARC, HP, OR IBM RISC CHIPS.
c$$$C
...
c$$$C      DATA LOG10(1),LOG10(2) / 1070810131, 1352628735 /, SC/987/
c$$$C
c$$$C     MACHINE CONSTANTS FOR LITTLE-ENDIAN (BINARY) IEEE ARITHMETIC
c$$$C     MACHINES IN WHICH THE LEAST SIGNIFICANT BYTE IS STORED FIRST,
c$$$C     E.G. IBM PCS AND OTHER MACHINES THAT USE INTEL 80X87 OR DEC
c$$$C     ALPHA CHIPS.
c$$$C
...
c$$$C      DATA LOG10(1),LOG10(2) / 1352628735, 1070810131 /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(1),LOG10(2) / 1091781651, 1352628735 /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(2) / ZC77800DC0 /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(2) / O0006677466732724 /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(2) / 16367571421742254654B /, SC/987/
c$$$C
c$$$C     MACHINE CONSTANTS FOR FTN5 ON THE CDC 6000/7000 SERIES.
c$$$C
c$$$C      DATA SMALL(1) / O"00564000000000000000" /
c$$$C      DATA SMALL(2) / O"00000000000000000000" /
c$$$C
c$$$C      DATA LARGE(1) / O"37757777777777777777" /
c$$$C      DATA LARGE(2) / O"37157777777777777774" /
c$$$C
c$$$C      DATA RIGHT(1) / O"15624000000000000000" /
c$$$C      DATA RIGHT(2) / O"00000000000000000000" /
c$$$C
c$$$C      DATA DIVER(1) / O"15634000000000000000" /
c$$$C      DATA DIVER(2) / O"00000000000000000000" /
c$$$C
c$$$C      DATA LOG10(1) / O"17164642023241175717" /
c$$$C      DATA LOG10(2) / O"16367571421742254654" /, SC/987/
c$$$C
c$$$C     MACHINE CONSTANTS FOR CONVEX C-1
c$$$C
c$$$C      DATA SMALL(1),SMALL(2) / '00100000'X, '00000000'X /
c$$$C      DATA LARGE(1),LARGE(2) / '7FFFFFFF'X, 'FFFFFFFF'X /
c$$$C      DATA RIGHT(1),RIGHT(2) / '3CC00000'X, '00000000'X /
c$$$C      DATA DIVER(1),DIVER(2) / '3CD00000'X, '00000000'X /
c$$$C      DATA LOG10(1),LOG10(2) / '3FF34413'X, '509F79FF'X /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(2) / 000007571421742254654B /, SC/987/
c$$$C
c$$$C     MACHINE CONSTANTS FOR THE DATA GENERAL ECLIPSE S/200
c$$$C
c$$$C     SMALL, LARGE, RIGHT, DIVER, LOG10 SHOULD BE DECLARED
c$$$C     INTEGER SMALL(4), LARGE(4), RIGHT(4), DIVER(4), LOG10(4)
c$$$C
...
c$$$C      DATA LOG10/40423K,42023K,50237K,74776K/, SC/987/
c$$$C
...
c$$$C      DATA LOG10(1),LOG10(2) / '23210115, '10237777 /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(1),LOG10(2) / O776464202324, O117571775714 /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(1),LOG10(2) / Z41134413, Z509F79FF /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(1),LOG10(2) / Z'41134413', Z'509F79FF' /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(1),LOG10(2) / "177464202324, "144117571776 /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(1),LOG10(2) / "177464202324, "047674776746 /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(1),LOG10(2) / 1067065498, -2063872008 /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(1),LOG10(2) / O07746420232, O20476747770 /, SC/987/
c$$$C
c$$$C     MACHINE CONSTANTS FOR PDP-11 FORTRANS SUPPORTING
...
c$$$C      DATA LOG10(3),LOG10(4) / -31493, -12296 /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(3),LOG10(4) / O102373, O147770 /, SC/987/
c$$$C
c$$$C     MACHINE CONSTANTS FOR THE PRIME 50 SERIES SYSTEMS
c$$$C     WITH 32-BIT INTEGERS AND 64V MODE INSTRUCTIONS,
c$$$C     SUPPLIED BY IGOR BRAY.
c$$$C
c$$$C      DATA SMALL(1),SMALL(2) / :10000000000, :00000100001 /
c$$$C      DATA LARGE(1),LARGE(2) / :17777777777, :37777677775 /
c$$$C      DATA RIGHT(1),RIGHT(2) / :10000000000, :00000000122 /
c$$$C      DATA DIVER(1),DIVER(2) / :10000000000, :00000000123 /
c$$$C      DATA LOG10(1),LOG10(2) / :11504046501, :07674600177 /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(1),LOG10(2) / $509F79FF,  $3FD34413 /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(1),LOG10(2) / O177746420232, O411757177572 /, SC/987/
c$$$C
...
c$$$C      DATA RIGHT(1),RIGHT(2) /       9344,           0 /
c$$$C      DATA DIVER(1),DIVER(2) /       9472,           0 /
c$$$C      DATA LOG10(1),LOG10(2) /  546979738,  -805796613 /, SC/987/
c$$$C
...
c$$$C      DATA LOG10(1),LOG10(2) / Z209A3F9A, ZCFF884FB /, SC/987/
c$$$C
c$$$C     MACHINE CONSTANTS FOR VAX/VMS VERSION 2.2
c$$$C
c$$$C      DATA SMALL(1),SMALL(2) /       '80'X,        '0'X /
c$$$C      DATA LARGE(1),LARGE(2) / 'FFFF7FFF'X, 'FFFFFFFF'X /
c$$$C      DATA RIGHT(1),RIGHT(2) /     '2480'X,        '0'X /
c$$$C      DATA DIVER(1),DIVER(2) /     '2500'X,        '0'X /
c$$$C      DATA LOG10(1),LOG10(2) / '209A3F9A'X, 'CFF884FB'X /, SC/987/
c$$$C
c$$$C  ***  ISSUE STOP 779 IF ALL DATA STATEMENTS ARE COMMENTED...
c$$$      IF (SC .NE. 987) THEN
c$$$         DMACH(1) = 1.D13
...
c$$$            LOG10(2) = 1352628735
c$$$         ELSE
c$$$               WRITE(*,*)'Adjust GAU2_D1MACH by uncommenting'
...
c$$$         END IF
c$$$C
c$$$C  ***  ISSUE STOP 778 IF ALL DATA STATEMENTS ARE OBVIOUSLY WRONG...
...
c$$$         WRITE(*,*) 'GAU2_D1MACH(I): I =',I,' is out of bounds.'
c$$$         STOP
c$$$         END IF
c$$$      GAU2_D1MACH = DMACH(I)
c$$$      RETURN
c$$$C
...
C/
C
      DOUBLE PRECISION GAU2_D1MACH, ZERO
      EXTERNAL GAU2_D1MACH
      DATA BIG/0.D+0/, ETA/0.D+0/, MACHEP/0.D+0/, ZERO/0.D+0/
      IF (BIG .GT. ZERO) GO TO 1
         BIG = GAU2_D1MACH(2)
         ETA = GAU2_D1MACH(1)
         MACHEP = GAU2_D1MACH(4)
 1    CONTINUE
C
C-------------------------------  BODY  --------------------------------
C
...
C  ***  LAST CARD OF DV7SWP FOLLOWS  ***
      END
      INTEGER FUNCTION GAU2_I1MACH(I)
C
C  I/O UNIT NUMBERS.
C
...
*               *** IBM MAINFRAME ***
               ...
               IMACH(16) = 63
            ELSE
               WRITE(*,*)'Adjust autodoubled GAU2_I1MACH by '
     :              //'uncommenting'
...
*               *** IBM ***
               ...
               SANITY = 987
            ELSE
               WRITE(*,*)'Adjust GAU2_I1MACH by uncommenting'
               WRITE(*,*)'data statements appropriate for your machine.'
               STOP 777
               END IF
            END IF
...
C/6S
C/7S
C      IF (I .EQ. 6) GAU2_I1MACH = 1
C/
      RETURN
C
 30   WRITE(*,*) 'GAU2_I1MACH(I): I =',I,' is out of bounds.'
C
*     CALL FDUMP
C
      STOP
...
C  +++  PORT VERSION FOLLOWS...
      INTEGER GAU2_I1MACH
      EXTERNAL GAU2_I1MACH
      INTEGER MDPERM(3)
      DATA MDPERM(1)/2/, MDPERM(2)/4/, MDPERM(3)/1/
      GAU2_I7MDCN = GAU2_I1MACH(MDPERM(K))
