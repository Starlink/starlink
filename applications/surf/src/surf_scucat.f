...
*    Local variables:
      ...
      CHARACTER*40  FILENAME (MAX_FILE) ! names of input files read
...
*  read the name of the file to be read
         ...
         IF (FILENAME(FILE) .EQ. 'end') THEN
            LOOPING = .FALSE.
            READING = .FALSE.
         ELSE
*  Try to open the HDS file and find out how many components are there

         CALL HDS_OPEN(FILENAME(FILE), 'READ', LOC, STATUS) 
...
* Have I found a photometry observation
         IF (N_PHOT .GT. 1) THEN
            ...
            CALL MSG_OUT(' ','Found data for the following bolometers:',
     :           STATUS)
...
* Only one, so I know what to do
         ELSE IF (N_PHOT.EQ.1) THEN
            ...
            CALL MSG_OUT(' ','Contains data for bolometer ^BOL', STATUS)
