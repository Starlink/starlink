...
#define NDG__OK     0
#define NDG__WNMF 101         
#define NDG__WPER 102
#define NDG__WLER 103
#define NDG__WMER 104
...
*     processed by the NDF library. Shell meta-characters can be included
...
*     a blank string as the file name and an error code (NDG__WNMF)
*     as the function value. Finally, a call to NDG1_EWILD with the
*     last returned value of Context will close down any files opened or
*     memory allocated by NDG1_WILD.

*  Language:
*     C, designed to be called from Fortran.

*  Invocation:
*     STATUS = NDG1_WILD( FILESPEC, FILENAME, CONTEXT )
...
*  History:
*     26-AUG-1999 (DSB):
*        - First version, based on IRG_WILD by PWD. This version differs
*        from IRG_WILD in that no checking is done to ensure that the
*        returned files can be processed by the NDF library. This is left
*        to the calling routine, which should be able to do it faster than 
*        a shell script. Doing away with the shell script also reduces
*        the management burden. Also, this version allows shell
*        metacharacters recognised by tcsh etc to be included in the 
*        filename pattern.
*        - Also moved the system "wait" call from ndg1_wild to ndg1_ewild.
*        Previously, the pipe was becoming full if there were many files
*        to list, resulting in both processes hanging. With the wait in
*        ndg1_ewild, ndg1_wild can continue to read from the pipe, thus
*        making room for the child process to write more to the pipe.
*     8-OCT-1999 (DSB):
...
/*  And now we execute the command.  This will write the set of files to 
    ...
    listed. Note, the "-f" option causes the .cshrc file to be ignored.
    This is necessary because (for instance) some people have .cshrc
    files which cause things to be written to stdout. Any such output
    from .cshrc would get picked up and used as file names by this 
    routine. */
...
/* If no characters were read, set status */
         if( Ichar == 0 ) Status = NDG__WNMF;

/* If Status is bad exit while loop */
         if (Status != NDG__OK ) break;
...
F77_INTEGER_FUNCTION(ndg1_ewild)( POINTER(Context) ) {
/*
*  Name:
*     NDG1_EWILD

*  Purpose:
*     Terminate a sequence of NDG1_WILD calls.

*  Description:
*     This routine should be called after a sequence of calls to NDG1_WILD
