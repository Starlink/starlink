      SUBROUTINE NDG1_HCUT( LOC1, STR, LOC2, STATUS )
*+
*  Name:
*     NDG1_HCUT
...
*  Invocation:
*     CALL NDG1_HCUT( LOC1, STR, LOC2, STATUS )
...
*  an error if it does not.
     ...
     :          ( STR( L : L ) .NE. ')' ) ) THEN
         STATUS = NDF__BNDIN
         CALL MSG_SETC( 'SUBSET', STR( F : L ) )
         CALL DAT_MSG( 'OBJECT', LOC1 )
         CALL ERR_REP( 'NDG1_HCUT_BND1',
...
*  if '()' was specified) and parse the dimension bounds expression.
            ELSE
               IF ( STR( F : L ) .EQ. '()' ) THEN
                  CALL NDG1_PSHDE( ' ', NDIM, DIM, LBND, UBND, STATUS )
               ELSE
                  CALL NDG1_PSHDE( STR( F + 1 : L - 1 ),
