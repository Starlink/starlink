      SUBROUTINE SCULIB_GET_FITS_L (MAX_FITS, N_FITS, FITS, NAME,
     :  VALUE, STATUS)
*+
*  Name:
*     SCULIB_GET_FITS_L
...
*      KEYWORD=INTEGER           / this is a comment
...
*  Invocation:
*     CALL SCULIB_GET_FITS_L (MAX_FITS, N_FITS, FITS, NAME, VALUE,
*    :  STATUS)
...
*  array, or if an error has occured
     ...
     :          (I .EQ. N_FITS))      THEN
         ...
         IF (STATUS .NE. SAI__OK) THEN
            CALL ERR_REP (' ', 'SCULIB_GET_FITS_L: error decoding -',
     :        STATUS)
...
*  check that a value for the parameter was found
      ...
      IF (.NOT.FOUND) THEN
         ...
         IF (STATUS .EQ. SAI__OK) THEN
            ...
            CALL ERR_REP (' ', 'SCULIB_GET_FITS_L: failed to find '//
