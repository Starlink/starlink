      SUBROUTINE SCULIB_STATD(N_POS, CLIP, SCUDATA, SCUQUAL, BADBIT,
...
*  Arguments Given:
      ...
      REAL    CLIP
      BYTE    BADBIT
      DOUBLE PRECISION SCUDATA(N_POS)
      BYTE    SCUQUAL(N_POS)

*  Arguments Returned:
      DOUBLE PRECISION QSORT(N_POS)
...
*  Local Variables:
      ...
      DOUBLE PRECISION LLIMIT      ! Lower limit of clip
      ...
      DOUBLE PRECISION ULIMIT      ! Upper limit of clip
...
*     Median
         ...
         IF (MOD(NGOOD, 2) .EQ. 0) THEN    ! Even
            MEDIAN = (QSORT(MIDPOINT) + QSORT(MIDPOINT+1))/2.0D0
         ELSE    ! Odd
            MEDIAN = QSORT(MIDPOINT+1)
         END IF
...
*     Now loop through and clip points above CLIP level
     ...
     :              STDEV.GT.0.0D0) THEN
                  ...
                  NREM = 0
                  ULIMIT =  MEAN + (DBLE(CLIP) * STDEV)
                  LLIMIT =  MEAN - (DBLE(CLIP) * STDEV)

                  DO I = 1, NGOOD
                     IF (QSORT(I) .NE. VAL__BADD .AND.
