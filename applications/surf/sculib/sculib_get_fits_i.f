      SUBROUTINE SCULIB_GET_FITS_I (MAX_FITS, N_FITS, FITS, NAME,
     :  VALUE, STATUS)
*+
*  Name:
*     SCULIB_GET_FITS_I
...
*  Invocation:
*     CALL SCULIB_GET_FITS_I (MAX_FITS, N_FITS, FITS, NAME, VALUE,
*    :  STATUS)
...
*  a /
     ...
     :                   (JPOS .EQ. IPOS+1)) THEN
                     ...
                     ELSE
                        ...
                        CALL CHR_CTOI (STEMP, VALUE, STATUS)
                     END IF
                  END IF
               END IF
            END IF
...
*  array, or if an error has occured
     ...
     :          (I .EQ. N_FITS))      THEN
         ...
         IF (STATUS .NE. SAI__OK) THEN
            CALL ERR_REP (' ', 'SCULIB_GET_FITS_I: error decoding -',
     :        STATUS)
...
*  check that a value for the parameter was found
      ...
      IF (VALUE .EQ. VAL__BADI) THEN
         IF (STATUS .EQ. SAI__OK) THEN
            ...
            CALL ERR_REP (' ', 'SCULIB_GET_FITS_I: failed to find '//
