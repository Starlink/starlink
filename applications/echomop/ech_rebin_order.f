...
*+
*  Name:
*     ECHOMOP - ECH_REBIN_ORDER

*  Purpose:
*     Scrunch an object order.
...
*  Initialise output arrays.
      DO I = 1, NX
         ...
         WORK( I, 1 ) = DBLE( I )
      END DO
      CALL ECH_REPORT( 0, ' Generating 2-D wavelength scale.' )
      AXMIN( 1 ) = 0.0
      AXMAX( 1 ) = DBLE( NX )
...
*                    Add contribution from input pixel.
                        IF ( QUALITY( USE, YCOORD ) .GT. 0 )
     :                     GOOD = QUALITY( USE, YCOORD )
                        IF ( ( TWO_D( JOFF, USE + 1 ) -
...
*              Set output pixels according to current trace offset.
     ...
     :                     ( 1.0 - AYOFFSET ) * VN
                     IF ( GOOD .GT. 0 ) THEN
                        REBINNED_QUALITY( I, J ) = GOOD
                        REBINNED_QUALITY( I, J + 1 ) = GOOD
                     END IF
