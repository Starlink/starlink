      SUBROUTINE ECH_SAVE_DIPSTK( NDFNAM, ISTKNPT, TOP, XSTK, YSTACK,
     :           ATITLE, AIRTOVAC, STATUS )
*+
*-
      IMPLICIT NONE
      ...
      INTEGER ISTKNPT
      INTEGER TOP
      DOUBLE PRECISION XSTK( TOP * ISTKNPT )
      REAL YSTACK( TOP * ISTKNPT )
      CHARACTER*( * ) ATITLE
      LOGICAL AIRTOVAC
      ...
      INTEGER MBSTSZE
      PARAMETER ( MBSTSZE = 50 )

      REAL XSTACK( MAXSTK * 5000 )
      REAL WORVST( MAXSTK )

      INTEGER BSTACK( MBSTSZE )
...
*  Check that given stack name ends in '_STK'; if it does not then append
*  this string.
      NCHAR1 = ECH_WORD_LEN( NDFNAM )
      IPOSN = 1
      CALL CHR_FIND( NDFNAM, '_STK', .TRUE., IPOSN )
      IF ( IPOSN .GT. NCHAR1 ) THEN
         CALL CHR_APPND( '_STK', NDFNAM, NCHAR1 )
      END IF

*  First stack entry is first order.
      BOT = 1

*  Calculate size of complete "stacks".
      STKSZE = TOP * ISTKNPT

*  Number of stack entries.
      NONSTK = TOP

*  Fill per-entry items.
      DO I = BOT, TOP

*     Break points.
         BSTACK( I ) = ISTKNPT

*     Number of breaks in entry.
         BSTNPT( I ) = 1

*     Pointer to start of entry.
         POINTR( I ) = ( I - BOT ) * ISTKNPT + 1

*     Number of points in entry.
         STKNPT( I ) = ISTKNPT

*     Pointer to start of break points.
         BPOINT( I ) = I - BOT + 1

*     Title.
         CALL CHR_ITOC( I, REF_STR1, NCHAR1 )
         IPOSN = CHR_LEN( ATITLE ) - 3
         STITLE( I ) = ATITLE( : IPOSN ) // ' ' //
     :         REF_STR1( :NCHAR1 ) // '  '

*     Entry holds wavelengths.
         WORVST( I ) = 1.0
      END DO
...
*  Index of last used element in data.
      STKLST = TOP * ISTKNPT

*  Index of last used break point.
      BSTLST = TOP

*  Cast to REAL of XSTACK entries.
      DO I = 1, STKLST
         XSTACK( I ) = REAL( XSTK( I ) )
      END DO

*  If needed, convert XSTACK entries from air to vacuum wavelengths.
      IF ( AIRTOVAC ) THEN
         CALL VCHLCON( XSTACK, STKLST, 0, .FALSE., .TRUE. )
      END IF

*  Message.
      REPORT_STRING = ' Writing results to DIPSO stack ' //
     :      NDFNAM( : ECH_WORD_LEN( NDFNAM ) ) // '.'
      CALL ECH_REPORT( 0, REPORT_STRING )

*  Create the stack.
      CALL SAVSTK( NDFNAM, BOT, TOP, STKSZE, BSTSZE, MAXSTK,
