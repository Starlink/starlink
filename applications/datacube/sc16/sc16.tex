...
%  Copyright:
%     Copyright (C) 1997-2005 Particle Physics and Astronomy
...
prevailing seeing conditions.\latexonly{ More information can be found
...
available reduction software\latexonly{ (see
...
arcsec.\latexonly{ More information can be found at {\tt
...
package.\latexonly{ More information about the instrument can be found
...
\htmlref{data cube}{sc16_teifufile}\latexonly{ (see
...
\latexonly{Links to more information on the ongoing development of the
...
Allington-Smith}{mailto:J.R.Allington-Smith@durham.ac.uk}\latexonly{
({\tt J.R.Allington-Smith@durham.ac.uk})}.

\latexonly{More information on the technical specifications of the
...
the figure)
\end{htmlonly}
\latexonly{Figure~\ref{sc16_uist_fig})} which is used as the input for the
...
software}{http://monet.astro.uiuc.edu/adass98/Proceedings/economouf/}\latexonly{
...
standard data cube formats\latexonly{ (see
...
spectroscopic \htmlref{file format}{sc16_iraf}\latexonly{
...
format}{sc16_iraf}\latexonly{ (see Section~\ref{sc16_iraf})} which is
...
cube}{sc16_mef2cub}\latexonly{ (see Section~\ref{sc16_mef2cub})} or
...
desirable\latexonly{ (see Section~\ref{sc16_mef2cub})}.
...
Astronomy Observatories (NOAO)\latexonly{ at {\tt
...
The Starlink \CONVERTref\ package \latexonly{(see SUN/55)} can be used
...
earlier}{sc16_teifufile}\latexonly{ (see
...
package from GSFC.\latexonly{ Further information about \HEASOFT, along
...
\htmlref{document}{sc16_fitsheader}\latexonly{ (see
...
ability\latexonly{ (see SUN/55 for more details)} and the \xref{{\tt
...
\latexonly{Detailed documentation for all of these tasks can be found
in SUN/95.}
...
be used to produce \htmlref{white-light and passband}{sc16_vis} images\latexonly{ (see 
...
\htmlref{arbitrary cube sections}{sc16_vis} \latexonly{ (again see Section~\ref{sc16_vis} for
details)}.
...
Replaces all occurrences of \xref{magic value}{sun95}{se_masking}\latexonly{(see SUN/95 for details)} 
...
created}{sc16_newndf} NDF\latexonly{ (see Section~\ref{sc16_newndf})},
...
Combined with the \htmlref{graphics devices commands}{sc16_graphics}\latexonly{ (see
...
involved in doing so\latexonly{ (see Section~\ref{sc16_mos})}, however,
...
\latexonly{(see SUN/86 for details)} of the NDF where it is available
...
\end{htmlonly} 
\latexonly{See Section~\ref{sc16_wcs2axis} for an example}
...
are available \latexonly{in SUN/95 {\em Using World Co-ordinate
Systems}}.
...
explained \htmlref{later}{sc16_mos}\latexonly{ in Section~\ref{sc16_mos}}.
...
the figure below)
\end{htmlonly}
\latexonly{Figure~\ref{sc16_gaia_cubetool_fig})} offering a choice of 
axis to step along or collapse, and a selection of methods.  You can
select a plane by giving its index or adjust the slider.  This could,
for instance, let you define lower and upper limits of interest that
are applied to the animation, or passband image (Collapse tab), tiled
image of several passbands (Chanmap tab). The chosen limits apply to
subsequent steps. If you need to access the Cube toolbox, go to the
{\tt Open cube...} option in the {\tt File menu}.

\myfig{sc16_gaia_cubetool.eps}{height=0.45\textheight}{sc16_gaia_cubetool.gif}{sc16_gaia_cubetool_fig}{The
\GAIA\ toolbox for displaying a cube.}

Once an image, be it a plane, passband image or map, you can use
\GAIA's wide selection of 
\xref{image-display facilities}{sun214}{image_display_capabilities}
\latexonly{described in SUN/214} (also see \xref{SC/17}{sc17}{}) to enhance 
the elements under investigation.  There are also many \xref{analysis
capabilities}{sun214}{image_analysis_capabilities} mostly through the
({\tt Image-Analysis} menu), including masking and flux measurement.

For cubes the main additional feature (at the time of writing) is a
dynamic spectrum display.  Once you have a representative image 
displayed, if you click on the mouse over the image a line plot along
the collapsed axis appears.  That will normally be a spectrum.
As you drag the cursor holding down the first mouse button, the
spectrum display updates to dynamically to reflect the current spatial
location.  If you single click, the scaling limis adjust for the current
pixel.  If you double-click at a location \GAIA\ creates a reference
spectrum.  Then as you subsequently drag the mouse you can compare any
of the spectra with the reference spectrum.   See 
\begin{htmlonly}
the figure below.
\end{htmlonly}
\latexonly{Figure~\ref{sc16_gaia_spectrum_fig}.}

\myfig{sc16_gaia_spectrum.eps}{height=0.7\textheight}{sc16_gaia_spectrum.gif}{sc16_gaia_spectrum_fig}{\GAIA\ 
displays a logarithmically scaled, collapsed cube, from which
two spectra are shown.  The green reference spectrum is from the spatial position
marked by the green cross, and the other spectrum corresponds to the
blue cross.}
...
the figures.
\end{htmlonly}
\latexonly{Figures~\ref{sc16_slicer3_cut_fig} and
...
fairly well
\htmladdnormallink{documented}{http://idlastro.gsfc.nasa.gov/contents.html}.\latexonly{
...
are available online\latexonly{ at {\tt
...
the figure).
\end{htmlonly}
\latexonly{Figure~\ref{sc16_squash_fig}).}
...
Once created you may view the passband image with \GAIAref\ or
\xref{{\bf display}}{sun95}{DISPLAY}.  To read off the
three-dimensional co-ordinates of features, the latter requires 
\xref{{\bf display}}{sun95}{CURSOR}.
...
the figure.
\end{htmlonly}
\latexonly{Figure~\ref{sc16_ripper_fig}.}
...
the figure.
\end{htmlonly}
\latexonly{Figure~\ref{sc16_compare_fig}.}
...
(see the figure),
\end{htmlonly}
\latexonly{(Figure~\ref{sc16_compare_fig}),} pressing the right-hand mouse
...
the figure)
\end{htmlonly}
\latexonly{Figure~\ref{sc16_stacker_fig})} and \xref{{\bf
...
the figure.
\end{htmlonly}
\latexonly{Figure~\ref{sc16_gridspec_fig}.}  Below is an
...
\paragraph{\label{sc16_velfit}\latexonly{\vspace*{0.8em}}By
...
the figure.
\end{htmlonly}
\latexonly{Figure~\ref{sc16_velmap_fig}.}  Below is an
example.
...
the figure.
\end{htmlonly}
\latexonly{Figure~\ref{sc16_velmoment_fig}.}  Below is an
example.
...
discussed \latexonly{in Section~\ref{sc16_velmap}, }
\begin{htmlonly}
...
the figure.
\end{htmlonly}
\latexonly{Figure~\ref{sc16_peakmap_fig}.}  Much like the {\bf velmap} script the
...
SpecFrame}}{sun95}{se_wcsuse}{\latexonly{ in SUN/95}}.
...
the figure).
\end{htmlonly}
\latexonly{Figure~\ref{sc16_mosaic_fig}).}  It should be noted that {\bf makemos} pays
...
the figures.
\end{htmlonly}
\latexonly{Figures~\ref{sc16_xvelmap_fig} and \ref{sc16_xvelmap2_fig}.}
\latexonly{More information about {\bf XDialog} can be found at
...
In compiling this document I have again leant heavily on already
