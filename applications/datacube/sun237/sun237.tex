...
\newcommand{\stardocdate}      {2005 October 20}
...
\sstroutine{
   ...
   \sstdescription{
      This shell script sits onto of a collection of A-tasks from the
      \xref{KAPPA}{sun95}{}, and \xref{FIGARO}{sun86}{} packages.  It
...
\sstroutine{
   ...
   \sstdescription{
      This shell script sits onto of a collection of A-tasks from the
      \xref{KAPPA}{sun95}{} package.  It reads a three-dimensional IFU
...
\sstroutine{
   ...
   \sstdescription{
      ...
      \xref{KAPPA}{sun95}{} and \xref{FIGARO}{sun86}{} packages.  It
      reads a three-dimensional IFU NDF as input and presents you with
      a white-light image of the cube.  You can then select and $X$-$Y$
      position using the cursor.  The script will extract and display
      this spectrum.  You will then be prompted to specify various
      fitting parameters, such as the peak position, using the cursor.
      The script will then attempt to fit the emission line.  The fit
      will be displayed and you are consulted regarding the goodness
      of fit.  If you consider the fit good enough, the script will
      attempt to perform similar fits to all spectra within the cube,
      building a two-dimensional NDF image of the velocity of the
      line.
...
\item The scripts have been generalised to other spectral co-ordinate
systems.  Units are reported too.
...
\item Additional -c option to specify the number of contours added
to \htmlref{{\bf peakmap}}{peakmap} and \htmlref{{\bf velmap}}{velmap}.
...
\item \htmlref{{\bf passband}}{passband} does not dump WCS mappings in NDFCOPY.
Ensured that the plots of spectra use the axis co-ordinate system.
Removed unused -r option.
...
WCSTRAN.  Added the -s option to define the co-ordinate system of the
