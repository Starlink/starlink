...
*     a set of points in an arbitrary number of dimensions. 
...
*     IN = LITERAL (Read)
*        A 2-dimensional NDF holding the positions to be transformed. The 
*        DATA array of this NDF is interpreted  as a table in which each 
*        column of pixels holds values for a specified quantity, some of 
*        which are the axis values at the positions to be transformed. 
*        Each row of pixels corresponds to a separate position. The columns
*        holding the axis values are specified using parameter INCOLS.
*     INCOLS() = INTEGER (Read)
*        A set of distinct column indices within the NDF specified by
*        parameter IN. These should identify the columns holding
...
*     OUT = LITERAL (Read)
*        A 2-dimensional NDF to receive the transformed positions. The 
*        DATA array of this NDF is interpreted  as a table in which each 
*        column of pixels holds values for a specified quantity, some of 
*        which are the axis values at the transformed positions. Each row
*        of pixels corresponds to a separate position. The columns
*        to receive the transformed axis values are specified using parameter 
*        OUTCOLS. The output NDF is formed by taking a copy of the input
*        NDF, and then expanding its bounds to accomodate any extra columns 
*        specified by parameter OUTCOLS (any such extra columns are
*        initialized to hold bad values). The initial values for the columns 
*        specified by parameter OUTCOLS are then over-written with the 
*        transformed axis values.
*     OUTCOLS() = INTEGER (Read)
*        A set of distinct column indices within the NDF specified by
*        parameter OUT. These should identify the columns in which
*        the transformed axis values should be stored, in the order
*        produced by the Mapping (see parameter FORWARD). There is no 
*        restriction on the values which may be supplied (the output NDF 
*        will be expanded to accomodate all supplied column indices).
*        If the number of input and output axes required by the Mapping
*        are equal, the run-time default is to use the same columns as
*        those used for parameter INCOLS. If the number of input and 
*        output axes are different, there is no run-time default and the
*        user is prompted. []
*     THIS = LITERAL (Read)
...
*  NAXIN.
     ...
     :                 'the supplied Mapping requires ^NI input axes '//
     :                 'but the input NDF ''^NDF'' only has ^NC '//
     :                 'columns.', STATUS )
         GO TO 999
      END IF
...
*  the defaults set up above.
         CALL PAR_GDR1I( 'INCOLS', NAXIN, INCOL, LBND( 1 ), UBND( 1 ), 
     :                    .TRUE., INCOL, STATUS)
