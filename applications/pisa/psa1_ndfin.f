      SUBROUTINE PSA1_NDFIN( IPOINT, NX, NY, ORIGX, ORIGY, STATUS )
*+
*  Name:
*     PSA1_NDFIN

*  Purpose:
*     To access and map in a NDF.

*  Language:
*     Starlink Fortran 77

*  Invocation:
*     CALL PSA1_NDFIN( IPOINT, NX, NY, ORIGX, ORIGY, STATUS )

*  Description:
*     The routine accesses a _WORD 2D NDF. If required the input can be
...
*  Warn users if input is NOT INTEGER*2
      ...
      IF ( NTYPE .NE. '_WORD' ) THEN
         CALL MSG_SETC( 'NTYPE', NTYPE )
         CALL MSG_OUT( ' ',
     :' Input NDF is of type ^NTYPE - this application can only'//
     :' process at _WORD precision; significance may be lost', STATUS )
      END IF
...
*   Now Map in the data
      CALL NDF_MAP( IDINS, 'Data', '_WORD', 'READ', IPOINT, NEL,
     :              STATUS )

*  Check for BAD pixels - cannot process these.
      CALL NDF_BAD( IDINS, 'Data', .TRUE., LBAD, STATUS )
      IF ( LBAD ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'HAS_BAD_PIX ',
     :' The input NDF contains "bad" pixels or values outside the'//
     :' range of _WORD, these cannot be correctly handled by this'//
     :' application.', STATUS )
      END IF
