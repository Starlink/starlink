      SUBROUTINE AIF_ABSET( SEPAR, OPTS, ARRAY, NELM, MCH, MINCH,
     :                      MAXNOC, STATUS )
*+
*  Name:
*     AIF_ABSET
...
*  Invocation:
*     CALL AIF_ABSET( SEPAR, OPTS, ARRAY, NELM, MINCH, MAXNOC, STATUS )
...
*        Original version.
*     1991 May 23 (MJC):
*        Fixed bug in extracting the last value (under certain 
*        circumstances) from the options.
*     1991 June 24 (MJC):
*        Made to cope with the special case when an option in full
*        equals the start of another option.
*     1991 September 12 (MJC):
*        Corrected last bug fix which was not quite correct. 
...
*         Extract the item, allowing for the offset within the list.
            ...
            MAXNOC = MAX( CHR_LEN( ARRAY( NELM )( :NC - LASTC + 1 ) ),
     :               MAXNOC )

        ELSE

*          Find the next separator in the substring that follows all the
*          items previously extracted from the input list.  Note the
*          sub-string must be used as the routine returns the column
*          number of the first occurrence of the separator.
...
*          Watch for the special case when the separator terminates the
*          list.
...
*       O(n!) to O(n).
         ...
         CALL AIF_SORT1C( NELM, ARRAY, STATUS )
...
*       sufficient, but this is a bizarre use of this routine, and most
