...
intensity thresholds. These intensities are consequently different for
each object. Traditionally this has not been the case and isophotal
areas/radii have been measured at fixed magnitude intervals above some
threshold. Consequently, for convenience and compatibility with
existing data and analysis methods, the \EXTRACTOR\ and Starlink
...
the isophotal radii estimates are:
\begin{itemize}
  \item \texttt{RAD\_TYPE}
  \item \texttt{RAD\_THRESH}
\end{itemize}
\texttt{RAD\_TYPE} can be either \texttt{SB} or
...
the first level and \texttt{zp} the data zero point, all in magnitudes
per square arcsecond. If only one value is given then the starting
point is assumed to be the analysis threshold and the zero point is
derived from the photometric value (\texttt{MAG\_ZEROPOINT}).
...
\end{quote}
where $I_{i}$ = threshold for $i$th level, $I_{t}$ = measurement
