...
intensities. These intensities are consequently different for each
object. Traditionally this has not been the case and isophotal
areas/radii have been measured at fixed intervals above some
threshold. Consequently, for convenience and compatibility with
existing data and analysis methods, the \EXTRACTOR\ and \SExtractor\
programs have been extended to provide a flexible configuration scheme
that allows this traditional behaviour to be recovered and extended.

The configuration options that control what intensities are used for
the isophotal radii estimates are \texttt{RAD\_TYPE} and 
\texttt{RAD\_THRESH}. \texttt{RAD\_TYPE} can be either \texttt{SB} or
\texttt{INT}, which indicate that the levels will be defined in terms
of surface brightnesses or plain intensities, respectively.

The \texttt{RAD\_THRESH} option can have up to three values qualifying
values, depending on the value of \texttt{RAD\_TYPE}. If
\texttt{RAD\_TYPE} is \texttt{SB} then you should enter a line
consisting of:
\begin{quote}
   \texttt{RAD\_THRESH        step[,start,zp]}
\end{quote}
in your \texttt{default.sex} file. The value \texttt{step} being the
required interval between levels, \texttt{start} being the value of
the first level and \texttt{zp} the data zero point (all in magnitudes per
square arcsecond). If only one value is given then the starting point
is assumed to be the analysis threshold and the zero point is derived
from the photometric value (\texttt{MAG\_ZEROPOINT}).

If a \texttt{RAD\_THRESH} value is not given (and any of the catalogue
parameters \texttt{RAD0} -- \texttt{RAD15} are requested) then a
default step of $0.75$ magnitudes per square arcsec is used.

If \texttt{RAD\_TYPE} is \texttt{INT} then the correct line format is:
\begin{quote}
   \texttt{RAD\_THRESH        step[,start]}
\end{quote}
\texttt{step} being the interval between levels in plain magnitudes
and \texttt{start} being the intensity of the first level. If
...
\begin{quote}
    $I_{i} = I_{t} * 2^{(i+2)},\ i = 2, 16$
\end{quote}
where $I_{i}$ = threshold for ith level, $I_{t}$ = measurement
threshold. This gives approx $0.75$ magnitude steps ($2.5*log(2)$).

The actual measurements are associated with the catalogue parameters,
\texttt{RAD0} through \texttt{RAD15}, these must be present in the
\texttt{default.param} file.
...
profile. Under this scheme each level is:
\begin{quote}
    $I_{i} = I_{t} * (I_{p}/I_{t})^{i/8}, \ i=1,8$
\end{quote}
where $I_{p}$ = peak intensity and $I_{t}$ and $I_{i}$ are as above.
So you get a range of levels for each object spanning the range from
its peak intensity down to the threshold.
...
\subsection{Version 1.2-0}
 ...
 significant changes:
 \begin{itemize}
   \item The version of \SExtractor\ has been updated to version 2.1.6. 
 ...
 \texttt{RAD\_THRESH} and \texttt{RAD\_TYPE}. These allow for up to 
 $16$ levels specified by the \texttt{RAD0} through \texttt{RAD15} 
 parameters.
