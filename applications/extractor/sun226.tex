...
\newcommand{\stardocauthors}   {A.J. Chipperfield\\
                                P.W. Draper}
\newcommand{\stardocdate}      {6 January 1999}
...
solutions, as well as FITS-WCS and AST native ones (native
\SExtractor\ will only use images that have FITS-WCS solutions). The
GAIA display tool is a convenient way to add such astrometrical
calibrations.

One current limitation is that no support is provided for the use of
NDF variance arrays (although these would probably map onto the
``weight'' image concept of \SExtractor).
...
extensively in the \EXTRACTOR\ incarnation, so it is quite probable
that problems may arise. 

In particular some areas of \SExtractor\ that have not been
implemented are:
\begin{itemize}
 \item The propagation of the input NDF to the check image (so the WCS
       information, for instance, is lost).
 \item The piping of catalogues to standard output.
\end{itemize}
The use of weight and flag images is not supported.

If you come across any problems with \EXTRACTOR\ please notify the
authors.
