...
held in a temporary storage file MAPPLANE.TMP\index{\verb+MAPPLANE.TMP+}, and
the last version is not deleted after use. This is a simple binary file, with
record length equal to the number of points in a row, which can be read or
mapped to an array in another program or read directly by the IMAGE
\index{Image (MONGO command)} command in Berkeley/MRAO MONGO\index{MONGO}. I
have recently also managed to read MAPPLANE.TMP directly into the
...
constants\index{constants>logical} TRUE\index{\verb+TRUE+} \&
FALSE\index{\verb+FALSE+}, are also predefined. 
...
\index{-}\index{^}\index{*}\index{/}\index{+}\index{minus>unary}
\begin{center}
...
\index{operators>relational}\index{\verb$=$}\index{\verb$<>$}\index{\verb$><$}
...
NOT\index{\verb+~+ (NOT)} (\verb+~+) and arithmetic unary minus\index{minus>unary}
(\verb+-+) are the same thing, but I do not guarantee that this will continue,
and in any case, expressions are easier to use if you distinguish between the
two uses. The two binary operators (\&\index{\& (AND)} and !\index{! (OR)})
...
STACKCOMM.INC and FLAGCOMM.INC (see appendix F.4 for further details of these
include files), and have a single formal parameter which
is an error return. Such subroutines can be linked easily to the program,
and provide a relatively easy access to user-defined functions. In the
near future I think I will define the stack area as a global section, so
that user {\em programs} can be invoked using the facility for calling
VMS commands in the command line.
...
(character) value. 
\index{\verb+X_NAME+}\index{\verb+X_UNITS+}\index{\verb+Y_NAME+}
\index{\verb+Y_UNITS+}
...
observing frequency or the vlsr (using VLSR \index{\verb+VLSR+}= nn.mm for example).
...
\verb+FRQCOEFF+\index{\verb+FRQCOEFF+} and can also be turned on and off by
setting the logical variable \verb+FCORRECT+.\index{\verb+FCORRECT+} If
...
not indicate a preferred or `centre' channel (IQCEN=0) \index{\verb+IQCEN+} then
...
`centre' quadrant as defined by the header variable IQCEN. \index{\verb+IQCEN+}
...
\index{mapplane.tmp} and the last version of this is retained after a plot is
...
Relevant flags:\\
\begin{tabular}{lll}\index{\verb+REPLACE+}\index{\verb+MAP_TOL+}
...
Relevant flags:\\
\index{\verb+INTERACTIVE+}\index{\verb+LINTYPE_POS+}\index{\verb+LINTYP_NEG+}
\index{\verb+LINTYPE_ZERO+}\index{\verb+NCONT+}\index{\verb+CONTOUR_0+}
\index{\verb+CONTOUR_INT+}\index{\verb+NCSET+}\index{\verb+CONTOUR_LEVS+}
\index{\verb+LINE_WEIGHT+}
\index{\verb+AUTOGREY+}
\index{\verb+GREYLIM+}
\index{\verb+COLOUR_TABLE+}
\index{\verb+OVERLAY_CONTOURS+}
\index{\verb+PLOT_GREY+}
\index{\verb+PLOT_CONT+}
\begin{tabular}{lll}
...
\index{\verb+EXTRACT-QUADRANT+}, in that the spectra so formed can be put back
...
Relevant flags:\\
\index{\verb+N_POLY+}
\index{\verb+POLY_COEFF+}
\index{\verb+N_SIN+}
\index{\verb+AMP_PHASE+}
\index{\verb+FREQ_COEFF+}
\index{\verb+N_INTS+}
\index{\verb+BLF_INTS+}
\index{\verb+FIT_TOL+}
\index{\verb+MAX_ITS+}
\index{\verb+FIT_DEBUG+}
\begin{tabular}{lll}
...
Relevant flags:\\
\index{\verb+NO_SPECTRA+>returned by INDEX-FILE}
\begin{tabular}{lll}
...
Relevant flags:\\
\index{\verb+NO_GSD_SPECTRA+>returned by INDEX-GSD-FILE}
\begin{tabular}{lll}
...
Relevant flags:\\
\index{\verb+BEAM_WIDTH+}
\index{\verb+BEAM_EXTENT+}
\index{\verb+VEL_WIDTH+}
\index{\verb+VEL_EXTENT+}
\index{\verb+CUBE_INTERPOLAT+}
\begin{tabular}{lll}
...
Relevant flags:\\
\index{\verb+CUBE_LOADED+}
\index{\verb+CUBE_SIZE+}
\begin{tabular}{lll}
...
Relevant flags:\\
\index{\verb+INTERACTIVE+}
\index{\verb+LINTYP_POS+}
\index{\verb+LINTYP_NEG+}
\index{\verb+LINTYP_ZERO+}
\index{\verb+LINE_WEIGHT+}
\index{\verb+NCONT+}
\index{\verb+AUTOGREY+}
\index{\verb+GREYLIM+}
...
Relevant flags:\\
\index{\verb+NO_NEW_SPECTRA+>returned by READ-GSD-DATA}
\begin{tabular}{lll}
...
Relevant flags:\\
\index{\verb+NO_SPECTRA+>returned by READ-SPECTRUM}
\begin{tabular}{lll}
  \verb+NO_SPECTRA+ &I4 & Number of spectra in file just read.
\end{tabular}
...
scale is flagged as non-linear (\verb+FCORRECT = T+) \index{\verb+FCORRECT+}
...
Relevant flags:\\
\index{\verb+NO_SPECTRA+>updated by REWRITE-SPECTRUM}
\begin{tabular}{lll}
...
Relevant flags:\\
\index{\verb+POSN_ANGLE+}\index{\verb+CUBE_ROTATED+}
\begin{tabular}{lll}
...
\index{\verb+AUTO_CONT+}
\index{\verb+NCONT+}
\index{\verb+CONTOUR_0+}
\index{\verb+CONTOUR_INT+}
\index{\verb+NCSET+}
\index{\verb+CONTOUR_LEVS+}
\begin{tabular}{lll}
...
\index{\verb+OVERLAY_CONTOURS+}
\index{\verb+PLOT_GREY+}
\index{\verb+COLOR5_START+}
\index{\verb+COLOR5_ROTATE+}
\index{\verb+COLOR5_EXPONENT+}
\begin{tabular}{lll}
   \verb+AUTOGREY+           & L4 & Set greyscale limits automatically from map
                                    max and min.\\
   \verb+GREYLIM(2)+         & R4 & Levels represented by black (min) and white (max)\\
...
Relevant flags:\\
\index{\verb+VELOCITY_FRAME+}
\index{\verb+VELOCITY_LAW+}
\index{\verb+VELOCITY+}
\begin{tabular}{lll}
...
The definition of the scan header variables, and their position within
the common block, are made within an include file STACKCOMM.INC A
simple way to read your own particular format of data into SPECX is
therefore to include this file into an EXTERNAL routine of your own
composition. By filling in the data array, and the ancilliary header
information in the COMMON block, you will make your spectrum available
for processing (or storage) by the rest of SPECX. 
