C-----------------------------------------------------------------
      ...
      SUBROUTINE SPECX_GSD_V52_DATA (INDEX, IRX, IERR)
...
C     Find indices to all other arrays
     ...
     &               NO, UNITS, TYPE, ARRAY, IND_FR, STATUS)
      CALL GSD_FIND (IFD, 'C3BEFENULO',
     &               NO, UNITS, TYPE, ARRAY, IND_LO, STATUS)
      CALL GSD_FIND (IFD, 'C3BETOTIF',
     &               NO, UNITS, TYPE, ARRAY, IND_IF, STATUS)
...
C              all spectra )
     ...
     :           ITSKY(NQUAD), LVAL, STATUS)
               CALL GSD_GET1D (IND_LO, 1, NRC, QUAD, QUAD,
     :           LOFREQ(NQUAD), LVAL, STATUS)
               CALL GSD_GET1D (IND_IF, 1, NRC, QUAD, QUAD,
     :           IFFREQ(NQUAD), LVAL, STATUS)
     ...
     :           CF, LVAL, STATUS)
               ...
               IF (BACKEND.eq.'DAS') THEN
                 NPTS(NQUAD)   = NO_BES_O_CH(QUAD) - 1
               ELSE
...
C              Take one less channel for the DAS to get header right

               IF (BACKEND.eq.'DAS') IENDS(1) = IENDS(1) - 1
...
C     Print out a picture of the stack with new spectra showing...
      ...
      NBYTES = 4*LSTK
      CALL IGETVM  (NBYTES, .TRUE., 'SPECX_GSD_V52_DATA', IPTR)
