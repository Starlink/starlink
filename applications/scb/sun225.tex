...
and it is perfectly possible to learn to use it by playing around
with it, perhaps with reference to section \ref{BROWSER-HELP}.
...
Renaming the script is also permissible.
...
In order for the browser to work, indices of the source files must be
...
being indexed (the package name is given before the `\#' sign on the 
...
Copying the indices should be done using the supplied utility
...
\end{verbatim}
\end{quote}
It would be possible to make personal index of Figaro only,
in which case the {\tt scbcp.pl} step could be avoided, 
...
the indices, in the same way as during the index build as described
above.
...
Thus, given that there is no {\tt help.f} in CCDPACK,
\begin{quote}
\begin{verbatim}
...
\subsection{\xlabel{sec:internals-indices}\label{sec:internals-indices}Index files}

The indexing program builds disk-based indices, and the browser program
interrogates them.  There are three indices:
\begin{description}
\item[Tasks]
...
The File and Routine indices are
...
It is possible to examine the File and Routines indices 
...
As well as the principle programs in the package 
...
in which the File and Routine indices are kept 
(see section \ref{sec:internals-indices}.
...
been written by {\tt scbindex.pl} (see section \ref{sec:internals-indices})
...
\end{verbatim}
Thus to create a private copy of the SCB indices, for instance
