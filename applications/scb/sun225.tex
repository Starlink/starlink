...
\subsection{\xlabel{sec:indexing}\label{sec:indexing}Generating the index}
...
\subsubsection{\xlabel{sec:indexing-all}\label{sec:indexing-all}Indexing the whole Starlink collection}
...
\subsubsection{\xlabel{sec:personalise}\label{sec:personalise}Creating a personalised index}
...
If a full index already existed in {\tt /star/etc/scb}, 
then the first invocation of {\tt scbindex.pl} could be replaced
by {\tt cp~/star/etc/scb/*~\$SCB\_INDEX}.
...
\xlabel{BROWSER-HELP}\label{BROWSER-HELP}
