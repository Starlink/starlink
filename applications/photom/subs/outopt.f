...
*  Arguments:
*     FOUT = INTEGER (Given)
*        Identifier for the results file used by FIO_.
*     INDEX = INTEGER (Given)
*        Index number of the object.
*     XCEN = REAL (Given)
*        X centre of the aperture in pixel coordinates.
*     YCEN = REAL (Given)
*        Y centre of the aperture in pixel coordinates.
*     ORIGIN( 2 ) = INTEGER (Given)
*        The origin of the NDF axes.
*     LPADU = REAL (Given)
*        Photons per data unit.
*     LSTAR = REAL (Given)
*        Integrated signal in the object aperture.
*     LAREA = REAL (Given)
*        Area of the object aperture.
*     LVSTAR = REAL (Given)
*        The data variance in the object aperture.
*     LSKY = REAL (Given)
*        Sky value per pixel.
*     LSKYARE = REAL (Given)
*        Area of the sky aperture in pixels.
*     LSIGMA = REAL (Given)
*        Standard deviation in the sky aperture.
*     LVSKY = REAL (Given)
*        Data variance per pixel in the sky aperture.
*     MAGS = LOGICAL (Given)
*        If TRUE then results are converted into magnitudes.
*     LSKYMAG = REAL (Given)
*        Magnitude of the sky when MAGS is TRUE.
*     PHOTON = INTEGER (Given)
*        How the errors are to be determined.  Allowed values are
*        1: photon noise, 2: sky variance, 3: data variance,
*        4: gaussian sky.
*     LBIASLE = REAL (Given)
*        Zero point for the photon noise calculation per pixel.
...
*  the signal and error, but we still need to put the result into
*  electrons.
      SIGNAL = PADU * STAR
      ERRSIG = PADU * VSTAR
