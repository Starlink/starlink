...
*        Flag to perform optimal extraction using Tim Naylor's
*        algorithm.
...
*        Allow for two altered subroutine argument lists.
*        Tidied, including removal of unused variables and tabs, and
*        added commentary.
...
*  Local Variables:
      ...
      CHARACTER * ( VAL__SZD ) APRWRD( 12 ) ! Object aperture info.
      ...
      LOGICAL CUTOFF             ! Does the aperture touch the edge?
      ...
      REAL CLIP                  ! Clipping radius
      REAL DPOS                  ! Centroiding radius of opt. extraction
      ...
      REAL MAJOR                 ! Length of semi-major axis
      REAL MAJSKY                ! Semi-major axis of sky region
      REAL OPTNRM                ! Normalise for what flux?
      REAL OUTER                 ! Outer scale factor of sky annulus
      REAL PEAK                  ! Peak counts
      REAL SEE                   ! Estimate of the seeing in pixels
...
*  Should only be one PSF star with index zero.
         CALL GRP_GET( PSFINF, 1, 1, BUFFER, STATUS )
         CALL CHR_DCWRD( BUFFER, 9, NWRD, START, STOP, APRWRD, LSTAT )
...
*  Extract supplied initial parameters.
...
*  PSF sky determination.
*  ======================

*  This can be done using either values supplied (SKYTYP = 4), or by
*  using an annular region or separate sky regions.
         IF ( SKYTYP .NE. 4 ) THEN
...
*  Check that we located an annulus region, otherwise use a constant
*  for the sky.
...
*  Define box bounds.
                  NXL = MAX( 1, INT( XPOS - OUTER - 0.5 ) )
                  NXH = MIN( NX, INT( XPOS + OUTER + 1.5 ) )
                  NYL = MAX( 1, INT( YPOS - OUTER - 0.5 ) )
                  NYH = MIN( NY, INT( YPOS + OUTER + 1.5 ) )
...
*  Arbitrary sky elliptical regions
*  --------------------------------
...
*  Locate the information about this sky region. Extract the
...
*  area of the grid array to be a box of size 2*MAJSKY.
                     NXL = MAX( 1, INT( XSKY - MAJSKY - 0.5 ) )
                     NXH = MIN( NX, INT( XSKY + MAJSKY + 1.5 ) )
                     NYL = MAX( 1, INT( YSKY - MAJSKY - 0.5 ) )
                     NYH = MIN( NY, INT( YSKY + MAJSKY + 1.5 ) )
...
*  If background estimation failed, skip the loop.
...
*  unless some problem has occurred in the estimation
                     NEWSKY = NEWSKY + 1
...
*  Check that we located at least one sky region, otherwise use a
*  constant for the sky.
               IF ( NEWSKY .EQ. 0 ) THEN
                  SKYTYP = 4
               ELSE
...
*  For the sky aperture evaluate the the variance per pixel and the
*  standard deviation.
         IF ( ISVAR ) THEN
            VSKY = VSKY
...
*  If the routine finished successfully then replace values of
*  XPOS, YPOS.
               IF ( STATUS .EQ. SAI__OK ) THEN
                  XPOS = XFINAL
                  YPOS = YFINAL

*  If an error has occurred in LOCATE then reset the status and use the
...
*  Locate the information about this sky region, set inner and
*  outer scale factors.
                       CALL GRP_GET( SKYINF, J, 1, BUFFER, STATUS )
                       CALL CHR_DCWRD( BUFFER, 2, NWRD, START, STOP,
     :                                 SKYWRD, LSTAT )
...
*  Check that we located an annulus region, otherwise use a constant for
*  the sky.
                 IF ( INNER .EQ. VAL__BADR ) THEN
                    SKYTYP = 4
                 ELSE
...
*  Arbitrary elliptical sky regions
*  --------------------------------
...
*  Prepare and validate results, which are then written to a buffer.
         BUFFER = ' '
...
*  Append trailing information (positions) and echo to the environment.
...
*  Locate and extract the information about this source.
...
*  Integrate in the object aperture.
*  ---------------------------------

*  Calculate the useful area of the grid array such the it fully
*  encompasses the ellipse (ignoring orientation) and has a margin
*  to ensure fractional pixels at the edges are included.
         NXL = MAX( 1, INT( XPOS - MAJOR - 0.5 ) )
         NXH = MIN( NX, INT( XPOS + MAJOR + 1.5 ) )
         NYL = MAX( 1, INT( YPOS - MAJOR - 0.5 ) )
         NYH = MIN( NY, INT( YPOS + MAJOR + 1.5 ) )
...
*  Initialise the grid elements to zero.
         CALL CLGRID( GSIZE, GSIZE, %VAL( CNF_PVAL( IG ) ), 1,
     :                NXH - NXL + 1, 1, NYH - NYL + 1 )

*  Create an ellipse centered on the supplied position.
         CALL MAKELL( XPOS, YPOS, MAJOR, ECCEN, ANGLE, NE, ELLIPS )

*  Determine areal weighting of each grid pixel.
...
*  Locate the information about this sky region, and extract inner and
*  outer scales.
                     CALL GRP_GET( SKYINF, J, 1, BUFFER, STATUS )
                     CALL CHR_DCWRD( BUFFER, 2, NWRD, START, STOP,
     :                               SKYWRD,LSTAT )
...
*  Integrate the sky region ellipse over an empty grid.  Use a modal
* filter on the sky background.
                     CALL PSX_CALLOC( NV, '_REAL', IV, STATUS )
                     IF ( STATUS .NE. SAI__OK ) GO TO 99
                     CALL PHO1_BACK( SKYTYP, NX, NY, IMAGE, IMVAR,
     :                               ISVAR, %VAL( CNF_PVAL( IG ) ),
...
*-----------------------------------------------------------------------
*  Update the GRP groups to contain the new photometry for this object.
   70    CONTINUE
         BUFFER = ' '
...
*  Append missing information and echo result to the environment.
         IAT = CHR_LEN( BUFFER ) + 2
         BUFFER( IAT: IAT ) = ' '
         IAT = IAT + 1
