      SUBROUTINE STR_PRH(RIGHT, FIELD, STR, FIRST, IVALUE, STATUS)
...
*
*   Description:
*      Grab hex value from parameter string using edit parameters.
*
*   History:
...
*
*   Method:
*      The string is scanned for an hex value specified by the edit
*      parameters.
...
*      FIELD unspecified means must search for valid format
 ...
 50      CONTINUE
         ...
         IF (POS.LE.LAST) THEN
            ...
            DIG = STR_INDEX('0123456789aAbBcCdDeEfF \\', STR(POS))
 ...
 100     CONTINUE
         ...
         IF (POS.LE.LAST) THEN
            ...
            IF (STR(POS).NE.32 .AND. STR(POS).NE.9) THEN
               ...
               IF (STR_INDEX('0123456789aAbBcCdDeEfF\\', STR(POS)).EQ.0)
     :              THEN
...
*   justification
      IF (STATUS.EQ.0) THEN
         ...
         CALL GEN_HTOI(VALUE, IVALUE, STATUS)
