      SUBROUTINE USR_PLFLUX( STATUS )
*+
*  Name:
*     SUBROUTINE USR_PLFLUX

*  Purpose:
*     The graphics display is opened, and the current flux spectrum
*     is plotted.
...
*   to define display limits if needed (only use).
      ...
      DO I = 1, N
         ...
         QPLOT( NPOINT + I ) = QFLX( I )
         YPLOT( NPOINT + I ) = REAL( SFLX( I ) )
      END DO
...
*      Plant axis labels.
         ...
         CALL STR_MOVE( FLABEL, MAXLABEL, YLAB )
         CALL STR_MOVE( FUNITS, MAXLABEL, YUN )
...
*   Make REAL copies of REAL*8 plot values.
      DO I = 1, NWAV
...
*   Plot flux spectrum.
      IF ( HISTLN ) THEN
         CALL GRF_HICURV( 3, NWAV, FWAVAIR, FSFLX, QFLX, CHAR( 0 ), 0,
     :                    QUAL, ZERO, SELFLG, RELFLG )

      ELSE
         CALL GRF_POCURV( 3, NWAV, FWAVAIR, FSFLX, QFLX, CHAR( 0 ), 0,
     :                    QUAL, ZERO, SELFLG, RELFLG )
      END IF
