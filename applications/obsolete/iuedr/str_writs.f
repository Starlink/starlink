...
*
*   Name:
*      SUBROUTINE str_WRITS
*
*   Description:
*      Encode string value into parameter string under format control.
*
*   Authors:
*      Jack Giddings
*
...
*
*   Method:
*      SVALUE is planted in the parameter string as specified by the
...
*   Local variables:
      ...
      BYTE LOCAL(MAXTOK)    ! local copy of value string
      BYTE VALUE(MAXTOK)    ! number coded into string
      ...
      DO WHILE (FIRST.LE.LAST)
         ...
         ELSE IF (TYPE.EQ.CODEDIT) THEN
            ...
            CALL str_MOVE(SVALUE, MAXTOK, LOCAL)

            IF (PREC.GE.0) CALL str_TERM(PREC, MAXTOK, LOCAL)

            CALL str_JUST(LOCAL, RIGHT, FIELD, MAXTOK, VALUE)
            CALL str_ADD(VALUE, MAXC, LINE, POS)
         ELSE
