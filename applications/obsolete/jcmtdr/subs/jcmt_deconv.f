...
*  Purpose:
*     deconvolve a single beam map from dual beam data.

*  Language:
*     Starlink Fortran 77

*  Invocation:
*      CALL JCMT_DECONV (ZIN, NX, NY, FBAD, XINC, BSEP, CFUNC,
*     :  ZOUT, STATUS)

*  Description:
*     Given all of the relevant map parameters and data this routine
*     produces the equivalent single beam map from a dual beam map
...
*        Original version.
*      8-JAN-1990 (REVAD::JFL): name changed from DECONV to JCMT_DECONV
*     22-MAY-1991 (REVAD::JFL): bad pixel flag now passed in explicitly
