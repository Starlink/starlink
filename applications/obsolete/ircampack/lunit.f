...
*  History:
*     13-OCT-1993 (DSB):
*        Original version, based on IRAS90 routine IRM1_LUNIT
*     {enter_changes_here}
...
*  If an IO error was detected, report an error.
            IF( IOS. NE. 0 ) THEN
               ...
               CALL ERR_REP( 'LUNIT_ERR1',
     :'LUNIT: Error inquiring status of Fortran unit ^UNIT: ^TEXT',
     :                       STATUS )
               GO TO 999
            END IF
...
*  If no free unit number could be found, report an error.
 999  CONTINUE
      IF( UNIT .EQ. -1 ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'LUNIT_ERR2',
     : 'LUNIT: No free Fortran I/O unit numbers can be found.',
     :                 STATUS )
      END IF
