...
# Create the gwm widget with scroll bars
    set message [catch {exec /star/bin/xdestroy xwindows}]
    global gwm
    set gwm [gwm_gwmWithScroll $w.frame -bg white -fg black]
...
# Pack a label along side the button frame to match the appearance of the frame
    pack [label $w.bottom.fill -text "Portable-CGS3DR" -relief sunken -border 2] -fill both -padx 3 -pady 3 -side left -expand y
...
# Create the command buttons
    ...
    checkbutton $w.buttons.crosshair -text Crosshair -padx 10 -variable CROSSHAIR -command crossHair
...
proc crossHair {} {
  ...
  if { $CROSSHAIR == 1 } {
    ...
    bind $gwm <Button-1> {cgs3drInform "GWM Widget Co-ords: X=%x Y=%y"}
