*+  CGS3DR_GENREDNAME - Generate a filename for a reduced CGS3 file
      SUBROUTINE CGS3DR_GENREDNAME (NUMBER, FILENAME, STATUS)
*    Description:
*     Given a run number this generates a file name for a reduced
*     CGS3 data file. This will have the form RODIR:RUNnnnn.
*    Invocation:
*     CALL CGS3DR_GENREDNAME (NUMBER, FILENAME, STATUS)
*    Parameters:
*     NUMBER = INTEGER (READ)
*           Run Number of data
...
*     20-Dec-95: change : to / for delimiter, run for RUN (KK)
*    Type Definitions:
      IMPLICIT NONE
*    Global constants:
      INCLUDE 'SAE_PAR'
