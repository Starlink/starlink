*+  POLCAL2 - calculates the polarization and position angle images from the
*            dark and sky subtracted images taken at the 4 waveplate positions

	SUBROUTINE POLCAL2 ( STATUS)

* Description :
*
* This routine ...
*
* Invocation : CALL POLCAL2 ( STATUS)
*
* Parameters :
*
* Method :
...
*  27-Nov-1995:  Made this version from polcal (caa@jach)
*
* Type definitions :
...
* Local variables :
	...
	INTEGER
     ...
     :    LOCI_5,  ! locator for 1st input data structure
     :    LOCI_6,  ! locator for 2nd input data structure
     :    LOCI_7,  ! locator for 3rd input data structure
     :    LOCI_8,  ! locator for 4th input data structure
     ...
     :    IDIMS_5( NDIMS),  ! dimensions of 1st input DATA_ARRAY
     :    IDIMS_6( NDIMS),  ! dimensions of 2nd input DATA_ARRAY
     :    IDIMS_7( NDIMS),  ! dimensions of 3rd input DATA_ARRAY
     :    IDIMS_8( NDIMS),  ! dimensions of 4th input DATA_ARRAY
     ...
     :    PNTRI_5,	   ! pointer to 1st input DATA_ARRAY component
     :    PNTRI_6,	   ! pointer to 2nd input DATA_ARRAY component
     :    PNTRI_7,	   ! pointer to 3rd input DATA_ARRAY component
     :    PNTRI_8 	   ! pointer to 4th input DATA_ARRAY component
...
*
	...
	CALL GETINP( 'INPIC5', LOCI_5, STATUS)
	CALL GETINP( 'INPIC6', LOCI_6, STATUS)
	CALL GETINP( 'INPIC7', LOCI_7, STATUS)
	CALL GETINP( 'INPIC8', LOCI_8, STATUS)
...
*
	  ...
	  CALL NDF_ANNUL( LOCI_5, STATUS)
	  CALL NDF_ANNUL( LOCI_6, STATUS)
	  CALL NDF_ANNUL( LOCI_7, STATUS)
	  CALL NDF_ANNUL( LOCI_8, STATUS)
	  RETURN
	END IF
*
...
*
     ...
     :                PNTRI_4, NELEMENTS, STATUS )
        ...
        CALL NDF_MAP( LOCI_5, 'DATA', '_REAL', 'READ',
     :                PNTRI_5, NELEMENTS, STATUS )
        CALL NDF_DIM( LOCI_5, NDIMS, IDIMS_5, ACTDIM, STATUS )

        CALL NDF_MAP( LOCI_6, 'DATA', '_REAL', 'READ',
     :                PNTRI_6, NELEMENTS, STATUS )
        CALL NDF_DIM( LOCI_6, NDIMS, IDIMS_6, ACTDIM, STATUS )

        CALL NDF_MAP( LOCI_7, 'DATA', '_REAL', 'READ',
     :                PNTRI_7, NELEMENTS, STATUS )
        CALL NDF_DIM( LOCI_7, NDIMS, IDIMS_7, ACTDIM, STATUS )

        CALL NDF_MAP( LOCI_8, 'DATA', '_REAL', 'READ',
     :                PNTRI_8, NELEMENTS, STATUS )
        CALL NDF_DIM( LOCI_8, NDIMS, IDIMS_8, ACTDIM, STATUS )
...
*
	IF( IDIMS_1( 1) .NE. IDIMS_2( 1) .OR.
     ...
     :	    IDIMS_1( 1) .NE. IDIMS_5( 1) .OR.
     :	    IDIMS_1( 1) .NE. IDIMS_6( 1) .OR.
     :	    IDIMS_1( 1) .NE. IDIMS_7( 1) .OR.
     :	    IDIMS_1( 1) .NE. IDIMS_8( 1) .OR.
     :	    IDIMS_1( 2) .NE. IDIMS_2( 2) .OR.
     :	    IDIMS_1( 2) .NE. IDIMS_3( 2) .OR.
     :	    IDIMS_1( 2) .NE. IDIMS_4( 2) .OR.
     :	    IDIMS_1( 2) .NE. IDIMS_5( 2) .OR.
     :	    IDIMS_1( 2) .NE. IDIMS_6( 2) .OR.
     :	    IDIMS_1( 2) .NE. IDIMS_7( 2) .OR.
     :	    IDIMS_1( 2) .NE. IDIMS_8( 2)) THEN
...
*      and position angle
*
	CALL POL_CALPT2( IDIMS_1(1), IDIMS_1(2), %VAL( PNTRI_1),
     :	                 %VAL( PNTRI_2), %VAL( PNTRI_3), %VAL( PNTRI_4),
     :	                 %VAL( PNTRI_5), %VAL( PNTRI_6), %VAL( PNTRI_7),
     :	                 %VAL( PNTRI_8), ELDN, ODIMS(1), ODIMS(2),
     :	                 %VAL( PNTROQ), %VAL( PNTROU), %VAL( PNTROP),
     :	                 %VAL( PNTROT), %VAL( PNTROI), %VAL( PNTROPI),
     :	                 %VAL( PNTROUPI), %VAL( PNTROPE), %VAL( PNTROTE),
     :	                 STATUS)
*
*      tidy up the input/output structures
*
