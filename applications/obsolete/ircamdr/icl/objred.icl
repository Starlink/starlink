{ PROCEDURE OBJRED : reduces a number of STARE/ND_STARE images + pho
proc objred var1 var2 var3 var4
  set precision 6
  numfs = 102
  numuk = 80
  extj = 0.100
  ...
  if filetype <> 1
    print "Sorry, you cannot run OBJRED on old format container files"
    return
  end if
  if name_prefix = "UNKNOWN"
    print "You must define IRCAM data file prefix before running OBJRED"
...
{ Auto-photometry on INDIVIDUAL images
  if std_phowhat = "BOTH"
    ...
    loop for jj = 1 to numin
      ...
      xst = integer(xobj0-35.0)
      yst = integer(yobj0-35.0)
      if std_sel = "C"
        ...
        obeyw rapi2d HISTO (diffim) (xst) (yst) 70 70 \
        getpar glob histo_max (maxval)
      ...
      if std_see = "Y"
        ...
        write efile (stc)
        fclose_e
        seefile = file_exists("objred_seeing.results")
        if seefile
          append efile "objred_seeing.results"
        else
          create efile "objred_seeing.results"
         a1 = "Image                        Filter    UT         "
...
{  print "  Object counts in aperture                = " (objpp)
      ...
      print "  Magnitude                                = " (mag)
      print " "
      phbfile = file_exists("objred_photometry.results")
      fclose_a
      if phbfile
        append afile "objred_photometry.results"
      else
        create afile "objred_photometry.results"
...
{ Cleanup
  ...
  print " "
  print "Photometry results appended to file : objred_photometry.results"
  print " "
  print "END of procedure OBJRED"
  print " "
end proc
