...
proc get_imagename num_obsele code_image name_out defval
  ynd = undefined(defval)
  get plt2d filetype (filetype)
  get plt2d subim_option (subim)
  if filetype <> 1
    ...
    if num_obsele2 = 0
      ...
      if ynd
        ...
        print "Last image plotted = " (defval)
        askname (name_out2) "Image Name \LAST\ : "
        if name_out2 = "LAST"
          name_out2 = defval
        end if
      end if
  ...
  if subim = 'S'
    print "Plt2d sub-area image display option enabled"
    print "use command setarea to modify"
  end if
  name_out = name_out2
end proc
