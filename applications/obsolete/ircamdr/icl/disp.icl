{ PROCEDURE DISP : plots STARE, SKY Difference on workstation
proc disp value1 value2 value3
  get plt2d image_workstn (image_workstn)
  if image_workstn <> 1
    print "DISP is not available from this terminal!"
    return
  end if
  ...
  if name_prefix = "UNKNOWN"
    print "Sorry, you must define the IRCAM data file before running DISP"
  ...
  if junk1 < 1 or junk1 > 5000
    print "Give OBJECT observation number (1-5000) : "
    asknum (junk1) "Object \1\ ? "
  end if
  temp = 1
  if filetype <> 1
  ...
  if junk2 < 1 or junk2 > 5000
    print "Give SKY observation number (1-5000) : "
    asknum (junk2) "Sky    \2\ ? "
  end if
  temp = 1
  if filetype <> 1
    formname (junk2) (temp) (name_out)
  else
    formname2 (junk2) (name_out)
  end if
  print "Forming OBJECT-SKY from " (junk1) " and " (junk2)
  tochar (junk1) (st1)
  tochar (junk1) (cnum)
  tochar (junk2) (st2)
  concat "f" (st1) (st3)
  ...
  obeyw rapi2d SUB (name_image) (name_out) (st4) 'DISP:OBJECT-SKY'
