*+  SUARAR - subtracts one array from another

      SUBROUTINE KFH_SUARAR(ARRA,INVA,ARRB,INVB,NVALS,ARRC,INVC,
     :                      MIN,MAX,STATUS)
*    Description :
*	It differences two 1-D arrays which are the same size, excluding
*	certain values, and calculating the min and max of the
*	resulting array at the same time.
*
*    Invocation :
*	CALL KFH_SUARAR(ARRA,INVA,ARRB,INVB,NVALS,ARRC,INVC,MIN,MAX,
*                       STATUS)
*    Parameters :
*	ARRA(NVALS)	_REAL
...
*               Status value
*    Method :
*	If either of the input pixels is invalid an invalid
*	value is stored in the output. Otherwise the difference of the
...
*
*   If status is bad on entry, then return to the main program.
*
...
*
         DO I=1,NVALS
            IF (ARRA(I).NE.INVA.AND.ARRB(I).NE.INVB) THEN
*
*            If two valid pixels then subtract them
*
