*+   DIARSC - DIvides an array by a scalar
      SUBROUTINE KFH_DIARSC(ARRA,INVA,NVALS,SCALAR,ARRC,INVC,MIN,MAX,
     :                      STATUS)
*    Description :
*      Every valid pixel of an input array is divided by a scalar
...
*
*    Invocation :
*     CALL KFH_DIARSC(ARRA,INVA,NVALS,SCALAR,ARRC,INVC,MIN,MAX,STATUS)
*    Parameters :
*	ARRA(NVALS)		_REAL
*		The input array
*	INVA			_REAL
...
*
*    Method :
*       It simply loops around dividing all valid pixels in the
...
*
*   If the status is bad, then return to the main routine.
*
...
*
*      If the scalar is zero then nothing can be done
*
         IF (SCALAR.NE.0.0) THEN
*
