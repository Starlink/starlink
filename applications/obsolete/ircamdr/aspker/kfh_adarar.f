*+  ADARAR - adds two arrays

      SUBROUTINE KFH_ADARAR(ARRA,INVA,ARRB,INVB,NVALS,ARRC,INVC,
     :                      MIN,MAX,STATUS)
*    Description :
*	It adds two 1-D arrays which are the same size, excluding
*	certain values, and calculating the min and max of the
*	resulting array at the same time.
*
*    Invocation :
*	CALL KFH_ADARAR(ARRA,INVA,ARRB,INVB,NVALS,ARRC,INVC,MIN,MAX,
*      :                STATUS)
*    Parameters :
*	ARRA(NVALS)	_REAL
...
*	value is stored in the output. Otherwise the sum of the
...
*
*   If status value is not correct, then return to main program.
*
...
*
         INVC=INVA
*
*      Give the flag an initial value
...
*
*            If two valid pixels then add them
*
