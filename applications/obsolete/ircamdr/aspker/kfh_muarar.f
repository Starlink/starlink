*+  MUARAR - multiplies two arrays (pixel by pixel)

      SUBROUTINE KFH_MUARAR(ARRA,INVA,ARRB,INVB,NVALS,ARRC,INVC,
     :                      MIN,MAX,STATUS)
*    Description :
*	It multiplies two 1-D arrays which are the same size, excluding
*	certain values, and calculating the min and max of the
*	resulting array at the same time.
*
*    Invocation :
*	CALL KFH_MUARAR(ARRA,INVA,ARRB,INVB,NVALS,ARRC,INVC,MIN,MAX,
*    :                  STATUS)
*    Parameters :
*	ARRA(NVALS)	_REAL
...
*	value is stored in the output. Otherwise the product of the
...
*
*   If there is a bad status, then return to the calling program.
*
...
*
*            If two valid pixels then multiply them
*
