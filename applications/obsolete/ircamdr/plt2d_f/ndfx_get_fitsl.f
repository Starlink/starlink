*+
      SUBROUTINE NDFX_GET_FITSL (NDF, ITEM, VALUE, STATUS)
*  Description:
*     Get a named FITS item from an NDF into a LOGICAL value
*  Invocation:
*     CALL NDFX_GET_FITSL (NDF, ITEM, VALUE, STATUS)
*  Arguments:
*     NDF = INTEGER (Given)
...
*  Export:
      LOGICAL VALUE                 ! Value to be returned
*  Status:
...
*       If no error then get the value
         CALL NDFX__GET_FITSL (EL, %val(PNTR(1)), POS, VALUE, STATUS,
     :   %val(LENGTH))
