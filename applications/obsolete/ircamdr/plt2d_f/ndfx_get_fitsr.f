*+
      SUBROUTINE NDFX_GET_FITSR (NDF, ITEM, VALUE, STATUS)
*  Description:
*     Get a named FITS item from an NDF
*  Invocation:
*     CALL NDFX_GET_FITSR (NDF, ITEM, VALUE, STATUS)
*  Arguments:
*     NDF = INTEGER (Given)
...
*       If no error then get the value
         CALL NDFX__GET_FITSR (EL, %val(PNTR(1)), POS, VALUE, STATUS,
     :   %val(LENGTH))
