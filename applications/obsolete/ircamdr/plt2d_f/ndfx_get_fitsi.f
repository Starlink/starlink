*+
      SUBROUTINE NDFX_GET_FITSI (NDF, ITEM, VALUE, STATUS)
*  Description:
*     Get a named FITS item from an NDF into an INTEGER value
*  Invocation:
*     CALL NDFX_GET_FITSI (NDF, ITEM, VALUE, STATUS)
*  Arguments:
*     NDF = INTEGER (Given)
...
*  Export:
      INTEGER VALUE                 ! Value to be returned
*  Status:
...
*       If no error then get the value
         CALL NDFX__GET_FITSI (EL, %val(PNTR(1)), POS, VALUE, STATUS,
     :   %val(LENGTH))
