*+
      SUBROUTINE NDFX_GET_FITSD (NDF, ITEM, VALUE, STATUS)
*  Description:
*     Get a named FITS item from an NDF into a DOUBLE PRECISION value
*  Invocation:
*     CALL NDFX_GET_FITSD (NDF, ITEM, VALUE, STATUS)
*  Arguments:
*     NDF = INTEGER (Given)
...
*  Export:
      DOUBLE PRECISION VALUE     ! Value to be returned
*  Status:
...
*       If no error then get the value
         CALL NDFX__GET_FITSD (EL, %val(PNTR(1)), POS, VALUE, STATUS,
     :   %val(LENGTH))
