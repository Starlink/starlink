*+ TRSTR5 - A copy of TRSTR1 used by TRACE for pseodeo recursion.
      SUBROUTINE TRSTR5( STRLOC, INDENT, FULL, STATUS )
*    Description :
...
*-
     ...
     :        STATUS )
            IF( .NOT. PRIM ) THEN
               IF( NDIM .EQ. 0 ) THEN
                  CALL LSDIR( LOC, INDENT+3, STATUS )
               ELSE
                  CALL TRARR6( LOC, NAME, SIZE, NDIM, DIMS, INDENT+3,
     :              FULL, STATUS )
               ENDIF
            ENDIF
