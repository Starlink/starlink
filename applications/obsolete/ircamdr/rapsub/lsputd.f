...
*+ LSPUTD - Put out the first few values of an DOUBLE object
      subroutine lsputd(loc, ndim, dims, size, line, leng)
*    Description :
...
*    read a limited part of the variable
      ...
      if (ndim .eq. 0) then
         call dat_get0d(loc, values, status)
      else
         call dat_vec(loc, vec, status)
         call dat_slice(vec, 1, 1, nvalue, slice, status)
         call dat_getvd(slice, MAXVAL, values, nvalue, status)
