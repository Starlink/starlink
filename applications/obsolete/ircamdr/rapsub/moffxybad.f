...
*     correct one.
*     Bad pixels are checked for by using the input bad pixel mask - in such
*     a mask, a non-zero value denotes a fixed bad pixel at that position in
*     the input images. Any pair of pixels being bad in either or both the
*     input images removes their difference from the mean deviation calcu-
*     lation. MOFFXYSUB is the root subroutine that does not account for
*     bad pixels.
*
*    Invocation :
*
*     CALL MOFFXYBAD( ARRAYA, DIMSA, ARRAYB, DIMSB, NXOFF, NYOFF, BOXSIZE,
*                     BADMASK, BADDIMS, BESTXOFF, BESTYOFF, STATUS )
*
*    Parameters :
*
...
*     BADMASK( BADDIMS(1), BADDIMS(2) )  =  REAL( READ )
*         Bad pixel mask - non-zero at bad pixel locations
*     BADDIMS( 2 )  =  INTEGER( READ )
*         Dimensions of bad pixel mask - should be same as images
