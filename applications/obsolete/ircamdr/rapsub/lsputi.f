...
*+ LSPUTI - Put out the first few values of an <integer> object
      subroutine lsputi(loc, ndim, dims, size, line, leng)
*    Description :
...
*    Local variables :
      ...
      integer values(MAXVAL)		! integer values
      integer ivalue			! value index
      logical full			! Whether text line is full
      integer status			! local status
D     integer vdims(DAT__MXDIM)		! DEBUG dimenensions of vector
D     integer actvd			! Dimensionality of vector
D     integer i				! Loop index
*-

*    read a limited part of the variable
      nvalue = min(MAXVAL, size)
C     print *,'lsputi:  size, nvalue ', size, nvalue
C     call dat_vec(loc, vec, status)
C     print *,'lsputi:  dat_vec => status ', status
C     call dat_shape(vec, DAT__MXDIM, vdims, actvd, status)
C     print *,'lsputi:  dat_shape(vec) => vdims, actvd, status ',
C    :  vdims, actvd, status
C     call dat_slice(vec, 1, 1, nvalue, slice, status)
C     print *,'lsputi:  dat_slice => status ', status
C     call dat_getvi(slice, MAXVAL, values, nvalue, status)
C     print *,'lsputi:  dat_getvi => status ', status

      if (ndim .eq. 0) then
         call dat_get0i(loc, values, status)
      else
         call dat_vec(loc, vec, status)
         call dat_slice(vec, 1, 1, nvalue, slice, status)
         call dat_getvi(slice, MAXVAL, values, nvalue, status)
