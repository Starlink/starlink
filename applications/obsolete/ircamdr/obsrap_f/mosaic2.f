*+  MOSAIC2 - merges 2 non-congruent images into one output image

      SUBROUTINE MOSAIC2 ( STATUS )

*    Description :
*
*     2 non-congruent images may be input, along with their relative
...
*    Local variables :
     ...
     :    LBND( 2 ),                   ! lower bounds of temporary array
...
*    start by getting setting number of frames to be merged together
      NUMBER = 2
...
*             cancel parameter
	          ...
	          CALL MSG_SETI( 'XO', XOFFSET(I))
	          CALL MSG_SETI( 'YO', YOFFSET(I))
	          CALL MSG_OUT( 'TEXT',
     :	            'Offset of image from 1st = ^XO, ^YO', STATUS)
...
*          map a DATA_ARRAY component and get dimensions
            CALL NDF_MAP( LOCO, 'DATA', '_REAL', 'WRITE',
     :                    PNTRO, NELEMENTS, STATUS )
            CALL NDF_DIM( LOCO, NDIMS, ODIMS, ACTDIM, STATUS )
