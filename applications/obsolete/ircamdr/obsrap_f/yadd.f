...
*        Get the start,number of rows to be added from input image
*        Create output image data structure
*        If no error so far then
*           Map an output DATA_ARRAY component
*           If no errors then
*              Add the input Y rows to form 2D output image
*              output
*           Endif
...
*     11-Aug-1994  Changed input DIM arguments for YADDSUB (SKL@JACH)
*
*    Type Definitions :
...
*          end of if-input--y-start/size-are-legal check
