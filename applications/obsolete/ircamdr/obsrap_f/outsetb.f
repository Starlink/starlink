*+  OUTSETB - sets value outside specified box to specified value

      SUBROUTINE OUTSETB ( STATUS )

*    Description :
*
*     A user-specified box of given X and Y size is taken and
*     all pixels outside this box are set to a specified value, the
*     pixels inside the box remaining unchanged.
*
*    Invocation :
*
*     CALL OUTSETB ( STATUS )
...
*     NEWVAL  =  REAL( READ )
*           Value to replace old values in pixel outside box
*
*    Method :
*
...
*            value to be substituted for pixels outside the box
*           If no error then
...
*                     into output one, except outside defined box
...
*     11-Aug-1994 Changed input DIM arguments to OUTSETBSUB (SKL@JACH)
*
*    Type Definitions :
...
*          get x and y start of box and size to be used
            ...
            CALL AIF_GET0I( 'XSTART', 1, 1, IDIMS( 1), XSTART,
     :                       STATUS )
            CALL AIF_GET0I( 'YSTART', 1, 1, IDIMS( 2), YSTART,
     :                       STATUS )
...
*          get replacement value for pixels outside defined box
...
*                   now call the subroutine that does the actual work
                     ...
                     CALL OUTSETBSUB( %VAL( PNTRI ), %VAL( PNTRO ),
     :                                IDIMS(1), IDIMS(2), XSTART,
     : 	                              YSTART, XSIZE, YSIZE, NEWVAL,
     :                                STATUS )
