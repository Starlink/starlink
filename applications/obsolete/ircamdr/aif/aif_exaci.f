*+  AIF_EXACI - Obtain exact number of INTEGER values from parameter system
      SUBROUTINE AIF_EXACI( PARNAM, NVALS, VALUES, STATUS )
*    Description :
*     This routine obtains an exact number of INTEGER values from the parameter
...
*    Invocation :
*      CALL AIF_EXACI( PARNAM, NVALS, VALUES, STATUS )
*    Parameters :
...
*               obtain values associated with parameter
                  CALL PAR_GETVI( PARNAM, NEEDED, WORK, NOBT, STATUS )
...
*            supply the complete VALUES array back to the parameter system
               CALL PAR_PUTVI( PARNAM, NVALS, VALUES, STATUS )
...
*      report the error
         ...
         CALL ERR_REP( 'AIF_EXAC_EXMX', 'AIF_EXACI: Request for '/
