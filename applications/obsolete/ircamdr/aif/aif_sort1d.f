...
*+  AIF_SORT1D - sort a 1-D array of type DOUBLE PRECISION
      SUBROUTINE AIF_SORT1D( DIM, ARRAY, STATUS )
*    Description :
*     This routine sorts a 1-D array of type DOUBLE PRECISION in situ using an
*     element swapping sort. This is a rather slow sort, but useful
*     for small arrays ( DIM < 100 ).
*    Invocation :
*     CALL AIF_SORT1D( DIM, ARRAY; STATUS )
*    Parameters :
*     DIM          = INTEGER( READ )
*           Dimension of array
*     ARRAY( DIM ) = DOUBLE PRECISION( UPDATE )
