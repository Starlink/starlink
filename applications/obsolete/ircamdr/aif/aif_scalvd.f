*+  AIF_SCALVD - scale and shift a vectorised array of type DOUBLE PRECISION
      SUBROUTINE AIF_SCALVD( DIM, BIAS, SCALE, ARRAY, STATUS )
*    Description :
...
*    Invocation :
*      CALL AIF_SCALVD( DIM, BIAS, SCALE; ARRAY, STATUS )
*    Parameters :
*     DIM           = INTEGER( READ )
*           The size of the array
*     BIAS          = DOUBLE PRECISION( READ )
*           Bias to add on to array
*     SCALE         = REAL( READ )
*           Scale factor to multiply array by
*     ARRAY( DIM )  = DOUBLE PRECISION( READ )
