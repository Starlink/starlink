*+  AIF_EXACR - Obtain exact number of REAL values from parameter system
      SUBROUTINE AIF_EXACR( PARNAM, NVALS, VALUES, STATUS )
*    Description :
*     This routine obtains an exact number of REAL values from the parameter
...
*    Invocation :
*      CALL AIF_EXACR( PARNAM, NVALS, VALUES, STATUS )
*    Parameters :
...
*               obtain values associated with parameter
                  CALL PAR_GETVR( PARNAM, NEEDED, WORK, NOBT, STATUS )
...
*            supply the complete VALUES array back to the parameter system
               CALL PAR_PUTVR( PARNAM, NVALS, VALUES, STATUS )
...
*      report the error
         ...
         CALL ERR_REP( 'AIF_EXAC_EXMX', 'AIF_EXACR: Request for '/
