*+  AIF_FILLVUW - Fill a vectorised INTEGER*2 array with a given value
      SUBROUTINE AIF_FILLVUW( VALUE, DIM, ARRAY, STATUS )
*    Description :
*     The vectorised array ARRAY( DIM ) of type INTEGER*2 is filled with
*     the given INTEGER*2 value VALUE.
*    Invocation :
*     CALL AIF_FILLVUW( VALUE, DIM, ARRAY, STATUS )
*    Parameters :
*     VALUE         = INTEGER*2( READ )
*           The value to fill the array with
*     DIM           = INTEGER( READ )
*           The size of the array to be filled. (i.e. the product of
*           the dimensions of ARRAY).
*     ARRAY( DIM ) = INTEGER*2( WRITE )
