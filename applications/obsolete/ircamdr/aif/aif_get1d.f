*+  AIF_GET1D - Obtain exact number of DOUBLE PRECISION values within given range
      SUBROUTINE AIF_GET1D( PARNAM, MIN, MAX, NVALS, VALUES, STATUS )
*    Description :
*     This routine obtains an exact number of DOUBLE PRECISION values, within a given
...
*         obtain exactly NVALS values
            CALL AIF_EXACD( PARNAM, NVALS, VALUES, STATUS )

*          check for error
            IF( STATUS .EQ. SAI__OK ) THEN

*             obtain the range of the values given
               CALL AIF_MMVD( NVALS, VALUES, VRANGE, STATUS )
...
*                report as an error
                  STATUS = SAI__ERROR
                  CALL MSG_SETD( 'MIN', MIN )
                  CALL MSG_SETD( 'MAX', MAX )
