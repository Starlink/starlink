*+  AIF_GET0D - Returns a scalar of type DOUBLE PRECISION in a given range.
      SUBROUTINE AIF_GET0D( PARNAM, DEFAUL, MIN, MAX, VALUE, STATUS )
*    Description :
*     A value is obtained for the DOUBLE PRECISION variable VALUE, associated with
...
*    Invocation :
*     CALL AIF_GET0D( PARNAM, DEFAUL, MIN, MAX, VALUE, STATUS )
*    Parameters :
*     PARNAM = CHARACTER*(*)( READ )
*           Parameter name associated with the value to be obtained.
*     DEFAUL = DOUBLE PRECISION( READ )
...
*       suggest value in DEFAUL to data system as dynamic default
         CALL PAR_DEF0D( PARNAM, DEFAUL, STATUS )
