*+  AIF_SUMX2L - Sum LOGICAL 2-D array in 1st (X) dimension to give 1-D array
      SUBROUTINE AIF_SUMX2L( DIMS, ARR2D, ARR1D, STATUS )
*    Description :
*     This takes the 2-dimensional LOGICAL array ARR2D and sums together its
*     contents in its first (X) dimension to give a 1-dimensional LOGICAL array
*     ARR1D.
*     E.g.
*                 ARR2D          ARR1D
*                1 6 3 7          17
...
*                8 9 5 6          28
*    Invocation :
*     CALL AIF_SUMX2L( DIMS, ARR2D, ARR1D, STATUS )
*    Parameters :
*     DIMS( 2 )                 = INTEGER( READ )
*           dimensions of 2-D array
*     ARR2D( DIMS(1), DIMS(2) ) = LOGICAL( READ )
*           2-D array to be summed in the X direction
*     ARR1D( DIMS(2) )          = LOGICAL( WRITE )
