*+  MALESS - return output array dims. and input array slice limits for MANIC
...
*    Description :
*     This routine handles the case when the output array has fewer dimensions
*     than the input array. Either an axis or a plane of the input array is
*     requested from which the output array will be created. Window limits
*     are then set in the appropriate dimensions and ranges obtained for the
*     remaining input array dimensions over which the input array will be
*     summed to create the output array. The outpu array dimensions and input
*     array slice bounds are calculated for eache of the three cases dealt
*     with. The returned values of CASE and MODE indicate how the data will be
*     handled by MANIC.
...
*            Parameter name associated with the window limits for extraction
*            of data from the first dimension of the input array.
*     YLIPAR = CHARACTER*(*)( READ )
*            Parameter name associated with the window limits for extraction
*            of data from the second dimension of the input array.
*     ZLIPAR = CHARACTER*(*)( READ )
*            Parameter name associated with the window limits for extraction
*            of data from the third dimension of the input array.
*     XRNPAR = CHARACTER*(*)( READ )
*            Parameter name associated with the summation range for the first
*            dimension of the input array.
*     YRNPAR = CHARACTER*(*)( READ )
*            Parameter name associated with the summation range for the second
*            dimension of the input array.
*     ZRNPAR = CHARACTER*(*)( READ )
*            Parameter name associated with the summation range for the third
*            dimension of the input array.
*     EL1PAR = CHARACTER*(*)( READ )
*            Parameter name associated with the axis of a 2-D array which is
*            to form a 1-D output array.
*     EL2PAR = CHARACTER*(*)( READ )
*            Parameter name associated with the axis of a 3-D array which is
*            to form a 1-D output array.
*     EPLPAR = CHARACTER*(*)( READ )
*            Parameter name associated with the plane of a 3-D arary which will
*            form a 2-D output array.
*     INDIM = INTEGER( READ )
...
*              Set CASE to 4, get plane for extraction from input array and
*              get window limits in specified plane and a range for summation
*              for the remaining axis. Calculate the output array dimensions in
*              each case.
*           Else
*              Must have 1-D output array so set CASE to 5, get line for
*              extraction from input array and get window limits in specified
*              axis and get ranges for summation in the remaining 2 axes.
*              Calculate output array dimensions for each case.
*           Endif
*           Calculate the lower and upper bounds for the input array slice.
*        Else
*           Must have 2-D input array and 1-D output array so set CASE to 7
*           and find out which axis of input array will form the output array.
*           Get window limits for given axis of the input array and get a range
*           for summation in the other axis. Calculate the output array
*           dimensions for each case. Calculate the lower and upper bounds of
*           the input array slice.
*        End if
*     Endif
*    Authors :
...
*       set up values for the default ranges along input array X and Y axes
         DXLIMS( 1 ) = 1
         DXLIMS( 2 ) = IDIMS( 1 )
         DYLIMS( 1 ) = 1
         DYLIMS( 2 ) = IDIMS( 2 )
...
*             get which PLANE to be extracted from input array
               CALL APPG0C( EPLPAR, 2, 'XY,YX,YZ,ZY,ZX,XZ', PLANE,
     :           STATUS )
...
*                get range for summation over Z axis
                  CALL APPGIR( ZRNPAR, DZLIMS, ZLIMS, STATUS )
...
*                get limits for extraction from YZ plane
                  CALL APPGIR( YLIPAR, DYLIMS, YLIMS, STATUS )
                  CALL APPGIR( ZLIPAR, DZLIMS, ZLIMS, STATUS )
...
*                get range for summation over X axis
                  CALL APPGIR( XRNPAR, DXLIMS, XLIMS, STATUS )
               ELSE
...
*                get range for summation for Y axis
                  CALL APPGIR( YRNPAR, DYLIMS, YLIMS, STATUS )
               ENDIF
            ELSE
...
*             get axis to be extracted from input array
               CALL APPG0C( EL2PAR, 1, 'X,Y,Z', LINE, STATUS )
...
*          get axis to be extracted from input array
            CALL APPG0C( EL1PAR, 1, 'X,Y', LINE, STATUS )
