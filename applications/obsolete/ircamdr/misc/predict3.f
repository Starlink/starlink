  ...
  10	type *, ' '
	...
	type *, ' '
	type *, 'Readout mode for data (STARE,NDR) ? '
	read( 5, '(a)') readmode
	call chr_ucase( readmode)
	if( readmode( 1:1) .eq. 'S') then
	  rdnoise = 11.0
	else if( readmode( 1:4) .eq. 'NDR') then
	  rdnoise = 6.0
	else
	  type *, 'Assumed normal STARE readout mode...'
	  rdnoise = 11.0
	end if
	type *, ' '
