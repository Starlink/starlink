	...
	open( unit=42, file='creobj.tmp', status='old')
	open( unit=43, file='creobj.icl', status='new')
  ...
  101	  continue
	  ...
	  type *, ' '
	  if( nobj .eq. nomit) then
            nobj = 888
	  end if
        else
	  type *, 'No file objects.list found...'
	  type *, 'Assuming reduction of ALL objects'
          nobj = 999
  	end if
	close( 45)
