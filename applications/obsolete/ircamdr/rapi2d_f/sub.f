*+  SUB - subtracts one image from another into new output image

      SUBROUTINE SUB ( STATUS )

*    Description :
*
*     This routine subtracts one image from another, placing the result
*     in a new output image. The two input images must have the same
*     dimensions.
*
*    Invocation :
*
...
*         Second input image
*     OUTPIC  =  IMAGE( WRITE )
*         Output image holding result of first image minus second
*     OTITLE  =  CHAR( READ )
...
*                       If no error then
*                          Call subroutine to subtract second image from
*                           first, results going into output image
*                       Endif
...
*                   for the results of subtraction
...
*                         call working subroutine to subtract second data
*                         array from first, result going into output
                           CALL SUBARR2D( %VAL(PNTRI1), %VAL(PNTRI2),
     :                         %VAL(PNTRO), DIMS1(1), DIMS1(2), STATUS )
