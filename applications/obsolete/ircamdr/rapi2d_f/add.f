*+  ADD - adds one image from another into new output image

      SUBROUTINE ADD ( STATUS )

*    Description :
*
*     This routine adds one image to another, placing the result in
*     a new output image. The two input images must have the same
*     dimensions.
*
*    Invocation :
*
...
*         Second input image
*     OUTPIC  =  IMAGE( WRITE )
*         Output image holding result of first image plus second
*     OTITLE  =  CHAR( READ )
...
*                       If no error then
*                          Call subroutine to add first image to
...
*     10-MAR-94    DAT_, CMP_ calls chamged to NDF_       (SKL@JACH)
*     12-Aug-1994  Changed input DIM arguments for ADDARR2D (SKL@JACH)
*
*    Type definitions :
...
*                get image dimensions and see if they match up
