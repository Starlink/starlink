...
*+  MULT - multiplies one image by another to form a new output image
...
*         Second input image
*     OUTPIC  =  IMAGE( WRITE )
*         Output image holding result of first image times second
*     OTITLE  =  CHAR( READ )
...
*                       If no error then
*                          Call subroutine to multiply first image by
...
*                   for the results of multiplication
