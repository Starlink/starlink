      SUBROUTINE IRQ1_RBIT( LOCS, BIT, STATUS )
*+
*  Name:
*     IRQ1_RBIT

*  Purpose:
*     Find an unused bit in the QUALITY component.
...
*     is written off the list of available bits. The smallest remaining
*     bit number is returned. If no bits remain, an error is reported.
...
*  Find the lowest bit number not in use.
      BIT = 0
      DO IBIT = 1, IRQ__QBITS
         IF( .NOT. BUSED( IBIT ) ) BIT = IBIT
      END DO

*  If no free bit could be found, report an error.
      IF( BIT .EQ. 0 .AND. STATUS .EQ. SAI__OK ) THEN
         STATUS = IRQ__XBITS
         CALL ERR_REP( 'IRQ1_RBIT_ERR1',
     :              'IRQ1_RBIT: No free bits left in QUALITY component',
     :                STATUS )
      END IF
