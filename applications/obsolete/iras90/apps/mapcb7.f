      SUBROUTINE MAPCB7( IDC, IDA, WFACT, DET, DETNO, SFACT,
     :                   SAMP0, START, END, SAMPLO, DINDLO, SAMPHI,
     :                   DINDHI, XLO, YLO, XHI, YHI, C2, NZSECT,
     :                   NYSECT, SECWGT, DATIN, PWGSZX, PWGSZY,
...
*  Purpose:
*     Paste a detector data stream, assuming no BAD pixels, no sample
...
*  Invocation:
*     CALL MAPCB7( IDC, IDA, WFACT, DET, DETNO, SFACT, SAMP0,
*                  START, END, SAMPLO, DINDLO, SAMPHI, DINDHI, XLO,
*                  YLO, XHI, YHI, C2, NZSECT, NYSECT, SECWGT, DATIN,
*                  PWGSZX, PWGSZY, NX, NY, IPPWG, NEW, DATOUT,
*                  WGTOUT, STATUS )

*  Description:
*     This routine adds the data stored in a single detector data
*     stream to the "running-sum" images, DATOUT and WGTOUT. No checks
*     for bad input data are performed. Each input data sample is given
*     a weight of unity (excluding the sector shrinkage factor, WFACT).
*     No output variances are created.
...
*  Initialise the total number of samples mapped from this detector.
      MAPPED = 0
...
*  coefficients, and calculate a new set of pixel weight grids.
            IF( CHANGE ) THEN
               ...
               CALL MSG_SETI( 'D', DET )
               CALL MSG_OUTIF( MSG__VERB, 'MAPCB7_MSG1',
...
*  detector.
      ...
      CALL MSG_OUTIF( MSG__VERB, 'MAPCB7_MSG2',
     :              '    ^N samples mapped from detector #^D.', STATUS )
