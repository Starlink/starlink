      SUBROUTINE SPOSB1( PARAM, PICID, X1, X2, Y1, Y2, STATUS )
*+
*  Name:
*     SPOSB1
...
*  Invocation:
*     CALL SPOSB1( PARAM, PICID, X1, X2, Y1, Y2, STATUS )
...
*  If a DATA picture could not be found, add a context message.
      IF( STATUS .EQ. AGI__NONAM ) THEN
         CALL ERR_REP( 'SPOSB1_ERR1',
     : 'SPOSB1: Unable to find a DATA picture which is contained '//
...
*  If not, report an error.
      IF( .NOT. CURAVL .AND. STATUS .EQ. SAI__OK ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'SPOSB1_ERR2',
     :    'SPOSB1: Specified graphics device does not support a cursor',
     :                 STATUS )
      END IF
...
*  Describe the picture to the user.
      ...
      IF( PICLAB .NE. ' ' ) THEN
         ...
         CALL MSG_OUTIF( MSG__NORM, 'SPOSB1_MSG1',
     :                   '  DATA picture ^LAB ("^COM") being used',
     :                      STATUS )
      ELSE
         CALL MSG_SETC( 'COM', PICCOM )
         CALL MSG_OUTIF( MSG__NORM, 'SPOSB1_MSG2',
