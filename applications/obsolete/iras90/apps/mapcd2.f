      SUBROUTINE MAPCD2( IDC, IDA, WFACT, DET, DETNO, SFACT, VFACT,
...
*  Invocation:
*     CALL MAPCD2( IDC, IDA, WFACT, DET, DETNO, SFACT, VFACT, SAMP0,
...
*     variance values, but any samples with zero variance are excluded
*     from the output image. Each input data sample is given a total
*     weight equal to the reciprocal of the its variance (converted
*     from a flux variance to a surface brightness variance). Output
*     variances will be created by MAPCC4 based on the spread of input
*     data values.
...
*        An image holding the sum of the weighted squared surface
*        brightness values. The input weighted squared data values are
*        added into this array by this routine.
...
*  Local Variables:
      ...
      INTEGER NBAD               ! No. of unusable samples mapped.
...
*  Initialise the total number of samples, and the number of unusable
...
*  coefficients, and calculate a new set of pixel weight grids.
            IF( CHANGE ) THEN
               ...
               CALL MSG_SETI( 'D', DET )
               CALL MSG_OUTIF( MSG__VERB, 'MAPCD2_MSG1',
...
*  detector.
      ...
      CALL MSG_OUTIF( MSG__VERB, 'MAPCD2_MSG2',
     :'    ^N samples mapped from detector #^D, of which ^B '//
     :' had zero variance.', STATUS )
