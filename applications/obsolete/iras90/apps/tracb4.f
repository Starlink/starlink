      SUBROUTINE TRACB4( NSMP, NLIN, DATA, LINNO, BG, ED, STATUS )
*+
*  Name:
*     TRACB4
...
*  Invocation:
*     CALL TRACB4( NSMP, NLIN, DATA, LINNO, BG, ED, STATUS )
...
*  Initialise the begin and end position of the non-zero samples.
      BG = -1
      ED = -1
...
*  Modify the returned indices to include the zero valued samples
*  adjacent to the non-zero section.
      IF( BG .GT. 1 ) BG = BG - 1
      IF( ED .LT. NSMP ) ED = ED + 1
