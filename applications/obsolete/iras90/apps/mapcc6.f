      SUBROUTINE MAPCC6( XLO, YLO, XHI, YHI, RLO, RHI, PLO, PHI, POFFX,
...
*  Purpose:
*     Paste data, weight and variance values for a single sample.

*  Language:
*     Starlink Fortran 77

*  Invocation:
*     CALL MAPCC6( XLO, YLO, XHI, YHI, RLO, RHI, PLO, PHI, POFFX, POFFY,
*                  TOTWGT, PWGSZX, PWGSZY, PWGRID, DATVAL, VARVAL,
*                  DATOUT, VAROUT, WGTOUT, INSIDE, STATUS )

*  Description:
*     This routine adds the data, variance and weight values for a
*     single detector sample to the "running-sum" images, DATOUT,
*     VAROUT and WGTOUT.
...
*     VARVAL = REAL (Given)
*        The variance of the sample value to be mapped.
...
*  Increment the value of the corresponding pixel in the image holding
*  the sum of the weighted variances.
               VAROUT( IX, IY ) = VAROUT( IX, IY ) + PWGT*PWGT*VARVAL
