      SUBROUTINE MAPCB4( IDC, IDA, WFACT, DET, DETNO, SFACT, VFACT,
     :                   SAMP0, START, END, SAMPLO, DINDLO, SAMPHI,
     :                   DINDHI, XLO, YLO, XHI, YHI, C2, NZSECT, NYSECT,
     :                   SECWGT, DATIN, VARIN, PWGSZX, PWGSZY, NX, NY,
     :                   IPPWG, NEW, DATOUT, VAROUT, WGTOUT, STATUS )
*+
*  Name:
*     MAPCB4

*  Purpose:
*     Paste a detector data stream, assuming no BAD pixels, sample
...
*  Invocation:
*     CALL MAPCB4( IDC, IDA, WFACT, DET, DETNO, SFACT, VFACT, SAMP0,
*                  START, END, SAMPLO, DINDLO, SAMPHI, DINDHI, XLO, YLO,
*                  XHI, YHI, C2, NZSECT, NYSECT, SECWGT, DATIN, VARIN,
*                  PWGSZX, PWGSZY, NX, NY, IPPWG, NEW, DATOUT, VAROUT,
*                  WGTOUT, STATUS )

*  Description:
*     This routine adds the data stored in a single detector data
*     stream to the "running-sum" images, DATOUT, VAROUT and WGTOUT.
*     No checks for BAD values are performed on the input data and
*     variance values. However, any samples which have variance values
*     of zero are excluded from the output map. Each input data sample
*     is given a total weight equal to the reciprocal of the its
*     variance (converted from a flux variance to a surface brightness
*     variance). Output variances are created based on the input
*     variance values.
...
*  coefficients, and calculate a new set of pixel weight grids.
            IF( CHANGE ) THEN
               ...
               CALL MSG_SETI( 'D', DET )
               CALL MSG_OUTIF( MSG__VERB, 'MAPCB4_MSG1',
...
*  If the variance value is zero, pass on to the next sample.
            IF( VARVAL .NE. 0.0 ) THEN
...
*  detector.
      ...
      CALL MSG_OUTIF( MSG__VERB, 'MAPCB4_MSG2',
     :'    ^N samples mapped from detector #^D, of which ^B had zero '//
     :'variance.', STATUS )
