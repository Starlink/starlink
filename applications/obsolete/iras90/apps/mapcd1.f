      SUBROUTINE MAPCD1( IDC, IDA, WFACT, DET, DETNO, SFACT, SAMP0,
     :                   START, END, SAMPLO, DINDLO, SAMPHI, DINDHI,
     :                   XLO, YLO, XHI, YHI, C2, NZSECT, NYSECT, SECWGT,
     :                   DATIN, PWGSZX, PWGSZY, NX, NY, IPPWG, NEW,
...
*     weighting, and INTERNAL output variances.

*  Language:
*     Starlink Fortran 77

*  Invocation:
*     CALL MAPCD1( IDC, IDA, WFACT, DET, DETNO, SFACT, SAMP0, START,
*                  END, SAMPLO, DINDLO, SAMPHI, DINDHI, XLO, YLO, XHI,
*                  YHI, C2, NZSECT, NYSECT, SECWGT, DATIN, PWGSZX,
*                  PWGSZY, NX, NY, IPPWG, NEW, DATOUT, VAROUT, WGTOUT,
*                  STATUS )
...
*     WFACT). Output variances will be created by MAPCC4 based on the
...
*        The input weighted squared data values are added into this
*        array by this routine.
...
*  than 1% (except for the coefficients giving the position of the
*  sample centre) since the last set of pixel weight grids were
*  calculated.
            CHANGE = .FALSE.

            IF( ABS( C3( 2 ) - C3SAVE( 2 ) )
     :          .GT. 0.01*ABS( C3SAVE( 2 ) ) ) THEN
               CHANGE = .TRUE.

            ELSE IF( ABS( C3( 3 ) - C3SAVE( 3 ) )
     :               .GT. 0.01*ABS( C3SAVE( 3 ) ) ) THEN
               CHANGE = .TRUE.

            ELSE IF( ABS( C3( 5 ) - C3SAVE( 5 ) )
     :               .GT. 0.01*ABS( C3SAVE( 5 ) ) ) THEN
               CHANGE = .TRUE.

            ELSE IF( ABS( C3( 6 ) - C3SAVE( 6 ) )
     :               .GT. 0.01*ABS( C3SAVE( 6 ) ) ) THEN
               CHANGE = .TRUE.
...
*  coefficients, and calculate a new set of pixel weight grids.
            IF( CHANGE ) THEN
               ...
               CALL MSG_SETI( 'D', DET )
               CALL MSG_OUTIF( MSG__VERB, 'MAPCD1_MSG1',
...
*  Store the current sample data value.
            DATVAL = DATIN( SAMP, DET )

*  If the sample value is bad, pass on to the next sample.
...
*  detector.
      ...
      CALL MSG_OUTIF( MSG__VERB, 'MAPCD1_MSG2',
