      SUBROUTINE IRA_DRPAR( IDA, A, B, INCA, SCS, LBND, UBND, STATUS )
*+
*  Name:
*     IRA_DRPAR
...
*  Invocation:
*     CALL IRA_DRPAR( IDA, A, B, INCA, SCS, LBND, UBND, STATUS )
...
*     graphics options TOLERANCE (See routine IRA_DROPT); highly curved
...
*        ACM_M3A0 = DOUBLE PRECISION (Write)
*           The sky longitude at the start of the parallel.
*        ACM_M3B0 = DOUBLE PRECISION (Write)
*           The sky latitude at the start of the parallel.
*        ACM_M3ID = INTEGER (Write)
*           The IRA identifier for the astrometry information.
*        ACM_M3IN = DOUBLE PRECISION (Write)
*           Longitude increment along the curve.
*        ACM_M3SC = CHARACTER (Write)
...
*  Check that the LBND and UBND arguments are OK.
     ...
     :    LBND( 2 ) .GE. UBND( 2 ) ) THEN
         ...
         CALL ERR_REP( 'IRA_DRPAR_ERR1',
     :'IRA_DRPAR: Viewport upper bounds (^U1,^U2) are inconsistent '//
...
*  Return without drawing anything if any of the inputs are bad.
     ...
     :    INCA .EQ. VAL__BADD ) GO TO 999

*  Store other information needed by IRA1_MAP3 in common.
      ACM_M3A0 = A
      ACM_M3B0 = B
...
*  Set up a flag indicating if lines are to be plotted.
...
*  Call IRA1_CURVE to draw the parallel section, using IRA1_MAP3 to
*  do the transformation from offset from the start of the curve to
*  image coordinates.
      CALL IRA1_CURVE( IRA1_MAP3, LBND( 1 ), UBND( 1 ), LBND( 2 ),
...
*  If an error occurred, give a context message.
 999  CONTINUE
      ...
      IF( STATUS .NE. SAI__OK ) THEN
         CALL ERR_REP( 'IRA_DRPAR_ERR2',
     :  'IRA_DRPAR: Error plotting a line of constant latitude.',
     :                 STATUS )
