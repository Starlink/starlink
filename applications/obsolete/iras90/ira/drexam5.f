      SUBROUTINE DREXAM5( STATUS )
*+
*  Name:
*     DREXAM5

*  Purpose:
*     Produce plot file for DRGRD example 5 in ID2.TEX.
...
*  Description:
*     Calls IRA_DRGRD to produce a grid showing an Gnomonic projection
*     in equatorial coordinates, covering a small area of the sky
*     close to the north pole. Tick marks are used. Device must be
*     CANON_PT or PSCRIPT_PTEX in order to produce files for inclusion
*     in TEX documents.
...
*  image centre, with image coordinates (0,0).
      ...
      P( 3 ) = 0.0D0
      P( 4 ) = 0.0D0
      P( 5 ) = 4.0D-3*IRA__DTOR
      P( 6 ) = 4.0D-3*IRA__DTOR
      P( 7 ) = 30.0*IRA__DTOR
      P( 8 ) = 0.0D0
...
*  Find the bounds of the image using these projection parameters.
      CALL IRA_XYLIM( IDA, P(1), P(2), 3.0*IRA__DTOR, 3.0*IRA__DTOR,
     :                LBND, UBND, STATUS )
