      SUBROUTINE IRA1_SORTD( ASCEND, KEYROW, NVAL, NROWS, DATA, NGOOD,
     :                      STATUS )
*+
*  Name:
*     IRA1_SORTD
...
*  Invocation:
*     CALL IRA1_SORTD( ASCEND, KEYROW, NVAL, NROWS, DATA, NGOOD, STATUS )
...
*  History:
*     10-MAR-1992 (DSB):
*        Original version, copied from IRM1_SORTD.
*     {enter_changes_here}
...
*  Report an error if KEYROW is not within the bounds of the array.
      IF( KEYROW .LT. 1 .OR. KEYROW .GT. NROWS ) THEN
         ...
         CALL ERR_REP( 'IRA1_SORTD_ERR1',
     :                'IRA1_SORTD: KEYROW ^K is out of range.', STATUS )
         GO TO 999
      END IF
