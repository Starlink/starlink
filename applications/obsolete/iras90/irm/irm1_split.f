      SUBROUTINE IRM1_SPLIT( INSTR, APNLIN, MAXLIN, LINENO,
     :                       SUBSTR, STATUS )
*+
*  Name:
*     IRM1_SPLIT
...
*  Invocation:
*     CALL IRM1_SPLIT( INSTR, APNLIN, MAXLIN, LINENO, SUBSTR, STATUS )
...
*     be appended to the line if the line length after appending the word
...
*     ended and a new line will begin until all the words in the string
*     have been used.


*  Arguments:
*     INSTR = CHARACTE*(*) (Given)
...
*  Local Variables:
      ...
      CHARACTER*( MAXWRD * MWRDLN ) STRING
                                 ! Input string
...
*  If the input string is blank, set status as error, report and exit.
      IF ( STRGLN .EQ. 0 ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'IRM1_SPLIT_ERR1',
     :           'IRM1_SPLIT: Attempt to split a blank string', STATUS )
         GOTO 999
      END IF
...
*  error, report error and exit.
      IF ( LSTAT .EQ. 1 ) THEN
         STATUS = SAI__ERROR
         CALL ERR_REP( 'IRM1_SPLIT_ERR2',
     :              'IRM1_SPLIT: Too many words in the sting.', STATUS )
         GOTO 999
      END IF
...
*  Form lines until all words have been used.
...
*  report the error and exit.
         IF( LINENO .GT. MAXLIN ) THEN
            STATUS = SAI__ERROR
            CALL ERR_REP( 'IRM1_SPLIT_ERR3',
     :         'IRM1_SPLIT: The number of lines exceed the max number.',
     :                     STATUS )
            GOTO 999
         END IF
...
*  Append words to the line until no more words for this line.
         DO WHILE( MORWRD )
