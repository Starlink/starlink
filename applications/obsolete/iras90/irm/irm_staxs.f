...
*  Local Constants:
      INTEGER MAXLAB             ! Allowed max length of a label
      PARAMETER ( MAXLAB = 120 )
...
*  Local Variables:
      ...
      CHARACTER*( MAXLAB ) TMPSTR
                                 ! A temporary string holding labels
...
*  line end mark.
      IF( REAL( XLBLN ) .LT. MXLBLN ) THEN
         TMPSTR = XLAB( : XLBLN )
         XLAB = TMPSTR( : XLBLN )//ENDMRK
      END IF
      IF( REAL( YLBLN ) .LT. MXLBLN ) THEN
         TMPSTR = YLAB( : YLBLN )
         YLAB = TMPSTR( : YLBLN )//ENDMRK
      END IF
