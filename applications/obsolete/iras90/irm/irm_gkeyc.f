      SUBROUTINE IRM_GKEYC( NCARD, BUFFER, SCARD, NAME, THERE, VALUE,
     :                      CARD, STATUS )
*+
*  Name:
*     IRM_GKEYC
...
*  Invocation:
*     CALL IRM_GKEYC( NCARD, BUFFER, SCARD, NAME, THERE, VALUE, CARD,
*                     STATUS )
...
*  History:
*     4-DEC-1992 (DSB):
*        Copied from KAPPA routine FTS1_GKEYC, written by MJC.
...
*  Find the positions of the first quote and slash.  It does not matter
*  at this point that the slash may be part of the value.  Here we want
*  to ensure that the quote is not part of the comment.
     ...
     :              ( COLIM .LT. LQCOL .AND. COLIM .NE. 0 ) ) THEN
                  ...
                  CALL MSG_OUT( 'IRM_GKEYC_MSG1',
     :              'A FITS character value must be enclosed in '/
     :              /'quotes. ^NAME does not. Inform the source '/
     :              /'institution.', STATUS )
                  CALL MSG_OUT( 'IRM_GKEYC_MSG2',
     :              'Using the remainder of the card image from '/
     :              /'column 11 as the value.', STATUS )

*  Set up an imaginary quote immediately following the equals.
                  LQCOL = EQUALS + 1
                  LEADQ = .FALSE.

*  Allow for the search offset of EQUALS characters to the leading
*  quote.
               ELSE
                  LQCOL = LQCOL + EQUALS
                  LEADQ = .TRUE.
...
*  Find the trailing quote.
               ...
               IF ( TQCOL .EQ. 0 .AND. LEADQ ) THEN
                  CALL MSG_SETC( 'NAME', KEYWRD )
                  CALL MSG_OUT( 'IRM_GKEYC_MSG3',
     :              'A FITS character value must be enclosed in '/
     :              /'quotes. ^NAME does not. Inform the source '/
     :              /'institution.', STATUS )
                  CALL MSG_OUT( 'IRM_GKEYC_MSG4',
     :              'Using the remainder of the card image as the '/
     :              /'value.', STATUS )

*  Set up an imaginary quote beyond the end of the card image.
                  TQCOL = 81 - LQCOL
               END IF

*  If the character length of VALUE is less than that required to hold
*  the string, it may be truncated.  Define new notional closing quote.
...
*  with the hierarchical keyword.  Therefore find the leading and
*  trailing quotes.  Though first find the positions of the first
*  slash.  It does not matter at this point that the slash may be part
*  of the value.  Here we want to ensure that the quote is not part of
*  the comment.
                  LQCOL = INDEX( BUFFER( CARD )( EQUALS + 1: ), '''' )
                  COLIM = INDEX( BUFFER( CARD )( EQUALS + 1: ), '/' )
     ...
     :                 ( COLIM .LT. LQCOL .AND. COLIM .NE. 0 ) ) THEN
                     CALL MSG_SETC( 'NAME', KEYWRD )
                     CALL MSG_OUT( 'IRM_GKEYC_MSG5',
     :                 'A FITS character value must be enclosed in '/
     :                 /'quotes. ^NAME does not. Inform the source '/
     :                 /'institution.', STATUS )
                     CALL MSG_OUT( 'IRM_GKEYC_MSG6',
     :                 'Using the remainder of the card image from '/
     :                 /'column 11 as the value.', STATUS )
...
*  Find the trailing quote, aborting if there is not one.  Do not
*  report another message if the string is devoid of quotes.
                  TQCOL = INDEX( BUFFER( CARD )( LQCOL + 1: ), '''' )
                  IF ( TQCOL .EQ. 0 .AND. LEADQ ) THEN
                     CALL MSG_SETC( 'NAME', KEYWRD )
                     CALL MSG_OUT( 'IRM_GKEYC_MSG7',
     :                 'A FITS character value must be enclosed in '/
     :                 /'quotes. ^NAME does not. Inform the source '/
     :                 /'institution.', STATUS )
                     CALL MSG_OUT( 'IRM_GKEYC_MSG8',
     :                 'Using the remainder of the card image as the '/
     :                 /'value.', STATUS )
