...
*  Global Constants:
      ...
      INCLUDE 'NDG_ERR'          ! NDG error values.
...
*  Check that the argument MAXSIZ and MINSIZ are OK.
      IF( MAXSIZ .LT. MINSIZ .AND. MAXSIZ .NE. 0 ) THEN
         ...
         CALL ERR_REP( 'IRM_WRNDF_ERR1',
     :'IRM_WRNDF: Maximum allowed number of NDFs (^MAX) is less than '//
...
*  flag.
         IF( SIZE .GT. MAXSIZ .AND. MAXSIZ .GT. 0 ) THEN
            ...
            IF( MAXSIZ .GT. 1 ) THEN
               CALL MSG_SETI( 'MAX', MAXSIZ )
               CALL MSG_OUTIF( MSG__NORM, 'IRM_WRNDF_MSG1',
     :       'WARNING: Only the first ^MAX NDFs names can be used',
     :                          STATUS )

            ELSE
               CALL MSG_OUTIF( MSG__NORM, 'IRM_WRNDF_MSG2',
...
*  that no more NDFs can be specified, and cancel the re-prompt flag.
            ELSE
               ...
               CALL MSG_BLANKIF( MSG__NORM, STATUS )
               CALL MSG_OUTIF( MSG__NORM, 'IRM_WRNDF_MSG4',
...
*  issued.
            IF( SIZE .LT. MINSIZ ) THEN
               ...
               IF( SIZE .GT. 0 ) THEN
                  ...
                  CALL MSG_OUTIF( MSG__NORM, 'IRM_WRNDF_MSG5',
     :              'WARNING: At least ^I more NDF(s) must be supplied',
     :                            STATUS )
               ELSE
                  CALL MSG_SETI( 'I', MINSIZ )
                  CALL MSG_OUTIF( MSG__NORM, 'IRM_WRNDF_MSG6',
