...
*  Invocation:
*     CALL IRM_RDNDF( PARAM, MAXSIZ, MINSIZ, TEXT, IGRP, SIZE, STATUS )
...
*  Check that the argument MAXSIZ and MINSIZ are OK.
      IF( MAXSIZ .LT. MINSIZ .AND. MAXSIZ .NE. 0 ) THEN
         ...
         CALL ERR_REP( 'IRM_RDNDF_ERR1',
     :'IRM_RDNDF: Maximum allowed number of NDFs (^MAX) is less than '//
...
*  flag.
         IF( SIZE .GT. MAXSIZ .AND. MAXSIZ .GT. 0 ) THEN
            ...
            IF( MAXSIZ .GT. 1 ) THEN
               CALL MSG_SETI( 'MAX', MAXSIZ )
               CALL MSG_OUTIF( MSG__NORM, 'IRM_RDNDF_MSG1',
     :       'WARNING: Only the first ^MAX NDF names can be used',
     :                          STATUS )

            ELSE
               CALL MSG_OUTIF( MSG__NORM, 'IRM_RDNDF_MSG2',
...
*  that no more NDFs can be specified, and cancel the re-prompt flag.
            ELSE
               ...
               CALL MSG_BLANKIF( MSG__NORM, STATUS )
               CALL MSG_OUTIF( MSG__NORM, 'IRM_RDNDF_MSG4',
...
*  issued.
            IF( SIZE .LT. MINSIZ ) THEN
               ...
               IF( SIZE .GT. 0 ) THEN
                  ...
                  CALL MSG_OUTIF( MSG__NORM, 'IRM_RDNDF_MSG6',
     :              'WARNING: At least ^I more NDF(s) must be supplied',
     :                            STATUS )
               ELSE
                  CALL MSG_SETI( 'I', MINSIZ )
                  CALL MSG_OUTIF( MSG__NORM, 'IRM_RDNDF_MSG7',
