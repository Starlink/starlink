*+  CHI_AOPWI - Perform work stack operation on I operands
      subroutine chi_aopwi(op, nextype, wptr, wstop, status)
*    Description :
*     Perform simple unary and binary operations on operand(s) in
*     Wival and puts the answer in the appropriate work stack
*    Invocation :
*     CALL CHI_AOPWI ( OP, NEXTYPE, WPTR, WSTOP, STATUS )
*    Parameters :
*     OP = INTEGER(INPUT)
*           Operator id
...
203   Wival(resptr)= Wival(resptr) + Wival(wptr)
      goto 250
204   Wival(resptr)= Wival(resptr) - Wival(wptr)
      goto 250
205   Wival(resptr)= Wival(resptr) * Wival(wptr)
      goto 250
206   Wival(resptr)= Wival(resptr) / Wival(wptr)
      goto 250
215   Wival(resptr)= Wival(resptr) ** Wival(wptr)
*
*   convert from I_type
250   continue
...
251   continue
      goto 500
*   to R
253   Wrval(resptr)= Wival(resptr)
      goto 500
*   to D
254   Wdval(resptr)= Wival(resptr)
      goto 500
*   to C
255   continue
      goto 500
...
*
309   Wlval(resptr)= Wival(resptr) .LT. Wival(wptr)
      goto 350
310   Wlval(resptr)= Wival(resptr) .LE. Wival(wptr)
      goto 350
311   Wlval(resptr)= Wival(resptr) .EQ. Wival(wptr)
      goto 350
312   Wlval(resptr)= Wival(resptr) .GE. Wival(wptr)
      goto 350
313   Wlval(resptr)= Wival(resptr) .GT. Wival(wptr)
      goto 350
314   Wlval(resptr)= Wival(resptr) .NE. Wival(wptr)
*
*   convert from L_type
350   continue
*
