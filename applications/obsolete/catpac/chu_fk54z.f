      SUBROUTINE
     : CHU_FK54Z( INPUT, OUTPUT, R2000, D2000, BEPOCH, R1950, D1950,
     : STATUS )
*+
*  Name:
*     CHU_FK54Z
...
*  Invocation:
*     CALL CHU_FK54Z( INPUT, OUTPUT, R2000, D2000, BEPOCH, R1950, D1950,
*    : STATUS )
...
*     coordinates. The new fields are calculated using SLA_FK54Z. See SUN 67
*
...
*
        ...
        DO FLDCOUNT = 1, NUMFLDS
          IF (NAMES(FLDCOUNT) .EQ. R2000) THEN
            R1FLG = .TRUE.
            R1FMT = FORMAT(FLDCOUNT)
          ELSEIF (NAMES(FLDCOUNT) .EQ. D2000) THEN
            D1FLG = .TRUE.
...
*
           IF (FIRSTPASS) THEN
             ...
             DO FLDCOUNT = 1, INPUTNFLDS
              IF (OLDFNAMES(FLDCOUNT) .EQ. R2000) THEN
                R1POS = FLDCOUNT
              ELSEIF (OLDFNAMES(FLDCOUNT) .EQ. D2000) THEN
                D1POS = FLDCOUNT
              ENDIF
             ENDDO
           ENDIF
...
*
             DBEPOCH = BEPOCH
             CALL SLA_FK54Z(DBR1, DBD1, DBEPOCH, DBR2, DBD2, DBDR2,
     :          DBDD2)
*
