*+  CHI_AOPWR - Perform work stack operation on R operands
      subroutine chi_aopwr(op, nextype, wptr, wstop, status)
*    Description :
*     Perform simple unary and binary operations on operand(s) in
*     Wival and puts the answer in the appropriate work stack
*    Invocation :
*     CALL CHI_AOPWR ( OP, NEXTYPE, WPTR, WSTOP, STATUS )
*    Parameters :
*     OP = INTEGER(INPUT)
*           Operator id
...
203   Wrval(resptr)= Wrval(resptr) + Wrval(wptr)
      goto 250
204   Wrval(resptr)= Wrval(resptr) - Wrval(wptr)
      goto 250
205   Wrval(resptr)= Wrval(resptr) * Wrval(wptr)
      goto 250
206   Wrval(resptr)= Wrval(resptr) / Wrval(wptr)
      goto 250
215   Wrval(resptr)= Wrval(resptr) ** Wrval(wptr)
*
*   convert from R_type
250   continue
...
251   continue
      goto 500
*   to I
253   Wival(resptr)= int(Wrval(resptr))
      goto 500
*   to D
254   Wdval(resptr)= Wrval(resptr)
      goto 500
*   to C
255   continue
      goto 500
...
*
309   Wlval(resptr)= Wrval(resptr) .LT. Wrval(wptr)
      goto 350
310   Wlval(resptr)= Wrval(resptr) .LE. Wrval(wptr)
      goto 350
311   Wlval(resptr)= Wrval(resptr) .EQ. Wrval(wptr)
      goto 350
312   Wlval(resptr)= Wrval(resptr) .GE. Wrval(wptr)
      goto 350
313   Wlval(resptr)= Wrval(resptr) .GT. Wrval(wptr)
      goto 350
314   Wlval(resptr)= Wrval(resptr) .NE. Wrval(wptr)
*
*   convert from L_type
350   continue
*
