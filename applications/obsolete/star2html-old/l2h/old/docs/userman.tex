...
\item [ -t \Meta{top-page-title}\label{toppagetitle}]
...
\item [ -short\_extn\label{shortextn}]
...
\item [ -long\_titles \Meta{num}\label{longtitles}]
...
\item [ -custom\_titles\label{customtitles}]
...
\item [ -dir \Meta{output-directory}\label{outputdir}]
...
\item [ -no\_subdir\label{nosubdir}]
...
\cbversion{96.1}\begin{changebar}
\item [ -prefix \Meta{filename-prefix}\label{prefix}]%
...
\item [ -auto\_prefix\label{autoprefix}]
...
\cbversion{97.1}\begin{changebar}\cbversion{97.1}
\item [ -no\_auto\_link\label{noautolink}]
...
\item [ -split \Meta{num}\label{splitdepth}]
...
\item [ -split +\Meta{num}\label{reldepth}]
...
\item [ -link \Meta{num}\label{linkdepth}]
...
using the \Cs{html\_version} switch described \htmlref{earlier}{htmlversion},
...
\cbversion{96.1}\begin{changebar}
\item [ -up\_url \Meta{URL}\label{upURL}]
...
\item [ -up\_title \Meta{string}\label{uptitle}]
...
\item [ -prev\_url \Meta{URL}\label{prevURL}]
...
\item [ -index \Meta{URL}\label{indexURL}]
...
\item [ -biblio \Meta{URL}\label{biblioURL}]
...
of \fn{\$ICONSERVER}.\\
When the \htmlref{\Cs{local\_icons}}{localicons} switch is used,
