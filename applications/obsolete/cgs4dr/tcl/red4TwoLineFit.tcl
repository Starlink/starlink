...
# Create, pack and bind row/col widget
    if {$type == "row"} {
      ...
      $Red4Widgets(TLF_ENT04) insert end 29
      $Red4Widgets(TLF_ENT05) insert end 29
    } else {
...
# Get the values from the dialogue box
    set data [string trim [$Red4Widgets(TLF_ENT01) get]]
    set r1   [string trim [$Red4Widgets(TLF_ENT02) get]]
    set r2   [string trim [$Red4Widgets(TLF_ENT03) get]]
    set rs   [string trim [$Red4Widgets(TLF_ENT04) get]]
    set re   [string trim [$Red4Widgets(TLF_ENT05) get]]
...
# Log some comments to file
   ...
   if {$type == "row"} {
      set comment "Analysing slit alignment in $data"
   } else {
      set comment "Analysing dispersion alignment in $data"
   }
...
# Extract the first spectrum
   ...
   set exts [expr {(1 > [expr $r1 - $nrows]) ? 1 : [expr $r1 - $nrows]}]
   set exte [expr {(256 < [expr $r1 + $nrows]) ? 256 : [expr $r1 + $nrows]}]
   if {$type == "row"} {
     set message "Extracting spectrum from rows $exts to $exte in $data to ${data}_spc1"
     cgs4drInform $taskname $message
     $taskname obey extract4 "image=$data ystart=$exts yend=$exte spectrum=${data}_spc1" \
        -inform "cgs4drInform $taskname %V" -endmsg {set ext1_done 1}
     tkwait variable ext1_done
   } else {
     set message "Extracting spectrum from columns $exts to $exte in $data to ${data}_spc1"
     cgs4drInform $taskname $message
     figaro1 obey ystract "image=$data xstart=$exts xend=$exte spectrum=${data}_spc1" \
        -inform "cgs4drInform $taskname %V" -endmsg {set ext1_done 1}
     tkwait variable ext1_done
   }

# Extract the second spectrum
   set exts [expr {(1 > [expr $r2 - $nrows]) ? 1 : [expr $r2 - $nrows]}]
   set exte [expr {(256 < [expr $r2 + $nrows]) ? 256 : [expr $r2 + $nrows]}]
   if {$type == "row"} {
     set message "Extracting spectrum from rows $exts to $exte in $data to ${data}_spc2"
     cgs4drInform $taskname $message
     $taskname obey extract4 "image=$data ystart=$exts yend=$exte spectrum=${data}_spc2" \
       -inform "cgs4drInform $taskname %V" -endmsg {set ext2_done 1}
     tkwait variable ext2_done
   } else {
     set message "Extracting spectrum from columns $exts to $exte in $data to ${data}_spc2"
     cgs4drInform $taskname $message
     figaro1 obey ystract "image=$data xstart=$exts xend=$exte spectrum=${data}_spc2" \
        -inform "cgs4drInform $taskname %V" -endmsg {set ext2_done 1}
     tkwait variable ext2_done
   }
...
# Run the figaro SPLOT function on spectrum 1
   ...
   cgs4drInform $taskname $message
   set params "soft='xwindows;$env(PID)xwin' spectrum=${data}_spc1 whole=F xstart=$rs xend=$re autoscale=T axes=T erase=T lines=T hardcopy=F colour='red'"
   if {$type == "row"} {
...
# Run the figaro SPLOT function on spectrum 2
   ...
   cgs4drInform $taskname $message
   set params "soft='xwindows;$env(PID)xwin' spectrum=${data}_spc2 whole=F xstart=$rs xend=$re autoscale=T axes=T erase=F lines=T hardcopy=F colour='yellow'"
   if {$type == "row"} {
...
# Do EMLT etc on first spectrum
   ...
   cgs4drInform $taskname $message
   figaro2 obey emlt "spectrum=${data}_spc1 xstart=$rs xend=$re lines=0 moments=F" \
   ...
   if {[file exists emlt.lis] == 1} {
     ...
     $taskname obey read_emlt "xmin=$rs xmax=$re" -inform "cgs4drInform $taskname %V" -endmsg {set remlt1_done 1}
     tkwait variable remlt1_done
     $taskname obey log_emlt "ycen=$r1" -inform "cgs4drInform $taskname %V" -endmsg {set lemlt1_done 1}
...
# Do EMLT etc on second spectrum
   ...
   cgs4drInform $taskname $message
   figaro2 obey emlt "spectrum=${data}_spc2 xstart=$rs xend=$re lines=0 moments=F" \
   ...
   if {[file exists emlt.lis] == 1} {
     ...
     $taskname obey read_emlt "xmin=$rs xmax=$re" -inform "cgs4drInform $taskname %V" -endmsg {set remlt3_done 1}
     tkwait variable remlt3_done
     $taskname obey log_emlt "ycen=$r2" -inform "cgs4drInform $taskname %V" -endmsg {set lemlt2_done 1}
...
# Calculate the slit angle
    ...
    cgs4drInform $taskname $message
    $taskname obey calc_slit_angle "ycen_p=$r1 ycen=$r2" -inform "cgs4drInform $taskname %V" -endmsg {set cslit_done 1}
    tkwait variable cslit_done
    $taskname obey report_slit "ycen_p=$r1 ycen=$r2" -inform "cgs4drInform $taskname %V" -endmsg {set rslit_done 1}
    tkwait variable rslit_done
    $taskname obey log_slit "ycen_p=$r1 ycen=$r2" -inform "cgs4drInform $taskname %V" -endmsg {set lslit_done 1}
    tkwait variable lslit_done
    $taskname obey read_obs "observation=$data" -inform "cgs4drInform $taskname %V" -endmsg {set robs_done 1}
