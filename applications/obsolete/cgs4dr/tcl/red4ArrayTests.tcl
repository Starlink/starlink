...
# Create and pack dialog box widgets
   ...
   $Red4Widgets(AT_ENT01) insert end 1
...
# Get the observation number etc
   set num  [string trim [$Red4Widgets(AT_ENT01) get]]
   destroy .red4Dialogue
   if {$num <=0} {
...
# Subtract two images
   set in1 "\$RODIR/ro$env(CGS4_DATE)_[expr $num + 2]"
   set in2 "\$RODIR/ro$env(CGS4_DATE)_[expr $num + 3]"
...
# Subtract two images
   set in1 "\$RODIR/ro$env(CGS4_DATE)_[expr $num + 5]"
   set in2 "\$RODIR/ro$env(CGS4_DATE)_[expr $num + 4]"
