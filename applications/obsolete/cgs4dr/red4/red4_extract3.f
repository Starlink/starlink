C+
      SUBROUTINE RED4_EXTRACT3 (STATUS)
C
C     E X T R A C T 4
C
...
C
C     This routine used John Lightfoot's algorithm, which determines
C     the mean and variance down each column.
C
...
C
     ...
     :'calculate the mean values of the input image columns and the ')
      CALL DSA_WRUSER (
     :'error on the mean. Designed mainly for CGS3 data.\N')
C
C     Open DSA routines
C
