...
*      Get the name of the STANDARD used from the group
         CALL DSA_GET_FITS_C( 'GRPFILE', 'STDUSED', 0, 
     :      STANDARD_NAME, COMMENT, DSA_STATUS )
         IF ( STANDARD_NAME .EQ. ' ' ) THEN
            STATUS = SAI__ERROR
            CALL MSG_SETC( 'GROUP', GROUP )
            CALL ERR_REP( ' ', 'RED4_FLUX_CALIBRATE: '/
     :         /'^GROUP has not been divided by a standard!', STATUS )
         END IF
...
*     Open the STANDARD file
         ...
         IF ( DSA_STATUS .NE. DSA__OK ) THEN
             ...
             CALL ERR_REP( ' ', 'RED4_FLUX_CALIBRATE: '/
     :         /'Error opening ^STANDARD', STATUS )
         END IF

*     Get some FITS items from both group and standard
         CALL DSA_GET_FITS_F( 'GRPFILE', 'DEXPTIME', 0, 
     :      GRP_EXPOSED, COMMENT, DSA_STATUS )
         CALL DSA_GET_FITS_F( 'STDFILE', 'DEXPTIME', 0, 
     :      STD_EXPOSED, COMMENT, DSA_STATUS )
         CALL DSA_GET_FITS_F( 'STDFILE', 'RFLAMBDA', 0, 
     :      RFLAMBDA, COMMENT, DSA_STATUS )
         CALL DSA_GET_FITS_F( 'STDFILE', 'TEFF', 0, 
     :      TEFF, COMMENT, DSA_STATUS )
...
*     Tell the user what we've found
         IF ( VERBOSE ) THEN
            ...
            CALL MSG_OUT( ' ', 
     :         'The group exposure time is ^GRP_EXPOSED', STATUS )
            CALL MSG_SETR( 'STD_EXPOSED', STD_EXPOSED )
            CALL MSG_OUT( ' ', 
     :         'The standard exposure time is ^STD_EXPOSED', STATUS )
            CALL MSG_SETR( 'RFLAMBDA', RFLAMBDA )
            CALL MSG_OUT( ' ',
     :         'Reference wavelength is ^RFLAMBDA microns', STATUS )
            CALL MSG_SETR( 'TEFF', TEFF )
            CALL MSG_OUT( ' ',
     :         'Effective temperature is ^TEFF Kelvin', STATUS )
         END IF

*     Convert flux or magnitude into selected units
         CALL RED4_FLUX_CALIBRATE_2( FLUX, BAND, UNIN, UNOUT, 
     :      RFLAMBDA, FLAM, STATUS )
...
*     Take exposure times into account
         ...
         IF ( VERBOSE ) THEN
            ...
            CALL MSG_OUT( ' ',
     :         'The image will be multiplied by ^FLAM', STATUS )
         END IF

*     Has all this worked?
         IF ( ( STATUS .EQ. SAI__OK ) .AND. 
     :        ( DSA_STATUS .EQ. DSA__OK ) ) THEN
