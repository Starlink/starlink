*+  RED4_EORMASK - Combine two mask arrays with exclusive OR operation.
      SUBROUTINE RED4_EORMASK( DIM1, DIM2, MASK1, MASK2, OUTPUT,
     :  STATUS )
*    Description :
*     This routine combines two bad pixel mask arrays to produce a third
*     using the logical EOR operation. The following truth table is used:-
*
*                             MASK1
*                    +----------+----------+
*                    |   GOOD   |   BAD    |
...
*
*    Invocation :
*      CALL RED4_EORMASK( DIM1, DIM2, MASK1, MASK2, OUTPUT, STATUS )
*    Parameters
*     DIM1                  = INTEGER( READ )
...
*           The output mask. OUTPUT = MASK1 .EOR. MASK2
...
*         (using a logical EOR operation). (Note that Fortran does
*         not have an .EOR. operator, so this must be simulated).
            MASK1_BAD  = ( MASK1(I,J) .NE. GOOD )
            MASK2_BAD  = ( MASK2(I,J) .NE. GOOD )
            OUTPUT_BAD = (      ( MASK1_BAD .OR. MASK2_BAD ) .AND.
     :                     .NOT.( MASK1_BAD .AND. MASK2_BAD ) )
