...
C  use CHECK_INPUT                          PND / JAC 18th Feb  1993
C+
      IMPLICIT NONE
      INCLUDE 'ADAMDEFNS'
      INCLUDE 'ADAMERRS'
...
C
      ...
      CALL RED4_CHECK_INPUT( SPECT, STATUS )
...
C
      ...
      CALL RED4_CHECK_INPUT( SPECT1, STATUS )
...
C
     ...
     :   A_SLOT, STATUS)
      A_PTR = ADDRESS
      IF (QUAL) THEN
         CALL DSA_MAP_QUALITY ('SPECT', 'READ', 'INTEGER', ADDRESS,
     :      Q_SLOT, STATUS)
         Q_PTR = ADDRESS
      ENDIF
      IF (ERRORS) THEN
     ...
     :   A1_SLOT, STATUS)
      A1_PTR = ADDRESS
      IF (QUAL) THEN
         CALL DSA_MAP_QUALITY ('SPECT1', 'READ', 'INTEGER', ADDRESS,
     :      Q1_SLOT, STATUS)
         Q1_PTR = ADDRESS
      ENDIF
      IF (ERRORS) THEN
...
C
     ...
     :   STATUS)
      DO_PTR = ADDRESS
      IF (QUAL) THEN
         CALL DSA_MAP_QUALITY ('OUTPUT', 'WRITE', 'INTEGER', ADDRESS,
     :      QO_SLOT, STATUS)
         QO_PTR = ADDRESS
      ENDIF
      IF (ERRORS) THEN
...
C
      IF (REVERSE) THEN
         IF (STATUS .EQ. ADAM__OK) THEN
            CALL GEN_MOVE(NX1*INTSIZE,%VAL(A1_PTR),%VAL(AO_PTR))
            CALL GEN_MOVE(NX*INTSIZE,%VAL(A_PTR),%VAL(AO_PTR+NX1*INTSIZE))
         ENDIF
      ELSE
         IF (STATUS .EQ. ADAM__OK) THEN
            CALL GEN_MOVE(NX*INTSIZE,%VAL(A_PTR),%VAL(AO_PTR))
            CALL GEN_MOVE(NX1*INTSIZE,%VAL(A1_PTR),%VAL(AO_PTR+NX*INTSIZE))
         ENDIF
      END IF
C
...
C
     ...
     :           'The data arrays will be sorted into increasing ')
            ...
            ENDIF
            SORT=.TRUE.
            BYTES=(NX+NX1)*INTSIZE*2
            CALL DSA_GET_WORKSPACE (BYTES, ADDRESS, W_SLOT, STATUS)
            VW_PTR = ADDRESS
            WK_PTR=VW_PTR+(NX+NX1)*INTSIZE
            IF (STATUS .EQ. ADAM__OK) THEN
...
C
      IF (STATUS .EQ. ADAM__OK) THEN
         IF (REVERSE) THEN
            CALL GEN_MOVE(NX1*INTSIZE,%VAL(D1_PTR),%VAL(DO_PTR))
            CALL GEN_MOVE(NX*INTSIZE,%VAL(D_PTR),%VAL(DO_PTR+NX1*INTSIZE))
            IF (QUAL) THEN
               CALL GEN_MOVE(NX1*INTSIZE,%VAL(Q1_PTR),%VAL(QO_PTR))
               CALL GEN_MOVE(NX*INTSIZE,%VAL(Q_PTR),%VAL(QO_PTR+NX1*INTSIZE))
            ENDIF
            IF (ERRORS) THEN
               CALL GEN_MOVE(NX1*INTSIZE,%VAL(V1_PTR),%VAL(VO_PTR))
               CALL GEN_MOVE(NX*INTSIZE,%VAL(V_PTR),%VAL(VO_PTR+NX1*INTSIZE))
            ENDIF
         ELSE
            CALL GEN_MOVE(NX*INTSIZE,%VAL(D_PTR),%VAL(DO_PTR))
            CALL GEN_MOVE(NX1*INTSIZE,%VAL(D1_PTR),%VAL(DO_PTR+NX*INTSIZE))
            IF (QUAL) THEN
               CALL GEN_MOVE(NX*INTSIZE,%VAL(Q_PTR),%VAL(QO_PTR))
               CALL GEN_MOVE(NX1*INTSIZE,%VAL(Q1_PTR),%VAL(QO_PTR+NX*INTSIZE))
            ENDIF
            IF (ERRORS) THEN
               CALL GEN_MOVE(NX*INTSIZE,%VAL(V_PTR),%VAL(VO_PTR))
               CALL GEN_MOVE(NX1*INTSIZE,%VAL(V1_PTR),%VAL(VO_PTR+NX*INTSIZE))
            ENDIF
         ENDIF
