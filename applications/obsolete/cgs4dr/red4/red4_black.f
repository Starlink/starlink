C+
      SUBROUTINE RED4_BLACK (STATUS)
C
C     R E D 4 _ B L A C K
C
...
C     7-Dec-1990: Bug fix. The Y axis label was being set to 'Flux',
C                 but this is meaningless in a 1-D spectrum. Data label
C                 set instead. (REVAD::SMB)
C    18-Feb-1993: Conform to error strategy (JACH::PND)
C-
C
      IMPLICIT  NONE
C
...
C
      ...
      CALL RED4_CHECK_INPUT( TEMPLATE, STATUS )
...
C
      CALL DSA_SET_OBJECT ('OUTPUT', 'Model Black-Body', STATUS)
      CALL DSA_PUT_FITS_F ('OUTPUT', 'BBTEMP', TEMPERATURE,
     :   'Temperature of black-body (K)', STATUS)
