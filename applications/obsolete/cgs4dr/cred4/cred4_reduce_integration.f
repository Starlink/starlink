*+  CRED4_REDUCE_INTEGRATION - Reduce an integration
      SUBROUTINE CRED4_REDUCE_INTEGRATION( INTEGRATION, ASK, WAIT, STATUS )
*    Description :
...
*    Deficiencies :
*     The INTEGRATION_AVAILABLE flag is only set if this routine
*     waits for the data reduction to complete. Otherwise it
*     does not know if the reduction has been REDUCTION_OK.
*    Authors :
...
*     11-Jan-1990: History added. ERR_REP used instead of
...
*     29-Mar-1990: INTEGRATION_AVAILABLE flag added.          (SMB)
...
*     28-Jun-1993: Pass full observation string etc           (PND)
...
*    Import :
      ...
      CHARACTER*(*) INTEGRATION     ! The name of the integration to be reduced
*    Status :
      INTEGER STATUS
*    External references :
...
*    enough to imply no other sequences will succeed
      ...
      CALL MSG_OUT (' ', 'Reducing integration ^INTEGRATION',
     :  STATUS)
     ...
     :           OUTVAL, STATUS)
               ...
               IF (STATUS .NE. DTASK__ACTCOMPLETE) THEN
                  IF (STATUS .EQ. DTASK__ACTINFORM) THEN
                     ...
                     CALL ERR_REP( ' ', 'CRED4_REDUCE_INTEGRATION: '/
...
*          Tell the RED4 task to reduce this integration.
*          Make up a command line containing the integration name
*          and other parameters:
            CPOS = 0
            CALL CHR_PUTC( 'INT_NAME="', INVAL, CPOS )
            CLEN = LEN( INTEGRATION )
            CALL CHR_PUTC( INTEGRATION(1:CLEN), INVAL, CPOS )

*          The name of the bad pixel mask (# for none).
            IF ( MASK .EQ. '#' ) THEN
...
*          The name of the linearisation coefficients file (# for none).
            IF ( LINCOEFFS .EQ. '#' ) THEN
...
*          proceeding; in which case it can't be important that it has failed)
            IF (STATUS .NE. DTASK__ACTSTART) THEN
               ...
               CALL MSG_SETI( 'ES', ERR_STATUS )
               CALL ERR_REP( ' ', 'CRED4_REDUCE_INTEGRATION: '/
     :            /'Failed to start integration reduction '/
     :            /'(Status = ^ES)', STATUS )
...
*                      explicit check for DSA error, report but otherwise ignore
     ...
     :                     /'Second DSA error has occured', STATUS )
                     ELSE
                        ...
                        CALL MSG_SETI( 'ES', ERR_STATUS )
                        CALL ERR_REP( ' ', 'CRED4_REDUCE_INTEGRATION: '/
     :                     /'Failed to reduce integration '/
     :                     /'(Status = ^ES)', STATUS )
                     ENDIF
                  ELSE

*                  The integration has been reduced REDUCTION_OKly,
*                  and the action has completed. Set a good status
*                  and the INTEGRATION_AVAILABLE flag.
                     CALL ERR_ANNUL( STATUS )
