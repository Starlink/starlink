...
*-
      IF (STATUS.NE.SAI__OK.OR.G_INITIALISED) RETURN

C      CALL PSX_GETENV('AST_GCB_DSCF',FILE,STATUS)
      CALL AST_PATH('AST_ETC','AST_GCB_DSCF','AST_GCB_DSCF',
     :                                        FILE,L,STATUS)
...
*-
      IF (STATUS.EQ.SAI__OK) THEN
        ...
        DO NSCAL=1,G_NSCAL
          ...
          DISP=G_SCDISP(NSCAL)
          L=G_SCSIZ(NSCAL)
          CALL GCB_GETSTR(%val(G_MEMPTR),DISP,L,BUFFER,STATUS)
