...
*    Version :
      ...
      PARAMETER (VERSION = 'ISTATS Version 2.1-0')
*-
      CALL USI_INIT()
...
*  is screen output required?
      ...
      ELSE
        ...
        CIRC=.FALSE.
        BOX=.FALSE.
        CALL USI_GET0L('CIRC',CIRC,STATUS)
        IF (.NOT.CIRC) THEN
...
*  if running from GUI - save to noticeboard
        IF (I_GUI) THEN
          ...
          CALL NBS_FIND_ITEM(I_NBID,'SUM',ID,STATUS)
          CALL NBS_PUT_VALUE(ID,0,VAL__NBR,TOT,STATUS)
