*+ SPEC_AB - Instrinsic ISM absorption cutoff
      SUBROUTINE SPEC_AB(NEN,ELBOUND,EUBOUND,PARAM,FAC,STATUS)
*    Description :
...
*     14 Nov 89 : Compton scattering removed -> instrinsic absorption model (TJP)
...
* Exit
 9000	IF(STATUS.NE.SAI__OK) CALL ERR_REP('EXERR','from SPEC_AB',STATUS)
	END


*+  SPEC_AB_SIGMA - Interstellar absorption cross-section
	REAL FUNCTION SPEC_AB_SIGMA(C0,C1,C2,E)
*    Description :
...
*-----------------------------------------------------------------------
	...
	IF(E.LT.1.0E4)THEN
	  SPEC_AB_SIGMA=(C0/E+C1+C2*E)/E**2
	ELSE
	  SPEC_AB_SIGMA=0.0
	ENDIF
	END
