*+ FIT_USER1 - Dummy user model for fitting software
      SUBROUTINE FIT_USER1(NMDAT,LBOUND,UBOUND,PARAM,PRED,STATUS)
*    Description :
...
* Output message and exit with bad status
	CALL ERR_REP('NO_MOD','User model 1 not linked',STATUS)
	STATUS=SAI__ERROR

* Exit
 9000	IF(STATUS.NE.SAI__OK) CALL ERR_REP('EXERR','from FIT_USER1',
     :  STATUS)
	END
