...
*    Version :
	...
	PARAMETER		(VERSION='SSIM VERSION 1.7-0')
*-
...
* Get model specification
	CALL DAT_ASSOC('FIT_MOD','READ',MLOC,STATUS)
	CALL FIT_MODGET(MLOC,MODEL,NPAR,PARAM,LB,UB,LE,UE,FROZEN,STATUS)
	IF(STATUS.NE.SAI__OK) GO TO 9000

* User input
	CALL DAT_ASSOC('BG_DATA','READ',BGLOC,STATUS)
	...
	ELSE
	  GO TO 9000
	ENDIF
	CALL PAR_GET0L('POISS_ERRS',POISS,STATUS)
	IF(POISS)THEN
	  CALL PAR_GET0R('OBS_LENGTH',TOBS,STATUS)
	ENDIF
        CALL SFIT_GETZ( Z, STATUS )

* Output dataset
	CALL DAT_CREAT('OUT','SPECTRUM',0,0,STATUS)
