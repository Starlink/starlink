*+ SPEC_AP - Partial covering absorption cutoff
      SUBROUTINE SPEC_AP(NEN,ELBOUND,EUBOUND,PARAM,FAC,STATUS)
*    Description :
...
*     where it is assumed that only a fraction F of the source is covered.
*     Contribution from Compton scattering is not included, since this is
*     not a loss process for intrinsic absorption. Note that for a redshifted
*     source the absorption model (e.g. edges) is set up in the source frame.
*     Attenuation factor is
*			FAC=F*exp(-SIG*NH) + (1-F)
*     where
*		NH = PARAM(1) is H column density in units of 1.0E21 /cm**2
*		F  = PARAM(2) is the covering fraction (in range 0-1)
...
*     either side of each absorption edge.
...
*    History :
*      6 Dec 90 : Original (adapted from SPEC_AB)
...
* Find first edge above ELBOUND(1) - edge no. N
	N=1
	DO I=1,15
	...
	C2=C2V(N)			!
	SIG=SPEC_AP_SIGMA(C0,C1,C2,E)
	OLDAB=F*EXP(-SIG*NH)+(1-F)
	ELAST=E
	DO I=1,NEN
	  DE=EUBOUND(I)-ELAST

* Step through any edges within bin I
	  IF(ECUT(N).GE.EUBOUND(I))THEN		! Next edge >= upper bin bound

* No edges within energy bin - perform quadratic integral over absorption
	    E=(ELAST+EUBOUND(I))/2		! Mean bin energy
	    SIG=SPEC_AP_SIGMA(C0,C1,C2,E)
	    ABMID=F*EXP(-SIG*NH)+(1-F)
	    E=EUBOUND(I)
	    SIG=SPEC_AP_SIGMA(C0,C1,C2,E)
	    ABS=F*EXP(-SIG*NH)+(1-F)
	    IF(E.LT.20.0)THEN
...
* Make linear superposition of absorption factors at edges and bin bounds
 ...
 50	    N=N+1				! Next ECUT segment
	    ...
	    SIG=SPEC_AP_SIGMA(C0,C1,C2,E)
	    OLDAB=F*EXP(-SIG*NH)+(1-F)		! Absorption at TOP of last
	    ELAST=E				! edge

	    IF(ECUT(N).LT.EUBOUND(I))THEN
...
* No further edges in this bin
	      ...
	      AFAC=AFAC+(OLDAB+ABS)*(E-ELAST)
	      FAC(I)=AFAC/(2*DE)
	      ELAST=E				! Assumes contiguous bins
	      OLDAB=ABS				! Ditto
	    ENDIF
	  ENDIF
...
* Exit
 9000	IF(STATUS.NE.SAI__OK) CALL ERR_REP('EXERR','from SPEC_AP',STATUS)
	END


*+  SPEC_AP_SIGMA - Interstellar absorption cross-section
	REAL FUNCTION SPEC_AP_SIGMA(C0,C1,C2,E)
*    Description :
*	Calculates absorption cross section per H atom in
*	units of 1.0E-21 cm**2 from the coefficients C0,C1 & C2 of
*	Morrison & McCammon.
*       NOTE: No contribution from scattering in this model.
*    Method :
*    Deficiencies :
*    Bugs :
*    Authors :
...
*    Type definitions :
      IMPLICIT NONE
*    Import :
	REAL C0,C1,C2		! Morrison and McCammon coefficients at E
...
*-
	...
	IF(E.LT.1.0E4)THEN
	  SPEC_AP_SIGMA=(C0/E+C1+C2*E)/E**2
	ELSE
	  SPEC_AP_SIGMA=0.0
	ENDIF
	END
