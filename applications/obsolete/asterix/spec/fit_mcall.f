...
*			[      flux in  photons/(cm**2 sec) in each bin    ]
	ELSE IF(GENUS.EQ.'SPEC')THEN
	  ...
	  ELSE IF(KEY.EQ.'MK')THEN
	    CALL SPEC_MK(GENUS,ND,NMDAT,MLBOUND,MUBOUND,PARAM,PRED,
     :                   STATUS)
	  ELSE IF(KEY.EQ.'MU')THEN
	    CALL SPEC_MU(NMDAT,MLBOUND,MUBOUND,PARAM,PRED,STATUS)
