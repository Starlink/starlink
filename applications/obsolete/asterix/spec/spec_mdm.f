*+  SPEC_MDM - Computes spectra from a differential emission measure model
      SUBROUTINE SPEC_MDM(GENUS,NDS,NEN,ELBOUND,EUBOUND,PARAM,FLUX,
     :            STATUS)
...
*  where DEM=d(EM)/dT rather than DEM=T.d(EM)/dT. Uses SPEC_MK model
*  for low level spectra
*
...
*
* Note also: A MKL grid matching the instrument response MUST be used,
...
*  sections and the MEKAL models at each temperature summed in a weighted
...
*
*     23 Apr 1996 (DJA):
*        Original version. Adapted from SPEC_DM
*
*    Type Definitions :
*
      IMPLICIT NONE
...
*      reset anyway by SPEC_MK_EMISS, and the normalisation will then
...
* Exit
9000	IF(STATUS.NE.SAI__OK) CALL ERR_REP('E','from SPEC_MDM',STATUS)
	END
