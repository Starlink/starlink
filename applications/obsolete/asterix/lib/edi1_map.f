      SUBROUTINE EDI1_MAP( NARG, ARGS, OARG, STATUS )
*+
*  Name:
*     EDI1_MAP

*  Purpose:
*     Service ListMap requests from the EDI system for HDS files

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL EDI1_MAP( NARG, ARGS, OARG, STATUS )

*  Description:
*     Services EDI map requests for HDS files.
...
*  Local Variables:
      CHARACTER*(DAT__SZLOC)	DLOC			! List DATA_ARRAY
...
*  First bound of zero means map the whole list
      IF ( LBND .EQ. 0 ) THEN
        CALL DAT_CLONE( DLOC, SLOC, STATUS )
      ELSE
        CALL DAT_SLICE( DLOC, 1, LBND, UBND, SLOC, STATUS )
      END IF
...
*  unmap cleanly
      CALL ADI_CNEWV0C( LID, '.MappedComponent', SLOC, STATUS )
...
*  Report any errors
      IF ( STATUS .NE. SAI__OK ) CALL AST_REXIT( 'EDI1_MAP', STATUS )
