      SUBROUTINE EDI_UNMAP( ID, LISTS, STATUS )
*+
*  Name:
*     EDI_UNMAP

*  Purpose:
*     Unmap the named lists

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL EDI_UNMAP( ID, LISTS, STATUS )
...
*    Invoke the function
        CALL ADI_FEXEC( 'ListUnmap', 3, ARGS, OARG, STATUS )
...
*  Report any errors
      IF ( STATUS .NE. SAI__OK ) CALL AST_REXIT( 'EDI_UNMAP', STATUS )
