      SUBROUTINE ARR_SELEM1<T>( PTR, DIM, INDEX, VAL, STATUS )
*+
*  Name:
*     ARR_SELEM1<T>

*  Purpose:
*     Sets element of <TYPE> array given pointer, index and value

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL ARR_SELEM1<T>( PTR, DIM, INDEX, VAL, STATUS )

*  Description:
*     Sets element of <TYPE> array given pointer, index and value. The index
...
*        Size of the array
*     INDEX = INTEGER (given)
*        Index of the element whose value is to be set
*     VAL = <TYPE> (given)
...
*    Index is legal?
        IF ( (INDEX.GT.0) .AND. (INDEX.LE.DIM) ) THEN
          CALL ARR_SELEM1<T>_INT( %VAL(PTR), INDEX, VAL )
...
*    Negative index?
     ...
     :                 STATUS)
        ...
        IF (STATUS.NE.SAI__OK) THEN
          CALL AST_REXIT( 'ARR_SELEM1<T>', STATUS )
        END IF
      ...
      SUBROUTINE ARR_SELEM1<T>_INT( ARRAY, INDEX, VAL )
*+
*  Name:
*     ARR_SELEM1<T>
...
*  Invocation:
*     CALL ARR_SELEM1<T>_INT( ARRAY, INDEX, VAL )
