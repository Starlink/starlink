*+ PSF_AXAF_S_INIT.F
      SUBROUTINE PSF_AXAF_S_INIT(PSID,STATUS)
*
*    Description :
*     PSF for AXAF S
*     Opens PSF cube
...
*
      EXTERNAL  PSF_AXAF_MARX_S      ! Name of PSF routine
      EXTERNAL  PSF_AXAF_MARX_HINT_S
*-
*    Locate cube
      CALL AST_PATH('AST_ETC','PSF_HYPER_CUBE','psf_hyper_cube_s',FNAME,
     :                                                         L,STATUS)
...
*    For 1.5 5.5 10.0 Bins[1] = 0.0~1.5 Bin[2]=1.51~5.5 Bin[3]=5.51~10.0
      ETEMP=0.0
      HX_LOCATOR_POSE = 0
      DO LP = 1,HX_CB_NEBIN
...
*    Pointer to PSF routine
      CALL PSF0_SETRTN( PSID, 'Data', PSF_AXAF_MARX_S, STATUS )
CC      CALL PSF0_SETRTN( PSID, 'Hint', PSF_AXAF_MARX_HINT, STATUS )


*    Would prefer name from the parameter system
      HX_PSF_NAME = 'AXAF_MARX_S'

 99   IF (STATUS .NE. SAI__OK) THEN
         CALL AST_REXIT('PSF_AXAF_S_INIT',STATUS)
      ENDIF

      END

*+  PSF_AXAF_MARX_HINT_S - XRT PSPC psf hint handler
      SUBROUTINE PSF_AXAF_MARX_HINT_S( PSID, HINT, DATA, STATUS )
*
*    Description :
*
