      SUBROUTINE EDI_QMAP( ID, LISTS, TYPE, MODE, LINDEX, HINDEX,
     :                    PTRS, STATUS )
*+
*  Name:
*     EDI_QMAP
...
*  Purpose:
*     Map the named lists' vector quanta with the specified type and mode

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL EDI_QMAP( ID, LISTS, TYPE, MODE, LINDEX, HINDEX, PTRS, STATUS )

*  Description:
*     Maps the lists vector quanta specified by the LISTS string with
*     a type and mode specified by TYPE and MODE. The pointers to the
*     resulting areas of memory are returned in PTRS.
...
*        List of lists whose vector quanta are to be mapped
...
*    Invoke the function
        CALL ADI_FEXEC( 'ListMapQuantum', 7, ARGS, OARG, STATUS )
...
*  Report any errors
      IF ( STATUS .NE. SAI__OK ) CALL AST_REXIT( 'EDI_QMAP', STATUS )
