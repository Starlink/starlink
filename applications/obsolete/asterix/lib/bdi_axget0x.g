      SUBROUTINE BDI_AXGET0<T>( ID, IAX, ITEMS, DATA, STATUS )
*+
*  Name:
*     BDI_AXGET0<T>

*  Purpose:
*     Get the named axis items as <COMM> scalars

*  Language:
*     Starlink Fortran

*  Invocation:
*     CALL BDI_AXGET0<T>( ID, IAX, ITEMS, DATA, STATUS )
...
*    Check the axis item
        CALL BDI_GET0<T>( ID, 'Axis_'//ASTR(:NDIG)//'_'//ITEMS(C1:C2),
     :                    DATA(IITEM), STATUS )
...
*  Report any errors
      IF ( STATUS .NE. SAI__OK ) CALL AST_REXIT( 'BDI_AXGET0<T>', STATUS )
