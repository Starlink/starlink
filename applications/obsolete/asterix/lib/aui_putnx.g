      SUBROUTINE AUI_PUTN<T>( ID, NAME, NDIM, DIMS, VALUES, STATUS )
*+
*  Name:
*     AUI_PUTN<T>
...
*  Invocation:
*     CALL AUI_PUTN<T>( ID, NAME, NDIM, DIMS, VALUES, STATUS )

*  Description:
*     Writes an array <COMM> auxilliary value to the specified dataset. Auxilliary
...
*  Create ADI object describing value
      CALL ADI_NEWV<T>( NDIM, DIMS, VALUES, ARGS(3), STATUS )
...
*  Report any errors
      IF ( STATUS .NE. SAI__OK ) CALL AST_REXIT( 'AUI_PUTN<T>', STATUS )
