      SUBROUTINE UDI0_CHKNTY( TYPE, LTYPE, STATUS )
*+
*  Name:
*     UDI0_CHKNTY

*  Purpose:
*     Check type name supplied is a valid numeric type
...
*  Local Constants:
      CHARACTER*41		NTYPES
        PARAMETER		( NTYPES =
     :        'UBYTE,BYTE,UWORD,WORD,INTEGER,REAL,DOUBLE' )
...
*  Check its allowed
      IF ( .NOT. CHR_INSET( NTYPES, LTYPE ) ) THEN
        STATUS = SAI__ERROR
        CALL ERR_REP( 'UDI0_CHKNTY', 'Object type supplied is not '/
     :     /' valid numeric type, must be one of '//NTYPES, STATUS )
      END IF
