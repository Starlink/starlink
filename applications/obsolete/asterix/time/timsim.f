...
*    Local variables :
*
      CHARACTER*(DAT__SZLOC) OLOC,ILOC          ! Object locators
      CHARACTER*20 DLAB,DUNI,AXLAB,AXUNI
      INTEGER NBACK,AXPTR,WIPTR,OPTR,QPTR,DIM,I,VPTR
      INTEGER CHOICE(NMAX)
      LOGICAL LOGCHO(NMAX),AXIS,INPRIM,OK,REG
      REAL DUR,WID,INIT
*    Local data :
*
*    Version :
      CHARACTER*30 VERSION
      PARAMETER (VERSION = 'TIMSIM Version 1.7-0')
*-
...
*	Get name of output file
	...
	CALL USI_ASSOCO('OUT','TIME',OLOC,STATUS)

*       Create axis structure and write labels and units

	CALL BDA_CREAXES(OLOC,1,STATUS)
	CALL PAR_GET0C('AXLAB',AXLAB,STATUS)
	CALL BDA_PUTAXLABEL(OLOC,1,AXLAB,STATUS)
	CALL PAR_GET0C('AXUNI',AXUNI,STATUS)
	CALL BDA_PUTAXUNITS(OLOC,1,AXUNI,STATUS)
	CALL PAR_GET0C('DLAB',DLAB,STATUS)
	CALL BDA_PUTLABEL(OLOC,DLAB,STATUS)
	CALL PAR_GET0C('DUNI',DUNI,STATUS)
	CALL BDA_PUTUNITS(OLOC,DUNI,STATUS)
...
*          Axis read in
	...
	IF(AXIS)THEN
	   ...
	   IF (STATUS.NE.SAI__OK) GOTO 9000
	   CALL BDA_COPAXIS(ILOC,OLOC,1,1,STATUS)
	   CALL BDA_MAPAXVAL(ILOC,'READ',1,AXPTR,STATUS)
	   CALL BDA_CHKAXVAL(ILOC,1,OK,REG,DIM,STATUS)
	   CALL BDA_MAPAXWID(ILOC,'READ',1,WIPTR,STATUS)
	   IF (STATUS.NE.SAI__OK) GOTO 9000

*          Axis created

	ELSE
	   CALL PAR_GET0R('DUR',DUR,STATUS)
	   CALL PAR_GET0R('WID',WID,STATUS)
	   CALL PAR_GET0R('INIT',INIT,STATUS)
	   IF (STATUS.NE.SAI__OK) GOTO 9000
	   DIM = NINT(DUR/WID)
	   CALL BDA_PUTAXVAL(OLOC,1,INIT,WID,DIM,STATUS)
           CALL DYN_MAPR(1,DIM,AXPTR,STATUS)
           CALL ARR_REG1R( INIT, WID, DIM, %VAL(AXPTR),STATUS )
	   CALL BDA_PUTAXWID(OLOC,1,WID,STATUS)
...
*	Data Simulation
	...
	CALL BDA_CREDATA(OLOC,1,DIM,STATUS)
	CALL BDA_MAPDATA(OLOC,'WRITE',OPTR,STATUS)
	CALL ARR_INIT1R(0.0,DIM,%VAL(OPTR),STATUS)
	CALL BDA_CREVAR(OLOC,1,DIM,STATUS)
...
*    Set output quality to good
      CALL BDA_CREQUAL(OLOC,1,DIM,STATUS)
      CALL BDA_MAPQUAL(OLOC,'WRITE',QPTR,STATUS)
      CALL ARR_INIT1B(QUAL_GOOD,DIM,%VAL(QPTR),STATUS)
...
*
*-
	CALL PAR_GET0R('LEV1',LEV,STATUS)
	DO I=1,DIM
	   DATA(I)=DATA(I)+LEV
	ENDDO
	END
...
*    Local variables :
      ...
      REAL LEV(2)
*-
	CALL PAR_GET0R('RAM1',LEV(1),STATUS)
	CALL PAR_GET0R('RAM2',LEV(2),STATUS)
	DO I=1,DIM
...
*-
	CALL PAR_GET0I('BSF',BSF,STATUS)
	CALL PAR_GET0I('NUMSIN',NUMSIN,STATUS)
	DO I=1,NUMSIN
	   IF(NUMSIN.GT.1)THEN
	      CALL MSG_SETI('NUMSIN',I)
	      CALL MSG_OUT('E','Sine wave number ^numsin',STATUS)
	   ENDIF
	   CALL PAR_GET0R('PER1',PER,STATUS)
	   CALL PAR_CANCL('PER1',STATUS)
	   CALL PAR_GET0R('PHA1',PHA,STATUS)
	   CALL PAR_CANCL('PHA1',STATUS)
	   CALL PAR_GET0R('AMP1',AMP,STATUS)
	   CALL PAR_CANCL('AMP1',STATUS)
	   DO J=1,DIM
	      DO K=0,BSF
...
*-
	CALL PAR_GET0I('BSF',BSF,STATUS)
	CALL PAR_GET0I('NUMSQU',NUMSQU,STATUS)
	DO I=1,NUMSQU
	   IF(NUMSQU.GT.1)THEN
	      CALL MSG_SETI('NUMSQU',I)
	      CALL MSG_OUT('E','Square wave number ^numsqu',STATUS)
	   ENDIF
	   CALL PAR_GET0R('PER2',PER,STATUS)
	   CALL PAR_CANCL('PER2',STATUS)
	   CALL PAR_GET0R('PHA2',PHA,STATUS)
	   CALL PAR_CANCL('PHA2',STATUS)
	   CALL PAR_GET0R('AMP2',AMP,STATUS)
	   CALL PAR_CANCL('AMP2',STATUS)
	   DO J=1,DIM
	      DO K=0,BSF
...
*-
	CALL PAR_GET0I('BSF',BSF,STATUS)
	CALL PAR_GET0I('NUMSAW',NUMSAW,STATUS)
	DO I=1,NUMSAW
	   IF(NUMSAW.GT.1)THEN
	      CALL MSG_SETI('NUMSAW',I)
	      CALL MSG_OUT('E','Saw tooth wave number ^numsaw',STATUS)
	   ENDIF
	   CALL PAR_GET0R('PER3',PER,STATUS)
	   CALL PAR_CANCL('PER3',STATUS)
	   CALL PAR_GET0R('PHA3',PHA,STATUS)
	   CALL PAR_CANCL('PHA3',STATUS)
	   CALL PAR_GET0R('AMP3',AMP,STATUS)
	   CALL PAR_CANCL('AMP3',STATUS)
	   DO J=1,DIM
	      DO K=0,BSF
	         Z=((AXIS(J)-AXWI(J)/2+(AXWI(J)*K/BSF))/PER)
	         Z=ABS(Z-INT(Z))
...
*-
	CALL PAR_GET0I('BSF',BSF,STATUS)
	CALL PAR_GET0I('NUMFLA',NUMFLA,STATUS)
	DO I=1,NUMFLA
	   IF(NUMFLA.GT.1)THEN
	      CALL MSG_SETI('NUMFLA',I)
	      CALL MSG_OUT('E','Flare number ^numfla',STATUS)
	   ENDIF
	   CALL PAR_GET0R('RIS',RIS,STATUS)
	   CALL PAR_CANCL('RIS',STATUS)
	   CALL PAR_GET0R('DEC',DEC,STATUS)
	   CALL PAR_CANCL('DEC',STATUS)
	   CALL PAR_GET0R('AMP4',AMP,STATUS)
	   CALL PAR_CANCL('AMP4',STATUS)
	   CALL PAR_GET0R('TIM',TIM,STATUS)
	   CALL PAR_CANCL('TIM',STATUS)
	   DO J=1,DIM
	      DO K=0,BSF
