...
*
	...
	CHARACTER ILOC*(DAT__SZLOC)
	CHARACTER OLOC*(DAT__SZLOC)
	CHARACTER*20 TYPE
...
*    Version :
	...
	PARAMETER  (VERSION='DIFDAT Version 1.2-1')
*-
...
*	Check for 1D
	   IF(NDIM.NE.1)THEN
	   CALL ERR_REP('NOT_1D','Dataset is not 1-D',STATUS)
	   STATUS=SAI__ERROR
	   ENDIF
...
*    Set flags
	   ...
	   VFLAG=.FALSE.
	   QFLAG=.FALSE.
	   CALL BDA_CREDATA(OLOC,NDIM,ONDAT,STATUS)
	   CALL BDA_MAPDATA(OLOC,'WRITE',ODPTR,STATUS)
	   CALL BDA_CREAXES(OLOC,1,STATUS)
	   CALL BDA_PUTAXVAL(OLOC,1,0,1,NDAT,STATUS)
	   CALL BDA_CREVAR(OLOC,1,NDAT,STATUS)
...
*    If ok=false...
	   IF(.NOT.OK)THEN
	      ...
	      STATUS=SAI__ERROR
	   ENDIF
	   IF(.NOT.REG)PRINT*,'WARNING, IRREGULAR AXIS'
	   IF(STATUS.NE.SAI__OK)GOTO 9000
*	CHECK FOR NVAL=NDAT
	   NDAT=DIMS(1)
     ...
     :     STATUS)
	   ...
	   CALL BDA_CHKQUAL(ILOC,OK,QNDIM,QDIMS,STATUS)
*    If ok=true...
	   IF( OK )THEN
...
*    Set flags
	   ...
	   QFLAG=.FALSE.
	   DIMS(1)=NOP
	   CALL BDA_CREDATA(OLOC,1,DIMS(1),STATUS)
	   CALL BDA_MAPDATA(OLOC,'WRITE',ODPTR,STATUS)
	   CALL BDA_COPTEXT(ILOC,OLOC,STATUS)
           CALL BDA_COPMORE(ILOC,OLOC,STATUS)
	   CALL BDA_CREAXVAL(OLOC,1,REG,DIMS(1),STATUS)
	   CALL BDA_MAPAXVAL(OLOC,'WRITE',1,OAXPTR,STATUS)
	   CALL BDA_COPAXTEXT(ILOC,OLOC,1,1,STATUS)
	   CALL BDA_CREVAR(OLOC,1,DIMS(1),STATUS)
	   CALL BDA_MAPVAR(OLOC,'WRITE',OVPTR,STATUS)
	   IF(STATUS.NE.SAI__OK)GOTO 9000
           IF(OK)THEN
	...
	ENDIF
*	History :
9000	CALL HIST_OK(ILOC,OK,STATUS)
	IF(OK)THEN
	...
	ENDIF
	CALL AST_CLOSE
        IF(STATUS.NE.SAI__OK)CALL ERR_REP('EXERR','from DIFDAT',STATUS)
	END
