...
*  Purpose:
*     Clears a image display device and purges its database entries.
...
*  Description:
*     This application software resets a image display device. In effect
...
*     DEVICE = DEVICE (Read)
*        The device to be cleared. [Current image display device]

*  Examples:
*     idclear
*        Clears the current image display device and purges its graphics
*        database entries.
*     idclear current
*        Clears the current picture on the current image display device.
*     idclear xw
*        Clears the xw device and purges its graphics database entries.

*  Related Applications:
*     KAPPA: IDSET, GDSTATE, GDSET, GDCLEAR, OVSET, OVCLEAR.
...
*  If an error occurred, add a context message.
      IF( STATUS .NE. SAI__OK ) THEN
         CALL ERR_REP( 'IDCLEAR_ERR', 'IDCLEAR: Unable to clear '//
     :                 'current image display device.', STATUS )
      END IF
